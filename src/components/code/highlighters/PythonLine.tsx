...
const KW =
  /\b(def|class|import|from|return|if|elif|else|for|in|while|try|except|finally|with|as|yield|async|await|None|True|False|self|cls|lambda|raise|pass|and|or|not|is|break|continue|global|nonlocal|assert|del)\b/g;
const STR =
  /("""[\s\S]*?"""|'''[\s\S]*?'''|"(?:[^"\\]|\\.)*"|'(?:[^'\\]|\\.)*')/g;
const DEC = /^(\s*@\w[\w.]*)/g;
