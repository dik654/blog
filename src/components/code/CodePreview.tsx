...
export default function CodePreview({ codeRef }: { codeRef: CodeRef }) {
  ...
  return (
    <div className="mt-2 rounded border border-[#d0d7de] dark:border-[#30363d] overflow-hidden text-xs font-mono">
      <div className="px-2 py-1 bg-[#f6f8fa] dark:bg-[#161b22] text-[#57606a] dark:text-[#8b949e] text-[9px] truncate">
        {codeRef.path} L{codeRef.highlight[0]}–
        {Math.min(codeRef.highlight[1], codeRef.highlight[0] + MAX - 1)}
        {truncated && " …"}
      </div>
          ...
          <tbody>
            {lines.map((line, i) => {
              ...
              return (
                <tr
                  key={ln}
                  className="hover:bg-[#f6f8fa] dark:hover:bg-[#161b22]"
                >
                  <td className="select-none text-right pr-2 pl-3 py-0 text-[#57606a] dark:text-[#636e7b] w-10 shrink-0 border-r border-[#eaecef] dark:border-[#21262d]">
                    {ln}
                  </td>
