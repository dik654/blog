...
function getColor(color: FlowNode["color"]) {
  if (color === "slate")
    return { border: SLATE.border, bg: SLATE.bg, text: SLATE.badgeText };
...
export function Arrow() {
  return (
    <div className="flex flex-col items-center py-0.5">
      ...
      <span className="text-[9px] text-foreground/75 leading-none">
        &#8595;
      </span>
    </div>
  );
...
const btnBase =
  "text-[9px] px-1.5 py-0.5 rounded border cursor-pointer leading-none";
...
export default function FlowNodeCard({
  ...
  return (
    <div className="w-full">
      <div
        className="rounded-lg border px-3 py-2.5 select-none"
        style={{
          borderLeftWidth: 4,
      ...
      >
        <div className="flex items-start gap-2">
          <div className="flex-1 min-w-0">
            <code
              className="text-[11px] font-bold block font-mono leading-tight"
              style={{ color: c.text }}
            >
              {node.fn}
            </code>
            ...
            <AnimatePresence>
              {showDetail && node.detail && (
                <motion.p
                  {...anim}
                  className="text-xs text-foreground/75 mt-1.5 leading-relaxed whitespace-pre-line overflow-hidden"
                >
                  {node.detail}
                </motion.p>
              )}
