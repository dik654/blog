...
export default function CodeSidebarHeader({
  ...
  return (
    <div className="flex items-center gap-2 px-4 py-2 border-b border-[#d0d7de] dark:border-[#30363d] bg-[#f6f8fa] dark:bg-[#161b22] shrink-0">
      {projectMeta && (
        <span
          className={`text-[9px] font-bold px-1.5 py-0.5 rounded-full border ${projectMeta.badgeClass}`}
        >
          {projectMeta.label}
        </span>
      )}
      <code className="flex-1 text-[11px] text-[#24292f] dark:text-[#e6edf3] font-mono truncate min-w-0">
        {codeRef.path}
      </code>
      <span className="text-xs text-[#57606a] dark:text-[#8b949e] shrink-0">
...
export function CodeSidebarDesc({
  ...
  desc: string;
  descOpen: boolean;
  setDescOpen: (fn: (v: boolean) => boolean) => void;
}) {
  return (
    <div className="shrink-0 border-b border-[#d0d7de] dark:border-[#30363d]">
      ...
      >
        <span className="text-[11px] font-semibold text-[#7d4e00] dark:text-[#e3a000]">
          코드 해설
        </span>
        <span className="ml-auto text-xs text-[#92400e] dark:text-[#d97706]">
          {descOpen ? "▲ 접기" : "▼ 펼치기"}
        </span>
      </button>
      <AnimatePresence>
        {descOpen && (
          ...
          >
            <div className="px-4 pb-3 bg-[#fffbeb] dark:bg-[#2d2200]">
              <p className="text-[12px] text-[#3d2900] dark:text-[#ffd966] leading-relaxed whitespace-pre-line">
                {desc}
              </p>
            </div>
          </motion.div>
