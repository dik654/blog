...
function Node({
  ...
  if (node.type === "dir") {
    return (
      <div>
        ...
        >
          <span className="text-[#57606a] dark:text-[#8b949e] text-xs w-3 shrink-0">
            {open ? "▾" : "▸"}
          </span>
  ...
  return (
    ...
    >
      <span
        className={`text-xs w-3 shrink-0 ${isCurrent ? "text-[#0969da] dark:text-[#388bfd]" : "text-[#57606a] dark:text-[#8b949e]"}`}
      >
        {isCurrent ? "◆" : hasRef ? "◇" : "·"}
      </span>
      <span
        className={`text-xs truncate ${isCurrent ? "text-[#0969da] dark:text-[#388bfd]" : "text-[#24292f] dark:text-[#e6edf3]"}`}
      >
        {node.name}
      </span>
    </button>
