    ...
    <span className="relative inline-flex items-baseline gap-0.5">
      ...
      <AnimatePresence>
        {open && (
          ...
          >
            <span
              className={`block rounded-lg border-l-4 ${accent} p-3 shadow-lg bg-background`}
            >
              ...
              {href && (
                <a
                  ...
                  className="text-xs text-blue-400 hover:underline mb-1 block"
                >
                  {href}
                </a>
              )}
              <span
                className={`text-sm leading-relaxed block ${type === "code" ? "font-mono" : ""}`}
              >
                {children}
              </span>
            </span>
