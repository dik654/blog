...
import { CODE_PANEL_COLORS as COLORS } from "./code-panel-colors";
            ...
            {annotations && annotations.length > 0 && (
              <div className="flex flex-wrap gap-2 px-4 py-2 border-b border-border/40">
                {annotations.map((a, i) => (
                  <span
                    key={i}
                    className={`text-[10px] px-2 py-0.5 rounded-full ${COLORS[a.color].badge}`}
                  >
                ...
                <tbody>
                  {lines.map((line, i) => {
                    ...
                    return (
                      ...
                      >
                        <td className="text-right pr-3 pl-3 py-0 text-muted-foreground/40 select-none w-8">
                          {num}
                        </td>
