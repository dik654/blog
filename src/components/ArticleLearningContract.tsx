...
function getStageEdges(contract: ArticleLearningContract, stageIndex: number) {
  ...
  for (const conceptId of current) {
    if (selected.some((edge) => edge.from === conceptId || edge.to === conceptId)) continue;
...
function getFollowupEdges(contract: ArticleLearningContract) {
  const introduced = new Set(contract.introducedHere.map((concept) => concept.id));
  ...
  for (const edge of KNOWLEDGE_EDGES) {
    ...
    const targetRoute = target.canonicalHref.match(
      /^\/([^/#]+\/[^/#]+)/,
    )?.[1];
    if (!targetRoute || targetRoute === currentRoute || !ARTICLE_LEARNING[targetRoute]) {
      continue;
    }
    ...
    >
      <div className="border-b border-border/60 bg-muted/20 p-5 sm:p-6">
        ...
        <h2 className="mt-2 text-xl font-black leading-8 text-foreground">
          새 용어를 하나씩 이해한 뒤 본문에서 조합합니다
        </h2>
        <p className="mt-3 max-w-4xl text-sm leading-7 text-foreground/80">
          먼저 익숙한 장면과 질문만 봅니다. 장면을 선택하면 그제야 이름과 정의,
          필요한 이유, 가장 작은 형태와 실패 경계를 공개합니다. 한 용어를 자기
          말로 설명할 수 있게 된 뒤 다음 장면으로 넘어가고, 마지막에만 본문에서
          메커니즘으로 연결합니다.
        </p>
      </div>
  ...
  return (
    ...
    >
      ...
      <section className="border-t border-border/60 px-4 py-5 sm:px-6">
        <div className="flex min-w-0 flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
          ...
          <div className="flex shrink-0 items-center gap-2 text-[11px] font-semibold text-muted-foreground">
            <span className="rounded border border-border/70 px-2 py-1">단계</span>
            <span aria-hidden="true">→</span>
            <span className="rounded border border-border/70 px-2 py-1">관계</span>
            <span aria-hidden="true">→</span>
            <span className="rounded border border-border/70 px-2 py-1">정본 개념</span>
          </div>
        </div>
        <div className="mt-4 divide-y divide-border/70">
          {displayStages.map((stage, index) => (
            <div key={stage.label} className="min-w-0 py-5 first:pt-0 last:pb-0">
              ...
              <div className="mt-4 min-w-0 border-l border-border/80 pl-3 sm:ml-[12rem] sm:pl-4">
                <p className="text-[11px] font-bold text-foreground/70">관계선과 연결 이유</p>
                ...
                return (
                  ...
                  >
                    ...
                    <p className="mt-1 text-[11px] font-semibold text-primary">
                      출발 개념 · {from.label} / 관계 · {RELATION_LABEL[edge.relation]}
                    </p>
                    <p className="mt-1 text-xs leading-5 text-muted-foreground">
                      {edge.reason}
                    </p>
                ...
                <div className="mt-4 border-t border-border/60 pt-4 text-sm leading-7 text-muted-foreground">
                  <dl className="grid gap-4 lg:grid-cols-2">
                    ...
                    {explanation.proofIdea && (
                      <div>
                        <dt className="font-bold text-foreground">증명 아이디어</dt>
                        <dd className="mt-0.5">{explanation.proofIdea}</dd>
                      </div>
                    )}
                    {explanation.counterexample && (
                      <div>
                        <dt className="font-bold text-foreground">전제가 깨지는 예</dt>
      ...
      <section className="border-t border-border/60 px-4 py-5 sm:px-6">
        ...
        <p className="mt-1 text-xs leading-5 text-muted-foreground">
          문제와 정답 체크리스트를 먼저 만들고, 각 항목을 설명하는 본문 section을
          연결합니다. 기본과 심화 중 하나라도 비면 글은 완료되지 않습니다.
        </p>
            ...
            <section key={level}>
              ...
              <ol className="mt-3 space-y-5 text-xs leading-5 text-muted-foreground">
                {contract.exercises
                  ...
                  .map((exercise, index) => (
                    <li key={exercise.question} className="border-l border-border/80 pl-3">
                      <a
              ...
              >
                ...
                <details className="mt-2 text-xs leading-5 text-muted-foreground">
                  ...
                  <dl className="mt-2 space-y-2 border-l border-border/80 pl-3">
                    ...
                    <div>
                      <dt className="font-bold text-foreground">말하지 않는 것</dt>
