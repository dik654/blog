...
function extractSections(root: HTMLElement): Section[] {
  ...
  const usedIds = new Set(
    [...sectionEls].map((section) => section.id).filter(Boolean),
  );

  const assignUniqueId = (heading: HTMLElement, fallback: string) => {
      const base = heading.dataset.autoSectionBase || heading.id || fallback;
    heading.dataset.autoSectionBase = base;
  ...
  if (sectionEls.length > 0) {
    sectionEls.forEach((sec) => {
      ...
      sec.querySelectorAll<HTMLElement>("h3").forEach((h3) => {
        ...
        if (!text) return;
        const headingId = assignUniqueId(h3, `${id}-${toSlug(text)}`);
        subs.push({ id: headingId, title: text });
      });
    ...
    headings.forEach((el) => {
      ...
      if (!title) return;
      assignUniqueId(el, toSlug(title));
      if (el.tagName === "H2") {
