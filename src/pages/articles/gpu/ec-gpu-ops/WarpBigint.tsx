  ...
  return (
    <section id="warp-bigint" className="mb-16 scroll-mt-20">
      ...
      <div className="prose prose-neutral dark:prose-invert max-w-none">
        <p>
          256비트 곱셈은 <strong>4x4 = 16번의 uint64 곱셈</strong>과 캐리 전파가
          필요하다.
          <br />
          이를 GPU에서 구현하는 방법은 두 가지다.
          <br />
          단일 스레드가 전부 처리하거나, 워프 내 여러 lane이 협력하는 것이다.
        </p>

        <CitationBlock
          source="Emmart et al. -- Faster Modular Exponentiation Using Double Precision"
          citeKey={4}
          type="paper"
        >
          <p className="italic">
            "Warp-cooperative arithmetic reduces register pressure at the cost
            of shuffle latency."
          </p>
        </CitationBlock>
        ...
        <p>
          ...
          <strong>~40개 레지스터</strong>를 소모하므로 SM당 활성 워프 수가
          줄어들고 점유율이 하락한다.
          <br />
          대신 lane 간 통신이 없어 레이턴시가 낮다.
        </p>
        <CodePanel
          title="단일 스레드 256비트 곱셈"
          code={singleThreadCode}
          annotations={[
        ...
        <p>
          4개 lane이 각각 1개 limb를 담당한다.
          <br />
          덧셈 후 캐리를 <code>__shfl_up_sync</code>로 상위 lane에 전달한다.
          <br />
          레지스터 사용량이 <strong>~12개로 감소</strong>하여 점유율이 크게
          올라간다.
          <br />
          단, shuffle 레이턴시(~5 클럭/hop)가 누적되는 단점이 있다.
        </p>
        <CodePanel
          title="Warp 협력 캐리 전파 (__shfl_up_sync)"
          code={warpCoopCode}
          annotations={[
            { lines: [4, 5], color: "sky", note: "각 lane이 1개 limb 담당" },
            {
              lines: [8, 15],
              color: "emerald",
              note: "__shfl_up_sync로 캐리 전달",
            },
            {
              lines: [18, 19],
              color: "amber",
              note: "레지스터 절약 vs shuffle 비용",
            },
          ]}
        />
        ...
        <p>
          <strong>ec-gpu-gen</strong>과 <strong>sppark</strong>는 단일 스레드
          방식을 사용한다.
          <br />
          Montgomery 곱셈의 복잡한 의존 관계가 warp 분산에 불리하기 때문이다.
          <br />
          반면 <strong>ICICLE</strong>은 NTT의 butterfly 연산처럼 구조가 단순한
          경우 warp 협력을 활용한다.
          <br />
