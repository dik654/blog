  ...
  return (
    <section id="overview" className="mb-16 scroll-mt-20">
      ...
      <div className="prose prose-neutral dark:prose-invert max-w-none">
        <p>
          ZK 증명의 핵심 연산은 두 가지다.
          <strong>MSM</strong>(Multi-Scalar Multiplication)은 수백만 개의
          타원곡선 점 덧셈이고,
          <strong>NTT</strong>(Number Theoretic Transform)는 수백만 번의 Fp
          곱셈이다.
          <br />두 연산 모두 <strong>독립적인 반복</strong>이므로 GPU 병렬화에
          적합하다.
        </p>

        <CitationBlock
          source="ec-gpu-gen -- Rust GPU EC library"
          citeKey={1}
          type="code"
          href="https://github.com/filecoin-project/ec-gpu-gen"
        >
          <p className="italic">
            "GPU-accelerated elliptic curve and finite field operations,
            targeting BN254 and BLS12-381."
          </p>
        </CitationBlock>

        <h3 className="text-xl font-semibold mt-8 mb-3">병목: 256비트 연산</h3>
        <p>
          GPU의 네이티브 정수 타입은 최대 <strong>uint64</strong>다.
          <br />
          BN254의 소수체는 254비트, BLS12-381은 381비트이므로 하나의 필드 원소를
          uint64 4개 또는 6개로 쪼개야 한다.
          <br />이 <strong>multi-limb 표현</strong>이 모든 GPU 필드 연산의
          출발점이다.
        </p>
        <CodePanel
          title="Multi-limb 필드 원소 표현 (CUDA C++)"
          code={limbCode}
          annotations={[
            { lines: [3, 5], color: "sky", note: "BN254: 4-limb (256비트)" },
            {
              lines: [8, 10],
              color: "emerald",
              note: "BLS12-381: 6-limb (384비트)",
            },
            {
              lines: [13, 15],
              color: "amber",
              note: "GPU는 64비트까지만 네이티브",
            },
          ]}
        />
