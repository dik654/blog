  ...
  return (
    <section id="events" className="mb-16 scroll-mt-20">
      ...
      <div className="prose prose-neutral dark:prose-invert max-w-none">
        <p>
          CUDA 이벤트(event)는 스트림 내 특정 시점을 표시하는 마커입니다.
          <br />
          두 가지 핵심 용도가 있습니다.
          <br />
        ...
        <p>
          ...
          <code>cudaEventElapsedTime</code>으로 경과 시간을 구합니다.
          <br />
          GPU 타이머를 사용하므로 CPU 타이머보다 정확합니다.
          <br />
          호스트-디바이스 간 지연(latency)을 포함하지 않아 순수 GPU 실행 시간을
          측정할 수 있습니다.
        </p>
        ...
        <p>
          <code>cudaStreamWaitEvent</code>를 사용하면 한 스트림이 다른 스트림의
          특정 지점을 대기할 수 있습니다. stream1에서 이벤트를 기록하고,
          stream2에서 해당 이벤트를 대기합니다. stream1의 나머지 작업은 영향
          없이 계속 실행됩니다.
        </p>

        <CodePanel
          title="크로스-스트림 의존성 (cudaStreamWaitEvent)"
          code={crossStreamCode}
          annotations={[
            {
              lines: [7, 8],
              color: "sky",
              note: "stream1에서 전처리 후 이벤트 기록",
            },
            {
              lines: [10, 12],
              color: "emerald",
              note: "stream2가 이벤트 대기 후 실행",
            },
            {
              lines: [14, 15],
              color: "amber",
              note: "stream1은 독립 작업 계속 실행",
            },
          ]}
        />

        <p>
          이 패턴은 DAG(방향 비순환 그래프) 형태의 복잡한 의존성을 표현할 때
          유용합니다.
          <br />
