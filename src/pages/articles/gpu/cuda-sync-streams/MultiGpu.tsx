    ...
    <section id="multi-gpu" className="mb-16 scroll-mt-20">
      ...
      <div className="prose prose-neutral dark:prose-invert max-w-none">
        <p>
          ...
          <code>cudaSetDevice</code>로 활성 GPU를 전환하고, 각 GPU에 독립적으로
          메모리 할당과 커널 실행을 수행합니다.
        </p>
        ...
        <p>
          가장 단순한 방법은 데이터를 균등 분할하여 각 GPU에 할당하는 것입니다.
          <br />
          GPU 간 데이터 교환이 필요하면 P2P(Peer-to-Peer) 접근을 활성화합니다.
          <br />
          NVLink로 연결된 GPU는 PCIe 대비 5~10배 높은 대역폭으로 직접
          통신합니다.
        </p>

        <CodePanel
          title="다중 GPU 데이터 분할 & P2P 접근"
          code={multiGpuCode}
          annotations={[
            {
              lines: [2, 10],
              color: "sky",
              note: "GPU별 데이터 분할 & 커널 실행",
            },
            {
              lines: [13, 20],
              color: "emerald",
              note: "P2P 접근 가능 여부 확인 & 활성화",
            },
            {
              lines: [22, 23],
              color: "amber",
              note: "GPU 간 직접 메모리 전송",
            },
          ]}
        />
        ...
        <p>
          비동기 스트림을 사용하면 GPU 커널 실행 중 CPU가 다른 작업을 수행할 수
          있습니다.
          <br />
          CPU는 I/O, 데이터 전처리, 결과 후처리를 담당하고, GPU는 병렬 연산에
          집중합니다.
        </p>
        ...
        <p className="mt-4">
          핵심 원칙은 CPU와 GPU 각각의 유휴 시간을 최소화하는 것입니다.
          <br />
          스트림과 이벤트를 조합하면 복사, 연산, 후처리가 파이프라인으로 중첩
          실행됩니다.
        </p>
      </div>
    </section>
