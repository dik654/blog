  ...
  return (
    <section id="bindings" className="mb-16 scroll-mt-20">
      ...
      <div className="prose prose-neutral dark:prose-invert max-w-none">
        <p>
          ICICLE은 C API 위에 Rust와 Go 바인딩을 제공한다.
          <br />
          패턴은 동일하다: 디바이스 설정 &rarr; 입력 준비 &rarr; Config 구성
          &rarr; 연산 호출 &rarr; 결과 회수.
        </p>

        <h3 className="text-xl font-semibold mt-8 mb-3">Rust 바인딩</h3>
        <p>
          <code>icicle-runtime</code>이 디바이스 관리를,{" "}
          <code>icicle-bn254</code> 같은 커브 크레이트가 타입을 제공한다.
          <code>HostSlice</code>로 호스트 메모리를 감싸면 라이브러리가
          내부적으로 GPU 전송을 처리한다.
        </p>
        <CodePanel
          title="Rust MSM (icicle-bn254)"
          code={rustCode}
          annotations={[
            { lines: [6, 6], color: "sky", note: "디바이스: CUDA GPU 0번" },
            {
              lines: [9, 10],
              color: "emerald",
              note: "HostSlice: 자동 H2D 전송",
            },
            {
              lines: [13, 14],
              color: "amber",
              note: "MSMConfig: 윈도우 + 사전계산",
            },
        ...
        <p>
          Go 바인딩은 cgo를 통해 C API를 호출한다. gnark의 <code>MultiExp</code>{" "}
          인터페이스를 ICICLE 백엔드로 교체하면, 기존 회로 코드를 수정하지 않고
          GPU 가속을 적용할 수 있다.
        </p>
        <CodePanel
          title="Go MSM (gnark 통합)"
          code={goCode}
          annotations={[
        ...
        />
        <CitationBlock
          source="ICICLE Rust/Go Bindings"
          citeKey={3}
          type="code"
          href="https://github.com/ingonyama-zk/icicle/tree/main/wrappers"
        >
          <p className="text-xs">
            v3부터 <code>icicle_runtime::set_device()</code>로 백엔드를 런타임에
            선택한다.
            <br />
            CUDA 미설치 환경에서는 자동으로 CPU 폴백이 적용되어, CI/CD
            파이프라인에서 GPU 없이도 테스트가 가능하다.
          </p>
        </CitationBlock>
      </div>
    </section>
