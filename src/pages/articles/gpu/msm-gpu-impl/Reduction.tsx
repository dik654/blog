  ...
  return (
    <section id="reduction" className="mb-16 scroll-mt-20">
      ...
      <div className="prose prose-neutral dark:prose-invert max-w-none">
        <p>
          버킷 누적이 끝나면 각 윈도우의 2^c개 버킷을 하나의 점으로 환원해야
          한다.
          <br />
          목표는 <code>sum(i * bucket[i])</code>이지만, 스칼라 곱을 직접
          수행하면 비용이 크다.
        </p>
        <p>
          <strong>삼각 합산</strong>(running sum trick)은 최상위 버킷부터
          내려오며 누적하는 방식이다.
          <br />
          O(2^c)번의 점 덧셈만으로 가중합을 완성한다. 곱셈이 전혀 필요 없다.
        </p>
        <p>
          이 단계는 윈도우당 하나의 스레드가 순차 실행한다.
          <br />
          윈도우 수가 16개 정도로 적으므로 병렬성은 낮지만, 버킷 수(2^c)가 크기
          때문에 전체 연산량은 무시할 수 없다.
        </p>
        <CodePanel
          title="삼각 합산 커널"
          code={reductionCode}
        ...
        <p>
          윈도우별 결과를 <code>w[j] * 2^(j*c)</code> 가중치로 합친다.
          <br />
          호너법(Horner's method)을 적용하면 더블링 c번과 덧셈 1번의 반복으로
          처리된다.
          <br />
