  ...
  return (
    <section id="bucket-kernel" className="mb-16 scroll-mt-20">
      ...
      <div className="prose prose-neutral dark:prose-invert max-w-none">
        <p>
          버킷 누적의 핵심 난제는 <strong>race condition</strong>이다.
          <br />
          여러 스레드가 동시에 같은 버킷에 점을 더하면 결과가 손상된다.
          <br />세 가지 해결책이 있다.
        </p>
        <p>
          <strong>방법 1</strong>: 원자적 점 덧셈. 구현이 간단하지만 직렬화되어
          느리다.
          <strong>방법 2</strong>: 버킷 인덱스 기준 정렬 후 순차 누적. 정렬
          비용은 O(n log n)이지만 누적은 충돌 없이 진행된다.
          <strong>방법 3</strong>: 스레드별 로컬 버킷 할당 후 병합. 메모리
          사용량이 크다.
        </p>
        <p>
          실전 구현(sppark, ICICLE)은 방법 2를 주로 사용한다.
          <br />
          CUDA의 <code>cub::DeviceRadixSort</code>가 GPU 기수 정렬을 효율적으로
          수행하기 때문이다.
        </p>
        <CodePanel
          title="정렬 기반 버킷 분류 전략"
          code={sortApproach}
          annotations={[
        ...
        <p>
          정렬이 끝나면 각 버킷의 시작 오프셋과 크기를 알 수 있다.
          <br />
          블록 하나가 버킷 하나를 담당하여 해당 구간의 점들을 순차적으로 더한다.
          <br />
