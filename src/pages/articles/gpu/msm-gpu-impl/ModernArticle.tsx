...
export default function ModernMsmGpuArticle() {
  return <article className="space-y-14">
    ...
    <section id="work-plan" className="space-y-6">
      ...
      <ExplainedFormula question="Scalar를 c-bit window 작업으로 어떻게 나눌까?" idea={<>각 scalar를 radix 2^c의 signed digits로 나타내면 window마다 같은 절댓값 digit을 bucket 하나에 모을 수 있습니다.</>} formula={String.raw`s_i=\sum_{j=0}^{W-1}d_{i,j}2^{cj},\qquad W=\left\lceil\frac{\ell}{c}\right\rceil`} terms={[
    ...
    <section id="bucket-ownership" className="space-y-6">
      ...
      <ExplainedFormula question="Bucket workspace의 상한을 어떻게 예산할까?" idea={<>Window와 partition마다 non-zero bucket point를 보관한다고 가정하면 bucket 수와 내부 point bytes의 곱으로 기본 storage를 잡습니다.</>} formula={String.raw`B_{bucket}=W\,G\,(2^{c-1})\,s_P`} terms={[
    ...
    <section id="reduction" className="space-y-6">
      ...
      <ExplainedFormula question="왜 backward running sum이 bucket index 가중치를 복원할까?" idea={<>B_b가 내부 누적합에 정확히 b번 등장하도록 suffix sums를 모두 더합니다.</>} formula={String.raw`R_j=\sum_{b=1}^{M}bB_{j,b}=\sum_{r=1}^{M}\left(\sum_{b=r}^{M}B_{j,b}\right),\qquad M=2^{c-1}`} terms={[
    ...
    <section id="release-gate" className="space-y-6">
      ...
      <ExplainedFormula question="GPU MSM 후보의 처리량과 채택 speedup을 어떻게 계산할까?" idea={<>Correct output에 실제로 반영된 pair만 세고 baseline과 candidate의 같은 end-to-end 경계를 비교합니다.</>} formula={String.raw`R_{pair}=\frac{n_{valid}}{t_{kernel}},\qquad S=\frac{T_{CPU}^{e2e}}{T_{GPU}^{e2e}}`} terms={[
