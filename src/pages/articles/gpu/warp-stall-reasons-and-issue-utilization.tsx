  ...
  return (
    <div id="overview" className="space-y-16">
      <section id="sampling" className="scroll-mt-20">
        ...
        <div className="prose prose-neutral max-w-none dark:prose-invert">
          <p className="text-lg leading-8">
            Kernel 이 느린 이유를 profiler 가 알려 주는 방식은 두 가지뿐입니다. 하나는
            SM 안의 hardware performance counter 가 issue 된 instruction 수나 DRAM 을
            지난 byte 수를 사건마다 세는 것이고, 다른 하나는 일정 clock 마다 warp 하나를
            무작위로 골라 그 순간의 상태를 적어 두는 sampling 입니다.
          </p>
          <p>
            Counter 는 정확하지만 이유를 말하지 못합니다. Issue 된 instruction 이 1000 clock
            에 400개였다는 사실은 알려 줘도, 나머지 600 clock 에 warp 들이 무엇을 기다렸는지는
            counter 에 없습니다. 그 이유를 채우는 것이 warp state sampling 입니다.
          </p>
          <p>
            Nsight Compute 의 sampler 는 SM 마다 고정 간격으로 active warp 하나를 골라
            program counter 와 scheduler 가 본 그 warp 의 상태를 기록합니다. 간격은 작은
            GPU 에서 32 clock, SM 이 많은 chip 에서는 2048 clock 까지 늘어나며, 결과는
            시간축 없이 instruction 주소별 표본 수로만 남습니다.
          </p>
          <p>
            표본은 통계입니다. 어떤 warp 의 표본 100개 가운데 60개가 memory 를 기다리는
            상태였다면 그 warp 는 시간의 약 60% 를 그 상태로 보낸 것이고, 이 비율을 원인별로
            나눈 이름표가 다음 절의 stall reason 입니다. 표본 수가 적은 짧은 kernel 에서는
            이 비율이 흔들립니다.
          </p>
          <p>
      ...
      <section id="stall-taxonomy" className="scroll-mt-20">
        ...
        <div className="prose prose-neutral max-w-none dark:prose-invert">
          <p>
            Scheduler 가 어느 clock 에 warp 하나를 보면 그 warp 는 issue 됐거나(selected),
            준비됐지만 다른 warp 에 밀렸거나(not selected), 무언가를 기다리는 중입니다.
            기다림의 종류가 stall reason 이고, Nsight Compute 는 기다리는 대상이 무엇인지로
            스무 가지 가까이 나눕니다.
          </p>
          <p>
            자주 보는 것은 여섯 가지입니다. Global·local·texture memory 의 결과를 기다리면
            long scoreboard, shared memory 나 특수 함수 unit 의 결과를 기다리면 short
            scoreboard, 같은 block 의 다른 warp 가 barrier 에 오기를 기다리면 barrier 입니다.
          </p>
          <p>
            산술 instruction 의 고정 latency 를 기다리면 wait, 쓰려는 실행 pipe 가 이미 차
            있으면 math pipe throttle, memory instruction 을 넣는 queue 가 차 있으면 MIO
            또는 LG throttle 입니다. 나머지는 instruction fetch, branch 해석, memory barrier
            같은 드문 경우입니다.
          </p>
          <p>
            수치로 읽어 보겠습니다. 어떤 warp 의 표본 100개가 long scoreboard 60, short
            scoreboard 15, barrier 10, not selected 10, selected 5 로 나뉘었다고 합시다.
            이 warp 는 100 clock 중 5 clock 만 issue 했으므로 issue 하나마다 평균 20 clock
            을 보냈고, 그 20 clock 의 절반 이상이 global memory 를 기다린 시간입니다.
          </p>
          <p>
            같은 subpartition 에 이런 warp 가 8개 있으면 scheduler 가 매 clock 하나를 고를
            수 있는 clock 은 대략 8 × 5% = 40% 입니다. 나머지 60% 의 clock 에는 고를 warp 가
            없어 issue slot 이 비고, 진단은 global memory latency 에 묶인 kernel 입니다.
          </p>
          <p>
            이 절의 원인 이름은 scheduler 의 판정을 이름 붙인 것이지 원인의 증명이 아닙니다.
            표본이 어느 instruction 에 몰렸는지를 source view 에서 함께 봐야 어떤 load 가
            기다림을 만들었는지 알 수 있고, 그 읽기 절차는 마지막 절의 pseudocode 로
            정리합니다.
          </p>
        </div>
      ...
      <section id="scoreboard-barrier" className="scroll-mt-20">
        ...
        <div className="prose prose-neutral max-w-none dark:prose-invert">
          <p>
            Long 과 short 의 차이는 기다림의 길이가 아니라 경로입니다. Nsight Compute 는
            L1TEX unit 을 거치는 global·local·surface·texture 접근을 long scoreboard 로,
            MIO queue 를 거치는 shared memory·특수 함수·dynamic branch 를 short scoreboard
            로 나눕니다. 이름의 long·short 는 두 경로의 전형적인 latency 를 딴 것입니다.
          </p>
          <p>
          ...
          <p>
            숫자로 보면 block 256 thread 는 warp 8개입니다. 그중 하나가 barrier 앞에서 100
            clock 짜리 예외 경로를 돌면 나머지 7개는 그 100 clock 을 barrier 에서 보내고,
            warp 8개의 표본 800개 가운데 700개가 barrier 로 찍힙니다. 문서는 이 경우를 barrier
            앞의 갈라진 경로로 진단합니다.
          </p>
          <p>
            처방은 barrier 앞의 일을 warp 마다 고르게 나누는 것이고, block 이 512 thread
            이상이면 더 작은 block 으로 쪼개는 것입니다. 쪼개면 한 barrier 를 기다리는 warp
            수가 줄어 eligible warp 가 늘지만, block 마다 shared memory 를 따로 잡으므로
            shared memory 가 새 occupancy 한도가 될 수 있습니다.
          </p>
        </div>
        <ProgressiveDetail
      ...
      <section id="eligible-issue-slot" className="scroll-mt-20">
        ...
        <div className="prose prose-neutral max-w-none dark:prose-invert">
          <p>
            Stall reason 을 읽기 전에 볼 숫자가 있습니다. Scheduler Statistics 의 eligible
            warps per scheduler 는 매 clock scheduler 가 고를 수 있었던 warp 수의 평균이고,
            issue slot utilization 은 active clock 가운데 실제로 instruction 을 낸 clock 의
            비율입니다. 문서는 scheduler 가 매 clock issue 하고 있다면 stall 을 보지 말라고
            씁니다.
          </p>
          <p>
            Scheduler 하나는 clock 당 warp 하나만 issue 합니다. 그래서 eligible 이 3개인
            clock 도 1개인 clock 도 issue 는 한 번이고, eligible 이 0개인 clock 만 slot 을
            비웁니다. Issue slot utilization 은 eligible 이 하나 이상인 clock 의 비율과
            같습니다.
          </p>
          <p>
            평균 eligible 이 0.8 이라면 slot 이 채워진 clock 은 아무리 많아도 80% 입니다.
            Eligible 이 1을 넘는 clock 이 하나라도 있으면 그만큼 다른 clock 이 0이어야
            평균이 맞으므로, 실제 utilization 은 80% 보다 낮습니다. Eligible 이 2와 0으로만
            번갈아 나타나면 40% 까지 떨어집니다.
          </p>
          <p>
          ...
          <p>
            앞 절의 예로 돌아가면 warp 하나가 100 clock 에 5번 issue 했으므로 매 clock
            slot 을 채우려면 warp 20개가 필요한데, subpartition 의 상한은 16개입니다.
            Occupancy 를 끝까지 올려도 utilization 은 80% 를 넘지 못하므로, 처방은 warp 수가
            아니라 issue 하나당 20 clock 을 만드는 long scoreboard 를 줄이는 것입니다.
          </p>
          <p>
...
\end{aligned}`}
      ...
      <section id="sm-utilization" className="scroll-mt-20">
        ...
        <div className="prose prose-neutral max-w-none dark:prose-invert">
          <p>
            같은 "이용률" 이라는 말이 세 층위에서 쓰입니다. SM utilization 은 kernel 이 도는
            동안 SM 에 warp 가 하나라도 있던 clock 의 비율이고, issue active 는 그 active
            clock 가운데 scheduler 가 instruction 을 낸 비율이며, SM throughput 은 issue 된
          ...
          <p>
            SM utilization 이 낮은 원인은 stall 이 아닙니다. H100 의 SM 132개에 block 66개짜리
            grid 를 띄우면 절반의 SM 은 처음부터 끝까지 비어 있고, block 수가 SM 수의 배수가
            아니면 마지막 wave 에서 일부 SM 만 일하는 tail 이 생깁니다. 이때 issue active 는
            높아도 chip 은 놀고 있습니다.
          </p>
          <p>
            반대로 SM utilization 이 98% 인데 issue active 가 40% 면 SM 은 꽉 찼지만
            scheduler 가 매 clock 의 60% 를 비운 것이고, 이때 비로소 stall reason 을 읽을
            차례입니다. Chip 이 비었는지, SM 이 놀았는지, pipe 가 느렸는지는 서로 다른 처방으로
            갑니다.
          </p>
          <p>
            Issue active 가 높고 SM throughput 도 높으면 kernel 은 compute-bound 이고 그
            판정은 roofline 의 pipe utilization 이 소유합니다. Issue active 가 높은데 throughput
            이 낮으면 issue 된 instruction 이 유효한 일을 적게 한 것이며, divergence 로 lane 이
            꺼졌거나 instruction 수 자체가 많은 경우입니다.
          </p>
        </div>
        <TermBreakdown
      ...
      <section id="reading-procedure" className="scroll-mt-20">
        ...
        <div className="prose prose-neutral max-w-none dark:prose-invert">
          <p>
            Nsight Compute 결과는 위에서 아래로 읽습니다. Chip 이 비었는지, SM 의 scheduler
            가 매 clock issue 했는지, eligible 이 남아돌았는지를 차례로 확인한 뒤에야 가장
            큰 stall reason 이 처방으로 이어지고, 그 처방은 표본이 몰린 instruction 하나를
            바꾸는 것으로 끝납니다.
          </p>
          <p>
            순서를 바꾸면 틀린 처방이 나옵니다. Issue active 가 95% 인 kernel 에서 long
            scoreboard 60% 를 보고 memory 를 고치면 scheduler 는 이미 매 clock 일하고
            있었으므로 elapsed 는 줄지 않고, tail 로 SM 절반이 놀던 kernel 에서 stall 을
            고쳐도 chip 의 절반은 여전히 비어 있습니다.
          </p>
          <p>
