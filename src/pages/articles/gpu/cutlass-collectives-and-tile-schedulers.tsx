    ...
    <div id="overview" className="space-y-16">
      <section id="collectives" className="scroll-mt-20">
        ...
        <div className="prose prose-neutral max-w-none dark:prose-invert">
          <p className="text-lg leading-8">
            CUTLASS 3.x 의 GEMM kernel 은 collective mainloop 과 collective epilogue 두 부품을
            tile scheduler 가 이어 붙인 것입니다. Collective 는 threadblock(또는 cluster) 하나가
            output tile 하나를 맡을 때 무엇을 받아 무엇을 돌려주는지의 계약이고, 그 tile 을
            어느 threadblock 이 언제 맡는지는 collective 밖의 scheduler 가 정합니다.
          </p>
          <p>
      ...
      <section id="pipeline-stages" className="scroll-mt-20">
        ...
        <div className="prose prose-neutral max-w-none dark:prose-invert">
          ...
          <p>
            Stage 를 더 두는 것이 늘 이득은 아닙니다. Shared memory 를 많이 쓰면 SM 에 함께
            머무는 threadblock 수가 줄어 다른 방식의 지연 숨김이 사라지고, K 가 짧아 k-iteration
            이 stage 수보다 적으면 앞서 발행할 것 자체가 없습니다. 128×256×64 처럼 B 가 32 KB
            인 tile 은 stage 4 에 192 KB 를 써서 SM 당 threadblock 하나만 남습니다.
          </p>
        </div>
        <ExplainedFormula
...
\end{aligned}`}
      ...
      <section id="tile-scheduler" className="scroll-mt-20">
        ...
        <div className="prose prose-neutral max-w-none dark:prose-invert">
          <p>
            Tile scheduler 는 output tile 의 격자를 어느 threadblock 이 어떤 순서로 맡을지 정하는
            부품입니다. 가장 단순한 scheduler 는 tile 수만큼 threadblock 을 띄우고 blockIdx 를
            tile 좌표로 바꿉니다. M=N=4096 을 128×128 tile 로 자르면 32×32 = 1024 개의
            threadblock 이 뜨고, hardware 가 SM 이 빌 때마다 다음 threadblock 을 올립니다.
          </p>
          <p>
            Persistent tile scheduler 는 grid 를 SM 수(cluster 를 쓰면 동시에 올릴 수 있는
            cluster 수)로 고정하고, 각 threadblock 이 자기 tile 을 끝낸 뒤 grid 크기만큼 건너뛴
            다음 tile 을 맡는 loop 를 돕니다. H100 의 SM 132개에 1024 tile 이면 threadblock 하나가
            7개 또는 8개를 차례로 처리합니다.
          </p>
          <p>
          ...
          <p>
            Persistent 가 필요한 이유는 launch 비용이 아니라 겹침입니다. 한 threadblock 이 다음
            tile 의 mainloop 을 시작하는 동안 앞 tile 의 epilogue 를 마무리할 수 있고, ping-pong
            schedule 은 consumer warp group 둘에 다른 tile 을 주어 한쪽의 epilogue 를 다른 쪽의
            MMA 뒤에 숨깁니다. Threadblock 이 tile 마다 새로 뜨면 이 겹침이 없습니다.
          </p>
          <p>
            Linear tile 번호를 (m, n) 좌표로 푸는 순서도 scheduler 의 일입니다. Raster order 가
            N 방향이면 같은 A 행 조각을 쓰는 tile 이 연달아 배정되고, swizzle 크기 k 는 tile 번호를
            k개씩 묶어 2차원 덩어리로 돌게 해 같은 시간에 L2 에 있는 A·B 조각의 재사용을 높입니다.
            이 swizzle 은 앞 글의 shared memory swizzle 과 이름만 같은 다른 것입니다.
          </p>
          <p>
            Wave quantization 은 이 배정 방식이 남기는 낭비입니다. 1024 tile 을 132 SM 에
            한 번에 하나씩 올리면 7번의 꽉 찬 wave 뒤에 100 tile 짜리 여덟째 wave 가 남고, 그
            wave 동안 SM 32개가 놉니다. 전체 slot 8×132 = 1056 가운데 1024 만 일하니 3% 낭비입니다.
          </p>
          <p>
            낭비는 tile 수가 SM 수의 배수에서 조금 넘칠 때 커집니다. M=N=2560 이면 tile 이 400개라
            세 wave 396 뒤에 4개가 남아 네 번째 wave 를 통째로 씁니다. 528 slot 에 400 이 일하니
            24% 가 비고, tile 4개를 위해 wave 하나만큼 시간이 늘어납니다.
          </p>
        </div>
        <AlgorithmBlock
      ...
      <section id="stream-k" className="scroll-mt-20">
        ...
        <div className="prose prose-neutral max-w-none dark:prose-invert">
          <p>
            Stream-K 는 wave quantization 을 없애는 분해입니다. Output tile 을 세는 대신 모든 tile
            의 k-iteration 을 하나로 늘어놓고, 그 총량을 SM 수로 나눠 각 threadblock 에 연속한
            구간을 줍니다. 구간이 tile 경계를 넘으면 한 threadblock 이 어떤 tile 은 통째로, 어떤
            tile 은 일부 K 만 계산합니다.
          </p>
          <p>
          ...
          <p>
            CUTLASS 의 sm90 Stream-K scheduler 는 tile 의 마지막 k 구간을 맡지 않은 unit 이
            accumulator 를 global workspace 에 쓰고 barrier 를 k-iteration 수만큼 올리며, 마지막
            구간을 맡은 unit 이 그 barrier 를 기다렸다가 partial 을 읽어 더한 뒤 epilogue 를 한
            번만 실행합니다.
          </p>
          <p>
            잘리는 tile 수를 줄이기 위해 CUTLASS 는 Stream-K 를 마지막 wave 에만 적용하는
            hybrid 를 씁니다. 1024 tile 이면 7 wave 924 tile 은 데이터 병렬로 그대로 돌고, 남은
            100 tile 의 6,400 iteration 만 132 threadblock 에 48 또는 49 씩 나눕니다. 잘리는 tile 은
            이 100개 안에서만 생기고 workspace 도 그만큼만 필요합니다.
          </p>
          <p>
...
\end{aligned}`}
      ...
      <section id="cluster-multicast" className="scroll-mt-20">
        ...
        <div className="prose prose-neutral max-w-none dark:prose-invert">
          ...
          <p>
            CUTLASS 가 cluster 를 쓰는 첫 이유는 TMA multicast 입니다. ClusterShape 2×1 이면 M 방향으로
            이웃한 두 threadblock 이 같은 B tile 을 필요로 합니다. 각자가 B 의 절반씩만 global 에서
            읽어 multicast 로 두 threadblock 의 shared memory 에 동시에 쓰면, B 의 L2 읽기 traffic 이
            절반이 됩니다. A 는 두 threadblock 이 다르므로 각자 읽습니다.
          </p>
          <p>
          ...
          <p>
            Stream-K 의 partial 교환이나 epilogue 의 reduction 이 global workspace 대신 이웃의
            shared memory 를 쓸 수 있지만, cluster 밖의 threadblock 과는 여전히 global 을 거칩니다.
          </p>
          <p>
            Cluster 는 scheduler 의 단위도 바꿉니다. Persistent grid 는 SM 수가 아니라 동시에 올릴
            수 있는 cluster 수 곱하기 cluster 크기로 잡히고, tile 번호는 cluster 안의 rank 를 먼저
            뗀 뒤 풀립니다.
          </p>
          <p>
        ...
        >
          <p>
            2×1 은 B 를, 1×2 는 A 를, 2×2 는 둘 다 절반으로 줄입니다. 그러나 2×2 cluster 는 SM 4개가
            한 GPC 안에서 동시에 비어야 뜨고, tile 격자가 cluster 모양으로 나눠떨어지지 않으면
            가장자리 cluster 의 일부 threadblock 이 빈 tile 을 받습니다. CUTLASS 의 profiler 가
      ...
      <section id="evidence" className="scroll-mt-20">
        ...
        <div className="prose prose-neutral max-w-none dark:prose-invert">
          ...
          <p>
            Cluster 크기 상한 8·16, threadblock 당 shared memory 227 KB, TMA multicast 는 NVIDIA
            Hopper tuning guide 의 값입니다. Profiler flag 는 CUTLASS profiler 문서의 것이며, 실측
            없이 어떤 구성이 빠르다고 말한 곳은 이 글에 없습니다.
          </p>
        </div>
