    ...
    <section id="state-of-art" className="mb-16 scroll-mt-20">
      ...
      <div className="prose prose-neutral dark:prose-invert max-w-none">
        <p>
          ...
          <strong>gnark</strong>가 CPU 멀티코어 레벨 병렬화를 가장 먼저
          구현했고,
          <strong>Scroll</strong>은 zkEVM prover에서 GPU 가속을 실험 중이다.
          <strong>circom</strong> 생태계는 아직 순차 실행이 기본이며, GPU 포팅이
          진행 중이다.
        </p>

        <CitationBlock
          source="gnark -- ConsenSys ZK Framework"
          citeKey={4}
          type="code"
        ...
        <CodePanel
          ...
          annotations={[
            { lines: [3, 4], color: "sky", note: "레벨별 순회" },
            {
              lines: [6, 12],
              color: "emerald",
              note: "goroutine으로 동일 레벨 병렬 실행",
            },
            {
              lines: [14, 14],
              color: "amber",
              note: "레벨 간 동기화 (WaitGroup)",
            },
            {
              lines: [17, 19],
              color: "violet",
              note: "GPU 포팅 시 100x+ 가속 가능성",
            },
          ]}
        />
        ...
        <p>
          <strong>FPGA/ASIC</strong>: Ingonyama, Cysic 등이 witness 생성 전용
          하드웨어를 개발 중이다.
          <br />
          결정론적 연산 패턴을 갖는 witness 생성은 고정 파이프라인 설계에
          적합하다.
          <strong>zkVM</strong> 계열(RISC Zero, SP1)은 범용 실행 트레이스에서
          witness를 추출하므로, GPU보다 메모리 대역폭이 병목이 되어 HBM 기반
          가속기가 유리할 수 있다.
        </p>
      </div>
    </section>
