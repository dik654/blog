  ...
  return (
    <section id="r1cs-solve" className="mb-16 scroll-mt-20">
      ...
      <div className="prose prose-neutral dark:prose-invert max-w-none">
        <p>
          R1CS는 <code>(A * w) . (B * w) = (C * w)</code> 형태의 제약
          시스템이다.
          <br />
          A, B, C는 희소 행렬이고, w는 모든 와이어 값을 담은 벡터다. witness
          생성은 public input이 주어졌을 때 나머지 w를 채우는 과정이다.
        </p>

        <CitationBlock
          source="circom -- R1CS Witness Calculator"
          citeKey={2}
          type="code"
          href="https://github.com/iden3/circom"
        >
          <p className="text-xs">
            circom의 witnesscalc는 제약을 위상 정렬 순서로 하나씩 풀어나간다.
            <br />각 제약에서 미지수가 정확히 하나가 되도록 순서를 보장해야
            한다.
          </p>
        </CitationBlock>

        <CodePanel
          title="순차 Witness Solver (Rust 의사코드)"
          code={sequentialCode}
          annotations={[
            {
              lines: [1, 2],
              color: "sky",
              note: "R1CS 정의: 행렬-벡터 곱의 원소별 곱",
            },
            {
              lines: [5, 7],
              color: "emerald",
              note: "public input으로 초기화",
            },
            {
              lines: [9, 13],
              color: "amber",
        ...
        <p>
          모든 제약 간의 의존성을 DAG(유향 비순환 그래프)로 표현할 수 있다.
          <br />이 DAG의 <strong>최장 경로(critical path)</strong>가 병렬화의
          이론적 한계를 결정한다.
          <br />
