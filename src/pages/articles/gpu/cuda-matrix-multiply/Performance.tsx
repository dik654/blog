    ...
    <section id="performance" className="mb-16 scroll-mt-20">
      ...
      <div className="prose prose-neutral dark:prose-invert max-w-none">
        ...
        <p className="leading-7">
          N x N 정사각 행렬 기준, 나이브와 타일링의 글로벌 메모리 로드 횟수를
          비교합니다.
          <br />
          TILE_SIZE를 T라 하면, 타일링은 글로벌 로드를 <strong>T배</strong>{" "}
          줄입니다.
        </p>
        ...
        <p className="leading-7">
          타일링 커널은 블록당 공유 메모리를 사용하므로, SM(Streaming
          Multiprocessor)에서 동시에 실행 가능한 블록 수가 제한됩니다.
          <br />
          T=16이면 블록당 2KB로 가벼우나, T=32이면 8KB로 증가합니다.
        </p>
        <p className="leading-7">
          SM의 공유 메모리 총량은 보통 48~100KB입니다.
          <br />
          T=32일 때 블록당 8KB이면 SM당 최대 6~12블록을 올릴 수 있습니다.
          <br />
          타일 크기를 늘리면 재사용률이 높아지지만, 점유율이 낮아질 수 있으므로
          균형이 필요합니다.
        </p>

        <h3 className="text-xl font-semibold mt-6 mb-3">부동소수점 정밀도</h3>
        <p className="leading-7">
          float(32비트)는 유효 숫자 약 7자리입니다.
          <br />
          N이 크면 내적 누적 과정에서 <strong>라운딩 오차</strong>가 쌓입니다.
          <br />
          double(64비트)은 유효 숫자 약 15자리로 정밀하지만, GPU 처리량이
          float의 1/2 ~ 1/32 수준으로 크게 떨어집니다.
        </p>
        ...
        <p className="leading-7">
          실전에서는 float 타일링 커널이 대부분의 응용에 충분합니다.
          <br />
          과학 계산이나 ZK 증명처럼 높은 정밀도가 필요한 경우에만 double 또는
          정수 연산을 사용합니다.
        </p>
      </div>
    </section>
