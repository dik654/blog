...
const teAnnotations = [
  {
    ...
    color: "sky" as const,
    note: "te.TransformerLayer: drop-in 교체",
  },
  {
    lines: [12, 15] as [number, number],
    color: "emerald" as const,
    note: "fp8_autocast: 자동 혼합 정밀도",
  },
];
    ...
    <section id="transformer-engine" className="mb-16 scroll-mt-20">
      ...
      <div className="prose prose-neutral dark:prose-invert max-w-none">
        <p className="leading-7">
          Transformer Engine은 H100의 4세대 Tensor Core와 소프트웨어가 협력하는
          혼합 정밀도 시스템입니다.
          <br />
          GEMM 연산을 FP8로 수행하되, 누적(accumulation)은 FP32로 유지합니다.
          <br />
          FP16 대비 <strong>연산 처리량 2배</strong>, 메모리 사용량 절반을
          달성하면서 학습 정확도 손실은 거의 없습니다.
        </p>

        <CitationBlock
          source="NVIDIA Transformer Engine Documentation"
          citeKey={5}
          type="paper"
          href="https://docs.nvidia.com/deeplearning/transformer-engine/"
        >
          <p className="italic">
            "Transformer Engine automatically manages mixed FP8/FP16 precision
            on a per-tensor basis, using delayed scaling to maximize FP8
            utilization while preserving convergence."
          </p>
        </CitationBlock>
        ...
        <p className="leading-7">
          FP8의 좁은 표현 범위를 보상하기 위해, 각 텐서마다 독립적인 scale
          factor를 유지합니다.
          <br />매 스텝마다 텐서의 <strong>amax(절대 최대값)</strong>를
          기록하고, 이력 윈도우를 기반으로 최적 스케일을 계산합니다.
          <br />
          이를 <strong>delayed scaling</strong>이라 부릅니다. 현재 스텝이 아닌
          이전 스텝 통계를 사용하므로 오버헤드가 최소화됩니다.
        </p>
        ...
        <p className="leading-7">
          ...
          <code>nn.Linear</code>를 <code>te.Linear</code>로 교체하고{" "}
          <code>fp8_autocast</code>로 감싸면 FP8이 자동 적용됩니다.
        </p>
        <CodePanel
          title="Transformer Engine 사용 예시"
          code={teCode}
          lang="python"
          annotations={teAnnotations}
        />
        ...
        <div className="overflow-x-auto">
          <table className="min-w-full text-sm border border-border">
            ...
            <tbody>
              {(
                [
                  ["GEMM 처리량", "312 TFLOPS", "~989 TFLOPS (3.2x)"],
                  ["메모리 사용", "기준 (1x)", "~0.5x"],
                  ["GPT-3 175B 학습", "기준", "~1.6x 처리량"],
                  ["정확도 차이", "기준", "<0.1% degradation"],
                ] as const
              ).map(([item, fp16, fp8]) => (
                <tr key={item}>
