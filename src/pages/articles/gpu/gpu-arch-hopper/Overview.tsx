  ...
  return (
    <section id="overview" className="mb-16 scroll-mt-20">
      <h2 className="text-2xl font-bold mb-6">
        Hopper vs 이전 세대 (GTX/Ampere)
      </h2>
      <div className="prose prose-neutral dark:prose-invert max-w-none">
        <p className="leading-7">
          NVIDIA GPU 아키텍처는 2012년 Kepler부터 약 2년 주기로 세대 교체를
          거쳤습니다.
          <br />
          2022년 등장한 <strong>Hopper(H100)</strong>는 데이터센터 AI 학습
          전용으로 설계된 최초의 아키텍처입니다.
          <br />
          이전 세대와 달리 그래픽 렌더링 유닛(RT Core)을 제거하고, AI 연산에
          트랜지스터 예산을 집중했습니다.
        </p>
        ...
        >
          <p className="italic">
            "H100 is the first GPU to introduce the Transformer Engine with FP8
            precision, Thread Block Clusters, and the Tensor Memory Accelerator
            (TMA)."
          </p>
        </CitationBlock>
        ...
        <div className="flex flex-wrap gap-2 mb-6">
          {generations.map((g, i) => (
            <div key={g.name} className="flex items-center gap-1 text-sm">
              <span
                className={`px-2 py-1 rounded font-mono ${i === generations.length - 1 ? "bg-emerald-500/15 text-emerald-400 font-bold" : "bg-muted text-muted-foreground"}`}
              >
                {g.name} ({g.year})
              </span>
          ...
          <table className="min-w-full text-sm border border-border">
            <thead>
              <tr className="bg-muted">
                ...
                <th className="border border-border px-4 py-2 text-left font-bold text-emerald-400">
                  H100 (Hopper)
                </th>
              </tr>
            </thead>
        ...
        <p className="leading-7">
          <strong>Tensor Memory Accelerator (TMA)</strong> -- 다차원 텐서를
          하드웨어가 직접 복사합니다. 주소 계산이 불필요합니다.
          <br />
          <strong>Thread Block Cluster</strong> -- 블록 간 공유 메모리 접근이
          가능한 새 계층입니다. GPC 단위로 블록을 묶습니다.
          <br />
          <strong>Transformer Engine</strong> -- FP8/FP16 혼합 정밀도를 자동
          전환합니다. LLM 학습 처리량이 2배 증가합니다.
        </p>
      </div>
    </section>
  );
