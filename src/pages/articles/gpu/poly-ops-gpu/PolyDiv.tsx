  ...
  return (
    <section id="poly-div" className="mb-16 scroll-mt-20">
      <h2 className="text-2xl font-bold mb-6">
        다항식 나눗셈: Vanishing Polynomial
      </h2>
      <div className="prose prose-neutral dark:prose-invert max-w-none">
        <p>
          PLONK 몫 다항식: h(x) = t(x) / Z(x). 평가 형태에서는 점별 나눗셈이다.
          <br />
          나눗셈 = 역원 곱셈이므로 Z(s_i)의 역원만 있으면 O(n) 원소별 곱셈으로
          끝난다.
        </p>
        <p>
          핵심 관찰: Z(x) = x^n - 1을 coset 점 g*omega^i에서 평가하면
          Z(g*omega^i) = g^n * (omega^n)^i - 1 = g^n - 1이다. omega^n = 1이므로{" "}
          <strong>모든 평가점에서 같은 값</strong>이 나온다.
          <br />
          역원 1개만 계산하면 n개 점 전부에 사용할 수 있다.
        </p>
        <CodePanel
          title="점별 나눗셈 CUDA 커널"
          code={divKernel}
          annotations={[
        ...
        <p>
          ...
          <strong>1회 역원 + 3(n-1)회 곱셈</strong>으로 줄인다.
          {"누적곱 계산 → 마지막 역원 1회 → 역순 복원의 3단계 구조"}
          GPU에서는 누적곱을 parallel prefix scan으로 가속할 수 있다.
        </p>
        <CodePanel
          title="Montgomery 배치 역원"
          code={batchInvCode}
          annotations={[
            {
              lines: [7, 10],
              color: "sky",
              note: "1단계: 누적곱 prefix[i] = a[0]*...*a[i]",
            },
            {
              lines: [12, 13],
              color: "emerald",
