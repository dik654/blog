...
const STEPS = [
  {
    label:
      "Round 1: 와이어 a,b,c 다항식 INTT 후 SRS로 MSM 커밋 (3 NTT + 3 MSM)",
  },
...
const ROUND_POSITIONS = ROUNDS.map((round, index) => ({
  ...round,
  x: 10 + ROUNDS.slice(0, index).reduce((sum, item) => sum + item.w + 20, 0),
}));
