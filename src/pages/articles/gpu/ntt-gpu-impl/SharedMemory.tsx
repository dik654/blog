  ...
  return (
    <section id="shared-memory" className="mb-16 scroll-mt-20">
      ...
      <div className="prose prose-neutral dark:prose-invert max-w-none">
        <p>
          NTT GPU 최적화의 핵심은 작은 stride 스테이지를 공유 메모리에서 한 번에
          처리하는 것이다.
          <br />
          글로벌 메모리 왕복을 1회로 줄이고, 블록 내 __syncthreads()로 스테이지
          간 동기화를 처리한다.
        </p>
        <CodePanel
          title="공유 메모리 다중-스테이지 NTT 커널"
          code={sharedKernelCode}
          annotations={[
        ...
        <p>
          전체 스테이지를 두 구간으로 나눈다. 작은 스테이지는 공유 메모리 커널
          1회, 큰 스테이지는 글로벌 커널을 스테이지마다 실행한다.
        </p>
        <CodePanel
          title="전략 요약과 성능"
          code={strategyCode}
        ...
        <p>
          NTT는 64비트 정수 연산이다. uint64_t가 2개 뱅크를 점유하므로 16번째
          스레드에서 충돌이 발생한다.
          <br />
