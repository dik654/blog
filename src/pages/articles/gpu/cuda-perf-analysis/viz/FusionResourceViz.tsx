    ...
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-2 gap-px border border-border bg-border text-center text-xs sm:grid-cols-4">
        {[
          ...
          <div key={item} className="bg-background px-3 py-3 font-semibold">
            {item}
          </div>
        ))}
      </div>
    ...
    <div className="grid gap-5 lg:grid-cols-[1fr_13rem] lg:items-center">
      ...
      <dl className="divide-y divide-border border-y border-border text-xs">
        <div className="flex justify-between py-3">
          <dt>Launch</dt>
          <dd className="font-bold text-primary">3 → 1</dd>
        </div>
        <div className="flex justify-between py-3">
          <dt>중간 HBM</dt>
          <dd className="font-bold text-primary">2 → 0</dd>
        </div>
        <div className="flex justify-between py-3">
          <dt>Live state</dt>
          <dd className="font-bold">작게 유지</dd>
        </div>
      </dl>
    </div>
  );
        ...
        {pressureRows.map(([label, value]) => (
          <div key={label}>
            ...
            <div className="h-2 border border-border bg-background">
              <motion.div
                className={
                  value < 40 ? "h-full bg-amber-600/70" : "h-full bg-primary/70"
                }
                animate={{ width: `${value}%` }}
    ...
    <div className="grid gap-5 lg:grid-cols-[9rem_1fr_9rem] lg:items-center">
      ...
      <div className="border border-primary/55 bg-primary/[0.025] p-5">
        ...
        <p className="mt-4 text-center text-xs leading-5 text-muted-foreground">
          SRAM/register에 맞는 tile만 유지하고 전체 N×N attention matrix는 HBM에
          materialize하지 않습니다.
        </p>
      </div>
    ...
    <div className="grid gap-5 lg:grid-cols-[13rem_1fr] lg:items-center">
      ...
      <div className="border border-primary/55 bg-primary/[0.025] p-5">
        ...
        <p className="mt-4 text-xs leading-5 text-muted-foreground">
          Launch를 줄이는 대신 queue contention, fairness, 종료 protocol, 고정
          residency와 다른 kernel의 scheduling을 새로 책임집니다.
        </p>
      </div>
    </div>
  );
...
const scenes = [
  {
    title: "여러 kernel",
    subtitle: "중간값이 HBM을 왕복",
    render: UnfusedScene,
  },
  {
    title: "작은 fusion",
    subtitle: "낮은 pressure로 왕복 제거",
    render: SmallFusionScene,
  },
  {
    title: "Megakernel",
    subtitle: "합친 자원 비용이 커짐",
    render: MegakernelScene,
  },
  {
    title: "FlashAttention",
    subtitle: "Tile 안에서 fusion 경계 선택",
    render: FlashAttentionScene,
  },
  {
    title: "Persistent",
    subtitle: "살아 있는 worker가 queue 소비",
    render: PersistentScene,
  },
] as const;
...
export function FusionMegakernelViz() {
  ...
  return (
    <figure
      data-viz="kernel-fusion-megakernel"
      tabIndex={0}
      aria-label="CUDA fusion and megakernel scene animation"
      onKeyDown={(event) => {
      ...
      className="not-prose my-9 overflow-hidden border-y border-border bg-background outline-none focus-visible:ring-2 focus-visible:ring-primary/60"
    >
      <figcaption className="flex flex-col gap-3 border-b border-border px-5 py-4 sm:flex-row sm:items-end sm:justify-between sm:px-6">
        <div>
          <p className="text-xs font-semibold text-primary">
            Fusion boundary animation
          </p>
          <p className="mt-1 text-base font-bold">
            Launch를 합칠수록 사라지는 비용과 새로 생기는 비용
          </p>
      ...
      <div
        className="grid grid-cols-2 border-b border-border sm:grid-cols-5"
        role="tablist"
        aria-label="Fusion 흐름 장면"
      >
        {scenes.map((item, index) => (
          ...
          >
            ...
            <span className="mt-1 block break-words text-xs font-bold">
              {item.title}
            </span>
...
export function RegisterResidencyViz() {
  const [registers, setRegisters] =
    useState<(typeof registerChoices)[number]>(64);
  const maxWarps = 64;
  const residentWarps = Math.min(
    maxWarps,
    Math.floor(65_536 / (32 * registers)),
  );
  const residentThreads = residentWarps * 32;

  return (
    <figure
      data-viz="register-residency"
      className="not-prose my-9 overflow-hidden border-y border-border bg-background"
    >
      <figcaption className="border-b border-border px-5 py-4 sm:px-6">
        <p className="text-xs font-semibold text-primary">
          CC 7.0 simplified register bound
        </p>
        <p className="mt-1 text-base font-bold">
          Thread당 register가 늘면 spill 전에도 resident warp가 줄어든다
        </p>
      </figcaption>
      <div className="grid gap-6 p-5 sm:p-6 lg:grid-cols-[15rem_1fr]">
        <div>
          <p className="text-xs font-bold text-muted-foreground">
            Thread당 32-bit registers
          </p>
          ...
          <dl className="mt-5 divide-y divide-border border-y border-border text-xs">
            <div className="flex justify-between py-3">
              <dt>Register-file bound</dt>
              <dd className="font-bold">{residentThreads} threads</dd>
            </div>
            <div className="flex justify-between py-3">
              <dt>Resident warp upper bound</dt>
              <dd className="font-bold text-primary">{residentWarps} / 64</dd>
            </div>
            <div className="flex justify-between py-3">
              <dt>단순 occupancy</dt>
              <dd className="font-bold">
                {Math.round((residentWarps / maxWarps) * 100)}%
              </dd>
            </div>
          </dl>
        </div>
        ...
        <div className="min-w-0">
          <div
            ...
            aria-label={`${residentWarps}개 resident warp와 ${maxWarps - residentWarps}개 비어 있는 warp slot`}
          >
          ...
          <div className="mt-5 grid gap-2 sm:grid-cols-[7rem_auto_8rem_auto_9rem] sm:items-center sm:justify-center">
            ...
            <span className="rotate-90 justify-self-center sm:rotate-0">
              <FlowArrow />
            </span>
          ...
          <p className="mt-4 text-xs leading-5 text-muted-foreground">
            위 계산은 register file만 본 upper bound입니다. 실제 residency는
            block 크기, register allocation granularity, shared memory와
            architecture limit의 최소값입니다. 255는 여러 최신 NVIDIA
            architecture 표에서 보이는 thread당 최대치이지 kernel 전체 한도가
            아닙니다.
          </p>
        </div>
      </div>
