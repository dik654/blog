  ...
  return (
    <section id="opencl-cuda" className="mb-16 scroll-mt-20">
      ...
      <div className="prose prose-neutral dark:prose-invert max-w-none">
        <p>
          ec-gpu-gen은 <strong>OpenCL</strong>과 <strong>CUDA</strong> 두 가지
          백엔드를 지원한다.
          <br />
          원래 OpenCL만 지원했으나, NVIDIA GPU에서 더 높은 성능을 위해 CUDA
          백엔드가 추가되었다.
        </p>
        <CodePanel
          title="OpenCL vs CUDA 백엔드 비교"
          code={backendCode}
          annotations={[
            {
              lines: [3, 11],
              color: "sky",
              note: "OpenCL: 범용 하드웨어 지원",
            },
            {
              lines: [13, 20],
              color: "emerald",
              note: "CUDA: NVIDIA 최적 성능",
            },
            {
              lines: [22, 24],
              color: "amber",
        ...
        <CitationBlock
          source="rust-gpu-tools -- GPU runtime abstraction"
          citeKey={4}
          type="code"
        ...
        >
          <p className="text-xs">
            ...
            <code>Device::all()</code>이 사용 가능한 GPU를 탐색하고, feature
            flag에 따라 OpenCL 또는 CUDA 런타임을 선택한다.
          </p>
        </CitationBlock>
        ...
        <p>
          최근 ZK 프로젝트들은 ec-gpu-gen 대신 <strong>ICICLE</strong>을
          선호한다.
          <br />
          ICICLE은 코드 생성 대신 C++ 템플릿 특수화를 사용하고, 사전 컴파일된
          라이브러리를 제공한다.
          <br />
          다만 Filecoin 메인넷은 여전히 ec-gpu-gen 기반이다.
        </p>
        <CodePanel
          title="ec-gpu-gen vs ICICLE 비교"
          code={migrationCode}
          annotations={[
            ...
            { lines: [11, 15], color: "emerald", note: "ICICLE 이점" },
            {
              lines: [17, 18],
              color: "amber",
              note: "ec-gpu-gen은 Filecoin에서 현역",
            },
          ]}
        />
      </div>
