  ...
  return (
    <section id="bellperson-integration" className="mb-16 scroll-mt-20">
      <h2 className="text-2xl font-bold mb-6">
        bellperson 통합 & GPU 디스패치
      </h2>
      <div className="prose prose-neutral dark:prose-invert max-w-none">
        <p>
          <strong>bellperson</strong>은 Filecoin의 Groth16 증명기다.
          <br />
          증명의 80% 이상을 차지하는 MSM(Multi-Scalar Multiplication) 연산을
          GPU로 오프로드한다. ec-gpu-gen이 생성한 커널을 rust-gpu-tools가
          런타임에 컴파일하고 실행하는 구조다.
        </p>
        <CodePanel
          title="bellperson GPU 아키텍처"
          code={archCode}
          annotations={[
            {
              lines: [3, 5],
              color: "sky",
              note: "bellperson: Groth16 MSM 호출",
            },
            {
              lines: [7, 8],
              color: "emerald",
              note: "ec-gpu-gen: 빌드 타임 커널 생성",
            },
            {
              lines: [10, 14],
              color: "amber",
              note: "rust-gpu-tools: 런타임 GPU 관리",
            },
          ]}
        />

        <CitationBlock
          source="bellperson GPU multiexp"
          citeKey={3}
          type="code"
          href="https://github.com/filecoin-project/bellperson"
        >
          <p className="text-xs">
            커널 소스는 첫 번째 multiexp 호출 시 GPU 드라이버에 의해 컴파일된다.
            <br />
            컴파일된 바이너리는 캐시되어 이후 호출에서 재사용한다.
            <br />이 지연 컴파일(lazy compilation) 전략 덕분에 빌드 타임에 GPU가
            없어도 된다.
          </p>
        </CitationBlock>
        ...
        <p>
          Pippenger 알고리즘 기반이다. 스칼라를 c-bit 윈도우로 분할하고, 각
          윈도우의 버킷 누적을 GPU 커널에서 병렬 실행한다.
          <br />
          최종 윈도우 합산은 CPU에서 수행한다.
        </p>
        <CodePanel
          title="bellperson multiexp 흐름 (요약)"
          code={multiexpCode}
          annotations={[
            {
              lines: [8, 10],
              color: "sky",
              note: "GPU 디바이스 선택 & 커널 로드",
            },
            {
              lines: [12, 14],
              color: "emerald",
