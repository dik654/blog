  ...
  return (
    <section id="overview" className="mb-16 scroll-mt-20">
      ...
      <div className="prose prose-neutral dark:prose-invert max-w-none">
        <p>
          <strong>ec-gpu-gen</strong>은 Filecoin 프로젝트가 개발한 Rust 빌드
          타임 코드 생성기다.
          <br />
          특정 타원곡선의 유한체/곡선 연산을 수행하는{" "}
          <strong>CUDA 또는 OpenCL 커널 소스 코드</strong>를 자동으로 생성한다.
        </p>
        <p>
          bellperson(Groth16 프루버)과 Neptune(Poseidon 해시)이 이 도구로 GPU
          커널을 얻는다.
        </p>
        <CodePanel
          title="문제: 커브마다 다른 파라미터"
          code={problemCode}
          annotations={[
            {
              lines: [3, 8],
              color: "sky",
              note: "BN254: 4-limb, 고유 Montgomery 상수",
            },
            {
              lines: [10, 16],
              color: "emerald",
              note: "BLS12-381: 6-limb, 다른 상수",
            },
            {
              lines: [18, 20],
              color: "amber",
              note: "커브별 수작업 → 유지보수 불가",
            },
          ]}
        />

        <CitationBlock
          source="filecoin-project/ec-gpu-gen"
          citeKey={1}
          type="code"
          href="https://github.com/filecoin-project/ec-gpu-gen"
        >
          <p className="italic text-xs">
            "CUDA/OpenCL code generator for elliptic curve and finite field
            operations, targeting curves used in zero-knowledge proof systems."
          </p>
        </CitationBlock>
        ...
        <p>
          ...
          <code>GpuField</code> 트레이트를 구현한 타입에서 소수, Montgomery
          상수, 커브 파라미터를 추출한 뒤, 템플릿에 주입하여 완성된 GPU 소스를{" "}
          <code>OUT_DIR</code>에 기록한다.
        </p>
        <CodePanel
          title="ec-gpu-gen 빌드 흐름"
          code={flowCode}
          annotations={[
            {
              lines: [3, 9],
              color: "sky",
              note: "build.rs: 커브 파라미터 추출",
            },
            {
              lines: [11, 13],
              color: "emerald",
            ...
            },
            {
              lines: [15, 17],
              color: "amber",
              note: "생성된 커널 → rust-gpu-tools로 실행",
            },
          ]}
        />
      </div>
