...
export default function QAT() {
  return <section id="qat" className="mb-16 scroll-mt-20">
    ...
    <div className="prose prose-neutral dark:prose-invert max-w-none">
      ...
      <p>Label 없이 빠르게 변환해야 한다면 frozen checkpoint와 representative calibration으로 시작하는 PTQ가 첫 후보입니다. 반대로 PTQ가 quality gate를 넘지 못하고 학습 data·compute를 쓸 수 있다면 QAT를 비교할 수 있습니다. 어느 쪽이든 fake-quant score에서 끝내지 않고 변환된 artifact가 실제 low-bit kernel을 타는지, 같은 workload에서 품질·latency·peak memory가 함께 통과하는지 확인해야 합니다.</p>
    </div>
    <ExplainedFormula
...
\hat x&=\operatorname{FQ}(x)\\
\frac{\partial\hat x}{\partial x}
&\approx \mathbf 1[r_{\min}\le x\le r_{\max}]
\end{aligned}`}
      terms={[
