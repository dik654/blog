    ...
    <section id="overview" className="mb-16 scroll-mt-20">
      ...
      <div className="prose prose-neutral dark:prose-invert max-w-none">
        <p className="text-lg leading-8">
          코딩 에이전트의 CLI를 단순한 채팅 입력창으로 보면 설계 책임을 놓치기 쉽습니다. CLI는 세션을 시작하고, 로컬 제어 명령과 모델에 보낼 프롬프트를 나누며, tool call과
          권한 요청을 사용자에게 보여주는 실행 인터페이스입니다. 대화형 REPL(Read-Eval-Print Loop)이면서 런타임을 조작하는 control plane 역할도 맡는
          셈입니다.
        </p>
        <p>
        ...
        <p>
          같은 실행도 사람이 터미널에서 볼 때와 CI가 파이프로 받을 때 필요한 출력이 다릅니다. 대화형 TTY에서는 text delta, tool 진행 상태, 권한 요청을 읽기 쉽게
          갱신할 수 있습니다. 비대화형 환경에서는 ANSI 커서 제어를 제거하고 순서가 보존된 JSONL 같은 형식이 필요합니다. 실행 이벤트를 먼저 구조화해 두면 두 표현 방식을 하나의
          코어 위에 올릴 수 있습니다.
        </p>
        <p>
          한 turn의 최소 receipt에는 input kind와 session·turn ID, command 또는 prompt identity가 들어갑니다. 여기에 event
          sequence, tool/permission terminal state, exit code와 output mode를 더합니다. 이 receipt를 기준으로 TTY와 JSONL을
          비교하면 색상이나 줄바꿈 차이를 무시하면서도 실행 의미가 사라지지 않았는지 확인할 수 있습니다.
        </p>
        <p>
          아래에서는 먼저 slash command의 로컬 실행 계약을 정리하고 이어서 스트리밍 이벤트를 터미널 화면으로 바꾸는 과정을 살펴봅니다. 마지막 초기화 절에서는 새 프로젝트를
          감지하되 기존 파일의 소유권을 침범하지 않는 방법까지 연결합니다.
        </p>
      </div>
    </section>
  );
