    ...
    <section id="roi-crop" className="scroll-mt-20">
      ...
      <div className="prose prose-neutral max-w-none dark:prose-invert">
        ...
        <p className="leading-7">
          모델마다 받아들이는 총 픽셀 예산이 정해져 있기 때문입니다. 어떤 모델은 들어온 이미지를 정해진
          넓이에 맞춰 다시 샘플링하고, 어떤 모델은 미리 정해 둔 해상도 집합 중 하나로 스냅합니다. 어느 쪽이든
          예산을 넘겨 보내면 그만큼을 버리라고 보내는 셈입니다.
        </p>

        <p className="leading-7">
          그래서 4메가픽셀짜리 크롭을 1메가픽셀 예산의 모델에 보내면, 모델은 그것을 1메가픽셀로 줄여서
          봅니다. 잘라 낸 의미가 사라질 뿐 아니라 그 안의 대상도 함께 작아집니다. 잘라 보내는 목적이 대상을
          크게 만드는 것이었는데 정반대가 됩니다.
        </p>

        <p className="leading-7">
          실제로 이 실수를 했습니다. 물건 교체가 지저분했던 원인 중 하나가 2메가픽셀 전체 프레임을 그대로
          넘긴 것이었고, 영역만 잘라 보내자 마스크 안 변화가 73.3에서 44.8로 떨어졌습니다.
        </p>
      </div>
      ...
      <div className="prose prose-neutral mt-8 max-w-none dark:prose-invert">
        <p className="leading-7">
          규칙은 두 단계입니다. 먼저 대상 주변으로 맥락이 들어갈 만큼 여유를 두고 자르고, 그다음 그 크롭이
          모델의 예산을 넘지 않는지 확인합니다. 넘으면 크롭을 줄이는 것이 아니라 크롭 안에서 대상이 차지하는
          비율을 유지한 채 예산에 맞춥니다.
        </p>

        <p className="leading-7">
          마스크 크기도 같은 축에 있습니다. 얼굴 부위를 고치는 편집에서 마스크를 눈부터 입까지의 바운딩 박스로
          잡으면 얼굴 절반을 재생성하게 되고, 정체성이 0.142까지 떨어집니다. 같은 모델·프롬프트·시드에서
          마스크만 뺨을 따라가는 좁은 띠로 바꾸자 0.943이 됐습니다.
        </p>
