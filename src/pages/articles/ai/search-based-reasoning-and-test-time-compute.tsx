  ...
  return (
    <div id="overview" className="space-y-16">
      <section id="problem" className="scroll-mt-20">
        ...
        <div className="prose prose-neutral max-w-none dark:prose-invert">
          <p className="text-lg leading-8">
            Test-time compute 를 쓰는 방법은 한 가지가 아닙니다. 후보를 N 개 독립적으로 만들어
            verifier 로 하나를 고르는 best-of-N, 중간 단계마다 후보를 넓히고 가지치기하는 tree
            search, 이미 낸 답을 스스로 다시 검토하는 self-correction 이 있습니다. 이 글은 이
            세 방법을 search-based reasoning 이라는 한 축의 서로 다른 구현으로 봅니다.
          </p>
          <p>
            세 방법은 같은 재료를 씁니다.{" "}
          ...
          <p>
            Snell 외 (2024) 는 같은 test-time compute 예산에서 두 접근을 비교했습니다. 쉬운
            문제는 답을 순차적으로 고쳐 나가는 revision 이 유리해 64개 revision 이 256개 독립
            샘플과 같은 정확도를 냈고, 중간 난이도 문제는 PRM 으로 방향을 잡는 tree·beam
            search 가 유리해 16개 평가가 64개 독립 샘플과 같은 효율을 냈습니다.
          </p>
        </div>
      ...
      <section id="best-of-n" className="scroll-mt-20">
        ...
        <div className="prose prose-neutral max-w-none dark:prose-invert">
          <p className="text-lg leading-8">
            Best-of-N 은 같은 prompt 에서 model 이 N 개의 candidate 를 서로 독립적으로 생성한 뒤,
            verifier 가 매긴 점수가 가장 높은 candidate 하나를 선택하는 방법입니다. 후보 사이에
            정보 교환이 없어 tree search 보다 구현이 단순합니다.
          </p>
          <p>
            N 을 늘리면 후보 중 정답이 하나라도 있을 확률(pass@N)은 오르지만, 실제로 그 정답을
            골라내는 것은 verifier 의 정확도에 달려 있습니다. Verifier 가 완벽하지 않으면
            pass@N 만큼 정답률이 오르지 않습니다.
          </p>
        </div>
        <ExplainedFormula
        ...
        <div className="prose prose-neutral max-w-none dark:prose-invert">
          <p>
            Lightman 외 (2023) 는 MATH 문제에서 N=1860 개 candidate 를 만들었을 때, 각 reasoning
            step 에 점수를 매기는 PRM 이 최종 답만 보는 ORM 보다 정확한 verifier 임을 보였습니다.
            PRM 선택 정확도 78.2%는 ORM 72.4%, 다수결 투표 69.6%보다 높았습니다.
          </p>
          <p>
      ...
      <section id="tree-search" className="scroll-mt-20">
        ...
        <div className="prose prose-neutral max-w-none dark:prose-invert">
          <p className="text-lg leading-8">
            Tree search 는 매 depth 마다 유지 중인 상태 각각에서 k 개의 다음 단계 후보(생각)를
            만들고, evaluator 로 점수를 매겨 상위 b 개만 다음 depth 로 남기는 탐색입니다.
            Best-of-N 과 달리 중간 상태를 평가해 방향을 바꿀 수 있습니다.
          </p>
          <p>
            Tree of Thoughts 의 Game of 24 실험은 b=5, k=5, depth d=3 을 썼습니다. Root 에서
            k=5 개 생각을 평가해 b=5 개를 남기고, 이후 두 depth 는 유지 중인 b=5 개 상태마다
            k=5 개씩 새로 만들어 평가합니다.
          </p>
        </div>
        <ExplainedFormula
        ...
        <div className="prose prose-neutral max-w-none dark:prose-invert">
          <p>
            Tree of Thoughts 는 Game of 24 에서 GPT-4 + chain-of-thought 의 성공률 4%를 74%로
            올렸습니다. Evaluator 는 각 생각을 sure/maybe/impossible 로 분류하도록 model 에
            요청하고, 노이즈를 줄이려고 후보마다 3회 샘플링해 점수를 모읍니다.
          </p>
        </div>
      ...
      <section id="self-correction" className="scroll-mt-20">
        ...
        <div className="prose prose-neutral max-w-none dark:prose-invert">
          <p className="text-lg leading-8">
            Self-correction 은 model 이 자신의 답을 스스로 비평하고 고치는 절차입니다.
            Self-Refine 은 같은 model 이 생성자·평가자·수정자 역할을 모두 맡아 피드백-수정
            loop 를 반복하며, 별도 학습이나 verifier 없이 7개 과제에서 평균 20%p 가까운 절대
            개선을 보고했습니다.
          </p>
          <p>
            하지만 reasoning 과제에서는 다른 결과가 나왔습니다. Huang 외 (2023) 는 외부 신호
            없이 자기 능력만으로 답을 고치는 intrinsic self-correction 을 GSM8K·CommonSenseQA·
            HotpotQA 에 적용했더니, 모든 model·모든 benchmark 에서 정확도가 오히려 떨어졌다고
            보고했습니다.
          </p>
          ...
          <p>
            두 결과가 서로 모순인 것은 아닙니다. Self-Refine 이 개선을 본 과제는 길이·감정·코드
            스타일처럼 model 스스로 점검할 수 있는 기준이 있는 과제이고, Huang 외가 하락을 본
            과제는 정답이 하나인 reasoning 이라 model 자신의 비평이 오히려 맞는 답을 틀린 답으로
            바꾸기 쉽습니다.
          </p>
          <p>
      ...
      <section id="boundary" className="scroll-mt-20">
        ...
        <div className="prose prose-neutral max-w-none dark:prose-invert">
          <p className="text-lg leading-8">
            Best-of-N 은 병렬로 만들고 사후에 고르고, tree search 는 단계마다 넓히고
            가지치기하며, self-correction 은 이미 낸 답을 사후에 재검토합니다. 셋 다 verifier
            나 self-evaluation 의 신뢰도에 결과가 갇힙니다.
          </p>
        </div>
        <TermBreakdown
        ...
        >
          <p>
            판단은 verifier 가 무엇을 볼 수 있는지에서 시작합니다. 최종 답만 채점할 수 있으면
            ORM 기반 best-of-N, 중간 step 을 채점할 수 있으면 PRM 기반 tree·beam search 가
            자연스럽습니다. 둘 다 없고 model 자신의 판단만 있다면, self-correction 을 쓰기 전에
            먼저 그 판단이 실제로 정답과 상관되는지부터 별도로 검증해야 합니다.
          </p>
          <p>
