    ...
    <section id="comparison" className="mb-16 scroll-mt-20">
      ...
      <div className="prose prose-neutral dark:prose-invert max-w-none">
        <p className="leading-7">
          ECOD는 적어도 한 feature의 tail에 anomaly가 드러날 때 자연스럽다. 개별 marginal에서는 평범한데 조합만 비정상인 row, 정상 cluster 안의
          local sparse point, 시간 순서가 깨진 사건은 놓칠 수 있다. 원 논문의 case study도 outlier가 모든 차원에서 정상점 사이에 섞이거나 중앙에 숨은
          dataset에서는 성능이 떨어졌다고 보고한다.
        </p>
        <p>
          correlation이 강한 열이나 같은 값을 다른 단위로 복제한 열을 함께 넣으면 independence approximation 탓에 같은 evidence를 여러 번 더할 수도
          있다. feature contribution이 가리키는 것은 조사할 열이다. causal attribution으로 읽어서는 안 된다. 의심되는 interaction은
          scatter·conditional rule과 다른 detector로 확인한다.
        </p>
      </div>
      ...
      <div className="prose prose-neutral dark:prose-invert max-w-none">
        ...
        <p>
          model 이름을 가린 상태에서 domain reviewer가 상위 row를 평가하고 기간·지역·사용자군별 alert rate와 feature contribution을 비교한다.
          synthetic anomaly는 특정 실패 모양에 대한 unit test로는 쓸 수 있다. 실제 anomaly를 대표한다는 보장은 없다. 이후 발생한 incident,
