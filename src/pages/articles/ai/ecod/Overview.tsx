  ...
  return (
    <section id="overview" className="mb-16 scroll-mt-20">
      ...
      <div className="prose prose-neutral dark:prose-invert max-w-none">
        ...
        <p>
          feature마다 값의 단위와 scale은 다르다. ECDF는 현재 값보다 작거나 같은 training sample의 비율을 세어 이를 0과 1 사이의 좌표로 바꾼다.
          Gaussian의 평균·표준편차나 histogram bin을 고르지 않는다는 뜻에서 non-parametric이다. 그러면서도 큰 값일수록 오른쪽, 작은 값일수록 왼쪽이라는 순위
          정보는 그대로 남는다.
        </p>
      </div>
      ...
      <div className="prose prose-neutral dark:prose-invert max-w-none">
        ...
        <p>
          ECOD 논문이 parameter-free라고 부르는 까닭은 neighbor 수, histogram bin, tree 수처럼 score 모양을 조절할 핵심
          hyperparameter가 없기 때문이다. 그래도 binary alert를 만들려면 threshold가 필요하고 reference 기간·feature 선택·재학습 주기는 운영자가
          정해야 한다. “설정 없이 이상치를 알아서 확정한다”는 뜻은 아니다.
        </p>
        <p>
          시계열 값도 row로 넣을 수는 있다. 그렇다고 ECOD가 시간 순서를 읽지는 않는다. lag, rolling statistic, 계절 위치 등을 feature로 만들어야 시간
          문맥이 생긴다. 미래 정보가 계산에 섞이지 않도록 cutoff도 지켜야 한다. streaming detector까지 필요하다면 window와 ECDF 갱신 정책도 별도의
          system contract가 된다.
        </p>
      </div>
    </section>
