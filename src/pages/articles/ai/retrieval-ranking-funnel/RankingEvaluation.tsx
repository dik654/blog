    ...
    <section id="ranking-evaluation" className="scroll-mt-20">
      ...
      <div className="prose prose-neutral max-w-none dark:prose-invert">
        <p className="text-lg leading-8">
          Candidate recall과 rerank NDCG만으로는 최종 사용자가 보는 상위
          결과의 품질을 다 설명하지 못합니다. Precision@k는 상위 k개 중
          실제 relevant 비율을, MRR은 첫 정답이 얼마나 앞에 나왔는지를
          봅니다.
        </p>
        <p>
          Ranking precision은 상위 결과 창 안에서의 정확도를 재는 일반적인
          이름이고, 그 창을 k개로 고정해 계산한 값이 Precision@k입니다.
          Recall@k가 relevant set 전체 대비 회수량을 본다면, Precision@k는
          사용자가 실제로 스크롤하는 범위 안에 잡음이 얼마나 섞여 있는지를
          봅니다.
        </p>
        <p>
          예를 들어 reranker의 top-5 output에서 1·2·4번째 문서가 relevant이고
          3·5번째가 아니라면 Precision@5는 3/5=0.6입니다. 같은 top-5가
          relevant set 전체 4개 중 3개를 담았다면 Recall@5는 3/4=0.75이고,
          두 지표는 서로 다른 값을 줍니다.
        </p>
        <p>
          MRR은 query 여러 개에 걸쳐 시스템이 정답을 얼마나 빨리 찾는지를
          한 숫자로 요약합니다. Query마다 상위 결과에서 처음 나온 relevant
          문서의 순위를 뒤집어 모으고, query 수로 나눈 평균입니다.
        </p>
      </div>
      <ExplainedFormula
      ...
      <div className="prose prose-neutral max-w-none dark:prose-invert">
        <p>
          이름이 비슷해 헷갈리기 쉬운 짝이 있습니다. Reciprocal Rank
          Fusion(RRF)과 Mean Reciprocal Rank(MRR)은 둘 다 순위의 역수를
          쓰지만 하는 일이 다릅니다. RRF는 한 query 안에서 여러 검색기의
          목록을 합치는 방법이고, MRR은 완성된 하나의 순위를 여러 query에
          걸쳐 평가하는 지표입니다.
        </p>
        <p>
          하나는 이 funnel의 fusion 단계에서 쓰고, 다른 하나는 결과가 나온
          뒤 평가에서 씁니다. 두 이름을 바꿔 쓰면 합치는 방법과 평가하는
          방법을 뒤섞는 셈입니다.
        </p>
      </div>
    </section>
