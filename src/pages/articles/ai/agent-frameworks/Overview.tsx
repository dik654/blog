  ...
  return (
    <section id="overview" className="mb-16 scroll-mt-20">
      ...
      <div className="prose prose-neutral dark:prose-invert max-w-none">
        <p>
          고객이 “주문 A-104를 환불해 주세요”라고 요청했다고 생각해 봅시다. 먼저 고객과 주문을 조회하고 요청 당시의 환불 정책을 확인해야 합니다.
        </p>
        <p>
          금액이 크다면 담당자의 승인을 기다립니다. 승인 뒤에는 환불 API를 한 번만 호출하고 반환된 영수증이 실제 환불 건과 일치하는지 검증해야 완료됩니다.
        </p>
        <p>
        ...
        <p>
          즉 더 정확한 답을 만들어 주는 새 모델이 아니라 애플리케이션이 직접 구현하던 실행 책임 일부를 가져가는 runtime입니다. 따라서 비교의 첫 질문은 “무엇이 가장 똑똑한가?”가
          아닙니다. “상태와 복구를 누가 소유해야 하는가?”입니다.
        </p>
      </div>
      ...
      <div className="prose prose-neutral dark:prose-invert max-w-none">
        ...
        <p>
          애플리케이션은 이 상태 가운데 모델의 현재 판단에 필요한 부분만 보여 줍니다. 모델이 답이나 tool call을 제안하면 schema와 권한을 검증하고 실행 결과를 다시 상태에
          기록합니다.
        </p>
    ...
    save_trace(state)`}</code>
        ...
        <p>
          요청 한 건 안에서 짧게 끝나고 process가 죽어도 처음부터 안전하게 다시 실행할 수 있다면 이 직접 구현이 더 이해하고 시험하기 쉽습니다. Framework를 쓰지 않는다고
