...
import BertPretrainDetailViz from './viz/BertPretrainDetailViz';
  ...
  return (
    <section id="pre-training" className="mb-16 scroll-mt-20">
      ...
      <div className="prose prose-neutral dark:prose-invert max-w-none mt-6">
        <h3 className="text-xl font-semibold mt-6 mb-3">MLM 손실 함수</h3>
        <M display>
          {`\\mathcal{L}_{\\text{MLM}} = -\\sum_{i \\in \\text{masked}} \\log \\underbrace{P(x_i \\mid x_{\\backslash i})}_{\\text{softmax}(W \\cdot h_i + b)[x_i]}`}
        </M>
        <h3 className="text-xl font-semibold mt-6 mb-3">전체 손실 함수</h3>
        <M display>
          {`\\mathcal{L}_{\\text{total}} = \\underbrace{\\mathcal{L}_{\\text{MLM}}}_{\\text{토큰 수준}} + \\underbrace{\\mathcal{L}_{\\text{NSP}}}_{\\text{문장 수준}} \\quad \\text{where } \\mathcal{L}_{\\text{NSP}} = -\\log P(y \\mid h_{\\text{[CLS]}})`}
        </M>
      </div>
      <BertPretrainDetailViz />
      <div className="prose prose-neutral dark:prose-invert max-w-none mt-6">
        <p className="leading-7">
