    ...
    <div id="long-context-scope" className="mb-16 scroll-mt-20">
      ...
      <div className="prose prose-neutral dark:prose-invert max-w-none">
        <p className="text-lg leading-8">
          Long-context modeling 이라는 말은 한 문제가 아니라 위치 확장, 메모리, 평가라는
          서로 다른 세 문제를 함께 가리킵니다. YaRN을 포함한 RoPE 계열 방법은 이 중 위치
          확장만 풀고, 나머지 둘은 다른 곳에서 따로 해결해야 합니다.
        </p>
        <p>
          먼저 두 길이를 구분합니다. Checkpoint가 실제로 학습에서 본 최대 길이를 native
          context length(L), 그보다 밀어붙이려는 목표 길이를 extended context length(L′)라고
          부릅니다. 배율 s=L′/L이 확장 정도를 나타냅니다.
        </p>
        <p>
      ...
      <div className="prose prose-neutral dark:prose-invert max-w-none">
        ...
        <p>
          이 dimension을 조정 없이 그대로 32768까지 늘리면 마지막 위치의 각도는 216.8도가
          됩니다. 모델이 학습 동안 실제로 본 범위는 0도에서 27도뿐이므로, 216.8도는 한 번도
          본 적 없는 위상입니다.
        </p>
        <p>
          Context length extrapolation의 실패는 이 낯선 위상에서 생깁니다. 같은 4096 token
          동안 가장 빠른 dimension(i=0)은 이미 652바퀴를 돌아, 위상이 0도에서 360도까지
          고르게 반복됩니다.
        </p>
        <p>
          이미 여러 번 돈 dimension은 더 먼 위치로 늘려도 낯선 각도가 나오지 않습니다. 위치
          확장 문제는 결국 이 낯선 각도를 다루는 문제이고, 다음 절부터 PI·NTK-aware·YaRN이
          각자 다른 방식으로 답합니다.
        </p>

        <h3>메모리와 평가는 위치 확장과 다른 문제입니다</h3>
        <p>
          메모리 문제는 다릅니다. Attention 계산량은 길이의 제곱에 가깝게, KV cache는 길이에
          비례해 커지므로 위치를 늘려도 이 비용은 그대로 남습니다. RoPE scaling은 위치를
          다루는 기술이지 이 비용을 줄이는 기술은 아닙니다.
        </p>
        <p>
