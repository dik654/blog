  ...
  return (
    <div id="overview" className="space-y-16">
      ...
      <section id="layout-and-order" className="scroll-mt-20">
        ...
        <div className="prose prose-neutral max-w-none dark:prose-invert">
          <p>
            Layout parsing은 문서를 title·paragraph·table 같은 영역으로 나누는 작업이고 reading order detection은 그 영역들을 사람이 실제로
            읽는 순서로 정렬하는 작업입니다. 이 둘을 합쳐 문서의 전체 구조를 뽑아내는 과정을 document structure extraction이라고 부릅니다.
          </p>
          <p>
            PDF는 텍스트를 저장한 순서가 화면에 보이는 순서와 다를 수 있습니다. 2단 컬럼 페이지에서 왼쪽 컬럼 문단을 채 읽기 전에 오른쪽 컬럼으로 건너뛰면 왼쪽 마지막 문장과
            오른쪽 첫 문장이 그대로 이어 붙어 전혀 다른 문장이 만들어집니다.
          </p>
          <p>
            PyMuPDF는 get_text(sort=True) 옵션으로 좌상단에서 우하단 순서로 텍스트를 재정렬하거나 blocks·dict 형식으로 각 텍스트 블록의 좌표를 받아
            reading order를 직접 계산하는 방법을 제공합니다. 좌표만으로 순서를 복원하면 이미지·표 영역이 섞인 페이지에서는 여전히 어긋날 수 있습니다.
          </p>
          <p>
            PDF parsing은 페이지 안 문자·선·이미지의 좌표를 읽어 냅니다. HTML parsing은 태그 구조를 DOM tree, 즉 요소를 부모·자식 관계로 나타낸 트리로
            바꿉니다. DOM tree는 태그 자체가 계층을 담고 있어 PDF보다 순서 복원이 쉽지만 rowspan·colspan 같은 표 셀 구조는 HTML에서도 따로 해석해야 합니다.
          </p>
          <p>
      ...
      <section id="ocr" className="scroll-mt-20">
        ...
        <div className="prose prose-neutral max-w-none dark:prose-invert">
          ...
          <p>
            스캔된 표 이미지를 일반 OCR로 그대로 읽으면 셀마다 있던 숫자가 한 줄로 이어져 "2024 Q1 120 Q2 150"처럼 어느 숫자가 어느 열에 속했는지 문자열만으로는 알
            수 없습니다.
          </p>
          <p>
            Layout-aware OCR은 먼저 layout parsing으로 영역을 나눕니다. 그 bounding box 좌표를 유지한 채 문자만 인식해 인식된 텍스트를 원래 영역·셀에
            좌표로 이어 붙입니다. Unstructured 라이브러리의 hi_res 전략이 이 순서로 동작하고 ocr_only 전략은 layout 분석 없이 이미지 전체를 텍스트로만
            읽습니다.
          </p>
          <p>
        ...
        >
          <p>
            auto 전략은 PDF에 이미 텍스트 layer가 있으면 OCR을 건너뛰고 스캔 이미지처럼 텍스트 layer가 없을 때만 자동으로 OCR 경로로 전환합니다. 문서 대부분이
            born-digital PDF라면 auto가 불필요한 OCR 호출을 줄여 줍니다.
          </p>
        </ProgressiveDetail>
      </section>
      ...
      <section id="table-structure" className="scroll-mt-20">
        ...
        <div className="prose prose-neutral max-w-none dark:prose-invert">
          <p>
            Table extraction은 문서에서 표가 있는 영역을 찾습니다. Table structure recognition은 그 영역 안에서 행·열과 병합
            셀(rowspan·colspan), 헤더 위치까지 복원합니다. 영역만 찾고 구조를 복원하지 않으면 표는 그냥 줄글 텍스트 덩어리가 됩니다.
          </p>
          <p>
            Microsoft의 Table Transformer는 표 100만 개로 만든 PubTables-1M 데이터셋에서 구조를 인식합니다. 셀 위치만 맞히는 GriTS_Top 점수는
            0.9849로 거의 완벽하지만 셀 내용까지 정확히 일치해야 하는 AccCon 점수는 0.8243으로 떨어집니다. 병합 셀·헤더가 섞인 표는 위치를 맞혀도 내용까지 맞히기가
            그만큼 더 어렵습니다.
          </p>
        </div>
        ...
        <div className="prose prose-neutral max-w-none dark:prose-invert">
          <p>
            ...
            colspan=2인 "2024" 두 셀뿐이고 2행은 빈칸·"Q1"·"Q2" 세 셀이며 데이터 행은 "매출·120·150" 세 칸이라고 합시다.
          </p>
          <p>
            colspan을 무시하고 셀 개수 그대로 두면 1행은 2칸, 2행과 데이터 행은 3칸으로 열 개수 자체가 어긋납니다. "2024"를 colspan만큼 두 칸으로 펼쳐야 1행도
            3칸이 되어 "2024"가 "Q1"·"Q2" 두 칸 모두와 짝지어지고 그제서야 "매출" 행의 120·150이 각각 맞는 열에 놓입니다.
          </p>
          <p>
            Multi-level header는 이렇게 표 위쪽에 여러 행으로 쌓인 헤더 구조를 가리킵니다. 이를 하나의 flat header로 합칠 때는 각 헤더 셀을 colspan만큼
            실제 열 개수로 펼칩니다. 그다음 같은 열에 속한 여러 행의 헤더 텍스트를 위에서 아래 순서로 이어 붙여 "2024 Q1"처럼 하나의 열 이름을 만듭니다.
          </p>
        </div>
        <TermBreakdown
      ...
      <section id="table-normalization" className="scroll-mt-20">
        ...
        <div className="prose prose-neutral max-w-none dark:prose-invert">
          ...
          <p>
            표를 LLM이 읽게 만드는 linearization은 크게 둘로 나뉩니다. Markdown 표 방식은 "| 지표 | 2024 Q1 | 2024 Q2 |"처럼 열 그대로의 표
            모양을 유지해 열 사이 관계가 한눈에 보이고 row-level 문장 방식은 행마다 열 이름을 반복해 행 하나만 떼어내도 뜻이 통합니다.
          </p>
          <p>
            Markdown 표는 헤더가 맨 위 한 줄뿐이라 표가 chunk 경계에서 잘리면 뒤쪽 행은 헤더 없이 숫자만 남습니다. Row-level 문장은 행마다 열 이름을 반복해 어느
            chunk에 들어가도 뜻을 유지하지만 같은 이름을 행마다 반복해 전체 크기는 markdown보다 커집니다.
          </p>
        </div>
        <AlgorithmBlock
      ...
      <section id="metadata-and-chunking" className="scroll-mt-20">
        ...
        <div className="prose prose-neutral max-w-none dark:prose-invert">
          ...
          <p>
            Document provenance는 추출된 표 값을 원본 문서의 정확한 위치에 잇는 계약입니다. 페이지·bounding box·파일 버전까지 이어야 답을 검증할 수 있습니다.
            Unstructured 라이브러리는 표를 chunk로 나눌 때 원본 요소를 metadata.orig_elements 필드에 남겨 나중에 페이지 번호와 좌표를 다시 조회할 수
            있게 합니다.
          </p>
          <p>
            Structure-preserving chunking은 구조가 있는 블록을 다르게 다룹니다. 표나 다단 레이아웃을 일반 chunking 규칙(고정 길이·문장 경계)으로 자르지
            않고 표 전체나 최소한 행 단위로 나눕니다. Unstructured의 chunking 전략은 표 요소를 다른 요소와 절대 합치지 않고 최대 크기를 넘으면 표 전용
            TableChunk로만 나눕니다.
          </p>
          <p>
            고정 길이 512 token 규칙을 표에 그대로 적용하면 헤더 행은 앞 chunk에 남고 데이터 행 절반은 다음 chunk로 넘어갑니다. 뒤 chunk만 읽는 검색에서는 숫자가
            무슨 열인지 알 수 없습니다. Structure-preserving chunking은 표를 통째로 한 chunk에 두거나 쪼개더라도 row-level
            representation처럼 행마다 헤더를 반복해 조각마다 뜻을 지킵니다.
          </p>
          <p>
