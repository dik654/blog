  ...
  return (
    <section id="overview" className="mb-16 scroll-mt-20">
      ...
      <div className="prose prose-neutral dark:prose-invert max-w-none">
        <p>
          기본 권한 정책만으로 모든 팀의 요구를 미리 알 수는 없습니다. 어떤 팀은 배포 명령 전에 티켓 번호를 확인해야 하고 다른 팀은 파일 변경 결과를 감사 시스템에 보내야 합니다.
          Hook은 코어 런타임을 수정하지 않고 이런 검증과 자동화를 특정 이벤트 전후에 연결하는 확장 지점입니다.
        </p>
        <p>
          다만 hook 자체도 외부 코드를 실행하므로 안전장치로만 볼 수는 없습니다. 신뢰하지 않은 스크립트에 tool input이나 환경 변수를 넘기면 새로운 유출 경로가 생깁니다.
          hook의 권한·timeout·출력 스키마도 본 실행과 같은 수준으로 관리합니다.
        </p>
      </div>
      ...
      <div className="prose prose-neutral dark:prose-invert max-w-none">
        ...
        <p>
          모든 이벤트에서 모든 hook을 실행하면 지연이 늘고 예상하지 못한 상호작용도 생깁니다. 도구 이름, 명령 패턴, 이벤트 종류처럼 명시적인 matcher로 적용 범위를 좁혀
          둡니다. 여러 hook이 함께 실행될 때는 순서와 실패 정책도 계약에 포함합니다. 앞선 hook의 거부를 뒤의 hook이 다시 허용하지 못하게 하는 편이 안전합니다.
        </p>
