    ...
    <section id="checkpoint" className="mb-16 scroll-mt-20">
      ...
      <div className="prose prose-neutral dark:prose-invert max-w-none">
        ...
        <p>
          ...
          이 파일은 validation으로 고른 <strong>후보 checkpoint</strong>다. 오버피팅 이전의 선택 상태를 보존하지만 아직 최종 성능 증거는 아니다.
        </p>
        <p>
          <strong>last_model.pt</strong> — 매 epoch 끝에 덮어쓰기<br />
          학습이 중단되었을 때(서버 크래시, 타임아웃) 이 파일에서 이어서 학습<br />
          두 파일을 분리한다. best는 평가할 후보, last는 resume용이며, 모든 선택을 끝낸 뒤 손대지 않은 test set으로 best를 한 번 평가한다.
        </p>
        ...
        <p>
          ...
          실험에서 사용하는 source를 모두 명시적으로 seed해야 변동 원인을 줄일 수 있다. Seed 고정은 필요조건이지만, release와 hardware를 넘는 동일 결과의 충분조건은 아니다.
        </p>
        <p>
          ...
          결정론적 연산은 더 느리거나 지원되지 않을 수 있다. 정확한 비용은 workload마다 측정해야 하며, 이 flag만으로 다른 release·hardware 사이의 완전 재현성이 보장되지는 않는다.
        </p>
      </div>
      ...
      <div className="bg-amber-50 dark:bg-amber-950/30 border-l-4 border-amber-400 p-4 my-6 rounded-r-lg">
        ...
        <p className="text-sm text-amber-700 dark:text-amber-300">
          ...
          worker_init_fn과 generator를 함께 관리해 각 워커의 seed 흐름을 통제한다. 그래도 library·driver·hardware 차이까지 포함한 완전 재현은 별도 환경 고정이 필요하다.
        </p>
      </div>
    </section>
  );
