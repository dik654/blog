    ...
    <section id="basic-tags" className="mb-16 scroll-mt-20">
      ...
      <div className="prose prose-neutral dark:prose-invert max-w-none">
        <p>
          ...
          text나 다른 element가 그 상자의 내용입니다. XML document 전체를
          parser로 읽으려면 가장 바깥에 하나의 root element가 있어야 하므로,
          여러 블록은 <code>&lt;request&gt;...&lt;/request&gt;</code> 아래에 둡니다.
        </p>
        <p>
          ...
          두 element가 교차하므로 parser가 거부합니다. Start/end tag의 대소문자도
          같아야 하고, attribute value는 <code>id=&quot;doc-1&quot;</code>처럼 quote로
          감쌉니다. 이 최소 조건을 만족하면 <strong>well-formed</strong> XML이라고
          부릅니다.
        </p>
      </div>
      ...
      <div className="prose prose-neutral dark:prose-invert max-w-none">
        ...
        <p>
          ...
          <code>&lt;output_requirements&gt;</code>처럼 이름만 보고 책임을 알 수 있게
          합니다. <code>&lt;data1&gt;</code>과 <code>&lt;data2&gt;</code>는 순서를
          바꾸는 순간 의미가 흐려지지만, 역할 이름은 순서와 독립적으로 읽힙니다.
        </p>
        <p>
  ...
  <output_requirements>주장마다 document id를 인용한다.</output_requirements>
      ...
      <div className="prose prose-neutral dark:prose-invert max-w-none">
        ...
        <p>
          ...
          XML character data에서 <code>R&amp;amp;D &amp;lt; 5</code>로
          serialize해야 parser가 다시 원문 <code>R&amp;D &lt; 5</code>로
          복원합니다. Attribute에 double quote가 포함되면
          <code>&amp;quot;</code>로 바꾸고, single quote attribute라면 필요한 경우
          <code>&amp;apos;</code>를 사용합니다.
        </p>
        <p>
          이 치환을 손으로 여러 번 적용하면 double escaping이나 누락이 생기기
          쉬우므로 XML library의 element builder와 serializer를 사용합니다.
          CDATA는 긴 text를 편하게 넣을 때 쓸 수 있지만 종료 표식
          <code>]]&gt;</code>를 그대로 담을 수 없고, parser의 권한이나 prompt
          injection을 막는 security boundary도 아닙니다. Data가 markup 구조를
          바꾸지 못하게 serialize하는 것과 model이 내용의 지시를 따르지 않게
          통제하는 것은 별개의 문제입니다.
        </p>
