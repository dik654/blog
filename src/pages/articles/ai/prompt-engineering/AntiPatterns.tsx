  ...
  return (
    <section id="anti-patterns" className="mb-16 scroll-mt-20">
      ...
      <div className="prose prose-neutral dark:prose-invert max-w-none">
        <p>
          긴 prompt가 항상 나쁜 것은 아닙니다. 실제 문제는 다른 데 있습니다. objective가 모호하거나 우선순위가 충돌하는 상태, 같은 규칙이 다른 표현으로 반복되는 상태,
          evidence와 instruction이 섞이고 완료·거부 조건이 빠진 상태입니다. “전문가처럼 행동하라”는 role 문장도 관점을 좁힐 수는 있지만 model에 없는 지식이나
          permission을 만들지는 못합니다.
        </p>
        <p>
          부정형 지시를 모두 없애야 한다는 규칙도 과합니다. 보안·정책 경계에는 명시적 금지가 필요하지만 금지 뒤에 허용되는 대안과 typed refusal을 함께 적어 다음 행동을 분명히
          해야 합니다. 예를 들어 PII를 출력하지 말라는 규칙과 함께 aggregate만 반환하고 원문이 필요하면 승인을 요청하게 합니다.
        </p>
      </div>
      ...
      <div className="prose prose-neutral dark:prose-invert max-w-none">
        ...
        <p>
          Vague instruction은 objective와 completion criteria를 고칩니다. 근거 부족에는 RAG나 tool을 붙이고 syntax 실패에는 schema와
          constrained decoding을 적용합니다. 권한 오류나 data exfiltration 위험은 prompt 문장을 늘리는 대신 runtime policy에서 차단해야
          합니다. 원인이 다른데 prompt만 계속 길게 만들면 context pollution과 충돌이 늘어납니다.
        </p>
      </div>
      ...
      <div className="prose prose-neutral dark:prose-invert max-w-none">
        ...
        <p>
          같은 의미를 가진 지시를 다른 문장으로 바꿔 썼을 뿐인데 완료율이나 형식 준수율이 달라집니다. 이것이 prompt sensitivity입니다. 예를 들어 “표를 만들어라”와 “표
          형식으로 정리해라”가 같은 model에서 성공률 92%와 71%로 갈릴 수 있습니다.
        </p>
        <p>
          Prompt robustness는 이 변동을 줄이는 설계 목표입니다. 같은 요청을 표현만 바꾼 문장 서너 개로 evaluation set을 만들어 평균과 최저 성공률을 함께
          보고하거나 여러 표현의 출력을 실제로 ensemble해 다수결로 답을 고르는 방식으로 확보합니다.
        </p>
        <p>
      ...
      <div className="prose prose-neutral dark:prose-invert max-w-none">
        ...
        <p>
          새 model로 옮길 때는 같은 regression suite와 production canary를 통과한 뒤 traffic을 늘립니다. quality나 constraint
          violation, p95 latency, token cost 중 하나라도 guardrail을 넘으면 rollback합니다.
        </p>
        <p>
          이 과정을 거치면 prompt engineering은 input contract와 evaluator를 함께 관리하는 engineering loop가 됩니다. 모델과 말씨를 맞추는
          개인 기술에 머물지 않습니다. 단 한 번의 좋은 demo는 아이디어를 보여 줄 뿐입니다. 완료 판정은 실패 case가 포함된 반복 가능한 evaluation이 맡아야 합니다.
        </p>
      </div>
    </section>
  );
