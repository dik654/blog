...
export default function ReconstructionAnomalyArticle() {
  return <div className="space-y-16">
    <section id="overview" className="scroll-mt-20">
      ...
      <div className="prose prose-neutral max-w-none dark:prose-invert"><p className="text-lg leading-8">정상 data로 학습한 autoencoder가 정상 패턴은 잘 복원하고 낯선 패턴은 크게 틀릴 것이라는 가정을 사용합니다. 먼저 input과 reconstruction의 거리를 한 sample의 score로 만들고, 판정은 다음 단계에서 별도로 calibration합니다.</p></div>
    ...
    <section id="threshold" className="scroll-mt-20">
      ...
      <ExplainedFormula question="Sample score를 어떻게 anomaly decision으로 바꿀까요?" idea={<p>Coordinate residual을 평균해 score를 만들고, validation에서 선택한 τ보다 클 때만 alert합니다. 두 줄은 model score와 운영 decision이 다른 소유자임을 보여 줍니다.</p>} formula={String.raw`s(x)=\frac1n\lVert x-g_\phi(f_\theta(x))\rVert_2^2,\quad \hat y=\mathbf1[s(x)>\tau]`} annotatedFormula={String.raw`\begin{aligned}s(x)&=\underbrace{\frac1n\sum_{j=1}^{n}(x_j-\hat x_j)^2}_{\text{sample의 복원 오차를 평균}}\\a(x)&=\underbrace{[s(x)>\tau]}_{\text{validation threshold 초과 여부}}\\\hat y(x)&=\underbrace{\mathbf1[a(x)]}_{\text{참일 때 anomaly alert 생성}}\end{aligned}`} operations={[
    ...
    <section id="release" className="scroll-mt-20">
      ...
      <div className="prose prose-neutral max-w-none dark:prose-invert"><p>Checkpoint·feature schema·scaler·missing policy·score reduction·validation window·threshold·precision/recall·false alarms/day·drift trigger를 서로 다른 줄로 기록합니다. Threshold를 바꾸면 model weight가 같아도 새 decision version입니다.</p></div>
