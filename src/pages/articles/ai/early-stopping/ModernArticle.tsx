...
export default function EarlyStoppingArticle() {
  return <div className="space-y-16">
    ...
    <section id="state-machine" className="scroll-mt-20">
      ...
      <ExplainedFormula question="각 validation event에서 state는 어떻게 바뀌나요?" idea={<p>
            loss 최소화 기준에서는 새 metric이 best에서 δ보다 더 내려가야 개선입니다. 개선이면 best를 저장하고 counter를 0으로 돌립니다. 아니면 counter만
            1 올립니다.
          </p>} formula={String.raw`I_j=[m_j<b_{j-1}-\delta]`} annotatedFormula={String.raw`\begin{aligned}I_j&=\underbrace{\mathbf 1[m_j<b_{j-1}-\delta]}_{\text{noise를 넘는 실제 개선인지 판정}}\\b_j&=\begin{cases}\underbrace{m_j}_{\text{새 best를 기록}},&I_j=1\\\underbrace{b_{j-1}}_{\text{이전 best 유지}},&I_j=0\end{cases}\\c_j&=\begin{cases}\underbrace{0}_{\text{개선했으므로 기다림 초기화}},&I_j=1\\\underbrace{c_{j-1}+1}_{\text{개선 없는 event를 누적}},&I_j=0\end{cases}\end{aligned}`} operations={[
    ...
    <section id="stop-and-restore" className="scroll-mt-20">
      ...
      <ExplainedFormula question="언제 멈추고 어느 checkpoint를 반환할까요?" idea={<p>
            counter가 patience를 초과하면 현재 update를 더 진행하지 않습니다. 반환 index는 metric이 가장 좋았던 evaluation을 가리킵니다. stop
            index가 아닙니다.
          </p>} formula={String.raw`j_{\mathrm{stop}}=\min\{j:c_j>P\},\quad j^*=\arg\min_k m_k`} annotatedFormula={String.raw`\begin{aligned}j_{\mathrm{stop}}&=\underbrace{\min\{j:c_j>P\}}_{\text{허용 비개선 횟수를 처음 넘은 event}}\\j^*&=\underbrace{\arg\min_{k\le j_{\mathrm{stop}}}m_k}_{\text{stop 이전 metric이 가장 좋았던 artifact}}\\\theta_{\mathrm{return}}&=\underbrace{\theta_{j^*}}_{\text{last가 아니라 immutable best를 복원}}\end{aligned}`} operations={[
