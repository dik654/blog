...
export default function Overview() {
  return <section id="overview" className="scroll-mt-20">
    ...
    <div className="prose prose-neutral max-w-none dark:prose-invert"><p className="text-lg leading-8">같은 자료를 읽고 같은 도구를 쓰며 하나의 결과만 만드는 작업이라면 agent를 늘려도 호출·조정·검증 비용만 커질 수 있습니다. 반면 서로 독립적인 repository를 조사하거나, 작성자와 검증자가 다른 context·권한을 가져야 하거나, 여러 설비의 분석 artifact를 병렬로 만들 때는 분리가 유용합니다.</p><p><a href="/ai/agent-delegation-contracts">Agent delegation 정본</a>은 delegation의 개념과 manager·handoff·parallel·actor–reviewer 선택을 설명합니다. 이 글은 그 결정을 실제 runtime의 state schema·worker contract·join·retry·side-effect safety로 구현하는 범위만 소유합니다.</p></div>
