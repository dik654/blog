  ...
  return (
    <section id="overview" className="mb-16 scroll-mt-20">
      ...
      <div className="prose prose-neutral dark:prose-invert max-w-none">
        ...
        <p>
          만능 근사 정리(universal approximation theorem)에 따르면 은닉층이 한 개인 신경망도 뉴런을 충분히 많이 두면 넓은 범위의 연속 함수를 근사할 수
          있습니다. 다만 주어진 함수에 필요한 뉴런 수, 유한한 데이터로 그 파라미터를 찾을 수 있는지, 처음 보는 데이터에서도 잘 작동하는지까지 이 정리가 보장하지는 않습니다.
        </p>

        <h3>깊이는 중간 계산을 재사용하는 방법이다</h3>
        <p>
          깊은 네트워크는 작은 함수를 여러 단계로 합성합니다. 앞 층에서 만든 특징을 뒤 층이 다시 사용하므로 반복되는 compositional structure를 한 층에 모두 펼쳐
          표현할 때보다 훨씬 작은 폭으로 나타낼 수 있는 함수족이 있습니다. 이미지·언어처럼 부분 구조가 반복해서 결합되는 데이터에서 깊이가 특히 자연스러운 이유입니다.
        </p>
      ...
      <div className="prose prose-neutral dark:prose-invert max-w-none">
        ...
        <p>
          깊이는 표현의 가능성을 늘리지만 gradient가 지나야 할 경로도 길게 만듭니다. 초기화가 좋지 않거나 activation이 포화되면 앞 층까지 학습 신호가 거의 도달하지 않을
          수 있고 training loss조차 더 얕은 모델보다 높아지는 optimization degradation도 생깁니다. Residual connection,
      ...
      >
        ...
        <p className="mt-2 text-sm leading-6 text-muted-foreground">
          Telgarsky는 ReLU 등을 포함하는 semi-algebraic gate 조건에서 깊은 network가 작은 규모로 표현하는 특정 함수를 훨씬 얕은 network가 근사하려면
          width가 지수적으로 커지는 경우를 구성했습니다. 깊이가 별도의 표현 자원이라는 근거입니다. 현실의 모든 dataset에서 더 깊은 model이 자동으로 잘 학습되거나
          정확해진다는 실험 결론까지는 아닙니다.
        </p>
      </div>
      ...
      >
        ...
        <p className="mt-2 text-sm leading-6 text-muted-foreground">
          LeCun·Bengio·Hinton은 2015년 리뷰에서 vision·speech·language의 성과를 여러 층의 abstraction을 학습하고 출력 오차로 각 층의
          internal parameter를 바꾸는 흐름으로 묶었습니다. 다만 이 글은 당시까지의 연구를 종합한 review이므로 특정 architecture의 보편적 우월성을 검증한 단일
          통제 실험으로 읽으면 안 됩니다.
        </p>
      </div>
    </section>
