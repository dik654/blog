export default function MissingViz() {
  ...
  return (
    <div data-viz className="rounded-2xl border border-border/70 bg-card/70 p-4 sm:p-6">
      <p className="text-xs font-semibold uppercase tracking-[0.16em] text-amber-600 dark:text-amber-300">Missingness workflow</p>
      <h3 className="mt-1 text-lg font-semibold text-foreground">비율 → 패턴 → 수집 원인 → 처리 실험</h3>
      <div className="mt-5 grid gap-3 md:grid-cols-3">
        {mechanisms.map(([name, meaning, action]) => (
          <article key={name} className="rounded-xl border border-amber-500/25 bg-amber-500/[0.06] p-4">
            <p className="font-mono text-sm font-bold text-amber-800 dark:text-amber-200">{name}</p>
      ...
      <div className="mt-4 rounded-xl border border-border/70 p-4">
        ...
        <div className="mt-3 grid gap-2 text-sm text-muted-foreground sm:grid-cols-4">
          {["NaN 유지", "단순 대체", "모델 기반 대체", "missing indicator"].map((item) => (
            <span key={item} className="rounded-lg bg-muted/70 px-3 py-2 text-center">{item}</span>
          ))}
        </div>
      </div>
    </div>
