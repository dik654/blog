export default function CorrelationViz() {
  ...
  return (
    <div data-viz data-viz-canvas className="min-w-0 rounded-lg border border-border/70 bg-background p-4 sm:p-6">
      <p className="text-xs font-semibold uppercase tracking-[0.16em] text-primary">Association, not causation</p>
      <h3 className="mt-1 text-lg font-semibold text-foreground">하나의 상관계수로 관계를 확정하지 않습니다</h3>
      <div className="mt-5 min-w-0 overflow-hidden rounded-lg border border-border/70">
      ...
      <div className="mt-4 grid gap-2 sm:grid-cols-3">
        {["scatter로 모양 확인", "split마다 재계산", "누출 · 공통 원인 점검"].map((item) => (
          <p key={item} className="rounded border border-border/70 px-3 py-2 text-center text-sm text-muted-foreground">{item}</p>
        ))}
      </div>
    </div>
  );
