  ...
  return (
    <section id="distribution" className="mb-16 scroll-mt-20">
      ...
      <div className="prose prose-neutral dark:prose-invert max-w-none">
        <p>
          타깃과 주요 피처의 분포를 보는 목적은 정규분포로 만드는 데 있지 않습니다. 값이 어느 범위에 몰려 있는지, 서로 다른 집단이 섞였는지, 측정 한계나 반올림 흔적이 있는지를 확인해
          데이터가 만들어진 과정부터 이해해야 합니다. 회귀라면 타깃의 긴 꼬리가 손실에 어떤 영향을 주는지 보고 분류라면 class 비율과 label이 시간·source별로 달라지는지
          확인합니다.
        </p>
        <p>
...
\end{aligned}`}
      ...
      <div className="prose prose-neutral dark:prose-invert max-w-none">
        ...
        <p>
          IQR이나 z-score는 확인할 행을 좁히는 진단 도구입니다. 삭제 규칙으로 쓰면 안 됩니다. 단위 오류, 센서 고장, 중복 집계처럼 데이터 계약을 위반한 값은 수정하거나 제외할
          수 있지만 실제로 발생한 큰 지연이나 고액 거래는 예측해야 할 신호일 수 있습니다. 이상값을 처리한 실험은 어떤 규칙으로 몇 행이 바뀌었는지 기록하고 처리 전후 성능을 같은
          validation split에서 비교합니다.
        </p>
        <p>
          트리 모델도 극단값에 완전히 무관하지는 않습니다. 순서 기반 split이라 선형 모델보다 민감도가 낮을 수 있지만 잘못된 값이 분할 후보와 leaf 통계를 바꿀 수 있습니다. 모델
          종류로 결론을 대신하지 말고 오류 여부와 validation 결과로 판단해야 합니다.
        </p>
      </div>
    </section>
  );
