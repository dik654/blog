    ...
    <section id="applications" className="mb-16 scroll-mt-20">
      ...
      <div className="prose prose-neutral dark:prose-invert max-w-none">
        <p className="leading-7">
            Random split이나 train fitted value로는 미래 성능을 판단할 수 없다. Forecast origin을 앞으로 이동시키며 매번 그 이전 데이터로
            preprocessing과 model을 다시 fit한 뒤 실제 운영 horizon을 예측한다. 한 번의 holdout은 우연히 쉬운 계절이나 구조 변화에 걸릴 수 있으므로 여러
            origin과 여러 series에서 error 분포를 본다.
          </p>
      ...
      <div id="paper-forecast-evaluation" className="not-prose my-8 scroll-mt-24 border-l border-primary/50 pl-4">
        ...
        <p className="mt-2 text-sm leading-6 text-muted-foreground">
          Tashman은 forecast origin·lead time·estimation window와 recalibration 주기를 분리해 실제 예측 순서를 재현하는 평가 설계를
          정리했습니다. 운영 때의 refit·horizon 조건을 backtest에 그대로 명시해야 accuracy estimate를 해석한다는 근거입니다. 특정 rolling scheme
          하나가 모든 series에 최선이라는 주장까지 담고 있지는 않습니다.
        </p>
      ...
      <div id="paper-mase" className="not-prose my-8 scroll-mt-24 border-l border-primary/50 pl-4">
        ...
        <p className="mt-2 text-sm leading-6 text-muted-foreground">
          Hyndman과 Koehler는 scale·zero·out-of-sample 비교에서 기존 percentage metric이 보이는 문제를 정리한 뒤 train naive
          error로 absolute error를 나누는 MASE를 제안했습니다. 이 지표는 분모가 0이 아닌 같은 series·seasonality·training fold에서 비교한다는
          전제 위에 서 있습니다. horizon별 위험·bias·interval calibration은 MASE 하나가 감당하는 범위 밖입니다.
        </p>
      ...
      <div className="prose prose-neutral dark:prose-invert max-w-none">
        ...
        <p>
            LSTM은 관측이 한 점씩 들어올 때 state를 순차 갱신하거나 여러 series가 공유하는 비선형 동역학을 학습할 때 쓸모가 있다. 긴 sequence의 병렬 학습에는
        ...
        <p>
            ...
            canary로 배포한다. error·coverage·state-reset 위반이 기준을 넘으면 이전 model과 transform artifact로 rollback한다.
          </p>
      </div>
      <div id="paper-dlinear" className="not-prose my-8 scroll-mt-24 border-l border-primary/50 pl-4">
        ...
        <p className="mt-2 text-sm leading-6 text-muted-foreground">
          Zeng 등이 만든 DLinear는 입력 trend와 remainder에 단순 linear projection을 적용합니다. 이 model을 같은 장기 예측 benchmark에서
          비교하면서 복잡한 architecture의 우위가 평가 protocol에 민감하다는 문제를 제기했습니다. 다만 이 결과가 성립하는 범위는 논문의 dataset·look-
          back·horizon·normalization 조건까지입니다. 모든 시계열과 운영 조건으로 넓히면 linear model이 LSTM·Transformer보다 낫다는 결론까지는
          나오지 않습니다.
        </p>
      ...
      <div id="paper-patchtst" className="not-prose my-8 scroll-mt-24 border-l border-primary/50 pl-4">
        ...
        <p className="mt-2 text-sm leading-6 text-muted-foreground">
          Nie 등이 제안한 PatchTST는 연속 시점을 patch token으로 묶고 channel-independent encoder를 사용합니다. 이 구조가 긴 look-back의
          token 수와 representation을 바꿉니다. 보고된 장기 예측 성능은 논문의 multivariate benchmark·patch·normalization·training
          recipe 범위입니다. 그렇다고 모든 cross-channel interaction이 불필요해지거나 실시간 latency가 저절로 낮아지지는 않습니다.
        </p>
