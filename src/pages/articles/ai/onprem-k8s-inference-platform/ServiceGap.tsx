    ...
    <section id="service-abstraction-gap" className="scroll-mt-20">
      ...
      <div className="prose prose-neutral max-w-none dark:prose-invert">
        <p className="text-lg leading-8">
          쿠버네티스에서 같은 일을 하는 파드 여러 개 앞에 두는 것은 서비스입니다. 서비스는 준비 상태인 파드
          목록을 들고 있다가 들어온 연결을 그중 하나로 보냅니다. 고르는 기준은 연결 수준의 단순한 규칙이고,
          그 파드가 지금 무엇을 하고 있는지는 보지 않습니다.
        </p>

        <p className="leading-7">
          웹 요청에서는 이 단순함이 문제가 되지 않습니다. 요청 하나의 처리 시간이 대체로 비슷해서 요청 수를
          고르게 나누면 부하도 고르게 나뉩니다. 추론 요청은 다릅니다. 입력 길이와 출력 길이에 따라 한 요청의
          비용이 수십 배씩 차이 나므로, 요청 수를 고르게 나눠도 부하는 고르지 않습니다.
        </p>

        <p className="leading-7">
          게다가 모델 서버는 상태를 가집니다. 방금 처리한 요청의 캐시가 남아 있어서 같은 앞부분을 공유하는 다음
          요청은 그 서버에서 훨씬 싸게 끝납니다. 연결 수준에서 고르는 규칙은 이 사실을 알 수 없습니다.
        </p>

        <p className="leading-7">
          그래서 필요한 것은 더 나은 분배 알고리즘이 아니라, 분배를 결정하는 지점에 모델 서버의 지표가 들어갈
          자리입니다. 이 자리를 표준화하는 확장이 최근에 자리 잡았습니다. 모델을 서빙하는 엔드포인트 묶음을
          하나의 자원으로 선언하고, 그 묶음 안에서 어느 엔드포인트로 보낼지는 별도의 선택기가 정하게 하는
          구조입니다.
        </p>
      </div>
      ...
      <div className="prose prose-neutral mt-8 max-w-none dark:prose-invert">
        ...
        <p className="leading-7">
          반대로 이 구조가 공짜는 아닙니다. 선택기가 모든 요청의 경로에 들어가므로 그것의 지연과 가용성이 곧
          서비스의 지연과 가용성이 됩니다. 지표를 얼마나 자주 긁어 올지도 선택이 필요합니다. 너무 자주면
          모델 서버에 부담이고, 너무 드물면 이미 지난 상태를 보고 고릅니다.
        </p>
      </div>
      ...
      <div className="mt-6">
        ...
        >
          ...
          <p className="leading-7">
            다만 이 값들도 즉시 반영되지는 않습니다. 지표를 긁는 주기만큼 지난 상태이고, 그 사이에 도착한
            요청들은 아직 반영돼 있지 않습니다. 그래서 같은 순간에 여러 요청이 같은 서버로 몰리는 현상이
            남고, 구현들은 보낸 직후의 요청을 따로 세는 식으로 이를 보정합니다.
          </p>
        </ProgressiveDetail>
      </div>
