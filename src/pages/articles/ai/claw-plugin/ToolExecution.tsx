  ...
  return (
    <section id="tool-execution" className="mb-16 scroll-mt-20">
      ...
      <div className="prose prose-neutral dark:prose-invert max-w-none">
        <p>
          서브프로세스 plugin은 host가 넘긴 한 요청을 처리하고 구조화된 결과를
          돌려주는 worker로 보는 편이 안전합니다. shell command 문자열을
          조립하거나 host 환경을 통째로 물려주면 manifest에서 검증한 경계가
          사라지므로, executable과 argv를 분리하고 입력·출력 protocol을
          versioned frame으로 고정해야 합니다.
        </p>
        ...
        <p>
          plugin이 stdout에 디버그 문자열 한 줄을 쓰는 순간 JSON parser가 깨질
          수 있습니다. stdout은 길이 prefix 또는 JSONL처럼 경계가 분명한
          protocol frame에만 사용하고, 진단은 stderr로 보냅니다. 두 채널 모두
          byte 상한을 두어 무한 출력으로 host memory가 소진되지 않게 하며, UTF-8
          경계와 부분 frame을 처리하는 incremental parser가 필요합니다.
        </p>
        ...
        <p>
          현재 process environment를 복사한 뒤 몇 개만 지우는 방식은 새 secret이
          추가될 때 자동으로 노출됩니다. locale·PATH의 검증된 일부·call metadata
          등 필요한 변수만 allowlist로 만들고, canonical working directory와
          read-only mount, 임시 쓰기 디렉터리, network policy를 capability에
          맞춰 구성합니다. 외부 API가 필요하면 host의 broad credential 대신 짧게
          만료되는 delegated credential이나 host-mediated request를 사용합니다.
        </p>
        ...
        <p>
          future만 취소하면 plugin이 만든 child process가 남을 수 있습니다.
          deadline이 지나면 먼저 graceful cancellation frame이나 signal을
          보내고, 짧은 grace period 뒤에도 남아 있으면 process group 또는 job
          object 전체를 종료합니다. pipe reader와 temp file도 같은 cancellation
          scope에 묶고, 종료가 확인된 뒤에만 slot과 credential을 반환합니다.
        </p>
