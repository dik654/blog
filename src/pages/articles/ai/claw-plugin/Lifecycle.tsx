  ...
  return (
    <section id="lifecycle" className="mb-16 scroll-mt-20">
      ...
      <div className="prose prose-neutral dark:prose-invert max-w-none">
        ...
        <p>
          검증된 artifact가 Starting으로 들어가 handshake를 마치면 Ready가 되고,
          disable이나 update가 시작되면 Draining으로 이동합니다. Draining은 새
          호출을 받지 않지만 이미 시작한 호출은 자신이 가진 generation에서 끝낼
          수 있습니다. 종료 deadline을 넘기면 취소 후 Stopped로 이동하며, 모든
          전이는 actor·reason·previous generation을 감사 이벤트로 남깁니다.
        </p>
        ...
        <p>
          process가 존재한다는 사실과 새 호출을 받을 준비가 됐다는 사실은
          다릅니다. 무거운 실제 작업을 짧은 주기로 반복하는 health check는
          장애를 더 만들 수 있으므로, 각 probe가 답할 질문과 timeout, 실패
          threshold를 분리합니다. 고정된 “5분마다 검사”보다 plugin 특성과 최근
          트래픽에 맞춘 interval과 jitter가 필요합니다.
        </p>
        ...
        <p>
          일시적 network 오류나 crash는 backoff 후 재시작할 수 있지만, 잘못된
          configuration과 protocol 위반을 같은 방식으로 반복하면 restart loop가
          됩니다. 오류를 transient, configuration, integrity로 분류하고 시간 창
          안의 시도 횟수를 제한합니다. retry budget을 소진하면 circuit를 열어 새
          호출을 빠르게 실패시키고 사람이 원인을 확인할 수 있게 합니다.
        </p>
        ...
        <p>
          shutdown에서 먼저 process를 죽이면 진행 중 tool call이 어떤 상태로
          끝났는지 알 수 없습니다. registry에서 generation을 Draining으로 바꿔
          새 호출을 막고, 진행 중 call에 cancellation을 전파한 뒤 deadline까지
          기다립니다. 마지막에는 process tree, pipe, temporary directory,
          delegated credential을 회수하고 telemetry flush는 별도 짧은 deadline
          안에서 수행합니다.
        </p>
