  ...
  return (
    <div className="rounded-lg border p-6">
      <p className="text-sm text-foreground/75 mb-3">
        슬라이더로 차원(i)을 변경하면 주파수가 어떻게 변하는지 확인할 수
        있습니다.
      </p>
      ...
      <Mafs height={200} viewBox={{ x: [0, 6.3], y: [-1.2, 1.2], padding: 0 }}>
        <Coordinates.Cartesian
          xAxis={{
            lines: Math.PI,
            labels: (v) => `${(v / Math.PI).toFixed(1)}π`,
          }}
          yAxis={{ lines: 0.5 }}
        />
        <Plot.OfX
      ...
      <div className="flex justify-center gap-6 mt-3 text-xs text-foreground/75">
        <span className="flex items-center gap-1">
          <span className="inline-block w-3 h-0.5 bg-blue-500 rounded" /> sin
          (짝수 차원)
        </span>
        <span className="flex items-center gap-1">
          <span className="inline-block w-3 h-0.5 bg-pink-500 rounded" /> cos
          (홀수 차원)
        </span>
      </div>
    </div>
