    ...
    <section id="stale-branch" className="mb-16 scroll-mt-20">
      ...
      <div className="prose prose-neutral dark:prose-invert max-w-none">
        <p className="leading-7">
          오래 활동이 없는 branch는 review 대상이 될 수 있습니다. 다만 시간이 지났다는 이유만으로 가치가 사라지지는 않습니다. release branch나 보류된 실험처럼 오래
          유지할 이유가 있을 수 있습니다. 자동 rebase와 삭제는 remote history와 사용자 작업을 바꾸는 destructive action이기도 합니다.
        </p>
        <p className="leading-7">
          stale detector는 signal을 모으는 장치로 두는 편이 안전합니다. 삭제를 결정하는 쪽이 아니라 owner가 상태를 재검토하게 만드는 쪽입니다. 임계값은
          repository policy에 속하며 “7일, 100 commits, 5회 실패” 같은 snapshot 숫자를 일반 원칙으로 쓰지 않습니다.
        </p>
      ...
      <div className="prose prose-neutral dark:prose-invert max-w-none">
        ...
        <p className="leading-7">
          target보다 뒤처진 commit 수만 보면 branch 자체의 새 commit과 merge-base를 놓칩니다. ahead·behind를 모두 계산하고 open PR과
          conflict preview, protected branch 여부까지 확인합니다. behind가 크다는 이유만으로 rebase를 시작하지는 않습니다.
        </p>
        <p className="leading-7">
          automation worker가 소유한 ephemeral branch와 사람이 장기 작업 중인 branch는 policy도 나눕니다. 앞쪽은 task lease와 terminal
          state로 정리할 수 있습니다. 뒤쪽은 owner acknowledgement가 없으면 알림과 label 변경까지만 자동화합니다.
        </p>
        ...
        <p className="leading-7">
          최신 target을 fetch한 뒤 clean temporary worktree에서 rebase나 merge를 시도하면 원래 checkout의 미커밋 변경을 건드리지 않습니다.
          conflict가 나면 abort하고 conflict list와 candidate resolution을 artifact로 남긴 뒤 owner나 recovery workflow에
          전달합니다.
        </p>
        <p className="leading-7">
        ...
        <p className="leading-7">
          branch를 정리하기 전에 current SHA를 가리키는 durable archive ref를 만들고 metadata와 owner, reason을 남깁니다. remote에서 그
          ref가 실제로 보이는지도 확인합니다. archive가 성공했다고 곧바로 삭제하지는 않습니다. retention 기간과 별도 approval을 둡니다.
        </p>
        <p className="leading-7">
