    ...
    <section id="identity-metric" className="scroll-mt-20">
      ...
      <div className="prose prose-neutral max-w-none dark:prose-invert">
        <p className="text-lg leading-8">
          두 얼굴이 같은 사람인지 재는 도구는 하나처럼 보이지만 실제로는 두 모델이 이어 붙은 것입니다. 먼저 탐지기가 이미지에서 얼굴 상자와 눈·코·입 위치를 찾고 그다음 인식기가 정렬된
          얼굴을 512차원 벡터로 바꿉니다. 유사도는 그 벡터 두 개의 내적입니다.
        </p>
        ...
        <p className="leading-7">
          두 벡터를 비교하는 방식도 짚어 둘 필요가 있습니다. 인식기의 출력은 단위 길이로 정규화되어 있으므로 내적이 곧 코사인이고 값의 범위는 −1에서 1입니다. 서로 관계없는 두 사람은
          0 근처에 모이고 같은 사람은 위쪽으로 갑니다. 이 분포가 겹치는 구간에서 임계값을 골라야 합니다.
        </p>
      </div>
      ...
      <div className="prose prose-neutral max-w-none dark:prose-invert">
        <p className="leading-7">
          저는 0.28을 근거 없이 운영값으로 써 왔습니다. 어디선가 본 숫자였고 그 위에 여러 회차의 판정이 쌓였습니다. 검증 방법은 단순합니다. 명백히 서로 다른 사람들을 만들어 모든
          쌍의 유사도를 재고 그중 몇 쌍이 임계값을 넘는지 보면 됩니다.
        </p>

        <p className="leading-7">
          나이·체구·얼굴형을 전부 다르게 지정한 여섯 명을 세 가지 스타일로 생성해 쌍마다 쟀습니다. 모든 쌍이 남남 쌍이므로 넘는 쌍은 전부 오판입니다. 0.28에서는 45쌍 중 6쌍이
          샜고 0.40에서는 0쌍이었습니다.
        </p>
        ...
        <p className="leading-7">
          다만 그 0.423은 이 글이 쓰는 인식기에 대한 값입니다. 같은 연구가 다른 인식기에는 0.253을 내놓습니다.
          임계값이 모델의 성질이 아니라 그 모델에서 측정해 정하는 값이라는 앞의 주장이 여기서 한 번 더
          확인됩니다.
        </p>
