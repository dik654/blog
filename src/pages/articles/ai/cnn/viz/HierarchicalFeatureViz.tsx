...
export default function HierarchicalFeatureViz() {
  return (
    <StepViz steps={STEPS}>
      {(step) => (
        <svg viewBox="0 0 560 195" className="w-full max-w-2xl" style={{ height: 'auto' }}>
          {/* Layer headers */}
          {LAYERS.map((l, i) => {
            const x = 10 + i * 130;
          ...
          {(step === 1 || step === 4) && (
            <motion.g initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={sp}>
              <PixelGrid data={SHAPE_L} ox={144} oy={42} color="#8b5cf6" label="ㄴ 모양" />
              <PixelGrid data={SHAPE_C} ox={200} oy={42} color="#8b5cf6" label="C 곡선" />
              <text x={198} y={96} textAnchor="middle" fontSize={9} fill="#8b5cf6">
                │+─ 조합 → 도형
              </text>
            </motion.g>
          )}
          ...
          {(step === 2 || step === 4) && (
            <motion.g initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={sp}>
              <PixelGrid data={PART_EYE} ox={274} oy={40} color="#f59e0b" label="눈 패턴" cellSize={9} />
              <text x={330} y={114} textAnchor="middle" fontSize={9} fill="#f59e0b">
                곡선+원 조합 → 부분
              </text>
              <text x={330} y={124} textAnchor="middle" fontSize={8} fill="#f59e0b">
                수용야: 92×92 px
              </text>
            </motion.g>
          )}
          ...
          {step === 4 && (
            <motion.g initial={{ opacity: 0, y: 5 }} animate={{ opacity: 1, y: 0 }}
              ...
              {LAYERS.map((l, i) => (
                <g key={`b${i}`}>
                  <line x1={10 + i * 130 + 58} y1={130} x2={10 + i * 130 + 58} y2={145}
                    stroke={l.color} strokeWidth={0.8} strokeDasharray="2 2" />
                  <rect x={10 + i * 130 + 30} y={145} width={56} height={18} rx={4}
                    fill={`${l.color}20`} stroke={l.color} strokeWidth={0.8} />
                  <text x={10 + i * 130 + 58} y={157} textAnchor="middle"
