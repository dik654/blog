  ...
  return (
    <section id="interaction" className="mb-16 scroll-mt-20">
      ...
      <div className="prose prose-neutral dark:prose-invert max-w-none">
        <p>
          광고 노출 횟수의 효과가 신규 고객과 기존 고객에게 다르다면 두 피처의 영향은 단순히 더해지지 않습니다. 면적, 부채비율, 객단가처럼 곱·비율· 차이가 독립된 업무 의미를 가질
          때도 마찬가지입니다. Tree나 깊은 network가 일부 관계를 스스로 학습할 수 있지만 작은 데이터와 선형 모델에서는 그 관계를 새 좌표로 직접 주면 필요한 함수가
          단순해집니다.
        </p>
        <p>
          계산 전에 단위와 정의부터 고정합니다. 매출을 거래 수로 나눈 값은 거래가 없을 때 정의되지 않습니다. 작은 상수를 무조건 분모에 더하기보다 “거래 없음”, “금액 0”, “기록
          누락”을 구분하는 indicator와 fallback을 정합니다. 두 원본 값이 같은 cutoff에 사용 가능한지도 확인합니다.
        </p>
      </div>
...
\end{aligned}`}
      ...
      <div className="prose prose-neutral dark:prose-invert max-w-none">
        ...
        <p>
          범주형 교차와 고차 polynomial은 입력 수가 늘수록 조합 수를 빠르게 늘리고 드문 조합을 만듭니다. EDA나 domain rule에서 근거가 있는 조합부터 추가하고
          regularization과 같은 fold의 ablation으로 확인합니다. Hierarchy principle에 따라 interaction을 넣을 때 관련 main effect도
          유지하면 coefficient 해석과 model 안정성이 좋아지는 경우가 많습니다.
        </p>
        <p>
          중요도가 낮다는 이유만으로 바로 제거하지도 않습니다. 상관된 원본 피처가 대신 사용되거나 특정 subgroup에서만 효과가 있을 수 있으므로 전체 metric과 함께
          subgroup 성능과 seed별 coefficient 안정성을 봅니다.
        </p>
      </div>
    </section>
  );
