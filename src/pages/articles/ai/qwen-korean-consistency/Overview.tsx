  ...
  return (
    <section id="overview" className="mb-16 scroll-mt-20">
      ...
      <div className="prose prose-neutral max-w-none dark:prose-invert">
        ...
        <p>
          고정 사례의 “首尔”은 중국어 문자이지만 사용자가 번역을 명시했으므로 정상입니다. 반면 계산 설명 중에 이유 없이 “因此”가 끼면 문자 단위 leakage이고 연구용
          reasoning trace 전체가 중국어로 바뀌면 구간 단위 language confusion입니다. 최종 답변만 검사하면 두 번째 실패를 놓치고 문자 집합만 검사하면 정상
          번역을 오탐합니다.
        </p>
      </div>
      ...
      <div className="prose prose-neutral max-w-none dark:prose-invert">
        ...
        <p>
          다만 출력된 reasoning trace를 모델 내부 계산의 완전하고 충실한 설명으로 간주해서는 안 됩니다. 이 글에서 trace는 연구자가 학습·평가를 위해 명시한 텍스트
          필드입니다. 제품에서는 숨겨진 chain-of-thought 공개를 요구하는 대신 정답과 짧고 검증 가능한 근거를 요청하면 됩니다.
        </p>

        <h3>기반 개념은 정본 글에서 가져옵니다</h3>
        <p>
          아래 개념을 이 글에서 다시 정의하면 설명이 중복되고 수정 지점도 흩어집니다. 낯선 항목만 먼저 읽고 돌아오면 이후 절의 수식이 어느 계산을 바꾸는지 따라갈 수 있습니다.
        </p>
      </div>
      ...
      <div className="prose prose-neutral max-w-none dark:prose-invert">
        ...
        <p>
          개입 전에 문자 혼용, 구간 전환, reasoning/final 불일치, 정상 예외를 별도 label로 저장합니다. 길이·도메인·sampling 조건도 함께 기록해야 프롬프트가
          약한 것인지, 특정 입력이 다른 언어를 유도한 것인지, 모델 version이 바뀐 것인지 재현할 수 있습니다. 다음 절은 가장 값싼 입력 정책부터 시작하지만 어떤 방법도 이 관찰
          단계를 대신하지는 못합니다.
        </p>
      </div>
    </section>
