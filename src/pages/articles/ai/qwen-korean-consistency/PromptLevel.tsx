  ...
  return (
    <section id="prompt-level" className="mb-16 scroll-mt-20">
      ...
      <div className="prose prose-neutral max-w-none dark:prose-invert">
        ...
        <p>
          그래서 “한국어로만 답하라”를 여러 번 반복하기보다 기본 언어와 허용 예외, 각 출력 구간의 계약을 먼저 적습니다. 고정 사례에서는 계산 설명과 최종 답변은 한국어여야 하지만 번역
          필드의 “首尔”은 반드시 남아야 합니다. 금지 규칙만 쓰면 정답 일부를 지우는 문제가 생깁니다.
        </p>
      </div>
      ...
      <div className="prose prose-neutral max-w-none dark:prose-invert">
        ...
        <p>
          모델이 판단해야 할 경계를 한 문장에 몰아넣지 않고 어떤 필드에 어떤 언어를 쓸지 구조적으로 제시합니다. 연구에서 reasoning trace를 수집한다면 그 필드의 언어를 명시할
          수 있지만 일반 제품에서는 숨겨진 chain-of-thought 공개를 요구하지 않고 “짧고 검증 가능한 근거”를 별도 필드로 요청하는 편이 안전합니다.
        </p>
      </div>
...
코드·수식·고유명사·명시적 원문 인용은 필요한 원래 표기를 유지한다.`}
      ...
      <div className="prose prose-neutral max-w-none dark:prose-invert">
        ...
        <p>
          평가 주제는 바꾸지 않되 요구 길이와 주변 문맥을 달리하면 프롬프트의 취약한 지점을 찾기 쉽습니다. 예를 들어 같은 나눗셈 문제에 한 줄 근거와 긴 검산을 각각 요구하고 수식
          표기나 중국어 번역 예외를 포함하거나 제외한 paired slice를 만듭니다. 이때 정답 3,200원은 같으므로 언어 정책을 바꾸다가 과제 정확도가 달라졌는지도 바로 확인합니다.
        </p>
      </div>
      ...
      <div className="prose prose-neutral max-w-none dark:prose-invert">
        ...
        <p>
          프롬프트 후보를 paired evaluation으로 비교했는데도 허용 실패율을 넘는다면 원하는 대상에 따라 다음 층을 고릅니다. 특정 문자군의 과도한 생성이 반복되고 오픈웨이트가
          있다면 Smoothie-Qwen 같은 출력층 보정을, reasoning trace 전체의 언어와 문제 풀이 policy를 바꿔야 한다면 SFT와 RL을 검토합니다. 모델을 바꿀 수
          없는 서비스라면 런타임 가드로 남은 실패를 분류하되 프롬프트 실패를 무한 재시도로 숨기지는 않습니다.
        </p>
      </div>
    </section>
  );
