  ...
  return (
    <div id="overview" className="space-y-16">
      <section id="problem" className="scroll-mt-20">
        ...
        <div className="prose prose-neutral max-w-none dark:prose-invert">
          <p className="text-lg leading-8">
            Agent reliability 는 한 번의 정답률이 아니라 여러 번 실행했을 때 실패가 나더라도 그 실패를 알아채고 되돌리거나 사람에게 넘겨 전체 작업이 안전하게 끝나는
            비율입니다. 같은 “실패”라는 말 아래에는 여러 유형이 섞여 있습니다. 목표를 잊는 것, 도구를 잘못 쓰는 것, 끝났다고 착각하는 것은 원인도 복구법도 서로 다릅니다.
          </p>
          <p>
      ...
      <section id="failure-taxonomy" className="scroll-mt-20">
        ...
        <div className="prose prose-neutral max-w-none dark:prose-invert">
          <p className="text-lg leading-8">
            Goal drift 는 매 단계의 판단이 그 자체로는 그럴듯한데 그 판단들이 이전 목표를 다시 확인하지 않고 이어 붙으면서 전체 방향이 처음 목표에서 조금씩 밀리는 현상입니다.
            Context drift 에서는 방향이 그대로입니다. 앞서 확인한 제약이나 근거를 이후 단계가 다시 불러오지 못해 그 근거와 어긋나는 행동을 하게 됩니다. 목표가 바뀌는 쪽과
            목표는 그대로인데 기억이 새는 쪽이 이렇게 갈립니다.
          </p>
          <p>
            예를 들어 “지난달 매출 요약 보고서를 쓰라”는 목표로 시작한 8 단계짜리 작업에서, 3 단계까지는 요청대로 매출 수치를 정리합니다. 4 단계부터 “더 유용해 보인다”는 이유로
            원가·재고까지 분석을 넓히고 8 단계에 이르면 원래 요청에 없던 전체 재무 detail 분석을 내놓습니다. 각 단계의 확장은 국소적으로는 합리적이지만 원래 acceptance
            조건과는 멀어졌습니다.
          </p>
          <p>
            반면 context drift 는 이렇습니다. 1 단계에서 “이 서버는 read-only replica 라 write 금지”를 확인해 두고도 20 턴 뒤 context 가
            요약(compaction)되며 그 제약이 압축 대상에서 빠집니다. 그리고 15 단계째 write tool 을 호출합니다. 목표(“replica 점검”)는 바뀌지 않았고 앞서
            세운 제약을 다시 불러오지 못한 것이 원인입니다.
          </p>
        </div>
        <ExplainedFormula
        ...
        <div id="paper-agent-error-taxonomy" className="not-prose my-8 scroll-mt-24">
          ...
          <p className="mt-3 text-sm leading-7 text-muted-foreground">
            보고된 개선폭(all-correct accuracy 기준 24%p)은 이 세 환경의 자기보고 수치입니다. 이 글의 goal drift·context drift·tool
            misuse 구분은 그 다섯 층을 실행 로그에서 흔히 보이는 이름으로 재정리한 것이며 논문 자체의 범주 이름은 아닙니다.
          </p>
        </div>
      </section>
      ...
      <section id="retry-idempotent" className="scroll-mt-20">
        ...
        <div className="prose prose-neutral max-w-none dark:prose-invert">
          <p className="text-lg leading-8">
            Idempotent action 은 같은 요청을 몇 번 다시 보내도 실제 효과가 한 번 실행한 것과 똑같이 남는 action 입니다. 실패 신호를 보고 같은 action 을
            다시 시도하는 절차가 retry loop 인데 action 이 idempotent 하지 않으면 “다시 시도”가 아니라 “한 번 더 실행”이 됩니다. 부작용이 그대로 중복됩니다.
          </p>
          <p>
            결제 tool 호출을 생각해 봅니다. Agent 가 카드 청구 API 를 호출했는데 응답이 오기 전에 연결이 끊깁니다. Agent 는 이 요청이 실패했다고 판단해 같은 청구를
            다시 호출합니다. 하지만 첫 요청이 서버에서는 이미 처리돼 카드가 청구된 뒤였다면 사용자는 같은 금액을 두 번 청구받습니다. Action 자체가 idempotent 하지 않으면
            “실패로 보였다”는 판단만으로는 안전하게 재시도할 수 없습니다.
          </p>
          <p>
            이 문제를 실제로 막는 방식은 action 을 부르기 전에 클라이언트가 고유한 idempotency key 를 만들어 함께 보내는 것입니다. 서버는 그 key 로 들어온 첫
            요청의 결과(성공이든 실패든)를 저장해 두고 같은 key 로 다시 요청이 오면 실제로 다시 실행하지 않고 저장된 결과를 그대로 돌려줍니다. Retry 는 안전해지지만 key 를
            만드는 몫은 여전히 호출하는 쪽(agent 나 harness)에 있습니다.
          </p>
        </div>
        <AlgorithmBlock
      ...
      <section id="side-effect-control" className="scroll-mt-20">
        ...
        <div className="prose prose-neutral max-w-none dark:prose-invert">
          <p className="text-lg leading-8">
            Side-effect control 은 idempotent 여부보다 한 칸 앞의 문제입니다. 되돌리기 어려운 action 이 검토 없이 실행되지 않도록 실행 경로 자체에 문턱을
            둡니다. Retry 는 “이미 실행된 뒤”의 위험을 다루지만 여기서 막으려는 것은 “아직 실행되지 않은 순간”의 위험입니다.
          </p>
          <p>
            Dry-run 은 실제 side effect 를 만들지 않고 그 action 이 무엇을 바꿀지만 계산해 보여주는 실행입니다. 예를 들어 “오래된 레코드를 삭제하라”는 요청에서
            실제 DELETE 대신 영향받을 행(row) 수와 예시 몇 건을 먼저 보여주면 조건절 실수로 테이블 전체가 대상이 되는 경우를 실행 전에 알아챌 수 있습니다.
          </p>
          <p>
            Confirmation gate 는 그 dry-run 결과를 사람에게 보여주고 승인·거부·수정 중 하나를 받은 뒤에만 실제 action 을 진행하는 문턱입니다. 위험도가 낮은
            read 성격 tool 은 gate 없이 자동 실행하고 삭제·결제·외부 발송처럼 되돌릴 수 없는 tool 에만 gate 를 두면 같은 자동화 안에서 위험만 골라 낮출 수
            있습니다.
          </p>
        </div>
        <TermBreakdown
        ...
        >
          <p>
            자연어 지시 “먼저 확인을 구하라”는 model 의 판단에 기대는 방식이라 항상 지켜진다는 보장이 없습니다. 안정적인 방식은 tool 실행 경로
            자체(runtime·harness)에 gate 를 두는 것입니다. 그러면 model 이 무엇을 요청하든 위험 tool 은 승인 없이 물리적으로 실행되지 않습니다. Model
            에게는 “gate 를 통과해야 한다”는 사실을 알릴 필요조차 없습니다.
          </p>
        </ProgressiveDetail>
      </section>
      ...
      <section id="recovery-checkpointing" className="scroll-mt-20">
        ...
        <div className="prose prose-neutral max-w-none dark:prose-invert">
          <p className="text-lg leading-8">
            Failure detection 은 verifier 실패·반복 action·timeout 같은 신호로 “지금 잘못되고 있다”를 알아채는 단계이고 checkpointing 은
            되돌아갈 수 있는 안전한 state 를 미리 저장해 두는 준비입니다. Recovery strategy 는 이 둘을 전제로 합니다. 감지된 실패를 checkpoint 시점으로
            되돌리거나 그 지점부터 다른 경로를 다시 시도하는 절차 전체가 여기에 들어갑니다.
          </p>
          <p>
            Checkpoint 가 없으면 failure detection 은 “실패를 알아챘다”는 사실만 남기고 되돌릴 지점이 없어 처음부터 다시 시작하거나 잘못된 state 위에서
            복구를 시도하게 됩니다. 반대로 checkpoint 만 있고 감지가 늦으면 이미 여러 단계 더 진행된 뒤에야 되돌리게 되어 그사이의 정상적인 작업까지 함께 버립니다. 이렇게 두
            요소는 서로의 효과를 제한합니다.
          </p>
          <p>
            실제 harness 에서는 매 단계 git commit 과 진행 기록 파일을 checkpoint 로 쓰는 방식이 보고돼 있습니다. 각 세션을 시작할 때 이전 진행 기록과
            commit 로그를 읽고 실제 개발 서버에서 기본 end-to-end 테스트를 한 번 돌립니다. Model 이 “완료됐다”고 적어 둔 내용과 실제 동작이 어긋나는
            premature termination 을 세션 경계에서 걸러 내기 위해서입니다.
          </p>
        </div>
      ...
      <section id="human-in-the-loop-escalation" className="scroll-mt-20">
        ...
        <div className="prose prose-neutral max-w-none dark:prose-invert">
          <p className="text-lg leading-8">
            Human-in-the-loop(HITL) 은 실행을 멈추는 지점입니다. 여기서 사람이 agent 가 제안한 action 을 실행 전에 검토·승인·수정· 거부할 수 있습니다.
          </p>
          <p>
          ...
          <p>
            공개된 구현 하나는 실행을 멈추는 primitive 를 호출해 graph 를 일시 정지합니다. 그다음 사람이 승인(approve)·인자 수정(edit)·거부(reject)·직접
            답변(respond) 중 하나를 정해 재개 명령과 함께 돌려줍니다. 이때 state 는 checkpoint 저장소에 남아 있어 사람이 바로 응답하지 않고 몇 시간 뒤에 돌아와도
            같은 지점에서 이어집니다.
          </p>
          <p>
            Escalation 은 실패의 최종 처리 방식이 아니라 checkpoint·recovery strategy 와 이어지는 한 경로입니다. 사람이 거부하면 agent 는 그
            checkpoint 로 되돌아가 다른 경로를 다시 시도하거나 애초에 이 유형의 실패는 retry 대상이 아니라는 판단을 그대로 유지합니다.
          </p>
        </div>
