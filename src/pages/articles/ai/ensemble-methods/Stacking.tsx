    ...
    <section id="stacking" className="mb-16 scroll-mt-20">
      ...
      <div className="prose max-w-none prose-neutral dark:prose-invert">
        <p>
          ...
          값들이 n×M OOF matrix Z가 되고 meta-model은 Z에서 target을 학습합니다. Base model의 training prediction을 넣으면
      ...
      <div className="prose max-w-none prose-neutral dark:prose-invert">
        ...
        <p>
          ...
          Unit test에는 fold manifest와 row checksum, class order, inverse transform, model별 aggregation rule을 넣고
          전처리부터 meta-model 입력까지 train과 serving의 column 수·순서·단위를 비교합니다. Meta gain은 이 경로 전체를 고정한 nested 또는
          untouched outer evaluation에서 판단합니다.
        </p>
        <h3>Oracle 결과의 증명 아이디어와 작은 표본 반례</h3>
        <p>
          핵심 증명 골격은 이렇습니다. 허용한 family 전체에서 cross-validated risk와 population risk의 차이가 균일하게 작아집니다. 그러면 CV risk를
          최소화한 조합의 true risk도 같은 family의 oracle risk보다 그 최대 오차 이상 크게 벗어나기 어렵습니다. 이 단계에는 bounded
          loss·prediction, 적절한 fold 구성, 표본보다 너무 빨리 커지지 않는 library 같은 조건이 필요합니다. 반대로 작은 데이터에서 수천 개의 meta 후보를 같은
          OOF score로 반복 선택하거나 in-sample base prediction을 넣으면 우연한 최소값과 과신을 고를 수 있습니다. 이 경우 finite sample에서는 단일
          모델보다 나빠질 수 있으며 asymptotic theorem이 이를 막아 주지 않습니다.
        </p>
      </div>
      ...
      <div id="paper-super-learner" className="not-prose my-8 scroll-mt-24 border-l border-primary/50 pl-4">
        ...
        <p className="mt-2 text-sm leading-6 text-muted-foreground">
          ...
          growth 등 조건에서 oracle selector와 비교하는 asymptotic 결과를 제공합니다. “아무 모델이나 stack하면 항상 단일 최고 모델보다 낫다”는
      ...
      <div id="standard-sklearn-stacking" className="not-prose my-8 scroll-mt-24 border-l border-border pl-4">
        ...
        <p className="mt-2 text-sm leading-6 text-muted-foreground">
          ...
          X에서 다시 fit한다고 설명합니다. cv="prefit"은 prefit model의 같은 training data prediction으로 final estimator를 학습하면
          매우 높은 overfitting 위험이 있으므로 설치 version과 data lineage를 확인합니다.
        </p>
