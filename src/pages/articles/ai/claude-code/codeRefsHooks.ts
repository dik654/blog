    ...
    annotations: [
      {
        lines: [36, 45],
        color: "sky",
        note: "검증 규칙 정의 — grep→rg, find→rg 대체 패턴",
      },
      {
        lines: [48, 53],
        color: "emerald",
        note: "_validate_command — 정규식 매칭으로 위반 목록 수집",
      },
      {
        lines: [56, 79],
        color: "amber",
        note: "main — stdin JSON 파싱 → 검증 → exit code 제어 (0=허용, 2=차단)",
      },
    ],
    desc: `문제: Claude Code가 실행하는 Bash 명령어 중 grep이나 find 같은 비효율적 도구를 사용하는 경우를 사전에 차단하고 싶습니다.
    ...
    annotations: [
      {
        lines: [35, 58],
        color: "sky",
        note: "evaluate_rules — 모든 규칙 순회, block/warn 분류",
      },
      {
        lines: [60, 84],
        color: "emerald",
        note: "차단 규칙 처리 — 이벤트 타입별 응답 포맷 분기 (Stop, PreToolUse 등)",
      },
      {
        lines: [86, 94],
        color: "amber",
        note: "경고 규칙 처리 — systemMessage로 경고만 표시, 작업은 허용",
      },
    ],
    desc: `문제: 훅 시스템에서 여러 규칙이 동시에 매치될 수 있으며, 차단과 경고를 구분해야 합니다.
    ...
    annotations: [
      {
        lines: [15, 29],
        color: "sky",
        note: "Condition 데이터클래스 — field/operator/pattern 조합",
      },
      {
        lines: [32, 42],
        color: "emerald",
        note: "Rule 데이터클래스 — 이름, 이벤트, 조건 목록, action(warn/block)",
      },
    ],
    desc: `문제: 훅 규칙을 코드 없이 선언적으로 정의할 수 있어야 합니다.
