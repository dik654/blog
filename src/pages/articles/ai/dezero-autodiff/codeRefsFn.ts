    ...
    annotations: [
      {
        lines: [1, 4],
        color: "sky",
        note: "Function trait — forward(순전파)와 backward(역전파) 쌍",
      },
      {
        lines: [10, 10],
        color: "emerald",
        note: "inputs: 강한 참조(Rc) — backward에서 접근 필요하므로",
      },
      {
        lines: [11, 11],
        color: "amber",
        note: "outputs: Weak — 순환 참조 방지. Variable -> FuncState -> Variable 순환 차단",
      },
    ],
  },
    ...
    annotations: [
      {
        lines: [2, 3],
        color: "sky",
        note: "데이터 추출 — borrow()로 불변 참조 후 clone",
      },
      {
        lines: [9, 9],
        color: "emerald",
        note: "ENABLE_BACKPROP 체크 — no_grad() 스코프에서는 건너뜀",
      },
      {
        lines: [19, 20],
        color: "amber",
        note: "set_creator — 출력 Variable의 creator를 이 함수로 연결",
      },
      {
        lines: [22, 23],
        color: "violet",
        note: "Rc::downgrade — outputs를 Weak로 저장해 순환 참조 방지",
      },
    ],
  },
};
