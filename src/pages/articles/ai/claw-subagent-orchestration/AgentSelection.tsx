    ...
    <section id="agent-selection" className="mb-16 scroll-mt-20">
      ...
      <div className="prose prose-neutral dark:prose-invert max-w-none">
        <p className="leading-7">
          sub-agent 선택은 이미 나눈 작업 계약을 어떤 실행 환경에 맡길지 정하는 문제입니다. 이름이 멋진 역할을 고르는 일이 아닙니다. 탐색 결과만 필요한 작업에 write
          tool을 줄 이유가 없고 같은 파일을 고치는 두 작업을 동시에 보내면 모델 성능과 무관하게 충돌이 납니다.
        </p>
        <p className="leading-7">
          많은 agent description을 매 turn마다 system prompt에 다 넣으면 선택 노이즈와 context 비용이 커질 수 있습니다. 먼저 task contract로
          후보를 좁히고 필요한 capability와 permission을 충족하는 소수만 노출하는 편이 단순합니다. “상위 11개” 같은 고정 숫자는 설계 원칙이 아니라 특정
          snapshot의 tuning 값으로만 다뤄야 합니다.
        </p>
      ...
      <div className="prose prose-neutral dark:prose-invert max-w-none">
        ...
        <p className="leading-7">
          agent definition에는 자연어 description만 넣지 않습니다. allowed tools와 readable·writable path, network policy,
          model class, expected output schema, isolation mode를 함께 둡니다. 선택기가 description을 잘 이해하더라도 runtime이 이
          제약을 강제하지 않으면 실제 권한은 달라질 수 있습니다.
        </p>
        <p className="leading-7">
          태그는 빠른 1차 filtering에 유용합니다. 다만 keyword와 file extension만으로 domain fit을 확정하지는 않습니다. 모호한 요청은 main
          agent가 먼저 deliverable과 scope를 구체화하고 그래도 여러 후보가 남을 때만 semantic similarity나 작은 routing model을 씁니다.
        </p>
        ...
        <p className="leading-7">
          tag overlap과 domain fit, 최근 성공률을 한 점수로 합치면 구현은 쉽습니다. 대신 낮은 비용과 높은 권한 위험이 같은 숫자 안에서 상쇄될 수 있습니다. 먼저
          hard constraint로 permission과 isolation 조건을 통과시킨 뒤 남은 후보의 품질·latency·비용을 비교하는 편이 안전합니다.
        </p>
        ...
        <p className="leading-7">
          최근 success rate도 “agent가 완료했다고 응답했는가”가 아니라 검증을 통과한 산출물로 계산합니다. 쉬운 작업만 받은 agent가 유리해지는 selection
          bias도 줄여야 합니다. 작업 난이도와 유형별로 지표를 나누고 표본이 적을 때는 confidence interval을 함께 봅니다.
        </p>
        ...
        <p className="leading-7">
          대화 중 단어가 조금 바뀔 때마다 후보를 다시 고르면 prompt cache가 흔들리고 역할도 일관되지 않습니다. pool은 새로운 deliverable이 생기거나 기존 작업
          계약이 끝났을 때 갱신합니다. 진행 중인 worker는 같은 contract와 tool set을 유지합니다.
        </p>
        <p className="leading-7">
