...
export default function EmbeddingEvaluationArticle() {
  return <div className="space-y-16">
    <section id="overview" className="scroll-mt-20">
      ...
      <div className="prose prose-neutral max-w-none dark:prose-invert"><p className="text-lg leading-8">한 query에 답이 되는 문서는 하나가 아닐 수 있습니다. 같은 규정을 설명하는 원문·FAQ·절차서가 모두 relevant일 수 있고, corpus가 바뀌면 document ID와 정답 집합도 달라집니다. 이 상태에서 single-positive label만 쓰면 실제 정답을 negative로 채점할 수 있습니다.</p></div>
    ...
    <section id="labels" className="scroll-mt-20">
      ...
      <div className="prose prose-neutral max-w-none dark:prose-invert"><p>Query “연차는 며칠인가?”에 정책 원문 A와 직원 FAQ B가 모두 답을 포함한다면 relevant set은 {`{A,B}`}입니다. A만 positive로 두고 B를 hard negative로 학습하거나 평가하면 model이 유용한 문서를 올린 일을 오답으로 셉니다.</p><p>Label snapshot에는 corpus revision, query definition, positive IDs, relevance grade, annotator agreement와 unresolved case를 남깁니다. 새 문서가 들어온 뒤에는 예전 metric과 직접 비교하지 않고 새 generation으로 다시 채점합니다.</p></div>
    </section>
    ...
    <section id="metrics" className="scroll-mt-20">
      ...
      <ExplainedFormula question="Relevant 문서가 여러 개일 때 top-k 결과를 어떻게 두 축으로 읽나요?" idea={<p>Recall은 relevant set 중 top-k 안에 들어온 비율입니다. NDCG는 각 문서의 relevance gain을 rank가 내려갈수록 할인해 더한 뒤, 가능한 이상적 순서의 값으로 나눕니다.</p>} formula={String.raw`\operatorname{Recall@}k=|\mathcal R_q\cap C_k(q)|/|\mathcal R_q|,\quad \operatorname{NDCG@}k=\operatorname{DCG@}k/\operatorname{IDCG@}k`} annotatedFormula={String.raw`\begin{aligned}n_{\rm hit}&=\underbrace{|\mathcal R_q\cap C_k(q)|}_{\text{top-k의 정답 수}}\\\operatorname{Recall@}k&=\underbrace{n_{\rm hit}/|\mathcal R_q|}_{\text{전체 정답 중 찾은 비율}}\\g_i&=\underbrace{2^{r_i}-1}_{\text{grade를 gain으로 변환}}\\w_i&=\underbrace{1/\log_2(i+1)}_{\text{낮은 rank를 할인}}\\\operatorname{DCG@}k&=\underbrace{\sum_{i=1}^{k}g_iw_i}_{\text{할인 gain을 누적}}\\\operatorname{NDCG@}k&=\underbrace{\operatorname{DCG@}k/\operatorname{IDCG@}k}_{\text{이상적 순서로 정규화}}\end{aligned}`} operations={[
