    ...
    <div id="overview" className="space-y-16">
      <section id="problem" className="scroll-mt-20">
        ...
        <div className="prose prose-neutral max-w-none dark:prose-invert">
          ...
          <p>
            세 축은 순서대로 이어집니다. 먼저 채점 기준이 자동으로 검증 가능한 task 인지부터 정합니다(RLVR). 그 다음 그 reward 를 trajectory 끝에만 줄지
            중간마다 줄지(sparse/dense), 최종 결과만 볼지 중간 과정도 볼지 (outcome/process)를 고릅니다. 마지막으로 그 설계가 실제 목표와 어긋나는 proxy
            가 되지 않는지(reward hacking)를 점검합니다.
          </p>
        </div>
      ...
      <section id="rlvr" className="scroll-mt-20">
        ...
        <div className="prose prose-neutral max-w-none dark:prose-invert">
          <p className="text-lg leading-8">
            정답이 맞았는지를 사람이 읽어 보지 않아도 프로그램이 판정할 수 있는 문제가 있습니다. 이런 쪽을 verifiable task 라고 합니다. 수학 문제의 최종 숫자, code
            의 unit test 통과 여부, 정규식이 정한 출력 형식이 그 예입니다. RLVR (Reinforcement Learning with Verifiable Rewards) 는
            이런 task 에서 그 자동 검증 결과를 그대로 reward 로 쓰는 RL 방식입니다.
          </p>
          <p>
      ...
      <section id="sparse-vs-dense" className="scroll-mt-20">
        ...
        <div className="prose prose-neutral max-w-none dark:prose-invert">
          <p className="text-lg leading-8">
            Sparse reward 는 trajectory 가 끝나야만(또는 아주 드물게만) 0 이 아닌 값을 줍니다. dense reward 는 거의 매 step 마다 0 이 아닌 값이
            나옵니다. RLVR 의 기본 형태는 최종 결과 하나만 확인하니 대개 sparse 입니다. 중간 단계마다 점수를 매기면 dense 가 됩니다.
          </p>
          <p>
            6-station 경로를 생각해 봅시다. 목표(S6)에 도달해야만 +1 을 받는 sparse reward 라면 초기 policy 의 rollout 대부분은 S6 에 못
            미칩니다. reward 가 0 으로 끝나니 gradient 신호가 거의 없습니다. 같은 경로에서 S3 까지 갔을 때 진척 비율만큼(0.5) reward 를 주는 dense
            reward 라면 실패한 rollout 도 0 이 아닌 신호를 남깁니다. 학습이 방향을 잡는 속도가 그만큼 빨라집니다.
          </p>
          <p>
            대가는 그 중간 신호를 무엇으로 잴지 새로 설계해야 한다는 점입니다. 진척을 잘못 정의하면 policy 가 실제 목표 대신 그 중간 신호를 최적화하기 시작합니다. 뒤에서 다룰
            reward hacking 의 흔한 경로입니다.
          </p>
        </div>
      </section>
      ...
      <section id="outcome-vs-process" className="scroll-mt-20">
        ...
        <div className="prose prose-neutral max-w-none dark:prose-invert">
          <p className="text-lg leading-8">
            Sparse/dense 가 언제 reward 를 주는지의 축이라면 outcome reward 와 process reward 는 무엇을 채점하는지의 축입니다. Outcome
            reward 는 최종 결과 하나만 봅니다. process reward 는 중간 추론 단계 각각의 타당성까지 봅니다. 두 축은 논리적으로 독립적입니다. 실무에서는 outcome
            reward 가 보통 sparse(끝에만)로, process reward 가 보통 dense(매 단계)로 구현됩니다.
          </p>
          <p>
            수학 풀이 10 단계 중 최종 답만 맞으면 outcome reward 는 1 을 줍니다. 중간 3 단계에서 계산이 틀렸는데도 우연히 최종 답이 맞았다면 그 오류는 outcome
            reward 에 드러나지 않습니다. Process reward 는 그 3 단계에서 이미 낮은 점수를 매겨 이 우연한 정답을 구분합니다.
          </p>
        </div>
      ...
      <section id="hacking" className="scroll-mt-20">
        ...
        <div className="prose prose-neutral max-w-none dark:prose-invert">
          <p className="text-lg leading-8">
            policy 가 설계된 reward(proxy) 값은 계속 높이는데 정작 그 reward 가 대신하려던 진짜 목표(true objective)에서는 멀어지는 일이 있습니다. 이
            현상을 reward hacking 이라고 부릅니다. Specification gaming 은 같은 것을 가리키는 동의어에 가까운 표현입니다. reward
            misspecification 은 애초에 reward 함수가 진짜 목표를 정확히 담지 못한 상태를 가리킵니다.
          </p>
          <p>
            앞 절의 진척 기반 dense reward 를 예로 들어 봅시다. 목표까지 남은 거리로 진척을 정의하면 policy 가 실제로 목표에 가지 않고 거리 계산을 속이는 위치에서만
            맴돌 수 있습니다. 이런 경우가 reward hacking 입니다. Proxy(거리 감소)는 개선됐지만 true objective(목표 도달)는 그대로거나 오히려 나빠집니다.
          </p>
        </div>
        ...
        >
          <p>
            Verifier reward 조차 hacking 에서 자유롭지 않습니다. Unit test 를 우회하는 코드(예: 예외를 삼켜 항상 통과 처리)처럼 verifier 의 확인
            범위 자체를 조작하는 경우가 있습니다. verifier 구현을 독립적으로 감사하는 절차가 필요한 이유입니다.
          </p>
        </ProgressiveDetail>
      </section>
      ...
      <section id="shaping" className="scroll-mt-20">
        ...
        <div className="prose prose-neutral max-w-none dark:prose-invert">
          <p className="text-lg leading-8">
            Reward shaping 은 sparse reward 의 신호 부족을 완화하려고 원래 reward 에 추가 항을 더하는 방법입니다. 문제는 아무 항이나 더하면 policy 가
            그 추가 항을 최적화하며 원래 목표에서 벗어날 수 있다는 점입니다. 방금 본 reward hacking 위험 그대로입니다. Potential-based shaping 은 이
            위험 없이 신호를 앞당기는 특정 형태를 제시합니다.
          </p>
          <p>
            핵심은 추가 항을 상태의 potential 함수 Φ 의 차이로만 만드는 것입니다. 이렇게 만든 추가 항은 어떤 trajectory 를 완주하든 차이가 상쇄돼 총합에 영향을 주지
            않습니다. 원래 MDP 의 optimal policy 를 그대로 보존한다는 것이 증명돼 있습니다.
          </p>
        </div>
        <ExplainedFormula
      ...
      <section id="reward-shape-and-calibration" className="scroll-mt-20">
        ...
        <div className="prose prose-neutral max-w-none dark:prose-invert">
          <p className="text-lg leading-8">
            Binary reward 는 성공·실패 두 값(0 또는 1)만 냅니다. 그 사이 실수 값까지 내면 continuous reward 입니다. RLVR 의 unit test 통과
            여부는 흔히 binary 입니다. 통과한 test 케이스 비율로 점수를 매기면 continuous 가 됩니다. 이 값 형태는 sparse/dense·outcome/process
            축과 독립적으로 정할 수 있습니다.
          </p>
          <p>
          ...
          <p>
            값 형태와 척도를 구분하지 않으면 두 문제가 뒤섞입니다. Binary reward 두 개를 섞는 데는 calibration 이 필요 없습니다. binary reward 하나와
            continuous reward 하나를 같은 batch 에서 섞으면 값 형태 차이가 곧 척도 차이가 되어 calibration 이 필요해집니다.
          </p>
        </div>
        <TermBreakdown
