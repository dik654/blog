  ...
  return (
    <section id="execution" className="mb-16 scroll-mt-20">
      ...
      <div className="prose prose-neutral dark:prose-invert max-w-none">
        <p>
          실행은 선택→전체 지침 로딩→input·dependency 준비→tool·script 실행→산출물 검증→handoff 순서로 봅니다. 이때 Skill이 model에게 알려 주는
          것은 “어떻게 일할지”뿐입니다. file write, external API, destructive action의 권한은 여기서 생기지 않습니다. caller identity,
          target scope, approval와 sandbox를 기존 policy로 검사하는 쪽은 runtime입니다. 거부·부분 성공·timeout도 observation으로 돌려줘야
          다음 판단이 이를 구분할 수 있습니다.
        </p>
      </div>
      ...
      <ExplainedFormula
        ...
        idea={
          <p>
            적용해야 하는 positive request와 적용하면 안 되는 hard negative request를 먼저 label합니다. 그다음 실제 선택 결과와 맞춰 보며 잘 선택한
            TP, 잘못 끼어든 FP, 놓친 FN을 셉니다. precision이 잡아내는 실패와 recall이 잡아내는 실패는 서로 다릅니다.
          </p>
        }
      ...
      <div className="prose prose-neutral dark:prose-invert max-w-none">
        <p>
          예를 들어 적용 대상 40건 중 36건을 고르고 비대상 요청에서 6건을 잘못 골랐다면 precision은 36/42, recall은 36/40입니다. 다음 수정에서
          description을 무조건 길게 늘리지는 않습니다. false positive와 false negative 사례를 직접 읽고 trigger 앞부분과 비적용 경계를 손본 뒤 같은
          eval set과 새 regression set에 다시 걸어 비교합니다.
        </p>
      </div>
    </section>
  );
