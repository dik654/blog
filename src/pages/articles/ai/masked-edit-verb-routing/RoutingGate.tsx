    ...
    <section id="routing-gate" className="scroll-mt-20">
      ...
      <div className="prose prose-neutral max-w-none dark:prose-invert">
        <p className="text-lg leading-8">
          표를 만들고 나면 그것을 어디에 둘지가 남습니다. 모델을 하나로 고정하면 여섯 동작 중 넷이 나빠지고,
          에이전트에게 고르게 하면 이 측정을 모르는 채로 최근에 본 이름을 고릅니다. 둘 다 표를 버리는 선택입니다.
        </p>
        ...
        <p className="leading-7">
          이 구조를 실제 요청 일곱 건으로 돌려 봤습니다. 사용자는 한 번도 모델 이름을 말하지 않았고, 선택된
          모델은 매번 표가 예측한 것이었습니다. 마스크 밖 변화량의 순서까지 표의 예측과 같은 순서로
          나왔습니다.
        </p>
      ...
      <div className="prose prose-neutral mt-8 max-w-none dark:prose-invert">
        <p className="leading-7">
          시간 비용도 이 배치에 들어갑니다. 같은 편집을 모델을 내린 직후에 돌리면 8초짜리가 23초가 되고,
          26초짜리가 55초가 됩니다. 차이의 절반 이상이 가중치를 올리는 시간입니다. 동작마다 다른 모델로 보내는
          라우팅은 편집마다 이 값을 낸다는 뜻이고, 같은 모델로 몰아 줄 수 있는 연속 편집에서는 두세 배가
          달라집니다.
        </p>

        <p className="leading-7">
          측정할 때 한 가지 함정이 있었습니다. 같은 시드로 다시 돌리면 1~2초가 나오는데, 그건 재현성이 아니라
          그래프 캐시가 답한 것입니다. 실제로 연산이 도는 값을 재려면 시드를 바꿔야 하고, 이 구분을 하지 않으면
          표의 시간 열에 가짜 값이 섞입니다. 실제로 섞여 있었습니다.
        </p>

        <p className="leading-7">
          정리하면 이 글의 결론은 표 자체가 아니라 표를 두는 자리입니다. 모델 선택을 사람의 기억이나 언어
          모델의 인상이 아니라 측정에 맡기고, 마스크 확장처럼 직관과 반대인 값도 함께 따라오게 하는 것입니다.
        </p>
