  ...
  return (
    <section id="lstm" className="mb-16 scroll-mt-20">
      ...
      <div className="prose prose-neutral dark:prose-invert max-w-none">
        <p>
          현재 입력과 이전 hidden state는 forget, input, output gate와 candidate를 위한 네 개의 projection으로 들어갑니다. 세 gate에는
          sigmoid를 적용해 0과 1 사이의 비율을 만들고 candidate에는 tanh를 적용해 새로 기록할 내용을 만듭니다.
        </p>
        <p>
...
\end{aligned}`}
      ...
      <div className="prose prose-neutral dark:prose-invert max-w-none">
        ...
        <p>
          교육용 구현은 gate별 Linear를 나누면 흐름을 읽기 쉽지만 실제 프레임워크는 보통 네 projection을 하나의 큰 행렬곱으로 합친 뒤 결과를 분할합니다. 이렇게 하면
          kernel launch와 메모리 접근이 줄어듭니다. 먼저 분리 구현으로 수치가 맞는지 검증한 다음 fused 형태와 결과를 비교합니다.
        </p>
      </div>
    </section>
  );
