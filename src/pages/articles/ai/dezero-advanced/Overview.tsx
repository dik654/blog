  ...
  return (
    <section id="overview" className="mb-16 scroll-mt-20">
      ...
      <div className="prose prose-neutral dark:prose-invert max-w-none">
        ...
        <p>
          고정 예제는 hidden dimension 1인 짧은 sequence입니다. 첫 chunk의 마지막 cell state를 다음 chunk로 넘기되 graph는 detach하고, 독립 sample이 시작되면 hidden·cell state를 모두 0으로 reset합니다. 같은 input을 train mode와 eval mode에서 실행해 dropout mask만 달라지는지, checkpoint 뒤 RNG와 state가 같은지도 함께 검사합니다.
        </p>
      </div>
      ...
      <div className="prose prose-neutral dark:prose-invert max-w-none">
        ...
        <p>
          Release에서는 분리한 네 gate와 fused projection의 forward·gradient를 같은
          weight로 비교하고, batch·sequence 경계마다 reset·carry·detach receipt를
          남깁니다. Constant input LayerNorm, 반복 ID Embedding, dropout train/eval,
          checkpoint 직전·직후 RNG를 negative fixture로 고정하며, shape·finite 값·
          analytic/finite-difference gradient와 continuous/resume 결과가 모두
          허용 오차 안에 들어온 뒤에만 새 implementation을 채택합니다.
        </p>
      </div>
