...
const STEPS = [
  {
    label: "결함 분류",
    body: "심사원이 발견한 문제는 결함(Minor)과 중결함(Major)으로 분류. 등급에 따라 후속 조치가 다르다.",
  },
  {
    label: "보완조치 전략",
    body: '일시적 땜질이 아닌 "재발 방지 체계"를 증적으로 제출. 절차서 수립 + 자동화 + 모니터링까지.',
  },
];
        ...
        >
          ...
          {step === 0 && (
            <motion.g initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
              ...
              <AlertBox
                ...
                h={35}
                label="결함 = 인증 실패가 아님"
                sub="보완조치의 품질이 핵심. '재발 방지 체계'를 증적으로 보여줘야 함"
                color={C.minor}
              />
            </motion.g>
          )}
          ...
          {step === 1 && (
            <motion.g initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
              ...
              >
                심사원은 "이번에 고쳤다"가 아니라 "앞으로 안 생긴다"를 본다
              </text>
            </motion.g>
          )}
        </svg>
