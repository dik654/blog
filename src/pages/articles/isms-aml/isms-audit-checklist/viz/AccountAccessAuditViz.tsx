...
const STEPS = [
  {
    label: "비밀번호 정책",
    body: "변경주기 90일, 최소 8자, 복잡도(영+숫+특수), 5회 실패 잠금, 재사용 3~5개 금지. 모든 시스템에 적용.",
  },
  {
    label: "계정 분리 · 장기 미접속",
    body: "서비스/관리자/백업/비상 계정 분리 필수. host가 %인 DB 계정 = 결함. 6개월 미접속 계정 비활성화.",
  },
  {
    label: "공용 계정 · MFA",
    body: "공용 계정 원칙 금지, 불가피 시 사용대장 필수. 관리자 MFA 미적용 = 결함. 모든 IAM에도 MFA.",
  },
  {
    label: "접근제어 로그",
    body: "접근제어 솔루션(DAC) 필수. 로그 항목(ID, IP, 쿼리), 보존 1년+, 위변조 방지, 비인가 접근 탐지 체계.",
  },
  {
    label: "권한 정기 검토",
    body: "반기 1회 전체 시스템 접근 권한 검토. 불필요 권한 회수, 최소 권한 원칙 준수 여부를 보고서로 증빙.",
  },
];
        ...
        >
          ...
          {step === 0 && (
            <motion.g initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
              ...
              >
                모든 시스템(서버, DB, 클라우드, 메일)에 동일 기준 적용
              </text>
            </motion.g>
          )}
          ...
          {step === 2 && (
            <motion.g initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
              ...
              <text
                ...
                fill={C.fail}
              >
                AWS: root MFA만으로 불충분 → 모든 IAM 사용자에 MFA 필수
              </text>
            </motion.g>
          )}
          ...
          {step === 4 && (
            <motion.g initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
              ...
              <ActionBox
                ...
                w={280}
                h={40}
                label="검토: 불필요 권한 회수 + 퇴직자 삭제 + 과도한 권한 조정"
                sub="검토일, 검토자, 대상 시스템, 발견 사항, 조치 결과"
                color={C.warn}
              />
              ...
              <AlertBox
                ...
                w={320}
                h={22}
                label="'편의상' / '원래부터 이렇게' = 결함 사유. 업무 근거 문서화 필수"
                sub=""
                color={C.fail}
              />
            </motion.g>
