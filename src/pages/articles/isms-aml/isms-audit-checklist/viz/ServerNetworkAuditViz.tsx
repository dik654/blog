...
const STEPS = [
  {
    label: "망분리 · ping 테스트",
    body: "DMZ와 내부망 사이 ICMP 차단 확인. DMZ 웹서버 → 내부 DB ping 시 100% packet loss가 정상.",
  },
  {
    label: "방화벽 규칙 점검",
    body: "ANY→ANY 규칙, Hit Count 0 규칙, 과도한 포트 범위를 필터링. 정기 검토 기록(반기 1회+)도 확인.",
  },
  {
    label: "불필요 포트 · OS 보안",
    body: "ss -tlnp로 리스닝 포트 조회. SSH root 허용, 패스워드 정책 미설정, 불필요 서비스 활성화를 점검.",
  },
  {
    label: "패치 현황",
    body: "마지막 패치 일자, 분기 1회 정기 패치, 긴급 CVE 패치 절차, 테스트 환경 검증 여부를 확인.",
  },
  {
    label: "네트워크 장비 · 클라우드",
    body: "기본 계정 변경, SNMP 커뮤니티 문자열, 펌웨어 버전. 클라우드는 Security Group의 0.0.0.0/0 규칙을 확인.",
  },
];
        ...
        >
          ...
          {step === 0 && (
            <motion.g initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
              ...
              >
                증적: 방화벽 ICMP 차단 규칙 + ping 실패 화면 캡처
              </text>
              <text
                x={240}
                y={210}
          ...
          {step === 3 && (
            <motion.g initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
              ...
              <AlertBox
                ...
                w={280}
                h={42}
                label="uname -r → EOL 커널이면 지적"
                sub="예: CentOS 7 (2024년 6월 지원 종료) → OS 마이그레이션 계획 필요"
                color={C.fail}
              />
              ...
              <text
                ...
                fill="var(--muted-foreground)"
              >
                운영 직접 적용 = 변경관리 미준수. 테스트 환경 검증 필수.
              </text>
            </motion.g>
          )}
          ...
          {step === 4 && (
            <motion.g initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
              ...
              <text
                ...
                fill="var(--muted-foreground)"
              >
                AWS SG / GCP 방화벽 규칙에서 인바운드 0.0.0.0/0 = 동일 지적
              </text>
            </motion.g>
          )}
        </svg>
