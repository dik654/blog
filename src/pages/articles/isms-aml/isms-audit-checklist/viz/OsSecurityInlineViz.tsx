...
const STEPS = [
  {
    label: "포트 점검 흐름",
    body: "ss -tlnp 명령어로 리스닝 포트 조회. 0.0.0.0 바인딩은 모든 IP에 노출 — 제한 필요.",
  },
  {
    label: "OS 설정 파일 확인",
    body: "/etc/login.defs 패스워드 정책, sshd_config root 로그인 차단, 불필요 서비스 비활성화.",
  },
];
        ...
        >
          ...
          {step === 0 && (
            <motion.g initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
              ...
              >
                모든 포트는 서비스 목적에 맞게 바인딩 IP 제한 필수
              </text>
            </motion.g>
          )}
          ...
          {step === 1 && (
            <motion.g initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
              ...
              <text
                ...
                fill="var(--muted-foreground)"
              >
                PAM pam_pwquality로 비밀번호 복잡도 강제 설정 여부도 확인
              </text>
              <text
                x={240}
                y={185}
                ...
                fill="var(--muted-foreground)"
              >
                PermitRootLogin yes → root 원격 직접 접속 가능 → 중대 결함
              </text>
            </motion.g>
          )}
        </svg>
