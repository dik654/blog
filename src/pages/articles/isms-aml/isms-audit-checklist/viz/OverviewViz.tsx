...
const STEPS = [
  {
    label: "심사 구조",
    body: "심사원 2~4명이 팀을 구성하여 영역별로 병렬 점검. 3~5일 동안 문서 확인, 인터뷰, 시스템 실사를 반복.",
  },
  {
    label: "3단계 검증 루프",
    body: "심사원은 한 항목에 대해 문서 확인 → 담당자 인터뷰 → 시스템 실사 세 가지를 조합하여 교차 검증.",
  },
  {
    label: "증적자료 준비",
    body: '증적은 "정책이 실제로 작동함"을 증명하는 자료. 스크린샷, 로그, 문서, 사진, 시스템 기록 등 다양한 형태.',
  },
  {
    label: "사전 체크리스트",
    body: "현장심사 2~4주 전: 증적 폴더 정리, 담당자 배정, 시연 환경 테스트, 리허설까지 완료.",
  },
  {
    label: "결함 등급과 보완",
    body: '결함(Minor)은 40일 내 개선. 중결함(Major)은 심사 중단 가능. 보완조치는 "재발 방지 체계"가 핵심.',
  },
];
          ...
          {step === 3 && (
            <motion.g initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
              ...
              <text
                ...
                fill="var(--muted-foreground)"
              >
                담당자 부재, 문서 날짜 불일치, 과도한 예외 처리도 결함 사유
              </text>
            </motion.g>
          )}
          ...
          {step === 4 && (
            <motion.g initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
              ...
              <text
                ...
                fill="var(--muted-foreground)"
              >
                대부분의 조직은 5~15개 결함. 결함 = 인증 실패가 아님.
              </text>
            </motion.g>
          )}
        </svg>
