...
const STEPS = [
  {
    label: "문서 확인",
    body: '정책서·지침서·절차서가 존재하는지 확인. "접근통제 정책이 있나요?" → 정책 문서 제시.',
  },
  {
    label: "담당자 인터뷰",
    body: '정책이 실무에 반영되는지 질문. 답변이 문서와 다르면 "문서-실행 괴리" 결함.',
  },
  {
    label: "시스템 실사",
    body: "실제 설정 화면을 열어 확인. 심사원이 직접 명령어를 요청하기도 한다.",
  },
];
          ...
          {step === 0 && (
            <motion.g initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
              ...
              <AlertBox
                ...
                w={320}
                h={40}
                label="문서가 없거나 형식만 갖춘 경우"
                sub="정책 문서 부재 → 즉시 결함. 날짜가 2년 전이면 '미갱신' 지적"
                color={C.fail}
              />
              ...
              >
                정책서, 지침서, 절차서 세 종류 모두 존재해야 완전한 체계
              </text>
            </motion.g>
          )}
          ...
          {step === 1 && (
            <motion.g initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
              ...
              <AlertBox
                ...
                w={320}
                h={40}
                label="구두 설명만으로는 적합 판정 불가"
                sub="'하고 있습니다' → 증적이 없으면 미이행으로 기록"
                color={C.fail}
              />
          ...
          {step === 2 && (
            <motion.g initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
              ...
              <AlertBox
                ...
                w={320}
                h={38}
                label="문서·답변·설정값 불일치 = 결함"
                sub="정책: 90일 / 답변: 90일 / 실제 PASS_MAX_DAYS: 99999 → 미이행"
                color={C.fail}
              />
              ...
              <text
                ...
                fill="var(--muted-foreground)"
              >
                심사원이 직접 cat /etc/ssh/sshd_config 등 명령어를 요청하기도
                한다
              </text>
            </motion.g>
          )}
