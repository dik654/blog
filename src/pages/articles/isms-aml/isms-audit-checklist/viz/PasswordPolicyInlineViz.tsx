...
const STEPS = [
  {
    label: "비밀번호 정책 5대 항목",
    body: "변경주기(90일), 최소길이(8자), 복잡도(3종 조합), 잠금(5회), 재사용 금지(3~5개).",
  },
  {
    label: "전 시스템 동일 적용",
    body: "Linux 서버뿐 아니라 웹 관리자, DB, AWS IAM, VPN, 메일 시스템까지 전부 확인.",
  },
];
        ...
        >
          ...
          {step === 0 && (
            <motion.g initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
              ...
              >
                login.defs + PAM + pam_faillock 세 곳 모두 확인
              </text>
            </motion.g>
          )}
          ...
          {step === 1 && (
            <motion.g initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
              ...
              <AlertBox
                ...
                w={360}
                h={32}
                label="한 시스템이라도 정책 미적용이면 결함"
                sub="서버는 OK인데 DB 콘솔 비밀번호 정책 없음 → 결함"
                color={C.fail}
              />
            </motion.g>
          )}
