...
const STEPS = [
  {
    label: "계정 분리 원칙",
    body: "서비스·관리자·백업·비상 — 용도별 계정 분리. root나 admin 하나로 모든 작업 수행하면 결함.",
  },
  {
    label: "장기 미접속 + 공용 계정",
    body: "6개월 이상 미접속 계정 조회. 퇴직자 계정 잔존 = 결함. 공용 계정은 사용대장 필수.",
  },
  {
    label: "MFA + 권한 검토",
    body: "관리자 2차인증 미적용 = 결함. 접근 권한 반기 1회 정기 검토 기록 필수.",
  },
];
          ...
          {step === 1 && (
            <motion.g initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
              ...
              <text
                ...
                fill="var(--muted-foreground)"
              >
                퇴직자 계정 삭제 대장 + 계정 생성/삭제 절차서도 증적 대상
              </text>
              <text
                x={240}
                y={180}
          ...
          {step === 2 && (
            <motion.g initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
              ...
              <AlertBox
                ...
                w={360}
                h={32}
                label="'편의상' 또는 '원래 이랬다' = 결함 사유"
                sub="모든 권한 부여에 업무적 근거 + 문서화 필수"
                color={C.fail}
              />
            </motion.g>
          )}
