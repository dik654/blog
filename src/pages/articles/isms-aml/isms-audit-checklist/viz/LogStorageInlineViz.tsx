...
const STEPS = [
  {
    label: "로그 수집 확인",
    body: "서버 접속, 앱, DB, 방화벽, 개인정보 시스템 — 5종 로그가 모두 수집되고 있는지 확인.",
  },
  {
    label: "보존 기간 + 위변조 방지",
    body: "서버 로그 6개월+, 개인정보 DB 1년+(5만명 이상 2년). 위변조 방지는 중앙 로그 서버 전송이 핵심.",
  },
];
          ...
          {step === 0 && (
            <motion.g initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
              ...
              <text
                ...
                fill="var(--muted-foreground)"
              >
                ls -lt auth.log* | tail 로 가장 오래된 로그 파일 확인
              </text>
            </motion.g>
          )}
          ...
          {step === 1 && (
            <motion.g initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
              ...
              <AlertBox
                ...
                sub="원본 서버에서 삭제해도 중앙 서버에 기록이 남아야 함"
                color={C.fail}
              />
            </motion.g>
          )}
