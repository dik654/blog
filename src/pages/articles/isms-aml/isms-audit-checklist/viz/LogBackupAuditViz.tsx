...
const STEPS = [
  {
    label: "로그 수집 · 보존",
    body: "서버 접속, 앱, DB, 방화벽, 개인정보 처리 로그를 수집. 법적 보존 기간(6개월~2년)을 충족해야 한다.",
  },
  {
    label: "로그 위변조 방지",
    body: "개인정보 처리 시스템 접속기록은 위변조 방지 법적 의무. 중앙 로그 서버(Syslog, ELK)로 실시간 전송이 핵심.",
  },
  {
    label: "백업 정상 수행",
    body: "크론 스케줄 확인, 백업 파일 연속성, 크기 일관성, 실패 알림, 암호화 여부, 소산 백업까지 점검.",
  },
  {
    label: "복구 테스트",
    body: "백업이 있어도 복구 불가능하면 무의미. 연 1회+ 복구 테스트 보고서(RPO/RTO, 결과, 개선사항) 필수.",
  },
  {
    label: "NTP · 변경관리",
    body: "서버 시간 동기화(NTP) 미설정 시 로그 시간 정합성 불가. 변경관리 기록(요청서, 이력대장, 긴급변경) 점검.",
  },
];
        ...
        >
          ...
          {step === 0 && (
            <motion.g initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
              ...
              >
                가장 오래된 로그 파일 날짜로 보존 기간 충족 여부를 검증
              </text>
            </motion.g>
          )}
          ...
          {step === 1 && (
            <motion.g initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
              ...
              <AlertBox
                ...
                w={320}
                h={32}
                label="같은 서버에만 저장 = 위변조 방지 미흡 결함"
                sub="개인정보보호법 시행령 제48조의2 위반"
                color={C.fail}
              />
            </motion.g>
          )}
          ...
          {step === 3 && (
            <motion.g initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
              ...
              <DataBox
                ...
                w={210}
                h={28}
                label="Docker DB import → 레코드 수 비교"
                color={C.ok}
              />
          ...
          {step === 4 && (
            <motion.g initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
              ...
              <AlertBox
                ...
                w={200}
                h={35}
                label="변경 건 有, 이력 無 = 결함"
                sub="audit.log와 대조하여 검증 가능"
                color={C.fail}
              />
