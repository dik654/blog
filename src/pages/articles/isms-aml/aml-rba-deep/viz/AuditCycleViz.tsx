...
const STEPS = [
  {
    label: "분기별 자체 점검 (2선)",
    body: "CCO가 분기마다 CDD 이행률, STR 적시성, FDS 오탐률, 교육 이수율 점검.",
  },
  {
    label: "연 1회 외부 감사 (3선)",
    body: "독립적 외부 전문가가 CDD·STR·FDS·위험평가·교육·기록보관을 객관 평가.",
  },
  {
    label: "지적사항 사후관리",
    body: "지적 등급별 개선 기한(30/60/90일). 미이행 시 자동 에스컬레이션. 2회 반복 시 근본원인 분석.",
  },
];
        ...
        >
          ...
          {step === 1 && (
            <motion.g initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
              ...
              >
                방법: 무작위 샘플링(30건+) + 고위험 전수 검사 + FIU 접수 확인서
                대조
              </text>
              {/* Criteria */}
              <rect
              ...
              <text
                ...
                fill={C.audit}
              >
                감사인 선정: 이해관계 없음 + AML 경험 + CAMS 인증 + 동일인 3년
                시 교체
              </text>
              <text
                x={240}
          ...
          {step === 2 && (
            <motion.g initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
              ...
              <AlertBox
                ...
                w={360}
                h={38}
                label="동일 지적 2회 반복 = 체계적 미흡"
                sub="근본원인 분석(Root Cause Analysis) → 단순 보완이 아닌 프로세스 재설계"
                color={C.red}
              />
            </motion.g>
          )}
