...
const STEPS = [
  {
    label: "문서화 5대 범주",
    body: "위험평가, 정책·절차, CDD 기록, 거래·보고, 교육·감사 — 전부 문서화 대상.",
  },
  {
    label: "보관 기간 — 5년 이상",
    body: "거래 관계 종료일 기준 5년. 검색 가능·무결성 보장·추출 가능 상태 유지.",
  },
  {
    label: "분기별 의무이행 점검",
    body: "CDD 이행률, STR 적시성, FDS 오탐률, 교육 이수율 — 2선이 자체 점검.",
  },
  {
    label: "지적사항 사후관리",
    body: "중대 30일·주요 60일·경미 90일 내 개선. 미이행 시 자동 에스컬레이션.",
  },
];
          ...
          {step === 0 && (
            <motion.g initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
              ...
              <text
                ...
                fill={C.red}
              >
                문서화 없으면 증명 불가 — 규제 검사·감사·FATF 상호평가 모두 문서
                기반
              </text>
              <text
                x={240}
                ...
                fill="var(--muted-foreground)"
              >
                체계적 문서화 = 조직 학습 기반 (위험 추이, 취약 영역 데이터
                확인)
              </text>
            </motion.g>
          )}
          ...
          {step === 3 && (
            <motion.g initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
              ...
              <text
                ...
                fill={C.amber}
              >
                동일 지적 2회 연속 → 체계적 미흡 → 근본 원인 분석 + 프로세스
                재설계
              </text>
            </motion.g>
          )}
