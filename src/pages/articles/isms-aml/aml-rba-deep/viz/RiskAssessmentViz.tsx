...
const STEPS = [
  {
    label: "1단계 — 위험 식별",
    body: "고객·상품·지역·채널 4개 범주에서 ML/TF 위험 요소를 찾아낸다.",
  },
  {
    label: "2단계 — 위험 분석",
    body: "발생 가능성(1~5) x 영향도(1~5) = 위험 점수 → 고/중/저 등급 산출.",
  },
  {
    label: "3단계 — 위험 평가",
    body: "고유위험 - 통제효과 = 잔여위험. 수용 가능 수준인지 경영진이 판단.",
  },
  {
    label: "4단계 — 지속 평가",
    body: "정기(연 1회) + 수시(법규 변경, 신상품) 재평가로 환경 변화 반영.",
  },
  {
    label: "위험 매트릭스",
    body: "4단계 결과를 매트릭스로 문서화 — 위험 요소별 점수·통제·잔여위험 기록.",
  },
];
        ...
        >
          ...
          {step === 1 && (
            <motion.g initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
              ...
              {/* Likelihood axis */}
              <text x={30} y={50} fontSize={9} fontWeight={600} fill={C.blue}>
                발생 가능성
              </text>
              <text x={30} y={65} fontSize={8} fill="var(--muted-foreground)">
                Likelihood
              </text>
              {[1, 2, 3, 4, 5].map((n) => (
                <g key={`l${n}`}>
                  <rect
                    ...
                    height={16}
                    rx={3}
                    fill={n >= 4 ? `${C.red}20` : `${C.blue}12`}
                    stroke={n >= 4 ? C.red : C.blue}
                    strokeWidth={0.5}
                  />
                  <text
                    x={40 + (n - 1) * 25}
              ...
              {/* Impact axis */}
              <text x={200} y={50} fontSize={9} fontWeight={600} fill={C.amber}>
                영향도
              </text>
              <text x={200} y={65} fontSize={8} fill="var(--muted-foreground)">
                Impact
              </text>
              {[1, 2, 3, 4, 5].map((n) => (
                <g key={`i${n}`}>
                  <rect
                    ...
                    height={16}
                    rx={3}
                    fill={n >= 4 ? `${C.red}20` : `${C.amber}12`}
                    stroke={n >= 4 ? C.red : C.amber}
                    strokeWidth={0.5}
                  />
                  <text
                    x={210 + (n - 1) * 25}
          ...
          {step === 3 && (
            <motion.g initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
              ...
              {/* Loop back arrow */}
              <path
                d="M 430 60 Q 450 60 450 30 Q 450 10 240 10 Q 50 10 50 30 Q 50 50 50 60"
                stroke={C.green}
                strokeWidth={0.8}
                fill="none"
                markerEnd="url(#ra-arrow)"
