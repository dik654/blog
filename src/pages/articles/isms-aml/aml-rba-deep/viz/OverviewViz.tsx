...
const STEPS = [
  {
    label: "Rule-based vs Risk-based",
    body: "규정 기반은 모든 고객에 동일 기준 → RBA는 위험에 비례하는 차등 적용.",
  },
  {
    label: "RBA 자원 배분 원리",
    body: "고위험에 집중 투입, 저위험은 간소화 → 한정된 컴플라이언스 자원의 효율 극대화.",
  },
  {
    label: "3대 이점: 효율·유연·실효",
    body: "비용 절감(효율), 신규 위험 대응(유연), 선별적 보고로 FIU 역량 집중(실효).",
  },
  {
    label: "FATF R.1 + 국내 법적 근거",
    body: "특금법 제5조의2(CDD 차등), 시행령 10조의5~7(분류 기준), FIU 가이드라인(실무).",
  },
];
        ...
        >
          ...
          {step === 1 && (
            <motion.g initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
              ...
              <text x={195} y={62} fontSize={8} fill={C.red} fontWeight={600}>
                자원 집중
              </text>
              ...
              <text x={265} y={62} fontSize={8} fill={C.warn}>
                표준 CDD
              </text>
              ...
              <text x={330} y={62} fontSize={8} fill={C.risk}>
                간소화
              </text>
          ...
          {step === 3 && (
            <motion.g initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
              ...
              <text
                ...
                fill={C.red}
              >
                RBA 전제: 위험평가가 부실하면 고위험을 저위험으로 오분류 → 체계
                자체가 통로
              </text>
            </motion.g>
          )}
