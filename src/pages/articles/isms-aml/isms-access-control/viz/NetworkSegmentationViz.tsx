  ...
  return (
    <g>
      ...
      <line
        ...
        strokeWidth={0.5}
      />
      <text x={x + 7} y={y + 50} textAnchor="middle" fontSize={7} fill={color}>
        FW
      </text>
    </g>
  );
        ...
        >
          ...
          {step === 1 && (
            <motion.g initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
              ...
              <AlertBox
                ...
                w={420}
                h={40}
                label="DMZ 원칙: 민감 데이터 저장 금지"
                sub="DB·월렛·개인정보는 반드시 내부망에 격리. DMZ에는 캐시·세션만 허용"
                color={C.ext}
              />
            </motion.g>
          )}
          ...
          {step === 2 && (
            <motion.g initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
              ...
              <text x={230} y={156} fontSize={8} fill="var(--muted-foreground)">
                DB: WAS IP에서 DB 포트(3306)만 허용
              </text>
              <text x={230} y={170} fontSize={8} fill="var(--muted-foreground)">
                월렛: 출금 서비스 → 서명 API 엔드포인트만
              </text>
              <text x={230} y={184} fontSize={8} fill="var(--muted-foreground)">
                관리: 점프서버 SSH 경유, MFA 필수
              </text>
              <text x={230} y={198} fontSize={8} fill="var(--muted-foreground)">
                로그: syslog 단방향 전송, 삭제 권한 없음
              </text>
