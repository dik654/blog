...
const STEPS = [
  {
    label: "준비 단계 (2~3개월)",
    body: "정보자산 목록화, 정책 문서 작성, 증적 폴더 구성. 모든 후속 심사의 기초가 되는 산출물을 만드는 단계.",
  },
  {
    label: "서면심사 (2~3주)",
    body: "심사원이 제출된 문서를 검토. 누락 항목과 정책-현실 괴리를 GAP 분석표로 도출.",
  },
  {
    label: "현장심사 (3~5일)",
    body: '심사원이 직접 서버 접속, 웹사이트 확인, 담당자 인터뷰. 문서가 아니라 "실제 동작"을 본다.',
  },
  {
    label: "보완조치 (1~2개월)",
    body: "도출된 결함을 수정하고 증적을 첨부하여 보완조치 내역서를 제출. 완료 공문까지가 한 세트.",
  },
  {
    label: "인증 획득 + 사후관리",
    body: "보완 적합 판정 → 인증서 발급. 3년 유효, 매년 사후심사. 지속적 관리가 핵심.",
  },
];
        ...
        >
          ...
          {step === 0 && (
            <motion.g initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
              ...
              {/* 타임라인 라벨 */}
              <text x={30} y={200} fontSize={9} fontWeight={600} fill={C.prep}>
                준비
              </text>
              <text x={115} y={200} fontSize={9} fill="var(--muted-foreground)">
                서면
              </text>
              <text x={200} y={200} fontSize={9} fill="var(--muted-foreground)">
                현장
              </text>
              <text x={300} y={200} fontSize={9} fill="var(--muted-foreground)">
                보완
              </text>
              <text x={400} y={200} fontSize={9} fill="var(--muted-foreground)">
                인증
              </text>
            </motion.g>
          )}
