...
const STEPS = [
  {
    label: "BEFORE: bcrypt 해싱",
    body: "bcrypt는 비밀번호 해싱에 최적화된 알고리즘. 하지만 국정원 검증 목록에 없어 ISMS 심사에서 결함.",
  },
  {
    label: "AFTER: SHA-256 + Salt",
    body: "국가 검증 기준에 부합하는 SHA-256으로 교체. 사용자별 고유 Salt를 생성하여 레인보우 테이블 공격 방지.",
  },
  {
    label: "2차인증 추가",
    body: "관리자 페이지에 아이디/비밀번호(1차) + 본인인증(2차) 체계 구축. 세션에 second_auth 플래그 기록.",
  },
  {
    label: "장기 미접속 처리",
    body: "last_login 기준 6개월 경과 시 계정복구 페이지로 이동. 본인인증 후 비밀번호 재설정.",
  },
];
        ...
        >
          ...
          {step === 0 && (
            <motion.g initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
              ...
              >
                bcrypt &#8594; 비밀번호 전용, salt 자동 생성, 느린 해시
              </text>
              <text
                x={240}
                y={183}
                ...
                fill={C.fail}
              >
                KISA 검증 알고리즘 목록: SHA-256, ARIA, SEED ...
              </text>
              <text
                x={240}
                y={198}
          ...
          {step === 2 && (
            <motion.g initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
              ...
              <AlertBox
                ...
                w={280}
                h={48}
                label="BEFORE: ID/PW만으로 관리자 접속 가능"
                sub="계정 탈취 시 즉시 민감 데이터 노출"
                color={C.fail}
              />
              ...
              <text
                ...
                fill="var(--muted-foreground)"
              >
                모든 관리 API: 서버에서 세션의 second_auth 값 확인 → 프록시 변조
                우회 불가
              </text>
            </motion.g>
          )}
          ...
          {step === 3 && (
            <motion.g initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
              ...
              <text x={300} y={22} fontSize={8} fill={C.pass}>
                N
              </text>
              <text x={300} y={56} fontSize={8} fill={C.fail}>
                Y
              </text>
