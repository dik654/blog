...
const STEPS = [
  {
    label: "BEFORE: root 하나로 전부",
    body: "root 또는 공용 계정 하나로 서비스, 원장, 백업, 관리 모두 접속. 누가 무슨 작업을 했는지 추적 불가.",
  },
  {
    label: "AFTER: 프로그램 계정 5종",
    body: "용도별로 filadmin, filuser, filledger, petra, backup 계정 분리. 각 계정은 필요한 DB·테이블에만 접근 가능.",
  },
  {
    label: "AFTER: 사람 계정 3종",
    body: "super_admin(DBA), service_admin(개발자), ledger_admin(원장담당). 기본 SELECT만, 추가 권한은 작업신청서로.",
  },
  {
    label: "PETRA 접근제어 흐름",
    body: "모든 DB 접속은 PETRA를 경유. 세션·쿼리·IP 통제 + 쿼리/차단/로그인 로그 → 월간 검토 → 보고서.",
  },
];
        ...
        >
          ...
          {step === 1 && (
            <motion.g initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
              ...
              {/* 권한 표시 (중앙) */}
              <text x={200} y={47} fontSize={8} fill="var(--muted-foreground)">
                DDL/DML 전체
              </text>
              <text x={200} y={87} fontSize={8} fill="var(--muted-foreground)">
                SELECT/INSERT/UPDATE
              </text>
              <text x={200} y={127} fontSize={8} fill="var(--muted-foreground)">
                SELECT/INSERT
              </text>
              <text x={200} y={167} fontSize={8} fill="var(--muted-foreground)">
                테이블/컬럼 목록만
              </text>
              <text x={200} y={207} fontSize={8} fill="var(--muted-foreground)">
                SELECT+LOCK+RELOAD
              </text>
          ...
          {step === 2 && (
            <motion.g initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
              ...
              <text x={295} y={96} fontSize={8} fill={C.db}>
                승인
              </text>
              <text x={295} y={151} fontSize={8} fill={C.db}>
                승인
              </text>
