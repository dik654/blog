...
const STEPS = [
  {
    label: "위탁 시 4가지 필수 조치",
    body: "위탁 계약서 체결(업무 내용·처리 제한·보호조치·재위탁 제한) → 수탁자 공개(처리방침 게시) → 관리 감독(연 1회 점검·교육) → 재위탁 제한(서면 동의).",
  },
  {
    label: "국외 이전 6가지 고지 사항",
    body: "이전받는 자, 이전 국가, 이전 일시·방법, 이전 항목, 이용 목적, 보호조치. 해외 클라우드도 글로벌 관리 인력 접근 시 국외 이전에 해당할 수 있다.",
  },
];
          ...
          {step === 1 && (
            <motion.g initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
              ...
              <line
                ...
                strokeDasharray="6 3"
              />
              <text x={112} y={162} fontSize={7} fill={C.warn}>
                국경
              </text>
              ...
              <AlertBox
                ...
                w={340}
                h={36}
                label="해외 클라우드 한국 리전도 국외 이전 가능성"
                sub="글로벌 관리 인력 접근 시 국외 이전 해당"
                color={C.warn}
              />
