...
const STEPS = [
  {
    label: "법적 근거: 제30조",
    body: "개인정보보호법 제30조 — 모든 개인정보처리자는 처리방침을 수립하고 공개해야 한다. 위반 시 1천만 원 이하 과태료.",
  },
  {
    label: "ISMS-P 3.x 영역 연동",
    body: "처리방침은 ISMS-P 3.x 22개 항목의 외부 공개 요약본. 3.1 수집 ~ 3.5 위탁까지 각 항목이 처리방침의 특정 기재 사항과 직접 매핑된다.",
  },
  {
    label: "필수 기재 8개 항목",
    body: "처리 목적, 수집 항목, 보유기간, 제3자 제공, 위탁, 정보주체 권리, CPO 연락처, 변경이력 — 하나라도 누락되면 부적합.",
  },
  {
    label: "공개 위치와 변경 절차",
    body: "Footer 1클릭 접근, 회원가입 페이지 연결, 앱 설정 메뉴 배치. 변경 시 7일(중요 30일) 전 사전 고지 후 시행.",
  },
];
          ...
          {step === 2 && (
            <motion.g initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
              ...
              <AlertBox
                ...
                w={240}
                h={36}
                label="하나라도 누락 시 부적합 판정"
                sub="시행령 제31조가 세부 사항 규정"
                color={C.warn}
              />
              ...
              <text
                ...
                fill="var(--muted-foreground)"
              >
                VASP: KYC로 수집하는 신분증·계좌·지갑주소 등을 항목 2에 반드시
                포함
              </text>
            </motion.g>
          )}
