...
const STEPS = [
  {
    label: "목적 + 항목 + 보유기간",
    body: '처리 목적은 구체적이어야 한다 ("서비스 개선"은 부적합). 항목은 필수/선택 구분. 보유기간은 법적 근거와 함께 명시.',
  },
  {
    label: "제공 + 위탁 + 권리",
    body: "제3자 제공: 누구에게, 왜, 무엇을. 위탁: 수탁자명과 업무. 정보주체 권리: 열람·정정·삭제·정지 행사 경로.",
  },
  {
    label: "CPO 연락처 + 변경이력",
    body: "CPO(개인정보보호책임자)의 성명·직위·부서·연락처 기재. 변경이력: 변경 일자와 내용을 기록하여 이전 버전과 비교 가능하게.",
  },
];
          ...
          {step === 0 && (
            <motion.g initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
              ...
              <AlertBox
                ...
                w={240}
                h={34}
                label="하나라도 누락 시 ISMS-P 부적합"
                sub="시행령 제31조 세부 사항 규정"
                color={C.warn}
              />
          ...
          {step === 1 && (
            <motion.g initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
              ...
              <text
                ...
                fill="var(--muted-foreground)"
              >
                제공과 위탁은 법적 성격이 다름 — 제공은 별도 동의, 위탁은
                계약+공개
              </text>
            </motion.g>
          )}
