...
const STEPS = [
  {
    label: "동의의 법적 근거",
    body: "제15조(수집·이용) + 제17조(제3자 제공) — 두 동의는 반드시 분리. 하나의 체크박스로 묶으면 무효 처리.",
  },
  {
    label: "필수항목 vs 선택항목",
    body: "제16조 — 선택항목 미동의를 이유로 서비스 거부하면 법 위반. 필수만으로 가입이 완료되어야 한다.",
  },
  {
    label: "적극적 동의 4대 조건",
    body: "명확한 고지, 적극적 의사 표시(사전 체크 금지), 개별 동의(필수/선택/제3자/마케팅 분리), 중요사항 강조.",
  },
  {
    label: "마케팅 동의 + 철회",
    body: "마케팅은 별도 동의 + 2년 주기 재동의. 철회는 수집 경로보다 쉬운 방법으로, 10일 이내 처리 완료.",
  },
  {
    label: "동의 기록 보관",
    body: '동의 일시·방법·내용·버전을 5년 보관. 입증 책임은 처리자에게 — "동의한 적 없다"에 대비.',
  },
];
          ...
          {step === 0 && (
            <motion.g initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
              ...
              <AlertBox
                ...
                w={280}
                h={38}
                label="반드시 별도 체크박스로 분리"
                sub="하나로 묶으면 제22조 위반 → 과태료"
                color={C.withdraw}
              />
          ...
          {step === 1 && (
            <motion.g initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
              ...
              <AlertBox
                ...
                w={320}
                h={32}
                label="선택 미동의 → 서비스 거부 = 법 위반"
                sub="필수항목만으로 가입 완료 가능해야 함"
                color={C.withdraw}
              />
            </motion.g>
          )}
          ...
          {step === 4 && (
            <motion.g initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
              ...
              <text
                ...
                fill="var(--muted-foreground)"
              >
                VASP KYC: 항목별 목적 분리 고지 — 신분증(본인확인),
                계좌(입출금), 셀카(신원검증)
              </text>
            </motion.g>
          )}
