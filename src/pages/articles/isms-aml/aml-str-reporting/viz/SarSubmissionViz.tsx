...
const STEPS = [
  {
    label: "SAR 필수 기재 5항목",
    body: "거래 일시/금액, 계정/지갑, 의심 사유(핵심), 증빙자료, VASP 조치 내역.",
  },
  {
    label: "제출 → 후속 조치",
    body: "준법감시인 승인 → FIU 전자 제출 → 모니터링 유지 + 수사 협조 + 교훈 반영. 끝이 아니라 시작.",
  },
];
          ...
          {step === 0 && (
            <motion.g initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
              ...
              {[
                ...
                >
                  <Arrow
                    ...
                    x2={item.x + 40}
                    y2={78}
                    color={item.highlight ? C.follow : C.write}
                  />
                  {item.highlight ? (
                    <ActionBox
                      x={item.x}
              ...
              >
                ...
                >
                  "가입 3일 차 대학생이 5천만 원 입금 후 30분 내 Tornado Cash
                  2홉 연결 주소로 전액 출금.
                </text>
                <text
                  x={240}
          ...
          {step === 1 && (
            <motion.g initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
              ...
              <AlertBox
                ...
                w={360}
                h={32}
                label="방어적 보고(Defensive Filing) 지양"
                sub="양보다 질 — 구체성 + 논리성이 FIU 평가 기준"
                color={C.warn}
              />
            </motion.g>
          )}
