...
const STEPS = [
  {
    label: "4가지 보관 범주",
    body: "CDD 자료, 거래 내역, SAR 사본+조치 기록, 내부 통제 기록. 모두 5년 이상.",
  },
  {
    label: "보관 방법 5대 요건",
    body: "전자 보관 + 암호화(AES-256) + 접근 통제(RBAC) + 무결성(WORM) + 이중 백업.",
  },
  {
    label: "SAR 사본 특별 관리",
    body: "SAR 내용 유출 = Tipping-off. AML 담당자+준법감시인만 접근. 별도 저장소 필수.",
  },
];
        ...
        >
          ...
          {step === 0 && (
            <motion.g initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
              ...
              />
              <text x={340} y={148} fontSize={8} fill={C.time} fontWeight={600}>
                5년
              </text>
              <text
                x={240}
          ...
          {step === 1 && (
            <motion.g initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
              ...
              <AlertBox
                ...
                w={400}
                h={32}
                label='"보관하고 있지만 못 찾겠다" = 미보관'
                sub="FIU/수사기관 요청 시 즉시 제공 불가하면 동일 제재"
                color={C.red}
              />
            </motion.g>
          )}
          ...
          {step === 2 && (
            <motion.g initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
              ...
              <AlertBox
                ...
                w={400}
                h={38}
                label="SAR 내용 유출 = Tipping-off"
                sub="CS팀·운영팀·일반 직원은 SAR 존재 자체를 모르는 것이 원칙"
                color={C.red}
              />
            </motion.g>
          )}
