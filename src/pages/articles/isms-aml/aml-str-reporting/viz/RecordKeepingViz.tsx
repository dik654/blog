...
const STEPS = [
  {
    label: "보관 대상 4범주",
    body: "CDD 자료(고객확인), 거래 내역(입출금/매매/TX 해시), SAR 사본(제출 전문+경보 로그), 내부 통제 기록(정책/교육/감사). 네 범주 전부 보관 의무.",
  },
  {
    label: "보관 기간 — 최소 5년, 기산점이 다르다",
    body: "CDD: 거래 관계 종료일부터, 거래 내역: 거래 발생일부터, SAR 사본: 제출일부터, 내부 통제: 작성일부터. 수사 진행 중이면 종료까지 연장.",
  },
  {
    label: "보관 방법 — 5대 보안 요건",
    body: '전자적 보관(즉시 검색), 암호화(AES-256), 접근 통제(RBAC), 무결성(WORM/해시 체인), 백업(이중화). "찾을 수 없다" = 미보관.',
  },
  {
    label: "정기 교육 — 대상별 주기와 내용",
    body: "전 직원 연 1회, CS팀 반기 1회, AML 담당자 분기 1회, 경영진 연 1회. 형식적 이행이 아닌 실질적 역량 강화가 목표.",
  },
  {
    label: "교육 기록 = 규제 점검 핵심 증빙",
    body: "참석자 명단, 교육 일시/장소, 교육 자료, 평가 결과, 개선 조치. 미이수자 보충 교육까지 기록해야 완결.",
  },
];
          ...
          {step === 2 && (
            <motion.g initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
              ...
              <AlertBox
                ...
                w={320}
                h={40}
                label="'보관했지만 찾을 수 없다' = 미보관"
                sub="수사기관/FIU 요청 시 즉시 제공 가능해야 의미 있음"
                color={C.warn}
              />
            </motion.g>
          )}
