...
const STEPS = [
  {
    label: "내부 감시 시스템",
    body: "임직원 거래 모니터링 + 비정상 패턴 감지. 24시간 자동화 필수. 규칙 기반 + ML 모델 병행.",
  },
  {
    label: "교육과 내부 신고 체계",
    body: "연 1회 전 임직원 교육 + 신규 입사자 별도. 내부 신고 채널(익명 가능) + 신고자 보호(보복 금지).",
  },
];
        ...
        >
          ...
          {step === 0 && (
            <motion.g initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
              ...
              >
                오탐(false positive) 관리 → 조사 인력 부담과 탐지 정밀도 균형
              </text>
            </motion.g>
          )}
          ...
          {step === 1 && (
            <motion.g initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
              ...
              <text x={270} y={124} fontSize={8} fill="var(--muted-foreground)">
                인사상 불이익 금지 + 신원 비공개
              </text>
              <text x={270} y={136} fontSize={8} fill="var(--muted-foreground)">
                보복 시 징계/법적 조치
              </text>
              ...
              <AlertBox
                ...
                w={320}
                h={30}
                label="기업 부정의 43%가 내부 제보로 발견 (ACFE)"
                sub=""
                color={C.detect}
              />
            </motion.g>
