...
const STEPS = [
  {
    label: "형사처벌 단계별 체계",
    body: "부당이득 규모에 따라 형량이 가중된다. 기본 1년 이상 → 5억~50억이면 3년 이상 → 50억 초과 시 무기징역까지 가능.",
  },
  {
    label: "벌금과 과징금: 이중 제재",
    body: "벌금은 형사(법원), 과징금은 행정(금융위). 벌금 3~5배 + 과징금 2배로 범죄 수익보다 더 큰 불이익을 부과.",
  },
];
          ...
          {step === 1 && (
            <motion.g initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
              ...
              <text
                ...
                fill="var(--muted-foreground)"
              >
                형사(벌금) + 행정(과징금) 동시 부과 가능 → "범죄는 안 남는 장사"
              </text>
            </motion.g>
          )}
        </svg>
