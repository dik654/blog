...
const STEPS = [
  {
    label: "정보 차단벽: 부서 간 분리",
    body: "상장심사팀과 트레이딩팀은 물리적+논리적으로 분리. 출입카드, 시스템 권한, 커뮤니케이션 모두 차단.",
  },
  {
    label: "Wall Crossing: 예외 절차",
    body: "불가피하게 차단벽을 넘어야 할 때 준법감시인 사전 승인 + 정보 수령자 거래 제한 + 전 과정 기록이 필수.",
  },
  {
    label: "정보 관리 체계: 생성~폐기",
    body: "기밀 등급 지정 → 최소 인원 접근 → 생성~폐기 전 과정 기록. 사후 추적 가능한 상태 유지.",
  },
];
        ...
        >
          ...
          {step === 0 && (
            <motion.g initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
              ...
              >
                커뮤니케이션 차단: 업무 관련 대화/이메일/메신저 교환 금지
              </text>
            </motion.g>
          )}
          ...
          {step === 2 && (
            <motion.g initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
              ...
              <text
                ...
                fill="var(--muted-foreground)"
              >
                "누가, 언제, 어떤 정보에 접근했는가" → 전 과정 추적 가능해야
              </text>
              <text
                x={240}
                y={178}
