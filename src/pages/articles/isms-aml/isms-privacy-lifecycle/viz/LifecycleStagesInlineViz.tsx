...
const STEPS = [
  {
    label: "수집 → 이용 → 보관 → 파기",
    body: "수집: 동의 획득, 최소 수집(제15·16·22조). 이용: 목적 범위 내(제17·18조). 보관: 안전성 확보, 분리보관(제29·21조). 파기: 복구 불가(제21조).",
  },
  {
    label: "보유기간 산정 원칙",
    body: '"보유기간 = max(수집 목적 달성 기간, 법령 보존 의무 기간)". 목적 달성 시 즉시 파기가 원칙이나, 법령 보존 의무가 있으면 해당 기간까지 보관 후 파기.',
  },
  {
    label: "보유 → 보존 → 파기 흐름",
    body: "보유(서비스 이용 중) → 목적 달성 → 보존(법정 기간, 분리보관) → 법정 기간 만료 → 파기(복구 불가). 이 흐름이 생명주기 후반부.",
  },
];
          ...
          {step === 1 && (
            <motion.g initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
              ...
              <StatusBox
                ...
                w={200}
                h={36}
                label="보유기간 = 둘 중 더 긴 기간"
                sub="법령 근거와 함께 처리방침에 명시"
                color={C.use}
              />
