...
const STEPS = [
  {
    label: "분리보관 접근통제 5단계",
    body: "최소 인원(CPO+법무+DBA) → 접근 사유 기록 → CPO/CISO 사전 승인 → 접근 로그 자동 기록(2년+) → 정기 감사(분기/반기).",
  },
  {
    label: "회원탈퇴 시 5단계 처리",
    body: "탈퇴 접수(본인인증) → 즉시 파기(보존 의무 없는 항목, 5일 내) → 분리보관 이동(법정 보존 대상) → 활성 DB 삭제 → 보존기간 후 파기.",
  },
];
...
function Arrow({
  ...
  return (
    <line
      ...
      stroke={color}
      strokeWidth={1}
      markerEnd="url(#acl-inline-arrow)"
    />
  );
}
        ...
        >
          ...
          {step === 1 && (
            <motion.g initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
              ...
              <text x={130} y={72} fontSize={8} fill={C.deny}>
                보존 의무 없음
              </text>
              <text x={350} y={72} fontSize={8} fill={C.separate}>
                법정 보존 대상
              </text>
