...
const STEPS = [
  {
    label: "덮어쓰기 3가지 표준 비교",
    body: "DoD 5220.22-M: 3회(0x00→0xFF→랜덤). Gutmann: 35회(자기 잔류 제거, 실무 비현실). NIST SP 800-88: 매체별 Clear/Purge/Destroy 3단계(가장 널리 참조).",
  },
  {
    label: "DB 레코드 파기 3가지 방법",
    body: "DELETE+VACUUM(빈 공간 덮어쓰기), Crypto Erase(암호화 키 폐기, 클라우드 최적), 비식별화(재식별 불가 시 파기 인정). 매체·환경에 따라 적합한 방법 선택.",
  },
];
          ...
          {step === 0 && (
            <motion.g initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
              ...
              <AlertBox
                ...
                w={320}
                h={36}
                label="SSD: 덮어쓰기 불완전 (웨어 레벨링)"
                sub="Secure Erase 또는 Crypto Erase 필요"
                color={C.ssd}
              />
