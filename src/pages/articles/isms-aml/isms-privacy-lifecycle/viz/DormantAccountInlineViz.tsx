...
const STEPS = [
  {
    label: "휴면계정 전환 흐름",
    body: "마지막 접속 → 11개월(30일 전 안내) → 1년(휴면 전환, 분리보관) → 재로그인 시 본인인증 후 복원. 정보통신망법 제29조 제2항.",
  },
  {
    label: "VASP 특이점: 자산 잔액 충돌",
    body: "휴면 이용자 지갑에 가상자산이 남아 있으면 자산 보관 의무와 개인정보 파기 의무가 충돌. 분리보관 + 자산 인출 안내(이메일/문자)를 병행.",
  },
];
          ...
          {step === 1 && (
            <motion.g initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
              ...
              <StatusBox
                ...
                w={320}
                h={38}
                label="해법: 분리보관 + 자산 인출 안내 병행"
                sub="최소 식별정보(지갑주소, 이름) 유지하며 인출 기회 제공"
                color={C.restore}
              />
