...
const STEPS = [
  {
    label: "온체인 vs 오프체인 저장 원칙",
    body: "개인정보(성명·연락처·계좌)는 오프체인 DB에만 저장. 온체인에는 해시·지갑주소 등 식별불가 데이터만. 블록체인 불변성 때문에 온체인 삭제 불가.",
  },
  {
    label: "파기 대장과 증적 관리",
    body: "파기 일자, 대상(항목+건수), 방법, 수행자, 확인자(CPO), 증적(쿼리 로그·캡처)을 기록. 파기 대장 최소 3년 보관, ISMS-P 심사 시 제출.",
  },
];
        ...
        >
          ...
          {step === 0 && (
            <motion.g initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
              ...
              <text x={238} y={45} fontSize={8} fontWeight={600} fill={C.link}>
                매핑
              </text>
              ...
              >
                개인정보를 온체인에 직접 저장하면 파기 의무 이행 불가 → 반드시
                오프체인
              </text>
            </motion.g>
          )}
          ...
          {step === 1 && (
            <motion.g initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
              ...
              <StatusBox
                ...
                w={220}
                h={30}
                label="파기 대장 최소 3년 보관"
                sub="ISMS-P 심사 시 제출 필수"
                color={C.offchain}
              />
            </motion.g>
          )}
