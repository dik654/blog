    ...
    <section id="outbound-connector" className="scroll-mt-20">
      ...
      <div className="prose prose-neutral max-w-none dark:prose-invert">
        <p className="text-lg leading-8">
          첫 번째 구현은 안쪽에 작은 프로그램을 두는 것입니다. 이 프로그램은 시작하자마자 바깥의 중계망으로
          연결을 겁니다. 방화벽 입장에서는 평범한 나가는 연결이라 대부분 기본 설정으로 허용되고, 들어오는 규칙은
          하나도 추가되지 않습니다.
        </p>
      ...
      <div className="prose prose-neutral mt-8 max-w-none dark:prose-invert">
        <p className="leading-7">
          대가는 신뢰 지점이 하나 늘어난다는 것입니다. 이 프로그램은 안쪽 네트워크에서 실행되면서 바깥으로
          나가는 통로를 유지하므로, 프로그램 자체나 그 자격 증명이 탈취되면 그 통로가 공격자의 것이 됩니다.
          방화벽 규칙을 없앤 대신 관리해야 할 비밀이 생긴 셈입니다.
        </p>
        ...
        <p className="leading-7">
          통로가 닿는 범위를 서비스 하나로 못 박으면 이 구조는 서비스 단위 접근이 되고, 안쪽 대역 전체로 열어
          두면 사설망과 다를 바 없어집니다. 같은 기술이 설정에 따라 양쪽 끝을 오간다는 점이 이 방식의 실제
          위험입니다.
        </p>
      </div>
      ...
      <div className="mt-6">
        ...
        >
          <p className="leading-7">
            나가는 연결을 전부 허용하는 환경이라면 이 프로그램은 아무 설정 없이 동작합니다. 편하지만, 같은
            느슨함을 다른 프로그램도 쓸 수 있다는 뜻이기도 합니다. 나가는 방향에 대한 통제가 없다는 사실은
            이 구조가 만든 문제가 아니라 이 구조가 드러낸 문제입니다.
          </p>
          <p className="leading-7">
            반대로 나가는 연결이 목적지별로 통제되는 환경에서는 이 프로그램이 붙을 목적지를 명시적으로 허용해야
            합니다. 이때는 허용 목록이 곧 통제 지점이 되고, 들어오는 규칙을 관리하던 것과 성격만 다를 뿐 관리
            부담은 남습니다.
          </p>
          <p className="leading-7">
