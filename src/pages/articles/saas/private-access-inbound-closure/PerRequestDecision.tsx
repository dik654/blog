  ...
  return (
    <section id="per-request-decision" className="scroll-mt-20">
      ...
      <div className="prose prose-neutral max-w-none dark:prose-invert">
        ...
        <p className="leading-7">
          이 구조에서 사설망 접속은 필수 단계가 아니라 하나의 신호로 내려갑니다. 접속 여부가 권한을 주지 않고 판정에 참고되는 여러 항목 중 하나가 됩니다. 그 결과 접속하지 않은
          상태에서도 조건을 만족하면 쓸 수 있고, 접속한 상태여도 조건이 나쁘면 거절됩니다.
        </p>
      </div>
      ...
      <div className="prose prose-neutral max-w-none dark:prose-invert">
        ...
        <p className="leading-7">
          그래서 이 구조는 짧은 수명을 기본값으로 둡니다. 자격 증명이 몇 시간 단위로 만료되고 계속 쓰려면 다시 판정을 받습니다. 철회가 필요할 때 모든 곳에 취소를 전파하는 대신 다음
          갱신을 거절하기만 하면 되므로 운영이 단순해집니다.
        </p>

        <p className="leading-7">
          비용은 갱신 경로가 가용성의 일부가 된다는 점입니다. 판정 지점이 멈추면 새 요청이 통과하지 못하고 수명이 짧을수록 그 영향이 빨리 도달합니다. 앞 글에서 다룬 감지와 이동의
          시간이 여기서도 그대로 문제가 됩니다.
        </p>
      ...
      <div className="mt-6">
        ...
        >
          ...
          <p className="leading-7">
            달라진 것은 그 지점이 고장 났을 때의 실패 방향입니다. 사설망 게이트웨이는 멈추면 닫히지만 이미 접속한 세션은 한동안 살아 있어 내부에서 계속 움직일 수 있었습니다. 판정
            지점은 멈추면 새 판정이 없으니 짧은 수명과 결합해 더 빨리 전부 닫힙니다.
          </p>
          <p className="leading-7">
