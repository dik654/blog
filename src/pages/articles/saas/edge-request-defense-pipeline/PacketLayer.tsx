    ...
    <section id="packet-layer" className="scroll-mt-20">
      ...
      <div className="prose prose-neutral max-w-none dark:prose-invert">
        ...
        <p className="leading-7">
          먼저 트래픽을 한곳에 모으지 않는 것이 전제입니다. 같은 주소를 여러 지점에서 광고하면 각 지점이 자기 쪽으로 온 트래픽만 처리하므로 한 지점에 감당 못할 양이 몰리는 상황 자체가
          줄어듭니다. 도착한 지점 안에서는 다시 여러 서버로 나눠 보냅니다.
        </p>

        <p className="leading-7">
          서버에 도착한 패킷은 커널 앞단 프로그램을 지납니다. 이 프로그램은 이미 만들어진 완화 규칙을 적용해 해당하는 패킷을 그 자리에서 버리고 동시에 표본을 분석기로 보냅니다. 공개된
          설명에 따르면 이 방식은 CPU 코어 하나로 초당 천만 패킷 이상을 버릴 수 있습니다.
        </p>

        <p className="leading-7">
          여기서 중요한 점은 규칙이 사람 손으로 만들어지지 않는다는 것입니다. 표본을 받은 분석기가 공격 트래픽의 공통 특징을 찾아 규칙을 생성하고 그 규칙을 앞단으로 내려보냅니다. 사람이
          대시보드를 보고 대응하기에는 공격이 너무 빠르게 시작되고 끝납니다.
        </p>
      </div>
      ...
      <div className="prose prose-neutral max-w-none dark:prose-invert">
        <p className="leading-7">
          분석기가 하는 일은 표본 패킷들에서 공통되는 필드 조합을 찾는 것입니다. 출발지 포트, 패킷 길이, 프로토콜 플래그처럼 여러 필드의 조합을 만들어 봅니다. 그중 공격 트래픽만 잘
          걸러 내면서 정상 트래픽을 적게 건드리는 조합을 고릅니다.
        </p>

        <p className="leading-7">
          이 선택이 핵심입니다. 지나치게 좁은 지문은 공격자가 필드 하나만 바꿔도 빠져나가고 지나치게 넓은 지문은 정상 트래픽까지 버립니다. 그래서 후보 지문을 여러 개 만들어 놓고
          관측되는 트래픽 분포에서 가장 효율적인 것을 고르는 방식이 쓰입니다.
        </p>

        <p className="leading-7">
          규칙이 어느 층에 놓이는지도 비용으로 결정됩니다. 양이 아주 많으면 더 앞단으로 내려 커널 이전에 버리고 그렇지 않으면 상위 층에서 차단이나 챌린지 같은 부드러운 조치를 씁니다.
          같은 공격이라도 규모에 따라 대응 위치가 바뀝니다.
        </p>
      </div>
      ...
      <div className="mt-6">
        ...
        >
          <p className="leading-7">
            패킷이 커널의 일반 처리 경로에 들어가면 자료구조 할당과 여러 계층 통과가 시작됩니다. 버릴 패킷에도 이 비용이 들기 때문에 공격 규모가 커지면 규칙이 맞아도 서버가 먼저
            지칩니다.
          </p>
          <p className="leading-7">
