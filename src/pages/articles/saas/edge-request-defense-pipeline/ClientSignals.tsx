    ...
    <section id="client-signals" className="scroll-mt-20">
      ...
      <div className="prose prose-neutral max-w-none dark:prose-invert">
        ...
        <p className="leading-7">
          그래서 이 메시지를 해시해 지문으로 쓰는 방법이 자리 잡았습니다. 초기 방식은 확장 순서에 의존해 무작위화에 약했습니다. 이후 방식은 순서 의존을 줄이고 상위 프로토콜 협상 정보
          같은 차원을 더해 더 안정적인 지문을 만듭니다.
        </p>

        <p className="leading-7">
          이 신호의 가치는 얻는 시점에 있습니다. 연결이 설정되는 동안 공짜로 도착하므로 스크립트가 돌기 전, 쿠키가 생기기 전에 이미 알 수 있습니다. 요청 내용을 보기 전에 대략의
          분류가 가능하다는 뜻이고 값싼 층에서 판단할 재료가 하나 늘어납니다.
        </p>
      ...
      <div className="prose prose-neutral max-w-none dark:prose-invert">
        <p className="leading-7">
          지문 하나로 결론을 내리지는 않습니다. 여러 신호를 모아 하나의 점수로 만들고 그 점수의 구간에 따라 통과·확인·차단을 배정합니다. 규칙은 이 점수를 조건으로 쓰기 때문에 운영자는
          임계만 조정하면 됩니다.
        </p>

        <p className="leading-7">
          점수화의 이점은 결정을 한곳에 모은다는 것입니다. 신호가 추가되거나 모델이 바뀌어도 규칙은 그대로 두고 점수 계산만 바꾸면 됩니다. 반대로 단점은 왜 이 점수가 나왔는지가
          운영자에게 잘 보이지 않는다는 점입니다. 그래서 지문별 관측 이력을 함께 제공하는 기능이 따라붙습니다.
        </p>

        <p className="leading-7">
          허용 목록도 이 층에 있습니다. 검색 엔진 수집기처럼 자동화이지만 막으면 안 되는 클라이언트가 있기 때문입니다. 이들은 별도로 검증된 목록으로 관리되며 자칭이 아니라 역방향 조회
          같은 확인을 거칩니다.
        </p>
      </div>
