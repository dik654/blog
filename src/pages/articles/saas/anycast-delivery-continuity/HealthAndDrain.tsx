    ...
    <section id="health-and-drain" className="scroll-mt-20">
      ...
      <div className="prose prose-neutral max-w-none dark:prose-invert">
        ...
        <p className="leading-7">
          가장 얕은 검사는 포트가 열려 있는지 보는 것입니다. 프로세스가 살아 있으면 통과하므로 데이터베이스 연결이 모두 끊긴 서버도 건강하다고 답합니다. 반대로 가장 깊은 검사는 실제
          종속성까지 확인하는 것인데, 이번에는 공용 데이터베이스가 느려지면 모든 서버가 동시에 아프다고 답합니다.
        </p>

        <p className="leading-7">
          두 극단이 각각 다른 방식으로 틀립니다. 얕으면 고장 난 서버에 계속 트래픽이 가고 깊으면 공용 문제 하나가 전체를 한꺼번에 제외시켜 남는 용량이 없어집니다. 그래서 실무에서는 그
          서버 자신이 책임질 수 있는 범위까지만 검사하고 공용 종속성의 문제는 별도 신호로 다룹니다.
        </p>
      </div>
      ...
      <div className="prose prose-neutral mt-8 max-w-none dark:prose-invert">
        ...
        <p className="leading-7">
          빼기가 성립하려면 두 가지가 필요합니다. 분배기가 "새 연결은 주지 말고 기존 연결은 유지"라는 중간 상태를 표현할 수 있어야 하고 남은 연결이 실제로 끝날 때까지 기다릴 최대
          시간이 정해져 있어야 합니다. 그 시간이 지나면 남은 연결은 끊고 진행합니다.
        </p>
      ...
      <div className="mt-6">
        ...
        >
          <p className="leading-7">
            주기 2초에 연속 3회 실패를 조건으로 두면 감지에 최대 6초가 걸립니다. 이 6초 동안 그 서버로 가는 요청은 계속 실패하므로 감지 시간은 그대로 장애 시간의 하한이 됩니다.
          </p>
          <p className="leading-7">
            그렇다고 주기 200밀리초에 1회 실패로 잡으면 순간적인 지연이나 검사 패킷 유실만으로 멀쩡한 서버가 빠집니다. 빠진 서버의 몫이 남은 서버로 가면서 그쪽이 느려지고 다시
            빠지는 흔들림이 생깁니다.
          </p>
          <p className="leading-7">
            실무에서는 빼는 조건과 되돌리는 조건을 다르게 둡니다. 빼는 것은 비교적 빠르게, 되돌리는 것은 더 오래 관찰한 뒤에 합니다. 흔들림을 줄이는 가장 단순한 방법입니다.
          </p>
        </ProgressiveDetail>
      </div>
    </section>
