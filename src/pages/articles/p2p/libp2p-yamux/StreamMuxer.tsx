...
export default function StreamMuxer({
  onCodeRef,
}: {
  ...
  return (
    <section id="stream-muxer" className="mb-16 scroll-mt-20">
      ...
      <div className="prose prose-neutral dark:prose-invert max-w-none mb-6">
        <p>
          Yamux <code>Muxer</code>는 libp2p의 <strong>StreamMuxer</strong>{" "}
          트레이트를 구현한다.
          <br />
          세 가지 poll 메서드가 핵심이다.
          <br />
          ...
          >
                ...
                {step.flow.map((f, i) => (
                  ...
                  >
                    ...
                    {i < step.flow.length - 1 && (
                      <span className="text-xs text-foreground/30">&rarr;</span>
                    )}
                  </motion.div>
                ))}
              </div>
            ...
            >
              <p className="text-[11px] text-foreground/60">
                <strong style={{ color: step.color }}>설계 판단:</strong>{" "}
                {step.why}
              </p>
            </div>
          </motion.div>
