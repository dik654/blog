...
const SOCKET_OPTS = [
  {
    ...
    why: "Nagle 알고리즘 비활성화",
    detail:
      "P2P 메시지는 작고 빈번하다. Nagle이 200ms 버퍼링하면 gossip 전파 지연이 누적된다.",
  ...
  },
  {
    ...
    detail:
      "노드 재시작 시 이전 소켓이 TIME_WAIT 상태라도 같은 포트를 즉시 바인드할 수 있다.",
    decision: "빠른 재시작",
    color: "#f59e0b",
  },
  {
    ...
    why: "NAT 홀펀칭용 포트 공유",
    detail:
      "리스닝 포트와 같은 포트로 dial해야 NAT 매핑을 재사용할 수 있다. Unix 전용.",
    decision: "NAT 우회",
    color: "#8b5cf6",
  },
  {
    ...
    why: "비동기 I/O",
    detail:
      "tokio/async-std 런타임이 poll 기반으로 소켓을 구동한다. 블로킹이면 스레드가 점유된다.",
    decision: "async 호환",
    color: "#06b6d4",
  },
];
    ...
    <section id="socket-creation" className="mb-16 scroll-mt-20">
      ...
      <div className="prose prose-neutral dark:prose-invert max-w-none mb-6">
        <p>
          ...
          <strong>socket2</strong> 크레이트로 OS 소켓을 만들고, 4가지 옵션을
          설정한다.
          <br />각 옵션에는 P2P 환경에서의 명확한 이유가 있다.
        </p>
      </div>
      ...
      <div className="prose prose-neutral dark:prose-invert max-w-none">
        ...
        <p>
          Nagle 알고리즘은 작은 패킷을 모아서 보낸다. HTTP처럼 대량 전송에는
          효율적이다.
          <br />
          하지만 P2P에서는 <strong>수십 바이트짜리 gossip 메시지</strong>가
          핵심이다. 200ms 지연이 합의 라운드에 누적되면 블록 전파가 느려진다.
        </p>
        <p>
          <code>SO_REUSEPORT</code>는 <strong>Unix 전용</strong>이고,
          <code>PortUse::Reuse</code>일 때만 활성화된다.
          <br />이 조건부 설정이 중요하다. 불필요한 포트 공유는 보안 위험이 있기
          때문이다.
        </p>
      </div>
      ...
      {onCodeRef && (
        <div className="not-prose flex flex-wrap gap-2 mt-6">
          <CodeViewButton
            onClick={() => onCodeRef("tcp-socket", codeRefs["tcp-socket"])}
          />
