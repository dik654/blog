...
const DIAL_STEPS = [
  {
    step: 1,
    label: "Multiaddr 파싱",
    desc: "/ip4/1.2.3.4/tcp/9000 → SocketAddr",
    color: "#64748b",
  },
  {
    step: 2,
    label: "소켓 생성",
    desc: "create_socket() — 옵션 설정 완료",
    color: "#10b981",
  },
  {
    step: 3,
    label: "포트 재사용 판단",
    desc: "port_reuse.local_dial_addr() + PortUse::Reuse",
    color: "#f59e0b",
  },
  {
    step: 4,
    label: "비동기 connect",
    desc: "EINPROGRESS는 정상 — 논블로킹 connect의 in-progress 응답",
    color: "#8b5cf6",
  },
  {
    step: 5,
    label: "폴백",
    desc: "바인드 실패 시 PortUse::New로 새 소켓 생성 후 재시도",
    color: "#ef4444",
  },
];
...
const LISTEN_EVENTS = [
  {
    event: "NewAddress",
    desc: "OS가 할당한 실제 주소를 Swarm에 보고",
    color: "#10b981",
  },
  {
    event: "Incoming",
    desc: "새 인바운드 연결 → upgrade Future 전달",
    color: "#06b6d4",
  },
  {
    event: "AddressExpired",
    desc: "인터페이스 변경 시 주소 만료 보고",
    color: "#f59e0b",
  },
  {
    event: "ListenerError",
    desc: "비치명적 에러 — 리스닝 계속됨",
    color: "#ef4444",
  },
];
    ...
    <section id="dial-listen" className="mb-16 scroll-mt-20">
      ...
      <div className="prose prose-neutral dark:prose-invert max-w-none">
        <h3 className="text-xl font-semibold mt-2 mb-3">
          왜 EINPROGRESS를 무시하는가?
        </h3>
        <p>
          논블로킹 소켓에서 <code>connect()</code>는 즉시 완료되지 않는다.
          <br />
          OS가 <strong>EINPROGRESS</strong>를 반환하면 "SYN을 보냈고 응답을
          기다리는 중"이라는 뜻이다.
          <br />
        ...
        <p>
          ...
          <code>PortUse::Reuse</code>일 때만 바인드를 시도한다.
          <br />
          실패하면 새 포트로 폴백한다. 이 <strong>시도 → 폴백</strong> 패턴이
          핵심이다.
        </p>
      </div>
      ...
      <div className="prose prose-neutral dark:prose-invert max-w-none mt-6">
        <p>
          <strong>IfWatcher</strong>는 네트워크 인터페이스 변경을 감지한다.
          <br />
          WiFi 전환, VPN 연결 등으로 IP가 바뀌면 AddressExpired → NewAddress를
          발행한다.
          <br />
      ...
      {onCodeRef && (
        <div className="not-prose flex flex-wrap gap-2 mt-6">
          ...
          <CodeViewButton
            onClick={() =>
              onCodeRef("transport-event", codeRefs["transport-event"])
            }
          />
          <span className="text-[10px] text-muted-foreground self-center">
            TransportEvent
