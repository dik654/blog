  ...
  return (
    <section id="enr" className="mb-16 scroll-mt-20">
      ...
      <div className="prose prose-neutral dark:prose-invert max-w-none">
        <p>
          EIP-778이 정의한 자기 인증(self-certified) 노드 레코드입니다. 노드가
          자신의 정보를 서명하여 배포합니다.
          <br />
          최대 <strong>300바이트</strong>로 제한되며, 키-값 쌍은 키 기준 사전순
          정렬을 보장합니다.
        </p>
        <h3 className="text-xl font-semibold mt-6 mb-3">Record 구조체</h3>
        <p>
          주요 키: <code>id</code>(identity scheme), <code>secp256k1</code>
          (공개키), <code>ip</code>, <code>tcp</code>, <code>udp</code>,{" "}
          <code>eth</code>(fork ID). RLP 인코딩 형태:{" "}
          <code>[sig, seq, k1, v1, k2, v2, ...]</code>.
        </p>
        <CodePanel
          title="enr.Record -- enr/enr.go"
          code={RECORD_STRUCT}
          startLine={86}
          annotations={[
            {
              lines: [87, 87],
              color: "sky",
              note: "변경마다 단조 증가하는 시퀀스",
            },
            {
              lines: [90, 90],
              color: "emerald",
              note: "키 기준 정렬 필수 (디코딩 시 검증)",
            },
            {
              lines: [98, 98],
              color: "amber",
              note: "UDP 패킷 크기 제한에 맞춘 상한",
            },
          ]}
        />
        <h3 className="text-xl font-semibold mt-6 mb-3">Set과 자동 seq 증가</h3>
        <p>
          <code>Set()</code>으로 엔트리를 수정하면 <code>invalidate()</code>가
          호출됩니다.
          <br />
          이미 서명된 레코드라면 seq를 1 증가시키고 서명/raw를 nil로
          초기화합니다.
          <br />
          다음에 <code>Node()</code>를 호출할 때 새로 서명됩니다.
        </p>
        <CodePanel
          title="Set & invalidate -- enr/enr.go"
          code={SET_INVALIDATE}
          startLine={150}
          annotations={[
            { lines: [153, 153], color: "sky", note: "수정 시 서명 무효화" },
            {
              lines: [163, 163],
              color: "emerald",
              note: "서명 상태에서만 seq 증가",
            },
          ]}
        />
        <h3 className="text-xl font-semibold mt-6 mb-3">
          LocalNode: IP 추적과 ENR 관리
        </h3>
        <p>
          <code>LocalNode</code>는 로컬 노드의 ENR을 관리합니다.
          <br />
          다른 노드로부터 받은 Pong의 <code>To</code> 필드를{" "}
          <strong>IPTracker</strong>에 수집합니다.
          <br />
          충분한 진술(10개 이상)이 모이면 외부 IP를 예측하여 ENR에 반영합니다.
          <br />
          이를 통해 NAT 뒤에서도 올바른 외부 주소를 광고할 수 있습니다.
        </p>
        <CodePanel
          title="LocalNode -- enode/localnode.go"
          code={LOCAL_NODE}
          startLine={47}
          annotations={[
            {
              lines: [48, 48],
              color: "sky",
              note: "atomic.Pointer로 lock-free 읽기",
            },
            {
              lines: [55, 56],
              color: "emerald",
              note: "IPv4/IPv6 별도 endpoint predictor",
            },
            {
              lines: [61, 61],
              color: "amber",
              note: "Pong.To를 수집하여 외부 IP 예측",
            },
          ]}
        />
