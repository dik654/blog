  ...
  return (
    <section id="overview" className="mb-16 scroll-mt-20">
      ...
      <div className="prose prose-neutral dark:prose-invert max-w-none mb-6">
        <p>
          <strong>GossipSub</strong>는 메시(mesh) 기반 pub/sub 프로토콜이다.
          <br />
          토픽마다 <strong>D=6개 메시 피어</strong>를 유지하고, 나머지에게는
          gossip(IHAVE/IWANT)으로 전파한다.
          <br />왜 flood가 아닌 mesh? 모든 피어에게 보내면 대역폭이 O(n)으로
          폭발한다. mesh는 O(D)로 제한하면서, gossip으로 누락을 복구한다.
        </p>
      </div>
      ...
      <div className="rounded-xl border border-border bg-card p-5 mb-6">
        ...
        <div className="flex flex-col gap-2">
          {MESH_VS_FLOOD.map((m, i) => (
            ...
            >
              <span
                className="text-xs font-mono font-bold w-24 shrink-0"
                style={{ color: m.color }}
              >
                {m.method}
              </span>
      ...
      {onCodeRef && (
        <div className="not-prose flex flex-wrap gap-2 mt-6">
          <CodeViewButton
            onClick={() =>
              onCodeRef("gossipsub-struct", codeRefs["gossipsub-struct"])
            }
          />
