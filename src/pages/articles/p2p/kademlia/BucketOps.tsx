...
export default function BucketOps({
  onCodeRef,
}: {
  ...
  return (
    <section id="bucket-ops" className="mb-16 scroll-mt-20">
      ...
      <div className="prose prose-neutral dark:prose-invert max-w-none">
        <p>
          K-버킷은 단순 배열이 아니다. 노드 추가/삭제마다{" "}
          <strong>IP 쿼터, 교체 목록, 활성도 검증</strong>을 확인한다.
          go-ethereum <code>table.go</code>의 핵심 로직을 코드 레벨로 추적한다.
        </p>
        ...
        {onCodeRef && (
          <div className="not-prose flex flex-wrap gap-2 my-4">
            <CodeViewButton
              onClick={() =>
                onCodeRef("geth-table-struct", codeRefs["geth-table-struct"])
              }
            />
            <span className="text-[10px] text-muted-foreground self-center">
              table.go — handleAddNode
            </span>
          </div>
        )}
        ...
        <p>
          FIND_NODE 응답으로 새 노드를 발견하면 <code>addFoundNode</code> → 채널
          → <code>handleAddNode</code>로 처리.
          <br />
          동기적으로 한 번에 하나씩 처리하여 경합을 방지한다.
        </p>
        <CodePanel
          title="handleAddNode 핵심 흐름"
          lang="go"
          code={`func (tab *Table) handleAddNode(req addNodeOp) bool {
    ...
    return false
        ...
        <p>
          버킷이 가득 찼을 때 새 노드를 바로 버리지 않고{" "}
          <strong>교체 목록</strong>(최대 10개)에 보관한다.
          <br />
          기존 노드가 재검증에서 실패하면, 교체 목록의 가장 최근 노드가
          승격된다.
          <br />이 구조 덕분에 죽은 노드가 빠지면 즉시 대체 노드로 채울 수 있다.
        </p>
        ...
        <CodePanel
          title="deleteInBucket"
          lang="go"
          code={`func (tab *Table) deleteInBucket(b *bucket, id enode.ID) *tableNode {
    // 버킷에서 노드 제거
    n := b.removeNode(id)
    if n == nil { return nil }
    ...
    return n
          ...
          annotations={[
            { lines: [2, 4], color: "sky", note: "버킷에서 노드 제거" },
            {
              lines: [6, 7],
              color: "amber",
              note: "IP 해제 + 재검증 큐에서 제거",
            },
            {
              lines: [9, 14],
              color: "emerald",
              note: "교체 목록 마지막(가장 최근) → 버킷으로 승격",
            },
          ]}
        />
