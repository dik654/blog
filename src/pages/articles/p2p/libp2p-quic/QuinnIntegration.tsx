...
const FIELDS = [
  {
    name: "quinn_config",
    desc: "QuinnConfig — 클라이언트/서버/엔드포인트 설정",
    color: "#8b5cf6",
  },
  {
    name: "listeners",
    desc: "SelectAll<Listener<P>> — 리스너 스트림 통합",
    color: "#10b981",
  },
  {
    name: "dialer",
    desc: "HashMap<SocketFamily, Endpoint> — 리스너 없을 때 다이얼러",
    color: "#f59e0b",
  },
  {
    name: "hole_punch_attempts",
    desc: "HashMap<SocketAddr, Sender> — 홀펀칭 대기",
    color: "#ef4444",
  },
];
...
const REASONS = [
  {
    lib: "quinn",
    why: "Rust QUIC 중 가장 성숙. tokio 네이티브, RFC 9000 완전 구현",
    pick: true,
  },
  {
    lib: "quiche",
    why: "Cloudflare C 라이브러리 바인딩. FFI 오버헤드 존재",
    pick: false,
  },
  {
    lib: "s2n-quic",
    why: "AWS 제작. 안정적이나 libp2p 통합 어려움",
    pick: false,
  },
];

export default function QuinnIntegration({
  onCodeRef,
}: {
  ...
  return (
    <section id="quinn-integration" className="mb-16 scroll-mt-20">
      ...
      <div className="prose prose-neutral dark:prose-invert max-w-none">
        ...
        <p>
          Rust QUIC 구현은 세 가지다. libp2p가 quinn을 선택한 이유는 명확하다.
        </p>
      </div>
      ...
      <div className="prose prose-neutral dark:prose-invert max-w-none">
        <p>
          <strong>Endpoint 생성:</strong> <code>new_endpoint()</code>는
          quinn::Endpoint를 래핑한다.
          <br />
          Provider 트레이트로 런타임을 추상화하고, tokio 플래그가 필요하다.
        </p>
        <p>
          <strong>Listener 구조:</strong> listen_on() 호출마다 Listener를
          생성한다.
          <br />
        ...
        {onCodeRef && (
          <div className="not-prose flex flex-wrap gap-2 my-4">
            <CodeViewButton
              onClick={() =>
                onCodeRef("quic-transport", codeRefs["quic-transport"])
              }
            />
