...
const STEPS = [
  {
    label: "dial(Listener)",
    desc: "eligible_listener()로 소켓 선택",
    color: "#8b5cf6",
  },
  {
    label: "hole_puncher()",
    desc: "UDP 패킷 → NAT 매핑 생성",
    color: "#f59e0b",
  },
  {
    label: "Sender 등록",
    desc: "hole_punch_attempts에 저장",
    color: "#06b6d4",
  },
  {
    label: "poll() 매칭",
    desc: "send_back_addr와 HashMap 비교",
    color: "#10b981",
  },
  {
    label: "Sender 전달",
    desc: "sender.send(upgrade) → receiver 수신",
    color: "#ef4444",
  },
];
...
const CMP = [
  {
    a: "NAT 매핑",
    tcp: "포트 고정 필요, 타임아웃 짧음",
    quic: "UDP 매핑 유지 쉬움",
    c: "#06b6d4",
  },
  {
    a: "SYN 충돌",
    tcp: "동시 SYN → NAT 드롭 가능",
    quic: "Initial 패킷 → 충돌 없음",
    c: "#10b981",
  },
  {
    a: "포트 재사용",
    tcp: "SO_REUSEPORT (OS 의존)",
    quic: "Endpoint 소켓 공유",
    c: "#f59e0b",
  },
];
  ...
  return (
    <section id="hole-punching" className="mb-16 scroll-mt-20">
      ...
      <div className="rounded-xl border border-border bg-card p-5 mb-6">
        ...
        <div className="flex flex-col gap-2">
          {STEPS.map((s, i) => (
            ...
            >
              <span
                className="text-[10px] font-mono font-bold w-5 shrink-0"
                style={{ color: s.color }}
              >
                {i + 1}
              </span>
      ...
      <div className="prose prose-neutral dark:prose-invert max-w-none">
        ...
        <p>
          핵심은 UDP다. TCP는 동시 SYN이 NAT에서 드롭될 수 있다.
          <br />
      ...
      <div className="prose prose-neutral dark:prose-invert max-w-none">
        <p>
          <strong>poll() 매칭:</strong> Incoming의 send_back_addr에서
          SocketAddr를 추출한다. hole_punch_attempts에 키가 있으면
          sender.send()로 전달, 없으면 일반 인바운드로 처리한다. send 실패 시
          upgrade를 Incoming으로 되돌린다. 연결을 버리지 않는 방어적 설계다.
        </p>
        {onCodeRef && (
