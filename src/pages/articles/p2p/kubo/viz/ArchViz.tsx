...
import {
  OVERVIEW_STEPS, ARCH_NODES, ARCH_EDGES, ARCH_VN, ARCH_VE,
} from '../OverviewData';
...
export default function ArchViz() {
  return (
    <StepViz steps={OVERVIEW_STEPS}>
      {(step) => (
        <svg viewBox="-70 0 400 225" className="w-full max-w-2xl" style={{ height: 'auto' }}>
          {ARCH_EDGES.map((e, i) => {
            ...
            return (
              <motion.g key={i} animate={{ opacity: vis ? 0.7 : 0.06 }} transition={sp}>
                ...
                <rect x={(fx + tx) / 2 + 4 - 26} y={(fy + ty) / 2 - 10} width={52} height={14} rx={2} fill="var(--card)" />
            ...
            return (
              <motion.g key={n.id} animate={{ opacity: vis ? 1 : 0.1 }} transition={sp}>
                ...
                <text x={n.x + BW / 2} y={n.y + BH / 2 + 4}
                  textAnchor="middle" fontSize={10} fontWeight={600} fill={n.color}>{n.label}</text>
              </motion.g>
            );
          })}
        </svg>
