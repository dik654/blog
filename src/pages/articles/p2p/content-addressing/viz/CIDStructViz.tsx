...
const STEPS = [
  {
    label: "Multibase 접두사",
    body: "b=base32, z=base58btc — 인코딩 자기 설명",
  },
  { label: "Version 필드", body: "0x01=CIDv1, CIDv0은 legacy sha256 전용" },
  {
    label: "Multicodec",
    body: "dag-pb(0x70), dag-cbor(0x71), raw(0x55) 디코딩 방법 내장",
  },
  {
    label: "Multihash",
    body: "해시 함수 코드 + 다이제스트 길이 + 해시값으로 알고리즘 교체 호환",
  },
];
...
const POSITIONS = PARTS.map(
  (_, index) =>
    20 + PARTS.slice(0, index).reduce((sum, part) => sum + part.w + 6, 0),
);
