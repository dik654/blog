...
const STEPS = [
  ...
  {
    label: "중간 노드 (서브디렉토리)",
    body: "자식 변경 시 이 노드의 CID도 변경",
  },
  {
    label: "Leaf 노드 (파일)",
    body: "실제 데이터를 담는 말단 노드, CID = hash(data)",
  },
  {
    label: "변경 시 연쇄 갱신",
    body: "lib.rs 수정이 Root CID까지 전파 — 루트로 전체 검증",
  },
];
        ...
        >
          ...
          {EDGES.map(([fi, ti], i) => {
            const f = NODES[fi],
              t = NODES[ti];
            const changed =
              step === 3 && ([0, 2, 4].includes(fi) || [0, 2, 4].includes(ti));
            return (
              <motion.line
                key={i}
                x1={f.x + BW / 2}
