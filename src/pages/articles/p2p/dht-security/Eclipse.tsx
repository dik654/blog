    ...
    <section id="eclipse" className="mb-16 scroll-mt-20">
      ...
      <div className="prose prose-neutral dark:prose-invert max-w-none">
        ...
        <p>
          ...
          <br />
          대상 노드는 네트워크를 볼 때 공격자의 노드만 보인다 — 네트워크에서
          "일식"당한 상태.
          <br />
          트랜잭션 전파를 차단하거나, 거짓 블록을 전달하거나, 이중지불 공격에
          활용된다.
        </p>
        <p>
          공격 벡터: 대상 노드에 계속 새 노드를 주입하여 모든 k-버킷을 악성
          노드로 채운다.
          <br />
        ...
        <p>
          버킷에 빈 자리가 없으면, 새 노드는 <code>replacements</code> 목록에
          저장된다(최대 10개).
          <br />
          기존 노드가 재검증에 실패하여 삭제될 때만, 교체 목록에서 랜덤으로
          하나를 승격시킨다.
          <br />
          공격자가 교체 목록을 채워도, 정상 노드가 죽지 않는 한 버킷에 진입할 수
          없다.
        </p>

        <h3>방어 3: 재검증으로 가짜 노드 제거</h3>
        <p>
          공격자의 가짜 노드가 교체 목록이나 버킷에 진입하더라도, 재검증에서
          PING에 응답하지 못하면 삭제된다.
          <br />
          fast list에서 빠르게 검증하므로, 응답하지 않는 가짜 노드는 수 초 내에
          걸러진다.
          <br />세 방어가 겹쳐 작동하여, Eclipse 공격의 성공 확률을 극히 낮춘다.
        </p>
