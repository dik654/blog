  ...
  return (
    <section id="handshake" className="mb-16 scroll-mt-20">
      ...
      <div className="prose prose-neutral dark:prose-invert max-w-none">
        <p>
          discv5 패킷 헤더의 <code>Flag</code> 필드가 핸드셰이크 상태를
          결정한다.
        </p>
        <div className="not-prose grid grid-cols-3 gap-3 my-6">
          {[
            ...
            {
              flag: "2 (flagHandshake)",
              desc: "핸드셰이크 응답 + 암호화 메시지",
            },
          ].map((f) => (
            <div
              key={f.flag}
        ...
        <h3>핸드셰이크 트리거</h3>
        <p>
          세션 없는 노드의 패킷은 복호화 불가. <code>handleUnknown</code>이
          WHOAREYOU 챌린지를 전송한다.
        </p>
        <div className="not-prose flex flex-wrap gap-2 my-4">
          ...
          <span className="text-[10px] text-muted-foreground self-center">
            handleUnknown() — WHOAREYOU 챌린지 생성
          </span>
        </div>
        ...
        <p>
          ...
          입력:{" "}
          <code>
            SHA256("discovery v5 identity proof" | challengeData | ephPubkey |
            destID)
          </code>
          .
        </p>
        <div className="not-prose flex flex-wrap gap-2 my-4">
          <CodeViewButton
            onClick={() =>
              onCodeRef("id-nonce-hash", codeRefs["id-nonce-hash"])
            }
          />
          <span className="text-[10px] text-muted-foreground self-center">
            idNonceHash() — 서명 해시 계산
          </span>
        </div>

        <h3>핸드셰이크 응답 재전송</h3>
        <p>
          WHOAREYOU를 받으면 <code>handleWhoareyou</code>가 원래 호출을
          핸드셰이크 패킷(flag=2)으로 재전송한다.
          <br />
        ...
        <div className="not-prose flex flex-wrap gap-2 my-4">
          <CodeViewButton
            onClick={() =>
              onCodeRef("handle-whoareyou", codeRefs["handle-whoareyou"])
            }
          />
          <span className="text-[10px] text-muted-foreground self-center">
            handleWhoareyou() — 핸드셰이크 재전송
          </span>
        </div>
