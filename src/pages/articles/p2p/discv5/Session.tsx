  ...
  return (
    <section id="session" className="mb-16 scroll-mt-20">
      ...
      <div className="prose prose-neutral dark:prose-invert max-w-none">
        ...
        <h3>ECDH 공유 비밀</h3>
        <p>
          임시(ephemeral) 개인키와 수신자 정적 공개키로 스칼라 곱셈을 수행한다.
          <br />
        ...
        <div className="not-prose flex flex-wrap gap-2 my-4">
          <CodeViewButton
            onClick={() => onCodeRef("derive-keys", codeRefs["derive-keys"])}
          />
          <span className="text-[10px] text-muted-foreground self-center">
            deriveKeys() — HKDF 세션 키 도출
          </span>
        </div>

        <h3>키 방향: keysFlipped</h3>
        <p>
          발신자의 <code>writeKey</code>는 수신자의 <code>readKey</code>다.
          <br />
        ...
        <div className="not-prose flex flex-wrap gap-2 my-4">
          <CodeViewButton
            onClick={() => onCodeRef("encrypt-gcm", codeRefs["encrypt-gcm"])}
          />
          <span className="text-[10px] text-muted-foreground self-center">
            encryptGCM() — AES-GCM 암호화
          </span>
        </div>
        ...
        <div className="not-prose rounded-lg border border-violet-500/20 bg-violet-500/5 p-4 my-6">
          ...
          <p className="text-sm mt-1">
            확립된 세션은 <code>(srcID, addr)</code> 키로 LRU(용량 1024)에
            저장된다.
            <br />
