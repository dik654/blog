  ...
  return (
    <section id="overview" className="mb-16 scroll-mt-20">
      ...
      <div className="prose prose-neutral dark:prose-invert max-w-none">
        <p>
          libp2p는 모듈형 P2P 네트워킹 프레임워크다.
          <br />
          IPFS, Ethereum, Polkadot, Filecoin 등에서 사용한다.
          <br />이 아티클에서는 Rust 구현체의 <strong>Swarm 내부 구조</strong>를
          함수 단위로 추적한다.
        </p>
        ...
        <p>
          Swarm은 Transport, ConnectionPool, NetworkBehaviour 세 계층을 하나의
          이벤트 루프로 통합한다.
          <code>poll_next_event()</code> 한 함수 안에서 Behaviour → Pool →
          Transport 순서로 폴링한다.
        </p>
        <p>
          <strong>왜 이런 설계인가?</strong> 모든 계층이 비동기{" "}
          <code>poll()</code> 기반이라 단일 루프에서 수천 개의 연결을 처리할 수
          있다.
          <br />
        ...
        <ul>
          ...
          <li>
            <strong>NetworkBehaviour</strong> — 프로토콜 로직 추상화. Kademlia,
            GossipSub 등이 구현. 연결당 Handler를 생성
          </li>
          <li>
        ...
        <p className="text-sm text-muted-foreground mt-4">
          아래 섹션에서 각 트레이트의 내부 구현과 Swarm 이벤트 루프의 poll
          순서를 코드 수준으로 추적한다.
        </p>
