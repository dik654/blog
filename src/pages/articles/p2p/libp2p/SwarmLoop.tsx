  ...
  return (
    <section id="swarm-loop" className="mb-16 scroll-mt-20">
      ...
      <div className="prose prose-neutral dark:prose-invert max-w-none">
        <p>
          <code>Swarm::poll_next_event()</code>는 libp2p의 심장이다.
          <br />
          하나의 <code>loop {"{}"}</code> 안에서 세 컴포넌트를{" "}
          <strong>우선순위 순</strong>으로 폴링한다.
        </p>
      </div>
      <div className="rounded-xl border border-border bg-card p-5 my-6">
        <p className="text-xs font-mono text-foreground/50 mb-3">
          poll_next_event 우선순위
        </p>
      ...
      <div className="prose prose-neutral dark:prose-invert max-w-none">
        <p>
          이 우선순위가 중요한 이유가 있다.
          <br />
          Behaviour가 Dial 요청을 내면 <strong>즉시 처리</strong>되어야 한다.
          <br />
        ...
        {onCodeRef && (
          <div className="not-prose flex flex-wrap gap-2 my-4">
            ...
            <CodeViewButton
              onClick={() =>
                onCodeRef("swarm-struct", codeRefs["swarm-struct"])
              }
            />
            <CodeViewButton
              onClick={() => onCodeRef("swarm-dial", codeRefs["swarm-dial"])}
            />
          </div>
        )}
        ...
        <p>
          Behaviour가 <code>NotifyHandler</code>를 반환하면 바로 전달되지
          않는다.
          <br />
          다음 루프에서 해당 연결을 찾아 이벤트를 넘긴다.
          <br />
        ...
        <div className="flex flex-col gap-1.5">
          {HANDLER_STEPS.map((s, i) => (
            <div key={i} className="flex items-start gap-2.5 text-xs">
              <span className="font-mono text-foreground/30 mt-0.5 w-4 shrink-0">
                {i + 1}
              </span>
