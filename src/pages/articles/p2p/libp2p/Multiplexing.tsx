  ...
  return (
    <section id="multiplexing" className="mb-16 scroll-mt-20">
      ...
      <div className="prose prose-neutral dark:prose-invert max-w-none">
        <p>
          멀티플렉서(Multiplexer)는 단일 연결 위에 다수의{" "}
          <strong>논리적 스트림</strong>을 생성합니다.
          <br />
          각 스트림은 독립적인 프로토콜을 실행합니다.
          <br />
          Head-of-Line blocking(앞선 패킷 지연에 의한 후속 패킷 차단) 없이
          동작합니다.
        </p>

        <h3>StreamMuxer 트레이트</h3>
        <CodePanel
          title="StreamMuxer 트레이트 정의"
          code={streamMuxerCode}
          annotations={streamMuxerAnnotations}
        />

        <h3>Yamux 프레임 구조 & 흐름 제어</h3>
        <p>
          // Yamux 프레임 구조 (12 바이트 헤더)
          <br />
        ...
        <table className="w-full text-xs border-collapse">
          <thead>
            <tr className="border-b border-border">
              {["Muxer", "상태", "최대 스트림", "흐름 제어", "오버헤드"].map(
                (h) => (
                  <th
                    key={h}
