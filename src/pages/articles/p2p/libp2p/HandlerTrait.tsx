...
const methods = [
  {
    fn: "listen_protocol()",
    desc: "인바운드 서브스트림에 적용할 프로토콜을 광고한다. SubstreamProtocol을 반환.",
  },
  {
    fn: "poll()",
    desc: "ConnectionHandlerEvent를 반환. OutboundSubstreamRequest 또는 NotifyBehaviour.",
  },
  {
    fn: "on_behaviour_event()",
    desc: "Behaviour가 NotifyHandler로 보낸 메시지를 수신한다.",
  },
  {
    fn: "on_connection_event()",
    desc: "서브스트림 협상 완료(FullyNegotiated) 또는 실패 이벤트를 처리.",
  },
];
...
export default function HandlerTrait({
  title,
  onCodeRef,
}: {
  title?: string;
  ...
  return (
    <section id="handler-trait" className="mb-16 scroll-mt-20">
      <h2 className="text-2xl font-bold mb-6">
        {title ?? "ConnectionHandler 트레이트"}
      </h2>
      <div className="prose prose-neutral dark:prose-invert max-w-none">
        <p>
          <strong>ConnectionHandler</strong>는 개별 연결(피어 1명)에 대한
          프로토콜 핸들러다.
          <br />
          Behaviour가 연결 <em>전체</em>를 관리하면, Handler는 연결{" "}
          <em>하나</em>를 관리한다.
        </p>
      ...
      <div className="space-y-2 mt-3">
        {methods.map((m, i) => (
          ...
          >
            <span className="font-mono text-xs text-emerald-500 mt-0.5 shrink-0">
              {i + 1}
            </span>
            <div>
      ...
      <div className="prose prose-neutral dark:prose-invert max-w-none mt-6">
        <h3>서브스트림 협상 흐름</h3>
        <p>
          Handler가 새 아웃바운드 스트림을 요청하면, Swarm이 Muxer(멀티플렉서)를
          통해 스트림을 할당하고 multistream-select 프로토콜로 양측이 합의한다.
        </p>
      </div>
      ...
      <div className="prose prose-neutral dark:prose-invert max-w-none mt-6">
        <h3>유휴 연결 관리</h3>
        <p>
          <code>connection_keep_alive()</code>가 <code>false</code>를 반환하면
          유휴 연결은 자동 종료된다.
          <br />
      ...
      {onCodeRef && (
        <div className="not-prose flex flex-wrap gap-2 mt-4">
          ...
          />
          <CodeViewButton
            onClick={() => onCodeRef("swarm-event", codeRefs["swarm-event"])}
          />
        </div>
      )}
