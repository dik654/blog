...
const METHODS = [
  {
    name: "listen_on()",
    desc: "ListenerId + Multiaddr → 리스닝 시작",
    color: "#10b981",
  },
  {
    name: "dial()",
    desc: "Multiaddr + DialOpts → Dial Future 반환",
    color: "#f59e0b",
  },
  {
    name: "poll()",
    desc: "TransportEvent (Incoming, NewAddress 등) 반환",
    color: "#8b5cf6",
  },
];
...
const COMPARE = [
  {
    transport: "TCP",
    upgrade: "Security + Mux 별도 업그레이드 필요",
    steps: 3,
    color: "#ef4444",
  },
  {
    transport: "QUIC",
    upgrade: "TLS 1.3 + 스트림 다중화 내장",
    steps: 1,
    color: "#06b6d4",
  },
];

export default function TransportTrait({
  onCodeRef,
}: {
  title?: string;
  ...
  return (
    <section id="transport-trait" className="mb-16 scroll-mt-20">
      ...
      <div className="prose prose-neutral dark:prose-invert max-w-none">
        <p>
          Transport 트레이트는 libp2p의 가장 아래 추상화 계층이다.
          <br />
          역할은 단순하다. <strong>주소를 받아 연결을 만든다.</strong>
        </p>
        <p>
          TCP, QUIC, WebSocket 전부 이 트레이트를 구현한다.
          <br />
        ...
        <p>
          Transport에 async fn이 아닌 <code>poll()</code>을 쓴 이유가 있다.
          <br />
          Swarm은 <strong>단일 루프</strong>에서 여러 Transport를 직접 폴링한다.
          <br />
          Future라면 각각 spawn해야 한다. poll이면 spawn 없이 순차 폴링이
          가능하다.
        </p>
      </div>
      ...
      <div className="prose prose-neutral dark:prose-invert max-w-none mt-6">
        <p>
          ...
          Security(Noise) + Mux(Yamux) 업그레이드가 필요하다.
          <br />
