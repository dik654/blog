...
const PHASES = [
  ...
  {
    label: "Stream 1: Kademlia",
    desc: "SYN 플래그로 스트림 열기 → multistream-select → DHT 쿼리",
  },
  {
    label: "Stream 2: GossipSub",
    desc: "독립 스트림으로 pub/sub 메시지 전송. Stream 1과 간섭 없음",
  },
  {
    label: "4개 동시 스트림",
    desc: "각 스트림이 독립적 흐름 제어. WindowUpdate로 백프레셔 적용",
  },
];
        ...
        <AnimatePresence>
          {STREAMS.slice(0, visible).map((s, i) => (
            ...
            >
              ...
              <span className="text-[10px] ml-auto" style={{ color: s.color }}>
                {s.label}
              </span>
            </motion.div>
          ))}
