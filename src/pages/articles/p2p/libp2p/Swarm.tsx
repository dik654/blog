  ...
  return (
    <section id="swarm" className="mb-16 scroll-mt-20">
      ...
      <div className="prose prose-neutral dark:prose-invert max-w-none">
        <p>
          Swarm은 libp2p의 핵심 이벤트 루프입니다.
          <br />
          Transport에서 들어오는 연결 이벤트와 NetworkBehaviour가 요청하는
          액션을 조율합니다.
        </p>
        ...
        {onCodeRef && (
          <div className="not-prose flex flex-wrap gap-2 my-4">
            <CodeViewButton
              onClick={() => onCodeRef("swarm-poll", codeRefs["swarm-poll"])}
            />
            <CodeViewButton
              onClick={() =>
            ...
            <CodeViewButton
              onClick={() =>
                onCodeRef("to-swarm-enum", codeRefs["to-swarm-enum"])
              }
            />
          </div>
        )}
        ...
        <CodePanel
          ...
          code={networkBehaviourCode}
          annotations={[
            {
              lines: [2, 3],
              color: "sky",
              note: "ConnectionHandler + 이벤트 타입 정의",
            },
            { lines: [6, 12], color: "emerald", note: "인바운드 연결 처리" },
            {
              lines: [17, 21],
              color: "amber",
              note: "poll — 비동기 이벤트 생성",
            },
          ]}
        />
        ...
        <CodePanel
          title="derive(NetworkBehaviour) 매크로"
          code={deriveCode}
          annotations={[
            {
              lines: [1, 6],
              color: "sky",
              note: "여러 프로토콜을 하나의 struct로 결합",
            },
            {
              lines: [10, 16],
              color: "emerald",
        ...
        <p>
          각 연결에는 프로토콜별 <code>ConnectionHandler</code>가 연결됩니다.
          <br />
        ...
        <div className="not-prose space-y-1.5 my-4">
          {eventFlowSteps.map((step, i) => (
            <div key={i} className="flex items-start gap-2 text-sm">
              <span className="text-xs font-mono text-foreground/40 mt-0.5 w-4">
                {i + 1}
              </span>
