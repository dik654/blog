  ...
  return (
    <section id="tx-submission" className="mb-16 scroll-mt-20">
      ...
      <div className="prose prose-neutral dark:prose-invert max-w-none mb-6">
        ...
        <p className="leading-7">
          Dandelion++ — <strong>Stem</strong>(줄기)과 <strong>Fluff</strong>
          (꽃)의 2단계 전파.
          <br />
      ...
      <div className="not-prose">
        ...
        <div className="flex items-center gap-2 mt-3 justify-end">
          <CodeViewButton
            onClick={() => onCodeRef("kh-dandelion", codeRefs["kh-dandelion"])}
          />
      ...
      <div className="prose prose-neutral dark:prose-invert max-w-none mt-6">
        ...
        <p className="text-sm text-muted-foreground mb-3">
          Bitcoin BIP 156 — 트랜잭션 전파 시 sender anonymity 보호
        </p>
        <div className="not-prose space-y-3 mb-4">
          ...
          <div className="bg-muted rounded-lg p-4">
            ...
            <p className="text-sm text-muted-foreground">
              Alice → <span className="text-blue-500">[stem]</span> → NodeA →{" "}
              <span className="text-blue-500">[stem]</span> → NodeB →{" "}
              <span className="text-green-500">[fluff]</span> → whole network
            </p>
            <p className="text-xs text-muted-foreground mt-2">
              관찰자 시점: Fluff 시작 노드 = NodeB (not Alice). 실제 sender 식별
              어려움.
            </p>
          </div>
          ...
          <div className="bg-muted rounded-lg p-4">
            ...
            <ul className="text-sm text-muted-foreground space-y-1">
              <li>
                <strong>Active adversary</strong>: "stem detect attack" →
                Dandelion++가 random stem 구조로 완화
              </li>
              <li>
                <strong>Passive adversary</strong>: 단순 관찰 → 본질적으로 안전
              </li>
        ...
        <div className="not-prose space-y-3 mb-4">
          ...
          <div className="bg-muted rounded-lg p-4">
            <p className="text-sm font-semibold mb-2">
              <code>start_epoch()</code> — Epoch 시작
            </p>
            <ul className="text-sm text-muted-foreground space-y-1">
              <li>
                <code>connected_peers()</code>에서 랜덤 stem graph 구축
              </li>
              <li>
                <code>stem_next = select_random_peer(&amp;peers)</code> — 단일
                random next hop 지정
              </li>
            </ul>
          </div>
          ...
          <div className="bg-muted rounded-lg p-4">
            <p className="text-sm font-semibold mb-2">
              <code>on_transaction(tx, from)</code> — TX 수신 처리
            </p>
            <ul className="text-sm text-muted-foreground space-y-1">
              <li>
                <strong>내 TX</strong> (<code>from == Self</code>):{" "}
                <code>stem_tx(tx)</code>로 stem 시작
              </li>
              <li>
                <strong>수신 TX</strong>:{" "}
                <code>random() &lt; FLUFF_PROBABILITY</code>이면{" "}
                <code>fluff_tx(tx)</code> (broadcast), 아니면{" "}
                <code>forward_to_stem_next(tx)</code>
              </li>
            </ul>
          </div>
          ...
          <div className="bg-muted rounded-lg p-4">
            <p className="text-sm font-semibold mb-2">
              <code>after_embargo(tx)</code> — Embargo Timer
            </p>
            <p className="text-sm text-muted-foreground">
              Stem 중 응답 없으면 (<code>!tx.seen_on_network()</code>) 강제{" "}
              <code>fluff_tx(tx)</code> — fallback
            </p>
          </div>
        </div>
        ...
        <div className="bg-amber-50 dark:bg-amber-950/30 border-l-4 border-amber-400 p-4 my-6 rounded-r-lg">
          ...
          <p>
            ...
            - P2P network level의 sender identity
            <br />- Passive observer의 inference 방어
          </p>
          <p className="mt-2">
            <strong>해결 못 하는 것</strong>:<br />
            ✗ On-chain: sender address는 TX에 포함됨
            <br />
            ✗ Ethereum은 account-based → address 공개
            <br />
            ✗ MEV builders의 mempool 분석
            <br />✗ Infura 직접 연결 시 IP 노출
          </p>
          <p className="mt-2">
            <strong>추가 보완</strong>:<br />
            - RAILGUN 같은 zk-privacy
            <br />
            - Tornado Cash 스타일 mixer
            <br />
