  ...
  return (
    <section id="private-rpc" className="mb-16 scroll-mt-20">
      ...
      <div className="prose prose-neutral dark:prose-invert max-w-none mb-6">
        ...
        <p className="leading-7">
          TEE(Trusted Execution Environment) 환경에서는 ORAM 프록시 자체를
          enclave에서 실행한다.
          <br />
      ...
      <div className="not-prose">
        ...
        <div className="flex items-center gap-2 mt-3 justify-end">
          <CodeViewButton
            onClick={() => onCodeRef("kh-oram", codeRefs["kh-oram"])}
          />
      ...
      <div className="prose prose-neutral dark:prose-invert max-w-none mt-6">
        ...
        <div className="not-prose space-y-3 mb-4">
          <div className="bg-muted rounded-lg p-4">
            <p className="text-sm font-semibold mb-2">
              <code>RPCORAM</code> 쿼리 흐름
            </p>
            <p className="text-xs text-muted-foreground mb-2">
              <code>K = 7</code> (더미 쿼리 수, privacy parameter)
            </p>
            <ol className="text-sm text-muted-foreground space-y-1">
              <li>
                1) K개 랜덤 더미 주소 생성 —{" "}
                <code>generateRandomAddresses(K)</code>
              </li>
              <li>
                2) 진짜 쿼리와 더미를 혼합 —{" "}
                <code>shuffle([realAddress, ...dummies])</code>
              </li>
              <li>
                3) K+1개 쿼리를 RPC에 병렬 전송 — <code>Promise.all()</code>
              </li>
              <li>
                4) 진짜 결과만 추출 —{" "}
                <code>allQueries.indexOf(realAddress)</code>
              </li>
            </ol>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <div className="bg-muted rounded-lg p-4">
              ...
              <p className="text-xs text-muted-foreground mb-2">
                서버가 보는 것: K+1 queries / 서버가 모르는 것: 어떤 query가
                진짜인지
              </p>
          ...
          <div className="bg-muted rounded-lg p-4">
            ...
            <div className="flex flex-wrap gap-2 text-xs">
              <span className="bg-background px-2 py-1 rounded">
                Batch queries (Multicall) → overhead 분산
              </span>
              <span className="bg-background px-2 py-1 rounded">
                Caching (real + dummy 섞어서)
              </span>
              <span className="bg-background px-2 py-1 rounded">
                Temporal obfuscation (timing jitter)
              </span>
            </div>
          </div>
        </div>
        ...
        <div className="not-prose space-y-3 mb-4">
          <div className="bg-muted rounded-lg p-4">
            <p className="text-sm font-semibold mb-2">
              TEE (SGX/TDX) 기반 ORAM Proxy 아키텍처
            </p>
          ...
          <div className="bg-muted rounded-lg p-4">
            ...
            <p className="text-sm text-muted-foreground mb-2">
              TEE가 있으면 trusted path, 없으면 ORAM fallback — 사용자가 선택
            </p>
            <div className="flex flex-wrap gap-2 text-xs">
              <span className="bg-background px-2 py-1 rounded">
