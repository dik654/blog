...
export default function ABCIIntegration({
  onCodeRef,
}: {
  ...
  return (
    <section id="abci-integration" className="mb-16 scroll-mt-20">
      ...
      <div className="prose prose-neutral dark:prose-invert max-w-none">
        <p>
          CometBFT 합의 후 <code>FinalizeBlock</code> 호출 → 앱이 블록 내 모든
          TX를 실행
          <br />
          ABCI 2.0: <code>PrepareProposal</code> + <code>ProcessProposal</code>
          으로 앱이 블록 구성에 개입 가능
        </p>
        ...
        <p>
          ProcessProposal 직후 <strong>투기적 실행</strong> 시작 — 합의 완료 전
          블록 실행을 선제 수행
          <br />
          FinalizeBlock 진입 시 해시 비교: 일치 → 결과 재사용, 불일치 → 상태
          리셋 + 재실행
          <br />
        ...
        <p>
          PrepareProposal → ProcessProposal → FinalizeBlock → Commit 순서
          <br />
          PrepareProposal: 제안자가 TX 선택/정렬 — MEV 방지 로직 삽입점
          <br />
          ProcessProposal: 검증자가 블록 유효성 확인
          <br />
          FinalizeBlock: beginBlock → TX 실행 → endBlock
          <br />
