...
import { ANTE_CODE, ANTE_ANNOTATIONS } from "./TransactionLifecycleData";
    ...
    <section id="tx-lifecycle" className="mb-16 scroll-mt-20">
      ...
      <div className="prose prose-neutral dark:prose-invert max-w-none">
        <div className="not-prose flex flex-wrap gap-2 my-4">
          <CodeViewButton
            onClick={() =>
              onCodeRef("abci-initchain", codeRefs["abci-initchain"])
            }
          />
          ...
          <CodeViewButton
            onClick={() =>
              onCodeRef("abci-finalizeblock", codeRefs["abci-finalizeblock"])
            }
          />
          <span className="text-[10px] text-muted-foreground self-center">
            FinalizeBlock
          ...
          <CodeViewButton
            onClick={() => onCodeRef("runmsgs", codeRefs["runmsgs"])}
          />
          <span className="text-[10px] text-muted-foreground self-center">
            runMsgs
          </span>
          <CodeViewButton
            onClick={() => onCodeRef("bank-send", codeRefs["bank-send"])}
          />
          <span className="text-[10px] text-muted-foreground self-center">
            Bank MsgServer
          </span>
        ...
        <p>
          이더리움의 트랜잭션 검증이 EVM 실행 전에 서명/nonce/gas를 확인하는
          것과 같습니다.
          <br />
          Cosmos SDK의 <strong>AnteHandler</strong>는 메시지 실행 전에 사전
          검증을 수행합니다.
          <br />
          체인 형태의 미들웨어(데코레이터 패턴)로 구성됩니다.
        </p>
        <CodePanel
          title="AnteHandler 데코레이터 체인"
          code={ANTE_CODE}
          annotations={ANTE_ANNOTATIONS}
        />
        ...
        <p>
          IBC(Inter-Blockchain Communication)는 이더리움 생태계의{" "}
          <strong>브릿지</strong>와 비교됩니다.
          <br />
          프로토콜 레벨에서 표준화된 크로스체인 통신입니다.
          <br />
          이더리움 롤업이 L1에 증명을 제출하는 것과 유사하게, Light Client
          증명으로 체인 간 상태를 검증합니다.
        </p>
        <p>
          IBC vs 이더리움 크로스체인:
          ...
          ← ConnOpenTry
          <br />
          ← ConnOpenAck →<br />← ConnOpenConfirm
        </p>
      </div>
    </section>
  );
