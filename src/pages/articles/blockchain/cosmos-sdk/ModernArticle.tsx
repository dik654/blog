...
export default function ModernCosmosSDKArticle() {
  return <article className="space-y-14">
    ...
    <section id="runtx-pipeline" className="space-y-6">
      ...
      <ExplainedFormula question="Transaction이 gas limit 안에서 완료되는지 어떻게 나누어 읽는가?" idea={<>Gas meter는 ante, 각 message handler와 post-handler의 소비를 같은 limit에 누적합니다. 이 식은 시간 단위가 아니라 application이 정한 deterministic accounting입니다.</>} formula={String.raw`G_{used}=G_{ante}+\sum_{i=1}^{m}G_{msg_i}+G_{post}\le G_{limit}`}
      annotatedFormula={String.raw`G_{used}=\underbrace{G_{ante}+\sum_{i=1}^{m}G_{msg_i}+G_{post}\le G_{limit}}_{\text{허용 경계 판정}}`}
      operations={[
        { expression: String.raw`G_{ante}+\sum_{i=1}^{m}G_{msg_i}+G_{post}\le G_{limit}`, annotation: ["계산한 양을 허용 경계와 비교해 상태를 판정합니다.","Gas meter는 ante, 각 message","handler와 post-handler의 소비를 같은","limit에 누적합니다."] },
      ]} terms={[{symbol:"G_{ante}",name:"ante gas",description:"Signature verification, tx bytes, fee/sequence check 등의 소비입니다."},{symbol:"G_{msg_i}",name:"message gas",description:"i번째 routed MsgServer와 keeper store access의 소비입니다."},{symbol:"G_{post}",name:"post gas",description:"Configured post-handler가 사용한 gas입니다."},{symbol:"G_{limit}",name:"transaction gas limit",description:"Transaction이 신청한 최대 application gas입니다."}]} assumptions={["Gas cost table은 chain release/config에 귀속됩니다.","Out-of-gas panic은 BaseApp recovery path에서 tx failure로 변환됩니다.","Block gas meter와 transaction gas meter를 구분합니다.","Gas unit은 wall-clock ns나 CPU instruction 수와 같지 않습니다."]} interpretation="Ante에서 30, MsgSend에서 50, post에서 5를 썼다면 total은 85입니다. Limit 80이면 child cache write를 merge하지 않지만 ante fee/sequence 처리의 정확한 rollback semantics는 mode와 SDK 구현을 따라 확인해야 합니다." />
