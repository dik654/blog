...
const STEPS = [
  {
    label: "Module-Keeper 아키텍처",
    body: "각 모듈이 Keeper를 통해 자신의 KVStore를 관리하고 다른 모듈과 통신.",
  },
  {
    label: "Keeper 인터페이스",
    body: "Get / Set / Has 등의 메서드로 상태 접근. 직접 스토어 접근 금지.",
  },
  {
    label: "Cross-Module 통신",
    body: "StakingKeeper가 BankKeeper를 DI로 주입받아 위임 보상을 전송.",
  },
  {
    label: "Gov → 모듈 제어",
    body: "Governance가 Staking/Bank Keeper를 통해 파라미터 변경을 실행.",
  },
];

const MW = 100,
  MH = 55;
const center = (m: (typeof MODS)[0]) => ({
  cx: m.x + MW / 2,
  cy: m.y + MH / 2,
});
        ...
        >
          ...
          {MODS.map((m) => {
            ...
            return (
              <motion.g
                key={m.id}
                animate={{ opacity: active || step >= 2 ? 1 : 0.3 }}
              >
                <rect
                  x={m.x}
                  y={m.y}
