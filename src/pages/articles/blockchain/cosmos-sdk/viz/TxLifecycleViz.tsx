...
const STEPS = [
  {
    label: "Cosmos TX 전체 생명주기",
    body: "인코딩 → CheckTx → PrepareProposal → FinalizeBlock → Commit.",
  },
  {
    label: "① Protobuf 인코딩 & 서명",
    body: "Protobuf Any 타입으로 직렬화. secp256k1/ed25519 서명. SignDoc 해시.",
  },
  {
    label: "② CheckTx — 멤풀 진입",
    body: "AnteHandler: 서명, nonce, fee 검증. GasMeter 설정. 멤풀 우선순위 결정.",
  },
  {
    label: "③ PrepareProposal",
    body: "ABCI 2.0: 앱이 직접 TX를 선택/재정렬. MEV 방지 로직 삽입 가능.",
  },
  {
    label: "④ FinalizeBlock — 상태 전이",
    body: "BeginBlock → Msg 실행 (Keeper) → EndBlock. Gas 차감, 이벤트 발생.",
  },
  {
    label: "⑤ Commit → app_hash",
    body: "MultiStore.Commit() → IAVL 루트 해시 → app_hash. 이더리움 stateRoot 대응.",
  },
];
          ...
          {step >= 1 && step <= 5 && (
            <motion.rect
              ...
              animate={{
                x: [
                  STAGES[step - 1].x + 5,
                  STAGES[Math.min(step, 4)].x + SW - 15,
                ],
              }}
