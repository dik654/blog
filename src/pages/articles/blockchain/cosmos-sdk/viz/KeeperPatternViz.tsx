...
const STEPS = [
  {
    label: "Keeper 초기화",
    body: "App 초기화 시 각 모듈의 Keeper를 생성하고 의존성을 주입합니다.",
  },
  {
    label: "인터페이스 의존성",
    body: "Keeper 간 의존성은 인터페이스로 정의되어 느슨한 결합을 유지합니다.",
  },
  {
    label: "KVStore 접근",
    body: "각 Keeper는 자체 KVStore에만 접근하여 모듈 간 상태 격리를 보장합니다.",
  },
  {
    label: "전체 구조",
    body: "컴파일 타임에 의존성이 결정되는 정적 의존성 주입 패턴입니다.",
  },
];

const EDGES: [string, string, string][] = [
  ["app", "bank", "NewKeeper()"],
  ["app", "staking", "NewKeeper()"],
  ["bank", "auth", "의존"],
  ["staking", "bank", "의존"],
  ["bank", "store", "storeKey"],
  ["staking", "store", "storeKey"],
  ["iface", "bank", "느슨한 결합"],
];
        ...
        return (
          ...
          >
            {EDGES.map(([fi, ti, lbl], ei) => {
              const f = NODES.find((n) => n.id === fi)!,
                t = NODES.find((n) => n.id === ti)!;
              ...
              <motion.g key={n.id} animate={{ opacity: v.has(n.id) ? 1 : 0.1 }}>
                <motion.rect
                  ...
                  animate={{
                    fill: `${n.color}18`,
                    stroke: n.color,
                    strokeWidth: v.has(n.id) ? 1.8 : 0.7,
                  }}
                />
                <text
                  x={cx(n)}
