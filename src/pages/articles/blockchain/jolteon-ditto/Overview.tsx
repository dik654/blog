    ...
    <section id="overview" className="mb-16 scroll-mt-20">
      ...
      <div className="prose prose-neutral dark:prose-invert max-w-none">
        <p className="leading-7">
          HotStuff(2019) → Jolteon(2022) → Ditto(2022) → DiemBFT v4 → Aptos.
          <br />각 세대 <strong>한 가지 한계 해결</strong>.<br />이 계보가 현대
          BFT의 mainstream.
        </p>
        ...
        <div className="not-prose grid grid-cols-1 sm:grid-cols-2 gap-3 mb-4">
          <div className="rounded-lg border bg-card p-4">
            ...
            <ul className="text-sm space-y-1 text-muted-foreground">
              <li>
                <strong>높은 정상 경로 지연</strong> — Basic 7 delays, Chained
                3δ. PBFT(3δ)보다 빠르지 않음
              </li>
              <li>
                <strong>Partial responsive</strong> — view change에 timeout
                필요, 자주 발생 시 문제
              </li>
              <li>
                <strong>Async 취약</strong> — GST 전 liveness 보장 못 함
              </li>
              <li>
                <strong>Leader 단순</strong> — round-robin fixed, 성능 나쁜
                leader도 equal turn
              </li>
            </ul>
          </div>
          <div className="rounded-lg border bg-card p-4">
            ...
            <div className="text-sm text-muted-foreground space-y-2">
              <p>
                <strong>Jolteon</strong> (FC 2022) — 2-chain commit, fast path
                2δ, slow path fallback, fully responsive
              </p>
              <p>
                <strong>Ditto</strong> (FC 2022) — Jolteon + DAG-based async
                fallback (Narwhal). 어떤 네트워크에서도 liveness
              </p>
              <p>
                <strong>DiemBFT v4</strong> — Jolteon 기반 + leader reputation.
                Aptos에서 계승
              </p>
              <p>
                <strong>Aptos</strong> (2022-) — DiemBFT v4 + Quorum Store +
                Block-STM. 100K+ TPS
              </p>
            </div>
          </div>
        ...
        <p className="leading-7">
          HotStuff 한계:{" "}
          <strong>latency, partial responsive, async 취약, leader 단순</strong>.
          <br />
          Jolteon/Ditto가 각각 해결 → DiemBFT v4 통합.
          <br />
            ...
            <tbody>
              {[
                [
                  "2019",
                  "HotStuff (PODC)",
                  "O(n) linear comm, threshold sig, 3-chain commit",
                ],
        ...
        <div className="not-prose grid grid-cols-1 gap-3 mb-6">
          <div className="rounded-lg border bg-card p-4">
            ...
            <div className="text-sm text-muted-foreground">
              각 버전이 명확한 한 가지 개선. 이론 → 실무 → 이론 cycle. Diem
              종료가 오히려 다양성 증가.
            </div>
          </div>
        </div>
        <p className="leading-7">
          HotStuff 계보:{" "}
          <strong>
            이론(2019) → 실무(Libra/Aptos) → 이론(HotStuff-2) → 실무(Sui)
          </strong>
          .<br />
          Diem 종료가 오히려 다양성 증가.
          <br />
        ...
        <div className="not-prose grid grid-cols-1 sm:grid-cols-2 gap-3 mb-6">
          <div className="rounded-lg border bg-card p-4">
            ...
            <ul className="text-sm space-y-1 text-muted-foreground">
              <li>
                <strong>Aptos</strong>: DiemBFT v4 (Jolteon 기반)
              </li>
              <li>
                <strong>Sui</strong>: Mysticeti (새 방향)
              </li>
              <li>
                <strong>Celo</strong>: IBFT 2.0 (PBFT 계열)
              </li>
              <li>
                <strong>Near</strong>: Doomslug BFT (Tendermint 계열)
              </li>
            </ul>
          </div>
          <div className="rounded-lg border bg-card p-4">
            ...
            <ul className="text-sm space-y-1 text-muted-foreground">
              <li>
                <code className="text-xs">O(n)</code> communication + instant
                finality
              </li>
        ...
        <p className="leading-7">
          선택 기준:{" "}
          <strong>latency, responsiveness, async safety, 구현 성숙도</strong>.
          <br />
          Aptos = DiemBFT v4, Sui = Mysticeti — 상반된 선택.
          <br />
          DAG-based가 최신 트렌드 (Mysticeti 이후).
        </p>

        <p className="text-sm border-l-2 border-amber-500/50 pl-3 mt-4">
          <strong>💡 왜 이 진화가 일어났나</strong> — Aptos의 100K TPS 목표.
          <br />
          Ethereum 2.0의 32 validators-per-committee로는 불가능.
          <br />
          BFT + parallel execution + DAG mempool 결합 필요.
          <br />
