    ...
    <section id="comparison" className="mb-16 scroll-mt-20">
      ...
      <div className="prose prose-neutral dark:prose-invert max-w-none">
        ...
        <ul className="list-disc pl-6 space-y-1">
          <li>
            <strong>FFT가 병목일 때</strong> — 제약 수가 <Math>{"2^{20}"}</Math>{" "}
            이상이면 NTT 비용이 지배적, sumcheck이 유리
          </li>
          <li>
            <strong>GPU/FPGA 친화적 증명</strong> — sumcheck의 각 라운드는
            독립적인 부분합이라 병렬화 용이
          </li>
          <li>
            <strong>투명 셋업이 필요할 때</strong> — Dory는 trusted setup 없이
            다중선형 commit 가능
          </li>
          <li>
            <strong>유연한 제약 구조</strong> — PLONKish 게이트에 국한되지 않고
            custom constraint 정의 가능
          </li>
        </ul>
        ...
        <ul className="list-disc pl-6 space-y-1">
          <li>
            <strong>성숙한 생태계</strong> — Halo2, Plonky2, gnark 등 검증된
            프레임워크 존재
          </li>
          <li>
            <strong>작은 증명 크기</strong> — KZG의 <Math>{"O(1)"}</Math> 증명은
            온체인 검증에 최적
          </li>
          <li>
            <strong>빠른 검증</strong> — pairing 2회로 <Math>{"O(1)"}</Math>{" "}
            검증, L1 gas 비용 최소화
          </li>
          <li>
            <strong>Plookup 통합</strong> — 테이블 룩업이 자연스럽게 결합됨
          </li>
        </ul>

        <h3 className="text-xl font-semibold mt-8 mb-4">수렴하는 방향</h3>
        <p>
          Zeromorph — KZG 기반으로 다중선형 commit 지원, HyperPLONK에{" "}
          <Math>{"O(1)"}</Math> 크기 증명 부여
          <br />
          ...
          에서 기반 구조를 먼저 이해하면 HyperPLONK의 설계 동기가 더 명확해짐
        </p>
      </div>
    </section>
  );
