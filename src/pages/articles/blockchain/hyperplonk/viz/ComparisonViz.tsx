...
const BARS = [
  { label: "Prover", plonk: 30, hyper: 10, unit: "s", max: 35 },
  {
    ...
    unit: "B",
    max: 6000,
    fmt: (v: number) => (v >= 1000 ? `${(v / 1000).toFixed(0)}KB` : `${v}B`),
  },
...
function BarChartSVG() {
  ...
  return (
    ...
    >
      ...
      <text x={X0 + 14} y={155} fontSize={8} fill="var(--muted-foreground)">
        PLONK
      </text>
      <rect
        x={X0 + 60}
        ...
        fill={`${HYPER_C}80`}
      />
      <text x={X0 + 74} y={155} fontSize={8} fill="var(--muted-foreground)">
        HyperPLONK
      </text>
    </svg>
  );
