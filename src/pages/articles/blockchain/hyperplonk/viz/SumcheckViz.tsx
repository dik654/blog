...
const STEPS = [
  {
    ...
    body:
      "f(x₁,x₂,...,xₙ)을 불리언 하이퍼큐브 {0,1}ⁿ 위에서 평가.\n" +
  ...
  },
  {
    ...
    body:
      "Prover가 g₁(X₁) = Σ_{x₂,...,xₙ} f(X₁, x₂,...,xₙ) 전송.\n" +
  ...
  },
  {
    ...
    body:
      "gᵢ(Xᵢ) = Σ_{xᵢ₊₁,...,xₙ} f(r₁,...,rᵢ₋₁, Xᵢ, xᵢ₊₁,...,xₙ).\n" +
  ...
  },
  {
    ...
    body:
      "모든 변수가 (r₁,...,rₙ)로 고정 → 단일 점 f(r₁,...,rₙ) 평가.\n" +
          ...
          {step === 0 && (
            <g>
              ...
              {cubeEdges.map(([a, b], i) => (
                <motion.line
                  key={i}
                  x1={CUBE_CX + cubeNodes[a].x}
                  y1={CUBE_CY + cubeNodes[a].y}
                  x2={CUBE_CX + cubeNodes[b].x}
                  y2={CUBE_CY + cubeNodes[b].y}
                  stroke={C.prover}
                  strokeWidth={0.7}
                  initial={{ opacity: 0 }}
