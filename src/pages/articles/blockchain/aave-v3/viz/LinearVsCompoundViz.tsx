...
export default function LinearVsCompoundViz() {
  ...
  const linearPath = linearPoints
    ...
    .join(" ");
  const compoundPath = compoundPoints
    .map((p, i) => `${i === 0 ? "M" : "L"} ${toX(p.t)} ${toY(p.v)}`)
    .join(" ");
    ...
    <div className="not-prose my-6 rounded-lg border border-border bg-card p-4">
      ...
      >
        <text
          ...
          fill="var(--foreground)"
        >
          Linear (예치) vs Compound (차입) 이자 누적
        </text>
        ...
        <rect
          ...
          fillOpacity={0.4}
        />
        <text x={50} y={52} fontSize={10} fontWeight={600} fill="#10b981">
          Linear (aToken)
        </text>
        <rect
          x={180}
          ...
          fillOpacity={0.4}
        />
        <text x={200} y={52} fontSize={10} fontWeight={600} fill="#f59e0b">
          Compound (debtToken)
        </text>
