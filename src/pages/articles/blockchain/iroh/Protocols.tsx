  ...
  return (
    <section id="protocols" className="mb-16 scroll-mt-20">
      ...
      <div className="prose prose-neutral dark:prose-invert max-w-none">
        <p>
          iroh는 <code>ProtocolHandler</code> 트레이트 하나로 모든 프로토콜을
          동일하게 확장합니다.
          <br />
        ...
        <div className="not-prose flex flex-wrap gap-2 mb-4">
          <CodeViewButton
            onClick={() =>
              onCodeRef("protocol-handler", codeRefs["protocol-handler"])
            }
          />
          ...
          <CodeViewButton
            onClick={() =>
              onCodeRef("router-accept-loop", codeRefs["router-accept-loop"])
            }
          />
          <span className="text-[10px] text-muted-foreground self-center">
            Router accept loop
          ...
          <CodeViewButton
            onClick={() =>
              onCodeRef("handle-connection", codeRefs["handle-connection"])
            }
          />
          <span className="text-[10px] text-muted-foreground self-center">
            handle_connection()
        ...
        <CodePanel
          title="ALPN 기반 프로토콜 라우터"
          code={ROUTER_CODE}
          annotations={[{ lines: [2, 5], color: "sky", note: "프로토콜 등록" }]}
        />
      ...
      <div className="prose prose-neutral dark:prose-invert max-w-none mt-8">
        ...
        />
        <p>
          이처럼 iroh 위에서 프로토콜을 만드는 것은 매우 단순합니다.
          <br />
          NAT 통과, 암호화, 연결 관리는 모두 iroh가 처리하며, 개발자는
          애플리케이션 로직에만 집중할 수 있습니다.
        </p>
      </div>
    </section>
  );
