...
const STATES: { id: PathState; label: string; desc: string }[] = [
  {
    id: "relay-only",
    label: "Relay 전용",
    desc: "연결 초기 or hole punching 실패. 모든 패킷이 relay 서버를 거쳐 전달된다. 지연 높음.",
  },
  {
    id: "punching",
    label: "Hole Punching 중",
    desc: "relay + UDP 직접 경로 동시 시도. DISCO Ping/Pong 교환으로 NAT 홀을 뚫는 중.",
  },
  {
    id: "direct",
    label: "직접 UDP",
    desc: "최적 상태. UDP로 직접 연결. relay는 비활성화. 연결 이동(IP 변경) 시 자동 재협상.",
  },
];
    ...
    <div className="not-prose rounded-xl border p-5 mb-6">
      ...
      <svg viewBox={`0 0 ${W} ${H}`} className="w-full" style={{ height: 130 }}>
        ...
        <rect
          ...
          rx={6}
          fill="#f59e0b"
          fillOpacity={showRelay ? 0.2 : 0.05}
          stroke="#f59e0b"
          strokeWidth={showRelay ? 1.5 : 0.5}
        />
        <text
          x={R.x}
          y={R.y + 5}
          ...
          fontSize={10}
          fontWeight={600}
          fill={showRelay ? "#f59e0b" : "#f59e0b"}
          fillOpacity={showRelay ? 1 : 0.3}
        >
          Relay
        </text>
        ...
        {showRelay && (
          <g>
            <line
              ...
              y2={R.y + 8}
              stroke="#f59e0b"
              strokeWidth={state === "relay-only" ? 2 : 1}
              strokeOpacity={0.6}
              strokeDasharray={state === "punching" ? "4 3" : "0"}
            />
            <line
              x1={R.x + 28}
              y1={R.y + 8}
        ...
        {showDirect && (
          <line
            ...
            y2={B.y}
            stroke="#10b981"
            strokeWidth={state === "direct" ? 2.5 : 1.5}
            strokeOpacity={0.8}
          />
        )}
        ...
        {/* Labels */}
        {showRelay && (
          <text x={190} y={72} textAnchor="middle" fontSize={10} fill="#f59e0b">
            relay 경로
          </text>
        )}
        {showDirect && (
