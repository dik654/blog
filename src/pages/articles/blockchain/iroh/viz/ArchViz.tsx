...
function cx(id: string) {
  return (COMPONENTS.find((c) => c.id === id)?.x ?? 0) + 50;
}
function cy(id: string) {
  return (COMPONENTS.find((c) => c.id === id)?.y ?? 0) + 18;
}
    ...
    <div className="not-prose rounded-xl border p-5 mb-6">
        ...
        {COMPONENTS.map((c) => (
          ...
          >
            <rect
              ...
              rx={8}
              fill={c.color}
              fillOpacity={active === c.id ? 0.9 : 0.2}
              stroke={c.color}
      ...
      {sel ? (
        <div className="mt-3 rounded-lg border border-border/60 bg-background/60 px-4 py-3 text-sm">
          <span className="font-semibold" style={{ color: sel.color }}>
            {sel.label}
          </span>
