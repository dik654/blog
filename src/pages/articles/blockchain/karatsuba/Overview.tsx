  ...
  return (
    <section id="overview" className="mb-16 scroll-mt-20">
      ...
      <div className="prose prose-neutral dark:prose-invert max-w-none mb-6">
        <p>
          Fp2 곱셈을 4번 &rarr; 3번으로 25% 절감. 타워에서 재귀 적용하면 144
          &rarr; 54.
        </p>
      </div>
      ...
      <div className="prose prose-neutral dark:prose-invert max-w-none mt-6">
        ...
        <div className="not-prose rounded-lg border-l-4 border-l-blue-500 bg-card p-4 mb-4">
          ...
          <p className="text-sm text-muted-foreground mb-2">
            1960년 Anatoly Karatsuba가{" "}
            <M>{"O(n^{\\log_2 3}) \\approx O(n^{1.585})"}</M> 정수 곱셈을 증명.
            최초의 sub-quadratic 곱셈 알고리즘으로, Kolmogorov의{" "}
            <M>{"O(n^2)"}</M> 최적 추측을 반증했다.
          </p>
        ...
        <div className="not-prose rounded-lg border bg-card p-4 mb-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            <div className="rounded bg-muted/50 p-3">
              ...
              <M display>
                {
                  "\\underbrace{(a + b \\cdot 10)}_{\\text{첫째 수}} \\underbrace{(c + d \\cdot 10)}_{\\text{둘째 수}} = \\underbrace{ac}_{\\text{곱①}} + \\underbrace{(ad + bc)}_{\\text{곱②③}} \\cdot 10 + \\underbrace{bd}_{\\text{곱④}} \\cdot 100"
                }
              </M>
              <p className="text-sm text-muted-foreground mt-2">
                <M>a, c</M>: 일의 자리, <M>b, d</M>: 십의 자리. 교차항{" "}
                <M>{"ad + bc"}</M>에 곱셈 2회가 필요하여 총 4회
              </p>
            </div>
            <div className="rounded bg-muted/50 p-3">
              <div className="text-xs font-mono text-muted-foreground mb-1">
              ...
              <M display>
                {
                  "\\underbrace{ad + bc}_{\\text{교차항}} = \\underbrace{(a+b)(c+d)}_{\\text{곱③}} - \\underbrace{ac}_{\\text{곱① 재사용}} - \\underbrace{bd}_{\\text{곱② 재사용}}"
                }
              </M>
              <p className="text-sm text-muted-foreground mt-2">
                <M>ac</M>와 <M>bd</M>는 실수부 계산에 이미 필요하므로 재사용. 새
                곱셈은 <M>{"(a+b)(c+d)"}</M> 1회뿐
              </p>
            </div>
          </div>
        ...
        <div className="not-prose rounded-lg border bg-card p-4 mb-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            <div className="rounded bg-muted/50 p-3">
              ...
              <M display>
                {
                  "T(n) = \\underbrace{3}_{\\text{부분 곱셈 수}} T(\\underbrace{n/2}_{\\text{반으로 분할}}) + \\underbrace{O(n)}_{\\text{덧셈/뺄셈}} = O(n^{\\log_2 3}) \\approx O(n^{1.585})"
                }
              </M>
              <p className="text-sm text-muted-foreground mt-2">
                <M>{"T(n)"}</M>: <M>n</M>자리 수의 곱셈 비용. 3개 부분 문제로
                분할하여 sub-quadratic 달성
              </p>
            </div>
            <div className="rounded bg-muted/50 p-3">
              <div className="text-xs font-mono text-muted-foreground mb-1">
              ...
              <M display>
                {
                  "T(n) = \\underbrace{4}_{\\text{부분 곱셈 수}} T(\\underbrace{n/2}_{\\text{반으로 분할}}) + \\underbrace{O(n)}_{\\text{덧셈/뺄셈}} = O(n^2)"
                }
              </M>
              <p className="text-sm text-muted-foreground mt-2">
                <M>{"T(n)"}</M>: <M>n</M>자리 수의 곱셈 비용. 4개 부분 문제 =
                quadratic 증가
              </p>
            </div>
          </div>
        ...
        <div className="not-prose rounded-lg border-l-4 border-l-emerald-500 bg-card p-4 mb-4">
          <div className="text-sm font-semibold mb-2">
            BN254/BLS12-381 확장체 타워 적용
          </div>
          <p className="text-sm text-muted-foreground mb-3">
            타워:{" "}
            <M>
              {
                "\\mathbb{F}_p \\to \\mathbb{F}_{p^2} \\to \\mathbb{F}_{p^6} \\to \\mathbb{F}_{p^{12}}"
              }
            </M>
          </p>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            ...
            <div className="rounded bg-muted/50 p-3">
              ...
              <ul className="text-sm text-muted-foreground space-y-1">
                ...
                <li className="font-semibold">
                  Total: 3 x 6 x 3 = 54 Fp mults (62% 절감)
                </li>
              </ul>
            </div>
          </div>
        ...
        <div className="not-prose rounded-lg border-l-4 border-l-amber-500 bg-card p-4 mb-4">
          ...
          <ul className="text-sm text-muted-foreground space-y-2">
            <li>
              <strong>Overflow</strong> &mdash; <M>{"(a+b)"}</M>가{" "}
              <M>{"2^n"}</M>을 초과할 수 있음. 해결: 1비트 여유 확보
            </li>
            <li>
              <strong>Cache</strong> &mdash; 재귀가 캐시를 어지럽힘. 해결: 작은
              base case(n &lt; 16)에서 인라인
            </li>
            <li>
              <strong>Underflow</strong> &mdash; <M>{"(a+b)(c+d) - ac - bd"}</M>
              가 음수될 수 있음. 해결: signed 산술 또는 careful unsigned
            </li>
          </ul>
        </div>
      </div>
