    ...
    <section id="save-state" className="mb-16 scroll-mt-20">
      ...
      <div className="prose prose-neutral dark:prose-invert max-w-none">
        <div className="not-prose flex flex-wrap gap-2 mb-4">
          ...
          />
          <span className="text-xs text-muted-foreground self-center">
            BlockExecutor struct
          </span>
        </div>

        {/* ── Save order ── */}
        <h3 className="text-xl font-semibold mt-4 mb-3">
          Save 순서 — App → CometBFT
        </h3>
        <p className="text-xs text-muted-foreground mb-3">
          <code>BlockExecutor.Commit(state, block, txResults)</code> — 저장
          순서가 crash safety 결정
        </p>
        ...
        <p className="leading-7">
          <strong>App → CometBFT 순서</strong>가 crash safety의 핵심이다.
          Application state를 먼저 commit하고 그 다음 CometBFT state를 저장해야,
          중간에 crash가 나더라도 replay를 통해 두 height를 다시 맞출 수 있다.
        </p>

        <p className="text-sm border-l-2 border-amber-500/50 pl-3 mt-4">
          <strong>💡 Commit() 후 Save() 순서</strong> — 앱이 먼저 Commit()으로
          상태를 확정한 뒤 CometBFT가 store.Save(state)를 호출한다. 역순이면
          crash 시 consensus state만 앞서 기록될 수 있다.
        </p>
        <p className="text-sm mt-3 border-l-2 border-sky-500/50 pl-3">
          <strong>💡 fireEvents</strong> — NewBlock, NewBlockHeader, Tx 이벤트를
          EventBus로 전파한다. Subscriber는 event 전달 자체를 durable storage로
          간주하지 말고 재조회나 replay 경로를 마련해야 한다.
        </p>
      </div>
    </section>
  );
