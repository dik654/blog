...
export default function ExecuteBlock({
  onCodeRef,
}: {
    ...
    <section id="execute-block" className="mb-16 scroll-mt-20">
      ...
      <div className="prose prose-neutral dark:prose-invert max-w-none">
        ...
        <h3 className="text-xl font-semibold mt-4 mb-3">
          ApplyBlock은 합의 결과를 application state로 반영한다
        </h3>
        <p className="text-xs text-muted-foreground mb-3">
          cometbft/state/execution.go — +2/3 precommit 확인 후 호출하는 최종
          실행 함수
        </p>
        ...
        <div className="not-prose mb-4">
          <div className="rounded-lg border bg-card p-4 mb-3">
            <p className="text-xs font-semibold text-purple-600 dark:text-purple-400 mb-2">
              <code>ApplyBlock</code> 9단계 순차 실행
            </p>
            <div className="grid grid-cols-3 gap-2 text-sm text-muted-foreground">
              ...
              <div className="rounded bg-muted/50 p-2">
                ...
                <p className="text-xs">
                  <code className="text-xs">execBlockOnProxyApp</code> ABCI
                  호출
                </p>
                <p className="text-xs text-amber-500 mt-1">
                  fail.Fail() #2
              ...
              <div className="rounded bg-muted/50 p-2">
                ...
                <p className="text-xs">
                  <code className="text-xs">store.SaveABCIResponses</code>
                </p>
                <p className="text-xs text-amber-500 mt-1">
                  fail.Fail() #3
                </p>
              ...
              <div className="rounded bg-muted/50 p-2">
                ...
                <p className="text-xs">
                  <code className="text-xs">updateState(...)</code>
                </p>
                <p className="text-xs text-amber-500 mt-1">
                  fail.Fail() #4
                </p>
              ...
              <div className="rounded bg-muted/50 p-2">
                ...
                <p className="text-xs">
                  <code className="text-xs">store.Save(state)</code>
                </p>
                <p className="text-xs text-amber-500 mt-1">
                  fail.Fail() #7
                </p>
              ...
              <div className="rounded bg-muted/50 p-2">
                ...
                <p className="text-xs">
                  <code className="text-xs">fireEvents(...)</code>
                </p>
              </div>
              <div className="rounded bg-muted/50 p-2">
                <p className="text-xs font-medium mb-1">9. Prune (optional)</p>
                <p className="text-xs">
                  <code className="text-xs">pruneBlockStore</code> if
                  retainHeight &gt; 0
                </p>
              </div>
            </div>
          ...
          <div className="rounded-lg border border-dashed border-amber-500/30 bg-amber-500/5 p-3">
            ...
            <p className="text-xs text-muted-foreground">
              테스트 모드에서 각 단계 사이 crash 시뮬레이션 — 재시작 후 복구
              가능한지 검증. production에서는 no-op.
            </p>
          </div>
        </div>

        <p className="leading-7">
          ApplyBlock은 block 검증에서 ABCI 실행, state update, commit, event,
          pruning으로 이어지는 상태 전이를 한곳에 모은다. 단계 사이의
          <code>fail.Fail()</code> 지점은 특정 시점에 crash를 주입해 재시작 후
          복구 경로를 검증하기 위한 장치다.
        </p>

        <p className="text-sm border-l-2 border-amber-500/50 pl-3 mt-4">
          <strong>💡 ApplyBlock이 전체 상태 전이를 집약</strong> — 검증, ABCI
          실행, 상태 갱신, DB 저장의 순서를 한 함수가 조율한다.
          fail.Fail() 크래시 주입 지점 — 각 단계 사이에서 크래시가 발생해도 복구
          가능함을 검증.
        </p>
      </div>
    </section>
