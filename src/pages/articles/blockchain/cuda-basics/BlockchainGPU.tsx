...
import { msmCode, nttCode, ecdsaCode } from "./BlockchainGPUData";
  ...
  return (
    <section id="blockchain-gpu" className="mb-16 scroll-mt-20">
      ...
      <div className="prose prose-neutral dark:prose-invert max-w-none">
        ...
        <p>
          ZK 증명 생성에서 가장 연산량이 큰 부분은 MSM(Multi-Scalar
          Multiplication, 다중 스칼라 곱)입니다.
          <br />
          이더리움의 precompile(BN254 pairing)에도 동일한 연산이 사용됩니다.
        </p>
        <CitationBlock
          source="Pippenger, On the evaluation of powers, 1980"
          citeKey={4}
          type="paper"
        >
          <p className="italic">
            "The Pippenger algorithm reduces multi-scalar multiplication from
            O(n) to O(n / log n)."
          </p>
        </CitationBlock>
        <CodePanel
          title="Pippenger MSM GPU 병렬화"
          code={msmCode}
          annotations={[
        ...
        <CodePanel
          ...
          annotations={[
            ...
            { lines: [7, 10], color: "emerald", note: "GPU 병렬: ~5ms" },
            {
              lines: [12, 19],
              color: "amber",
              note: "주요 GPU 가속 라이브러리",
            },
          ]}
        />
        <CitationBlock
          source="Tian et al., gECC: GPU-accelerated ECC"
          citeKey={5}
          type="paper"
        >
          <p className="italic">
            "gECC achieves 5.56x speedup for ECDSA verification on NVIDIA A100
            GPUs."
          </p>
        </CitationBlock>
        <h3 className="text-xl font-semibold mt-6 mb-3">
