export const msmCode = `MSM: result = Σ(sᵢ × Gᵢ) for i = 1..n
...
Pippenger 알고리즘:
  ...
  - 시간: O(n/w + 2^w) — w 최적화로 O(n/log n)`;

export const nttCode = `NTT = 유한체 위의 FFT (ZK 증명의 다항식 연산에 필수)
...
CUDA 구현:
  ...
  - NTT가 ZK 증명 생성 지연의 ~90% 차지`;

export const ecdsaCode = `배치 ECDSA 검증 (이더리움 트랜잭션 검증):
...
핵심 도전:
  ...
  → secp256k1 (이더리움/비트코인 곡선)에 특화된 최적화`;

export const cudaSamplesCode = `cuda-samples/ 블록체인 관련 예제:
