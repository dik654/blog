  ...
  const parts = [
    {
      ...
      desc: "q_L*a + q_R*b + q_O*c + q_M*ab + q_C",
      c: CV,
    },
    {
      label: "alpha * perm1",
      desc: "Z*(id product) - Z(wx)*(sigma product)",
      c: CE,
    },
    ...
    <g>
      <motion.g initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
        <text
          ...
          fill="var(--foreground)"
        >
          combined = gate + alpha*P1 + alpha^2*P2
        </text>
      </motion.g>
      {parts.map((p, i) => (
        <motion.g
        ...
        >
          <rect
            ...
            strokeWidth={0.5}
          />
          <text x={30} y={38 + i * 28} fontSize={8} fontWeight={600} fill={p.c}>
            {p.label}
          </text>
          <text
            x={160}
