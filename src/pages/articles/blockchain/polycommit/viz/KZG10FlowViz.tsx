...
const STEPS = [
  {
    label: "Trusted Setup -- SRS 생성",
    body: "비밀 beta로 G1, G2 위 거듭제곱 점을 생성한 뒤 beta를 삭제. 이후 모든 커밋/검증에 SRS만 사용한다.",
  },
  {
    label: "Commit -- p(x) -> G1 점",
    body: "다항식 계수와 SRS_G1을 MSM(다중 스칼라 곱)으로 결합. 결과는 G1 점 하나 = 48바이트.",
  },
  {
    label: "Open -- witness 다항식 계산",
    body: "인수정리로 (p(x)-v)/(x-z) 가 다항식이면 p(z)=v. witness를 G1 점으로 커밋하면 증명 완성.",
  },
  {
    label: "Verify -- Pairing Check O(1)",
    body: "페어링 2회로 인수정리 등식을 확인. 다항식 크기와 무관하게 상수 시간 검증.",
  },
];
...
function CommitDiagram() {
  return (
    <motion.g initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={sp}>
      ...
      <defs>
        <clipPath id="cm-clip1">
          <rect x={20} y={34} width={130} height={40} rx={8} />
        </clipPath>
        <clipPath id="cm-clip2">
