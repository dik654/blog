...
const STEPS = [
  {
    label: "CPU: 강력한 소수 코어",
    body: "CPU는 복잡한 제어 흐름에 최적화된 4~64개의 고성능 코어를 보유합니다.",
  },
  {
    label: "GPU: 대량의 경량 코어",
    body: "GPU는 수천 개의 단순 코어로 동일 명령을 대량 데이터에 병렬 적용합니다.",
  },
  {
    label: "SIMT 실행 모델",
    body: "SIMT(Single Instruction, Multiple Threads): 32개 스레드가 한 워프로 묶여 동일 명령을 실행합니다.",
  },
];
          ...
          <motion.g animate={{ opacity: step === 0 ? 1 : 0.2 }}>
            ...
            {[0, 1, 2, 3].map((i) => (
              <motion.rect
                ...
                strokeWidth={1.5}
                initial={{ scale: 0.8 }}
                animate={{ scale: step === 0 ? 1 : 0.9 }}
                style={{
                  transformOrigin: `${67 + (i % 2) * 70}px ${54 + Math.floor(i / 2) * 50}px`,
                }}
              />
            ))}
            {[0, 1, 2, 3].map((i) => (
