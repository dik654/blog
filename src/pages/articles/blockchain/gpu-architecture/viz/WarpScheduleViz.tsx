        ...
        >
          {step === 0 && (
            <motion.g initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
              <text x={8} y={16} fontSize={10} fontWeight={600} fill={C.sched}>
                타임 슬롯 →
              </text>
              {WARPS.map((w, wi) => (
                <g key={wi}>
                  ...
                  {Array.from({ length: SLOTS }, (_, si) => {
                    ...
                    return (
                      <motion.rect
                        ...
                        animate={{ scaleX: 1 }}
                        style={{
                          transformOrigin: `${60 + si * 46}px ${35 + wi * 28}px`,
                        }}
              ...
              <g transform="translate(60,140)">
                <rect
                  ...
                  opacity={0.6}
                />
                <text x={14} y={7} fontSize={10} fill="var(--muted-foreground)">
                  실행
                </text>
                <rect
                  x={50}
                  ...
                  opacity={0.3}
                />
                <text x={64} y={7} fontSize={10} fill="var(--muted-foreground)">
                  대기
                </text>
              </g>
            </motion.g>
          ...
          {step === 2 && (
            <motion.g initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
              <text x={30} y={20} fontSize={10} fontWeight={600} fill={C.sched}>
                Warp 내 32 스레드
              </text>
              {Array.from({ length: 32 }, (_, i) => {
                ...
                return (
                  <motion.rect
                    key={i}
                    x={30 + (i % 16) * 24}
                    y={30 + Math.floor(i / 16) * 40}
                    width={20}
                    height={14}
                    rx={2}
                    fill={diverged ? C.stall + "20" : C.active + "20"}
                    stroke={diverged ? C.stall : C.active}
                    strokeWidth={1.5}
                    initial={{ opacity: 0 }}
                    animate={{ opacity: 1 }}
              ...
              <text x={30} y={102} fontSize={10} fill={C.active}>
                if 경로 (T0~T15): 1차 실행
              </text>
              <text x={30} y={116} fontSize={10} fill={C.stall}>
                else 경로 (T16~T31): 2차 실행
              </text>
