...
function PBFTMesh() {
  ...
  for (let i = 0; i < 4; i++)
    for (let j = i + 1; j < 4; j++) lines.push([i, j]);
  ...
  return (
    <g>
      ...
      {N.map((n, i) => (
        <Node key={i} x={n.x} y={n.y} label={`R${i}`} color={C.node} />
      ))}
      <text
        x={110}
        y={170}
...
function HotStuffStar() {
  ...
  return (
    <g transform="translate(230, 0)">
      ...
      {replicas.map((r, i) => (
        <Node key={i} x={r.x} y={r.y} label={`R${i + 1}`} color={C.node} />
      ))}
      <text
        x={110}
        y={170}
  ...
  return (
    <div className="not-prose rounded-xl border p-4 mb-6">
      ...
      <p className="text-xs text-center text-foreground/75 mt-2">
        PBFT: 모든 노드가 서로 통신 → n(n-1)/2 연결 / HotStuff: 리더 경유 → n-1
        연결
      </p>
    </div>
  );
