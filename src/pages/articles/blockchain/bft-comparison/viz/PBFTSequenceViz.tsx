...
const STEPS = [
  ...
  {
    label: "Pre-Prepare",
    body: "Primary가 시퀀스 번호를 부여하고 모든 Replica에게 브로드캐스트",
  },
  {
    label: "Prepare",
    body: "2f+1 Prepare 메시지 수집 → 순서 합의. O(n²) 통신",
  },
  { label: "Commit", body: "2f+1 Commit 수집 → 요청 실행. 되돌릴 수 없음" },
  {
    label: "Reply",
    body: "실행 결과를 클라이언트에 반환. f+1 동일 Reply로 확인",
  },
];
        ...
        >
          ...
          {EDGES.map((lbl, i) => {
            ...
            return (
              ...
              >
                <path
                  d={`M${x1},${SY - NH / 2} Q${mx},${LY} ${x2},${SY - NH / 2}`}
                  fill="none"
          ...
          {step === 5 && (
            <motion.g initial={{ opacity: 0 }} animate={{ opacity: 0.6 }}>
              <path
                d={`M${nx(5) + NW / 2},${SY + NH / 2} Q${270},${SY + 50} ${nx(0) + NW / 2},${SY + NH / 2}`}
                fill="none"
                stroke="#ef4444"
                strokeWidth={1}
                markerEnd="url(#pbft-ah)"
            ...
            return (
              ...
              >
                <rect
                  ...
                  height={NH}
                  rx={6}
                  fill={glow ? n.color + "22" : "var(--background)"}
                  stroke={n.color}
