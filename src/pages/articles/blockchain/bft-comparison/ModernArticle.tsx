  ...
  return (
    <article className="space-y-14">
      ...
      <section id="comparison" className="space-y-6">
        ...
        <ExplainedFormula question="Equal-weight BFT에서 두 commit quorum이 적어도 한 honest replica를 공유하려면 quorum은 얼마나 커야 하는가?" idea={<>두 집합의 원소 수를 더하면 전체 n을 넘는 부분이 반드시 교집합입니다. n=3f+1, q=2f+1을 대입하면 2q-n=f+1이 되어 최대 Byzantine 수 f보다 하나 많습니다.</>} formula={String.raw`\begin{aligned}|Q_1\cap Q_2|&\ge 2q-n\\&=f+1>f\end{aligned}`} terms={[{symbol:"n",name:"Replica 수",description:"같은 membership snapshot에 있는 equal-weight voting replicas입니다."},{symbol:"f",name:"Fault bound",description:"Protocol이 허용한다고 가정한 최대 Byzantine replica 수입니다."},{symbol:"q",name:"Quorum 크기",description:"한 certificate에 필요한 distinct valid signers 수로 여기서는 2f+1입니다."},{symbol:"Q_1,Q_2",name:"두 signer 집합",description:"서로 충돌할 수 있는 certificate의 signer sets입니다."}]} assumptions={["n=3f+1인 fixed equal-weight membership입니다.","Signer identity·phase·view·value와 signature를 검증하고 중복 signer를 한 번만 셉니다.","Honest replica가 protocol의 lock·vote rule을 지킵니다.","Stake-weighted quorum에는 signer 수가 아니라 historical voting weight를 사용합니다."]} interpretation="n=4,f=1,q=3이면 두 quorum은 최소 2명을 공유해 그중 적어도 한 명은 honest입니다. q를 2로 낮추면 {A,B}와 {C,D}처럼 교집합 없는 certificate가 가능하므로 이 safety 논리를 쓸 수 없습니다." />
        <BFTAxisTableViz />
