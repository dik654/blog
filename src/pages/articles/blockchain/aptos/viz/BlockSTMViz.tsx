...
const STEPS = [
  {
    label: "모든 TX 낙관적 병렬 실행",
    body: "블록 내 모든 TX 동시 실행 시작. 사전 계정 선언 불필요",
  },
  {
    label: "Read/Write Set 추적",
    body: "각 TX의 읽기/쓰기 키를 MVHashMap에 기록",
  },
  {
    label: "검증: 충돌 감지",
    body: "TX 1이 읽은 값을 TX 0이 변경 → TX 1 재실행 필요",
  },
  {
    label: "재실행 & 완료",
    body: "TX 1만 재실행, 나머지 검증 통과. 블록 실행 완료",
  },
];
const REFS = [
  "apt-blockstm-exec",
...
export default function BlockSTMViz({
  onOpenCode,
}: {
