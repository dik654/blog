    ...
    <section id="finalize-commit" className="mb-16 scroll-mt-20">
      ...
      <div className="prose prose-neutral max-w-none dark:prose-invert">
        <p>
          ...
          validator·consensus parameter updates, AppHash를 반환합니다. 이 결과는 CometBFT가 저장하지만 application state의 영속화는
          아직 끝나지 않았습니다. 그 height가 durable하게 저장됐다고 보려면 뒤따르는 Commit까지 반환돼야 합니다.
        </p>
      </div>
      ...
      <div className="prose prose-neutral max-w-none dark:prose-invert">
        ...
        <p>
          FinalizeBlock이 반환한 AppHash는 다음 block header의 AppHash로 들어갑니다. 장애 로그에는 decided block height,
          FinalizeBlock result AppHash, application committed height, 다음 header의 AppHash를 각각 분리해 남겨야 합니다. 같은
          이름만 보고 현재 block payload의 hash나 CometBFT 자체 database root로 해석하면 안 됩니다.
        </p>
      </div>
      <div className="not-prose my-4 flex flex-wrap gap-3">
        <CodeViewButton
      ...
      <div className="prose prose-neutral max-w-none dark:prose-invert">
        ...
        <p>
          Block만 저장됐으면 FinalizeBlock 이후를 replay합니다. CometBFT result/state는 저장됐는데 application Commit이 끝나지 않았으면
          FinalizeBlock 결과를 재현해 저장된 결과와 대조한 뒤 Commit합니다. Application이 더 높은 height를 보고하면 blindly 진행하지 않고
          handshake·recovery invariant를 실패시킵니다. Replay 가능한 transition과 idempotent Commit이 필요하며 payment·webhook
          같은 external effect는 state outbox에서 별도로 reconcile합니다.
        </p>
      </div>
      <div id="paper-cometbft-app-requirements-v040" className="not-prose my-8 scroll-mt-24 border-l border-primary/50 pl-4">
        ...
        <p className="mt-2 text-sm leading-6 text-muted-foreground">
            규격이 application 의무로 명시하는 항목은 proposal coherence·determinism·candidate state·connection
            ordering·crash recovery입니다. 올바른 integration 조건은 설명해 주지만 특정 database의 atomicity나 외부 시스템 side
            effect까지 자동으로 보장해 주지는 않습니다.
          </p>
