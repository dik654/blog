...
function Node({
  ...
  if (node.type === "dir") {
    return (
      <div>
        ...
        >
          <span className="text-[10px] w-3 shrink-0 text-[#57606a]">
            {open ? "\u25BE" : "\u25B8"}
          </span>
  ...
  return (
    <button
      onClick={() => node.codeKey && hasRef && onSelect(node.codeKey)}
      disabled={!hasRef}
      style={{ paddingLeft: pl }}
    ...
    >
      ...
      <span className="text-[10px] truncate text-[#24292f] dark:text-[#e6edf3]">
        {node.name}
      </span>
    </button>
  );
...
export default function SP1FileTree({
  root,
  onSelect,
}: {
  root: FileNode;
