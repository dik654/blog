    ...
    annotations: [
      {
        lines: [4, 12],
        color: "sky",
        note: "4가지 프로버 백엔드: CPU, Mock(테스트), Light, CUDA(GPU)",
      },
      {
        lines: [16, 16],
        color: "emerald",
        note: "ProverClient: 진입점. builder 패턴으로 백엔드 선택",
      },
      {
        lines: [21, 22],
        color: "amber",
        note: "Prover 트레이트: setup/prove/verify 인터페이스 정의",
      },
      {
        lines: [24, 27],
        color: "violet",
        note: "핵심 re-export: ELF 로딩, stdin, 검증 키 등",
      },
    ],
  },
    ...
    annotations: [
      {
        lines: [2, 5],
        color: "sky",
        note: "Clone+Send+Sync: 멀티스레드 안전. GAT로 ProveRequest 정의",
      },
      {
        lines: [10, 11],
        color: "emerald",
        note: "setup: ELF를 파싱하여 ProvingKey(+VerifyingKey) 생성",
      },
      {
        lines: [13, 15],
        color: "amber",
        note: "prove: 빌더 패턴 반환. .core()/.compressed()/.groth16() 체이닝",
      },
      {
        lines: [22, 26],
        color: "violet",
        note: "verify: public values 해시 + 증명 자체를 검증",
      },
    ],
  },
};
