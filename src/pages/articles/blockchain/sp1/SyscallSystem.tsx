    ...
    <section id="syscall-system" className="mb-16 scroll-mt-20">
      ...
      <div className="prose prose-neutral dark:prose-invert max-w-none">
        <p>
          ...
          <strong>ECALL 시스템 콜</strong>로 가속합니다.
          <br />
          Guest가 일반 Rust 함수를 호출하면 SP1 패치 크레이트가 자동으로 ECALL로
          대체해 전용 AIR 칩에서 처리합니다.
        </p>
        <CodePanel
          title="Syscall trait"
          code={SYSCALL_TRAIT_CODE}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2 mt-4 mb-6">
        {SYSCALL_CATEGORIES.map((c) => (
          ...
          >
            ...
            {c.items.map((item) => (
              <p key={item} className="text-xs text-foreground/65 font-mono">
                {item}
              </p>
            ))}
          </div>
      ...
      <div className="prose prose-neutral dark:prose-invert max-w-none">
        <CodePanel
          title="ECALL 처리 흐름 (handle_ecall)"
          code={ECALL_CODE}
