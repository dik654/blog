...
const STEPS = [
  {
    label: "GPU 가속 전체",
    body: "CUDA 커널이 NTT, MSM, Poseidon2, FRI를 병렬 처리합니다.",
  },
  { label: "CPU 경로 (~15초)", body: "CPU는 순차 처리로 ~15초가 걸립니다." },
  {
    label: "GPU 경로 (~0.3초)",
    body: "GPU는 수천 코어로 병렬 처리해 ~0.3초 만에 완료합니다.",
  },
];
          ...
          {step !== 1 &&
            OPS.map((op, i) => (
              <motion.line
                ...
                x2={160 + i * 58}
                y2={107}
                stroke={step === 2 ? op.color : "#84cc16"}
                strokeWidth={step === 2 ? 1.5 : 1}
                initial={{ pathLength: 0 }}
                animate={{ pathLength: 1, opacity: step === 2 ? 0.8 : 0.3 }}
