...
const STEPS = [
  ...
  {
    label: "디코딩",
    body: "opcode, 목적지(op_a), 소스(op_b, op_c), 즉시값 플래그를 추출합니다.",
  },
  {
    label: "피연산자 읽기",
    body: "imm_b/imm_c 플래그에 따라 레지스터 또는 즉시값을 읽습니다.",
  },
  {
    label: "연산 수행",
    body: "opcode에 따라 ALU, 메모리(LW/SW), 분기(BEQ/JAL) 연산을 수행합니다.",
  },
  {
    label: "결과 저장",
    body: "연산 결과를 op_a(목적지) 레지스터에 기록합니다.",
  },
  ...
  return (
    <StepViz steps={STEPS}>
      {(step) => (
        <svg
          viewBox="0 0 600 120"
          className="w-[560px] !min-w-[560px] !max-w-none shrink-0 sm:w-full sm:!min-w-0 sm:!max-w-2xl"
          style={{ height: "auto" }}
        >
          <defs>
            <marker
          ...
          {step === 5 && (
            <motion.path
              d={`M ${nx(5) + NW / 2} ${SY + 42} Q ${nx(5) + NW / 2} ${SY + 66} ${nx(0) + NW / 2} ${SY + 66} L ${nx(0) + NW / 2} ${SY + 42}`}
              fill="none"
              stroke="#ec4899"
              strokeWidth={1.2}
              strokeDasharray="4 2"
