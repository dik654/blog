...
const STEPS = [
  ...
  {
    label: "Shrink",
    body: "BabyBear 원소를 BN254 스칼라체로 임베딩해 재증명합니다.",
  },
  {
    label: "Wrap",
    body: "BN254 STARK를 Groth16 R1CS 회로의 입력 형식으로 변환.",
  },
  { label: "Groth16", body: "BN254 페어링 기반 R1CS 증명 생성. ~192바이트." },
  {
    label: "온체인 검증",
    body: "Solidity 검증 컨트랙트에서 ~250k gas로 검증.",
  },
];
  ...
  return (
    <StepViz steps={STEPS}>
      {(step) => (
        <svg
          viewBox="0 0 550 112"
          className="w-[520px] !min-w-[520px] !max-w-none shrink-0 sm:w-full sm:!min-w-0 sm:!max-w-2xl"
          style={{ height: "auto" }}
        >
          <defs>
            <marker
