...
const LAYERS = [
  {
    label: "CPU (64코어)",
    items: ["SHA-256 PC1", "C1 포함증명", "Orchestrator"],
    color: "#6366f1",
  },
  {
    label: "NVMe (16x)",
    items: ["SPDK 직접제어", "스트라이핑", "F2FS"],
    color: "#10b981",
  },
  {
    label: "GPU (RTX 4090)",
    items: ["Poseidon PC2", "MSM/NTT C2", "64 스트림"],
    color: "#f59e0b",
  },
  {
    label: "Memory (512GB)",
    items: ["Huge Pages 1GB", "NUMA 인식", "DMA 고정"],
    color: "#ec4899",
  },
];
    ...
    <StepViz steps={STEPS}>
      {(step) => (
        <div className="max-w-2xl">
          ...
          <motion.p
            className="text-[11px] mt-2 text-muted-foreground/80"
            initial={{ opacity: 0 }}
            animate={{ opacity: 0.8 }}
            key={step}
          >
