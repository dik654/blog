...
const STEPS = [
  {
    label: "Sealing 파이프라인",
    body: "Filecoin 봉인 6단계: Add Piece -> PC1 -> PC2 -> Wait -> C1 -> C2.",
  },
  {
    label: "PC1 병목",
    body: "SHA-256 기반 DRG 그래프 생성이 전체 병목입니다. SupraSeal이 여기를 집중 최적화합니다.",
  },
  {
    label: "PC2 GPU 가속",
    body: "Poseidon 해시를 GPU로 가속하여 머클 트리를 병렬 생성합니다.",
  },
  {
    label: "C1/C2 증명",
    body: "C1 포함 증명과 C2 Groth16 zkSNARK 생성을 GPU로 가속합니다.",
  },
];
              ...
              return (
                <g key={s.label}>
                  ...
                  <motion.text
                    ...
                    fontSize={10}
                    fontWeight={600}
                    animate={{ fill: s.color, opacity: active ? 1 : 0.2 }}
                    transition={sp}
                  >
                    {s.label}
                  </motion.text>
                  <motion.text
                    ...
                    textAnchor="middle"
                    fontSize={9}
                    animate={{ fill: s.color, opacity: active ? 0.5 : 0.1 }}
                    transition={sp}
                  >
                    {s.sub}
                  </motion.text>
