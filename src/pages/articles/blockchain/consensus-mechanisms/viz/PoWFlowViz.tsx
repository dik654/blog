...
const STEPS = [
  {
    label: "트랜잭션 수집",
    body: "채굴자가 mempool에서 트랜잭션을 선택합니다.",
  },
  {
    label: "블록 헤더 구성",
    body: "Merkle root, 이전 블록 해시, 타임스탬프로 헤더를 생성합니다.",
  },
  {
    label: "Nonce 탐색",
    body: "0부터 2^32까지 nonce를 순회하며 유효한 해시를 찾습니다.",
  },
  {
    label: "SHA-256 해싱",
    body: "블록 헤더 + nonce를 SHA-256으로 해싱합니다.",
  },
  {
    label: "난이도 비교",
    body: "해시가 target보다 작으면 성공, 아니면 nonce를 증가시켜 재시도합니다.",
  },
  {
    label: "블록 확정",
    body: "유효한 블록을 네트워크에 전파하고 보상을 받습니다.",
  },
];
        ...
        >
          ...
          {step >= 4 && (
            <motion.g initial={{ opacity: 0 }} animate={{ opacity: 0.5 }}>
              <path
                d={`M${nx(4) + NW / 2},${SY + NH / 2} Q${nx(4) + NW / 2},${SY + 48} ${nx(2) + NW / 2},${SY + NH / 2}`}
                fill="none"
                stroke="#ef4444"
                strokeWidth={1}
                markerEnd="url(#pow-ah)"
