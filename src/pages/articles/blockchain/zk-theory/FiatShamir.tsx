      ...
      <div className="prose prose-neutral dark:prose-invert max-w-none mt-6">
        ...
        <div className="not-prose rounded-lg border-l-4 border-l-blue-500 bg-card p-4 mb-4">
          ...
          <div className="flex items-center justify-center gap-4 my-3">
            ...
            <div className="rounded bg-blue-50 dark:bg-blue-900/20 border border-blue-500/30 p-2 text-center text-sm">
              ...
              <p className="text-xs text-muted-foreground">
                <M>{"e \\leftarrow H(a, \\text{stmt}, \\ldots)"}</M>
              </p>
            </div>
          </div>
        </div>
            ...
            <div className="space-y-1">
              {[
                ...
                },
                {
                  ...
                  t: (
                    <>
                      <M>{"e = H(g, y, a)"}</M> &larr; Verifier 역할 대체
                    </>
                  ),
                },
                {
                ...
                >
                  <span className="text-xs font-mono bg-muted rounded px-1.5 py-0.5 shrink-0">
                    {i.s}
                  </span>
                  <span>{i.t}</span>
                </div>
        ...
        <div className="not-prose rounded-lg border bg-card p-4 mb-4">
          ...
          <p className="text-sm text-muted-foreground">
            해시 <M>H</M>를 random oracle로 가정 &mdash; 입력마다 독립 랜덤
            출력. 실제 <M>H</M>와 차이 있을 수 있음.
            <br />
            보안 분석 (Pointcheval-Stern 1996): ZK 유지 + soundness 감소
            negligible
          </p>
        </div>
        ...
        <div className="not-prose grid grid-cols-1 md:grid-cols-3 gap-3 mb-4">
          <div className="rounded-lg border bg-card p-4">
            ...
            <p className="text-sm text-muted-foreground">
              <M>e</M>는 statement + commitment + public params 모두 포함 필수
              &mdash; 생략 시 공격 가능
            </p>
          </div>
          <div className="rounded-lg border bg-card p-4">
          ...
          <div className="rounded-lg border bg-card p-4">
            ...
            <p className="text-sm text-muted-foreground">
              <M>a</M>만 해싱하면 위험 &mdash; Strong FS: 모든 public data 포함
              필수
            </p>
          </div>
        </div>
        ...
        <div className="not-prose grid grid-cols-2 sm:grid-cols-4 gap-2">
          {[
            ...
            {
              name: "SNARKs/STARKs",
              desc: "모든 ZK 시스템이 FS 사용 → 온체인 검증",
            },
            { name: "Bulletproofs", desc: "Multiple rounds를 FS로 압축" },
            {
              name: "PLONK / Halo2",
              desc: "여러 interactive phase → non-interactive",
            },
          ].map((u) => (
