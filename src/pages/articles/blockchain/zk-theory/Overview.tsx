  ...
  return (
    <section id="overview" className="mb-16 scroll-mt-20">
      ...
      <div className="prose prose-neutral dark:prose-invert max-w-none mb-6">
        <p>
          영지식 증명(Zero-Knowledge Proof, ZKP) — 비밀을 공개하지 않고 "나는 이
          사실을 안다"를 증명하는 암호학적 프로토콜.
          <br />세 가지 성질을 동시에 만족해야 유효한 ZKP다.
        </p>
      </div>
      ...
      <div className="prose prose-neutral dark:prose-invert max-w-none mt-6">
        ...
        <div className="not-prose grid grid-cols-1 md:grid-cols-3 gap-3 mb-4">
          <div className="rounded-lg border bg-card p-4">
            ...
            <p className="text-sm text-muted-foreground mb-2">
              정직한 Prover + 정직한 Verifier &rarr; 참 명제는 항상 승인
            </p>
            <M display>
              {
                "\\forall (\\underbrace{x}_{\\text{공개 입력}}, \\underbrace{w}_{\\text{증인}}) \\in R: \\Pr[\\underbrace{V(x, \\pi)}_{\\text{검증}} = \\text{ACCEPT}] \\geq 1 - \\underbrace{\\text{negl}}_{\\text{무시 가능}}"
              }
            </M>
          ...
          <div className="rounded-lg border bg-card p-4">
            ...
            <M display>
              {
                "\\forall \\underbrace{x \\notin L}_{\\text{거짓 명제}},\\, \\forall \\underbrace{P^*}_{\\text{악의적 증명자}}: \\Pr[V(x, \\underbrace{\\pi^*}_{\\text{위조 증명}}) = \\text{ACCEPT}] \\leq \\underbrace{\\varepsilon}_{\\text{건전성 오류}}"
              }
            </M>
            <p className="text-sm text-muted-foreground mt-2">
              L = 참인 명제의 집합(language), P* = 임의의(악의적) 증명자, pi* =
              위조된 증명, epsilon = 건전성 오류 상한(예: 2^-128). 어떤 전략을
              써도 거짓 명제를 승인받을 확률이 epsilon 이하.
            </p>
          </div>
          <div className="rounded-lg border bg-card p-4">
            ...
            <p className="text-sm text-muted-foreground mb-2">
              Verifier는 "x가 참" 외에 어떤 정보도 얻지 못함
            </p>
            <p className="text-sm text-muted-foreground">
              Simulator <M>S</M> 존재 &mdash; <M>{"S(x)"}</M>만으로 실제
              transcript와 구별 불가능한 분포 생성
            </p>
          </div>
        </div>
        ...
        <div className="not-prose grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3 mb-4">
          {[
            {
              title: "Privacy Blockchains",
              items: [
                "Zcash (Groth16 / Halo2)",
                "Monero (Bulletproofs)",
                "Aztec (private ZK-rollup)",
              ],
            },
            {
              title: "ZK-Rollups (L2)",
            ...
            },
            {
              ...
              items: ["Cloud outsourcing", "zkML (ML 검증)", "Game integrity"],
            },
            {
              title: "Cross-chain Bridges",
              items: ["Light client proofs", "Succinct verification"],
            },
          ].map((c) => (
        ...
        <div className="not-prose rounded-lg border-l-4 border-l-amber-500 bg-card p-4">
          ...
          <p className="text-sm text-muted-foreground">
            $5B+ 투자 (2022-2024) / 수백 개 ZK 프로젝트 / Ethereum rollup 지배 /
            AI + ZK (zkML) 급부상
          </p>
        </div>
      </div>
