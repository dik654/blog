...
const STEPS = [
  {
    label: "왜 소수 p를 쓰는가?",
    body: "소수 mod에서만 모든 원소가 역원을 가진다 — 합성수에선 역원이 존재하지 않는 원소 발생.",
  },
  {
    label: "왜 소수 위수 q의 부분군인가?",
    body: "위수가 합성수면 Pohlig-Hellman 공격으로 소인수별 분해 가능 — 소수 위수면 분해 불가.",
  },
  {
    label: "왜 생성원 g인가?",
    body: "생성원은 부분군 전체를 생성하여 a가 균일 분포 — 비생성원은 일부만 반복.",
  },
];
          ...
          {step === 0 && (
            <motion.g initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
              ...
              {[
                ...
                <text key={i} x={50} y={52 + i * 14} fontSize={9} fill={C.ok}>
                  {t}
                </text>
              ))}
              {/* mod 6 — BAD */}
