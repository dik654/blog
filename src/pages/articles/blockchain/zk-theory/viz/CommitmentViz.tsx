...
const STEPS = [
  {
    label: "Pedersen 커밋먼트 — 세팅",
    body: "소수 p=23 위 군에서 독립 생성원 g, h를 공개. log_g(h)를 모르면 바인딩이 성립하고, 알면 위조가 가능해진다.",
  },
  {
    label: "① Commit — C = g^m * h^r",
    body: "메시지 m과 랜덤 블라인딩 팩터 r로 커밋값 C를 계산한 뒤 공개. m과 r은 비밀로 유지.",
  },
  {
    label: "② Open — (m, r) 공개",
    body: "검증자에게 m, r을 전달하면 C와 대조해서 일치를 확인. 다른 값으로 같은 C를 만들 수 없다.",
  },
  {
    label: "두 가지 성질",
    body: "은닉(Hiding): r이 균일하면 C에서 m을 유추 불가. 바인딩(Binding): 같은 C에 다른 m을 넣으려면 DLP를 풀어야 한다.",
  },
];
...
function OpenDiagram() {
  return (
    <motion.g initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={sp}>
      ...
      <defs>
        <clipPath id="op-clip1">
          <rect x={20} y={36} width={90} height={44} rx={8} />
        </clipPath>
        <clipPath id="op-clip2">
          <rect x={178} y={36} width={130} height={44} rx={6} />
        </clipPath>
        <marker
          id="op-arr"
          markerWidth="6"
  ...
  return (
    <motion.g initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={sp}>
      ...
      >
        ...
        />
        <text x={107} y={68} textAnchor="middle" fontSize={10} fill={C.cm}>
          ?
        </text>
        {/* m = ? box */}
        <rect
      ...
      <defs>
        <clipPath id="hi-clip1">
          <rect x={20} y={32} width={210} height={90} rx={8} />
        </clipPath>
        <clipPath id="hi-clip2">
