...
const STEPS = [
  {
    label: "대화형 Sigma 프로토콜 (3-move)",
    body: "P가 커밋 a를 보내고, V가 랜덤 챌린지 e를 생성, P가 응답 z를 보내는 3단계 프로토콜. V가 온라인이어야 하므로 블록체인에서 사용 불가.",
  },
  {
    label: "Fiat-Shamir: e = H(a, stmt)",
    body: "V의 랜덤 챌린지를 해시 함수로 대체. 커밋 a가 정해진 뒤에만 e가 결정되므로 조작이 불가능하다.",
  },
  {
    label: "비대화형 증명 pi = (a, z)",
    body: "증명자가 혼자 a, e, z를 모두 계산하고 (a, z)만 전송. 검증자는 같은 해시로 e를 복원해 검증한다.",
  },
  {
    label: "Random Oracle Model (ROM)",
    body: "H를 완전한 랜덤 함수로 모델링하면, 대화형과 동일한 건전성이 보장된다. 실제 구현에서는 SHA-256이나 Poseidon을 사용.",
  },
];
    ...
    <motion.g initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={sp}>
      ...
      {moves.map((m, i) => {
        ...
        return (
          ...
          >
            <line
              x1={m.from + (m.dir > 0 ? 4 : -4)}
              y1={m.y}
              x2={m.to + (m.dir > 0 ? -4 : 4)}
              y2={m.y}
              stroke={m.color}
              strokeWidth={0.8}
              markerEnd={`url(#fs-arr-${i})`}
            />
            <rect
              x={midX - 44}
              y={m.y - 18}
      ...
      >
        ...
        <text
          ...
          fill="var(--muted-foreground)"
        >
          V 온라인 필요 → 블록체인에서 사용 불가
        </text>
      </motion.g>

      <defs>
        <clipPath id="fs-pclip">
          <rect x={pX} y={28} width={boxW} height={boxH} rx={8} />
        </clipPath>
        <clipPath id="fs-vclip">
    ...
    <motion.g initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={sp}>
      ...
      >
        ...
        <rect
          ...
          strokeWidth={0.4}
        />
        <text x={263} y={72} textAnchor="middle" fontSize={7} fill={C.fs}>
          mod q
        </text>
      </motion.g>
