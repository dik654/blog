...
const STEPS = [
  {
    label: "증명 시스템 분류",
    body: "SNARKs, STARKs, IOP — 각각 다른 가정과 트레이드오프를 가진 영지식 증명 체계.",
  },
  {
    label: "SNARKs — 간결한 비대화형 증명",
    body: "Trusted Setup 필요. 증명 크기 O(1), 검증 O(1). Groth16, PLONK 등. 타원곡선 기반.",
  },
  {
    label: "STARKs — 투명 셋업 + 양자 내성",
    body: "해시 기반 투명 설정, 양자 내성. 증명 크기 O(log^2 n).",
  },
  {
    label: "IOP — 이론적 프레임워크",
    body: "증명자가 오라클 제출, 검증자가 쿼리하는 STARK/SNARK의 이론적 토대.",
  },
];
                  ...
                  return (
                    <motion.g key={vi} animate={{ opacity: op }}>
                      <rect
                        ...
                        height={18}
                        rx={3}
                        fill={`${SYSTEMS[vi].color}08`}
                        stroke={SYSTEMS[vi].color}
                        strokeWidth={0.5}
                      />
                      <text
                        x={SYSTEMS[vi].x}
