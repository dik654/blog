  ...
  return (
    <article className="space-y-14">
      ...
      <section id="sigma" className="space-y-6">
        ...
        <p>작은 cyclic group에서 public key Y=gʷ이고 prover가 discrete log w를 안다는 Schnorr protocol을 보겠습니다. Prover는 nonce r로 R=gʳ을 먼저 보내고, verifier가 예측 불가능한 e를 고르면 s=r+ew로 답합니다. Verifier는 gˢ=R·Yᵉ를 검사합니다.</p>
        <ExplainedFormula
      ...
      <section id="simulation" className="space-y-6">
        ...
        />
        <p>Commitment의 hiding과 ZK도 동일하지 않습니다. 예를 들어 Pedersen commitment C=gᵐhʳ은 r이 uniform이고 log_g h가 알려지지 않을 때 m을 perfectly hiding하면서 DLP 아래 binding을 가질 수 있습니다. 하지만 protocol이 m의 함수나 reuse된 r을 다른 메시지로 흘리면 commitment 하나의 hiding만으로 전체 protocol ZK가 되지 않습니다.</p>
        <ExplainedFormula
      ...
      <section id="noninteractive-boundary" className="space-y-6">
        ...
        <p>Fiat–Shamir에서는 e=H(protocol-id, version, statement, public key, R, context)처럼 challenge를 계산합니다. Statement나 chain/domain을 빼면 proof가 다른 맥락에서 replay될 수 있고, ambiguous serialization은 서로 다른 입력이 같은 byte string을 만들 수 있습니다. Multi-round protocol을 단순히 마지막 메시지 하나만 hash하는 것으로 일반화해서도 안 됩니다.</p>
        <p>ZK release gate에는 honest accept, wrong witness/public input, malformed/subgroup point, nonce reuse, altered statement/context, transcript reorder, replay, simulator-distribution test를 포함합니다. False accept와 witness leakage를 먼저 확인한 뒤 prove/verify time, proof bytes, CRS·memory를 비교합니다. Side channel·randomness failure는 추상 ZK theorem이 자동으로 막지 못합니다.</p>
