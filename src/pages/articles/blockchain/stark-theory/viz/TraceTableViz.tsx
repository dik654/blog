...
const STEPS = [
  ...
  {
    label: "행 0: 초기값",
    body: "a₀=1, b₀=1. 경계 제약(boundary constraint)이 이 값을 고정한다.",
  },
  {
    label: "행 1–2: 전이 적용",
    body: "전이 제약(transition constraint): 다음 행은 이전 행에서 결정된다.",
  },
  {
    label: "행 3–5: 추적 완성",
    body: "6행 × 2열 테이블 = 실행 추적. 각 열을 다항식으로 보간한다.",
  },
];
          ...
          >
            ...
            {ROWS.map((row, i) =>
              row.map((val, j) => (
                <motion.g
                  key={`${i}-${j}`}
                  animate={{ opacity: i < visRows ? 1 : 0.08 }}
                ...
                >
                  <rect
                    ...
                    height={RH}
                    fill={
                      i < visRows ? `${j === 0 ? C1 : C2}08` : "transparent"
                    }
                    stroke="var(--border)"
                    strokeWidth={0.3}
                  />
