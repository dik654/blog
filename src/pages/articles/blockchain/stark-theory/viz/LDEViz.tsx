...
const STEPS = [
  {
    label: "LDE 개요 -- 왜 확장하는가?",
    body: "8점만 검사하면 가짜도 통과. blowup rho=4로 32점 확장 시 오류가 증폭되어 탐지 가능.",
  },
  {
    label: "Trace → 다항식 보간",
    body: "8개 trace 값을 IFFT로 유일한 degree-7 다항식 f(x)로 변환.",
  },
  {
    label: "LDE: 32개 점으로 확장 평가",
    body: "동일 다항식 f(x)를 32개 점에서 평가 후 Merkle 커밋. 저차 다항식이면 모든 점이 곡선 위.",
  },
  {
    label: "가짜 trace → 오류 증폭",
    body: "Schwartz-Zippel 보조정리: 가짜 다항식은 78%+ 점에서 불일치. FRI 쿼리로 탐지.",
  },
];
      ...
      {(step) => (
        ...
        >
          ...
          {step === 1 && (
            ...
            >
              {/* Trace values */}
              <text x={20} y={18} fontSize={9} fontWeight={600} fill={C.tr}>
                Trace 값
              </text>
              {[
                { x: "omega^0", v: "1" },
            ...
            >
              ...
              >
                <rect
                  ...
                  strokeWidth={0.4}
                />
                <text x={30} y={70} fontSize={8} fill="var(--muted-foreground)">
                  LDE 도메인 (32점)
                </text>
                ...
                {Array.from({ length: 16 }).map((_, i) => (
                  <motion.circle
                    ...
                    cy={
                      [
                        105, 80, 95, 85, 75, 100, 90, 80, 110, 95, 80, 100, 85,
                        75, 105, 90,
                      ][i]
                    }
                    r={3}
            ...
            >
              ...
              >
                <rect
                  ...
                  strokeWidth={0.5}
                />
                <text x={30} y={68} fontSize={7.5} fill={C.lde}>
                  정상: 모든 점이 곡선 위
                </text>
                {/* curve + dots aligned */}
                <line
              ...
              >
                <rect
                  ...
                  strokeWidth={0.5}
                />
                <text x={260} y={68} fontSize={7.5} fill={C.err}>
                  가짜: 78%+ 점이 곡선 이탈
                </text>
