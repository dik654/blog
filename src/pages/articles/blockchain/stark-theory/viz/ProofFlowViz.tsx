...
const STEPS = [
  {
    label: "STARK 증명 파이프라인 개요",
    body: "5단계로 실행 추적의 정당성을 다항식 저차 검증으로 환원. Trusted Setup 불필요.",
  },
  {
    label: "1. Trace Commit",
    body: "실행 추적을 다항식으로 보간 후 LDE 확장, Merkle Root로 커밋하여 바인딩.",
  },
  {
    label: "2. Constraint Composition",
    body: "랜덤 계수로 제약을 합산하고 vanishing polynomial로 나눠 몫 Q(x)를 생성.",
  },
  {
    label: "3. FRI -- 저차 검증",
    body: "다항식을 반복 접어(fold) degree를 절반으로 줄여 최종 상수가 되면 저차 확인.",
  },
  {
    label: "4-5. Query & Verify",
    body: "랜덤 쿼리 위치에서 Merkle proof + 제약 + FRI 일관성을 검사해 accept/reject.",
  },
];
                ...
                {["T₁", "T₂", "B₁", "B₂"].map((name, i) => (
                  <g key={i}>
                    <DataBox
                      ...
                      h={26}
                      label={name}
                      color={i < 2 ? C.tr : C.fri}
                    />
                  </g>
                ))}
                <text x={295} y={70} fontSize={9} fill={C.cst}>
                  → C(x) = Sum alpha_i * c_i(x)
                </text>
              </motion.g>
