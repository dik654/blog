...
const STEPS = [
  {
    label: "Table T 정의",
    body: "허용 값 집합 T를 정의하고 보간 다항식 t(x)로 변환. range check, XOR 등에 활용.",
  },
  {
    label: "Query 벡터 f",
    body: "증명 대상 값 f의 모든 원소가 T에 포함되는지 검증. Grand product로 O(n+m) 달성.",
  },
  {
    label: "정렬 병합 s = sort(T U f)",
    body: "T와 f를 합쳐 정렬. s의 연속 원소가 같거나 T의 다음 값이면 포함관계 성립.",
  },
  {
    label: "Grand Product Z(X)",
    body: "beta, gamma로 누적곱 Z(X)를 구성. Z가 1로 돌아오면 정렬 조건 성립 즉 f가 T의 부분집합.",
  },
];
    ...
    <StepViz steps={STEPS}>
      {(step) => (
        ...
        >
          ...
          <motion.g
            initial={{ opacity: 0 }}
            animate={{ opacity: step >= 0 ? 1 : 0.15 }}
            transition={sp}
          >
            <text x={20} y={18} fontSize={9} fontWeight={600} fill={C.t}>
              Table T (allowed values)
            </text>
            {T.map((v, i) => (
              <g key={`t-${i}`}>
                <motion.rect
          ...
          <motion.g
            ...
            animate={{ opacity: step >= 1 ? 1 : 0.1, y: step >= 1 ? 0 : 8 }}
            transition={sp}
          >
            <text x={20} y={68} fontSize={9} fontWeight={600} fill={C.f}>
              Query f
            </text>
            {F.map((v, i) => (
              <g key={`f-${i}`}>
          ...
          <motion.g
            initial={{ opacity: 0, y: 8 }}
            animate={{ opacity: step >= 2 ? 1 : 0.05, y: step >= 2 ? 0 : 8 }}
            transition={sp}
          >
            <text x={20} y={118} fontSize={9} fontWeight={600} fill={C.s}>
              s = sort(T U f)
            </text>
            {S.map((v, i) => {
