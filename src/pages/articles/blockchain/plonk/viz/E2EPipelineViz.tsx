...
const STEPS = [
  {
    label: "① 회로 작성 — 게이트 + 와이어",
    body: "개발자가 Composer API로 산술 회로를 구성한다.",
  },
  {
    label: "② 컴파일 — 다항식 전처리",
    body: "선택자/순열을 Lagrange 보간 후 KZG 커밋.",
  },
  {
    label: "③ SRS + 키생성",
    body: "MPC 세레모니 SRS로 prover key와 verifier key 산출.",
  },
  {
    label: "④ 증명 — 5-Round 프로토콜",
    body: "witness 대입 → 커밋 → 몫 → 평가 → 오프닝. O(n log n).",
  },
  {
    label: "⑤ 검증 — 페어링 2회",
    body: "검증자는 π + public inputs만으로 O(1) 검증. 768B 증명.",
  },
];
    ...
    <StepViz steps={STEPS}>
      {(step) => (
        ...
        >
            ...
            return (
              <g key={p.label}>
                <motion.rect
                  ...
                  rx={4}
                  animate={{
                    fill: active ? `${p.color}20` : `${p.color}06`,
                    stroke: p.color,
                ...
                {i > 0 && (
                  <motion.line
                    ...
                    stroke={p.color}
                    strokeWidth={0.7}
                    animate={{ opacity: active ? 0.5 : 0.08 }}
                    transition={sp}
                  />
                )}
              </g>
          ...
          <motion.g
            animate={{ opacity: step >= 4 ? 0.7 : 0.15 }}
            transition={sp}
          >
