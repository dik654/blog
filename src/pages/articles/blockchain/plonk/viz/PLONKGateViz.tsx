...
const STEPS = [
  {
    label: "덧셈 게이트 — q_L·a + q_R·b + q_O·c = 0",
    body: "q_L=1, q_R=1, q_O=-1. R1CS에서는 별도 제약 필요, PLONKish에서는 동일 게이트.",
  },
  {
    label: "곱셈 게이트 — q_M·a·b + q_O·c = 0",
    body: "q_M=1, q_O=-1. 비선형항 a·b를 셀렉터 하나로 표현.",
  },
  {
    label: "상수 게이트 — q_L·a + q_C = 0",
    body: "q_L=1, q_C=-k. 상수값 k를 와이어에 바인딩.",
  },
  {
    label: "커스텀 게이트 — 복합식 1개 제약",
    body: "q_L=q_R=q_M=1, q_O=-1. R1CS는 ~2 제약이지만 PLONKish는 1개.",
  },
];
                ...
                {g.vals.map((v, vi) => (
                  <motion.g key={vi}>
                    <motion.rect
                      ...
                      rx={2}
                      animate={{
                        fill: active ? `${g.color}25` : `${g.color}06`,
                        stroke: g.color,
