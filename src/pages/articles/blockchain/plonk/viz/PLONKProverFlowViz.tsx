...
const STEPS = [
  {
    label: "Round 1 → Fiat-Shamir β,γ",
    body: "Wire commits [a],[b],[c]를 KZG로 전송. transcript 해시로 β,γ 도출.",
  },
  {
    label: "Round 2 → Fiat-Shamir α",
    body: "Z(x) grand product로 copy constraint 검증. [Z]₁ 전송 후 α 도출.",
  },
  {
    label: "Round 3 → Fiat-Shamir ζ",
    body: "t(x) = 모든 제약/Zₕ(x). 3개 청크 commit 후 ζ 도출.",
  },
  {
    label: "Round 4 → Fiat-Shamir ν",
    body: "ζ에서 6개 스칼라 평가값 전송. ν 도출.",
  },
  {
    label: "Round 5 → Verifier",
    body: "배치 KZG opening으로 W_ζ, W_ζω 생성. 페어링 2회로 O(1) 검증.",
  },
];
          ...
          {ROWS.map((row, i) => {
            ...
            if ("fs" in row) {
              ...
              return (
                <motion.g
                  key={i}
                  animate={{ opacity: active ? 0.6 : 0.1 }}
                  transition={sp}
                >
                  <line
                    x1={OX + RW - 4}
            ...
            return (
              <g key={i}>
                <motion.rect
                  ...
                  rx={3}
                  animate={{
                    fill: active ? `${row.color}18` : `${row.color}06`,
                    stroke: row.color!,
                    strokeWidth: glow ? 1.8 : 0.4,
