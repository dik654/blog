...
const STEPS = [
  {
    label: "Universal Setup -- SRS",
    body: "MPC 세레모니로 비밀 tau를 생성하고, tau의 거듭제곱을 타원곡선 점으로 인코딩. 1회 생성 후 재사용.",
  },
  {
    label: "Commit -- 다항식을 G1 점으로",
    body: "f(x)의 계수와 SRS를 MSM으로 결합하여 G1 점 하나(64바이트)로 압축. tau를 모르지만 계산 가능.",
  },
  {
    label: "Open -- 인수정리로 증명",
    body: "f(z)=y를 증명하려면 q(x)=(f(x)-y)/(x-z)를 계산. 인수정리에 의해 q가 존재하면 f(z)=y 참.",
  },
  {
    label: "Verify -- 페어링 2회 O(1)",
    body: "e([f]-y*G1, G2) = e([q], [tau]-z*G2). 페어링 2회로 회로 크기와 무관하게 상수 시간 검증.",
  },
];
  ...
  return (
    <StepViz steps={STEPS}>
      {(step) => (
        ...
        >
          {NODES.map((n, i) => {
            const activeNodes = [...Array(step + 1)].flatMap(
              (_, s) => stepMap[s] || [],
            );
          ...
          <motion.g
            initial={{ opacity: 0 }}
            animate={{ opacity: step >= 0 ? 0.5 : 0.05 }}
            transition={sp}
          >
            <line
              x1={50}
          ...
          <motion.g
            initial={{ opacity: 0 }}
            animate={{ opacity: step >= 1 ? 0.5 : 0.05 }}
            transition={sp}
          >
            <line
              x1={155}
          ...
          <motion.g
            initial={{ opacity: 0 }}
            animate={{ opacity: step >= 2 ? 0.5 : 0.05 }}
            transition={sp}
          >
            <line
              x1={190}
