    ...
    <section id="kzg" className="mb-16 scroll-mt-20">
      ...
      <div className="prose prose-neutral dark:prose-invert max-w-none">
        <div className="not-prose flex flex-wrap gap-2 my-4">
          <CodeViewButton
            onClick={() => onCodeRef("kzg-srs", codeRefs["kzg-srs"])}
          />
          <span className="text-[10px] text-muted-foreground self-center">
            SRS 구조체
          </span>
          <CodeViewButton
            onClick={() => onCodeRef("kzg-commit", codeRefs["kzg-commit"])}
          />
          <span className="text-[10px] text-muted-foreground self-center">
            commit() + open()
          </span>
          <CodeViewButton
            onClick={() => onCodeRef("kzg-verify", codeRefs["kzg-verify"])}
          />
          <span className="text-[10px] text-muted-foreground self-center">
            verify() 페어링
          </span>
        ...
        <p>
          Groth16은 <strong>회로별 trusted setup</strong>이 필요하다.
          <br />
          회로가 바뀌면 setup을 처음부터 다시 해야 한다.
          <br />
          KZG(Kate-Zaverucha-Goldberg)는 <strong>universal setup</strong>으로 이
          문제를 해결한다.
          <br />
          비밀{" "}
        ...
        <div className="not-prose my-4">
          <div className="rounded-lg border border-sky-500/30 p-4">
            ...
            <p className="text-sm text-muted-foreground mt-2">
              즉, <M>{"f(x) - y = q(x) \\cdot (x - z)"}</M>인 다항식{" "}
              <M>{"q(x)"}</M>가 존재
            </p>
            <div className="mt-3 rounded border border-emerald-500/30 p-3">
              ...
              <ol className="text-sm space-y-0.5 text-muted-foreground list-decimal pl-5">
                ...
                <li>
                  <M>{"q(x) = (f(x) - y) / (x - z)"}</M>를 계산해서 제출
                </li>
                <li>검증자: 타원곡선 위에서 pairing으로 확인</li>
              </ol>
              <M display>
                {"e([q]_1,\\; [\\tau - z]_2) = e([f - y]_1,\\; G_2)"}
              </M>
            </div>
          </div>
        </div>
        ...
        <p>
          비밀{" "}
          <code className="bg-accent px-1.5 py-0.5 rounded text-sm">&tau;</code>
          를 직접 노출하지 않고, 타원곡선 위의 점으로 인코딩한다.
        </p>
        <div className="not-prose my-4">
          <div className="rounded-lg border border-border/60 p-4">
            ...
            <M display>
              {
                "\\text{SRS} = \\{ [\\tau^0]_1, [\\tau^1]_1, \\ldots, [\\tau^d]_1, [\\tau]_2 \\}"
              }
            </M>
            <p className="text-sm text-muted-foreground mt-2">
              여기서 <M>{"[x]_1 = x \\cdot G_1,\\; [x]_2 = x \\cdot G_2"}</M>.{" "}
              <M>{"\\tau"}</M> 자체는 MPC 세레모니 후 폐기 (toxic waste)
            </p>
          </div>
        </div>
        ...
        <div className="not-prose my-4 space-y-3">
          ...
          <div className="rounded-lg border border-violet-500/30 p-4">
            ...
            <M display>
              {
                "e(\\pi,\\; [\\tau - z]_2) \\stackrel{?}{=} e(C - [y]_1,\\; G_2)"
              }
            </M>
            <p className="text-sm text-muted-foreground mt-1">
              양변 = <M>{"[q(\\tau)(\\tau - z)]_T = [(f(\\tau) - y)]_T"}</M>
            </p>
          </div>
        </div>
        ...
        <p>
          여러 다항식을 같은 점{" "}
          <code className="bg-accent px-1.5 py-0.5 rounded text-sm">z</code>에서
          한 번에 열 수 있다. 검증자가 랜덤{" "}
          <code className="bg-accent px-1.5 py-0.5 rounded text-sm">&nu;</code>
          를 선택하여 선형 결합한다.
        </p>
        <div className="not-prose my-4">
          <div className="rounded-lg border border-border/60 p-4">
            ...
            <div className="rounded border border-sky-500/30 p-3 mb-3">
              ...
              <M display>
                {
                  "\\text{combined}(x) = f_0(x) + \\nu \\cdot f_1(x) + \\nu^2 \\cdot f_2(x) + \\cdots"
                }
              </M>
              <M display>
                {
              ...
              <p className="text-sm text-muted-foreground mt-1">
                ...
                <M>
                  {
                    "q(x) = (\\text{combined}(x) - \\text{combined\\_y}) / (x - z)"
                  }
                </M>
              </p>
            </div>
