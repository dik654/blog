...
export const plonkishCodeRefs: Record<string, CodeRef> = {
  "plonk-gate": {
    ...
    lang: "rust",
    highlight: [1, 22],
    desc: `ArithmeticGate는 PLONK의 범용 게이트 제약을 정의합니다.
...
pub(crate) fn compute_quotient_i(
    ...
    annotations: [
      {
        lines: [1, 2],
        color: "sky",
        note: "범용 게이트 등식 — quotient polynomial의 i번째 항 계산",
      },
      {
        lines: [4, 7],
        color: "emerald",
        note: "4개 wire 값 (a, b, c, d) — w_4는 TurboPlonk 확장",
      },
      {
        lines: [16, 20],
        color: "amber",
        note: "5개 selector: q_M(곱셈), q_L(좌), q_R(우), q_O(출력), q_C(상수)",
      },
      {
        lines: [22, 22],
        color: "violet",
        note: "최종 게이트 계산 — 0이면 제약 만족",
      },
    ],
  },
  ...
  "plonk-copy": {
    ...
    lang: "rust",
    highlight: [1, 28],
    desc: `Permutation argument는 copy constraint를 다항식으로 인코딩합니다.
    ...
    annotations: [
      {
        lines: [2, 8],
        color: "sky",
        note: "Z(x) 계산 진입 — beta, gamma는 Fiat-Shamir 챌린지",
      },
      {
        lines: [11, 11],
        color: "emerald",
        note: "Z(ω⁰) = 1 — accumulator 초기값",
      },
      {
        lines: [15, 19],
        color: "amber",
        note: "분자: 원래 위치 기반 — (wᵢ + β·ωⁱ·kⱼ + γ)",
      },
      {
        lines: [20, 24],
        color: "violet",
        note: "분모: permutation 위치 기반 — (wᵢ + β·σ(i) + γ)",
      },
    ],
  },
};
