  ...
  return (
    <section id="circuit-compilation" className="mb-16 scroll-mt-20">
      ...
      <div className="prose prose-neutral dark:prose-invert max-w-none">
        ...
        <p>
          Composer API로 <strong>게이트와 와이어</strong>를 정의한다. 산술,
          범위, 논리 게이트를 조합하여 원하는 연산을 표현한다.
        </p>
        ...
        <p>
          게이트의 선택자 값을 <strong>Lagrange 보간</strong>하여 다항식으로
          변환하고, KZG로 커밋한다.
        </p>
        <div className="not-prose my-4">
          <div className="rounded-lg border border-border/60 p-4">
            ...
            <ol className="text-sm space-y-2 text-muted-foreground list-decimal pl-5">
              <li className="rounded border border-sky-500/30 p-2">
                ...
                <M>{"q_M(X), q_L(X), \\ldots"}</M> → 게이트별 selector를
                Lagrange 보간
              </li>
              <li className="rounded border border-amber-500/30 p-2">
                <span className="text-amber-400 font-medium">순열 다항식:</span>{" "}
                <M>{"\\sigma_a(X), \\sigma_b(X), \\sigma_c(X)"}</M> → copy
                constraint 위치 매핑
              </li>
              <li className="rounded border border-violet-500/30 p-2">
                <span className="text-violet-400 font-medium">KZG commit:</span>{" "}
                <M>{"[q_M]_1, [q_L]_1, \\ldots, [\\sigma_a]_1, \\ldots"}</M> →
                검증 키에 포함
              </li>
            </ol>
          </div>
        ...
        <p>
          SRS + 전처리 결과로 <strong>Prover Key</strong>와{" "}
          <strong>Verifier Key</strong>를 생성한다.
        </p>
        <div className="not-prose my-4">
          <div className="rounded-lg border border-border/60 p-4">
            <p className="font-semibold text-sm text-muted-foreground mb-2">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              ...
              <div className="rounded border border-violet-500/30 p-3">
                ...
                <ul className="text-sm space-y-0.5 text-muted-foreground">
                  <li>
                    선택자 commitments <M>{"[q_M]_1, [q_L]_1, \\ldots"}</M>
                  </li>
                  <li>
                    순열 commitments{" "}
                    <M>{"[\\sigma_a]_1, [\\sigma_b]_1, [\\sigma_c]_1"}</M>
                  </li>
                  <li>
                    <M>{"[\\tau]_2"}</M> (SRS에서)
                  </li>
                  <li>
                    도메인 크기 <M>{"n"}</M>, 생성자 <M>{"\\omega"}</M>
                  </li>
                </ul>
              </div>
            </div>
