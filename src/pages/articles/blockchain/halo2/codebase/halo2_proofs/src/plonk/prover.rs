...
>(
            ...
            pk.vk
                ...
                .map(|lookup| {
                    lookup.commit_permuted(
                        pk, params, domain,
                        &value_evaluator, &mut coset_evaluator,
                        theta,
                        advice_values, &fixed_values, instance_values,
                        advice_cosets, &fixed_cosets, instance_cosets,
                        &mut rng, transcript,
                    )
                })
                .collect()
    ...
    let permutations: Vec<permutation::prover::Committed<C, _>> = instance
        ...
        .map(|(instance, advice)| {
            pk.vk.cs.permutation.commit(
                params, pk, &pk.permutation,
                &advice.advice_values, &pk.fixed_values, &instance.instance_values,
                beta, gamma,
                &mut coset_evaluator, &mut rng, transcript,
            )
        })
        .collect::<Result<Vec<_>, _>>()?;
    ...
    let vanishing = vanishing.construct(
        params, domain, coset_evaluator, expressions, y, &mut rng, transcript,
    )?;
    ...
    let instances = instance
        ...
        .flat_map(|(((instance, advice), permutation), lookups)| {
            iter::empty()
                .chain(
                    pk.vk.cs.instance_queries.iter()
