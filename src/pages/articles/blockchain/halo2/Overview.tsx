  ...
  return (
    <section id="overview" className="mb-16 scroll-mt-20">
      ...
      <div className="prose prose-neutral dark:prose-invert max-w-none">
        <p>
          <strong>Halo2</strong>(zcash/halo2)는 KZG 다항식 커밋 + PLONKish
          산술화 기반의 ZK 증명 프레임워크입니다. 개발자는 <code>Circuit</code>{" "}
          트레이트를 구현하여 회로를 정의하고, <code>create_proof</code>로
          증명을 생성합니다.
        </p>
        {/* 크레이트 구조 */}
        <div className="not-prose rounded-lg border border-border/60 p-4 mb-4">
          <p className="font-semibold text-sm text-blue-400 mb-3">
            크레이트 구조 (halo2_proofs/src/)
          </p>
          <div className="grid grid-cols-2 gap-2 text-sm">
            <div className="rounded border bg-card p-3">
              ...
              <p className="text-xs text-muted-foreground mt-1">
                keygen, prover, verifier &mdash; 증명 파이프라인 핵심
              </p>
            </div>
            <div className="rounded border bg-card p-3">
              <p className="font-semibold">
                <code>circuit.rs</code>
              </p>
              <p className="text-xs text-muted-foreground mt-1">
                <code>Circuit</code> 트레이트, <code>Layouter</code>,{" "}
                <code>Region</code> 정의
              </p>
            </div>
            <div className="rounded border bg-card p-3">
              ...
              <p className="text-xs text-muted-foreground mt-1">
                다항식 연산 &mdash; FFT, KZG/IPA commitment
              </p>
            </div>
            <div className="rounded border bg-card p-3">
              <p className="font-semibold">
                <code>transcript.rs</code>
              </p>
              <p className="text-xs text-muted-foreground mt-1">
                Fiat-Shamir 트랜스크립트 (Blake2b / Poseidon)
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="not-prose rounded-lg border border-border/60 p-4 mb-4">
          <p className="font-semibold text-sm text-emerald-400 mb-3">
            Circuit 트레이트 (circuit.rs)
          </p>
          <div className="space-y-2 text-sm">
            <div className="rounded border bg-card p-3">
              <p className="font-semibold">
                <code>{"type Config"}</code>
              </p>
              <p className="text-xs text-muted-foreground mt-1">
                회로가 사용하는 열/게이트 설정 묶음 &mdash;{" "}
                <code>configure()</code>에서 반환
              </p>
            </div>
            <div className="rounded border bg-card p-3">
              <p className="font-semibold">
              ...
              <p className="text-xs text-muted-foreground mt-1">
                열 선언, custom gate 등록, lookup 추가 &mdash; keygen 시점에 1회
                호출
              </p>
            </div>
            <div className="rounded border bg-card p-3">
              <p className="font-semibold">
                <code>
                  {
                    "fn synthesize(config: Self::Config, layouter: impl Layouter<F>)"
                  }
                </code>
              </p>
              <p className="text-xs text-muted-foreground mt-1">
                witness 할당 + 제약 배치 &mdash; keygen과 proving 양쪽에서 호출
              </p>
            </div>
          </div>
        </div>
        ...
        <p>
          PLONKish 회로는 2차원 그리드(rows x columns)로 구성됩니다. R1CS가 단일
          곱셈 게이트만 표현하는 것과 달리, PLONKish는 임의 차수의 다항식 제약을
          허용합니다.
        </p>
        ...
        <div className="not-prose rounded-lg border-l-4 border-l-purple-500 bg-card p-4 mb-4">
          <p className="font-semibold text-sm text-purple-400 mb-2">
            Custom Gate &mdash; 다항식 제약
          </p>
          <M display>{"s(X) \\cdot \\bigl(a(X) + b(X) - c(X)\\bigr) = 0"}</M>
          <p className="text-xs text-muted-foreground mt-2">
            selector <M>{"s"}</M> 가 1인 행에서 <M>{"a + b = c"}</M> 를 강제.
            rotation을 사용하면 <M>{"a(X) \\cdot b(X \\cdot \\omega)"}</M> 처럼
            인접 행 참조 가능
          </p>
        </div>
        ...
        <div className="not-prose rounded-lg border border-border/60 p-4 mb-4">
          ...
          <div className="grid grid-cols-2 gap-2 text-sm">
            <div className="rounded border bg-card p-3">
              ...
              <p className="text-xs text-muted-foreground mt-1">
                R1CS는 2차(bilinear)만 가능, PLONKish는 임의 차수 제약
              </p>
            </div>
            <div className="rounded border bg-card p-3">
              <p className="font-semibold">행당 다중 제약</p>
              <p className="text-xs text-muted-foreground mt-1">
                R1CS는 행당 1개, PLONKish는 여러 gate를 한 행에 적용
              </p>
            </div>
            <div className="rounded border bg-card p-3">
              <p className="font-semibold">Column Rotation</p>
              <p className="text-xs text-muted-foreground mt-1">
                <M>{"a(X \\cdot \\omega^k)"}</M> 로 k행 떨어진 셀 참조 &mdash;
                상태 전파 표현
              </p>
            </div>
            <div className="rounded border bg-card p-3">
        ...
        <div className="not-prose space-y-3 mb-4">
          <div className="rounded-lg border-l-4 border-l-sky-500 bg-card p-4">
            ...
            <ul className="text-sm text-muted-foreground space-y-1">
              <li>
                Universal trusted setup &mdash; 하나의 setup으로 다양한 회로
                사용
              </li>
              <li>Polynomial commitment 기반 증명</li>
              <li>
                Custom gates 도입 &mdash; 고정된 덧셈/곱셈 게이트를 넘어 확장
              </li>
            </ul>
          </div>
          <div className="rounded-lg border-l-4 border-l-emerald-500 bg-card p-4">
            <p className="font-semibold text-sm text-emerald-400 mb-2">
              Halo2 (2020, ZCash) &mdash; PLONK 확장
            </p>
            <ul className="text-sm text-muted-foreground space-y-1">
              <li>
                Lookup arguments 추가 (Plookup) &mdash; 테이블 검증으로 range
                check 효율화
              </li>
              <li>Custom gates 확장 &mdash; 고차 다항식 + rotation 조합</li>
              <li>
                Rust 구현 (<code>zcash/halo2</code>)
              </li>
            </ul>
          </div>
        </div>
        ...
        <div className="not-prose rounded-lg border border-border/60 p-4 mb-4">
          ...
          <div className="grid grid-cols-3 gap-2 text-sm">
            <div className="rounded border bg-card p-3">
              ...
              <p className="text-xs text-muted-foreground mt-1">
                zkEVM에 필수 &mdash; range check, bytecode 검증에 사용
              </p>
            </div>
            <div className="rounded border bg-card p-3">
              <p className="font-semibold">유연한 커밋 스킴</p>
              <p className="text-xs text-muted-foreground mt-1">
                KZG(trusted setup) 또는 IPA(trustless) 선택 가능
              </p>
            </div>
            <div className="rounded border bg-card p-3">
              <p className="font-semibold">생태계</p>
              <p className="text-xs text-muted-foreground mt-1">
                ZCash Orchard, Scroll zkEVM, PSE, Axiom, Taiko
              </p>
            </div>
          </div>
        </div>
