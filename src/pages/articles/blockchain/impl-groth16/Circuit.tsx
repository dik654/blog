    ...
    <section id="circuit" className="mb-16 scroll-mt-20">
      ...
      <div className="prose prose-neutral dark:prose-invert max-w-none mb-8">
        <p className="leading-7">
          Circuit trait — synthesize(&self, cs) 하나만 구현하면
          setup/prove/verify 모두에서 재사용
          <br />
        ...
        <p className="leading-7">
          ...
          <br />
          마지막 제약에서 .add()로 x와 상수 5를 연결 — 추가 제약 없이
          선형결합으로 표현
        </p>
      </div>
      ...
      <div className="prose prose-neutral dark:prose-invert max-w-none">
        ...
        <p className="leading-7">
          ...
          <br />
          조건문(if-else)도 R1CS로 표현 가능: b·(1-b)=0(부울) + b·(x-y)=t +
          (y+t)·1=result
          <br />
          is_satisfied()로 회로 디버깅 → which_unsatisfied()로 실패 제약 위치
          확인
        </p>
      </div>
      ...
      <div className="prose prose-neutral dark:prose-invert max-w-none mt-6">
        ...
        <div className="bg-muted/50 rounded-lg p-4 mb-4">
          ...
          <div className="text-sm space-y-1">
            <p>
              <code>Circuit&lt;F: Field&gt;</code> —{" "}
              <code>
                synthesize(self, cs: &mut CS) -&gt; Result&lt;(),
                SynthesisError&gt;
              </code>{" "}
              하나만 구현
            </p>
            <p className="text-muted-foreground">
              동일 코드가 R1CS 생성(setup)과 witness 생성(proving) 모두에 사용
            </p>
          </div>
        </div>
        ...
        <div className="bg-muted/50 rounded-lg p-4 mb-4">
          ...
          <div className="grid grid-cols-1 gap-2 text-sm">
            <div className="bg-background rounded p-3">
              <p>
                <code>alloc_instance(&mut self, val: F) -&gt; Variable</code> —
                공개 변수 할당 (verifier에게 보임)
              </p>
            </div>
            <div className="bg-background rounded p-3">
              <p>
                <code>alloc_witness(&mut self, val: F) -&gt; Variable</code> —
                비공개 변수 할당 (prover만 앎)
              </p>
            </div>
            <div className="bg-background rounded p-3">
              <p>
                <code>enforce(&mut self, a: LC, b: LC, c: LC)</code> — 곱셈 제약{" "}
                <code>a * b = c</code> 추가
              </p>
            </div>
            <div className="bg-background rounded p-3">
              <p>
                <code>ONE</code> — 상수 1을 나타내는 특수 변수 (인덱스 0)
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="bg-muted/50 rounded-lg p-4 mb-4">
          ...
          <div className="text-sm space-y-3">
            <p>
              <code>CubicCircuit&lt;F&gt;</code> —{" "}
              <code>x: Option&lt;F&gt;</code> (witness)
            </p>
            <div className="bg-background rounded p-3">
              <p className="font-medium mb-1">변수 할당</p>
              <p>
                <code>x</code> = <code>alloc_witness</code>, <code>y</code> ={" "}
                <code>alloc_instance(35)</code>
              </p>
            </div>
            <div className="bg-background rounded p-3">
              <p className="font-medium mb-1">제약 1: x * x = t1</p>
              <p>
                <code>enforce(LC(x), LC(x), LC(t1))</code> — 보조 변수{" "}
                <code>t1 = x²</code>
              </p>
            </div>
            <div className="bg-background rounded p-3">
              ...
              <p>
                <code>enforce(LC(t1), LC(x), LC(t2))</code> — 보조 변수{" "}
                <code>t2 = x³</code>
              </p>
            </div>
            <div className="bg-background rounded p-3">
              ...
              <p>
                <code>enforce(LC(t2).add(x).add(5·ONE), LC(ONE), LC(y))</code>
              </p>
              <p className="text-muted-foreground">
                덧셈은 LC 체이닝으로 흡수 — 추가 제약 불필요
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="bg-muted/50 rounded-lg p-4 mb-4">
          ...
          <div className="text-sm">
            <p>
              <code>
                LC::new().add(coeff₁, var₁).add(coeff₂, var₂).add(coeff₃, var₃)
              </code>
            </p>
            <p className="text-muted-foreground mt-1">
              = Σ(coeffᵢ · varᵢ) — 덧셈은 제약 소비 없이 무료
            </p>
          </div>
        </div>
        ...
        <div className="bg-muted/50 rounded-lg p-4 mb-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-sm">
            ...
            <div className="bg-background rounded p-3">
              ...
              <p>
                <code>enforce(b, true-false, result-false)</code>
              </p>
        ...
        <div className="bg-muted/50 rounded-lg p-4 mb-4">
          ...
          <div className="grid grid-cols-3 gap-3 text-sm">
            <div className="bg-background rounded p-3">
              ...
              <p className="text-muted-foreground">
                R1CS 행렬 수집 → ProvingKey 생성
              </p>
            </div>
            <div className="bg-background rounded p-3">
              <p className="font-medium">Prove 모드</p>
              <p>
                <code>x = Some(val)</code>
              </p>
        ...
        <div className="bg-muted/50 rounded-lg p-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-sm">
            <div className="bg-background rounded p-3">
              ...
              <p>
                <code>is_satisfied()</code> — 모든 제약 만족 여부
              </p>
              <p>
                <code>which_unsatisfied()</code> — 실패 제약 인덱스
              </p>
              <p>
                <code>num_constraints()</code> — 회로 크기
              </p>
            </div>
            <div className="bg-background rounded p-3">
              <p className="font-medium">프로덕션 프레임워크</p>
              <p>
                <strong>arkworks</strong> (Rust) · <strong>bellman</strong>{" "}
                (Zcash) · <strong>circom</strong> (DSL)
              </p>
              <p>
                <strong>gnark</strong> (Go) · <strong>libsnark</strong> (C++)
              </p>
            </div>
          </div>
        </div>
