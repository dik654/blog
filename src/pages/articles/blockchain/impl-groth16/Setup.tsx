    ...
    <section id="setup" className="mb-16 scroll-mt-20">
      ...
      <div className="prose prose-neutral dark:prose-invert max-w-none mb-8">
        <p className="leading-7">
          ...
          <br />
          실제 프로덕션은 MPC 세레모니로 어떤 단일 참여자도 전체 값을 알 수
          없도록 보장
        </p>
        <p className="leading-7">
          QAP 다항식을 비밀 τ에서 평가 → 커브 포인트로 인코딩
          <br />
          공개 변수는 /γ로 나누어 IC (검증키), 비공개 변수는 /δ로 나누어 L
          (증명키)
          <br />
      ...
      <div className="prose prose-neutral dark:prose-invert max-w-none">
        <h4>구현 인사이트</h4>
        <p className="leading-7">
          Rust의 소유권 시스템 — toxic waste는 함수 로컬 변수로, 함수 종료 시
          스택에서 자동 소멸
          <br />
          γ와 δ로 공개/비공개를 분리하는 이유: 검증 방정식에서 각각 독립적으로
          소거되어 구조 보장
          <br />
      ...
      <div className="prose prose-neutral dark:prose-invert max-w-none mt-6">
        ...
        <div className="bg-muted/50 rounded-lg p-4 mb-4">
          ...
          <div className="grid grid-cols-1 gap-2 text-sm">
            <div className="bg-background rounded p-3">
              ...
              <p>
                <M>{"[\\alpha]_1"}</M>, <M>{"[\\beta]_1"}</M> (G1) /{" "}
                <M>{"[\\beta]_2"}</M> (G2) / <M>{"[\\delta]_1"}</M> (G1) /{" "}
                <M>{"[\\delta]_2"}</M> (G2)
              </p>
            </div>
            <div className="bg-background rounded p-3">
              ...
              <p>
                <M>{"[\\tau^0]_1, [\\tau^1]_1, \\ldots, [\\tau^{m-1}]_1"}</M>
              </p>
            </div>
            <div className="bg-background rounded p-3">
              ...
              <p>
                <M>
                  {
                    "\\text{IC}[j] = \\left[\\frac{\\beta \\cdot a_j(\\tau) + \\alpha \\cdot b_j(\\tau) + c_j(\\tau)}{\\gamma}\\right]_1"
                  }
                </M>
              </p>
            </div>
            <div className="bg-background rounded p-3">
              ...
              <p>
                <M>
                  {
                    "L[j] = \\left[\\frac{\\beta \\cdot a_j(\\tau) + \\alpha \\cdot b_j(\\tau) + c_j(\\tau)}{\\delta}\\right]_1"
                  }
                </M>
              </p>
            </div>
            <div className="bg-background rounded p-3">
              ...
              <p>
                <M>
                  {
                    "H[i] = \\left[\\frac{\\tau^i \\cdot t(\\tau)}{\\delta}\\right]_1"
                  }
                </M>{" "}
                (<M>{"i = 0 \\ldots m{-}2"}</M>)
              </p>
        ...
        <div className="bg-muted/50 rounded-lg p-4 mb-4">
          ...
          <div className="grid grid-cols-2 gap-2 text-sm">
            <div className="bg-background rounded p-2">
              <M>{"[\\alpha]_1, [\\beta]_2"}</M> — <M>{"e(\\alpha, \\beta)"}</M>{" "}
              사전 계산용
            </div>
            <div className="bg-background rounded p-2">
              <M>{"[\\gamma]_2"}</M> — IC 검사용
            ...
            <div className="bg-background rounded p-2">
              <M>{"\\text{IC}[0], \\ldots, \\text{IC}[l]"}</M> — 공개 입력
              생성기
            </div>
          </div>
        </div>
          ...
          <div className="grid grid-cols-2 gap-3 text-sm">
            <div className="bg-background rounded p-3">
              <p className="font-medium">
                <M>{"\\alpha, \\beta"}</M>: 지식 계수
              </p>
              <p className="text-muted-foreground">
                prover가 임의의 A, B 사용 방지 —{" "}
                <M>{"e(A,B) = e(\\alpha,\\beta) \\cdot \\ldots"}</M> 로 연결
                강제
              </p>
            </div>
            <div className="bg-background rounded p-3">
            ...
            <div className="bg-background rounded p-3">
              ...
              <p className="text-muted-foreground">
                비공개 witness 기여분 그룹화, 공개 부분과 교차 오염 방지
              </p>
            </div>
            <div className="bg-background rounded p-3">
              <p className="font-medium">건전성 기반</p>
              <p className="text-muted-foreground">
                q-PKE 가정, 이산 로그 난제, 쌍선형 군 보안
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="bg-muted/50 rounded-lg p-4 mb-4">
          ...
          <div className="text-sm space-y-2">
            <p>
              <strong>문제:</strong>{" "}
              <M>{"(\\tau, \\alpha, \\beta, \\delta)"}</M>를 아는 자 = 가짜 증명
              위조 가능
            </p>
            <p>
              <strong>해결:</strong> 참여자 <M>{"i"}</M>가 자신만의{" "}
              <M>{"r_i"}</M> 기여 →{" "}
              <M>{"\\text{CRS}_i = \\text{CRS}_{i-1} \\cdot r_i"}</M> (동형
              갱신)
            </p>
            <p className="text-muted-foreground">
              참여자 중 단 한 명만 정직하면 최종 랜덤값은 누구도 알 수 없음
            </p>
          </div>
        ...
        <div className="bg-muted/50 rounded-lg p-4 mb-4">
          ...
          <div className="grid grid-cols-2 gap-3 text-sm">
            <div className="bg-background rounded p-3">
              ...
              <p>
                <M>{"[\\tau^0]_1, \\ldots, [\\tau^N]_1"}</M> +{" "}
                <M>{"[\\alpha \\tau^k]_1, [\\beta \\tau^k]_1"}</M>
              </p>
              <p className="text-muted-foreground">
                회로 무관 범용 — 프로젝트 간 재사용 가능
              </p>
            </div>
            <div className="bg-background rounded p-3">
        ...
        <div className="bg-muted/50 rounded-lg p-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-sm">
            <div className="bg-background rounded p-3">
              ...
              <p>
                <M>{"O(m)"}</M> group elements
              </p>
              <p className="text-muted-foreground">
                <M>{"m = 10^6"}</M> → ~100 MB / <M>{"m = 10^8"}</M> → ~10 GB
              </p>
            </div>
            <div className="bg-background rounded p-3">
              <p className="font-medium">Verifying key 크기</p>
              <p>
                <M>{"O(l)"}</M> (<M>{"l"}</M> = 공개 입력 수)
              </p>
            ...
            <div className="bg-background rounded p-3 sm:col-span-2">
              ...
              <p>
                Plonk, Sonic, Marlin — 세레모니 1회로 모든 회로에 재사용, 대신
                증명 크기 약간 증가 + 검증 속도 저하
              </p>
            </div>
          </div>
        </div>
