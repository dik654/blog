  ...
  return (
    <section id="overview" className="mb-16 scroll-mt-20">
      ...
      <div className="prose prose-neutral dark:prose-invert max-w-none">
        <p>
          <strong>Scroll zkEVM</strong> — EVM 실행을 Halo2 회로로 증명
          <br />각 EVM 오퍼코드를 <code>ExecutionGadget</code>으로 구현, 공유
          테이블(RwTable, BytecodeTable 등)로 서브회로 간 일관성 유지
        </p>
        {onCodeRef && (
          <div className="not-prose flex flex-wrap gap-2 my-4">
            ...
            <CodeViewButton
              onClick={() =>
                onCodeRef("execution-trait", codeRefs["execution-trait"])
              }
            />
            <span className="text-[10px] text-muted-foreground self-center">
              execution.rs
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3 not-prose mb-6">
          <div className="rounded-lg border border-sky-200 dark:border-sky-800 bg-sky-50 dark:bg-sky-950/30 p-4">
            ...
            <ul className="text-sm space-y-1 text-foreground/80">
              <li>
                <code className="text-xs">evm_circuit.rs</code> — EVM 실행 흐름
                메인 회로
              </li>
              <li>
                <code className="text-xs">execution/</code> — 오퍼코드별{" "}
                <code className="text-xs">ExecutionGadget</code> (ADD, CALL,
                SLOAD 등)
              </li>
              <li>
                <code className="text-xs">bytecode_circuit.rs</code> —
                바이트코드 무결성 검증
              </li>
              <li>
                <code className="text-xs">keccak_circuit.rs</code> — Keccak256
                해시 회로
              </li>
              <li>
                <code className="text-xs">mpt_circuit.rs</code> — Merkle
                Patricia Trie 상태 검증
              </li>
              <li>
                <code className="text-xs">copy_circuit.rs</code> —
                메모리/calldata 복사 추적
              </li>
              <li>
                <code className="text-xs">sig_circuit.rs</code> — ECDSA 서명
                검증
              </li>
              <li>
                <code className="text-xs">ecc_circuit.rs</code> — 점 곱셈 등
                타원곡선 연산
              </li>
            </ul>
          </div>
          <div className="rounded-lg border border-emerald-200 dark:border-emerald-800 bg-emerald-50 dark:bg-emerald-950/30 p-4">
            ...
            <ul className="text-sm space-y-1 text-foreground/80">
              <li>
                <code className="text-xs">RwTable</code> — 모든 읽기/쓰기 연산
                순서 기록
              </li>
              <li>
                <code className="text-xs">BytecodeTable</code> — 바이트코드
                바이트 → 인덱스 매핑
              </li>
              <li>
                <code className="text-xs">TxTable</code> — 트랜잭션 메타데이터
              </li>
              <li>
                <code className="text-xs">KeccakTable</code> — Keccak 입출력 쌍
              </li>
              <li>
                <code className="text-xs">CopyTable</code> — 복사 이벤트 기록
              </li>
            </ul>
          </div>
        </div>
        ...
        <div className="not-prose rounded-lg border border-border bg-muted/30 p-4 mb-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3 text-sm">
            ...
            <div className="rounded border border-emerald-200 dark:border-emerald-800 bg-emerald-50 dark:bg-emerald-950/30 p-3">
              ...
              <ul className="space-y-1 text-foreground/80">
                <li>
                  <code className="text-xs">tx_table</code>,{" "}
                  <code className="text-xs">rw_table</code>
                </li>
                <li>
                  <code className="text-xs">bytecode_table</code>,{" "}
                  <code className="text-xs">block_table</code>
                </li>
                <li>
                  <code className="text-xs">copy_table</code>,{" "}
                  <code className="text-xs">keccak_table</code>
                </li>
                <li>
                  <code className="text-xs">exp_table</code>,{" "}
                  <code className="text-xs">sig_table</code>,{" "}
                  <code className="text-xs">ecc_table</code>
                </li>
              </ul>
            </div>
            <div className="rounded border border-amber-200 dark:border-amber-800 bg-amber-50 dark:bg-amber-950/30 p-3">
              ...
              <ul className="space-y-1 text-foreground/80">
                ...
                <li>
                  <code className="text-xs">configure_gadget!</code> 매크로로 각
                  가젯 초기화
                </li>
