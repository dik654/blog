  ...
  return (
    <section id="halo2-integration" className="mb-16 scroll-mt-20">
      ...
      <div className="prose prose-neutral dark:prose-invert max-w-none">
        <p>
          Scroll zkEVM은 <strong>Halo2</strong> 영지식 증명 백엔드를 사용합니다.
          <br />
          모든 회로가 <code>Circuit</code> 트레이트를 구현하고,
          <code>SubCircuit</code> 트레이트로 서브회로 간 일관된 인터페이스를
          제공합니다.
        </p>
        ...
        <div className="not-prose rounded-lg border border-border bg-muted/30 p-4 mb-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3 text-sm">
            <div className="rounded border border-sky-200 dark:border-sky-800 bg-sky-50 dark:bg-sky-950/30 p-3">
              ...
              <ul className="space-y-1 text-foreground/80">
                <li>
                  <code className="text-xs">Config</code> — 회로 설정 (테이블,
                  컬럼)
                </li>
                <li>
                  <code className="text-xs">FloorPlanner</code> — 레이아웃 전략
                </li>
                <li>
                  <code className="text-xs">Params</code> — 파라미터
                </li>
              </ul>
            </div>
            ...
            <div className="rounded border border-amber-200 dark:border-amber-800 bg-amber-50 dark:bg-amber-950/30 p-3">
              ...
              <ul className="space-y-1 text-foreground/80">
                <li>
                  <code className="text-xs">configure(meta)</code> — 제약 등록
                </li>
                <li>
                  <code className="text-xs">synthesize(config, layouter)</code>{" "}
                  — witness 할당
                </li>
              </ul>
            </div>
          ...
          <p className="text-xs text-muted-foreground mt-3">
            SuperCircuit:{" "}
            <code className="text-xs">
              Config = (SuperCircuitConfig&lt;Fr&gt;, Challenges)
            </code>
            , <code className="text-xs">FloorPlanner = SimpleFloorPlanner</code>
          </p>
        </div>
        ...
        <div className="not-prose rounded-lg border border-border bg-muted/30 p-4 mb-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3 text-sm">
            <div className="rounded border border-sky-200 dark:border-sky-800 bg-sky-50 dark:bg-sky-950/30 p-3">
              ...
              <ul className="space-y-1 text-foreground/80">
                ...
                <li>
                  <code className="text-xs">unusable_rows() -&gt; usize</code> —
                  블라인딩 행 (기본 256)
                </li>
                <li>
                  <code className="text-xs">new_from_block(block)</code> —
                  Block에서 생성
                </li>
                <li>
                  <code className="text-xs">instance()</code> — 공개 입력
                </li>
              </ul>
            </div>
            <div className="rounded border border-emerald-200 dark:border-emerald-800 bg-emerald-50 dark:bg-emerald-950/30 p-3">
              ...
              <ul className="space-y-1 text-foreground/80">
                <li>
                  <code className="text-xs">
                    synthesize_sub(config, challenges, layouter)
                  </code>
                </li>
                <li>
                  <code className="text-xs">
                    min_num_rows_block(block) -&gt; (usize, usize)
                  </code>{" "}
                  — 동적 크기
                </li>
              </ul>
          ...
          <div className="mt-3 pt-3 border-t border-border">
            ...
            <ul className="text-sm space-y-1 text-foreground/80">
              <li>
                <code className="text-xs">type ConfigArgs</code> — 설정 인자
                타입
              </li>
              <li>
                <code className="text-xs">fn new(meta, args) -&gt; Self</code> —
                ConstraintSystem에서 생성
              </li>
            </ul>
          </div>
        </div>
        ...
        <p>
          CellManager는 셀 할당을 최적화합니다. EVM Circuit은{" "}
          <strong>수평 정렬</strong>
        ...
        <div className="not-prose rounded-lg border border-border bg-muted/30 p-4 mb-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3 text-sm">
            <div className="rounded border border-sky-200 dark:border-sky-800 bg-sky-50 dark:bg-sky-950/30 p-3">
              ...
              <ul className="space-y-1 text-foreground/80">
                <li>
                  <code className="text-xs">width: usize</code> — 컬럼 개수
                </li>
                <li>
                  <code className="text-xs">height: usize</code> — 최대 높이
                </li>
                <li>
                  <code className="text-xs">
                    cells: Vec&lt;Cell&lt;F&gt;&gt;
            ...
            <div className="rounded border border-amber-200 dark:border-amber-800 bg-amber-50 dark:bg-amber-950/30 p-3">
              ...
              <ul className="space-y-1 text-foreground/80">
                <li>
                  <code className="text-xs">StoragePhase1</code> — Phase 1 일반
                  저장소
                </li>
                <li>
                  <code className="text-xs">StoragePermutation</code> — Phase 1
                  복사 제약용
                </li>
                <li>
                  <code className="text-xs">StoragePhase2</code> — Phase 2 일반
                  저장소
                </li>
                <li>
                  <code className="text-xs">StoragePermutationPhase2</code> —
                  Phase 2 복사 제약용
                </li>
                <li>
                  <code className="text-xs">Lookup(table)</code> — 특정 테이블
                  룩업 전용
                </li>
                <li>
                  <code className="text-xs">LookupByte</code> — 바이트 룩업 전용
                </li>
              </ul>
            </div>
          </div>
          <p className="text-xs text-muted-foreground mt-3">
            할당 방식: <code className="text-xs">query_cell(cell_type)</code> —
            가장 높이가 낮은 컬럼을 선택하여 셀 할당
          </p>
        </div>
        ...
        <div className="bg-amber-50 dark:bg-amber-950/30 border-l-4 border-amber-400 p-4 my-6 rounded-r-lg">
          <p className="font-semibold mb-2">
            인사이트: Scroll의 Halo2 선택 이유
          </p>
          <p>
            <strong>Universal setup</strong>: Circuit 변경 시 재설정 불필요
            <br />
            <strong>Lookup arguments</strong>: zkEVM table system에 필수
            <br />
            <strong>PLONKish arithmetization</strong>: 유연한 custom gates
            <br />
