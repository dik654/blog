...
export default function ModernDaiArticle() {
  return <article className="space-y-14">
    ...
    <section id="vault-debt-collateral" className="space-y-6">
      ...
      <ExplainedFormula question="설명용 Vault의 collateral ratio와 현재 debt를 어떻게 분리해 계산할까요?" idea="Collateral token 수량에 oracle price를 곱해 가치 V를 만들고, normalized debt에 누적 rate를 곱해 debt D를 만든 뒤 비율을 계산합니다." formula={String.raw`\begin{aligned}D&=art\times rate\\CR&=\frac{V_{collateral}}{D}\times100\%\end{aligned}`}
      annotatedFormula={String.raw`\begin{aligned}D&=\underbrace{art\times rate}_{\text{accumulated debt 계산}}\\CR&=\underbrace{\frac{V_{collateral}}{D}\times100\%}_{\text{기준량당 비율}}\end{aligned}`}
      operations={[
        { expression: String.raw`art\times rate`, annotation: ["accumulated debt multiplier이(가) 식의","결과에 기여하는 방식을 계산합니다.","Collateral token 수량에 oracle price를","곱해 가치 V를 만들고, normalized debt에 누적"] },
        { expression: String.raw`\frac{V_{collateral}}{D}\times100\%`, annotation: ["분자에 둔 관심량을 분모의 기준량으로 정규화합니다.","Collateral token 수량에 oracle price를","곱해 가치 V를 만들고, normalized debt에 누적","rate를 곱해 debt D를 만든 뒤 비율을 계산합니다."] },
      ]} terms={[{symbol:"art",name:"normalized debt",description:"Vault가 보유한 rate 적용 전 debt unit입니다."},{symbol:"rate",name:"accumulated debt multiplier",description:"Collateral type별 stability fee accrual을 누적한 multiplier입니다."},{symbol:"V_collateral",name:"oracle-valued collateral",description:"담보 수량에 versioned oracle price와 unit conversion을 적용한 가치입니다."},{symbol:"CR",name:"collateral ratio",description:"현재 debt 대비 oracle-valued collateral의 비율입니다."}]} assumptions={["동일 시점의 oracle, rate와 token decimals를 사용합니다.","Liquidation threshold는 collateral type의 versioned parameter로 별도 조회합니다.","Oracle stale·debt ceiling·auction liquidity를 이 비율 하나로 대체하지 않습니다.","0 debt에서는 비율로 liquidation 판단을 하지 않습니다."]} interpretation="V=150, art=100, rate=1이면 D=100, CR=150%입니다. Rate가 1.05로 오르면 debt=105, CR≈142.86%가 됩니다." />
    </section>
