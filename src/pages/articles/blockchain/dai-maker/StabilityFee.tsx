...
// 시간 지나면 갚을 DAI 증가`}</pre>
        <p>
          <strong>rate는 곱셈 계수</strong>: 사용자 잔고 수정 없이 전체 누적
          <br />
          Maker의 <strong>"rate accumulator" 패턴</strong>
          <br />
    ...
    function drip(bytes32 ilk) external returns (uint256 rate) {
        ...
        <p>
          <strong>drip() 함수</strong>: rate 업데이트 + Vat.fold() 호출
          <br />
          rpow (rmul to power): 복리 계산
          <br />
...
4. MKR 공급 감소 → 가치 상승 (이론)`}</pre>
        <p>
          <strong>MKR은 deflationary</strong>: SF 수익으로 MKR 소각
          <br />
          예시: $10M surplus → $10M DAI로 MKR 매입 → 소각
          <br />
...
contract Pot {
    ...
    function balanceOf(address user) external view returns (uint256) {
        ...
        <p>
          <strong>Pot = DAI 예금 컨트랙트</strong>
          <br />
          DSR은 SF보다 낮게 설정 — 프로토콜 spread 유지
          <br />
...
→ DAI 보유자에게 "risk-adjusted" 매력`}</pre>
        ...
        <div className="bg-amber-50 dark:bg-amber-950/30 border-l-4 border-amber-400 p-4 my-6 rounded-r-lg">
          ...
          <p className="mt-2">
            금리 정책:
            <br />
            - DAI 페그 약화 → SF/DSR 상승 (긴축)
            <br />
            - DAI 페그 강함 → SF/DSR 하락 (완화)
            <br />- 전통 Fed 금리와 연동 (실세 연동)
          </p>
          <p className="mt-2">
          ...
          <p className="mt-2">
            이 실험이 성공하면 — <strong>알고리드믹 중앙은행</strong>의 청사진
            <br />
