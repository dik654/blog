...
export default function PriceDecayViz() {
  ...
  const toY = (p: number) =>
    56 + h - ((p - minPrice) / (maxPrice - minPrice)) * h;

  const pathData = points
    .map((p, i) => `${i === 0 ? "M" : "L"} ${toX(p.t)} ${toY(p.p)}`)
    .join(" ");
    ...
    <div className="not-prose my-6 rounded-lg border border-border bg-card p-4">
      ...
      >
        <text
          ...
          fill="var(--foreground)"
        >
          StairstepExponentialDecrease — cut · step 조정
        </text>
        <text
          x={260}
          y={40}
        ...
        {breakEven && (
          <g transform="translate(186, 244)">
            <rect
              ...
              strokeWidth={0.6}
            />
            <text x={12} y={18} fontSize={10} fontWeight={700} fill="#10b981">
              시장가 도달
            </text>
            <text
              x={146}
            ...
            <text x={12} y={36} fontSize={10} fill="var(--muted-foreground)">
              낙찰 가능 지점
            </text>
          </g>
        )}
        ...
        <g transform="translate(346, 244)">
          <rect
            ...
            strokeWidth={0.6}
          />
          <text x={12} y={18} fontSize={10} fontWeight={700} fill="#8b5cf6">
            권장값
          </text>
          <text
            x={138}
