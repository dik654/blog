  ...
  return <article className="space-y-14">
    <section id="overview" className="space-y-6">
      ...
      <p className="text-lg leading-8 text-foreground/90">
            Directed acyclic graph(DAG)는 vertex와 한 방향 edge로 만든 순환 없는 그래프입니다. DAG-BFT에서는 validator마다 매 round
            transaction batch를 담은 vertex를 만들고 이전 round의 여러 certified vertices를 parent로 참조합니다. 이 구조는 data 전파를
            병렬화하지만 그래프 자체가 하나의 total order를 이루지는 않습니다.
          </p>
      <p>
            A와 B가 동시에 vertex를 만들었다면 어떤 node는 A를 먼저, 다른 node는 B를 먼저 받을 수 있습니다. 두 vertex 사이에 causal edge가 없으면 둘
            다 올바른 topological order가 여러 개 존재합니다. State machine replication은 모든 replica가 같은 순서로 transaction을
            적용해야 합니다. 그래서 protocol은 anchor를 고르고 anchor가 도달하는 causal history를 정한 뒤 동률까지 처리하는 deterministic
            linearization rule을 추가합니다.
          </p>
      <DAGCausalFlowViz />
    ...
    <section id="narwhal" className="space-y-6">
      ...
      <p>
            Narwhal의 worker는 transaction batch를 다른 validators에 전파하고 저장 acknowledgement를 모읍니다. Primary는 batch
            digest와 이전 round parents를 묶은 header를 만들며 충분한 validators가 availability를 확인하면 certificate가 됩니다.
            Certificate는 “이 digest의 data를 fault bound 아래에서 나중에 retrieve할 근거가 있다”는 뜻입니다. Transaction이 total
            order에 들어갔다거나 application에서 유효하다는 데까지 가지는 않습니다.
          </p>
      <p>
            한 vertex가 이전 round의 2f+1 certified vertices를 가리키면 round 사이에 강한 causal 연결이 생깁니다. n=4,f=1에서 두 honest
            vertices가 각각 세 parents를 선택하면 두 parent sets는 최소 두 개가 겹칩니다. 그래도 edge가 없는 동시 vertices의 상대 순서는 정해지지
            않습니다. Certificate signer가 payload를 영구 보관한다거나 모든 executor가 즉시 다운로드했다는 보장 역시 별도 retention·fetch
            contract가 있어야 성립합니다.
          </p>
    ...
    <section id="bullshark" className="space-y-6">
      ...
      <p>
            Bullshark는 이미 구축된 DAG를 해석해 Byzantine atomic broadcast order를 만듭니다. Round들을 wave로 묶고 정해진 anchor
            vertex가 후속 vertices에서 충분한 strong support를 받았는지 확인합니다. Commit rule을 만족한 anchor는 그 이전에 미commit된
            anchors와 causal history를 끌고 옵니다. 모든 honest node가 동일한 deterministic traversal을 사용하므로 같은 sequence가
            나옵니다.
          </p>
      <p>
            중요한 safety 경계는 local arrival order를 쓰지 않는 것입니다. A가 x→y 순으로 받고 B가 y→x 순으로 받았을 때 “도착한 순서대로 apply”하면
            같은 certified DAG에서도 state가 갈라집니다. Round와 author의 canonical identity, digest처럼 모든 node가 공유하는 값으로
            tie-break합니다. Parent가 아직 없다면 fetch·validate가 끝날 때까지 해당 prefix는 apply 대상이 아닙니다.
          </p>
    ...
    <section id="linearization" className="space-y-6">
      ...
      <DAGLayerBoundaryViz />
      <div className="grid gap-4 md:grid-cols-2"><div className="rounded-lg border border-border bg-card p-4"><h3 className="text-sm font-semibold">도착 순서 반례</h3><p className="mt-2 text-sm leading-6 text-muted-foreground">
            x와 y가 독립인데 R1은 x,y, R2는 y,x로 받았습니다. Balance에 x=+10, y=×2를 적용하면 20과 10처럼 결과가 달라질 수 있습니다. Arrival
            order가 SMR order를 대신하지 못하는 이유입니다.
          </p></div><div className="rounded-lg border border-border bg-card p-4"><h3 className="text-sm font-semibold">Certificate-only 반례</h3><p className="mt-2 text-sm leading-6 text-muted-foreground">
            Digest certificate는 있지만 payload holder가 동시에 offline이면 anchor를 정해도 execution input을 가져오지 못합니다.
            Signer threshold와 erasure/replication, retention, fetch deadline이 함께 검증 대상입니다.
          </p></div></div>
      <p>
            DAG width가 커지면 data throughput 여지는 늘지만 vertex metadata와 signatures, parent references, storage,
            garbage collection 비용도 함께 늘어납니다. Slow validator가 만든 오래된 vertex를 언제 포함할지, duplicate transaction을 어느
            stable ID로 제거할지, committed causal prefix를 prune하기 전에 checkpoint와 state sync가 준비됐는지도 운영 protocol의
            일부입니다.
          </p>
    </section>
    ...
    <section id="comparison" className="space-y-6">
      ...
      <p>
            같은 membership·key·transaction arrival과 network schedule로 leader-BFT baseline과 DAG candidate를
            실행합니다. 정상 경로만이 아니라 missing batch와 invalid certificate, equivocation, delayed parent를 주입합니다. 여기에
            minority partition, anchor leader omission, restart, prune 경계까지 넣습니다. 모든 replica의 committed vertex
            sequence와 state root가 같고 duplicate apply가 0이 되고 나서야 성능 비교로 넘어갑니다.
          </p>
      ...
      <h3 className="text-xl font-semibold">이 글만으로 풀어야 하는 10문제</h3><p>
            기초 6문제는 DAG·causal edge와 Narwhal certificate, parent overlap, data/order/execution 분리, Bullshark
            anchor, deterministic linearization을 묻습니다. 심화 4문제로 넘어가면 arrival-order 반례와 payload unavailable 반례,
            4-validator fault trace, baseline과의 paired release gate를 직접 설계합니다.
          </p>
    </section>
  </article>;
}
