...
const STEPS = [
  {
    label: "HADES 라운드 배치",
    body: "RF/2 full → RP partial → RF/2 full 순서.\nBN254: 4 + 57 + 4 = 65 라운드.",
  },
  {
    label: "Full Round 내부 과정",
    body: "모든 T개 원소에 AddRC → S-box → MDS.\n전체 비선형성으로 차분·통계적 공격 방어.",
  },
  {
    label: "Partial Round 내부 과정",
    body: "첫 번째 원소에만 S-box, 나머지는 통과.\n대수적 공격 방어하면서 제약 수 1/3로 절감.",
  },
  {
    label: "제약 수 비교",
    body: "HADES: 8×9 + 57×3 = 243 제약.\n전부 Full이면: 65×9 = 585 → 58% 절감.",
  },
];
...
function LayoutOverview() {
  ...
  return (
    <motion.g initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
      ...
      >
        <text x={70} y={y + h + 18} textAnchor="middle" fontSize={9} fill={C1}>
          모든 원소 S-box
        </text>
        <text x={222} y={y + h + 18} textAnchor="middle" fontSize={9} fill={C2}>
          첫 원소만 S-box
        </text>
        <text x={374} y={y + h + 18} textAnchor="middle" fontSize={9} fill={C1}>
          모든 원소 S-box
        </text>
      </motion.g>
      <defs>
...
function ConstraintBars() {
  ...
  return (
    <motion.g initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
      ...
      <text x={40} y={y + 14} fontSize={9} fill="var(--muted-foreground)">
        전부 Full
      </text>
      <motion.rect
        x={40}
      ...
      <text x={40} y={y + 68} fontSize={9} fill={C3}>
        HADES
      </text>
      <motion.rect
        x={40}
...
export default function HADESViz() {
  return (
    <StepViz steps={STEPS}>
      {(step) => (
        <svg
          viewBox={step === 1 || step === 2 ? "0 0 420 270" : "0 0 444 140"}
