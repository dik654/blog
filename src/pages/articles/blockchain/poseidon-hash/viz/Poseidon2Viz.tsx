...
const STEPS = [
  {
    label: "Poseidon1 vs Poseidon2",
    body: "핵심 차이: Partial round의 MDS를 대각 행렬로 교체.\n곱셈 O(T²) → O(T)로 줄여 ~30% 속도 향상.",
  },
  {
    label: "Poseidon1: dense MDS 연산",
    body: "Partial round에서도 dense MDS 사용.\nT=3 기준 9회 곱셈. T=16이면 256회 → 비효율.",
  },
  {
    label: "Poseidon2: 대각 MDS 연산",
    body: "Internal round에서 D+1 대각 행렬 사용.\nT=3 기준 3회 곱셈. T=16이면 16회 → 16배 절감.",
  },
];
...
export default function Poseidon2Viz() {
  return (
    <StepViz steps={STEPS}>
      {(step) => (
        <svg
          viewBox={step === 0 ? "0 0 480 160" : "0 0 480 170"}
