...
const STEPS = [
  {
    label: "상태 초기화",
    body: "T=3 벡터 0 초기화. cap=보안(비노출), rate=입출력.",
  },
  {
    label: "흡수 ①: 입력 XOR",
    body: "입력 [m₁,m₂]를 rate에 XOR. cap은 변경하지 않는다.",
  },
  {
    label: "흡수 ②: 순열 π 적용",
    body: "Poseidon SPN π를 전체 상태에 적용 → cap에도 입력 확산.",
  },
  {
    label: "압출 (Squeeze)",
    body: "최종 상태에서 rate 부분만 출력으로 추출.\ncapacity는 끝까지 비노출 → 역추적 불가능.",
  },
];
...
function StateInit() {
  ...
  return (
    <motion.g initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
      ...
      <text x={cx[0] + w / 2} y={y - 8} textAnchor="middle" fontSize={10}>
        🔒
      </text>
      <rect
        x={cx[0] - 4}
...
function AbsorbPerm() {
  ...
  return (
    <motion.g initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
      ...
      {/* arrows into π */}
      {cx.map((c) => (
        <VArrow key={c} x={c + w / 2} y1={64} y2={78} color={C1} />
      ))}
      ...
      />
      <text x={56} y={90} fontSize={9} fontWeight={600} fill={C1}>
        π 내부:
      </text>
      ...
      <text x={390} y={134} fontSize={9} fill={C1}>
        × R회
      </text>

      {/* 화살표 → 출력 */}
      {cx.map((c) => (
        <VArrow key={`o${c}`} x={c + w / 2} y1={162} y2={176} color={C1} />
      ))}
...
function Squeeze() {
  ...
  return (
    <motion.g initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
      ...
      <text x={cx[0] + w / 2} y={30} textAnchor="middle" fontSize={10}>
        🔒
      </text>
      {/* X mark on capacity */}
      <motion.g
...
export default function SpongeViz() {
  return (
    <StepViz steps={STEPS}>
      {(step) => (
        <svg
          viewBox={step === 2 ? "0 0 420 240" : "0 0 420 200"}
