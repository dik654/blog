...
const STEPS = [
  {
    label: "결정론적 시뮬레이션",
    body: "동일 시드 → 동일 결과. 네트워크 파티션, 비잔틴 장애, 링크 손실을 시뮬레이션. 디버거에서 단계별 실행·재현 가능.",
  },
  {
    label: "Alto: 미니멀 블록체인",
    body: "threshold_simplex + authenticated P2P + MMR + BLS12-381. 블록 ~200ms, Finality ~300ms, CPU 65% 절감.",
  },
  {
    label: "채택 현황 & 로드맵",
    body: "Tempo(테스트넷), Alto(벤치마크). 향후: QMDB 통합, 추가 프리미티브, MCP 서버, 문서화.",
  },
];
          ...
          {step === 2 && (
            <motion.g initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
              ...
              {[
                {
                  proj: "Tempo",
                  prims: "simplex, broadcast, storage",
                  status: "테스트넷",
                  c: C1,
                },
                {
                  proj: "Alto",
                  prims: "threshold_simplex, p2p, mmr",
                  status: "벤치마크",
                  c: C2,
                },
              ].map((p, i) => (
