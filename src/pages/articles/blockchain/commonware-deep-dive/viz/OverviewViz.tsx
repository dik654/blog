...
const STEPS = [
  {
    label: "블록체인 개발 스펙트럼",
    body: "모놀리식(Bitcoin) → 프레임워크(Cosmos SDK) → Anti-Framework(Commonware) → 처음부터 구축. 오른쪽일수록 유연하지만 개발 비용 증가.",
  },
  {
    label: "프레임워크의 한계",
    body: "Sei, Celestia, dYdX, Penumbra 모두 CometBFT Fork 필수. 합의 커스터마이징, DA 특화, 오더북 최적화 등 각각의 이유로 Fork 후 유지보수 부담.",
  },
  {
    label: "6가지 프리미티브",
    body: "consensus(합의) · broadcast(전파) · storage(저장) · cryptography(암호화) · p2p(네트워킹) · runtime(런타임). 각각 독립적으로 선택·조합 가능.",
  },
];
            ...
            <motion.g initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
              {[
                ...
                <g key={i}>
                  ...
                  <text x={310} y={37 + i * 34} fontSize={10} fill="#ef4444">
                    {r.issue}
                  </text>
                </g>
              ))}
