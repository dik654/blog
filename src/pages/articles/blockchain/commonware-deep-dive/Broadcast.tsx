    ...
    <section id="broadcast" className="mb-16 scroll-mt-20">
      ...
      <div className="prose prose-neutral dark:prose-invert max-w-none mb-8">
        <p className="leading-7">
          ...
          <br />
          문제 — 대역폭 낭비(중복 전송), 리더 병목(단일 노드 의존), 리더 비응답
          시 성능 저하
        </p>
        <p className="leading-7">
          <strong>ordered_broadcast</strong> — Commonware의 데이터 전파
          프리미티브
          <br />
          ...
          <br />
          연결된 인증서: 각 메시지가 이전 메시지의 임계 서명을 포함 —
          Pre-Consensus Receipt
        </p>
        <p className="leading-7">
          <strong>DSMR</strong>(Decoupled State Machine Replication) — Replicate
          → Sequence → Execute를 독립 단계로 분리
          <br />
          <strong>ZODA</strong>(Zero-Overhead Data Availability) — Reed-Solomon
          인코딩으로 샤드 분할, 신뢰 설정 불필요
        </p>
      </div>
      ...
      <div className="prose prose-neutral dark:prose-invert max-w-none mt-6">
        ...
        <p className="text-sm text-muted-foreground mb-4">
          Decoupled State Machine Replication — 전통 BFT의 monolithic 구조를
          3단계로 분해
        </p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 not-prose mb-6">
          ...
          <div className="rounded-lg border border-border bg-card p-4">
            ...
            <ul className="text-sm space-y-0.5 text-muted-foreground">
              <li>
                <strong className="text-foreground">Narwhal/Bullshark</strong> —
                Aptos, Sui
              </li>
              <li>
                <strong className="text-foreground">Celestia</strong> — DA layer
              </li>
              <li>
                <strong className="text-foreground">EigenDA</strong>
              </li>
            </ul>
          </div>
        </div>
        ...
        <p className="text-sm text-muted-foreground mb-4">
          Zero-Overhead Data Availability — 블록 데이터(MBs~GBs)를 모든 노드가
          저장할 수 없는 문제를 해결, "data is available" 증명을 제공
        </p>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 not-prose mb-6">
          <div className="rounded-lg border border-border bg-card p-4">
            ...
            <ul className="text-sm space-y-0.5 text-muted-foreground">
              ...
              <li>
                Redundancy factor: <code className="text-xs">(n+k)/n</code>
              </li>
            </ul>
          </div>
          <div className="rounded-lg border border-border bg-card p-4">
            ...
            <ul className="text-sm space-y-0.5 text-muted-foreground">
              ...
              <li>
                <code className="text-xs">O(sqrt(n))</code> verification
              </li>
            </ul>
          </div>
          <div className="rounded-lg border border-border bg-card p-4">
        ...
        <div className="rounded-lg border border-border bg-card p-5 not-prose mb-6">
          <h4 className="font-semibold text-sm mb-2 text-muted-foreground">
            성능 지표 (1GB block, 100 validators)
          </h4>
          <div className="grid grid-cols-2 gap-2 text-sm">
            <div>
              <strong className="text-foreground">노드당 저장</strong>{" "}
              <span className="text-muted-foreground">— ~20MB (2%)</span>
            </div>
            <div>
              <strong className="text-foreground">Sample 검증</strong>{" "}
              <span className="text-muted-foreground">
                — <code className="text-xs">O(log n)</code>
              </span>
            </div>
            <div>
