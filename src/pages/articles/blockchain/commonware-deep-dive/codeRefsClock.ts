    ...
    annotations: [
      {
        lines: [6, 9],
        color: "sky",
        note: "governor::Clock 구현 — 내장 rate limiter가 Clock을 직접 참조",
      },
      {
        lines: [12, 12],
        color: "emerald",
        note: "current() — deterministic에서는 시뮬레이션 시간, tokio에서는 실제 시간",
      },
      {
        lines: [15, 16],
        color: "amber",
        note: "sleep() — deterministic에서는 즉시 완료 가능 (시간 전진 시)",
      },
      {
        lines: [23, 27],
        color: "violet",
        note: "timeout() — 기본 구현: select! { result = future, _ = sleep }",
      },
    ],
  },
};
