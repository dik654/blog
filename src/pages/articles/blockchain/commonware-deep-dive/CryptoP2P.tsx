    ...
    <section id="crypto-p2p" className="mb-16 scroll-mt-20">
      ...
      <div className="prose prose-neutral dark:prose-invert max-w-none mb-8">
        <p className="leading-7">
          <strong>cryptography::bls12381</strong> — BLS12-381 타원 곡선 기반
          암호화 프리미티브
          <br />
          DKG(분산 키 생성, 보드리스 공유 분배) → Threshold Signatures(임계
          서명, O(1) 96 bytes)
          <br />
        ...
        <p className="leading-7">
          ...
          <br />
          <strong>bls12381</strong> — 서명 집계, 임계 서명. 합의·크로스체인
          검증에 핵심
          <br />
          <strong>secp256r1</strong> — NIST P-256, HSM 호환. 엔터프라이즈 환경
        </p>
        <p className="leading-7">
          <strong>p2p::authenticated</strong> — ECIES 암호화 연결 + 서명 기반
          피어 인증 + 멀티플렉싱(다중 채널)
          <br />
          Blocker 인터페이스로 악의적 피어 차단 · 동적 피어 관리 · 네트워크
          파티션 복구
          <br />
          <strong>p2p::simulated</strong> — 결정론적 시뮬레이션을 위한 가상
          네트워크
        </p>
      </div>
      ...
      <div className="prose prose-neutral dark:prose-invert max-w-none mt-6">
        ...
        <p className="text-sm text-muted-foreground mb-4">
          BLS (Boneh-Lynn-Shacham) — 3명의 암호학자 이름. Deterministic, short
          signatures(48 bytes), aggregation, threshold signing 지원
        </p>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 not-prose mb-6">
          <div className="rounded-lg border border-border bg-card p-5">
            ...
            <div className="text-sm space-y-1 text-muted-foreground">
              <div>
                <code className="text-xs">σ_agg = σ_1 + σ_2 + ... + σ_n</code>
              </div>
              <div>
                <code className="text-xs">
                  verify_agg(σ_agg, pk_1+...+pk_n, msg) == true
                </code>
              </div>
            </div>
          </div>
          <div className="rounded-lg border border-border bg-card p-5">
            ...
            <ol className="text-sm space-y-1 text-muted-foreground list-decimal list-inside">
              <li>
                <strong className="text-foreground">DKG</strong> — n parties
                generate shares, no single party knows full key
              </li>
              <li>
                <strong className="text-foreground">Signing</strong> — each
                party creates partial sig, t parties combine → valid
              </li>
              <li>
                <strong className="text-foreground">Verification</strong> —
                standard BLS verify, verifier doesn't know who signed
              </li>
            </ol>
          </div>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 not-prose mb-6">
          <div className="rounded-lg border border-border bg-card p-4">
            ...
            <div className="grid grid-cols-2 gap-1 text-sm">
              ...
              <div>
                <strong>48 bytes</strong> (G1)
              </div>
              <div className="text-muted-foreground">Public key</div>
              <div>
                <strong>96 bytes</strong> (G2)
              </div>
              <div className="text-muted-foreground">Verification</div>
              <div>
                <strong>3 pairings</strong> (~2ms)
              </div>
              <div className="text-muted-foreground">Aggregation</div>
              <div>
                <code className="text-xs">O(n)</code> point additions
              </div>
              <div className="text-muted-foreground">DKG setup</div>
              <div>
                <code className="text-xs">O(n²)</code> interactive
              </div>
            </div>
          </div>
          <div className="rounded-lg border border-border bg-card p-4">
        ...
        <h3 className="text-xl font-semibold mt-8 mb-3">
          P2P Authenticated Network
        </h3>
        <div className="rounded-lg border border-border bg-card p-5 not-prose mb-6">
          <h4 className="font-semibold text-sm mb-3">
            <code className="text-xs">p2p::authenticated</code> module
          </h4>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <h5 className="font-semibold text-sm mb-2">
                <code className="text-xs">AuthenticatedPeer</code> 구조체
              </h5>
              <ul className="text-sm space-y-0.5 text-muted-foreground">
                <li>
                  <code className="text-xs">peer_id: PublicKey</code> — Ed25519
                  pubkey = identity
                </li>
                <li>
                  <code className="text-xs">secret_key: SecretKey</code>
                </li>
                <li>
                  <code className="text-xs">
        ...
        <div className="rounded-lg border border-border bg-card p-4 not-prose mb-6">
          <h4 className="font-semibold text-sm mb-2">
            <code className="text-xs">Blocker</code> trait
          </h4>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            <div>
              <ul className="text-sm space-y-0.5 text-muted-foreground">
                ...
                <li>
                  <code className="text-xs">
                    fn on_misbehavior(&mut self, peer: &PeerId, kind:
                    Misbehavior)
                  </code>
                </li>
              </ul>
