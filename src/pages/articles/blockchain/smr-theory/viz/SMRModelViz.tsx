...
const STEPS = [
  {
    ...
    body: "모든 복제본이 동일한 초기 상태에서 같은 순서로 명령을 실행하면 같은 결과에 도달한다. — Schneider, 1990",
  },
  {
    label: "클라이언트 요청",
    body: "클라이언트가 명령(command)을 제출한다. 예: 트랜잭션, 상태 변경 요청.",
  },
  {
    label: "합의 & 순서 결정",
    body: "리더가 명령을 수집하고, 합의 프로토콜로 전체 순서(total order)를 결정한다.",
  },
  {
    label: "결정론적 실행",
    body: "모든 복제본이 합의된 순서대로 명령을 실행한다. f(S, cmd) → S'. 동일한 상태 전이.",
  },
];
        ...
        >
          ...
          {REPLICAS.map((r) => {
            ...
            return (
              <motion.g
                key={r.label}
                animate={{ opacity: active ? 1 : step === 0 ? 0.5 : 0.3 }}
              >
                <rect
                  x={r.x}
                  y={r.y}
