...
export default function BulletproofsFlowViz() {
  return <figure data-viz="bulletproofs-range-flow" data-viz-canvas className="not-prose my-8 min-w-0 rounded-xl border border-border/70 bg-card p-4 sm:p-6">
    <figcaption><p className="text-xs font-semibold uppercase tracking-[0.16em] text-primary">Commitment → range relation → IPA</p><p className="mt-2 text-sm leading-6 text-muted-foreground">
            같은 hidden w가 commitment와 bit relation 양쪽에 들어가고 긴 relation만 logarithmic rounds로 접습니다.
          </p></figcaption>
