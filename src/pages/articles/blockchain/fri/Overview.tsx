    ...
    <section id="overview" className="mb-16 scroll-mt-20">
      ...
      <div className="prose prose-neutral dark:prose-invert max-w-none">
        <p>
          ...
          <strong> 다항식 차수 바운드를 효율적으로 검증</strong>하는
          프로토콜이다.
          <br />
        ...
        <p>
          ...
          <br />
          검증자(Verifier)는 이 다항식의 차수가 <Math>{"d"}</Math> 미만인지
          확인해야 한다.
          <br />
          문제는 다항식 전체를 읽으면 <Math>{"O(d)"}</Math>개 값을 확인해야
          한다는 것이다.
          <br />
          FRI는 <strong>전체를 읽지 않고</strong> <Math>{"O(\\log d)"}</Math>개
          쿼리만으로 검증한다
        </p>
        ...
        <p>
          차수 <Math>{"d"}</Math>인 다항식을 차수 <Math>{"d/2"}</Math> 문제로
          바꾼다.
          <br />
          다시 <Math>{"d/2"}</Math>를 <Math>{"d/4"}</Math>로,{" "}
          <Math>{"d/4"}</Math>를 <Math>{"d/8"}</Math>로 줄인다.
          <br />
          <Math>{"\\log_2 d"}</Math>번 반복하면 상수 다항식이 되어 직접 확인할
          수 있다
        </p>
        ...
        <p>
          ...
          <br />
          STARK, Plonky2, Plonky3 등{" "}
          <strong>투명 셋업(transparent setup)</strong> 증명 시스템의 핵심 빌딩
          블록이다.
          <br />
          KZG 커밋먼트가 페어링 기반이라면, FRI는 해시 기반 — trusted setup이
          필요 없다
        </p>
      </div>
    </section>
