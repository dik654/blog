  ...
  const systems: { name: string; desc: string; color: string }[] = [
    {
      ...
      desc: "Circle STARK에 FRI 사용. 가장 오래된 상용 STARK 시스템이다",
      color: "indigo",
    },
    {
      name: "Plonky2 (Polygon)",
      desc: "Goldilocks 필드(64비트) + FRI. 재귀 증명에 최적화했다",
      color: "emerald",
    },
    {
      name: "Plonky3 (Polygon)",
      desc: "BabyBear 필드(31비트) + FRI. 모듈형 STARK 프레임워크다",
      color: "amber",
    },
    {
      name: "RISC Zero",
      desc: "RISC-V 범용 zkVM. BabyBear + FRI로 임의 프로그램을 증명한다",
      color: "indigo",
    },
  ];
  ...
  return (
    <section id="stark-usage" className="mb-16 scroll-mt-20">
      ...
      <div className="prose prose-neutral dark:prose-invert max-w-none">
        <p>
          STARK(Scalable Transparent ARgument of Knowledge)는 FRI를 핵심
          서브프로토콜로 사용한다.
          <br />
        ...
        <p>
          ...
          는 차수 <Math>{"< d"}</Math> 다항식의 평가값 집합이다.
          <br />
        ...
        <p>FRI 기반 STARK의 특성을 KZG 기반 SNARK와 비교하면:</p>
        <ul className="list-disc pl-6 space-y-1">
          <li>
            증명 크기: <Math>{"O(\\log^2 d)"}</Math> — KZG의{" "}
            <Math>{"O(1)"}</Math>보다 크다
          </li>
          <li>
            검증 시간: <Math>{"O(\\log d)"}</Math> — KZG의 <Math>{"O(1)"}</Math>{" "}
            페어링보다 느리다
          </li>
          <li>
            증명 시간: <Math>{"O(d \\log d)"}</Math> — KZG MSM과 비슷하다
          </li>
          <li>
            <strong>trusted setup 불필요</strong> — 해시 함수만 있으면 된다
            (투명 셋업)
          </li>
          <li>
            <strong>양자 내성</strong> — 타원곡선/페어링에 의존하지 않는다
          </li>
        </ul>
        <p>
          증명 크기가 커지는 대가로 신뢰 가정을 완전히 제거한다.
          <br />
          실제 STARK 증명은 수십~수백 KB로 SNARK(256B)보다 크지만, 재귀 증명으로
          압축 가능하다
        </p>
      </div>
