...
const STEPS = [
  {
    label: "기존 구조: 합의 + 전파 결합",
    body: "리더가 전체 블록을 모든 검증자에게 전송. 대역폭 낭비 + 리더 병목 + 비응답 시 지연.",
  },
  {
    label: "Commonware 3계층 분리",
    body: "Broadcaster trait(전파 추상화) → ordered_broadcast(인증서 체인) → Zoda(DA 샤딩). 각 계층 독립 동작.",
  },
  {
    label: "데이터 흐름: 트랜잭션 → 인증서 → 실행",
    body: "시퀀서가 ordered_broadcast로 전파 → 검증자 2f+1 서명 → 인증서 형성 → Simplex가 순서 결정 → VM 실행.",
  },
];
