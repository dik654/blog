    ...
    annotations: [
      {
        lines: [4, 7],
        color: "sky",
        note: "Broadcaster trait — 세 연관 타입: Recipients(수신자), Message(직렬화), Response(응답)",
      },
      {
        lines: [18, 24],
        color: "emerald",
        note: "broadcast() → oneshot::Receiver<Response> 반환. 비동기 결과 수신",
      },
    ],
  },
  "buffered-engine": {
    path: "broadcast/src/buffered/engine.rs",
    code: engineRs,
    lang: "rust",
    highlight: [5, 18],
    ...
    annotations: [
      {
        lines: [5, 18],
        color: "sky",
        note: "Engine 필드: items(전역 캐시) + deques(피어별 LRU) + counts(refcount)",
      },
      {
        lines: [21, 42],
        color: "emerald",
        note: "select_loop! — mailbox(Broadcast/Subscribe/Get) + network + peer 변경 동시 처리",
      },
    ],
  },
  "buffered-ingress": {
    path: "broadcast/src/buffered/ingress.rs",
    code: ingressRs,
    lang: "rust",
    highlight: [2, 20],
    ...
    annotations: [
      {
        lines: [2, 20],
        color: "sky",
        note: "Message enum — Broadcast/Subscribe/Get 세 종류. 각각 oneshot 응답 채널 포함",
      },
      {
        lines: [28, 42],
        color: "amber",
        note: "Mailbox가 Broadcaster trait 구현 — 외부에서는 trait만 보면 됨",
      },
    ],
  },
};
