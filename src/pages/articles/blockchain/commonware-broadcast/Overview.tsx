    ...
    <section id="overview" className="mb-16 scroll-mt-20">
      ...
      <div className="prose prose-neutral dark:prose-invert max-w-none mb-6">
        <p className="leading-7">
          ...
          <br />
          대역폭 낭비(중복 전송) · 리더 병목(단일 노드 의존) · 리더 비응답 시
          전체 지연
        </p>
        <p className="leading-7">
          Commonware의 접근: <strong>합의와 전파를 완전 분리</strong>
          <br />
          3계층 — <code>Broadcaster</code> trait(전파 추상화) →{" "}
          <code>ordered_broadcast</code>(인증서 체인) → <code>Zoda</code>(DA
          샤딩)
        </p>
      </div>
      ...
      <div className="prose prose-neutral dark:prose-invert max-w-none mt-6">
        ...
        <div className="not-prose bg-muted/50 rounded-lg border p-4 my-4 space-y-4">
            ...
            <div className="bg-background rounded-md border p-3 text-sm">
              <p className="font-semibold">
                ...
                <span className="text-muted-foreground font-normal">
                  (github.com/commonwarexyz)
                </span>
              </p>
              <p className="text-xs text-muted-foreground mt-1">
                블록체인 인프라 구축용 Rust 툴킷 — 모듈 단위 프리미티브: P2P,
          ...
          <div>
            ...
            <div className="bg-background rounded-md border p-3 text-sm">
              <p>
                Reed-Solomon으로 블록을 N개 샤드로 분할 → 각 검증자가 1/N만 전송
                → 대역폭 분산
              </p>
              <p className="text-xs text-muted-foreground mt-1">
                단, 샤드 조립(재구성) 단계가 필요
          ...
          <div>
            ...
            <p className="text-xs text-muted-foreground mt-2">
              영감: Narwhal/Bullshark DAG 합의 · Celestia DA · EigenDA · 가용성
              증명
            </p>
          </div>
        </div>
          ...
          <div className="bg-background rounded-md border p-3">
            <div className="flex items-center gap-2 mb-1">
              ...
              <span className="font-semibold text-sm">
                <code>Broadcaster</code> trait (전파 추상화)
              </span>
            </div>
            <p className="text-xs text-muted-foreground">
              최소 인터페이스 · 구현 세부사항 은닉 · 플러거블 엔진
            </p>
          </div>
          <div className="bg-background rounded-md border p-3">
            <div className="flex items-center gap-2 mb-1">
              ...
              <span className="font-semibold text-sm">
                <code>ordered_broadcast</code> (인증서 체인)
              </span>
            </div>
            <p className="text-xs text-muted-foreground">
              시퀀서별 독립 체인 · height 인덱싱 · 쿼럼 인증서 · 인과 순서 보장
            </p>
          </div>
          <div className="bg-background rounded-md border p-3">
            <div className="flex items-center gap-2 mb-1">
              ...
              <span className="font-semibold text-sm">
                <code>Zoda</code> (DA + 이레이저 코딩)
              </span>
            </div>
            <p className="text-xs text-muted-foreground">
              Reed-Solomon 인코딩 · Merkle 커밋 샤드 · 가용성 증명 · 1-of-n 정직
              홀더면 충분
            </p>
          </div>
        </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
            <div className="bg-background rounded-md border p-3">
              ...
              <p className="text-[11px] text-muted-foreground mt-1">
                <code>Broadcaster</code> trait 정의
              </p>
            </div>
            <div className="bg-background rounded-md border p-3">
            ...
            <div className="bg-background rounded-md border p-3">
              ...
              <p className="text-[11px] text-muted-foreground mt-1">
                <code>engine.rs</code> · <code>ack_manager.rs</code> ·{" "}
                <code>tip_manager.rs</code> · <code>types.rs</code>
              </p>
            </div>
            <div className="bg-background rounded-md border p-3">
              <p className="font-semibold text-xs font-mono">zoda/</p>
              <p className="text-[11px] text-muted-foreground mt-1">
                <code>scheme.rs</code> · <code>validating_scheme.rs</code> ·{" "}
                <code>phased_scheme.rs</code>
              </p>
            </div>
          </div>
        </div>
          ...
          <div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
              <div className="bg-background rounded-md border px-3 py-2">
                ...
                <p className="text-[11px] text-muted-foreground">
                  <code>Broadcaster</code> trait → 다중 엔진 구현 가능
                </p>
              </div>
              <div className="bg-background rounded-md border px-3 py-2">
                <p className="text-xs font-semibold">Async-first</p>
                <p className="text-[11px] text-muted-foreground">
                  <code>oneshot</code> 채널로 비동기 응답 전달
                </p>
              </div>
              <div className="bg-background rounded-md border px-3 py-2">
                <p className="text-xs font-semibold">버퍼링</p>
                <p className="text-[11px] text-muted-foreground">
                  메시지 큐잉 후 네트워크 정책에 따라 flush
                </p>
              </div>
              <div className="bg-background rounded-md border px-3 py-2">
