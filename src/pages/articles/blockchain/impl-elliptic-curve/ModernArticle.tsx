...
export default function ModernCurveImplementationArticle() {
  return <article className="space-y-14">
    ...
    <section id="curve-profile" className="space-y-6">
      ...
      <ExplainedFormula question="Curve profile이 어떤 객체를 구현하는지 최소한 어떻게 확인할까?" idea={<>Generator가 equation 위에 있고 subgroup order를 곱하면 identity가 되는지 artifact 생성 단계에서 확인합니다.</>} formula={String.raw`G=(x_G,y_G):\ y_G^2=x_G^3+ax_G+b\pmod p,\qquad [r]G=\mathcal O`}
      annotatedFormula={String.raw`G=\underbrace{(x_G,y_G):\ y_G^2=x_G^3+ax_G+b\pmod p,\qquad [r]G=\mathcal O}_{\text{Identity 계산}}`}
      operations={[
        { expression: String.raw`(x_G,y_G):\ y_G^2=x_G^3+ax_G+b\pmod p,\qquad [r]G=\mathcal O`, annotation: ["Identity이(가) 식의 결과에 기여하는 방식을","계산합니다.","Generator가 equation 위에 있고 subgroup","order를 곱하면 identity가 되는지 artifact"] },
      ]} terms={[
    ...
    <section id="point-encoding" className="space-y-6">
      ...
      operations={[
        { expression: String.raw`x^3+ax+b\pmod p,\qquad y\in\{y_0,\ p-y_0\},\qquad [r]P=\mathcal O`, annotation: ["Identity이(가) 식의 결과에 기여하는 방식을","계산합니다.","Equation의 square root는 두 후보를 만들며","sign bit가 하나를 고릅니다."] },
      ]} terms={[
    ...
    <section id="coordinate-ops" className="space-y-6">
      ...
      annotatedFormula={String.raw`(x,y)=\underbrace{\left(\frac{X}{Z^2},\frac{Y}{Z^3}\right),\qquad Z\ne0}_{\text{기준량당 비율}}`}
      operations={[
        { expression: String.raw`\left(\frac{X}{Z^2},\frac{Y}{Z^3}\right),\qquad Z\ne0`, annotation: ["분자에 둔 관심량을 분모의 기준량으로 정규화합니다.","Z의 역원을 매 덧셈마다"] },
      ]} terms={[
    ...
    <section id="release-gate" className="space-y-6">
      ...
      annotatedFormula={String.raw`e([a]P,[b]Q)=\underbrace{e(P,Q)^{ab},\qquad e(P,Q)\ne1}_{\text{Target exponentiation 계산}}`}
      operations={[
        { expression: String.raw`e(P,Q)^{ab},\qquad e(P,Q)\ne1`, annotation: ["Target exponentiation이(가) 식의 결과에","기여하는 방식을 계산합니다.","Scalar multiplication을 pairing 전후","어느 쪽에 적용해도 같은 target-group 값이 나와야"] },
      ]} terms={[
