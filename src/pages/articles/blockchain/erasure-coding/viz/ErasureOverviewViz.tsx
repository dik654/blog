...
const STEPS = [
  ...
  {
    label: "인코딩: n 조각으로 확장",
    body: "이레이저 코딩으로 n개 조각을 생성합니다 (n > k). 추가 조각은 패리티입니다.",
  },
  {
    label: "일부 손실 (최대 n-k 조각)",
    body: "전송/저장 중 일부가 손실되어도 n-k개까지 복구할 수 있습니다.",
  },
  {
    label: "임의 k 조각으로 복구",
    body: "임의의 k개 조각만 있으면 원본을 완전히 복원합니다.",
  },
];
const K = 4,
  N = 8;
        ...
        >
          {Array.from({ length: N }).map((_, i) => {
            ...
            return (
              <motion.g
                key={i}
                animate={{ opacity: lost ? 0.12 : visible ? 1 : 0 }}
              >
                <rect
                  x={x}
                  y={30}
                  ...
                  rx={5}
                  fill={
                    lost ? "#ef444418" : restored ? `${C[2]}22` : `${color}15`
                  }
                  stroke={lost ? "#ef4444" : restored ? C[2] : color}
                  strokeWidth={1.2}
                  strokeDasharray={lost ? "3 2" : "none"}
                />
                <text
                  x={x + 22}
                  y={50}
                  ...
                  fontSize={9}
                  fontWeight={600}
                  fill={lost ? "#ef4444" : restored ? C[2] : color}
                >
                  {lost ? "X" : `D${i + 1}`}
                </text>
                <text
