...
const STEPS = [
  {
    label: "데이터를 2D 매트릭스 배치",
    body: "원본 k x k 데이터를 정사각 매트릭스로 배치합니다.",
  },
  {
    label: "행/열별 RS 인코딩 확장",
    body: "각 행과 열에 RS 코딩을 적용하여 2k x 2k로 확장합니다.",
  },
  {
    label: "DAS: 랜덤 셀 샘플링",
    body: "라이트 노드가 랜덤 셀을 샘플링하여 가용성을 검증합니다.",
  },
  {
    label: "손실 시 행/열 복구",
    body: "행 또는 열 단위로 독립 복구가 가능해 효율적입니다.",
  },
];
const GS = 22,
  PAD = 30;
            ...
            Array.from({ length: step >= 1 ? 8 : 4 }).map((_, c) => {
              ...
              return (
                <motion.rect
                  ...
                  rx={3}
                  fill={
                    sampled ? `${C[2]}30` : lost ? "#ef444415" : `${color}12`
                  }
                  stroke={color}
                  strokeWidth={sampled ? 1.5 : lost ? 1.5 : 0.8}
