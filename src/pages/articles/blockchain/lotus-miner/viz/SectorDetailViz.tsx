...
import { STEPS, STEP_REFS, STEP_LABELS } from './SectorDetailVizData';
import { Step0, Step1, Step2, Step3 } from './SectorDetailSteps';

const R = [Step0, Step1, Step2, Step3];

interface Props { onOpenCode?: (key: string) => void }

export default function SectorDetailViz({ onOpenCode }: Props) {
  return (
    <StepViz steps={STEPS}>
      {(step) => {
        const S = R[step];
