  ...
  return (
    <section id="fr-scalar" className="mb-16 scroll-mt-20">
      ...
      <div className="prose prose-neutral dark:prose-invert max-w-none">
        ...
        <table className="w-full text-sm border-collapse">
          <thead>
            <tr className="border-b">
              ...
              <th className="text-left p-2">Fp (base field)</th>
              <th className="text-left p-2">Fr (scalar field)</th>
            </tr>
          </thead>
          <tbody>
            <tr className="border-b">
              <td className="p-2 font-semibold">역할</td>
              <td className="p-2">점의 좌표 범위</td>
              <td className="p-2">스칼라 (점을 몇 번 더할지) 범위</td>
            </tr>
            <tr className="border-b">
              <td className="p-2 font-semibold">용도</td>
              <td className="p-2">G1 좌표 (x, y)</td>
              <td className="p-2">ZK witness, 회로 변수</td>
            </tr>
          </tbody>
        </table>
        <p className="mt-3">
          비유하면, 지도의 좌표는 위도/경도(Fp)로 표현하고 &quot;몇 km
          이동&quot;은 거리(Fr)로 표현한다.
        </p>
        ...
        <h3 className="text-xl font-semibold mt-6 mb-3">
          define_prime_field! 매크로로 코드 재사용
        </h3>
        <p>
          Fr의 add, sub, mont_mul, inv 모두 Fp와 로직이 동일하고 상수만 다르다.
          Rust의{" "}
        ...
        <CodePanel
          title="define_prime_field! 매크로"
          code={macroCode}
          annotations={[
            {
              lines: [2, 5],
              color: "sky",
              note: "3가지 상수만 파라미터로 받음",
            },
            {
              lines: [7, 7],
              color: "emerald",
              note: "구조체부터 trait impl까지 전부 생성",
            },
          ]}
        />
        <p>
          하나의 매크로 호출로 16가지가 생성된다: 상수(MODULUS, R, R2, INV),
          구조체, 메서드(from_raw, mont_mul, add, sub, pow, inv 등), 연산자(+,
          -, *), Display.
        </p>
        ...
        />
        <p>
          상수 3개(MODULUS, R, R2)만 바꾸면 새 유한체를 생성할 수 있다. 나중에
          BLS12-381 등 다른 커브를 추가할 때도 같은 매크로를 재사용할 수 있다.
        </p>
      </div>
    </section>
  );
