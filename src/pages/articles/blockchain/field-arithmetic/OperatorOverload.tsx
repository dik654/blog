  ...
  return (
    <section id="operator-overload" className="mb-16 scroll-mt-20">
      ...
      <div className="prose prose-neutral dark:prose-invert max-w-none">
        ...
        <p>
          Rust에서{" "}
          <code className="bg-accent px-1.5 py-0.5 rounded text-sm">a + b</code>
          를 쓰면 컴파일러가 트레잇 메서드를 호출한다.
        </p>
        <CodePanel
          title="연산자 → 트레잇 디스패치"
          code={dispatchCode}
        ...
        />
        <p>
          <code className="bg-accent px-1.5 py-0.5 rounded text-sm">Fp</code>는{" "}
          <code className="bg-accent px-1.5 py-0.5 rounded text-sm">Copy</code>
          이므로 값으로 받아도 32바이트 복사 비용이 거의 없다.
        </p>

        <h3 className="text-xl font-semibold mt-6 mb-3">
          Mul이 mont_mul을 호출하는 이유
        </h3>
        <CodePanel
          title="Mul → mont_mul"
          code={mulImplCode}
          ...
          annotations={[
            {
              lines: [3, 3],
              color: "sky",
              note: "곱셈의 실체는 Montgomery 곱셈",
            },
          ]}
        />

        <h3 className="text-xl font-semibold mt-6 mb-3">
          Display: Montgomery form이 아닌 실제 값 출력
        </h3>
        <CodePanel
          title="Display 트레잇"
          code={displayCode}
          annotations={[
            { lines: [3, 3], color: "sky", note: "Montgomery → normal 변환" },
            {
              lines: [4, 5],
              color: "emerald",
              note: "big-endian 순서로 hex 출력",
            },
          ]}
        />
        <p>
          내부 저장값은 Montgomery form(a * R mod p)이라 그대로 출력하면 의미
          없는 숫자가 나온다.{" "}
        ...
        />
        <p>
          Montgomery 구현은 상수 하나만 틀려도 모든 곱셈이 조용히 틀린 값을
          낸다. 공리 테스트는 구현이 수학적으로 올바른 체인지 한꺼번에 검증하는
          통합 테스트 역할을 한다.
        </p>
      </div>
    </section>
