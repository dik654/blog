...
const STEPS = [
  {
    label: "입력: 일반 정수 a, b",
    body: "a=7, b=5. 아직 Montgomery 형태가 아닌 일반 정수.",
  },
  {
    label: "R-도메인 진입: to_mont(a)",
    body: "ã = a·R mod p. R=2²⁵⁶. 한 번의 변환으로 진입.",
  },
  {
    label: "CIOS 곱셈: ã × b̃",
    body: "limb 4개씩 곱셈+축소. 나눗셈 없이 시프트만.",
  },
  {
    label: "CIOS 내부: limb-by-limb",
    body: "T += aᵢ·b̃, m=T₀·p′ mod 2⁶⁴, >>= 64. 4회.",
  },
  {
    label: "R-도메인 탈출: from_mont",
    body: "result·R⁻¹ mod p 복원. 나눗셈 0회 달성!",
  },
];
        ...
        >
          ...
          <motion.g animate={{ opacity: step >= 2 ? 1 : 0.15 }} transition={sp}>
            <rect
              ...
              fill={`${C.cios}10`}
              stroke={C.cios}
              strokeWidth={step === 2 || step === 3 ? 1.5 : 0.6}
              strokeDasharray={step === 3 ? "0" : "3 2"}
            />
            <text
              x={210}
              y={22}
            ...
            {[0, 1, 2, 3].map((i) => (
              <motion.rect
                ...
                rx={2}
                animate={{
                  fill: step === 3 ? `${C.cios}30` : `${C.cios}10`,
                  strokeWidth: step === 3 ? 1.2 : 0.5,
                }}
                stroke={C.cios}
                transition={{ ...sp, delay: step === 3 ? i * 0.12 : 0 }}
              />
            ))}
            {[0, 1, 2, 3].map((i) => (
              <text
