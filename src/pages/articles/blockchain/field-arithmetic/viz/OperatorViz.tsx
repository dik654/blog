...
const STEPS = [
  {
    label: "Rust 연산자 오버로딩",
    body: "Fp 구조체에 Add, Sub, Mul 등 trait를 구현하여 a + b, a * b 문법을 사용합니다.",
  },
  {
    label: "Add: limb별 carry 전파",
    body: "limb[0]부터 더하고 carry를 다음 limb에 전파. 결과 ≥ p이면 조건부 감산.",
  },
  {
    label: "Mul: Montgomery CIOS",
    body: "4×4 limb 곱셈 → 즉시 축소. 가장 비싼 연산이지만 나눗셈 0회.",
  },
  {
    label: "Inv: Fermat 소정리",
    body: "a⁻¹ = a^(p-2) mod p. 약 254번의 제곱+곱. 드물게 사용하지만 필수.",
  },
];
    ...
    <StepViz steps={STEPS}>
      {(step) => (
        ...
        >
          ...
          {/* Cost bar chart */}
          <text x={20} y={60} fontSize={9} fill="var(--muted-foreground)">
            상대 비용
          </text>
          {OPS.map((o, i) => {
            const x = 22 + i * 64;
          ...
          >
            ...
            >
              {step === 1
                ? "let c = a + b;  // Add<Output=Fp> → carry + 조건감산"
                : step === 2
                  ? "let c = a * b;  // Mul<Output=Fp> → Montgomery CIOS"
                  : step === 3
                    ? "let c = a.invert();  // a^(p-2) mod p"
