  ...
  return (
    <section id="overview" className="mb-16 scroll-mt-20">
      <h2 className="text-2xl font-bold mb-6">
        개요 &amp; StableSwap invariant
      </h2>
      <div className="prose prose-neutral dark:prose-invert max-w-none">
        <StableSwapCurveViz />

        <h3 className="text-xl font-semibold mt-6 mb-3">
          Curve Finance의 문제 인식
        </h3>
        <p>
          Uniswap V2 (x·y=k)의 한계:
          <br />- USDC/USDT 같은 <strong>페그 자산</strong>에는 너무 변동성 큼
          <br />
          - $1 = $1 교환 시에도 슬리피지 발생
          <br />- 자본 효율 낮음 (대부분 유동성이 "비현실적 가격"에 잠김)
        </p>
        <p>
          Curve(2020)의 해답: <strong>StableSwap AMM</strong>
          <br />
...
n:   토큰 개수`}</pre>
        <p>
          <strong>2개 파라미터</strong>: D (풀 크기), A (증폭 계수)
          <br />
          A = 0: Constant Product (Uniswap 동일)
          <br />
          A = ∞: Constant Sum (완전 페그)
          <br />
...
A ↓: 페그 근처 효율 ↓, 디페그 회복 더 빠름`}</pre>
        <p>
          <strong>A는 "페그 가정"의 강도</strong>
          <br />
          A 크면 → "이 자산들은 1:1이다" 강한 가정
          <br />
          A 작으면 → "가격 변동 가능하다" 보수적
          <br />
          Terra UST 붕괴 시 Curve 3pool의 A값이 재조정됨
        </p>

        <h3 className="text-xl font-semibold mt-8 mb-3">
          D 계산 — Newton&apos;s method
        </h3>
...
def get_D(xp: List[uint256], amp: uint256) -> uint256:
    ...
    raise ValueError("D calculation: did not converge")`}</pre>
        <p>
          <strong>닫힌 형식 해 없음</strong>: Newton&apos;s method로 수치 해
          <br />
          보통 4-6회 반복으로 수렴 — 가스 비용 ~50K
          <br />
          255회 상한 — 극단 상황 방어
        </p>

        <h3 className="text-xl font-semibold mt-8 mb-3">
          실제 슬리피지 — USDC/USDT (A=100)
        </h3>
        ...
        <div className="bg-amber-50 dark:bg-amber-950/30 border-l-4 border-amber-400 p-4 my-6 rounded-r-lg">
          <p className="font-semibold mb-2">
            인사이트: 도메인 특화 AMM이 갖는 가치
          </p>
          <p>
            Uniswap: 범용 AMM — 모든 페어에 작동
            <br />
          ...
          <p className="mt-2">
            트레이드오프:
            <br />
            - Uniswap은 "exotic token"도 지원하지만 stable 페어에 비효율
            <br />- Curve는 stable에 100배 효율, 변동성 자산은 불가
          </p>
          <p className="mt-2">
            <strong>결과: DeFi는 AMM 다양화</strong>
            <br />
            - Uniswap: 변동성 자산 (ETH, WBTC 등)
            <br />
            - Curve: stable swap (USDC, DAI, USDT)
            <br />
            - Balancer: 다중 자산 풀<br />- Liquidity Book (TraderJoe): 구간
            기반
          </p>
          <p className="mt-2">
            교훈: <strong>"모든 것에 맞는 해결책은 없다"</strong>
            <br />
