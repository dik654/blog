...
A · n^n · Σx + D = A · D · n^n + D^(n+1) / (n^n · Πx)`}</pre>
        <p>
          <strong>핵심 아이디어</strong>: 균형 상태(<code>x_i = D/n</code>)에서
          CS로 작동, 이탈하면 CP로 전환
          <br />
          가중치 χ가 자동 조정 — <strong>부드러운 전환</strong>
          <br />
...
// invariant 유지: y'를 찾아 Δy = y - y' 계산`}</pre>
        <p>
          <strong>스왑 로직</strong>: 입력 후 새 invariant 만족하는 출력 계산
          <br />
          D는 고정 (swap 시 변화 없음)
          <br />y 해를 Newton&apos;s method로 수치 해
        </p>
...
# Newton's method for: y² + b·y + c = D·y`}</pre>
        <p>
          <strong>2차방정식 수치 해</strong>: <code>y² + b·y + c = D·y</code>
          <br />
          b, c는 다른 토큰들의 잔액·invariant로 결정
          <br />
          수렴까지 평균 5-10 iteration
        </p>

        <h3 className="text-xl font-semibold mt-8 mb-3">
          exchange() — Curve 핵심 함수
        </h3>
...
def exchange(
    ...
    return dy`}</pre>
        <p>
          <strong>Vyper 언어 사용</strong>: Curve의 주요 컨트랙트는 Vyper로 작성
          <br />
          Vyper는 Solidity보다 <strong>간결하고 읽기 쉬움</strong> — 수학 중심
          컨트랙트에 적합
          <br />
...
// 외부 입출력 시 RATES로 변환`}</pre>
        <p>
          <strong>decimals 통일</strong>: 모든 계산은 18 decimals 기준
          <br />
          USDC (6) → 18로 scale up, 사용자 인터페이스에서는 다시 6으로 scale
          down
          <br />이 정규화 없으면 각 토큰 비율 계산 복잡
        </p>
...
// 의미: LP가 풀 균형 깨면 페널티`}</pre>
        <p>
          <strong>스테이블 페어라 낮은 수수료</strong>: 0.04% (Uniswap V2의 1/7)
          <br />
          Imbalance fee: LP가 풀 편향 유발 시 추가 수수료
          <br />이 메커니즘이 <strong>균형 유지 인센티브</strong> 제공
        </p>
        ...
        <div className="bg-amber-50 dark:bg-amber-950/30 border-l-4 border-amber-400 p-4 my-6 rounded-r-lg">
          ...
          <p className="mt-2">
            ...
            <br />
            ✓ 255회 상한 — 무한 루프 방지
            <br />✓ <code>abs(diff) ≤ 1</code> 수렴 조건 — 1 wei 정밀도 충분
            <br />✓ 평균 5-10회 수렴 — 가스 예측 가능
          </p>
          <p className="mt-2">
            <strong>교훈</strong>: DeFi 수학이 항상 닫힌 형식일 필요 없음
            <br />
            반복 계산이 실용적이고 예측 가능하면 괜찮음
            <br />
