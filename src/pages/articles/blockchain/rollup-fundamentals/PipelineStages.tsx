  ...
  return (
    <section id="pipeline-stages" className="mb-16 scroll-mt-20">
      <h2 className="text-2xl font-bold mb-6">
        Derivation Pipeline: 7단계 상세
      </h2>
      <div className="prose prose-neutral dark:prose-invert max-w-none mb-6">
        <p className="leading-7">
          op-node의 Derivation Pipeline은 L1 데이터를 L2 블록으로 변환하는 7단계
          pull 파이프라인이다.
          <br />
          "pull"이란 뒤쪽 스테이지(AttributesQueue)가 앞쪽(L1Traversal)에
          데이터를 요청하는 방식이다.
          <br />
          필요할 때만 가져오므로 메모리를 절약하고, 역압(backpressure)이 자동
          적용된다.
        </p>
      </div>
      {/* Pull vs Push 비교 */}
      <div className="not-prose rounded-lg border border-border/60 p-4 mb-4">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-3">
          <div className="rounded bg-emerald-50/50 dark:bg-emerald-950/10 border border-emerald-200/50 dark:border-emerald-800/30 p-3">
            <p className="font-mono font-bold text-xs text-emerald-700 dark:text-emerald-400 mb-1">
              Pull (OP Stack 선택)
            </p>
          ...
          <div className="rounded bg-red-50/50 dark:bg-red-950/10 border border-red-200/50 dark:border-red-800/30 p-3">
            <p className="font-mono font-bold text-xs text-red-700 dark:text-red-400 mb-1">
              Push (미사용)
            </p>
        ...
        {sel && (
          ...
          >
            ...
            {sel.id === "l1-traversal" && (
              <CodeViewButton
                onClick={() =>
                  onCodeRef("l1-traversal", codeRefs["l1-traversal"])
                }
              />
            )}
            {sel.id === "channel-bank" && (
              <CodeViewButton
                onClick={() =>
                  onCodeRef("channel-bank", codeRefs["channel-bank"])
                }
              />
            )}
            {sel.id === "attributes-queue" && (
              <CodeViewButton
                onClick={() =>
                  onCodeRef("attributes-queue", codeRefs["attributes-queue"])
                }
              />
            )}
          </motion.div>
