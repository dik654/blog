  ...
  return (
    <section id="comparison" className="mb-16 scroll-mt-20">
      ...
      <div className="prose prose-neutral dark:prose-invert max-w-none mb-6">
        <p>
          두 방식 모두 "실행은 오프체인, 검증은 L1"이라는 롤업 원칙을 따른다.
          <br />
          차이는 검증 방법 — 사기 증명(fraud proof) vs 유효성 증명(validity
          proof). 아래 카테고리를 클릭하면 각 측면의 상세 비교를 확인할 수 있다.
        </p>
      </div>
      {/* Category selector */}
      <div className="not-prose grid grid-cols-2 sm:grid-cols-4 gap-3 mb-4">
        {CATEGORIES.map((c) => (
          <button
            ...
            style={{
              borderColor:
                selected === c.id ? "#6366f1" : "var(--color-border)",
              background:
                selected === c.id ? "rgba(99,102,241,0.06)" : undefined,
            }}
          >
      ...
      <div className="prose prose-neutral dark:prose-invert max-w-none mb-6">
        ...
        <p>
          op-batcher(L2 배치 데이터를 L1에 제출하는 컴포넌트)는 블록을 채널로
          압축(zlib/brotli)하여 L1 데이터 비용을 최소화한다. processBlocks()에서
          채널을 최대한 채운 뒤 한 번에 제출한다.
          <br />
        ...
        <div className="not-prose flex flex-wrap gap-2 my-3">
          <CodeViewButton
            onClick={() =>
              onCodeRef("process-blocks", codeRefs["process-blocks"])
            }
          />
          ...
          <CodeViewButton
            onClick={() => onCodeRef("tx-data", codeRefs["tx-data"])}
          />
          <span className="text-[10px] text-muted-foreground self-center">
            DA 타입 전환
          </span>
        ...
        <h3 className="text-lg font-semibold mb-3">
          보안 모델: 1-of-N vs 수학적 증명
        </h3>
        <p>
          Optimistic — <strong>"정직한 검증자 1명이면 충분"</strong>. Bisection
          Game(분쟁 범위를 반씩 좁히는 이진 분할 게임)의 체스 클럭(Chess
          Clock)이 양측에 공정한 응답 시간을 보장한다.
          <br />
          ZK — 수학적 증명 의존. 정직 가정 불필요하지만, 증명 회로 버그 위험이
          있다.
        </p>
        <div className="not-prose flex flex-wrap gap-2 my-3">
          <CodeViewButton
            onClick={() => onCodeRef("game-state", codeRefs["game-state"])}
          />
          <span className="text-[10px] text-muted-foreground self-center">
            Game 인터페이스
          </span>
          <CodeViewButton
            onClick={() => onCodeRef("output-root", codeRefs["output-root"])}
          />
          <span className="text-[10px] text-muted-foreground self-center">
            Output Root
          </span>
