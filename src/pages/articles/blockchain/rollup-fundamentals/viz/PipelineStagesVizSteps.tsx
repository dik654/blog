...
export function PullArrowOverlay() {
  ...
  return (
    ...
    >
      <path
        d={`M${last.x + BW / 2},${last.y + BH + 8} L${last.x + BW / 2},${last.y + BH + 20} L${first.x + BW / 2},${last.y + BH + 20} L${first.x + BW / 2},${first.y + BH + 8}`}
        fill="none"
        stroke="#ec4899"
        strokeWidth={1.2}
        strokeDasharray="4 3"
...
export function StageDetailOverlay({ indices }: { indices: number[] }) {
  const DETAIL_LINES: Record<number, [string, string]> = {
    0: [
      "l1Block := l1Client.BlockByNumber(cursor)",
      "// L1 블록 순회 + reorg 감지",
    ],
    1: [
      "txs := filterBatcherTxs(block, batcherAddr)",
      "// 배처 TX 필터 → calldata/blob 추출",
    ],
    2: ["frames := decodeFrames(rawData)", "// raw → Frame{id, data} 디코딩"],
    3: [
      "if ch.IsReady() { return ch }",
      "// 같은 ID 프레임 조립, 타임아웃 드롭",
    ],
    4: [
      "batch := decompress(channel.Data())",
      "// zlib/brotli 해제 → 배치 복원",
    ],
    5: [
      "if batch.Timestamp < safeHead.Time { skip }",
      "// 타임스탬프 검증 + 순서 정렬",
    ],
    6: [
      "attrs := PayloadAttributes{NoTxPool: true}",
      "// op-geth에 전달할 블록 속성",
    ],
  };
  return (
