...
export function FaultStep0() {
  const fields = [
    {
      line: "Position  {depth, indexAtDepth}",
      desc: "이진 트리 좌표",
      c: C.pos,
    },
    {
      line: "Value     common.Hash",
      desc: "상태 해시 (output root)",
      c: C.val,
    },
    { line: "Bond      *big.Int", desc: "예치금 (패배 시 몰수)", c: C.val },
    {
      line: "CounteredBy common.Address",
      desc: "반박자 주소 (mutable)",
      c: C.err,
    },
      ...
      {fields.map((f, i) => {
        ...
        return (
          ...
          >
            ...
            <text x={340} y={y + 15} fontSize={10} fill="var(--foreground)">
              {f.desc}
            </text>
          </motion.g>
        );
  ...
  return (
    <g>
      <text
        ...
        fill="var(--foreground)"
      >
        Position → TraceIndex (maxDepth=3 예시)
      </text>
      <motion.g
        initial={{ opacity: 0, y: 8 }}
        animate={{ opacity: 1, y: 0 }}
