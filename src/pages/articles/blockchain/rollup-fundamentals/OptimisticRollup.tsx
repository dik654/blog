...
export default function OptimisticRollup({
  onCodeRef,
}: {
  ...
  return (
    <section id="optimistic" className="mb-16 scroll-mt-20">
      <h2 className="text-2xl font-bold mb-6">
        Optimistic Rollup: OP Stack 코드 추적
      </h2>
      <div className="prose prose-neutral dark:prose-invert max-w-none mb-6">
        <p>
          "일단 믿고, 틀리면 증명한다" — Optimistic Rollup의 핵심 원리. 모든
          트랜잭션을 유효하다고 가정하고, 7일 챌린지 기간(Fraud Proof 제출
          가능한 이의 제기 창) 내에 이의가 없으면 확정한다.
          <br />
          아래 카드를 클릭하면 각 파이프라인의 상세 동작과 설계 판단을 확인할 수
          있다.
        </p>
      </div>
                      ...
                      <div className="flex flex-wrap gap-2">
                        {s.codeRefKeys.map((k, i) => (
                          <div key={k} className="flex items-center gap-1">
                            <CodeViewButton
                              onClick={() => onCodeRef(k, codeRefs[k])}
                            />
