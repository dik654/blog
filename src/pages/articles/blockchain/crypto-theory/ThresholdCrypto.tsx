  ...
  return (
    <section id="threshold-crypto" className="mb-16 scroll-mt-20">
      ...
      <div className="prose prose-neutral dark:prose-invert max-w-none mb-6">
        <p>
          Shamir(1979) — 비밀을 여러 share로 분산하여 t개 이상 모여야 복원. 단일
          장애점 제거.
        </p>
      </div>
      ...
      <div className="prose prose-neutral dark:prose-invert max-w-none mt-6">
        ...
        <p>
          ...
          <M>{"t"}</M>개 이상 모이면 복원하고 <M>{"t-1"}</M>개 이하면 정보가 0인
          체계. 원리는 <strong>다항식 보간(polynomial interpolation)</strong>
        </p>
      </div>
      ...
      <div className="not-prose rounded-lg border border-amber-500/20 bg-amber-500/5 p-5 mb-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div>
            ...
            <p className="text-xs text-foreground/50 mt-2 mb-1 font-semibold">
              생성된 shares
            </p>
            <div className="flex flex-wrap gap-2">
              {[
                ...
                <span
                  key={x}
                  className="rounded border border-amber-500/10 bg-amber-500/5 px-2 py-0.5 font-mono text-xs text-foreground/70"
                >
                  ({x}, {y})
                </span>
              ))}
          ...
          <div>
            <ul className="text-sm text-foreground/80 space-y-1.5 list-disc list-inside">
              <li>
                3개 모으면 <M>{"f(0) = 100"}</M> 복원 가능
              </li>
              <li>
                2개만 있으면 무한한 <M>{"f(x)"}</M>가 가능 — 정보 없음
              </li>
            </ul>
      ...
      <div className="prose prose-neutral dark:prose-invert max-w-none mt-8">
        ...
        <p>
          비밀 키를 SSS로 분산 → 서명도 분산 계산. 외부에서 보면 일반 서명과
          동일
        </p>
      </div>
      ...
      <div className="not-prose grid grid-cols-1 sm:grid-cols-3 gap-4 mb-4">
        ...
        <div className="rounded-lg border border-indigo-500/20 bg-indigo-500/5 p-5">
          ...
          <p className="text-sm text-foreground/80">
            일반 서명과 구분 불가 — 검증자는 threshold 구조를 알 필요 없음
          </p>
        </div>
      </div>
      ...
      <div className="not-prose grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="rounded-lg border border-foreground/10 bg-muted/30 p-5">
          ...
          <div className="grid grid-cols-2 gap-2">
            {[
              ...
              <div
                key={p.name}
                className="rounded border border-foreground/5 bg-background/50 p-2"
              >
        ...
        <div className="rounded-lg border border-emerald-500/20 bg-emerald-500/5 p-5">
          ...
          <p className="text-sm text-foreground/70">
            Fireblocks, ZenGo, Coinbase / Validator key management / Multi-sig
            대체 / HSM 분산화
          </p>
        </div>
      </div>
