  ...
  return (
    <section id="key-exchange" className="mb-16 scroll-mt-20">
      ...
      <div className="prose prose-neutral dark:prose-invert max-w-none mb-6">
        <p>
          안전하지 않은 채널에서 공유 비밀을 수립하는 프로토콜. DH(1976)가 원조,
          ECDH가 현대 표준.
        </p>
      </div>
      ...
      <div className="prose prose-neutral dark:prose-invert max-w-none mt-6">
        ...
        <p>
          공개 파라미터: 큰 소수 <M>{"p"}</M>, 원시근(generator){" "}
          <M>{"g \\in \\mathbb{Z}_p^*"}</M>
        </p>
      </div>
      ...
      <div className="not-prose rounded-lg border border-amber-500/20 bg-amber-500/5 p-5 mb-4">
        ...
        <p className="text-sm text-foreground/70 mt-2">
          ...
          <M>{"a"}</M> 또는 <M>{"b"}</M>를 구해야 <M>{"s"}</M>를 계산할 수 있다
          — Discrete Logarithm Problem(이산로그 문제)이므로 어렵다
        </p>
      </div>
      ...
      <div className="not-prose grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
        ...
        <div className="rounded-lg border border-emerald-500/20 bg-emerald-500/5 p-5">
          ...
          <ul className="text-sm text-foreground/80 space-y-1.5 list-disc list-inside">
            ...
            <li>
              Alice pub: <code className="text-xs">x25519(priv, 9)</code>
            </li>
            <li>Bob priv: 32 bytes random</li>
            <li>
              Bob pub: <code className="text-xs">x25519(priv, 9)</code>
            </li>
            <li>
              Shared:{" "}
              <code className="text-xs">x25519(Alice.priv, Bob.pub)</code> ={" "}
              <code className="text-xs">x25519(Bob.priv, Alice.pub)</code>
            </li>
          </ul>
        </div>
      </div>
      ...
      <div className="not-prose grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="rounded-lg border border-foreground/10 bg-muted/30 p-5">
          <p className="font-semibold text-sm text-foreground/60 mb-2">
            실무 적용
          </p>
        ...
        <div className="rounded-lg border border-rose-500/20 bg-rose-500/5 p-5">
          ...
          <p className="text-sm text-foreground/80 mt-2">
            <strong>해결:</strong> 디지털 서명으로 공개키 인증 — TLS 인증서, PGP
            등
          </p>
        </div>
      </div>
