  ...
  return (
    <StepViz steps={STEPS}>
      {(step) => (
        ...
        >
          {/* NAF bit cells */}
          <text x={8} y={18} fontSize={9} fill="var(--muted-foreground)">
            NAF
          </text>
          {NAF.map((b, i) => {
            const x = 30 + i * 26;
          ...
          <text x={345} y={18} fontSize={9} fill="var(--muted-foreground)">
            …
          </text>
          {/* Doubling arrows — every bit */}
          <motion.g animate={{ opacity: step >= 1 ? 1 : 0.1 }} transition={sp}>
            <text x={8} y={42} fontSize={9} fill={DB} fontWeight={600}>
              dbl
            </text>
            {NAF.map((_, i) => {
              const x = 30 + i * 26;
          ...
          <motion.g animate={{ opacity: step >= 2 ? 1 : 0.1 }} transition={sp}>
            <text x={8} y={60} fontSize={9} fill={AD} fontWeight={600}>
              add
            </text>
            {NAF.map((b, i) => {
              if (b === 0) return null;
          ...
          <motion.g animate={{ opacity: step >= 1 ? 0.7 : 0 }} transition={sp}>
            <text x={30} y={74} fontSize={9} fill={DB}>
              Doublings: {NAF.length}+… (매 비트)
            </text>
            <text x={200} y={74} fontSize={9} fill={AD}>
              Additions: {NAF.filter((b) => b !== 0).length}/12 (비영)
            </text>
          </motion.g>
          {/* BN correction */}
          {step === 3 && (
          ...
          <motion.g
            ...
            key={step}
          >
            <AnnotationBox x={360} y={10} w={132} h={70} label={BODIES[step]} color={DB} />
          </motion.g>
        </svg>
      )}
    </StepViz>
