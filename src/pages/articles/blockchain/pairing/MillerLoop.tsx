  ...
  return (
    <section id="miller-loop" className="mb-16 scroll-mt-20">
      ...
      <div className="prose prose-neutral dark:prose-invert max-w-none">
        <p>
          Optimal Ate Pairing은 <strong>Miller Loop</strong>와 Final
          Exponentiation 두 단계로 구성됩니다.
          <br />
          Miller Loop는 루프 파라미터 <code>|6u+2|</code> (약 65비트)의
          NAF(Non-Adjacent Form, 비인접 형식) 표현을 MSB부터 순회합니다.
          <br />매 비트마다 <strong>doubling step</strong>(접선)을 수행합니다.
          <br />
        ...
        <p>
          타원곡선의 점 덧셈에서 두 점을 지나는 직선(접선 또는 할선)의
          방정식입니다.
          <br />
          ...
          D-type twist를 통해{" "}
          <code>
            {"w^3 * l = yP + (-lambda*xP)*w + (lambda*xT - yT)*w*v"}
          </code>{" "}
          형태로 전개됩니다.
          <br />
        ...
        <CodePanel
          title="Line function → Fp12 매핑"
          code={`// line function 결과를 Fp12로 매핑
fn line_eval(lambda: Fp2, xt: Fp2, yt: Fp2, p: &G1Affine) -> Fp12 {
    Fp12::new(
        ...
        <p>
          <code>|6u+2|</code>를 NAF로 표현하면 비영 비트가 약 1/3로 줄어
          addition step 횟수가 감소합니다. 65개 항 중 약 22개만 비영(
          {"{-1, 0, 1}"})이며, 전체 연산량은 약 64 squarings + 88
          multiplications입니다.
        </p>

        <h3 className="text-xl font-semibold mt-6 mb-3">Miller Loop 코드</h3>
        <CodePanel
          title="miller_loop 구현"
    ...
    f
          ...
          annotations={[
            { lines: [1, 4], color: "sky", note: "초기화: f=1, T=Q" },
            {
              lines: [6, 9],
              color: "emerald",
              note: "doubling step (매 비트)",
            },
            {
              lines: [11, 14],
              color: "amber",
              note: "addition step (NAF 비영 비트)",
            },
          ]}
        />
        ...
        <p>
          ...
          <code>{"T <- T + pi(Q)"}</code>, <code>{"T <- T + (-pi^2(Q))"}</code>{" "}
          두 addition step을 추가하여 Tate 페어링과의 동치성을 보장합니다.
          <br />
          Frobenius{" "}
          <code>{"pi(x,y) = (conj(x)*gamma_11, conj(y)*gamma_21)"}</code>에서
          보정 상수는 <code>{"xi^((p-1)/3)"}</code>과{" "}
          <code>{"xi^((p-1)/2)"}</code>입니다.
        </p>
      </div>
    </section>
  );
