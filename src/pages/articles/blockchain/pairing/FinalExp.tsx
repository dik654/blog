...
export default function FinalExp() {
  return (
    <section id="final-exp" className="mb-16 scroll-mt-20">
      ...
      <div className="not-prose mb-8"><FinalExpViz /></div>
      <div className="prose prose-neutral dark:prose-invert max-w-none">
        ...
        <CodePanel title="지수 분해" code={`(p^12 - 1) / r = (p^6 - 1) * (p^2 + 1) * (p^4 - p^2 + 1)/r
                  ~~~~~~~~~~   ~~~~~~~~~~   ~~~~~~~~~~~~~~~~~~~
                  Easy Part 1  Easy Part 2      Hard Part`} defaultOpen annotations={[
          { lines: [1, 1], color: 'sky', note: '세 인자로 분해' },
          { lines: [2, 3], color: 'emerald', note: 'Easy part는 저비용, Hard part가 핵심' },
        ]} />

        <h3 className="text-xl font-semibold mt-6 mb-3">Easy Part 1: f^(p^6 - 1)</h3>
        <p>
          <code>{"f^(p^6) = conjugate(f)"}</code>가 성립합니다.<br />
          Fp12 = Fp6[w]/(w^2-v)에서 <code>{"w^(p^6) = -w"}</code>이므로
          (c0 + c1*w)의 p^6승은 c0 - c1*w, 즉 conjugate입니다.<br />
          연산: conjugate 1회 + inv 1회 + mul 1회.
        </p>

        <h3 className="text-xl font-semibold mt-6 mb-3">Easy Part 2: result^(p^2 + 1)</h3>
        <p>
          <code>{"result^(p^2+1) = frobenius2(result) * result"}</code>로 계산합니다.<br />
          Fp12 Frobenius^2에서는 Fp2 원소의 p^2승이 항등이므로,
          v와 w 계수에 보정 상수(<code>{"xi^((p^2-1)/3)"}</code> 등)만 곱하면 됩니다.<br />
          이 상수들은 모두 실수(허수부 = 0)입니다.
        </p>

        <h3 className="text-xl font-semibold mt-6 mb-3">Hard Part: (p^4 - p^2 + 1)/r</h3>
        <p>
          약 761비트 지수를 square-and-multiply로 계산합니다.<br />
          프로덕션에서는 BN 파라미터 u를 사용한 addition chain으로 최적화하지만,
          교육용 구현에서는 지수를 12개 u64 limb로 하드코딩하여 naive pow를 사용합니다.
        </p>
        <CodePanel title="final_exponentiation 구현" code={`fn final_exponentiation(f: Fp12) -> Fp12 {
    ...
    r2.pow(&hard_exp)
          ...
          { lines: [2, 4], color: 'sky', note: 'Easy Part 1: conjugate + inv' },
          { lines: [6, 7], color: 'emerald', note: 'Easy Part 2: frobenius + mul' },
          { lines: [9, 18], color: 'amber', note: 'Hard Part: 761비트 지수승' },
        ]} />
