  ...
  return (
    <section id="math-foundation" className="mb-16 scroll-mt-20">
      ...
      <div className="prose prose-neutral dark:prose-invert max-w-none">
        ...
        <p>
          유리함수 f의 인수(divisor) <code>{"div(f) = sum(ni * (Pi))"}</code>는
          영점(ni &gt; 0)과 극점(ni &lt; 0)을 형식적 합으로 기록합니다.
          Miller 함수 f_n,Q는 <code>{"div(f_{n,Q}) = n(Q) - ([n]Q) - (n-1)(O)"}</code>인 유리함수로,
        ...
        <pre className="bg-accent rounded-lg p-4 overflow-x-auto text-sm"><code>{`Doubling:  f_{2i,Q} = f_{i,Q}^2 * (l_{[i]Q,[i]Q} / v_{[2i]Q})
Addition:  f_{i+1,Q} = f_{i,Q} * (l_{[i]Q,Q} / v_{[i+1]Q})

수직선 v는 별도로 계산하지 않습니다:
  v의 평가값은 Fp* 에 속하고, Fp* ⊂ (Fp12*)^r 이므로
  final exponentiation f^((p^12-1)/r) 에서 자동으로 소거됩니다.`}</code></pre>

        <h3 className="text-xl font-semibold mt-6 mb-3">Twist Homomorphism</h3>
        <p>
          G2는 원래 E(Fp12) 위에 정의되지만, sextic twist를 통해 E&#39;(Fp2)로 축소합니다.
        ...
        <pre className="bg-accent rounded-lg p-4 overflow-x-auto text-sm"><code>{`E:  Y^2 = X^3 + b       (원래 커브, b = 3)
E': Y'^2 = X'^3 + b/xi  (트위스트 커브)

psi: E'(Fp2) -> E(Fp12)
  좌표 크기: Fp2 x 2 = Fp x 4  (Fp12 x 2 = Fp x 24 대비 6배 절감)`}</code></pre>
        ...
        <pre className="bg-accent rounded-lg p-4 overflow-x-auto text-sm"><code>{`u^2 = -1  -> u^p = -u                    (conjugate)
...
G2 Frobenius:
  ...
  pi^2(x,y) = (x*xi^((p^2-1)/3), -y)   // gamma_22 = -1`}</code></pre>
        <p>
