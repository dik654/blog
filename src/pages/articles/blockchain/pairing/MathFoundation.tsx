    ...
    <section id="math-foundation" className="mb-16 scroll-mt-20">
      ...
      <div className="prose prose-neutral dark:prose-invert max-w-none">
        <h3 className="text-xl font-semibold mt-6 mb-3">
          Divisors and Miller Function
        </h3>
        <p>
          유리함수 f의 인수(divisor, 영점과 극점의 형식적 합){" "}
          <code>{"div(f) = sum(ni * (Pi))"}</code>를 정의합니다.
          <br />
          영점(ni &gt; 0)과 극점(ni &lt; 0)을 기록합니다.
          <br />
          Miller 함수 f_n,Q는{" "}
          <code>{"div(f_{n,Q}) = n(Q) - ([n]Q) - (n-1)(O)"}</code>인
          유리함수입니다.
          <br />
        ...
        <CodePanel
          title="Miller 재귀 공식"
          code={`Doubling:  f_{2i,Q} = f_{i,Q}^2 * (l_{[i]Q,[i]Q} / v_{[2i]Q})
Addition:  f_{i+1,Q} = f_{i,Q} * (l_{[i]Q,Q} / v_{[i+1]Q})

수직선 v는 별도로 계산하지 않습니다:
  v의 평가값은 Fp* 에 속하고, Fp* ⊂ (Fp12*)^r 이므로
  final exponentiation f^((p^12-1)/r) 에서 자동으로 소거됩니다.`}
          defaultOpen
          annotations={[
        ...
        <p>
          G2는 원래 E(Fp12) 위에 정의되지만, sextic twist를 통해 E&#39;(Fp2)로
          축소합니다.
          <br />
          D-type twist: <code>{"psi(X', Y') = (X'*v, Y'*w^3)"}</code> where{" "}
          <code>{"w^6 = xi = 9+u"}</code>. 타워의 non-residue xi와 twist
          파라미터가 동일하여 하나의 상수가 두 역할을 겸합니다.
        </p>
        <CodePanel
          title="Twist 커브와 사상"
          code={`E:  Y^2 = X^3 + b       (원래 커브, b = 3)
E': Y'^2 = X'^3 + b/xi  (트위스트 커브)

psi: E'(Fp2) -> E(Fp12)
  좌표 크기: Fp2 x 2 = Fp x 4  (Fp12 x 2 = Fp x 24 대비 6배 절감)`}
          defaultOpen
          annotations={[
            { lines: [1, 2], color: "sky", note: "원래 커브 vs 트위스트 커브" },
            {
              lines: [4, 5],
              color: "emerald",
              note: "Fp2 → Fp12 사상: 6배 절감",
            },
          ]}
        />

        <h3 className="text-xl font-semibold mt-6 mb-3">
          Frobenius Correction Constants
        </h3>
        <p>
          모든 보정 상수는 <code>{"alpha^n = c"}</code>이면{" "}
          <code>{"alpha^(p^k) = alpha * c^((p^k-1)/n)"}</code>인 원리에서
          파생됩니다.
        </p>
        <CodePanel
          title="Frobenius 보정 상수"
          code={`u^2 = -1  -> u^p = -u                    (conjugate)
...
G2 Frobenius:
  ...
  pi^2(x,y) = (x*xi^((p^2-1)/3), -y)   // gamma_22 = -1`}
          annotations={[
        ...
        />
        <p>
          <code>{"xi^((p^2-1)/2) = -1"}</code>인 이유: 만약 1이라면 xi가 Fp2에서
          QR이 되어 6차 확장이 불가능하므로 모순입니다. Frobenius^2의 보정
          상수가 모두 실수인 이유는 p^2승이 Fp2 위에서 항등이고, p ≡ 1 (mod
          6)이기 때문입니다.
        </p>
      </div>
    </section>
