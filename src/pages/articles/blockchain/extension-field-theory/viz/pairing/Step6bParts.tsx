...
export function FUpdateSection() {
  ...
  return (
    <g>
      <motion.g {...fade(2.8)}>
        <text x={20} y={164} fontSize={12} fontWeight={600} fill={C.gt}>
          ③ f ← f² · ℓ(P)
        </text>
      </motion.g>

      {/* f before */}
      <motion.text x={20} y={186} fontSize={10} fill={C.m} {...fade(3.0)}>
        f =
      </motion.text>
      {fSlots.map((_, i) => (
        <motion.rect
      ...
      <motion.g {...fade(3.8)}>
        ...
        <text x={224} y={210} fontSize={10} fill={C.ml}>
          × ℓ(P)
        </text>
      </motion.g>

      {/* Result f' */}
      <motion.text x={20} y={236} fontSize={10} fill={C.gt} {...fade(4.0)}>
        f' =
      </motion.text>
      {fSlots.map((_, i) => (
        <motion.rect
