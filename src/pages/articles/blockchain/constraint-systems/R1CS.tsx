  ...
  return (
    <section id="r1cs" className="mb-16 scroll-mt-20">
      ...
      <div className="prose prose-neutral dark:prose-invert max-w-none">
        ...
        <p>
          ZK 증명(Zero-Knowledge Proof) — 비밀 x를 공개하지 않고 f(x) = y임을
          증명하는 것이 목표
          <br />
          R1CS — 임의의 계산을 증명 시스템이 이해하는 형태로 번역하는 중간
          표현(IR)
        </p>
        ...
        <p>
          체(Field) F 위의 R1CS 인스턴스 — 행렬 A, B, C와 witness(비밀 입력)
          벡터 s로 구성
          <br />각 제약은 곱셈 게이트 하나를 표현.{" "}
          <strong>덧셈은 무료(선형결합)</strong>, 곱셈만 비용 발생
        </p>
        ...
        <div className="not-prose space-y-3 mb-4">
          <div className="rounded-lg border-l-4 border-l-sky-500 bg-card p-4">
            ...
            <M display>
              {
                "\\langle \\mathbf{a}, \\mathbf{s} \\rangle \\cdot \\langle \\mathbf{b}, \\mathbf{s} \\rangle = \\langle \\mathbf{c}, \\mathbf{s} \\rangle"
              }
            </M>
          </div>
          <div className="rounded-lg border-l-4 border-l-emerald-500 bg-card p-4">
            ...
            <M display>
              {
                "(A \\cdot \\mathbf{s}) \\circ (B \\cdot \\mathbf{s}) = C \\cdot \\mathbf{s}"
              }
            </M>
            <ul className="text-sm text-muted-foreground space-y-1 mt-2">
              <li>
                <M>{"A, B, C \\in \\mathbb{F}^{m \\times n}"}</M> &mdash;{" "}
                <code>m</code> = 제약 수, <code>n</code> = 변수 수
              </li>
              <li>
                <M>
                  {
                    "\\mathbf{s} = (1,\\, x_1, \\ldots, x_l,\\, w_1, \\ldots, w_m)"
                  }
                </M>{" "}
                &mdash; witness 벡터
              </li>
              <li>
                <M>{"\\circ"}</M> = Hadamard product (원소별 곱)
              </li>
            </ul>
          </div>
        ...
        <p>
          회로 빌드 인터페이스 — 변수 할당 &rarr; 제약 추가 &rarr; 만족 여부
          확인
        </p>
        ...
        <div className="not-prose space-y-3 mb-4">
          <div className="rounded-lg border-l-4 border-l-sky-500 bg-card p-4">
            ...
            <ul className="text-sm text-muted-foreground space-y-1">
              <li>
                <code>x = cs.alloc_witness(Fr::from_u64(3))</code> &mdash;
                비공개
              </li>
              <li>
                <code>y = cs.alloc_witness(Fr::from_u64(4))</code> &mdash;
                비공개
              </li>
              <li>
                <code>z = cs.alloc_instance(Fr::from_u64(12))</code> &mdash;
                공개
              </li>
            </ul>
          </div>
          <div className="rounded-lg border-l-4 border-l-emerald-500 bg-card p-4">
            ...
            <p className="text-sm text-muted-foreground">
              <code>cs.enforce(A=x, B=y, C=z)</code> &mdash;{" "}
              <M>{"x \\cdot y = z"}</M>
            </p>
          </div>
          <div className="rounded-lg border-l-4 border-l-violet-500 bg-card p-4">
            ...
            <p className="text-sm text-muted-foreground">
              <code>cs.is_satisfied()</code> &rarr; <M>{"3 \\times 4 = 12"}</M>{" "}
              &check;
            </p>
          </div>
        </div>
        ...
        <p>
          Witness 생성 — 함수를 실제로 실행하는 것과 동일
          <br />
          증명자(Prover) — f(x) 계산으로 전체 witness 벡터 채움
          <br />
          검증자(Verifier) — 계산 없이 증명만으로 f(x)=y 확인
        </p>
        ...
        <div className="not-prose space-y-3 mb-4">
          <div className="rounded-lg border border-border/60 p-4">
            <p className="font-semibold text-sm mb-3">
              Witness 벡터 구성: <M>{"f(x) = x^3 + x + 5"}</M>, 입력{" "}
              <M>{"x = 3"}</M>
            </p>
        ...
        <ul className="list-disc pl-6 space-y-1">
          <li>
            <strong>덧셈 a + b</strong>: 0개 (무료 — 선형결합으로 흡수)
          </li>
          <li>
            <strong>곱셈 a * b</strong>: 1개
          </li>
          <li>
            <strong>거듭제곱 a^n</strong>: ~log₂(n)개 (square-and-multiply)
          </li>
          <li>
            <strong>비트 분해</strong>: ~254개 (각 비트에 boolean 제약)
          </li>
          <li>
            <strong>Poseidon hash</strong>: ~250개 (ZK-friendly)
          </li>
          <li>
            <strong>SHA-256</strong>: ~25,000개 (비트 연산 기반 &rarr; 비효율)
          </li>
        </ul>
      </div>
      ...
      <div className="prose prose-neutral dark:prose-invert max-w-none mt-6">
        ...
        <div className="not-prose space-y-4 mb-6">
          <div className="rounded-lg border border-border/60 p-4">
            ...
            <p className="text-sm text-muted-foreground">
              변수: <code>[1, out, x, sym_1, sym_2, sym_3]</code> ={" "}
              <code>[1, 35, 3, 9, 27, 30]</code>
            </p>
          </div>
          ...
          <div className="rounded-lg border border-border/60 p-4">
            <p className="font-semibold text-sm mb-3">
              4개 곱셈 게이트 &rarr; 4개 제약
            </p>
            <div className="space-y-3">
              {[
                {
                  n: 1,
                  desc: "x \\cdot x = \\text{sym\\_1}",
                  a: "[0,0,1,0,0,0]",
                  b: "[0,0,1,0,0,0]",
                  c: "[0,0,0,1,0,0]",
                  note: "x 선택",
                ...
                {
                  n: 2,
                  desc: "\\text{sym\\_1} \\cdot x = \\text{sym\\_2}",
                  a: "[0,0,0,1,0,0]",
                  b: "[0,0,1,0,0,0]",
                  c: "[0,0,0,0,1,0]",
                  note: "",
                ...
                {
                  n: 3,
                  desc: "(\\text{sym\\_2} + x) \\cdot 1 = \\text{sym\\_3}",
                  a: "[0,0,1,0,1,0]",
                  b: "[1,0,0,0,0,0]",
                  c: "[0,0,0,0,0,1]",
                  note: "sym_2 + x 선형결합",
                ...
                {
                  n: 4,
                  desc: "(\\text{sym\\_3} + 5) \\cdot 1 = \\text{out}",
                  a: "[5,0,0,0,0,1]",
                  b: "[1,0,0,0,0,0]",
                  c: "[0,1,0,0,0,0]",
                  note: "+5를 A에 흡수",
        ...
        <div className="not-prose space-y-3 mb-6">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            ...
            <div className="rounded-lg border border-emerald-500/30 p-4">
              <p className="font-semibold text-sm text-emerald-400 mb-2">
                AIR (Algebraic Intermediate Repr.)
              </p>
        ...
        <div className="not-prose space-y-3 mb-6">
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
            {[
              {
                name: "Groth16",
                desc: "가장 작은 증명 (3 group elements), circuit-specific trusted setup",
                ok: true,
              },
              {
                name: "Bulletproofs",
                desc: "trusted setup 불필요, O(log n) 증명 (Monero)",
                ok: true,
              },
              { name: "Spartan", desc: "Sumcheck 기반, FFT 불필요", ok: true },
              {
                name: "Modern zkVMs",
                desc: "Plonkish + custom gates + lookup 선호",
                ok: false,
              },
            ].map((s) => (
              <div
                key={s.name}
                className={`rounded-lg border p-3 ${s.ok ? "border-green-500/30" : "border-red-500/30"}`}
              >
        ...
        <div className="not-prose grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 mb-6">
          {[
            {
              name: "bellman (Rust)",
              desc: "R1CS + Groth16. Zcash 오리지널",
              org: "Zcash",
            },
            {
              name: "arkworks (Rust)",
              desc: "범용 R1CS traits. 다중 증명 시스템",
              org: "arkworks",
            },
            {
              name: "circom (DSL)",
              desc: "R1CS-first DSL. 제약과 1:1 대응",
              org: "iden3, Tornado Cash",
            },
            {
              name: "circomlib",
              desc: "표준 가젯 라이브러리. Poseidon, Pedersen, ECDSA",
              org: "iden3",
            },
            {
              name: "gnark (Go)",
              desc: "R1CS frontend. Groth16 + Plonk backend",
              org: "Consensys",
            },
          ].map((f) => (
        ...
        <h3 className="text-xl font-semibold mt-6 mb-3">
          R1CS 만족 가능성 (NP-complete)
        </h3>
        <div className="not-prose rounded-lg border border-border/60 p-4 mb-4">
          <p className="text-sm text-muted-foreground mb-2">
            공개된 행렬 <M>{"A, B, C"}</M>에 대해 witness <M>{"\\mathbf{s}"}</M>
            를 찾는 문제
          </p>
          <M display>
            {
          ...
          <p className="text-sm text-muted-foreground mt-2">
            <M>{"\\circ"}</M> = Hadamard product. Circuit-SAT를 포함 &rarr;
            NP-complete
          </p>
        </div>
      </div>
