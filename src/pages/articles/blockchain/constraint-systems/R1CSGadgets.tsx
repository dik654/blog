  ...
  return (
    <section id="r1cs-gadgets" className="mb-16 scroll-mt-20">
      ...
      <div className="prose prose-neutral dark:prose-invert max-w-none">
        ...
        <p>
          가젯(Gadget)은 반복적으로 사용되는 R1CS 패턴입니다.
          <br />
          복잡한 회로를 기본 가젯의 조합으로 구성합니다.
          <br />
          native 코드와 circuit 코드의 출력이 동일해야 합니다(native = circuit
          원칙).
        </p>

        <h3 className="text-xl font-semibold mt-6 mb-3">
          S-box 가젯: x &rarr; x⁵ (3개 제약)
        </h3>
        <p>
          Poseidon 해시의 핵심 비선형 연산입니다.
          <br />
          square-and-multiply(제곱-곱셈 방식)로 3개 제약만 사용합니다.
        </p>
        ...
        <div className="not-prose space-y-3 mb-4">
          ...
          <div className="rounded-lg border-l-4 border-l-emerald-500 bg-card p-4">
            ...
            <ul className="text-sm text-muted-foreground space-y-1">
              <li>
                <code>x2 = cs.alloc_witness(x_val.square())</code>
              </li>
              <li>
                <code>x4 = cs.alloc_witness(x2_val.square())</code>
              </li>
              <li>
                <code>x5 = cs.alloc_witness(x4_val * x_val)</code>
              </li>
              <li className="pt-1">
                <code>cs.enforce(x, x, x2)</code> &mdash;{" "}
                <M>{"x \\times x = x^2"}</M>
              </li>
              <li>
                <code>cs.enforce(x2, x2, x4)</code> &mdash;{" "}
                <M>{"x^2 \\times x^2 = x^4"}</M>
              </li>
              <li>
                <code>cs.enforce(x4, x, x5)</code> &mdash;{" "}
                <M>{"x^4 \\times x = x^5"}</M>
              </li>
            </ul>
          </div>
        ...
        <h3 className="text-xl font-semibold mt-6 mb-3">
          Boolean 가젯: b &isin; &#123;0,1&#125; (1개 제약)
        </h3>
        <p>
          변수 b가 반드시 0 또는 1임을 강제합니다.
          <br />이 제약 없이는 공격자가 b=2 같은 값으로 mux 결과를 조작할 수
          있습니다.
        </p>
        ...
        <div className="not-prose space-y-3 mb-4">
          <div className="rounded-lg border-l-4 border-l-violet-500 bg-card p-4">
            ...
            <p className="text-sm text-muted-foreground mt-2">
              A = <code>[(b, 1)]</code>, &ensp; B ={" "}
              <code>[(One, 1), (b, -1)]</code>, &ensp; C = <code>0</code>
            </p>
          </div>
        ...
        <p>
          bit 값에 따라 두 입력 중 하나를 선택합니다.
          <br />
          result = when_false + bit * (when_true - when_false) 공식을
          사용합니다.
        </p>
          ...
          <div className="rounded-lg border-l-4 border-l-sky-500 bg-card p-4">
            ...
            <ul className="text-sm text-muted-foreground space-y-1">
              <li>
                ...
                <M>
                  {
                    "t = \\text{bit} \\cdot (\\text{when\\_true} - \\text{when\\_false})"
                  }
                </M>
              </li>
              <li>
                ...
                <M>
                  {
                    "\\text{bit} \\cdot (\\text{when\\_true} - \\text{when\\_false}) = t"
                  }
                </M>
              </li>
              <li>
                제약 2:{" "}
                <M>{"(\\text{when\\_false} + t) \\cdot 1 = \\text{result}"}</M>
              </li>
            </ul>
          </div>
          <div className="rounded-lg border-l-4 border-l-emerald-500 bg-card p-4">
            ...
            <p className="text-sm text-muted-foreground">
              <code>bit=1</code>, <code>when_true=42</code>,{" "}
              <code>when_false=99</code>
            </p>
            <ul className="text-sm text-muted-foreground space-y-0.5 mt-1">
              <li>
                <M>{"t = 1 \\cdot (42 - 99) = -57"}</M>
              </li>
              <li>
                <M>{"\\text{result} = 99 + (-57) = 42"}</M> &larr; when_true
                선택 &check;
              </li>
            </ul>
          </div>
        ...
        <p>
          Poseidon의 65라운드(Full 8 + Partial 57)를 R1CS로 변환합니다.
          <br />
          AddRC와 MDS(Maximum Distance Separable 행렬)는 선형 연산이지만,
          선형결합을 변수로 고정하는 추가 제약이 필요합니다.
        </p>

        {/* Poseidon 제약 수 분석 */}
        <div className="not-prose rounded-lg border border-border/60 p-4 mb-4">
          <p className="font-semibold text-sm mb-3">
            Poseidon 제약 수 분석 (T=3)
          </p>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-sm">
            <div className="rounded border bg-card p-3">
              <p className="font-semibold">Full round</p>
              <p className="text-muted-foreground">
                AddRC 3 + S-box 9 + MDS 3 = <strong>15 제약</strong>
              </p>
              <p className="text-xs text-muted-foreground mt-1">
                8 라운드 &times; 15 = 120
              </p>
            ...
            <div className="rounded border bg-card p-3">
              ...
              <p className="text-muted-foreground">
                AddRC 3 + S-box 3 + MDS 3 = <strong>9 제약</strong>
              </p>
              <p className="text-xs text-muted-foreground mt-1">
                57 라운드 &times; 9 = 513
              </p>
        ...
        <p>
          Merkle proof 검증은 여러 가젯의 조합입니다.
          <br />
          각 레벨마다 Boolean(경로 방향) + Mux(입력 순서 결정) +
          Poseidon(해시)을 반복합니다.
          <br />
          최종 해시가 공개 root와 일치하는지 확인합니다.
        </p>
        ...
        <div className="not-prose space-y-3 mb-4">
          <div className="rounded-lg border-l-4 border-l-sky-500 bg-card p-4">
            ...
            <p className="text-sm text-muted-foreground mb-2">
              각 레벨: Boolean(1) + Mux&times;2(4) + Poseidon(634) &asymp;{" "}
              <strong>639 제약</strong>
            </p>
            <div className="grid grid-cols-3 gap-2 text-sm text-center">
              <div className="rounded border bg-muted/50 p-2">
        ...
        <div className="not-prose space-y-4 mb-6">
          <div className="rounded-lg border border-border/60 p-4">
            <p className="font-semibold text-sm mb-2">
              1. Range check: <M>{"0 \\leq x < 2^n"}</M>
            </p>
            <p className="text-sm text-muted-foreground mb-2">
              비트 분해: <M>{"x = \\sum_{i=0}^{n-1} b_i \\cdot 2^i"}</M>, 각{" "}
              <M>{"b_i"}</M>에 boolean 제약
            </p>
            <div className="grid grid-cols-2 gap-2 text-sm">
              <div className="rounded border bg-card p-2">
                <p className="text-muted-foreground">
                  비용: <strong>n + 1</strong> 제약 (n booleans + 1 combine)
                </p>
                <p className="text-xs text-muted-foreground mt-1">
                  예: n=64 &rarr; 65 제약
                </p>
              ...
              <div className="rounded border bg-card p-2">
                <p className="text-muted-foreground">
                  최적화: 4-bit 청크 lookup table (Plonkish)
                </p>
                <p className="text-xs text-muted-foreground mt-1">
                  &rarr; O(n/4) 제약
                </p>
          ...
          <div className="rounded-lg border border-border/60 p-4">
            ...
            <div className="grid grid-cols-2 gap-2 text-sm">
              <div className="rounded border bg-card p-2">
                ...
                <p className="text-muted-foreground">
                  <M>{"b - a - 1"}</M>의 range check &mdash; 255 제약
                </p>
              </div>
              <div className="rounded border bg-card p-2">
                ...
                <p className="text-muted-foreground">
                  양쪽 bit 분해 후 비트별 비교 &mdash; ~512 제약
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="rounded-lg border border-border/60 p-4">
            <p className="font-semibold text-sm mb-2">
              3. 조건부 할당 (if cond then a else b)
            </p>
            <p className="text-sm text-muted-foreground">
              <code>Boolean(cond)</code> +{" "}
              <M>
                {
                  "\\text{result} = \\text{cond} \\cdot a + (1 - \\text{cond}) \\cdot b"
                }
              </M>
            </p>
            <p className="text-sm text-muted-foreground mt-1">
              비용: <strong>2 제약</strong> (boolean + mux)
            </p>
          </div>
          ...
          <div className="rounded-lg border border-border/60 p-4">
            ...
            <p className="text-sm text-muted-foreground mb-2">
              BN254 base field 위의 scalar multiplication (foreign-field
              arithmetic 고비용)
            </p>
            <div className="grid grid-cols-3 gap-2 text-sm text-center">
              <div className="rounded border bg-red-500/10 p-2">
            ...
            <p className="text-xs text-muted-foreground mt-2">
              최적화: Window NAF scalar mult, GLV endomorphism, precomputed
              tables
            </p>
          </div>
              ...
              {[
                {
                  name: "SHA-256",
                  cost: "~25,000",
                  note: "비트 연산 기반, Ethereum/Bitcoin 호환",
                  color: "border-red-500/30",
                },
                {
                  name: "Keccak-256",
                  cost: "~150,000",
                  note: "SHA-3 유사, bit-heavy, Ethereum state",
                  color: "border-red-500/30",
                },
                {
                  name: "Poseidon",
                  cost: "~250",
                  note: "ZK-friendly, MDS + S-box",
                  color: "border-green-500/30",
                },
                {
                  name: "Rescue-Prime",
                  cost: "~300",
                  note: "alternating S-box, provable security",
                  color: "border-green-500/30",
                },
                {
                  name: "MiMC",
                  cost: "~600",
                  note: "초기 ZK-friendly hash (deprecated)",
                  color: "border-amber-500/30",
                },
              ].map((h) => (
                <div
                  key={h.name}
                  className={`rounded-lg border p-2 ${h.color}`}
                >
          ...
          <div className="rounded-lg border border-border/60 p-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-2 text-sm">
              <div className="rounded border bg-card p-2">
                ...
                <p className="text-xs text-muted-foreground">
                  circuit-specific. 변경 시 새 trusted setup 필요
                </p>
              </div>
              <div className="rounded border bg-card p-2">
                <p className="font-semibold">Plonk (universal)</p>
                <p className="text-xs text-muted-foreground">
                  단일 setup, prove time에 회로 기술
                </p>
              </div>
              <div className="rounded border bg-card p-2">
                <p className="font-semibold">Recursive (Nova)</p>
                <p className="text-xs text-muted-foreground">
                  IVC &mdash; 무한 계산에 constant-size proof
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="rounded-lg border border-border/60 p-4">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 text-sm">
              <div className="rounded border bg-card p-2">
                ...
                <p className="text-xs text-muted-foreground">
                  valid witness 생성 &rarr; <code>cs.is_satisfied()</code> =
                  true
                </p>
              </div>
              <div className="rounded border bg-card p-2">
                ...
                <p className="text-xs text-muted-foreground">
                  witness 변조 &rarr; <code>is_satisfied()</code> = false
                </p>
              </div>
              <div className="rounded border bg-card p-2">
                <p className="font-semibold">Constraint count</p>
                <p className="text-xs text-muted-foreground">
                  <code>cs.num_constraints()</code> 기대값 일치
                </p>
              </div>
              <div className="rounded border bg-card p-2">
                <p className="font-semibold">Fuzz test</p>
                <p className="text-xs text-muted-foreground">
                  랜덤 입력, native vs circuit 출력 비교
                </p>
              </div>
            </div>
          </div>
