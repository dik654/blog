...
const STEPS = [
  ...
  {
    label: "곱셈 게이트 분해",
    body: "모든 연산을 곱셈 게이트로 분해. 덧셈은 무료(선형결합).",
  },
  {
    label: "R1CS 생성",
    body: "각 게이트를 (A·s)*(B·s)=C·s 제약으로 표현합니다.",
  },
  {
    label: "Lagrange 보간",
    body: "R1CS 행렬의 각 열을 도메인 D 위에서 다항식으로 보간.",
  },
  {
    label: "QAP 결합",
    body: "witness 벡터로 다항식 결합: a(x)b(x)-c(x) = h(x)t(x).",
  },
  {
    label: "h(x) 검증",
    body: "a(x)b(x)-c(x)가 t(x)로 나누어떨어지면 R1CS 만족. O(1) 검증.",
  },
];
              ...
              <g key={n.label}>
                ...
                {i > 0 && (
                  <motion.line
                    ...
                    strokeWidth={0.8}
                    markerEnd="url(#arr)"
                    animate={{ opacity: active ? 0.6 : 0.08 }}
                    transition={sp}
                  />
                )}
              </g>
