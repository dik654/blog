...
const STEPS = [
  {
    label: "Cairo 컴파일 파이프라인 전체 구조",
    body: "Cairo 소스 코드가 6단계를 거쳐 실행 가능한 CASM으로 변환됩니다.",
  },
  {
    label: "파싱 & 의미 분석",
    body: "Parser가 AST를 생성하고, Semantic 분석이 타입 체크와 스코프 해석을 수행합니다.",
  },
  {
    label: "Lowering → Sierra IR",
    body: "고수준 Cairo 구문을 Sierra 중간표현으로 변환합니다. 안전성이 구조적으로 보장됩니다.",
  },
  {
    label: "Sierra → CASM",
    body: "Sierra 명령어를 Cairo VM에서 실행 가능한 어셈블리(CASM)로 최종 변환합니다.",
  },
];
        ...
        >
          {EDGES.map((e, i) => {
            const f = mid(e.from),
              t = mid(e.to);
            const show =
              ACTIVE[step].includes(e.from) && ACTIVE[step].includes(e.to);
            return (
              <motion.line
                key={i}
                x1={f.x}
