  ...
  return (
    <section id="overview" className="mb-16 scroll-mt-20">
      ...
      <div className="prose prose-neutral max-w-none dark:prose-invert">
        ...
        <p>
          Height는 지금 결정할 block 위치, round는 같은 height에서 proposer와 기다림을 바꾸는 재시도 번호, step은 현재 만들어야 할 evidence
          종류입니다. Event handler를 볼 때는 “무슨 함수인가”보다 좌표를 봅니다. 입력 event가 현재 state보다 과거·현재·미래 중 어디인지, 어떤 validation
          뒤 어느 좌표로 이동하는지, durable WAL과 outgoing vote가 어느 transition에 연결되는지를 확인합니다.
        </p>
        <h3>Safety와 liveness의 owner가 다릅니다</h3>
        <p>
          Signed quorum과 lock rule은 conflicting commit을 막아 safety를 지킵니다. Timeout과 proposer rotation은 network가
          다시 bounded delay에 들어온 뒤 progress할 기회를 만드는 liveness 장치입니다. Timeout이 길다는 이유만으로 safety가 강해지거나 timeout을
          짧게 줄였다는 이유만으로 처리량이 항상 좋아지지는 않습니다.
        </p>
      </div>
      <div id="paper-cometbft-consensus-v040" className="not-prose my-8 scroll-mt-24 border-l border-primary/50 pl-4">
        ...
        <p className="mt-2 text-sm leading-6 text-muted-foreground">
            풀어야 할 문제는 이렇습니다. Byzantine validator와 delayed message가 있는 환경에서 proposal·prevote·precommit·round
            change로 한 block을 결정하는 것입니다. 규격의 proof는 validator power·authentication·partial-synchrony 전제 안에서만
            성립하며 application execution이나 고정 latency는 보장 범위 밖입니다.
          </p>
