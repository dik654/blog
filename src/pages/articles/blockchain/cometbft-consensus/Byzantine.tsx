...
{`// Tendermint BFT가 방어하는 비잔틴 공격:

// 1. Double Signing (이중 서명)
//    - 같은 Height+Round+Type에 2개 다른 Vote
//    - 가장 흔한 slashable offense
//    - Evidence: DuplicateVoteEvidence

// 2. Equivocation (에퀴보케이션)
//    - Proposer가 서로 다른 proposal 방송
//    - 여러 peer에게 다른 view
//    - Evidence: ConflictingVotes

// 3. Amnesia Attack (기억상실)
//    - 이전 Lock 무시하고 새 block에 Prevote
//    - BFT safety 위반 시도
//    - 감지: 과거 Lock 기록과 현재 Prevote 비교

// 4. Light Client Attack
//    - light client에 falsified state 전달
//    - commit이 없는 invalid block
//    - Evidence: LightClientAttackEvidence

// 5. Long Range Attack
//    - 이미 unbonded validator의 옛 키로 재서명
//    - 과거 시점에 대체 체인 제시
//    - 방어: weak subjectivity period

// 6. DDoS / Censorship
//    - validator 오프라인 유지 (selection 실패)
//    - TX 검열
//    - 직접 slashing 불가 (liveness-only)

// 탐지 지점:
// - tryAddVote(): double signing 즉시 감지
// - light client: attack evidence 생성
// - verification: block validation 시 체크

// Slashing 결과:
// - Stake 5% slash (double signing)
// - Validator tombstone (영구 제외)
// - Delegators 동반 slash`}
        </pre>
