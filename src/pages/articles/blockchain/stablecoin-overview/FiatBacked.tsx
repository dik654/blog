  ...
  return (
    <section id="fiat-backed" className="mb-16 scroll-mt-20">
      ...
      <div className="prose prose-neutral dark:prose-invert max-w-none">
        ...
        <IssuerStructureViz />
        <p>
          <strong>1:1 백업 원칙</strong>: 발행된 토큰마다 동일 달러 보유
          <br />
          온체인 토큰은 <strong>IOU</strong> (I Owe You) — issuer가 USD 지급
          약속
          <br />
        ...
        <div className="bg-muted/50 border border-border rounded-lg p-5 my-4">
          ...
          <div className="space-y-3 text-sm">
            <div className="bg-background rounded px-3 py-2">
              ...
              <p>
                <code className="text-xs">owner</code> — 발행 권한 보유 주소
              </p>
              <p>
                <code className="text-xs">blacklisted</code> — 주소별 동결 매핑
              </p>
              <p>
                <code className="text-xs">paused</code> — 전체 전송 중단 플래그
              </p>
            </div>
            <div className="bg-background rounded px-3 py-2">
              <p className="font-semibold text-xs text-muted-foreground mb-1">
                <code className="text-xs">mint(to, amount)</code> — 발행
              </p>
              <p>
                Issuer가 USD 입금 확인 후 호출,{" "}
                <code className="text-xs">onlyOwner</code> 제한
              </p>
            </div>
            <div className="bg-background rounded px-3 py-2">
              <p className="font-semibold text-xs text-muted-foreground mb-1">
                <code className="text-xs">burn(amount)</code> — 환급
              </p>
              <p>사용자가 호출 → issuer가 USD 지급</p>
            </div>
            <div className="bg-background rounded px-3 py-2">
              <p className="font-semibold text-xs text-muted-foreground mb-1">
                <code className="text-xs">blacklist(account)</code> — 동결
              </p>
              <p>해당 주소의 토큰 전송 차단 (OFAC 제재 준수)</p>
            </div>
            <div className="bg-background rounded px-3 py-2">
              <p className="font-semibold text-xs text-muted-foreground mb-1">
                <code className="text-xs">transfer(to, amount)</code> — 전송
              </p>
              <p>
                <code className="text-xs">blacklisted</code> 확인 +{" "}
                <code className="text-xs">paused</code> 확인 후 전송
              </p>
            </div>
          </div>
        </div>
        <p>
          <strong>중앙 권한 3가지</strong>: mint, blacklist, pause
          <br />각 기능은 issuer가 <strong>규제 준수를 위해 필요</strong>
          <br />
          사용자 입장에서는 <strong>검열 위험</strong> — USDC 동결 사례 존재
          (Tornado Cash)
        </p>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 my-4">
          <div className="bg-muted/50 border border-border rounded-lg p-4">
            ...
            <ul className="text-sm space-y-1 list-disc list-inside">
              <li>
                <strong>고유동성</strong> — 빠른 환급 가능
              </li>
              <li>
                <strong>저위험</strong> — 미국 국채 중심
              </li>
              <li>
                <strong>수익 발생</strong> — Circle 수익원
              </li>
            </ul>
          </div>
          <div className="bg-muted/50 border border-border rounded-lg p-4">
            ...
            <ul className="text-sm space-y-1 list-disc list-inside">
              <li>
                <strong>은행 파산</strong> — 2023 SVB 사태: USDC 일시 디페그
              </li>
              <li>
                <strong>국채 금리 변동</strong>
              </li>
              <li>
                <strong>감사인 신뢰도</strong>
              </li>
            </ul>
          </div>
        </div>
        <p>
          <strong>SVB 사태(2023년 3월)</strong>: Circle이 SVB에 $3.3B 예치
          <br />
          은행 파산 뉴스 → USDC 일시 $0.88까지 하락
          <br />
          미국 정부 FDIC 보증 확장 후 페그 회복 →{" "}
          <strong>은행 리스크 현실화</strong>
        </p>
        ...
        <div className="bg-muted/50 border border-border rounded-lg p-5 my-4">
          ...
          <p className="text-sm mb-3">
            각 체인의 USDC는 별도 컨트랙트 — 발행량 합계 = Circle 준비금과 일치
          </p>
          ...
          <div className="mt-3 bg-background rounded px-3 py-2 text-sm">
            ...
            <p>
              Native USDC 간 직접 전환 — <code className="text-xs">burn</code>{" "}
              on chain A → <code className="text-xs">mint</code> on chain B
            </p>
          </div>
        </div>
        ...
        <div className="bg-amber-50 dark:bg-amber-950/30 border-l-4 border-amber-400 p-4 my-6 rounded-r-lg">
          <p className="font-semibold mb-2">
            인사이트: Fiat-backed의 근본적 한계
          </p>
          <p>
            <strong>Fiat-backed 스테이블코인 = 크립토가 아님</strong>
            <br />
          ...
          <p className="mt-2">
            ...
            ✗ 은행 리스크 (SVB)
            <br />
            ✗ 규제 변경 리스크 (MiCA, 미국 스테이블코인 법)
            <br />✗ 크립토 철학에 반 (중앙화 수용)
          </p>
          <p className="mt-2">
            <strong>현실</strong>: "DeFi의 기축통화"로 쓰이지만{" "}
            <strong>진정한 crypto primitives 아님</strong>
            <br />
