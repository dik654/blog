...
export function LogBar({ step }: { step: number }) {
  ...
  return (
    <g>
      <text x={10} y={18} fontSize={10} fill={C.log} fontWeight={600}>
        Operations Log (MMR)
      </text>
      {ops.map((op, i) => {
        const x = 10 + i * 88;
  ...
  return (
    <g>
      <rect
        ...
        strokeWidth={1}
      />
      <text x={20} y={90} fontSize={10} fill={C.snap} fontWeight={600}>
        snapshot (HashMap)
      </text>
    ...
    <motion.g initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={sp}>
      ...
      <defs>
        <marker
          ...
          orient="auto"
        >
          <path d="M0,0 L5,2.5 L0,5" fill={C.snap} />
        </marker>
      </defs>
    </motion.g>
  );
