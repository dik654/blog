...
function SsdStep() {
  return (
    <motion.g initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={sp}>
      ...
      <text x={350} y={50} fontSize={10} fill="var(--muted-foreground)">
        SSD 쓰기 증폭 최소화
      </text>
      <text x={350} y={65} fontSize={10} fill="var(--muted-foreground)">
        GC 불필요 (비활성 마킹)
      </text>
    </motion.g>
  );
  ...
  return (
    <motion.g initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={sp}>
      ...
      <text
        ...
        fill="var(--muted-foreground)"
      >
        모든 프리미티브(MMR, QMDB, Archive)가 구현
      </text>
      <defs>
        <marker
          id="ov-arw"
