  ...
  return (
    <g>
      ...
      {lx.map((x, i) => (
        <g key={i}>
          <Nd
            x={x}
            y={leafY}
            fill={hi && i === 0 ? `${C.proof}22` : `${C.main}11`}
            stroke={hi && i === 0 ? C.proof : C.main}
          />
          <text
            x={x}
            y={leafY + 15}
      ...
      <Nd
        x={36}
        y={midY}
        fill={hi ? `${C.proof}22` : `${C.main}11`}
        stroke={hi ? C.proof : C.main}
      />
  ...
  return (
    ...
    >
      ...
      <line
        ...
        stroke={C.add}
        strokeWidth={1.2}
        markerEnd="url(#mmr-arw)"
        strokeDasharray="3 2"
      />
      <text x={310} y={leafY + 3} fontSize={10} fill="var(--muted-foreground)">
        O(1) 순차 쓰기
      </text>
      <text x={310} y={leafY + 15} fontSize={10} fill="var(--muted-foreground)">
        peak 높이 일치 → 병합
      </text>
    </motion.g>
  );
...
export function ProofPath() {
  return (
    <motion.g initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={sp}>
      ...
      <text x={280} y={34} fontSize={10} fill="var(--muted-foreground)">
        fold_prefix: (없음)
      </text>
      <text x={280} y={48} fontSize={10} fill="var(--muted-foreground)">
        path: L1 → H(0,1) → Peak0
      </text>
      <text x={280} y={62} fontSize={10} fill="var(--muted-foreground)">
        others: Peak1, Peak2
      </text>
      <text x={280} y={76} fontSize={10} fill="var(--muted-foreground)">
        root = Hash(leaves || fold(peaks))
      </text>
    </motion.g>
  );
}
