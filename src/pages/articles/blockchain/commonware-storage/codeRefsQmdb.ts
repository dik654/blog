    ...
    annotations: [
      {
        lines: [6, 8],
        color: "sky",
        note: "log — 인증된 Journal (MMR 머클화)",
      },
      {
        lines: [10, 12],
        color: "emerald",
        note: "inactivity_floor — 비활성 경계",
      },
      {
        lines: [17, 19],
        color: "amber",
        note: "snapshot — 활성 키→위치 인덱스",
      },
    ],
  },
  "any-get": {
    ...
    annotations: [
      {
        lines: [28, 29],
        color: "sky",
        note: "snapshot.get(key) — 해시 인덱스 조회",
      },
      {
        lines: [31, 35],
        color: "emerald",
        note: "log.reader().read(loc) — 실제 값 읽기",
      },
    ],
  },
  "any-proof": {
  ...
  "current-db": {
    ...
    annotations: [
      ...
      { lines: [9, 11], color: "emerald", note: "status — 활성/비활성 비트맵" },
      {
        lines: [13, 15],
        color: "amber",
        note: "grafted_mmr — 비트맵+ops 결합 MMR",
      },
      {
        lines: [20, 22],
        color: "violet",
        note: "root — hash(ops || grafted [|| partial])",
      },
    ],
  },
  grafting: {
    ...
    annotations: [
      {
        lines: [14, 15],
        color: "sky",
        note: "grafting height = log2(chunk_size_bits)",
      },
      {
        lines: [19, 22],
        color: "emerald",
        note: "chunk_idx → ops Position 변환",
      },
      {
        lines: [25, 28],
        color: "amber",
        note: "grafted leaf = hash(chunk || subtree_root)",
      },
    ],
  },
  "current-proof": {
    ...
    annotations: [
      {
        lines: [12, 15],
        color: "sky",
        note: "OperationProof 구조: loc + chunk + range_proof",
      },
      {
        lines: [19, 21],
        color: "emerald",
        note: "get_bit_from_chunk — 활성 비트 확인",
      },
      {
        lines: [24, 25],
        color: "amber",
        note: "range_proof.verify — grafted 루트 검증",
      },
    ],
  },
};
