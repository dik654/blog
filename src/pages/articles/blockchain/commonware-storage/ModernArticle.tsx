...
import { ProofBoundaryViz, StorageBatchViz } from "./viz/ModernCommonwareStorageViz";
...
export default function ModernCommonwareStorageArticle(){return <article className="space-y-14">
  ...
  <section id="proof-layout" className="space-y-6"><header><p className="text-sm font-semibold text-primary">01 · MMR peaks and proof</p><h2 className="mt-2 text-2xl font-bold">MMR은 append할 때 기존 node 위치를 바꾸지 않고 여러 perfect-tree peaks를 bagging한다</h2></header>
    ...
    operations={[
      { expression: String.raw`popcount(N)`, annotation: ["set-bit count이(가) 식의 결과에 기여하는 방식을","계산합니다.","N의 binary 표현에서 1인 bit 하나가 완전 이진트리","하나를 뜻합니다."] },
      { expression: String.raw`2N-popcount(N)`, annotation: ["set-bit count이(가) 식의 결과에 기여하는 방식을","계산합니다.","N의 binary 표현에서 1인 bit 하나가 완전 이진트리","하나를 뜻합니다."] },
    ]} terms={[{symbol:"N",name:"leaf count",description:"Append된 elements 수입니다."},{symbol:"popcount(N)",name:"set-bit count",description:"N의 binary 표현에서 1인 bit 수입니다."},{symbol:"peaks(N)",name:"peak count",description:"서로 다른 높이의 perfect-tree roots 수입니다."},{symbol:"nodes(N)",name:"total node count",description:"Leaves와 internal nodes를 모두 합친 MMR size입니다."}]} assumptions={["Commonware MMR family의 append-only post-order layout을 사용합니다.","Root는 leaf count와 configured peak-bagging policy에 결속됩니다.","Hash domain과 node positions가 verifier와 같습니다.","Inclusion proof는 value의 현재성이나 database durability를 보장하지 않습니다."]} interpretation="11은 이진수 1011이므로 set bit가 3개이고 peaks도 3개입니다. 전체 nodes는 22-3=19입니다. 같은 peaks라도 bagging 순서나 leaf count를 바꾸면 다른 root여야 합니다." />
    <div id="paper-commonware-mmr"><CitationBlock source="Commonware storage MMR source · v2026.7.0" citeKey={1} type="code" href="https://github.com/commonwarexyz/monorepo/blob/5950bf7179bb0650a57ed58b9e0478822944b335/storage/src/merkle/mmr/mod.rs"><p><strong>문제:</strong> Append-only elements의 stable position과 succinct inclusion·range proof가 필요합니다.</p><p><strong>기여:</strong> MMR family, leaf/location mapping, peaks, proof types와 configurable bagging을 구현·문서화합니다.</p><p><strong>전제:</strong> v2026.7.0 commit 5950bf7의 family limits, positioned hashing과 동일 bagging policy를 사용합니다.</p><p><strong>근거 범위:</strong> Pinned MMR layout·root·proof implementation에 한정합니다.</p><p><strong>말하지 않는 것:</strong> MMR inclusion이 key의 최신 value, transaction finality 또는 disk durability를 증명한다는 뜻은 아닙니다.</p></CitationBlock></div>
  </section>
  <section id="database-paths" className="space-y-6"><header><p className="text-sm font-semibold text-primary">02 · Any, Current, QMDB batch</p><h2 className="mt-2 text-2xl font-bold">Any는 과거 value를, Current는 그 value가 아직 active인지까지 증명한다</h2></header>
    <ProofBoundaryViz /><p><code>qmdb::any</code>는 key가 어떤 value를 가졌던 operation의 succinct proof를 제공하지만, 이후 update·delete가 없었다는 사실까지는 말하지 않습니다. <code>qmdb::current</code>는 operation마다 active bit를 두고 bitmap chunk와 operations subtree를 graft한 하나의 authenticated root로 현재성을 증명합니다.</p>
    <p>모든 mutable variant는 new batch→write/delete→merkleize→inspect root→apply batch 흐름을 따릅니다. 같은 parent에서 만든 sibling A와 B 중 A가 먼저 적용되면 B는 stale입니다. B를 나중에 읽거나 child를 만들거나 apply하는 것은 이전 snapshot처럼 보이더라도 invalid object 사용입니다. 이는 consensus fork rollback과 비슷해 보이지만 storage branch validity라는 별도 책임입니다.</p>
