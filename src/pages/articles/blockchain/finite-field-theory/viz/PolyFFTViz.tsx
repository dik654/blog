...
const STEPS = [
  {
    label: "다항식의 두 가지 표현",
    body: "같은 다항식을 계수 또는 평가 결과로 저장할 수 있다.",
  },
  {
    label: "평가란? — x를 대입하면 값이 나온다",
    body: "3개의 점(x,y)을 알면 2차 다항식이 유일하게 결정된다.",
  },
  {
    label: "NTT — 계수 ↔ 평가를 빠르게 변환",
    body: "NTT/INTT로 계수 ↔ 평가를 O(n log n)에 상호 변환.",
  },
  {
    label: "곱셈 가속: O(n²) → O(n log n)",
    body: "NTT → 점별 곱 O(n) → INTT. 총 O(n log n).",
  },
];
        ...
        step === 0 ? (
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="rounded-lg border border-indigo-500/30 bg-indigo-500/10 p-4 space-y-2">
                ...
                <div className="text-sm">
                  <Math>{"[a_0, a_1, \\ldots, a_{n-1}]"}</Math>
                </div>
          ...
          >
            ...
            <motion.g animate={{ opacity: step >= 2 ? 1 : 0.15 }}>
              ...
              />
              <polygon
                points={`${RX - 15},35 ${RX - 9},38 ${RX - 15},41`}
                fill={C2}
              />
              <text
                x={(LX + BW + RX) / 2}
              ...
              />
              <polygon
                points={`${LX + BW + 15},57 ${LX + BW + 9},60 ${LX + BW + 15},63`}
                fill={C1}
              />
              <text
                x={(LX + BW + RX) / 2}
                  ...
                  <g key={i}>
                    {i > 0 && (
                      <>
                        ...
                        />
                        <polygon
                          points={`${s.x - 6},119 ${s.x},122 ${s.x - 6},125`}
                          fill="var(--muted-foreground)"
                        />
                      </>
                    )}
