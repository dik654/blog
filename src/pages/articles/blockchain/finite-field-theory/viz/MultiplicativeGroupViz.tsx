...
const STEPS = [
  {
    label: "F₇ (체) vs F₇* (군)",
    body: "F₇*는 0을 제외한 곱셈군(6개). 0은 곱셈 역원이 없어 제외.",
  },
  {
    label: "순환군 — g=3 반복 곱셈",
    body: "3의 거듭제곱이 {1~6} 전부 순회 → 순환군. g=3이 생성원.",
  },
  {
    label: "Fermat 소정리",
    body: "어떤 원소든 p-1번 곱하면 1로 복귀 — aᵖ⁻¹ ≡ 1 (mod p).",
  },
  {
    label: "역원 = 거듭제곱",
    body: "aᵖ⁻² = a⁻¹ — 나눗셈을 거듭제곱(mod 연산)으로 변환.",
  },
];
    ...
    <StepViz steps={STEPS}>
      {(step) => (
        ...
        >
          ...
          {step === 2 && (
            <motion.g initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
              <text x={60} y={EY + 48} fontSize={9} fontWeight={500} fill={C.b}>
                공식
              </text>
              <rect
                x={60}
              ...
              <text x={20} y={EY + 92} fontSize={9} fontWeight={500} fill={C.b}>
                예시 (F₇, p=7)
              </text>
              {[
