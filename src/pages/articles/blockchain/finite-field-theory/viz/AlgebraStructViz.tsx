...
const STEPS = [
  {
    label: "대수 구조: 군 → 환 → 체",
    body: "공리를 추가할수록 강력해진다. 체가 가장 완성된 구조.",
  },
  {
    label: "군 — 하나의 연산 + 4개 공리",
    body: "닫힘, 결합법칙, 항등원, 역원.",
  },
  {
    label: "환 — 두 연산 + 분배법칙",
    body: "군의 공리 + 곱셈과 분배법칙. 예: 정수 Z.",
  },
  {
    label: "체 — 곱셈 역원까지 존재",
    body: "환 + 0 외 모든 원소에 곱셈 역원. ZKP의 핵심 무대.",
  },
];
        ...
        >
          ...
          {[0, 1].map((i) => (
            <g key={i}>
              ...
              />
              <polygon
                points={`${BX + BW + 17},${by(i)} ${BX + BW + 20},${by(i) - 4} ${BX + BW + 23},${by(i)}`}
                fill="var(--muted-foreground)"
              />
            </g>
          ))}
