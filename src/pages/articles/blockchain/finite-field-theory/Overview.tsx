    ...
    <section id="overview" className="mb-16 scroll-mt-20">
      ...
      <div className="prose prose-neutral dark:prose-invert max-w-none">
        <p className="text-lg leading-8">
          영지식 증명은 아주 큰 정수를 그대로 다루지 않고 정해진 소수 p로 나눈 나머지 안에서 덧셈·곱셈·나눗셈을 반복합니다. 이때 계산 결과가 유한한 집합 안에 머물면서도 0이 아닌
          값으로 한 연산을 되돌릴 수 있어야 합니다. 이를 만족하는 대수 구조가 유한체(finite field)입니다.
        </p>
        <p>
      ...
      <div className="prose prose-neutral dark:prose-invert max-w-none">
        ...
        <p>
          군(group)은 한 연산에 대해 결합법칙·항등원·역원을 갖습니다. 환은 덧셈군에 곱셈과 분배법칙을 더하지만 정수 환처럼 곱셈 역원이 없는 원소가 있을 수 있습니다.
          체(field)는 0을 제외한 곱셈까지 군이므로 모든 0이 아닌 값으로 나눌 수 있습니다. ZK 회로가 field를 쓰는 이유는 단지 값의 범위가 작아서가 아니라 다항식의 나눗셈과
          보간이 항상 정의되기 때문입니다.
        </p>
        <p>
      ...
      >
        ...
        <p className="mt-2 text-sm leading-6 text-muted-foreground">
          이 규격은 prime field와 binary field 위 타원곡선·서명 파라미터의 검증 조건을 정의합니다. 유한체 산술이 실제 암호 규격에서 어떻게 제약되는지 보여 주지만 특정
          ZK field의 성능이나 모든 extension representation의 안전성을 보장하는 문서는 아닙니다.
        </p>
        <a
