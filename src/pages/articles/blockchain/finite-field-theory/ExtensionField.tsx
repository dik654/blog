  ...
  return (
    <section id="extension-field" className="mb-16 scroll-mt-20">
      ...
      <div className="prose prose-neutral dark:prose-invert max-w-none mb-6">
        <p>
          기존 체에 기약 다항식의 근을 추가하여 더 큰 체 구성 — BN254 페어링에서
          필수.
        </p>
      </div>
      ...
      <div className="prose prose-neutral dark:prose-invert max-w-none mt-6">
        ...
        <div className="not-prose rounded-lg border-l-4 border-l-blue-500 bg-card p-4 mb-4">
          ...
          <p className="text-sm text-muted-foreground">
            <M>{"K \\subset L"}</M> &mdash; 체 <M>K</M>에서 체 <M>L</M>로 확장.{" "}
            <M>{"[L:K]"}</M> = 확장 차수.
          </p>
          <p className="text-sm text-muted-foreground mt-2">
            구성: 기약 다항식 <M>{"f(x) \\in K[x]"}</M> 선택 &rarr;{" "}
            <M>{"L = K[x] / f(x)"}</M> (몫환)
          </p>
        </div>
        ...
        <h4 className="text-lg font-semibold mt-5 mb-3">
          예시: <M>{"\\mathbb{F}_p \\to \\mathbb{F}_{p^2}"}</M>
        </h4>
        <div className="not-prose rounded-lg border bg-card p-4 mb-4">
          <p className="text-sm text-muted-foreground mb-2">
            <M>{"f(x) = x^2 + 1"}</M> (<M>{"p=3"}</M>에서 기약) &rarr;{" "}
            <M>{"\\mathbb{F}_9 = \\mathbb{F}_3[x]/(x^2+1)"}</M>
          </p>
          <p className="text-sm text-muted-foreground mb-3">
            원소: <M>{"a + bi"}</M> (여기서 <M>{"i^2 = -1"}</M>)
          </p>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
            <div className="rounded bg-muted/50 p-3">
              ...
              <M display>
                {
                  "\\underbrace{(a+bi)}_{\\text{첫째 원소}} + \\underbrace{(c+di)}_{\\text{둘째 원소}} = \\underbrace{(a{+}c)}_{\\text{실수부 합}} + \\underbrace{(b{+}d)i}_{\\text{허수부 합}}"
                }
              </M>
              <p className="text-sm text-muted-foreground mt-2">
                a, b, c, d는 기저체 원소. 각 성분별로 독립적으로 더한다.
              </p>
            </div>
            <div className="rounded bg-muted/50 p-3">
              ...
              <M display>
                {
                  "\\underbrace{(a+bi)}_{\\text{첫째}} \\underbrace{(c+di)}_{\\text{둘째}} = \\underbrace{(ac{-}bd)}_{\\text{실수부}} + \\underbrace{(ad{+}bc)i}_{\\text{허수부}}"
                }
              </M>
              <p className="text-sm text-muted-foreground mt-2">
                i^2 = -1 관계를 적용해 전개. ac-bd가 실수부, ad+bc가 허수부 --
                복소수 곱셈과 동일한 구조.
              </p>
            </div>
          </div>
        ...
        <div className="not-prose grid grid-cols-1 sm:grid-cols-2 gap-3 mb-4">
          <div className="rounded-lg border bg-card p-4">
            ...
            <p className="text-xs text-muted-foreground mb-2">
              Base field: <M>{"\\mathbb{F}_p"}</M>, <M>{"p \\sim 2^{254}"}</M>
            </p>
            ...
            <p className="text-xs text-muted-foreground mt-2">
              Pairing: <M>{"e: G_1 \\times G_2 \\to G_T"}</M>
            </p>
          </div>
          <div className="rounded-lg border bg-card p-4">
            ...
            <p className="text-xs text-muted-foreground">
              <M>
                {
                  "\\mathbb{F}_{p^{12}} = \\mathbb{F}_{p^2} \\cdot \\mathbb{F}_{p^6}"
                }
              </M>
            </p>
          </div>
        ...
        <div className="not-prose rounded-lg border bg-card p-4 mb-4">
          ...
          <M display>
            {
              "\\underbrace{\\varphi}_{\\text{프로베니우스 사상}}: L \\to L, \\quad \\varphi(x) = \\underbrace{x^p}_{\\text{p제곱 사상}}"
            }
          </M>
          <p className="text-sm text-muted-foreground mt-2">
            phi = Frobenius endomorphism, L = 확장체, p = 기저체의
            특성(characteristic). 기저체 원소는 고정(x^p = x)하고 확장 원소만
            변환하는 자기동형사상 -- 확장체의 갈루아 군을 생성하며, 페어링
            연산과 효율적 지수 계산에 핵심.
          </p>
        </div>
          ...
          <div className="rounded-lg border bg-card p-4">
            ...
            <div className="grid grid-cols-2 gap-1">
              {[
                ...
                >
                  <code>{l.name}</code>{" "}
                  <span className="text-muted-foreground/60">({l.lang})</span>
                </div>
              ))}
            </div>
          </div>
