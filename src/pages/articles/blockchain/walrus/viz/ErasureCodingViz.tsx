...
const STEPS = [
  {
    label: "전체 아키텍처",
    body: "Walrus RedStuff 2D 이레이저 코딩의 전체 구조입니다.",
  },
  {
    label: "클라이언트 레이어",
    body: "CLI에서 블롭을 업로드하고 BFT 인코딩 설정(n, f, k1, k2)을 계산합니다.",
  },
  {
    label: "인코딩 레이어",
    body: "BlobEncoder가 2D Reed-Solomon으로 Primary(열)/Secondary(행) 슬라이버를 생성합니다.",
  },
  {
    label: "검증 & 배포",
    body: "Merkle 트리로 검증 후 Sui 체인에 BlobId를 등록하고 노드에 슬라이버를 배포합니다.",
  },
];
...
export default function ErasureCodingViz() {
  return (
    <StepViz steps={STEPS}>
      {(step) => {
        const active =
          step === 0 ? NODES.map((_, i) => i) : (GROUPS[step - 1] ?? []);
        return (
          <svg
            viewBox="0 0 480 130"
