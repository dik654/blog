  ...
  return (
    <section id="overview" className="mb-16 scroll-mt-20">
      ...
      <div className="prose prose-neutral dark:prose-invert max-w-none">
        <p>
          <strong>Walrus</strong>는 Sui 블록체인 위에 구현된 탈중앙 블롭(Blob,
          대용량 이진 데이터) 저장소입니다.
          <br />
          핵심 설계는 <strong>RedStuff</strong>라는 2D Reed-Solomon 이레이저
          코딩(Erasure Coding, 소거 부호화) 알고리즘입니다.
          <br />
          n개의 저장 노드 중 f개가 비잔틴 장애를 일으켜도 (n=3f+1) 데이터를
          복구할 수 있습니다.
        </p>
        {onCodeRef && (
          <div className="not-prose flex flex-wrap gap-2 my-4">
            <CodeViewButton
              onClick={() => onCodeRef("walrus-bft", codeRefs["walrus-bft"])}
            />
            ...
            <CodeViewButton
              onClick={() =>
                onCodeRef("walrus-config", codeRefs["walrus-config"])
              }
            />
            <span className="text-[10px] text-muted-foreground self-center">
              config.rs
        ...
        <CodePanel
          title="BFT 파라미터 & 인코딩 설정"
          code={BFT_CODE}
          annotations={BFT_ANNOTATIONS}
        />

        <h3>RedStuff 핵심 아이디어</h3>
        <p>
          // 2D 메시지 행렬: primary_k × secondary_k 심볼
          <br />
          // ← secondary_k 열 →<br />
