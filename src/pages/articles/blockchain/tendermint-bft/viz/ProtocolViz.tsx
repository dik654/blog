...
const DATA: { title: string; color: string; rows: R }[] = [
  ...
  {
    title: "Phase 2: Prevote",
    color: C.vote,
    rows: [
      [
        "pᵢ: valid(block) → ⟨PREVOTE, h, r, H(block)⟩",
        "var(--foreground)",
        0.15,
      ],
      [
        "잠금 확인: lockedRound ≤ pol_round → 허용",
        "var(--muted-foreground)",
        0.3,
      ],
      ['+2/3 Prevote(id) 수신 = "Polka" 형성', C.vote, 0.45],
      [
        "+2/3 Prevote(nil) → Precommit(nil) 전송",
        "var(--muted-foreground)",
        0.6,
      ],
    ],
  ...
  {
    title: "Phase 3: Precommit (잠금 설정)",
    color: C.pre,
    rows: [
      [
        "Polka(B) 수신: lockedValue=B, lockedRound=r",
        "var(--foreground)",
        0.15,
      ],
      ...
      [
        "+2/3 미달 → timeout_precommit → round+1",
        "var(--muted-foreground)",
        0.6,
      ],
    ],
