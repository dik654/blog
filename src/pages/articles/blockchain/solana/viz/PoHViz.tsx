...
const STEPS = [
  {
    label: "초기 해시 생성",
    body: "SHA-256(초기 상태)로 첫 해시 생성. PoH 체인 시작점",
  },
  {
    label: "순차적 해싱",
    body: '이전 해시 → 다음 해시 순차 계산. 각 해시 = "시간의 틱"',
  },
  {
    label: "TX 삽입",
    body: "hash_k = SHA256(prev || tx). TX가 해당 시점에 존재했음을 증명",
  },
  {
    label: "병렬 검증",
    body: "체인을 구간 분할 → 다중 코어 동시 검증. O(n/cores)",
  },
];
const REFS = ["sol-poh-tick", "sol-poh-tick", "sol-poh-record", "sol-poh-tick"];

export default function PoHViz({
  onOpenCode,
}: {
              ...
              return (
                ...
                >
                  <motion.rect
                    ...
                    rx={6}
                    animate={{
                      fill: isTx ? "#f59e0b20" : `${vc}15`,
                      stroke: isTx ? "#f59e0b" : vc,
                      strokeWidth: isTx ? 2 : 1,
                    }}
                    transition={sp}
                  />
                  <text
