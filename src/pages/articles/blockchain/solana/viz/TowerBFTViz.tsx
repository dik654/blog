...
const STEPS = [
  {
    label: "첫 투표: slot 100",
    body: "conf=1, lockout=1. 즉시 포크 전환 가능",
  },
  {
    label: "연속 투표: 스택 성장",
    body: "새 투표 → 기존 conf 증가 → 락아웃 2배",
  },
  {
    label: "락아웃 누적",
    body: "5회 연속 → 최하위 lockout=31. 포크 전환 비용 지수 증가",
  },
  {
    label: "최종성 (rooted)",
    body: "32 확인 → 2^32 슬롯 락아웃 ≈ 영구. 블록 rooted",
  },
];

const BAR_MAX = 200;

export default function TowerBFTViz({
  onOpenCode,
}: {
      ...
      {(step) => (
        <div className="w-full">
          ...
          >
            ...
            <text x={310} y={14} fontSize={9} fill="var(--muted-foreground)">
              Lockout
            </text>
            {SLOTS.map((s, i) => {
              const y = 24 + i * 24;
              const visible =
                step === 0 ? i === 0 : step === 1 ? i <= 2 : i <= 4;
              ...
              return (
                <motion.g
                  key={i}
                  animate={{ opacity: visible ? 1 : 0.08 }}
                  transition={sp}
                >
                  <text
                    x={10}
