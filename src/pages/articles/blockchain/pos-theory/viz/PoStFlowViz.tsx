...
const STEPS = [
  {
    label: "PoSt: 주기적 챌린지 구조",
    body: "PoSt는 시간 축을 따라 주기적으로 저장을 증명하는 프로토콜입니다.",
  },
  {
    label: "T1: 첫 번째 챌린지 통과",
    body: "에폭 T1에서 랜덤 챌린지 C1에 정상 응답. 저장 지속 확인.",
  },
  {
    label: "T2: 연속 저장 증명",
    body: "T2에서도 새로운 챌린지에 응답. 연속성 증명.",
  },
  {
    label: "T3: 실패 → 슬래싱",
    body: "T3에서 응답 실패 시 Fault가 기록되고 담보가 슬래싱됩니다.",
  },
  {
    label: "T4: 복구 후 재증명",
    body: "T4에서 복구 후 다시 정상 응답. 페널티 기간 종료.",
  },
];
const BW = 80,
  BH = 36,
          ...
          {EPOCHS.map((e, i) => {
            ...
            return (
              <motion.g
                key={e.t}
                animate={{
                  opacity: step === 0 || visible || active ? 1 : 0.15,
                }}
              >
                {/* epoch marker */}
                <circle
