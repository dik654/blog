  ...
  if (step === 0)
    return (
      <div className="space-y-5 w-full px-4">
        <Math
          display
        >{`Z_H(x) = \\prod_{\\omega_i \\in H} (x - \\omega_i)`}</Math>
          ...
          <table className="text-sm">
            <thead>
              <tr className="text-muted-foreground">
                ...
                {DOMAIN.map((d) => (
                  <th key={d} className="px-4 py-1.5 font-medium">
                    {d}
                  </th>
                ))}
            ...
            <tbody>
              <tr>
                <td className="px-4 py-1.5 font-semibold" style={{ color: Y }}>
                  Z_H(x)
                </td>
                {DOMAIN.map((d) => (
                  <td
  ...
  if (step === 1)
    return (
      <div className="space-y-4 w-full px-4">
        <Math
          display
        >{`Z_H(x) = \\textcolor{${R}}{(x-0)} \\cdot \\textcolor{${B}}{(x-1)} \\cdot \\textcolor{${G}}{(x-2)}`}</Math>
        <div className="flex justify-center gap-6 text-sm">
          {FACTORS.map((f, i) => (
            <div key={i} className="flex flex-col items-center gap-1">
              <span
                className="font-semibold"
                style={{ color: FACTOR_COLORS[i] }}
              >
                {f}
              </span>
        ...
        <div className="flex justify-center gap-4 text-sm">
          {DOMAIN.map((d) => (
            <div key={d} className="rounded-lg border px-4 py-2 text-center">
              <Math>{`Z_H(${d})`}</Math>
              <span className="font-bold ml-1" style={{ color: Y }}>
                = 0
              </span>
            </div>
          ))}
  ...
  if (step === 3)
    return (
      <div className="space-y-4 w-full px-4">
        <Math
          display
        >{`C(\\omega_i) = 0 \\;\\; \\forall\\, \\omega_i \\in H \\quad \\Longleftrightarrow \\quad Z_H(x) \\mid C(x)`}</Math>
        <Math
          display
        >{`C(x) = \\textcolor{${P}}{Q(x)} \\cdot \\textcolor{${Y}}{Z_H(x)}`}</Math>
  ...
  return (
    <div className="space-y-4 w-full px-4">
      <div className="flex items-center justify-center gap-3 text-sm">
        ...
        <span className="text-lg font-bold" style={{ color: Y }}>
          →
        </span>
        <div
          ...
          style={{ borderColor: Y }}
        >
          <p className="font-semibold" style={{ color: Y }}>
            한 번 나눗셈
          </p>
      ...
      <Math
        display
      >{`\\underbrace{C(x)}_{\\text{제약}} = \\textcolor{${P}}{Q(x)} \\cdot \\textcolor{${Y}}{Z_H(x)}`}</Math>
    </div>
  );
}
