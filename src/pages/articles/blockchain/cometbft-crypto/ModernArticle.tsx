...
export default function ModernCometBFTCryptoArticle() {
  ...
  <article className="space-y-14">
    ...
    <section id="merkle" className="space-y-6">
      ...
      <ExplainedFormula question="같은 SHA-256을 쓰면서 leaf와 inner node를 어떻게 서로 다른 영역으로 나누는가?" idea={<>Input 앞의 한 byte를 type tag로 쓰면 leaf payload와 child-hash pair가 같은 byte string으로 해석되지 않습니다. Root는 leaf 순서와 tree shape까지 commitment합니다.</>} formula={String.raw`H_{leaf}(x)=\operatorname{SHA256}(0x00\,\|\,x),\qquad H_{inner}(L,R)=\operatorname{SHA256}(0x01\,\|\,L\,\|\,R)`}
      annotatedFormula={String.raw`H_{leaf}(x)=\underbrace{\operatorname{SHA256}(0x00\,\|\,x),\qquad H_{inner}(L,R)=\operatorname{SHA256}(0x01\,\|\,L\,\|\,R)}_{\text{child hashes 계산}}`}
      operations={[
        { expression: String.raw`\operatorname{SHA256}(0x00\,\|\,x),\qquad H_{inner}(L,R)=\operatorname{SHA256}(0x01\,\|\,L\,\|\,R)`, annotation: ["child hashes이(가) 식의 결과에 기여하는 방식을","계산합니다.","Input 앞의 한 byte를 type tag로 쓰면 leaf","payload와 child-hash pair가 같은 byte"] },
      ]} terms={[{symbol:"x",name:"leaf bytes",description:"상위 type이 canonical encoding한 하나의 item입니다."},{symbol:"L,R",name:"child hashes",description:"왼쪽·오른쪽 subtree의 32-byte roots입니다."},{symbol:"0x00,0x01",name:"domain prefixes",description:"Leaf와 inner node를 다른 input domain으로 구분합니다."},{symbol:String.raw`\|`,name:"concatenation",description:"Bytes를 순서대로 연결합니다."}]} assumptions={["SHA-256 collision resistance를 전제합니다.","Leaf encoding과 item order가 먼저 canonical해야 합니다.","Proof verifier는 index·total·aunt order와 root length를 검사해야 합니다.","Commitment는 confidentiality나 application validity를 제공하지 않습니다."]} interpretation="x의 한 byte라도 바뀌거나 path의 왼쪽·오른쪽 순서를 바꾸면 root가 달라져야 합니다. Root가 같다고 x의 의미가 유효하거나 숨겨진다고 읽으면 안 됩니다." />
      <HashCommitmentViz />
      <div className="not-prose flex flex-wrap gap-3">
