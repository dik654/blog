    ...
    annotations: [
      {
        lines: [5, 7],
        color: "sky",
        note: "genesis뷰 제외 — 뷰 0은 제안 불가 (genesis 블록이 차지)",
      },
      {
        lines: [8, 14],
        color: "emerald",
        note: "should_propose — 리더인지, 이미 building 중인지 확인",
      },
      {
        lines: [17, 24],
        color: "amber",
        note: "find_parent — 이전 certified 뷰 탐색. 부모 미확보 시 제안 불가",
      },
      {
        lines: [25, 31],
        color: "violet",
        note: "Context 구성 — round(epoch+view), leader, parent(view+payload) 반환",
      },
    ],
  },
  "construct-notarize": {
    ...
    annotations: [
      {
        lines: [4, 7],
        color: "sky",
        note: "round.construct_notarize() — 투표 가능 상태인지 확인 후 Proposal 반환",
      },
      {
        lines: [10, 11],
        color: "emerald",
        note: "Notarize::sign() — scheme.sign(Subject::Notarize{proposal})으로 서명 생성",
      },
    ],
  },
    ...
    annotations: [
      {
        lines: [32, 33],
        color: "sky",
        note: "broadcast_notarization() — Round가 쿼럼 달성 여부 확인 후 Notarization 반환",
      },
      {
        lines: [35, 37],
        color: "emerald",
        note: "resolver.updated — resolver에게 뷰 완료 알림 (추가 요청 중단)",
      },
      {
        lines: [38, 41],
        color: "amber",
        note: "handle + sync + broadcast — 저장, fsync, 전체 브로드캐스트 순서",
      },
    ],
  },
  "construct-nullify": {
    ...
    annotations: [
      {
        lines: [5, 7],
        color: "sky",
        note: "현재 뷰만 nullify 가능 — 과거 뷰는 이미 인증서가 존재하거나 무관",
      },
      {
        lines: [9, 10],
        color: "emerald",
        note: "Round.construct_nullify() — broadcast_finalize면 None (이미 확정 진행 중)",
      },
      {
        lines: [12, 21],
        color: "amber",
        note: "타임아웃 원인 분류: 제안 유무로 LeaderTimeout vs CertificationTimeout 구분",
      },
    ],
  },
    ...
    annotations: [
      {
        lines: [25, 29],
        color: "sky",
        note: "Round.broadcast_nullification() — 이미 브로드캐스트했으면 None 반환",
      },
    ],
  },
  "add-finalization": {
    ...
    annotations: [
      {
        lines: [8, 9],
        color: "sky",
        note: "last_finalized 갱신 — 이 뷰 이하는 더 이상 추적 불필요",
      },
      {
        lines: [12, 17],
        color: "emerald",
        note: "정리: certification_candidates 가지치기 + outstanding_certifications abort",
      },
      {
        lines: [20, 22],
        color: "amber",
        note: "enter_view + set_leader — 즉시 다음 뷰! Simplex Change #1",
      },
    ],
  },
  certified: {
    ...
    annotations: [
      {
        lines: [30, 33],
        color: "sky",
        note: "round.certified(success) — CertifyState::Certified(bool)로 전환",
      },
      {
        lines: [38, 39],
        color: "emerald",
        note: "성공: enter_view(next) — 다음 뷰로 전진. finalize 투표 가능 상태",
      },
      {
        lines: [40, 41],
        color: "amber",
        note: "실패: trigger_timeout — FailedCertification으로 nullify 경로 진입",
      },
    ],
  },
};
