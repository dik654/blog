    ...
    <section id="overview" className="mb-16 scroll-mt-20">
      ...
      <div className="prose prose-neutral dark:prose-invert max-w-none mb-8">
        <p className="leading-7">
          Simplex — Benjamin Chan · Rafael Pass가 TCC 2023에서 발표한 BFT 합의
          프로토콜
          <br />
          PBFT(1999) → Tendermint(2014) → HotStuff(2018) →{" "}
          <strong>Simplex(2023)</strong>
        </p>
        <p className="leading-7">
          4가지 핵심 혁신으로 기존 프로토콜의 한계를 돌파:
          <br />
          <strong>① 즉시 View 전환</strong> — Cert(k,x) 수집 즉시 view k+1로
          이동. View-change 전송 불가
          <br />
          <strong>② No-Commit 증명</strong> — n-f View-change = 해당 view에서
          결정 없었음 증명
          <br />
        ...
        <p className="leading-7">
          Commonware 구현: <code>consensus::simplex</code>(기본) +{" "}
          <code>consensus::threshold_simplex</code>(VRF+BLS)
        </p>
      </div>
      ...
      <div className="not-prose mt-6">
        ...
        <div className="rounded-lg border border-border bg-card p-5 mb-4">
          ...
          <p className="text-sm text-muted-foreground mb-2">
            "Simplex Consensus: A Simple and Fast Consensus Protocol" — Benjamin
            Y. Chan, Rafael Pass (TCC 2023, eprint.iacr.org/2023/463)
          </p>
          <p className="text-sm text-muted-foreground">
            핵심 주장: "Simpler than HotStuff" · "Faster than existing
            protocols" · "Linear view-change complexity 유지"
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-4">
          <div className="rounded-lg border border-border bg-card p-4">
            ...
            <p className="text-xs text-muted-foreground">
              <code className="text-xs">Cert(view=k, val=x)</code> 확인 즉시
              view k+1로 이동. 타임아웃 대기 없음. 별도 "view-change" 메시지
              단계 없음.
            </p>
          </div>
          <div className="rounded-lg border border-border bg-card p-4">
            ...
            <p className="text-xs text-muted-foreground">
              n-f 개 view-change 투표 = 해당 view에서 커밋 없었음을 증명. 이
              증명으로 안전성 보장.
            </p>
          </div>
          <div className="rounded-lg border border-border bg-card p-4">
            ...
            <p className="text-xs text-muted-foreground">
              HotStuff: 리더가 쿼럼 대기 2Δ. Simplex: view 진입 즉시 제안.
          ...
          <div className="rounded-lg border border-border bg-card p-4">
            ...
            <p className="text-xs text-muted-foreground">
              HotStuff: 6Δ timeout/view. Simplex: 3Δ timeout/view. 비잔틴 리더
              하에서 더 빠른 view 진행.
            </p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-4">
          ...
          <div className="rounded-lg border border-border bg-card p-4">
            ...
            <ul className="text-xs text-muted-foreground space-y-1">
              ...
              <li>
                <code className="text-xs">Notarize(v, digest)</code> — 제안 투표
              </li>
              <li>
                <code className="text-xs">Nullify(v)</code> — 다음 view 건너뛰기
              </li>
              <li>
                <code className="text-xs">Finalize(v, digest)</code> — 블록 확정
              </li>
            </ul>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-4">
          <div className="rounded-lg border border-border bg-card p-4">
            ...
            <p className="text-xs text-muted-foreground">
              충돌하는 두 블록이 동시에 확정 불가. 쿼럼 교차(quorum
              intersection)로 보장. f &lt; n/3 비잔틴 내성.
            </p>
          </div>
          <div className="rounded-lg border border-border bg-card p-4">
            <h4 className="font-semibold text-sm mb-2">Liveness</h4>
            <p className="text-xs text-muted-foreground">
              동기 조건에서 진행 보장. 정직한 리더: 3Δ 안에 view 완료. 비잔틴
              리더: 3Δ에 감지. f+1 연속 정직 리더 → 확정.
            </p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-4">
          <div className="rounded-lg border border-border bg-card p-4">
            <h4 className="font-semibold text-sm mb-2">
              <code className="text-sm">consensus::simplex</code>
            </h4>
            <ul className="text-xs text-muted-foreground space-y-1">
              <li>기본 변형 — 개별 서명</li>
              <li>O(n) 인증서 크기</li>
          ...
          <div className="rounded-lg border border-border bg-card p-4">
            <h4 className="font-semibold text-sm mb-2">
              <code className="text-sm">consensus::threshold_simplex</code>
            </h4>
            <ul className="text-xs text-muted-foreground space-y-1">
