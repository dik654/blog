    ...
    annotations: [
      {
        lines: [6, 10],
        color: "sky",
        note: "context·scheme·elector — 런타임(clock/rng), 서명 스킴, 리더 선출기 주입",
      },
      {
        lines: [14, 18],
        color: "emerald",
        note: "타이머 3종 — leader_timeout(제안 대기), certification_timeout(인증 대기), timeout_retry(nullify 재시도)",
      },
      {
        lines: [19, 22],
        color: "amber",
        note: "view·last_finalized·views — 현재 뷰, 확정 뷰, 뷰별 Round 상태. 합의의 핵심 좌표",
      },
    ],
  },
  "round-struct": {
    ...
    annotations: [
      {
        lines: [4, 6],
        color: "sky",
        note: "start·scheme·round — 생성 시각, 서명 스킴, (epoch,view) 쌍",
      },
      {
        lines: [8, 14],
        color: "emerald",
        note: "leader·proposal·deadlines — 리더 정보, 제안 슬롯, 타이머 3종",
      },
      {
        lines: [17, 30],
        color: "amber",
        note: "인증서 + broadcast 플래그 — notarization/nullification/finalization 각각 보유 여부와 전송 여부를 추적",
      },
    ],
  },
  "enter-view": {
    ...
    annotations: [
      {
        lines: [34, 36],
        color: "sky",
        note: "view <= self.view → false: 이미 지나간 뷰로 후퇴 불가 (단조 증가 보장)",
      },
      {
        lines: [38, 40],
        color: "emerald",
        note: "leader_deadline + certification_deadline: 뷰 진입과 동시에 타이머 시작",
      },
      {
        lines: [42, 44],
        color: "amber",
        note: "self.view = view: 상태 머신의 현재 뷰를 전진. 이후 try_propose()가 이 뷰에서 동작",
      },
    ],
  },
};
