    ...
    annotations: [
      {
        lines: [5, 8],
        color: "sky",
        note: "Config trait — build()로 초기화. 합의가 올바른 participant set으로 호출",
      },
      {
        lines: [11, 16],
        color: "emerald",
        note: "Elector trait — elect(round, certificate) → Leader. cert는 view 1에서만 None",
      },
    ],
  },
  "round-robin": {
    ...
    annotations: [
      {
        lines: [24, 27],
        color: "sky",
        note: "modulo(view, len) — 뷰 번호로 리더 인덱스 결정. O(1) 연산",
      },
    ],
  },
  "random-elector": {
    ...
    annotations: [
      {
        lines: [40, 44],
        color: "sky",
        note: "View 1 fallback — 첫 뷰는 인증서 없으므로 round-robin으로 대체",
      },
      {
        lines: [46, 51],
        color: "emerald",
        note: "VRF 리더 선출 — SHA256(cert) → modulo. 임계값 미만 참여자가 결과 조작 불가",
      },
    ],
  },
};
