...
const STEPS = [
  {
    label: "BFT 프로토콜 진화",
    body: "PBFT(O(n²) 메시지) → Tendermint(O(n), 6Δ timeout) → HotStuff(O(n), 파이프라인) → Simplex(O(n), 3Δ, 리더 대기 없음, 구현 단순)",
  },
  {
    label: "Simplex 정상 경로 (2 라운드)",
    body: "Round 1: Propose(k, x, Cert) → Vote(k, x). Round 2: Finalize(k, x, Cert) → Decide. 즉시 view k+1 이동.",
  },
  {
    label: "Timeout 경로 (3Δ)",
    body: "타이머 만료 → View-change(k) 브로드캐스트. n-f 수집 → view k+1. 새 리더 즉시 제안 (대기 없음).",
  },
];
                  ...
                  <g key={i}>
                    ...
                    {[p.rounds, p.vc, p.wait, p.msg, p.impl].map((v, j) => (
                      <text
                        ...
                        fill={isSx && j === 2 ? CH : "var(--foreground)"}
                      >
                        {v}
                      </text>
                    ))}
