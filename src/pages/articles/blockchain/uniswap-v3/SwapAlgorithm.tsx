  ...
  return (
    <section id="swap-algorithm" className="mb-16 scroll-mt-20">
      ...
      <div className="prose prose-neutral dark:prose-invert max-w-none">
        ...
        <p>
          V2 swap: 단일 x·y=k 곡선에서 단 1회 계산으로 완료
          <br />
          V3 swap: 여러 "활성 유동성 구간"을 넘나듦 —{" "}
          <strong>tick crossing</strong> 발생
          <br />각 tick 경계 통과마다 유동성 L이 변화 (LP 진입/퇴출)
        </p>
          ...
          <div className="p-5 space-y-3">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-sm">
              ...
              <div>
                ...
                <p>
                  <code className="text-xs bg-muted px-1 rounded">tick</code> —
                  현재 tick
                </p>
                <p>
                  <code className="text-xs bg-muted px-1 rounded">
            ...
            <div className="bg-muted rounded-lg p-3 text-sm border border-border">
              ...
              <p>
                <code className="text-xs">
                  while (amountSpecifiedRemaining != 0 &amp;&amp; sqrtPriceX96
                  != sqrtPriceLimitX96)
                </code>
              </p>
              <p className="text-xs text-muted-foreground mt-1">
                입력 양 소진 또는 가격 한계 도달 시까지 구간 순회
              </p>
            </div>
          </div>
        </div>
        <p>
          <strong>while 루프가 핵심</strong>: 입력 양 소진되거나 가격 한계 도달
          시까지 반복
          <br />각 iteration이 <strong>한 tick 구간 처리</strong>
          <br />
          <code>sqrtPriceLimitX96</code>: 슬리피지 보호 — 가격이 이 값 초과 시
          중단
        </p>
          ...
          <div className="p-5 space-y-2">
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 text-sm">
              ...
              <div className="bg-muted rounded-lg p-3">
                ...
                <p className="text-xs">
                  <code className="text-xs bg-background px-1 rounded">
                    ticks.cross() &rarr; liquidityNet
                  </code>
                </p>
                <p className="text-xs text-muted-foreground mt-1">
                  경계 도달 시 L에 진입/퇴출 LP 반영
                </p>
              </div>
            </div>
          </div>
        ...
        <p>
          <strong>6단계 per iteration</strong>: 다음 tick 찾기 → 가격 변환 →
          계산 → 잔여 업데이트 → 수수료 누적 → 크로싱 체크
          <br />
          crossing 시 <code>liquidityNet</code> 적용 — 해당 tick에서 시작/끝나는
          포지션들의 L 변화
        </p>

        <h3 className="text-xl font-semibold mt-8 mb-3">
          computeSwapStep — 한 구간 내 계산
        </h3>
        <div className="not-prose my-4 bg-muted/50 rounded-xl border border-border overflow-hidden">
          <div className="bg-muted px-5 py-2 border-b border-border">
            <span className="text-sm font-semibold">
          ...
          <div className="p-5 space-y-3">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-sm">
              <div>
                ...
                <p>
                  <code className="text-xs bg-muted px-1 rounded">feePips</code>{" "}
                  — 수수료율 (ppm)
                </p>
              </div>
              <div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <div className="bg-emerald-50 dark:bg-emerald-950/30 rounded-lg p-3 border border-emerald-200 dark:border-emerald-800">
                ...
                <p className="text-sm">
                  <code className="text-xs bg-background px-1 rounded">
                    amountRemainingLessFee &ge; amountIn
                  </code>
                </p>
                <p className="text-xs text-muted-foreground mt-1">
                  목표 가격 도달 &rarr; 다음 step 진행
                </p>
              </div>
              <div className="bg-amber-50 dark:bg-amber-950/30 rounded-lg p-3 border border-amber-200 dark:border-amber-800">
                ...
                <p className="text-xs text-muted-foreground mt-1">
                  중간 가격에서 멈춤 &rarr; 루프 종료
                </p>
              </div>
            </div>
            <p className="text-sm border-t border-border pt-2">
              수수료:{" "}
              <code className="text-xs bg-muted px-1 rounded">
                feeAmount = amountIn &times; feePips / (1e6 - feePips)
              </code>{" "}
              — 올림(roundingUp) 적용
            </p>
          </div>
        ...
        <p>
          ...
          1. 입력 양이 충분 → 구간 끝까지 도달, 다음 step 진행
          <br />
        ...
        <p>
          <strong>LP 구간이 조밀 → swap 가스 ↑</strong>
          <br />
          Stable 페어(0.01% tier)는 LP들이 좁은 구간에 집중 → crossing 빈번 →
          가스 ↑<br />
          이를 보완하기 위해 <code>tickSpacing=1</code>인 stable tier는 특별
          최적화
        </p>

        <h3 className="text-xl font-semibold mt-8 mb-3">
          MEV & slippage 보호 — sqrtPriceLimitX96
        </h3>
        <div className="not-prose my-4 bg-muted/50 rounded-xl border border-border overflow-hidden">
          <div className="bg-muted px-5 py-2 border-b border-border">
            <span className="text-sm font-semibold">
              Router — exactInputSingle() 슬리피지 보호
            </span>
          </div>
          <div className="p-5 space-y-3">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-sm">
              <div className="bg-muted rounded-lg p-3">
                <p className="text-xs font-semibold text-muted-foreground mb-1">
                  sqrtPriceLimitX96 처리
                </p>
                <p className="text-xs">
                  <code className="text-xs bg-background px-1 rounded">
                    == 0
                  </code>{" "}
                  &rarr; 한계 없음 (MIN/MAX_SQRT_RATIO 대체)
                </p>
              ...
              <div className="bg-muted rounded-lg p-3">
                ...
                <p className="text-xs mt-1">
                  <code className="text-xs bg-background px-1 rounded">
                    require(amountOut &ge; amountOutMinimum)
                  </code>
                </p>
                <p className="text-xs text-muted-foreground mt-1">
                  미달 시 전체 트랜잭션 revert
        ...
        <p>
          ...
          1. <code>sqrtPriceLimitX96</code>: pool 레벨 가격 한계 — 도달 시 swap
          중단
          <br />
          2. <code>amountOutMinimum</code>: router 레벨 출력 검증 — 미달 시
          revert
        </p>

        <div className="bg-amber-50 dark:bg-amber-950/30 border-l-4 border-amber-400 p-4 my-6 rounded-r-lg">
          <p className="font-semibold mb-2">
            인사이트: V3의 "계산 비용 vs 자본 효율" 트레이드오프
          </p>
          <p>
            V3는 V2 대비 <strong>50-200% 가스 증가</strong> 감수
            <br />
          ...
          <p className="mt-2">
            이 트레이드오프가 정당화되는 조건:
            <br />
            ✓ 유동성 규모가 큼 (swap당 가스 비용이 거래 대비 작음)
            <br />
            ✓ 거래 빈도 높음 (LP 수익 누적)
            <br />✓ 정교한 LP 전략 가능 (알고리즘 기반 재조정)
          </p>
          <p className="mt-2">
            반대: 소규모 swap·저유동성 페어는 V2/V4가 유리
            <br />
            실제: <strong>ETH/USDC 같은 고유동성 페어는 V3가 압도적</strong>,
            exotic 페어는 V2가 더 많이 쓰임
            <br />
