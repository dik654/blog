  ...
  return (
    <section id="tick-math" className="mb-16 scroll-mt-20">
      ...
      <div className="prose prose-neutral dark:prose-invert max-w-none">
        ...
        <p>
          V3는 모든 가격 계산에 <strong>√P (sqrt price)</strong>를 사용
          <br />
          이유: 유동성 수학이 <code>√P</code>로 표현하면 선형이 됨<br />L
          (liquidity) = √(x · y) 관계에서 자연스럽게 도출
        </p>
        ...
        <div className="not-prose my-4 bg-muted/50 rounded-xl border border-border overflow-hidden">
          ...
          <div className="p-5 space-y-3">
            ...
            <p className="text-sm text-muted-foreground">
              각 tick이 1 basis point(0.01%) 가격 차이
            </p>
            ...
            <div className="text-sm space-y-1 pt-2 border-t border-border">
              <p>
                ...
                <M>
                  {
                    "\\text{tick} = \\log_{1.0001}(P) = \\frac{\\ln P}{\\ln 1.0001}"
                  }
                </M>
              </p>
              <p>
                ...
                — 최소 <M>{"\\approx 2.9 \\times 10^{-39}"}</M>, 최대{" "}
                <M>{"\\approx 3.4 \\times 10^{38}"}</M>
              </p>
            </div>
          </div>
        </div>
        <p>
          <strong>1 tick = 0.01% 가격 변화</strong> — 충분히 세밀하면서 계산
          효율적
          <br />
          가격 범위가 극단적으로 넓음 (2e-39 ~ 3e38) — 어떤 토큰 페어도 수용
          <br />
        ...
        <div className="not-prose my-4 bg-muted/50 rounded-xl border border-border overflow-hidden">
          ...
          <div className="p-5 space-y-3">
            ...
            <div className="bg-muted rounded-lg p-3 text-sm">
              ...
              <p>
                <M>{"\\sqrt{1.5} \\approx 1.22474"}</M>
              </p>
              <p className="mt-1">
                <code className="text-xs bg-background px-1 rounded">
                  sqrtPriceX96
                </code>{" "}
                = 1.22474 &times; 2<sup>96</sup> &asymp; 9.7 &times; 10
                <sup>28</sup>
              </p>
            </div>
            <div className="border-t border-border pt-3">
              <p className="text-sm font-semibold mb-2">
                tick &harr; sqrtPriceX96 변환
              </p>
              <p className="text-sm">
                <code className="text-xs bg-muted px-1 rounded">
                ...
                &rarr;{" "}
                <code className="text-xs bg-muted px-1 rounded">uint160</code>
              </p>
              <ul className="text-sm mt-2 space-y-1 text-muted-foreground">
                <li>
                  절대값 tick을 비트 분해 &rarr; 20개 상수 부분곱으로{" "}
                  <M>{"1.0001^{2^k}"}</M> 계산
                </li>
                <li>
                  Q128.128 형식 상수 사용 &rarr; O(1) 연산으로 고정밀 변환
                </li>
                <li>
                  양수 tick이면 역수 변환:{" "}
                  <code className="text-xs bg-muted px-1 rounded">
        ...
        <p>
          <strong>Q64.96</strong>: 160비트 고정소수점 — uint160 주소 크기와 일치
          <br />
          <code>getSqrtRatioAtTick</code>: 20개 상수 곱으로 tick→√P 변환 — O(log
          n) 대신 O(1)
          <br />각 상수는 <code>1.0001^(2^k)</code>를 Q128.128 형식으로 인코딩
        </p>
        ...
        <div className="not-prose my-4 bg-muted/50 rounded-xl border border-border overflow-hidden">
          <div className="bg-muted px-5 py-2 border-b border-border">
            <span className="text-sm font-semibold">
              가격 구간 [P_a, P_b]에 L 예치 시 필요 토큰 양
            </span>
          </div>
          <div className="p-5 grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div className="bg-muted rounded-lg p-4">
              ...
              <M display>
                {"y = L \\cdot \\left(\\sqrt{P_b} - \\sqrt{P_a}\\right)"}
              </M>
              <p className="text-sm text-center mt-1">
                <code className="text-xs bg-background px-1 rounded">
                  x = 0
        ...
        <p>
          <strong>3가지 case</strong>: 가격이 구간 아래/안/위 중 어디인지에 따라
          분기
          <br />
          구간 내: 양쪽 토큰 모두 필요
          <br />
        ...
        <div className="not-prose my-4 bg-muted/50 rounded-xl border border-border overflow-hidden">
          <div className="bg-muted px-5 py-2 border-b border-border">
            <span className="text-sm font-semibold">
              Swap 시 &radic;P 증분 공식
            </span>
          </div>
          <div className="p-5 grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <p className="text-xs font-semibold text-muted-foreground mb-2">
                &Delta;x 입력 시 &radic;P 변화
              </p>
              <M display>
                {
            ...
            <div>
              <p className="text-xs font-semibold text-muted-foreground mb-2">
                &Delta;y 입력 시 &radic;P 변화
              </p>
              <M display>
                {"\\sqrt{P_{new}} = \\sqrt{P} + \\frac{\\Delta y}{L}"}
              </M>
            </div>
            <div className="md:col-span-2 border-t border-border pt-3">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                <M display>
                  {
                    "\\Delta x = L \\cdot \\left(\\frac{1}{\\sqrt{P_{new}}} - \\frac{1}{\\sqrt{P}}\\right)"
                  }
                </M>
                <M display>
                  {
                    "\\Delta y = L \\cdot \\left(\\sqrt{P_{new}} - \\sqrt{P}\\right)"
                  }
                </M>
              </div>
            </div>
        ...
        <p>
          <strong>L 고정, √P 업데이트</strong>: 스왑은 유동성 L을 변화시키지
          않음
          <br />
          가격 변화 = reserve 비율 변화 = √P 변화
          <br />
          구간 내에서는 V2의 x·y=k와 동일한 수학 적용
        </p>

        <h3 className="text-xl font-semibold mt-8 mb-3">
          Tick Bitmap — 초기화된 tick 추적
        </h3>
          ...
          <div className="p-5 space-y-3">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <div className="bg-muted rounded-lg p-3 text-sm">
                ...
                <p>
                  tick &rarr; 16비트 <strong>word</strong> + 8비트{" "}
                  <strong>bit</strong>
                </p>
                <p className="mt-1 text-muted-foreground text-xs">
                  tick 200: word=0, bit=200
                </p>
        ...
        <p>
          <strong>Tick Bitmap</strong>: 2560개 tick을 256비트 word 하나에
          packing
          <br />
          bit 연산으로 다음 초기화된 tick 찾기 — O(1) with bit tricks
          <br />
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mt-4 text-sm">
              <div className="bg-emerald-50 dark:bg-emerald-950/30 rounded-lg p-3 border border-emerald-200 dark:border-emerald-800">
                ...
                <p className="text-sm mt-1">
                  세밀한 구간 가능, tick crossing 빈번 &rarr; 가스 상승
                </p>
              </div>
              <div className="bg-amber-50 dark:bg-amber-950/30 rounded-lg p-3 border border-amber-200 dark:border-amber-800">
                ...
                <p className="text-sm mt-1">
                  구간 거칠어짐, tick crossing 드묾 &rarr; 가스 절감
                </p>
              </div>
            </div>
          </div>
        ...
        <p>
          <strong>스왑 가스 비용이 tick spacing과 반비례</strong>
          <br />
          Stable 페어는 가격 변동 적음 → spacing=1 허용 (0.01% tier)
          <br />
        ...
        <div className="bg-amber-50 dark:bg-amber-950/30 border-l-4 border-amber-400 p-4 my-6 rounded-r-lg">
          ...
          <p className="mt-2">
            ✓ <strong>선형 관계</strong>: Δy = L·Δ(√P), Δx = L·Δ(1/√P)
            <br />✓ <strong>가산성</strong>: 인접 구간 유동성을 선형 합산 가능
            <br />✓ <strong>정밀도</strong>: 가격의 제곱근이 극단값에서 덜 왜곡
          </p>
          <p className="mt-2">
            <strong>대안 (P 직접 사용)</strong>:<br />
            - 비선형 관계 — 구간별 계산 복잡
            <br />- 정밀도 손실 — 가격이 극단적으로 클 때 overflow
          </p>
          <p className="mt-2">
            √P는 <strong>수학적 필연</strong> — Concentrated Liquidity의 가장
            깔끔한 표현
            <br />이 선택이 V3의 gas 효율과 정밀도를 모두 가능하게 함
          </p>
        </div>
      </div>
    </section>
