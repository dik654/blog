    ...
    <section id="setup-detail" className="mb-16 scroll-mt-20">
      ...
      <div className="prose prose-neutral dark:prose-invert max-w-none">
        <p>
          Setup 과정은{" "}
          <strong>
            Toxic Waste 생성 → 회로 합성 → QAP 변환 → 키 계산 → MPC
          </strong>
          의 5단계로 구성됩니다. 각 단계의 구현 세부사항을 살펴봅니다.
        </p>
        ...
        <div className="rounded-lg border p-4 not-prose text-sm space-y-3">
          <h4 className="font-semibold text-base mb-2">
            SynthesisMode::Setup → R1CS → QAP
          </h4>
          <div className="bg-sky-50 dark:bg-sky-950/30 rounded p-3 space-y-1">
            <p className="text-xs font-medium text-muted-foreground mb-1">
              Setup 모드: 값 없이 구조만 수집
            </p>
            <p className="text-xs">
              <code>cs.alloc()</code> → 변수 개수만 카운트 (값 미할당)
            </p>
            <p className="text-xs">
              <code>enforce_constraint()</code> → R1CS 매트릭스 A, B, C 수집
            </p>
          </div>
          <div className="bg-emerald-50 dark:bg-emerald-950/30 rounded p-3 space-y-1">
            <p className="text-xs font-medium text-muted-foreground mb-1">
              Lagrange 보간으로 다항식 생성
            </p>
            <p className="text-xs">
        ...
        <div className="rounded-lg border p-4 not-prose text-sm space-y-3">
          ...
          <div className="bg-amber-50 dark:bg-amber-950/30 rounded p-3 space-y-1">
            ...
            <p className="font-mono text-xs">
              <code>a_query[j]</code> = [aⱼ(τ)]₁ — A 계산용
            </p>
            <p className="font-mono text-xs">
              <code>b_g1_query[j]</code> = [bⱼ(τ)]₁ — C 계산용
            </p>
            <p className="font-mono text-xs">
              <code>b_g2_query[j]</code> = [bⱼ(τ)]₂ — B 계산용
            </p>
            <p className="font-mono text-xs">
              <code>h_query[i]</code> = [τⁱ · t(τ) / δ]₁ — h(x) 증명용
            </p>
            <p className="font-mono text-xs">
              <code>l_query[j']</code> = [lcⱼ / δ]₁ — private LC용
            </p>
          </div>
