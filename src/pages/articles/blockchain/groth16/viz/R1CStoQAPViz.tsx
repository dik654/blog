...
const STEPS = [
  {
    label: "R1CS 매트릭스 구성",
    body: "곱셈 게이트를 A, B, C 행렬로 인코딩. 각 행이 하나의 제약을 표현.",
  },
  {
    label: "IFFT 보간 -> QAP 다항식",
    body: "각 열을 단위근 도메인에서 역 FFT하여 다항식 계수로 변환.",
  },
  {
    label: "QAP 다항식 조합",
    body: "witness 벡터 w로 가중합하여 A(x), B(x), C(x) 단일 다항식 생성.",
  },
  {
    label: "Vanishing Polynomial t(x)",
    body: "t(x) = x^m - 1. 모든 단위근에서 0이 되어 R1CS 만족 여부를 다항식 나눗셈으로 판별.",
  },
  {
    label: "h(x) 몫 다항식 계산",
    body: "h(x) = (A*B - C) / t(x). 코셋 FFT로 O(n log n)에 수행.",
  },
];
        ...
        >
                    ...
                    return (
                      <g key={ci}>
                        <rect
                          ...
                          stroke={v === "1" ? C.r1cs : "var(--border)"}
                          strokeWidth={v === "1" ? 1.2 : 0.5}
                        />
                        <text
                          x={bx + 12}
                          y={74}
                          ...
                          fill={v === "1" ? C.r1cs : "var(--muted-foreground)"}
                        >
                          {v}
                        </text>
                      </g>
          ...
          {step === 1 && (
            ...
            >
              ...
              {/* Domain note */}
              <text x={360} y={55} fontSize={9} fill={C.ifft}>
                domain = unit roots
              </text>
              <text x={360} y={70} fontSize={8} fill="var(--muted-foreground)">
                각 변수 j마다 보간
              </text>
            </motion.g>
          )}
          ...
          {step === 2 && (
            ...
            >
              ...
              {/* Formula */}
              <text x={410} y={55} fontSize={9} fill={C.qap}>
                A*B - C = 0
              </text>
              <text x={410} y={70} fontSize={8} fill="var(--muted-foreground)">
                at all roots
              </text>
            </motion.g>
          )}
          ...
          {step === 4 && (
            ...
            >
              ...
              {/* Coset FFT pipeline */}
              <text x={100} y={85} fontSize={9} fontWeight={600} fill={C.h}>
                Coset FFT pipeline
              </text>
