...
const STEPS = [
  {
    label: "IC 벡터 합산 -- 공개 입력 결합",
    body: "ic[0] + s1*ic[1] + ... 으로 공개 입력을 하나의 G1 포인트로. 검증자의 유일한 O(l) 연산.",
  },
  {
    label: "LHS: e(A, B) 페어링",
    body: "proof의 A, B를 페어링하여 GT 원소 생성. miller loop + final exponentiation.",
  },
  {
    label: "RHS: 3개 페어링 곱",
    body: "e(alpha,beta) 캐시 + e(IC,gamma) + e(C,delta). 실제 페어링 2회만 실행.",
  },
  {
    label: "LHS == RHS -> Accept",
    body: "GT 원소(Fp12) 12좌표 비교. 약 4ms, 온체인 280k gas.",
  },
];
      ...
      {(step) => (
        ...
        >
          {step === 0 && (
            ...
            >
              {/* IC vector elements */}
              <text x={20} y={16} fontSize={10} fontWeight={600} fill={C.ic}>
                IC vector (from VK)
              </text>
              ...
              {/* Public inputs */}
              <text x={20} y={75} fontSize={9} fill="var(--muted-foreground)">
                public inputs:
              </text>
              {["s1", "s2", "..."].map((label, i) => (
                <motion.g
              ...
              <ActionBox
                ...
                w={160}
                h={36}
                label="MSM (public inputs)"
                sub="O(l) -- only linear op"
                color={C.ic}
              />
            ...
            >
              ...
              {[
                ...
                >
                  ...
                  {p.cached && (
                    <text x={190} y={p.y + 24} fontSize={8} fill={C.rhs}>
                      cached
                    </text>
                  )}
                </motion.g>
