...
const STEPS = [
  {
    label: "Toxic Waste 생성",
    body: "5개 비밀 스칼라를 BN254 Fr 필드에서 균일 랜덤 추출.",
  },
  {
    label: "SynthesisMode::Setup -- R1CS 수집",
    body: "값 할당 없이 구조(변수 인덱스 + 계수)만 희소 매트릭스로 기록.",
  },
  {
    label: "IFFT 보간 -- QAP 다항식",
    body: "각 변수 j에 대해 열 벡터를 단위근 도메인에서 역 FFT하여 다항식 복원.",
  },
  {
    label: "MSM 배치 -- query 벡터 생성",
    body: "변수별 다항식 계수와 SRS tau 포인트를 MSM으로 결합하여 PK/VK 구성.",
  },
  {
    label: "MPC 세레모니",
    body: "Phase1(Powers of Tau) + Phase2(회로별 주입). N명 중 1명만 정직하면 안전.",
  },
];
                ...
                {[0, 1, 2].map((col) => (
                  <g key={col}>
                    <rect
                      ...
                      stroke={col === 1 ? C.r1cs : "var(--border)"}
                      strokeWidth={col === 1 ? 1 : 0.5}
                    />
                    <text
                      x={429 + col * 20}
                      y={21}
                      textAnchor="middle"
                      fontSize={8}
                      fill={col === 1 ? C.r1cs : "var(--muted-foreground)"}
                    >
                      {col === 1 ? "1" : "0"}
                    </text>
                  </g>
            ...
            >
              ...
              >
                ...
                <ModuleBox
                  ...
                  w={180}
                  h={36}
                  label="Phase 2: Circuit-specific"
