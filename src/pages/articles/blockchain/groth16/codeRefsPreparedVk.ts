...
export const preparedVkCodeRefs: Record<string, CodeRef> = {
  "groth16-prepared-vk": {
    ...
    lang: "rust",
    highlight: [1, 22],
    desc: `PreparedVerifyingKey는 검증 최적화를 위한 사전 계산 구조체입니다.
    ...
    annotations: [
      {
        lines: [5, 6],
        color: "sky",
        note: "e(α, β) 사전 계산 — 모든 검증에서 재사용",
      },
      {
        lines: [7, 12],
        color: "emerald",
        note: "G2 negation 사전 계산 — 검증 시 부정 연산 절약",
      },
      {
        lines: [17, 17],
        color: "amber",
        note: "pairing(α, β) — 1회 계산 후 캐싱",
      },
      {
        lines: [19, 21],
        color: "violet",
        note: "G2Prepared::from(-g2) — Miller loop 최적화용 전처리",
      },
    ],
  },
};
