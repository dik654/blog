  ...
  return (
    <section id="proving-detail" className="mb-16 scroll-mt-20">
      ...
      <div className="prose prose-neutral dark:prose-invert max-w-none">
        <p>
          증명 생성의 핵심은{" "}
          <strong>Witness 할당 → A,B,C 원소의 MSM 계산</strong>입니다.
          <br />각 원소의 수학적 구성과 멀티스레드 최적화를 상세히 분석합니다.
        </p>
        ...
        <div className="rounded-lg border p-4 not-prose text-sm space-y-2">
          ...
          <div className="bg-sky-50 dark:bg-sky-950/30 rounded p-3 space-y-1">
            ...
            <p className="text-xs">
              <code>cs.alloc()</code> → 실제 값 할당 (witness 계산)
            </p>
            <p className="text-xs">
              <code>enforce_constraint()</code> → 스킵 (Setup에서 이미 수집)
            </p>
          </div>
        ...
        <div className="rounded-lg border p-4 not-prose text-sm space-y-2">
          ...
          <M display>
            {
              "A = [\\alpha]_1 + \\sum_j w_j \\cdot a\\_query[j] + r \\cdot [\\delta]_1"
            }
          </M>
          ...
          <p className="text-xs text-muted-foreground mt-2">
            MSM: Pippenger 윈도우 방식 + 멀티스레드 → <code>rayon::scope</code>
            로 각 윈도우 병렬 처리
          </p>
        </div>
        ...
        <div className="rounded-lg border p-4 not-prose text-sm space-y-3">
          ...
          <div className="bg-emerald-50 dark:bg-emerald-950/30 rounded p-3 space-y-1">
            ...
            <M display>
              {
                "B_{g2} = [\\beta]_2 + \\sum_j w_j \\cdot b\\_g2\\_query[j] + s \\cdot [\\delta]_2"
              }
            </M>
            <M display>
              {
                "B_{g1} = [\\beta]_1 + \\sum_j w_j \\cdot b\\_g1\\_query[j] + s \\cdot [\\delta]_1"
              }
            </M>
          </div>
          <div className="bg-violet-50 dark:bg-violet-950/30 rounded p-3 space-y-1">
            ...
            <p className="text-xs text-muted-foreground">
              → 두 MSM을 <code>rayon::join</code>으로 동시 실행
            </p>
          </div>
        </div>
        ...
        <div className="rounded-lg border p-4 not-prose text-sm space-y-3">
          <h4 className="font-semibold text-base mb-2">
            C = private LC + h·t + 블라인딩
          </h4>
          <div className="bg-sky-50 dark:bg-sky-950/30 rounded p-3 space-y-1">
            <p className="text-xs font-medium mb-1">3개 MSM 구성 요소</p>
            <p className="font-mono text-xs">
              <M>{"\\sum_{j \\in priv} w_j \\cdot l\\_query[j\\,']"}</M> —
              private LC
            </p>
            <p className="font-mono text-xs">
              <M>{"\\sum_i h_i \\cdot h\\_query[i]"}</M> — QAP 만족 증거
            </p>
            <p className="font-mono text-xs">
              <M>{"s \\cdot A + r \\cdot B_{g1} - rs \\cdot [\\delta]_1"}</M> —
              블라인딩
            </p>
          </div>
