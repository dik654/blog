...
const STEPS = [
  {
    label: "Merkle Tree 구조",
    body: "리프에 데이터 해시, 내부 노드에 자식 해시의 해시를 저장.\n루트 하나로 전체 데이터 무결성 검증.",
  },
  {
    label: "멤버십 증명 (Inclusion Proof)",
    body: "특정 데이터가 트리에 포함됨을 증명.\nsibling 해시 log n개 + 루트만 있으면 검증 가능.",
  },
  {
    label: "ZKP에서의 활용",
    body: "ZK 회로 안에서 Merkle 경로를 검증.\nPoseidon 해시 × 트리 깊이(log n)로 멤버십 증명.",
  },
];
        ...
        >
          ...
          >
            {(LEVEL_NOTES[step] ?? []).map(([y, text, color]) => (
              <text key={y} x={275} y={y + 4} fontSize={11} fill={color}>
                {text}
              </text>
            ))}
          </motion.g>
