...
const STEPS = [
  {
    label: "Sponge 전체 흐름",
    body: "rate에만 입출력, cap은 절대 외부 비노출.",
  },
  {
    label: "1. 상태 초기화",
    body: "상태 전체 0. rate=입출력 창구, cap=보안 버퍼.",
  },
  {
    label: "2. 흡수 — XOR",
    body: "메시지 블록 m₁을 rate 부분에 XOR.\ncap은 변경하지 않는다.",
  },
  {
    label: "3. 흡수 — 순열 π",
    body: "XOR 후 전체 상태에 순열 π를 적용.\nrate + cap 전체를 뒤섞는 고정 함수.",
  },
  {
    label: "4. 흡수 반복",
    body: "다음 블록 m₂도 같은 방식: rate에 XOR → π.\n모든 메시지 블록을 처리할 때까지 반복.",
  },
  {
    label: "5. 압출 (Squeeze)",
    body: "모든 블록 흡수 후 rate에서 출력을 복사해 추출.\ncap은 끝까지 비노출.",
  },
];
...
function StateRow({
  ...
  const capCX =
    x0 + rate.length * (BW + GAP) + (cap.length * (BW + GAP) - GAP) / 2;
    ...
    <g>
      ...
      {showLabels && (
        <>
          ...
          <text x={capCX} y={y - 6} textAnchor="middle" fontSize={11} fill={C2}>
            cap
          </text>
        </>
      )}
      ...
      {cap.map((v, i) => (
        <g key={`c${i}`}>
          ...
          />
          <text
            x={x0 + (rate.length + i) * (BW + GAP) + BW / 2}
            y={y + BH / 2 + 6}
            textAnchor="middle"
            fontSize={13}
            fontWeight={600}
        ...
        >
          ...
          {step === 4 && (
            <motion.g initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
              ...
              <text x={CX} y={172} textAnchor="middle" fontSize={13} fill={C3}>
                π 적용
              </text>
