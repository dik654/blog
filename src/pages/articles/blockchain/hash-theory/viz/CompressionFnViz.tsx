...
const STEPS = [
  {
    label: "전체 흐름",
    body: "압축함수 f는 이전 해시 + 메시지 블록을 받아 64번 섞은 뒤 새 해시를 출력한다.",
  },
  {
    label: "입력 준비",
    body: "이전 해시 → 8개 변수 a~h. 메시지 → 확장 → 64개 워드.",
  },
  {
    label: "워드 확장",
    body: "원본 16워드에서 σ₀·σ₁ 비트 회전 혼합으로 48개 추가 생성.",
  },
  {
    label: "라운드 내부",
    body: "a~h + Wₜ + Kₜ → T₁·T₂ 경로 혼합. a'와 e' 갱신, 나머지 시프트.",
  },
  {
    label: "64회 반복",
    body: '같은 라운드 구조를 64번 반복한다. 매번 다른 Wₜ와 Kₜ가 주입된다.\n위 "라운드 내부" 동작이 64번 연쇄 적용되는 것.',
  },
  {
    label: "최종 덧셈",
    body: "64라운드 후 결과를 원래 입력에 더해서 이 블록의 출력 hᵢ를 만든다.",
  },
];
          ...
          {step === 4 && (
            <motion.g initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
              ...
              <text
                ...
                fill="var(--muted-foreground)"
              >
                매 라운드마다 a와 e 자리에 새 값이 계산됨 — 64번 반복 후 모든
                값이 완전히 변환
              </text>
            </motion.g>
          )}
