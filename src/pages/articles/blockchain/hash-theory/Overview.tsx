      ...
      <div className="prose prose-neutral dark:prose-invert max-w-none mt-6">
        ...
        <div className="not-prose rounded-lg border bg-card p-4 mb-4">
          ...
          <p className="text-sm text-muted-foreground">
            <M>{"H: \\{0,1\\}^* \\to \\{0,1\\}^n"}</M> &mdash; 임의 길이 입력
            &rarr; 고정 길이 출력
          </p>
        </div>
        ...
        <div className="not-prose grid grid-cols-1 md:grid-cols-3 gap-3 mb-6">
          <div className="rounded-lg border bg-card p-4">
            ...
            <p className="text-sm text-muted-foreground">
              ...
              복잡도: <M>{"2^n"}</M> (brute force). SHA-256: <M>{"2^{256}"}</M>{" "}
              시도 필요.
            </p>
          </div>
          <div className="rounded-lg border bg-card p-4">
            ...
            <p className="text-sm text-muted-foreground">
              주어진 <M>{"x_1"}</M>에 대해 <M>{"H(x_1) = H(x_2)"}</M>인{" "}
              <M>{"x_2 \\neq x_1"}</M> 찾기 어려움. 복잡도: <M>{"2^n"}</M>.
            </p>
          </div>
          <div className="rounded-lg border bg-card p-4">
            <div className="text-sm font-semibold mb-2">
            ...
            <p className="text-sm text-muted-foreground">
              ...
              어려움. 복잡도: <M>{"2^{n/2}"}</M> (생일 공격). 가장 강한 속성.
            </p>
          </div>
        </div>
        ...
        <div className="not-prose rounded-lg border-l-4 border-l-amber-500 bg-card p-4 mb-6">
          ...
          <p className="text-sm text-muted-foreground mb-1">
            <M>n</M>명 중 생일이 겹치는 사람이 있을 확률 &mdash; 23명이면 50%
            (일반 직관: 183명).
          </p>
          <p className="text-sm text-muted-foreground">
            해시 충돌: <M>{"2^{n/2}"}</M> 샘플에서 50% 확률로 충돌. SHA-256 (
            <M>n=256</M>): <M>{"2^{128}"}</M> samples / MD5 (<M>n=128</M>):{" "}
            <M>{"2^{64}"}</M> samples (이미 깨짐).
          </p>
        </div>
          ...
          <table className="w-full text-sm border-collapse">
            ...
            <tbody className="text-muted-foreground">
              <tr className="border-b border-border/50">
                <td className="p-2">MD5 (1991)</td>
                <td className="p-2">128-bit</td>
                <td className="p-2 text-red-500">깨짐 (2004)</td>
              </tr>
              <tr className="border-b border-border/50">
                <td className="p-2">SHA-1 (1995)</td>
                <td className="p-2">160-bit</td>
                <td className="p-2 text-red-500">깨짐 (2017)</td>
              </tr>
              <tr className="border-b border-border/50">
                <td className="p-2">SHA-2 (2001)</td>
                <td className="p-2">224/256/384/512-bit</td>
                <td className="p-2 text-emerald-500">안전</td>
              </tr>
              <tr className="border-b border-border/50">
                <td className="p-2">SHA-3 (2015)</td>
                <td className="p-2">Keccak 기반</td>
                <td className="p-2 text-emerald-500">안전</td>
              </tr>
              <tr>
                <td className="p-2">BLAKE2/3 (2012)</td>
                <td className="p-2">가변</td>
                <td className="p-2 text-emerald-500">안전, SHA-2보다 빠름</td>
              </tr>
            </tbody>
          </table>
        </div>
        ...
        <div className="not-prose grid grid-cols-1 sm:grid-cols-2 gap-3 mb-6">
          <div className="rounded-lg border bg-card p-4">
            ...
            <p className="text-sm text-muted-foreground">
              블록 해시 = 블록 식별자. 이전 블록 해시 포함 &rarr; 체인 구조.
              변조 즉시 탐지.
            </p>
          </div>
          <div className="rounded-lg border bg-card p-4">
            ...
            <p className="text-sm text-muted-foreground">
              nonce를 찾아 <M>{"H(\\text{block}) < \\text{target}"}</M>.
              일방향성 활용, 계산 비용 = 채굴 난이도.
            </p>
          </div>
          <div className="rounded-lg border bg-card p-4">
            <div className="text-sm font-semibold mb-2">Merkle Root</div>
            <p className="text-sm text-muted-foreground">
              거래들의 해시 트리 루트. SPV (Simple Payment Verification).{" "}
              <M>{"O(\\log n)"}</M> 증명.
            </p>
          </div>
          <div className="rounded-lg border bg-card p-4">
            ...
            <p className="text-sm text-muted-foreground">
              Bitcoin: <code>RIPEMD-160(SHA-256(pubkey))</code>
              <br />
          ...
          <div className="rounded-lg border bg-card p-4">
            ...
            <p className="text-sm text-muted-foreground">
              <code>sign(hash(message))</code> &mdash; 전체 메시지가 아닌 해시에
              서명. 효율성 + 결정성.
            </p>
          </div>
        </div>
          ...
          <table className="w-full text-sm border-collapse">
            ...
            <tbody className="text-muted-foreground">
              <tr className="border-b border-border/50">
                ...
                <td className="p-2">SHA-256 (double)</td>
              </tr>
              <tr className="border-b border-border/50">
                <td className="p-2">Ethereum</td>
                <td className="p-2">Keccak-256 (SHA-3 변형)</td>
              </tr>
              <tr className="border-b border-border/50">
                <td className="p-2">Zcash</td>
                <td className="p-2">BLAKE2b</td>
              </tr>
              <tr className="border-b border-border/50">
                <td className="p-2">Solana</td>
                <td className="p-2">SHA-256, Poseidon (ZK)</td>
              </tr>
              <tr className="border-b border-border/50">
                <td className="p-2">Filecoin</td>
                <td className="p-2">SHA-256, Poseidon, BLAKE2b</td>
              </tr>
              <tr>
                <td className="p-2">Ethereum 2.0</td>
