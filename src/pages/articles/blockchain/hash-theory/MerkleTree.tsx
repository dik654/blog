  ...
  return (
    <section id="merkle-tree" className="mb-16 scroll-mt-20">
      ...
      <div className="prose prose-neutral dark:prose-invert max-w-none mb-6">
        <p>루트 해시 하나로 전체 데이터 무결성 증명. 증명 크기 O(log n).</p>
      </div>
      ...
      <div className="prose prose-neutral dark:prose-invert max-w-none mt-6">
        ...
        <div className="not-prose rounded-lg border bg-card p-4 mb-4">
          ...
          <ul className="text-sm text-muted-foreground space-y-1">
            <li>
              Leaf: <M>{"\\text{leaf}_i = H(\\text{data}_i)"}</M>
            </li>
            <li>
              Node: <M>{"\\text{node} = H(\\text{left} \\| \\text{right})"}</M>
            </li>
            <li>Root: 최상위 node</li>
          </ul>
        </div>
        ...
        <div className="not-prose rounded-lg border-l-4 border-l-blue-500 bg-card p-4 mb-4">
          ...
          <div className="text-sm text-muted-foreground space-y-1">
            ...
            <p>
              검증: <M>{"h_1 = H(D_1)"}</M> &rarr;{" "}
              <M>{"h_{12} = H(h_1 \\| H_2)"}</M> &rarr;{" "}
              <M>{"\\text{root}' = H(h_{12} \\| H_{34})"}</M>
            </p>
            <p>
              <M>{"\\text{root}' = \\text{root}"}</M> 확인
            </p>
          </div>
        </div>
        ...
        <div className="not-prose grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3 mb-6">
          ...
          <div className="rounded-lg border bg-card p-4">
            ...
            <p className="text-sm text-muted-foreground">
              거래 배치 커밋, Fraud/Validity proofs
            </p>
          </div>
          <div className="rounded-lg border bg-card p-4">
            <div className="text-sm font-semibold mb-1">IPFS / Filecoin</div>
            <p className="text-sm text-muted-foreground">
              Content addressing, CID = root hash
            </p>
          </div>
        </div>
        ...
        <div className="not-prose rounded-lg border bg-card p-4 mb-6">
          ...
          <p className="text-sm text-muted-foreground mb-1">
            Modified Merkle-Patricia &mdash; 각 블록 헤더에 root 포함
          </p>
        ...
        <div className="not-prose grid grid-cols-1 sm:grid-cols-2 gap-3 mb-6">
          <div className="rounded-lg border bg-card p-4">
            ...
            <ul className="text-sm text-muted-foreground space-y-1">
              ...
              <li>
                <M>{"O(\\log n)"}</M> depth (<M>n</M> = leaves)
              </li>
              <li>추가 시 트리 재구성</li>
            </ul>
          </div>
        ...
        <div className="not-prose rounded-lg border bg-card p-4 mb-4">
          ...
          <div className="text-sm text-muted-foreground space-y-1">
            <p>
              Address space: <M>{"2^{256}"}</M> (SHA-256 key 기준) / Depth: 256
              / Leaves: <M>{"2^{256}"}</M> (대부분 비어있음)
            </p>
            <p>
              <code>key=0x01...</code> &rarr; path:{" "}
              <code>[0,0,0,0,0,0,0,1, ...]</code> (비트별로 left/right 결정)
            </p>
          </div>
        </div>
        ...
        <div className="not-prose grid grid-cols-1 sm:grid-cols-2 gap-3 mb-6">
          ...
          <div className="rounded-lg border-l-4 border-l-blue-500 bg-card p-4">
            ...
            <p className="text-sm text-muted-foreground">
              "key X는 트리에 없다" &mdash; Merkle path까지 empty hash로 정확한
              증명 가능. 일반 Merkle Tree에서는 불가.
            </p>
          </div>
        </div>
