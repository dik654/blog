    ...
    annotations: [
      {
        lines: [72, 74],
        color: "sky",
        note: "lift() -- SegmentReceipt(rv32im STARK)를 재귀회로 STARK로 변환",
      },
      {
        lines: [75, 83],
        color: "emerald",
        note: "재귀회로 실행 -> claim 디코드 -> merge -> SuccinctReceipt 반환",
      },
    ],
    desc: `lift()는 재귀 증명 파이프라인의 첫 번째 단계입니다.
    ...
    annotations: [
      {
        lines: [115, 122],
        color: "sky",
        note: "join() -- 같은 Session의 두 SuccinctReceipt를 하나로 합성",
      },
      {
        lines: [123, 132],
        color: "emerald",
        note: "재귀 prover 실행 -> claim 병합 -> 단일 SuccinctReceipt 반환",
      },
    ],
    desc: `join()은 같은 세션의 두 SuccinctReceipt를 하나로 합칩니다.
    ...
    annotations: [
      {
        lines: [357, 364],
        color: "sky",
        note: "identity_p254 -- Poseidon BabyBear → Poseidon BN254 해시 전환",
      },
      {
        lines: [365, 389],
        color: "emerald",
        note: "BN254 기반 증명 생성 + Merkle inclusion proof 구성",
      },
    ],
    desc: `identity_p254()는 Groth16 변환 직전의 마지막 재귀 단계입니다.
