    ...
    annotations: [
      {
        lines: [53, 56],
        color: "sky",
        note: "prove() 진입 -- VerifierContext 생성 후 prove_with_ctx 위임",
      },
      {
        lines: [58, 65],
        color: "emerald",
        note: "ELF 바이너리 실행 -> Session 생성 -> prove_session 호출",
      },
    ],
    desc: `Prover의 최상위 진입점입니다. prove()는 ExecutorEnv(호스트 환경)와 ELF 바이너리를 받아:
    ...
    annotations: [
      {
        lines: [68, 81],
        color: "sky",
        note: "prove_session 진입 -- poseidon2 해시 함수 검증",
      },
      {
        lines: [86, 96],
        color: "emerald",
        note: "세그먼트 순회 -- 각 segment를 개별 STARK 증명으로 변환",
      },
      {
        lines: [146, 161],
        color: "amber",
        note: "CompositeReceipt 조립 -- segments + assumptions + verifier_parameters",
      },
      {
        lines: [163, 173],
        color: "violet",
        note: "ReceiptKind::Composite면 여기서 반환 (재귀 압축 생략)",
      },
    ],
    desc: `prove_session()은 실행 트레이스(Session)를 받아 증명을 만드는 핵심 로직입니다.
    ...
    annotations: [
      {
        lines: [175, 182],
        color: "sky",
        note: "Succinct 압축 -- composite_to_succinct()로 재귀 STARK 압축",
      },
      {
        lines: [184, 194],
        color: "emerald",
        note: "ReceiptKind::Succinct면 여기서 반환",
      },
      {
        lines: [196, 208],
        color: "amber",
        note: "Groth16 변환 -- succinct_to_groth16()으로 SNARK 생성 (~200바이트)",
      },
    ],
    desc: `증명 파이프라인의 마지막 단계입니다.
