  ...
  return (
    <section id="fp2" className="mb-16 scroll-mt-20">
      ...
      <div className="prose prose-neutral dark:prose-invert max-w-none">
        <p>
          ...
          의 해가 실수에 없어서 복소수가 필요한 것처럼, Fp를 확장한{" "}
          <strong>Fp2</strong> 위에서 해를 찾는다.
        </p>
        ...
        <CodePanel
          title="Fp2::mul — Karatsuba"
          code={`pub fn mul(&self, rhs: &Fp2) -> Fp2 {
    ...
    Fp2 { c0, c1 }
        ...
        <p>
          ...
          (복소수 켤레와 동일). <strong>Norm</strong>:{" "}
          <code className="bg-accent px-1.5 py-0.5 rounded text-sm">
            a·conj(a) = a₀²+a₁² ∈ Fp
          </code>{" "}
          — 결과가 Fp2가 아니라 <strong>Fp</strong>로 내려간다.
        </p>
        <p>
          <strong>Inverse</strong>: Fp2에서 직접 역원을 구하면{" "}
          <code className="bg-accent px-1.5 py-0.5 rounded text-sm">p²−2</code>
          승이 필요하지만,{" "}
          <code className="bg-accent px-1.5 py-0.5 rounded text-sm">
            a⁻¹ = conj(a) / norm(a)
          </code>
          로 쓰면 Fp의 inv(Fermat p−2승) 1번 + Fp 곱셈 2번으로 끝난다.
        </p>
        ...
        <p>
          ...
          (p ≡ 3 mod 4)이므로 Frobenius(x→x^p)는 conjugate와 동일하다.
          <br />
          Frobenius를 2번 적용하면 원래 값으로 돌아오는데, Fp2가 Fp의{" "}
          <strong>2차</strong> 확장이기 때문이다.
        </p>
      </div>
    </section>
  );
