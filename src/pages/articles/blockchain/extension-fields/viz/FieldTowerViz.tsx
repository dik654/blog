...
const STEPS = [
  {
    label: "Fp: 기초 소수체 (256 bit)",
    body: "BN254의 기초. G1 점의 좌표. 모든 확장의 토대가 됩니다.",
  },
  {
    label: "Fp → Fp2: 이차 확장",
    body: "u²=-1로 확장. a₀+a₁u 형태. 복소수처럼 동작. G2 좌표 공간.",
  },
  {
    label: "Fp2 → Fp6: 삼차 확장",
    body: "v³=ξ(=9+u)로 확장. Fp2 원소 3개로 구성. Karatsuba로 곱셈 최적화.",
  },
  {
    label: "Fp6 → Fp12: 이차 확장 → GT",
    body: "w²=v로 확장. Fp 원소 12개. e(G1,G2) ∈ GT ⊂ Fp12*.",
  },
];
            ...
            return (
              ...
              >
                ...
                {i > 0 && visible && (
                  ...
                  >
                    <text x={x + w + 6} y={l.y + 12} fontSize={9} fill={l.c}>
                      {l.ext}
                    </text>
                  </motion.g>
                )}
                ...
                {visible && (
                  ...
                  >
                    {Array.from({ length: Math.min(Number(l.size), 12) }).map(
                      (_, j) => (
                        <motion.rect
                          key={j}
                          x={210 + j * 10}
