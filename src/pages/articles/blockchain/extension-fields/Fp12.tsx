  ...
  return (
    <section id="fp12" className="mb-16 scroll-mt-20">
      ...
      <div className="prose prose-neutral dark:prose-invert max-w-none">
        ...
        <CodePanel title="타워 vs 직접 구성 비용" code={`직접 구성: Fp12 = Fp[x]/(x¹²−...)  → naive 144 Fp 곱셈
타워 구성: Fp → Fp2 → Fp6 → Fp12   → Karatsuba 적용

  Fp→Fp2:  4회 → 3회
  Fp2→Fp6: 9회 → 6회
  Fp6→Fp12: 4회 → 3회
  합계: 3 × 6 × 3 = 54 Fp 곱셈 (naive 대비 약 1/3)`} defaultOpen annotations={[
          { lines: [1, 2], color: 'sky', note: '144회 vs Karatsuba 적용' },
          { lines: [4, 7], color: 'emerald', note: '각 층에서 Karatsuba 절감' },
        ]} />
        ...
        <CodePanel title="Fp12::mul — Karatsuba" code={`pub fn mul(&self, rhs: &Fp12) -> Fp12 {
    ...
    Fp12 { c0, c1 }
        ...
        <p>
          <strong>Inverse</strong>: norm = c₀² − v·c₁² ∈ <strong>Fp6</strong>으로 내려간다.<br />
          전체 inv 호출 체인은 Fp12 → Fp6 → Fp2 → Fp로, 최종적으로 Fermat 소정리 하나로 귀결된다.
        </p>

        <h3 className="text-xl font-semibold mt-6 mb-3">페어링과의 연결</h3>
        <CodePanel title="페어링에서 Fp12 연산 사용처" code={`e: G1 × G2 → GT ⊂ Fp12*
...
모든 확장체 연산이 페어링에서 사용된다:
  ...
  Fp12::frobenius        → final exp hard part`} annotations={[
