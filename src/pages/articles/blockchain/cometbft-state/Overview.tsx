...
export default function Overview({
  ...
  return (
    <section id="overview" className="mb-16 scroll-mt-20">
      <h2 className="text-2xl font-bold mb-6">State store는 합의 결과와 다음 height의 입력을 이어 준다</h2>
      <div className="not-prose mb-8">
        <ContextViz />
      </div>
      <div className="prose prose-neutral dark:prose-invert max-w-none">
        <p className="leading-7">
          CometBFT는 합의 진행 상태를 State · BlockStore · EvidencePool 세
          책임으로 나누어 저장한다. 이 글에서는 각 계층의 Go 구조체와
          저장·조회 경로를 코드 수준으로 추적한다.
        </p>

        {/* ── 3 계층 storage ── */}
        <h3 className="text-xl font-semibold mt-6 mb-3">
          3 계층 Storage — 각 계층의 역할
        </h3>
        ...
        <div className="not-prose grid grid-cols-1 sm:grid-cols-2 gap-3 mb-6">
          <div className="rounded-lg border bg-card p-4">
            ...
            <ol className="text-sm space-y-1 text-muted-foreground list-decimal list-inside">
              <li>
                <code className="text-xs">loadState(N-1)</code> → state.db 읽기
              </li>
              <li>
                <code className="text-xs">executeBlock(state, block)</code>
              </li>
              <li>
                <code className="text-xs">updateState(block, results)</code>
              </li>
              <li>
                <code className="text-xs">saveState(state)</code> → state.db
                쓰기
              </li>
              <li>
                <code className="text-xs">saveBlock(block)</code> →
                blockstore.db 쓰기
              </li>
            </ol>
          </div>
        ...
        <p className="leading-7">
          State, block, evidence는 access pattern과 보존 기간이 다르므로 저장
          책임을 분리한다. 실제 backend 선택지와 database layout은 build와
          release에 따라 달라질 수 있으니 현재 configuration과 migration 문서를
          함께 확인해야 한다.
        </p>
      </div>
    </section>
