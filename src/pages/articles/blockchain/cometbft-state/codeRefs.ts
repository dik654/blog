...
export const codeRefs: Record<string, CodeRef> = {
    ...
    annotations: [
      {
        lines: [16, 17],
        color: "sky",
        note: "Save() — applyBlock 후 configured DB에 직렬화하는 crash-recovery 기준점",
      },
      {
        lines: [18, 19],
        color: "emerald",
  ...
  "block-store": {
    ...
    lang: "go",
    highlight: [18, 35],
    desc: "BlockStore snapshot — configured DB 위의 block records와 base~height metadata",
    annotations: [
      {
        lines: [19, 19],
        color: "sky",
        note: "db — backend abstraction. 실제 물리 저장소는 node configuration에 따른다",
      },
      {
        lines: [22, 23],
        color: "emerald",
  ...
  "block-save": {
    ...
    annotations: [
      ...
      },
      {
        lines: [54, 60],
        color: "amber",
        note: "BlockMeta와 각 part를 height/index key로 configured DB에 기록",
      },
      {
        lines: [64, 64],
        color: "violet",
