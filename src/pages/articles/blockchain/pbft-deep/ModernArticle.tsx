...
import { PbftNormalViz, PbftRecoveryViz } from "./viz/ModernPbftViz";

export default function ModernPbftArticle(){return <article className="space-y-14">
  ...
  <section id="normal-case" className="space-y-6">
    <header><p className="text-sm font-semibold text-primary">01 · normal case</p><h2 className="mt-2 text-2xl font-bold">PREPARE와 COMMIT은 같은 표를 두 번 세는 단계가 아니라 view 안과 view 사이의 서로 다른 문제를 푼다</h2></header>
    <p>Primary는 request에 sequence를 붙여 PRE-PREPARE를 보냅니다. Backup은 request와 primary signature, digest, current view, 동일 slot의 conflicting PRE-PREPARE 부재, low/high watermark를 검사합니다. 하나라도 틀리면 log를 바꾸지 않습니다. 승인한 backup은 matching PREPARE를 all-to-all로 보냅니다. Paper의 prepared predicate는 local request·PRE-PREPARE와 서로 다른 backups의 matching PREPARE <code>2f</code>개를 요구합니다.</p>
    <p>Prepared가 되면 replica는 COMMIT을 보냅니다. Local replica가 prepared이고 distinct matching COMMIT <code>2f+1</code>개를 받았을 때 committed-local입니다. Network가 순서 없이 messages를 전달해 sequence 43이 먼저 commit될 수도 있지만, replica는 sequence 42까지 실행한 뒤 43을 실행합니다. Consensus arrival와 deterministic application order를 분리하는 이유입니다.</p>
    <ExplainedFormula question="PBFT replica가 slot을 실행 가능 상태로 올리려면 어떤 두 predicate를 차례로 만족해야 하는가?" idea={<>Prepared는 primary proposal과 backups의 동일-slot 관찰을 묶고, committed-local은 그 prepared value가 view change와 다른 replicas에도 남을 quorum을 요구합니다.</>} formula={String.raw`\begin{aligned}prepared_i&=PP_i\land |P_i|\ge 2f\\commit_i&=prepared_i\\&\quad\land |C_i|\ge 2f+1\end{aligned}`}
    annotatedFormula={String.raw`\begin{aligned}\underbrace{prepared_i}_{\text{오른쪽 항으로 결과 계산}}&=\underbrace{PP_i\land |P_i|\ge 2f}_{\text{판정 조건 결합}}\\commit_i&=prepared_i\\&\quad\land |C_i|\ge \underbrace{2f+1}_{\text{판정 조건 결합}}\end{aligned}`}
    operations={[
      { expression: String.raw`PP_i\land |P_i|\ge 2f`, annotation: ["필요한 gate가 모두 참일 때만 전체 조건을 통과시킵니다.","Prepared는 primary proposal과","backups의 동일-slot 관찰을 묶고,","committed-local은 그 prepared value가"] },
      { expression: String.raw`prepared_i`, annotation: ["왼쪽 결과를 오른쪽의 실제 항으로 계산합니다.","Prepared는 primary proposal과","backups의 동일-slot 관찰을 묶고,","committed-local은 그 prepared value가"] },
      { expression: String.raw`2f+1`, annotation: ["Byzantine fault bound이(가) 식의 결과에","기여하는 방식을 계산합니다.","Prepared는 primary proposal과","backups의 동일-slot 관찰을 묶고,"] },
    ]} terms={[{symbol:"PPᵢ",name:"accepted PRE-PREPARE",description:"Replica i가 같은 view·sequence·digest와 request를 검증해 log에 둔 proposal입니다."},{symbol:"Pᵢ",name:"matching PREPARE set",description:"서로 다른 backups가 보낸 같은 tuple의 valid PREPARE messages입니다."},{symbol:"Cᵢ",name:"matching COMMIT set",description:"서로 다른 replicas가 보낸 같은 tuple의 valid COMMIT messages입니다."},{symbol:"f",name:"Byzantine fault bound",description:"3f+1 replicas 가운데 arbitrary behavior를 할 수 있는 최대 수입니다."}]} assumptions={["Message signatures, client timestamp, view·sequence·digest와 watermarks를 먼저 검증합니다.","Signer 중복을 제거하고 paper의 equal-weight 3f+1 model을 사용합니다.","Committed-local 뒤에도 더 낮은 sequence requests를 순서대로 실행해야 합니다.","이 predicate는 client external settlement나 disk durability를 보장하지 않습니다."]} interpretation="n=4,f=1이면 PRE-PREPARE와 backups PREPARE 2개가 prepared를 만들고, local prepared와 COMMIT 3개가 committed-local을 만듭니다. COMMIT 3개만 따로 모아 request를 실행해서는 안 됩니다." />
    <div id="paper-pbft-normal"><CitationBlock source="PBFT paper §4.2 — Normal-Case Operation" citeKey={1} href="https://www.usenix.org/legacy/publications/library/proceedings/osdi99/full_papers/castro/castro_html/node4.html#SECTION00042000000000000000"><p><strong>문제:</strong> Faulty primary가 같은 slot에 다른 requests를 보내더라도 honest replicas의 실행 순서가 갈라지지 않아야 합니다.</p><p><strong>기여:</strong> PRE-PREPARE admission, prepared·committed-local predicates와 ordered execution을 정의합니다.</p><p><strong>전제:</strong> Exact view·sequence·digest domain, valid authentication, watermarks와 n=3f+1 fault model을 사용합니다.</p><p><strong>근거 범위:</strong> PBFT normal-case message processing과 execution semantics에 한정합니다.</p><p><strong>말하지 않는 것:</strong> PREPARE 또는 COMMIT message 하나가 client success·state durability를 뜻하지 않습니다.</p></CitationBlock></div>
  </section>
