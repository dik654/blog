...
const STEPS = [
  {
    label: "① 초기화 + 인덱스 결정",
    body: "params = { key_bits: 2048, threshold: t, n: 3 }\n\n각 참가자 Pᵢ: rᵢ ← random()\nbroadcast(commit(rᵢ))\nreveal rᵢ → sort → index 할당\n// 공정한 인덱스: 누구도 위치 조작 불가.",
  },
  {
    label: "② 분산 소수 생성 + Jacobi 검증",
        ...
        >
          {step === 0 && (
            <motion.g
              ...
              transition={sp}
            >
              <text x={20} y={18} fontSize={10} fontWeight={600} fill={C.init}>
                초기화
              </text>
              {[
          ...
          {step === 1 && (
            <motion.g
              ...
              transition={sp}
            >
              <text x={20} y={18} fontSize={10} fontWeight={600} fill={C.prime}>
                분산 소수 생성
              </text>
              {[
          ...
          {step === 2 && (
            <motion.g
              ...
              transition={sp}
            >
              <text x={20} y={18} fontSize={10} fontWeight={600} fill={C.mul}>
                N = p × q
              </text>
              {[
                "p = Σ pᵢ,  q = Σ qᵢ",
          ...
          {step === 3 && (
            <motion.g
              ...
              transition={sp}
            >
              <text x={20} y={18} fontSize={10} fontWeight={600} fill={C.key}>
                Paillier 키 완성
              </text>
              {[
