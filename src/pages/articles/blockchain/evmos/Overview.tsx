...
const STEPS = [
  {
    label: "Cosmos EVM — 4개 핵심 모듈",
    body: "Cosmos SDK 체인에 EVM 호환성을 추가하는 플러그앤플레이 솔루션.\ngo-ethereum의 EVM을 Cosmos 모듈(x/vm)로 통합.",
  },
  {
    label: "x/vm: 이더리움 가상머신",
    body: "Keeper 패턴으로 accountKeeper, bankKeeper, feeMarketKeeper와 상호작용.\nApplyMessage() → EVM 실행 → Cosmos 상태 업데이트.",
  },
  {
    label: "x/feemarket: EIP-1559 동적 수수료",
    body: "블록 가스 사용량이 50% 넘으면 Base Fee 상승, 낮으면 하락.\nBaseFeeChangeDenominator + ElasticityMultiplier로 세부 조정.",
  },
  {
    label: "x/erc20: Cosmos Coin ↔ ERC20 변환",
    body: "TokenPair로 양방향 매핑 관리.\nIBC 전송 시에도 자동 변환 — 크로스체인 ERC20 토큰 지원.",
  },
  {
    label: "x/precisebank: 18자리 소수점",
    body: "Cosmos SDK 표준(6자리)과 이더리움(18자리) 정밀도 차이 해결.\n정수부 + 소수부 분리 → 소액 거래 정확성 보장.",
  },
];
  ...
  return (
    <section id="overview" className="mb-16 scroll-mt-20">
      ...
      <p className="text-muted-foreground mb-4 text-sm leading-relaxed">
        Cosmos SDK 체인에 EVM 호환성을 추가하는 플러그앤플레이 솔루션.
        <br />
        go-ethereum EVM을 Cosmos 모듈로 통합, 이더리움 스마트 컨트랙트 그대로
        실행.
      </p>
      <StepViz steps={STEPS}>
        {(step) => (
      ...
      <div className="max-w-none mt-6 space-y-5">
        ...
        <div className="rounded-lg border bg-card p-4">
          ...
          <div className="flex flex-col items-center gap-1 text-xs">
            <span className="px-3 py-1.5 rounded bg-blue-500/10 text-blue-400 font-medium">
              User Applications (dApps, wallets)
            </span>
            <span className="text-muted-foreground">
              ↓ JSON-RPC (<code className="text-[11px]">eth_*</code> methods)
            </span>
            <span className="px-3 py-1.5 rounded bg-blue-500/10 text-blue-400 font-medium">
              JSON-RPC Server
            </span>
            <span className="text-muted-foreground">↓ Cosmos TX wrap</span>
            <span className="px-3 py-1.5 rounded bg-purple-500/10 text-purple-400 font-medium">
              Cosmos SDK Baseapp
            </span>
            ...
            <span className="text-muted-foreground">↓</span>
            <span className="px-3 py-1.5 rounded bg-green-500/10 text-green-400 font-medium">
              EVM (go-ethereum fork)
            </span>
            <span className="text-muted-foreground">↓</span>
            <span className="px-3 py-1.5 rounded bg-amber-500/10 text-amber-400 font-medium">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          <div className="rounded-lg border bg-card p-4">
            ...
            <ul className="text-xs text-muted-foreground space-y-1 list-disc list-inside">
              <li>
                <code className="text-[11px]">ApplyMessage()</code> — EVM 실행
              </li>
          ...
          <div className="rounded-lg border bg-card p-4">
            ...
            <ul className="text-xs text-muted-foreground space-y-1 list-disc list-inside">
              <li>
                <code className="text-[11px]">TokenPair</code> mappings
              </li>
