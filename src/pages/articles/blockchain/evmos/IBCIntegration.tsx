...
const STEPS = [
  {
    label: "IBC EVM 통합 — 미들웨어 스택",
    body: "IBC Transfer Module 위에 ERC20 변환 + 스마트 컨트랙트 콜백을 미들웨어로 추가.\n크로스체인 ERC20 토큰 자동 변환 지원.",
  },
  {
    label: "IBC Callbacks: 스마트 컨트랙트 콜백",
    body: "IIBCCallback 인터페이스 구현 시 패킷 수신/확인/타임아웃에 컨트랙트 함수 자동 호출.\n크로스체인 DEX, 스테이킹 등에 활용.",
  },
  {
    label: "ERC20 Middleware: 자동 변환",
    body: "OnRecvPacket에서 TokenPair 확인 → Cosmos Coin을 ERC20으로 자동 변환.\nOnSendPacket에서 ERC20을 Cosmos Coin으로 역변환 후 IBC 전송.",
  },
  {
    label: "IBC Transfer Module: 기본 전송",
    body: "표준 ICS-20 토큰 전송.\n미들웨어 스택이 위에서 아래로 처리 후 기본 전송 모듈 도달.",
  },
  {
    label: "수신/송신 양방향 흐름",
    body: "수신: 패킷 → Callbacks MW → ERC20 MW → Transfer Module.\n송신: Transfer Module → ERC20 MW(역변환) → 패킷 전송.",
  },
];
      ...
      <div className="max-w-none mt-6 space-y-5">
        ...
        <div className="rounded-lg border bg-card p-4">
          ...
          <div className="flex flex-col items-center gap-0 text-xs w-full max-w-xs mx-auto">
            <span className="w-full text-center px-3 py-1.5 rounded-t border bg-blue-500/10 text-blue-400 font-medium">
              IBC Transfer App
            </span>
            <span className="w-full text-center px-3 py-1.5 border-x border-b bg-purple-500/10 text-purple-400 font-medium">
              ERC20 Middleware{" "}
            ...
            <span className="w-full text-center px-3 py-1.5 border-x border-b bg-muted text-muted-foreground font-medium">
              Transfer Module (ICS-20)
            </span>
            <span className="w-full text-center px-3 py-1.5 rounded-b border-x border-b bg-muted text-muted-foreground font-medium">
              IBC Core
            </span>
          </div>
          <p className="text-xs text-muted-foreground mt-3 text-center">
            <code className="text-[11px]">OnRecvPacket</code>: Core → Transfer →
            ERC20 → Callbacks &nbsp;|&nbsp;
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          <div className="rounded-lg border border-green-500/30 bg-card p-4">
            ...
            <ol className="text-xs text-muted-foreground space-y-1 list-decimal list-inside">
              ...
              <li>
                ERC20 <code className="text-[11px]">TokenPair</code> 확인
              </li>
          ...
          <div className="rounded-lg border border-amber-500/30 bg-card p-4">
            ...
            <ol className="text-xs text-muted-foreground space-y-1 list-decimal list-inside">
              ...
              <li>
                <code className="text-[11px]">TokenPair</code> 확인
              </li>
              <li>ERC20 burn</li>
        ...
        <div className="rounded-lg border bg-card p-4">
          ...
          <p className="text-xs text-muted-foreground mb-2">
            ...
            <code className="text-[11px]">
              onPacketAcknowledgement(bytes, bytes)
            </code>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mt-3">
            <div className="rounded bg-muted p-3">
              ...
              <p className="text-xs text-muted-foreground">
                IBC 전송 → 다른 체인에서 swap → callback으로 결과 수신 → 스마트
                컨트랙트 자동 처리
              </p>
            </div>
            <div className="rounded bg-muted p-3">
              ...
              <p className="text-xs text-muted-foreground">
                IBC로 다른 체인에 delegate → reward callback 재분배 → 복합 yield
                farming
              </p>
            </div>
          </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          <div className="rounded-lg border bg-card p-4">
            ...
            <ul className="text-xs text-muted-foreground space-y-1 list-disc list-inside">
              ...
              <li>
                <code className="text-[11px]">sender</code>:{" "}
                <code className="text-[11px]">string</code>
              </li>
              <li>
                <code className="text-[11px]">receiver</code>:{" "}
                <code className="text-[11px]">string</code>
              </li>
              <li>
                <code className="text-[11px]">memo</code>:{" "}
                <code className="text-[11px]">string</code> — callback 정보
                인코딩
              </li>
            </ul>
          </div>
