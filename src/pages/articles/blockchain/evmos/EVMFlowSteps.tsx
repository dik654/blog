...
const LINES = [
  {
    code: "ethTx := tx.GetMsgs()[0].(*MsgEthereumTx)  // MetaMask TX 수신",
    color: "#6366f1",
  },
  {
    code: "if err := ante.Verify(ethTx); err != nil { return err }",
    color: "#10b981",
  },
  {
    code: "res, err := k.ApplyTransaction(ctx, ethTx)  // EVM 컨트랙트 실행",
    color: "#f59e0b",
  },
  {
    code: "k.CommitCachedContexts(ctx)  // EVM → Cosmos KVStore 업데이트",
    color: "#8b5cf6",
  },
  {
    code: "ctx.EventManager().EmitEvents(evmLogs)  // Logs → Cosmos 이벤트",
    color: "#ef4444",
  },
];
