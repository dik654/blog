...
export default function ModernEvmosArticle() {
  return <article className="space-y-14">
    ...
    <section id="evm-module" className="space-y-6">
      ...
      <ExplainedFormula question="EIP-1559 transaction이 실제로 지불하는 gas price의 상한은 어떻게 결정되는가?" idea={<>Base fee에 tip cap을 더하되 Alice가 서명한 max fee를 넘지 않게 작은 값을 취합니다. Ante check와 EVM execution이 같은 chain config·base fee를 사용해야 합니다.</>} formula={String.raw`p_{effective}=\min\!\left(f_{max},\;f_{base}+f_{tip}\right),\qquad Fee=G_{used}\,p_{effective}`}
      annotatedFormula={String.raw`p_{effective}=\underbrace{\min\!\left(f_{max},\;f_{base}+f_{tip}\right),\qquad Fee=G_{used}\,p_{effective}}_{\text{경계 후보 선택}}`}
      operations={[
        { expression: String.raw`\min\!\left(f_{max},\;f_{base}+f_{tip}\right),\qquad Fee=G_{used}\,p_{effective}`, annotation: ["허용 후보 중 목적에 맞는 경계값을 선택합니다.","Base fee에 tip cap을 더하되 Alice가 서명한","max fee를 넘지 않게 작은 값을 취합니다."] },
      ]} terms={[{symbol:"f_{max}",name:"max fee per gas",description:"Sender가 지불을 허용한 gas 당 최대 가격입니다."},{symbol:"f_{base}",name:"block base fee",description:"Chain fee-market state에서 block에 귀속된 기본 fee입니다."},{symbol:"f_{tip}",name:"priority fee cap",description:"Proposer에게 제공하려는 추가 가격의 상한입니다."},{symbol:"G_{used}",name:"used EVM gas",description:"Intrinsic gas와 opcode/state access를 포함한 실제 사용량입니다."}]} assumptions={["Dynamic-fee transaction과 pinned Evmos v20 chain configuration을 사용합니다.","Amounts와 gas price는 chain의 integer denomination precision으로 계산합니다.","Refund, proposer reward/burn의 정확한 분배는 module policy를 따릅니다.","Max fee는 최종 total fee가 아니라 gas 당 상한입니다."]} interpretation="Base fee 20, tip cap 3, max fee 30이면 effective price는 23입니다. Max fee가 21이면 21이 되지만 base fee보다 낮은 transaction은 먼저 유효성 검사에서 거절될 수 있습니다." />
