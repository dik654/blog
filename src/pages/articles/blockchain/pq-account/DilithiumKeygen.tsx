...
export default function DilithiumKeygen({
  onCodeRef,
}: {
  ...
  return (
    <section id="dilithium-keygen" className="mb-16 scroll-mt-20">
      <h2 className="text-2xl font-bold mb-6">
        Dilithium 키 생성 (Module-LWE)
      </h2>
      <div className="prose prose-neutral dark:prose-invert max-w-none">
        <p>
          CRYSTALS-Dilithium은 Module-LWE(Learning With Errors) 문제에
          기반합니다. 공개키 <code>t = A*s1 + s2</code>에서 비밀 벡터 s1, s2를
          복원하는 것이 격자 위의 최단 벡터 문제(SVP)로 환원되며, 양자
          컴퓨터로도 효율적으로 풀 수 없습니다.
        </p>
        ...
        <ul>
          <li>
            <code>q = 8,380,417</code> — 소수 모듈러스
          </li>
          <li>
            <code>n = 256</code> — 다항식 차수
          </li>
          <li>
            <code>k = l = 4</code> — 행렬/벡터 차원
          </li>
          <li>
            <code>eta = 2</code> — 비밀 벡터 범위 &#123;-2..2&#125;
          </li>
        </ul>
        <p className="text-sm border-l-2 border-blue-400 pl-3 bg-blue-50/50 dark:bg-blue-950/20 py-2 rounded-r">
          ...
          Dilithium: 공개키 1312B, 비밀키 2528B. 40배 크지만 양자 내성이라는
          근본적 보안 향상.
        </p>
      </div>
      <div className="mt-8">
        ...
        <div className="not-prose space-y-4 my-4">
          <div className="rounded-lg border border-border/60 p-4">
            ...
            <p className="text-sm text-muted-foreground mb-2">
              문제: 주어진 <M>{"(A, b)"}</M> where{" "}
              <M>{"b = A \\cdot s + e \\mod q"}</M>, <M>{"s"}</M> 찾기
            </p>
          ...
          <div className="rounded-lg border border-border/60 p-4">
            ...
            <ul className="text-sm space-y-1 text-muted-foreground">
              ...
              <li>
                Best quantum attack: <M>{"2^{0.292 \\cdot \\dim}"}</M> time
              </li>
            </ul>
          </div>
          <div className="rounded-lg border border-border/60 p-4">
            ...
            <p className="text-sm text-muted-foreground mb-2">
              LWE의 변형: polynomial ring{" "}
              <M>{"R_q = \\mathbb{Z}_q[X]/(X^n + 1)"}</M> 사용
            </p>
            <ul className="text-sm space-y-1 text-muted-foreground">
              <li>
                Vector elements가 polynomials &mdash; 더 효율적 (FFT 적용 가능)
              </li>
          ...
          <div className="rounded-lg border border-border/60 p-4">
            ...
            <ol className="text-sm space-y-1.5 text-muted-foreground list-decimal list-inside">
              ...
              <li>
                <M>
                  {"A = \\text{expand}(\\text{seed}) \\in R_q^{k \\times l}"}
                </M>
              </li>
              <li>
                <M>{"s_1 \\in S_\\eta^l,\\; s_2 \\in S_\\eta^k"}</M> (short
                polynomials)
              </li>
              <li>
                <M>{"t = A \\cdot s_1 + s_2"}</M>
              </li>
              <li>
                <M>
                  {
                    "(t_{\\text{high}}, t_{\\text{low}}) = \\text{power2round}(t)"
                  }
                </M>
              </li>
            </ol>
            <div className="grid grid-cols-2 gap-3 text-sm text-muted-foreground mt-3">
              <div>
                <strong>Public key</strong>: (seed, <M>{"t_{\\text{high}}"}</M>)
              </div>
              <div>
                <strong>Secret key</strong>: (seed, tr,{" "}
                <M>{"s_1, s_2, t_{\\text{low}}"}</M>)
              </div>
            </div>
          </div>
          <div className="rounded-lg border border-border/60 p-4">
            ...
            <div className="grid grid-cols-3 gap-3 text-sm text-center">
              <div>
                <p className="text-muted-foreground">Dilithium2</p>
                <p className="font-mono">Level 2 (~AES-128)</p>
              </div>
              <div>
                <p className="text-muted-foreground">Dilithium3</p>
                <p className="font-mono">Level 3 (~AES-192)</p>
              </div>
              <div>
                <p className="text-muted-foreground">Dilithium5</p>
                <p className="font-mono">Level 5 (~AES-256)</p>
              </div>
            </div>
          </div>
        </div>
