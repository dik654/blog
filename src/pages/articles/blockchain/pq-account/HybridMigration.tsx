  ...
  return (
    <section id="hybrid-migration" className="mb-16 scroll-mt-20">
      ...
      <div className="prose prose-neutral dark:prose-invert max-w-none">
        ...
        <ol>
          <li>
            <strong>Phase 1</strong> — AA 스마트 계정 배포, 기존 ECDSA로 운영
          </li>
          <li>
            <strong>Phase 2</strong> — Dilithium 공개키 추가 등록
          </li>
          <li>
            <strong>Phase 3</strong> — 하이브리드 모드: ECDSA + Dilithium 동시
            검증
          </li>
          <li>
            <strong>Phase 4</strong> — ECDSA 제거, PQ 전용 운영
          </li>
        </ol>
        <p>
        ...
        <p className="text-sm border-l-2 border-blue-400 pl-3 bg-blue-50/50 dark:bg-blue-950/20 py-2 rounded-r">
          <strong>Insight</strong> — EOA는 프로토콜 레벨 하드포크 없이는 서명
          방식을 변경할 수 없습니다. AA 스마트 계정만이 사용자 수준에서 양자
          내성으로 전환할 수 있는 유일한 경로입니다.
        </p>
      </div>
      <div className="mt-8">
        <MigrationViz />
      ...
      <div className="prose prose-neutral dark:prose-invert max-w-none mt-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="rounded-lg border border-border/60 p-4">
              ...
              <p className="text-xs text-muted-foreground mb-1">
                <code>SmartAccountV1</code>
              </p>
              <ul className="text-sm space-y-1 text-muted-foreground">
                <li>
                  <code>owner</code>: <code>address</code> (ECDSA)
                </li>
                <li>
                  <code>validateUserOp()</code> &mdash;{" "}
                  <code>ECDSA.recover(hash, sig) == owner</code>
                </li>
              </ul>
            </div>
            <div className="rounded-lg border border-border/60 p-4">
              <p className="font-semibold text-sm text-green-400 mb-2">
                Phase 2: Dilithium key 등록
              </p>
              <p className="text-xs text-muted-foreground mb-1">
                <code>SmartAccountV2</code>
              </p>
              <ul className="text-sm space-y-1 text-muted-foreground">
                <li>
                  <code>dilithiumPkHash</code>: <code>bytes32</code> (추가)
                </li>
                <li>
                  <code>registerDilithium(pk)</code> &mdash;{" "}
                  <code>keccak256(pk)</code> 저장
                </li>
                <li>
                  <code>dilithiumEnabled = false</code> (아직 비활성)
                </li>
              </ul>
            </div>
            <div className="rounded-lg border border-amber-500/30 p-4">
              ...
              <p className="text-xs text-muted-foreground mb-1">
                <code>SmartAccountV3</code>
              </p>
              <ul className="text-sm space-y-1 text-muted-foreground">
                <li>
                  signature를 <code>abi.decode</code>로 ECDSA + Dilithium 분리
                </li>
                <li>
                  <strong>둘 다 통과</strong>해야 유효:{" "}
                  <code>ECDSA.recover</code> +{" "}
                  <code>DilithiumVerifier.verify</code>
                </li>
              </ul>
            </div>
            <div className="rounded-lg border border-green-500/30 p-4">
              ...
              <p className="text-xs text-muted-foreground mb-1">
                <code>SmartAccountV4</code>
              </p>
              <ul className="text-sm space-y-1 text-muted-foreground">
                <li>ECDSA 제거</li>
                <li>
                  <code>DilithiumVerifier.verify(pk, hash, sig)</code>만 사용
                </li>
              </ul>
            </div>
          </div>
        ...
        <div className="not-prose space-y-4 my-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="rounded-lg border border-border/60 p-4">
              ...
              <ul className="text-sm space-y-1 text-muted-foreground">
                <li>
                  <strong>Argent</strong>: AA 기반, PQ 연구 중
                </li>
                <li>
                  <strong>Safe (Gnosis)</strong>: 멀티시그, PQ module 가능
                </li>
                <li>
                  <strong>ZeroDev</strong>: AA SDK, PQ 통합 용이
                </li>
                <li>
                  <strong>Biconomy, Pimlico</strong>: PQ paymasters 계획
                </li>
              </ul>
            </div>
          </div>
        ...
        <div className="bg-amber-50 dark:bg-amber-950/30 border-l-4 border-amber-400 p-4 my-6 rounded-r-lg">
          ...
          <p>
            <strong>문제</strong>: EOA는 프로토콜 레벨에서 ECDSA 고정
            <br />
            <strong>양자 이후</strong>: secp256k1 서명 scheme 영구 취약
            <br />
