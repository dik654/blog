  ...
  return (
    <section id="entrypoint" className="mb-16 scroll-mt-20">
      <h2 className="text-2xl font-bold mb-6">
        EntryPoint.handleOps() 내부 추적
      </h2>
      <div className="prose prose-neutral dark:prose-invert max-w-none">
        <p>
          <code>handleOps()</code>는 번들러가 수집한 UserOp 배열을 3단계로
          처리합니다. 검증과 실행을 분리하여, 하나의 실패가 전체 번들에 영향을
          주지 않도록 설계되었습니다.
        </p>
        ...
        <p className="text-sm border-l-2 border-blue-400 pl-3 bg-blue-50/50 dark:bg-blue-950/20 py-2 rounded-r">
          <strong>Insight</strong> — 검증-실행 분리의 이유: Phase 1에서 모든
          UserOp의 가스비를 선확보하므로, Phase 2 실행 중 가스 부족으로 번들러가
          손해보는 것을 방지합니다.
        </p>
      </div>
      <div className="mt-8">
        ...
        <div className="not-prose space-y-4 my-4">
          <div className="rounded-lg border border-border/60 p-4">
            ...
            <p className="text-xs text-muted-foreground mb-2">
              <code>EntryPoint.handleOps(ops[], beneficiary)</code>
            </p>
            <ol className="text-sm space-y-1.5 text-muted-foreground list-decimal list-inside">
              <li>
                <strong>1a) Deploy</strong> &mdash; <code>initCode != ""</code>
                이면 <code>deployAccount(sender, initCode)</code>
              </li>
              <li>
                <strong>1b) Validate</strong> &mdash;{" "}
                <code>account.validateUserOp(userOp, hash, missingFunds)</code>:
                signature check + nonce check + prefund
              </li>
              <li>
                <strong>1c) Paymaster</strong> &mdash;{" "}
                <code>paymasterAndData != ""</code>이면{" "}
                <code>paymaster.validate(...)</code>
              </li>
            </ol>
          </div>
          <div className="rounded-lg border border-border/60 p-4">
            ...
            <ol className="text-sm space-y-1.5 text-muted-foreground list-decimal list-inside">
              <li>
                <strong>2a) Execute</strong> &mdash;{" "}
                <code>account.execute(callData)</code> &rarr; 성공/실패 기록
                (실패 시 state revert, gas refund 없음)
              </li>
              <li>
                <strong>2b) PostOp</strong> &mdash; paymaster 사용 시{" "}
                <code>paymaster.postOp(opSuccess, actualGasCost)</code>
              </li>
            </ol>
          </div>
          <div className="rounded-lg border border-border/60 p-4">
            ...
            <p className="text-sm text-muted-foreground">
              <code>EntryPoint</code> &rarr;{" "}
              <code>beneficiary.transfer(total_gas * effective_price)</code>
            </p>
          </div>
          <div className="rounded-lg border border-green-500/30 p-4">
            ...
            <ul className="text-sm space-y-1 text-muted-foreground">
              <li>
                <strong>Fail-safe</strong>: 한 op 실패해도 다른 op에 영향 없음
              </li>
              <li>
                <strong>Bundler 보상 보장</strong>: Phase 1에서 예치금 선확보
              </li>
              <li>
                <strong>Paymaster 비용 통제</strong>: postOp에서 정산
              </li>
            </ul>
          </div>
        </div>
        ...
        <div className="not-prose space-y-4 my-4">
          <div className="rounded-lg border border-border/60 p-4">
            ...
            <ul className="text-sm space-y-1 text-muted-foreground">
              ...
              <li>
                <code>eth_estimateGas</code> + <code>eth_call</code>로 유효성
                simulate
              </li>
              <li>
                배치로 묶어 <code>handleOps</code> 제출
              </li>
              <li>MEV 수익 획득 가능</li>
            </ul>
          </div>
