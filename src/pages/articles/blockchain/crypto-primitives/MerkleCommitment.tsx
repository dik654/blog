    ...
    <section id="merkle-commitment" className="mb-16 scroll-mt-20">
      ...
      <div className="prose prose-neutral dark:prose-invert max-w-none">
        ...
        <p>
          ...
          <br />
          Sparse Merkle Tree는 2&#x00B2;&#x2075;&#x2076;개의 고정된 리프(Leaf,
          말단 노드) 공간을 가집니다.
          <br />
          대부분은 기본값(0)이고 소수의 리프만 실제 값을 가집니다.
        </p>
        <CodePanel
          title="Merkle Tree 구조"
          code={`        root = H(H₁₂, H₃₄)
       /                    \\
  H₁₂ = H(H₁, H₂)      H₃₄ = H(H₃, H₄)
    /        \\              /        \\
...
검증: O(log N) 해시 연산`}
        ...
        <CodePanel
          title="Merkle 증명 검증 알고리즘"
          code={`// 증명 검증: 리프부터 root를 재구성
current = H(key, value)
...
assert current == root  // collision resistance에 의존`}
          defaultOpen
          annotations={[
        ...
        <p>
          Commitment(커밋먼트)은 값을 숨기되 나중에 열 수 있는 암호학적
          봉투입니다.
          <br />두 가지 성질을 만족해야 합니다.
        </p>
        <ul>
          <li>
            <strong>Hiding</strong> — commitment을 보고 원래 값을 알 수 없음
          </li>
          <li>
            <strong>Binding</strong> — commit한 값과 다른 값으로 열 수 없음
          </li>
        </ul>
        ...
        <p>
          ...
          <br />
          DLP(Discrete Logarithm Problem, 이산로그 문제)에 기반하여 정보이론적
          hiding과 계산적 binding을 제공합니다.
          <br />
          동형성(homomorphic, 연산 보존) 성질도 가져서 commit(a) + commit(b) =
          commit(a+b)가 성립합니다.
        </p>
      </div>
    </section>
