  ...
  return (
    <section id="poseidon" className="mb-16 scroll-mt-20">
      ...
      <div className="prose prose-neutral dark:prose-invert max-w-none">
        <p>
          SHA-256 — 비트 연산(XOR, 회전, 시프트) 기반
          <br />
          ZK 회로에서 각 비트를 변수로 표현 → 해시 하나에 ~25,000 R1CS 제약 필요
          <br />
        ...
        <p>
          Poseidon — 체 원소(Fr) 단위 SPN 구조 사용
          <br />각 라운드 세 단계 구성
        </p>
        <ul>
          <li>
            <strong>AddRoundConstants</strong> — 대칭성 파괴를 위해 상수를 더함
          </li>
          <li>
            <strong>S-box (x → x&#x2075;)</strong> — 비선형성(confusion) 제공
          </li>
          <li>
            <strong>MDS Matrix Mix</strong> — 확산(diffusion) 제공
          </li>
        </ul>

        <h3 className="text-xl font-semibold mt-6 mb-3">S-box: x&#x2075;</h3>
        <p>
          S-box 지수 α=5 선택 이유 — 순열 조건 gcd(α, r-1) = 1을 만족하는 최소
          홀수. BN254에서 α=3은 gcd(3, r-1) = 3이므로 불가
        </p>
        <CodePanel
          title="S-box: x → x⁵"
          code={`// S-box: x → x⁵ (Fr 곱셈 3회)
let x2 = x.square();   // x²
let x4 = x2.square();  // x⁴
x4 * x                 // x⁵

// R1CS 제약: 3개 (곱셈 게이트 3개)
// SHA-256의 비트 연산 하나가 ~32개 제약인 것과 비교`}
          defaultOpen
          annotations={[
        ...
        <p>
          HADES 설계 — RF/2 full → RP partial → RF/2 full rounds 구성
          <br />
          BN254 파라미터: T=3, RF=8, RP=57, 총 65 라운드
          <br />
        ...
        <CodePanel
          title="Sponge 구조와 제약 수"
          code={`Sponge: state = [capacity=0, rate₀=left, rate₁=right]
  → 65 라운드 permutation
  → output = state[1]

Full round:  T=3 S-box × 3 제약 = 9 제약/라운드
Partial:     1 S-box × 3 제약   = 3 제약/라운드
총: 8×9 + 57×3 = 72 + 171 = 243 제약 ≈ 250`}
          defaultOpen
          annotations={[
        ...
        <p>
          Sponge capacity=1 → 보안 비트 = min(254/2, 254) = 127 bits
          <br />
          RF=8 — 보간/대수적 공격 방어, RP=57 — 차분/통계적 공격 방어
          <br />
