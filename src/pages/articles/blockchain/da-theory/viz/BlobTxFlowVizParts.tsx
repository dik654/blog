...
export function BlobTxFlowStep2({
  checks,
}: {
  checks: { label: string; pass: boolean; color: string }[];
}) {
  return (
      ...
      {checks.map((c, i) => (
        ...
        >
          ...
          <text
            ...
            fontFamily="monospace"
          >
            Line {i + 1}: if !check{c.label}(tx) {"{"} return ErrInvalid
            {c.label} {"}"}
          </text>
        </motion.g>
      ))}
    ...
    <motion.g initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }}>
      ...
      >
        <text x={20} y={108} fontSize={10} fill={F.muted}>
          [48]byte Commitment → SHA256 → version byte 0x01 → [32]byte versioned
          hash
        </text>
      </motion.g>
    </motion.g>
