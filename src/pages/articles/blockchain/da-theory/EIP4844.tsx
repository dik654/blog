      ...
      {steps.map((s, i) => (
        ...
        >
          <div className="flex items-center gap-3">
            <span
              className={`w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold ${i === step ? "bg-emerald-500 text-white" : "bg-muted text-muted-foreground"}`}
            >
              {i + 1}
            </span>
...
export default function EIP4844({
  onCodeRef,
}: {
  ...
  return (
    <section id="eip-4844" className="mb-16 scroll-mt-20">
      <h2 className="text-2xl font-bold mb-6">
        EIP-4844: Blob TX &amp; Proto-Danksharding
      </h2>

      <div className="prose prose-neutral dark:prose-invert max-w-none mb-4">
        <h3 className="text-xl font-semibold">BlobTx 구조</h3>
        <p className="leading-7">
          <strong>왜 별도 트랜잭션 타입인가?</strong> 롤업 데이터는 EVM(이더리움
          가상 머신) 실행에 불필요하다.
          <br />
          calldata에 넣으면 영구 저장되어 비용이 높다.
          <br />
      ...
      <div className="prose prose-neutral dark:prose-invert max-w-none mb-4">
        ...
        <p className="leading-7">
          <strong>왜 독립 수수료 시장인가?</strong> blob 수요와 일반 가스 수요는
          독립적이다.
          <br />
      ...
      <div className="prose prose-neutral dark:prose-invert max-w-none mb-4">
        <h3 className="text-xl font-semibold">
          EVM 옵코드: BLOBHASH &amp; BLOBBASEFEE
        </h3>
        <p className="leading-7">
          <strong>왜 전용 옵코드가 필요한가?</strong> 롤업 컨트랙트가 blob
          데이터를 직접 읽을 수 없다.
          <br />
