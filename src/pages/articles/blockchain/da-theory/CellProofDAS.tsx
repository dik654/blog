...
export default function CellProofDAS({
  onCodeRef,
}: {
    ...
    <section id="cell-proof-das" className="mb-16 scroll-mt-20">
      ...
      <div className="prose prose-neutral dark:prose-invert max-w-none mb-8">
        <p className="leading-7">
          Blob 전체를 한 번에 검증하면 라이트 노드에게 부담이 크다.
          <br />
          Cell proof는 blob을 128개 조각(cell)으로 분할하고, 각 cell에 독립 KZG
          증명을 붙인다.
          <br />
          라이트 노드는 랜덤 cell 몇 개만 요청해도 전체 가용성을 확률적으로
          보장할 수 있다.
        </p>
      </div>
        ...
        <div className="grid grid-cols-2 gap-3 not-prose">
          {VERSION_COMPARE.map((v) => (
            <div key={v.version} className="rounded-lg border p-3">
              ...
              {open && (
                <div className="mt-2">
                  <CodeViewButton onClick={() => open(v.codeKey)} />
                </div>
              )}
            </div>
          ))}
