    ...
    <section id="das" className="mb-16 scroll-mt-20">
      ...
      <div className="prose prose-neutral dark:prose-invert max-w-none mb-8">
        ...
        <p className="leading-7">
          풀 노드가 모든 블록 데이터를 다운로드하면 확장이 불가능하다.
          <br />
          블록이 커질수록 노드 운영 비용이 올라가고, 탈중앙화가 약화된다.
          <br />
          DAS(Data Availability Sampling, 무작위 셀 샘플링으로 데이터 가용성을
          확률적으로 검증하는 기법)는 라이트 노드가{" "}
          <strong>일부 셀만 샘플링</strong>해도 높은 확률(예: 75개 셀 →
          99.99%)로 전체 데이터의 가용성을 보장할 수 있게 한다.
        </p>

        <h3 className="text-xl font-semibold mt-8">KZG 커밋먼트 워크플로</h3>
        <p className="text-sm text-foreground/60 mb-3">
          KZG(Kate-Zaverucha-Goldberg)는 다항식 커밋먼트 스킴이다.
          <br />
          데이터를 다항식으로 인코딩하고, 그 다항식에 대한 짧은 증명을 만들어
          검증한다.
        </p>
      </div>
        ...
        {KZG_STEPS.map((s, i) => (
          ...
          >
            <div className="flex items-center gap-3">
              <span
                className={`w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold ${i === step ? "bg-sky-500 text-white" : "bg-muted text-muted-foreground"}`}
              >
                {i + 1}
              </span>
      ...
      <div className="prose prose-neutral dark:prose-invert max-w-none mb-8">
        ...
        <p className="leading-7">
          <strong>왜 128개 cell로 분할하는가?</strong> 라이트 노드가 전체 128KB
          blob을 받을 필요 없이, 소수의 cell만 요청해 대역폭을 절감하기
          위해서다.
        </p>
