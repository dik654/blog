  ...
  return (
    <article className="space-y-14">
      <section id="overview" className="space-y-6">
        ...
        <p>
          도서관이 책의 지문(hash)을 게시했다고 생각해 봅시다. 내 손의 한 페이지가 그 책에 속하는지는 지문과 proof로 확인할 수 있지만, 나머지 페이지를 도서관이 숨겼다면 책
          전체를 복원할 수 없습니다. DA protocol은 이 간극을 이렇게 다룹니다. 원본에 복원용 redundancy를 넣고 무작위 위치를 여러 peer에게 요청하며, 충분한 조각이
          모이면 reconstruction을 실제로 수행합니다.
        </p>
      ...
      <section id="erasure-code" className="space-y-6">
        ...
        <p>
          가장 작은 toy example로 k=4개 source symbol을 생각하겠습니다. 유한체 위에서 이 네 값을 지나는 degree 3 이하 polynomial을 하나 정하고
          새로운 네 지점에서 값을 더 계산하면 n=8개 symbol이 됩니다. 올바른 서로 다른 네 평가값만 있으면 polynomial을 interpolation해 source를 복원할 수
      ...
      <section id="eip4844" className="space-y-6">
        ...
        <p>
          Type-3 blob transaction의 execution payload에는 blob 원문이 들어가지 않고 versioned hash 목록이 들어갑니다. Blob, KZG
          commitment와 proof는 consensus sidecar로 함께 전파됩니다. EVM contract는 blob byte를 직접 읽지 못하고 commitment에서 파생된
          versioned hash와 point-evaluation 검증 경계를 사용합니다. 이 분리 덕분에 rollup data를 execution state에 영구 저장하는 비용은
          피하면서 transaction과 sidecar는 cryptographically binding합니다.
        </p>
        <ExplainedFormula
        ...
        />
        <p>
          중요한 역사적 경계가 있습니다. EIP-4844 자체에서는 beacon node가 자신이 검증하는 blob sidecar 전체를 download합니다. “EIP-4844에서
          light node가 임의 cell 몇 개만 받아 availability를 확인한다”는 설명이 어긋나는 지점이 여기입니다. 부분 custody와 sampling은 후속
          PeerDAS protocol의 책임입니다.
        </p>
      ...
      <section id="kzg" className="space-y-6">
        ...
        <p>
          KZG polynomial commitment는 큰 polynomial을 고정 길이 group element 하나에 묶고 특정 point에서 나온 평가값이 맞다는 짧은
          opening proof를 제공합니다. PeerDAS cell proof는 “이 cell의 값들이 원래 blob polynomial의 확장된 평가값과 일치한다”는
          authenticity를 확인하는 데 쓰입니다. 검증에 성공해도 다른 column을 누가 보관하는지, 요청이 독립적인 peer로 갔는지, reconstruction
          threshold를 충족하는지는 알려 주지 않습니다.
        </p>
      ...
      <section id="peerdas" className="space-y-6">
        ...
        <p>
          PeerDAS에서 block의 각 blob은 하나의 row처럼 취급되고 1D Reed–Solomon extension으로 두 배 길어집니다. 같은 column index의
          cell을 모든 blob row에서 모은 것이 data column입니다. Node는 자신의 node ID로 정해진 custody column을 보관하고 block마다 다른
          column을 peer에게 sampling합니다. 전체 column의 절반 이상을 얻으면 각 row의 polynomial을 interpolation해 extended matrix를
          복원할 수 있습니다.
        </p>
        <p>
          PeerDAS를 “각 blob을 2D square로 만든다”고 설명하는 것도 정확하지 않습니다. 2D처럼 보이는 matrix는 여러 1D-extended blob row를 쌓아서
          생기며 parity는 row 방향에만 추가됩니다. Sampling unit도 단일 random byte가 아니라 모든 blob row에 걸친 column입니다.
        </p>
      ...
      <section id="celestia" className="space-y-6">
        ...
        <p>
          Celestia 계열의 extended data square는 k×k source square에서 시작해 각 row를 2k로 늘리고 그 결과의 각 column도 2k로 늘려
          2k×2k square를 만듭니다. k=4라면 source 16 cells가 총 64 cells가 되므로 전체 code rate는 1/4입니다. 행과 열 양쪽에 복원 관계가 있어
          light node는 여러 좌표를 sampling하고 full node는 충분한 share로 square를 reconstruct합니다.
        </p>
        <p>
          2D 구조의 장점은 withholding과 잘못된 encoding을 좌표·row·column proof로 다룰 수 있다는 점입니다. 다만 “몇 번 sample하면 안전하다”는
          결론은 숨겨야 하는 최소 fraction과 index 선택의 예측 불가능성, 요청 독립성, peer eclipse 방지, honest rebroadcast,
          reconstruction threshold에 의존합니다. 그림의 1/4 rate를 PeerDAS의 1/2 row extension에 그대로 옮기면 틀립니다.
        </p>
      ...
      <section id="comparison" className="space-y-6">
        ...
        <p>
          기초 6문제는 DA·validity·authenticity 구분부터 (8,4) code 계산, EIP-4844 full-download 경계, PeerDAS 1D column,
          Celestia 2D square, KZG cell proof 경계까지 이어집니다. 심화 4문제는 sampling miss 수치와 전제, 세 protocol의 같은 축 비교,
          eclipse·bad proof·wrong encoding의 adversarial matrix, pinned-version release gate입니다. 답안의 모든 항목은 위
          설명과 수치만으로 만들어져야 합니다. moving blob schedule은 암기값이 아니라 fork-pinned 입력으로 처리합니다.
        </p>
