...
export default function BlobGasMath({
  onCodeRef,
}: {
    ...
    <section id="blob-gas-math" className="mb-16 scroll-mt-20">
      ...
      <div className="prose prose-neutral dark:prose-invert max-w-none mb-8">
        <p className="leading-7">
          Blob 수수료는 EIP-1559의 base fee와 동일한 원리로 동작한다.
          <br />
          블록당 target(3 blobs)보다 많이 쓰면 다음 블록의 blob 가격이 오르고,
          적으면 내린다.
          <br />
          핵심 공식은{" "}
          <code>price = MIN_PRICE × e^(excess / UPDATE_FRACTION)</code>이다.
        </p>
        <p className="leading-7">
          <strong>왜 지수함수인가?</strong> 선형 가격 조절은 급격한 수요 변동에
          느리게 반응한다.
          <br />
        ...
        {GAS_MATH_STEPS.map((s, i) => (
          ...
          >
            <div className="flex items-center gap-3">
              <span
                className={`w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold ${i === step ? "bg-amber-500 text-white" : "bg-muted text-muted-foreground"}`}
              >
                {i + 1}
              </span>
      ...
      <div className="prose prose-neutral dark:prose-invert max-w-none mb-4">
        ...
        <p className="leading-7">
          <code>VerifyEIP4844Header</code> → <code>calcExcessBlobGas</code> →{" "}
          <code>CalcBlobFee</code> → <code>fakeExponential</code>의 호출 순서를
          추적한다.
        </p>
      </div>
