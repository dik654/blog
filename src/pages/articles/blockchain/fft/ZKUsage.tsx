  ...
  return (
    <section id="zk-usage" className="mb-16 scroll-mt-20">
      ...
      <div className="prose prose-neutral dark:prose-invert max-w-none mb-6">
        <p>
          NTT는 ZKP 증명 생성에서{" "}
          <strong>가장 많은 시간을 소비하는 연산</strong>이다.
          <br />
        ...
        <p>
          두 다항식 <Math>{"A(x), B(x)"}</Math>의 곱{" "}
          <Math>{"C(x) = A(x) \\cdot B(x)"}</Math>를 구하려면:
        </p>
        <div className="not-prose grid grid-cols-1 gap-3 my-4">
          {[
            {
              step: "1. NTT",
              desc: "A(x), B(x)를 평가 표현으로 변환 — O(n log n) × 2",
              color: "indigo",
            },
            {
              step: "2. Pointwise 곱",
              desc: "C(ωⁱ) = A(ωⁱ) · B(ωⁱ) — O(n)",
              color: "emerald",
            },
            {
              step: "3. INTT",
              desc: "C의 평가 표현을 계수로 역변환 — O(n log n)",
              color: "amber",
            },
          ].map((p) => (
            <div
