  ...
  return (
    <section id="zk-usage" className="mb-16 scroll-mt-20">
      ...
      <div className="prose prose-neutral dark:prose-invert max-w-none">
        <p>
          계수 길이가 각각 Lₐ,Lᵦ인 두 다항식의 linear product에는 Lₐ+Lᵦ−1개
          coefficient가 필요합니다. Transform length가 더 작으면 high-degree
          coefficient가 xⁿ=1 관계로 앞쪽에 감겨 cyclic convolution이 됩니다.
          보통 필요한 길이 이상인 지원 radix 크기로 zero-padding합니다.
        </p>
        <p>
      ...
      <div className="prose prose-neutral dark:prose-invert max-w-none">
        ...
        <p>
          PLONK 계열은 witness의 coefficient/evaluation form 변환, quotient
          계산, coset evaluation에서 여러 NTT를 수행하고, FRI 계열은 low-degree
          extension을 위해 큰 evaluation domain을 만듭니다. 그러나 “증명 시간의
          몇 퍼센트” 같은 숫자는 protocol, domain size, field arithmetic, batch,
          CPU/GPU와 memory layout에 따라 달라지므로 보편값으로 쓰지 않습니다.
          실제 선택은 forward/inverse 횟수, permutation pass, twiddle storage,
          field multiplication throughput, host-device transfer와 end-to-end
          trace로 판단해야 합니다.
        </p>
        <h3>구현 검증 체크리스트</h3>
        <p>
          작은 vector에서는 direct O(n²) NTT와 fast NTT를 비교하고,
          INTT(NTT(a))=a round trip을 검사합니다. 이어서 random polynomial
          product를 schoolbook result와 비교하고, natural/bit-reversed order와
          inverse normalization을 API contract에 기록합니다. Benchmark는 같은
          input·field· transform direction·batch에서 kernel time과 end-to-end
          time을 함께 측정합니다.
        </p>
      </div>
    </section>
