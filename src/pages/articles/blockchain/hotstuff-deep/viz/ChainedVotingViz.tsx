...
const DATA: { title: string; color: string; rows: R }[] = [
  {
    title: "View 1: Prepare Phase for B₁",
    color: C.p,
    rows: [
      [
        "L₁→all: ⟨PROPOSE, view=1, B₁, highQC=genesis⟩",
        "var(--foreground)",
        0.15,
      ],
  ...
  {
    title: "View 2: B₁ Pre-Commit + B₂ Prepare",
    color: C.pc,
    rows: [
      ["L₂→all: ⟨PROPOSE, view=2, B₂, highQC=QC₁⟩", "var(--foreground)", 0.15],
      [
        "B₂ Prepare 투표 = B₁ Pre-Commit (파이프라인)",
        "var(--muted-foreground)",
        0.3,
      ],
      ["B₁: 2-chain, B₂: 1-chain", C.pc, 0.45],
      [
        "핵심: view마다 1회 투표로 여러 블록 동시 진행",
        "var(--muted-foreground)",
        0.6,
      ],
    ],
  ...
  {
    title: "View 3: B₁ Commit (3-chain 완성)",
    color: C.cm,
    rows: [
