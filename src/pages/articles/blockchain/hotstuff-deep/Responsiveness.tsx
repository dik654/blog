  ...
  return (
    <section id="responsiveness" className="mb-16 scroll-mt-20">
      ...
      <div className="prose prose-neutral dark:prose-invert max-w-none">
        <CitationBlock
          source="Yin et al., PODC 2019 — §6"
          citeKey={3}
          type="paper"
        >
          <p className="italic">
            "A protocol is responsive if the leader can make progress as soon as
            it receives messages from a quorum of replicas, without waiting for
            a known upper bound on network delay."
          </p>
        </CitationBlock>

        {/* ── Responsiveness 정의 ── */}
        <h3 className="text-xl font-semibold mt-6 mb-3">
          Responsiveness 형식 정의
        </h3>
        <div className="not-prose grid grid-cols-1 gap-3 mb-4">
          <div className="rounded-lg border bg-card p-4">
            ...
            <div className="text-sm text-muted-foreground space-y-1">
              <p>
                Responsive: 합의 시간이 실제 메시지 지연 δ에 비례 —{" "}
                <code className="text-xs">Time_consensus &le; c * δ</code>
              </p>
              <p>
                Non-responsive: 고정 timeout Δ (worst-case) 대기. 실제 δ
                &lt;&lt; Δ여도 Δ 대기
              </p>
            ...
            <tbody>
              {[
                [
                  "PBFT",
                  "3δ (responsive)",
                  "timeout 기반 (non-responsive)",
                  "O(Δ)",
                ],
                [
                  "HotStuff",
                  "3δ (responsive)",
                  "2f+1 NewView (responsive)",
                  "O(δ) under GST",
                ],
                [
                  "Tendermint",
        ...
        <p className="leading-7">
          ...
          non-responsive는 고정 timeout Δ 대기 (pessimistic).
          <br />δ ≪ Δ 에서 큰 성능 차이.
        </p>
        ...
        <div className="not-prose grid grid-cols-1 sm:grid-cols-2 gap-3 mb-4">
          <div className="rounded-lg border bg-card p-4">
            <div className="text-xs font-semibold text-emerald-600 dark:text-emerald-400 mb-2">
              Normal Operation — Fully Responsive
            </div>
            <ul className="text-sm space-y-1 text-muted-foreground">
              <li>leader가 2f+1 vote 받으면 즉시 다음 phase</li>
              <li>timeout 대기 없음</li>
              <li>
                latency: <code className="text-xs">3δ</code> per block (chained)
              </li>
            </ul>
          </div>
          <div className="rounded-lg border bg-card p-4">
            <div className="text-xs font-semibold text-amber-600 dark:text-amber-400 mb-2">
              View Change — Partial Responsive
            </div>
            <div className="text-sm text-muted-foreground space-y-1">
              <p>
                <strong>Phase 1</strong> timeout detection:{" "}
                <code className="text-xs">Δ</code> 대기 (non-responsive)
              </p>
              <p>
                <strong>Phase 2</strong> NewView collection:{" "}
                <code className="text-xs">δ</code> (responsive)
              </p>
              <p>
                <strong>Phase 3</strong> new propose:{" "}
                <code className="text-xs">δ</code> (responsive)
              </p>
              <p>
                총: <code className="text-xs">Δ + 2δ</code>
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="not-prose grid grid-cols-1 sm:grid-cols-2 gap-3 mb-4">
          <div className="rounded-lg border bg-card p-4">
            ...
            <ul className="text-sm space-y-1 text-muted-foreground">
              ...
              <li>
                latency: normal <code className="text-xs">2δ</code>, view change{" "}
                <code className="text-xs">Δ + δ</code>
              </li>
            </ul>
          </div>
          <div className="rounded-lg border bg-card p-4">
        ...
        <div className="not-prose grid grid-cols-1 gap-3 mb-6">
          <div className="rounded-lg border bg-card p-4">
            <div className="text-xs font-semibold text-rose-600 dark:text-rose-400 mb-2">
              Responsiveness의 실제 가치
            </div>
            <div className="text-sm text-muted-foreground">
              LAN: δ ≈ 1ms, Δ ≈ 100ms → <strong>100x</strong> 차이 / WAN: δ ≈
              100ms, Δ ≈ 1s → <strong>10x</strong> 차이
            </div>
          </div>
        </div>
        <p className="leading-7">
          HotStuff normal = <strong>responsive</strong>, view change ={" "}
          <strong>partial responsive</strong>.<br />
          leader 장애 감지에만 timeout 필요.
          <br />
        ...
        <div className="not-prose grid grid-cols-1 sm:grid-cols-2 gap-3 mb-6">
          <div className="rounded-lg border bg-card p-4">
            ...
            <ul className="text-sm space-y-1 text-muted-foreground">
              <li>
                <strong>WAN + 응답성</strong>: HotStuff-2, Jolteon
              </li>
              <li>
                <strong>LAN + 단순성</strong>: Tendermint
              </li>
              <li>
                <strong>범용</strong>: HotStuff
              </li>
              <li>
                <strong>고처리량</strong>: Jolteon + DAG
              </li>
            </ul>
          </div>
          <div className="rounded-lg border bg-card p-4">
            ...
            <ul className="text-sm space-y-1 text-muted-foreground">
              <li>
                TPS: <code className="text-xs">100,000+</code>
              </li>
              <li>
                latency: <code className="text-xs">~1s</code> (end-to-end)
              </li>
        ...
        <p className="leading-7">
          WAN 환경에서 responsiveness 차이가 큼.
          <br />
          HotStuff 300ms → HotStuff-2 200ms → Jolteon 200ms.
          <br />
        ...
        <div className="grid gap-3 sm:grid-cols-2">
          ...
          <div className="rounded-lg border p-4">
            ...
            <p className="text-sm">
              리더 장애 감지에 타임아웃 필요.
              <br />이 타임아웃이 실제 네트워크 지연보다 클 수 있음
            </p>
          </div>
        </div>
        ...
        <p className="text-sm border-l-2 border-amber-500/50 pl-3 mt-4">
          <strong>💡 왜 responsiveness가 BFT 진화 방향인가</strong> — 인터넷은 δ
          &lt; Δ.
          <br />
          보안 마진으로 Δ 크게 설정 필요, but 실제 δ는 훨씬 작음.
          <br />
          responsive 프로토콜은 그 차이만큼 성능 향상.
          <br />
