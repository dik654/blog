...
const phases = [
  {
    ...
    challenge: null,
    detail:
      "회로 합성 → 어드바이스 열 값 채움 → KZG Commit(advice_poly, blind)\n→ 각 커밋을 transcript에 기록 (Fiat-Shamir 시작)",
  },
  {
    id: "lookup",
    label: "② θ — Plookup",
  ...
  return (
    <div className="rounded-xl border p-5 space-y-3 not-prose">
      <p className="text-sm font-medium">
        create_proof — Fiat-Shamir 도전값 순서
      </p>
            ...
            <motion.div key={p.id} whileTap={{ scale: 0.99 }}>
              ...
              >
                <div className="flex items-center justify-between gap-2">
                  ...
                  {p.challenge && (
                    <span
                      className={`text-xs px-2 py-0.5 rounded font-mono ${badgeColors[p.color]}`}
                    >
                      {p.challenge}
                    </span>
                  )}
      ...
      <p className="text-xs text-foreground/60">
        각 도전값은 이전 단계의 커밋을 transcript에 기록한 후 squeeze — 비대화형
        변환 (Fiat-Shamir)
      </p>
    </div>
  );
