  ...
  return (
    <div>
      <p className="text-xs text-foreground/50 mb-3">
        ...
        <code className="mx-1">meta.create_gate()</code>에서 제약 다항식을
        조합합니다.
      </p>
      ...
      <div className="mt-3 p-3 rounded-lg border border-border text-xs text-foreground/50 leading-relaxed">
        <strong className="text-foreground/70">게이트 예시:</strong> 곱셈 게이트
        a × b - c = 0 →
        <code className="mx-1">Product(Advice(a), Advice(b)) - Advice(c)</code>
        를 벡터로 반환. Selector로 해당 행에서만 활성화.
      </div>
    </div>
  );
}
