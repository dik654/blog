...
const STEPS = [
  {
    label: "노드 배치: 온라인/오프라인 식별",
    body: `${N}개 노드 중 ${OFF.length}개가 오프라인(회색). 나머지 정직 노드가 복구에 참여합니다.`,
  },
  {
    label: "복구 대상 선택 및 요청 브로드캐스트",
    body: "오프라인 노드(#2)에 대해 복구 요청을 온라인 노드들에게 전파합니다.",
  },
  {
    label: "정직 노드가 복구 심볼 전송",
    body: "각 온라인 노드가 자신의 슬라이버에서 디코딩 심볼을 계산하여 대상 노드로 전송합니다.",
  },
  {
    label: "f+1 심볼 수집 → 슬라이버 복원",
    body: `f+1=${Math.floor((N - 1) / 3) + 1}개 이상 수집하면 try_recover_sliver로 원본 슬라이버를 재구성합니다.`,
  },
];
