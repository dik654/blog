...
export const PC2_ITEMS = [
  {
    label: "TreeC 생성",
    color: "bg-purple-500",
    desc: "column_hash[i] = Poseidon(layer[1..11][i]) → Merkle 트리 → comm_c",
  },
  {
    label: "복제본 인코딩",
    color: "bg-orange-500",
    desc: "replica[i] = AES-256(label[11][i], data[i]) — GPU 가속",
  },
  {
    label: "TreeR 생성",
    color: "bg-red-500",
    desc: "Poseidon Arity-8 Merkle(replica) → comm_r_last",
  },
  {
    label: "comm_r 계산",
    color: "bg-emerald-500",
    desc: "comm_r = SHA256(comm_c ‖ comm_r_last) → 온체인 등록",
  },
];
