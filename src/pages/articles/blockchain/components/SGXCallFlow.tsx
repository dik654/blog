  ...
  ecall: [
    {
      ...
      detail: "sgx_ecall(eid, fn_id, args) 호출",
    },
    {
      zone: "urts",
      label: "URTS (sgx_urts)",
      detail: "__morestack 경유 → AEX 전환 준비",
    },
    {
      zone: "cpu",
      label: "CPU (AEX)",
      detail:
        "EENTER: 호스트 → 엔클레이브 컨텍스트 전환\nRSP/RBP/XMM 저장 → SSA(State Save Area) 기록",
    },
    {
      zone: "trts",
      label: "TRTS (trts_ecall.cpp)",
      detail:
        "is_ecall_allowed() → 동적 ECALL 권한 확인\ndo_ecall() → 랜덤 스택 오프셋 (Spectre 완화)\n함수 디스패치 실행",
    },
    {
      zone: "enclave",
      label: "Enclave Function",
      detail: "신뢰 코드 실행\nEPC 메모리 내에서만 동작",
    },
  ],
  ocall: [
    {
      zone: "enclave",
      label: "Enclave Code",
      detail: "sgx_ocall(idx, args) 호출",
    },
    {
      zone: "trts",
      label: "TRTS (trts_ocall.cpp)",
      detail: "OCALL_FLAG 설정\n__morestack → EEXIT 준비",
    },
    {
      zone: "cpu",
      label: "CPU (EEXIT)",
      detail: "EEXIT: 엔클레이브 → 호스트 컨텍스트 복원\n비신뢰 스택으로 복귀",
    },
    {
      zone: "urts",
      label: "URTS (do_oret)",
      detail: "OCALL_FLAG 검증 (재진입 공격 방어)\n비신뢰 함수 실행",
    },
    {
      zone: "app",
      label: "Host Function",
      detail: "비신뢰 OS 함수 실행 후 결과 반환\n→ 다시 ECALL로 재진입",
    },
  ],
};
      ...
      <div className="flex flex-col gap-1">
        {current.map((step, i) => (
          ...
          >
            ...
            >
              <span
                className={`shrink-0 rounded px-2 py-0.5 text-xs font-mono ${zoneColors[step.zone]}`}
              >
                {step.label}
              </span>
              {i < current.length - 1 && (
