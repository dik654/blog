...
import { STEPS, STEP_REFS, STEP_LABELS } from './DealDetailVizData';
import { Step0, Step1, Step2 } from './DealDetailSteps';

const R = [Step0, Step1, Step2];

interface Props { onOpenCode?: (key: string) => void }

export default function DealDetailViz({ onOpenCode }: Props) {
  return (
    <StepViz steps={STEPS}>
      {(step) => {
        const S = R[step];
