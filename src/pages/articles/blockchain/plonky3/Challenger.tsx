  ...
  return (
    <section id="challenger" className="mb-16 scroll-mt-20">
      <h2 className="text-2xl font-bold mb-6">
        {title ?? "Fiat-Shamir 챌린저"}
      </h2>
      <div className="prose prose-neutral dark:prose-invert max-w-none">
        <p>
          <strong>DuplexChallenger</strong>는 Poseidon2 스펀지 기반 Fiat-Shamir
          변환을 구현합니다. 증명자와 검증자가 동일한 순서로 데이터를
          흡수(observe)하고 챌린지를 샘플(sample)하여 비대화형 증명을 가능하게
          합니다.
        </p>

        <h3>DuplexChallenger 구조</h3>
        <CodePanel
          title="스펀지 구조 & 흡수/듀플렉싱"
          code={DUPLEX_CODE}
...
//        (Verifier도 동일 계산으로 verify)`}</pre>
        ...
        <div className="bg-amber-50 dark:bg-amber-950/30 border-l-4 border-amber-400 p-4 my-6 rounded-r-lg">
          ...
          <p>
            <strong>Random Oracle Model</strong>: hash function을 이상적 random
            function으로 가정
            <br />
            <strong>현실과 차이</strong>: 실제 hash는 구체적 구현
            <br />
            <strong>위험</strong>: Weak hash → soundness 깨짐
            <br />
            <strong>Best practice</strong>: Cryptographic-grade hash +
            transcript 완전성
          </p>
        </div>
      </div>
