...
const STEPS = [
  {
    label: "BeaconKit ABCI 2.0 블록 생명주기",
    body: "표준 Cosmos SDK 모듈 미사용 — CometBFT 합의 엔진만 유지, 나머지 자체 구현.\n직렬화: Protobuf 대신 이더리움의 SSZ(Simple Serialize) 채택.",
  },
  {
    label: "PrepareProposal → forkchoiceUpdated",
    body: "제안자가 Engine API로 EVM 페이로드 빌드 요청.\nCometBFT 라운드 로빈 → 이더리움의 RANDAO 선출 대체.",
  },
  {
    label: "ProcessProposal → newPayload 검증",
    body: "EL에서 페이로드 실행 & 검증.\n검증자들이 동기 BFT(Prevote/Precommit)로 투표 — 비동기 Attestation 대체.",
  },
  {
    label: "FinalizeBlock → 상태 확정",
    body: "즉시 최종성 — 1 블록으로 확정 (이더리움: 2 에폭, ~12.8분).\n포크 불가: BFT safety 보장.",
  },
  {
    label: "Commit → Optimistic Payload Building",
    body: "BeaconKit 핵심 최적화 — ProcessProposal에서 이미 StateRoot 검증.\n다음 블록 N+1 페이로드를 병렬로 선행 빌드 → 블록 타임 ~40% 단축.",
  },
];
  ...
  return (
    <section id="beaconkit-arch" className="mb-16 scroll-mt-20">
      ...
      <p className="text-muted-foreground mb-4 text-sm leading-relaxed">
        ABCI 2.0 콜백 → Engine API → EVM 실행 — 이더리움 블록 제안 흐름을
        CometBFT에서 재현.
      </p>
      <StepViz steps={STEPS}>
        {(step) => (
      ...
      <div className="prose prose-neutral dark:prose-invert max-w-none mt-6">
        <h3 className="text-xl font-semibold mt-6 mb-3">
          BeaconKit ABCI 2.0 + Engine API
        </h3>
        {/* ABCI 2.0 개요 */}
        <div className="bg-muted rounded-lg p-4 mb-4">
          <h4 className="font-semibold text-sm mb-2">
            ABCI 2.0 (Application Blockchain Interface)
          </h4>
          <p className="text-sm text-muted-foreground mb-2">
            CometBFT와 애플리케이션 간 경계 인터페이스.
          </p>
          <div className="grid grid-cols-2 gap-x-6 gap-y-1 text-sm">
            ...
            <span>
              이더리움 스펙을 감싸는 ABCI 앱 — CometBFT 이벤트 → Engine API
              호출로 변환
            </span>
          </div>
        </div>
          ...
          <div className="space-y-2 text-sm">
            ...
            <div className="grid grid-cols-[24px_minmax(0,1fr)] gap-2">
              ...
              <div className="min-w-0">
                ...
                <div className="mt-2 grid min-w-0 grid-cols-1 gap-x-3 gap-y-1 text-xs text-muted-foreground sm:ml-4 sm:grid-cols-[160px_minmax(0,1fr)] sm:gap-y-0.5 [&>span]:min-w-0 [&>span]:break-words">
                  <span>
                  ...
                  <span>beacon_state.randao_mix</span>
                  <span>
                    <code className="bg-background/50 px-1 rounded">
                      fee_recipient
                    </code>
                  </span>
                  <span>validator_addr</span>
                  <span>
                    <code className="bg-background/50 px-1 rounded">
                      withdrawals
                    </code>
                  </span>
                  <span>beacon_state.pending_withdrawals</span>
                </div>
              </div>
            </div>
            ...
            <div className="grid grid-cols-[24px_1fr] gap-2">
              ...
              <div>
                ...
                <div className="ml-4 mt-1 text-xs text-muted-foreground grid grid-cols-[120px_1fr] gap-x-3 gap-y-0.5">
                  <span>
                    <code className="bg-background/50 px-1 rounded">slot</code>
                  </span>
                  <span>height</span>
                  <span>
                    <code className="bg-background/50 px-1 rounded">
                  ...
                  <span>round_robin_leader(height)</span>
                  <span>
                    <code className="bg-background/50 px-1 rounded">
                      execution_payload
                    </code>
                  </span>
                  <span>EL이 빌드한 payload</span>
                  <span>
                    <code className="bg-background/50 px-1 rounded">
                      randao_reveal
                    </code>
                  </span>
                  <span>sign(epoch)</span>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="space-y-2 text-sm">
            <div className="grid grid-cols-[24px_1fr] gap-2">
              ...
              <span>
                SSZ 디코딩 —{" "}
                <code className="text-xs bg-background/50 px-1 rounded">
                  BeaconBlock.unmarshal_ssz(proposed_block)
                </code>
              </span>
            </div>
            <div className="grid grid-cols-[24px_1fr] gap-2">
            ...
            <div className="grid grid-cols-[24px_1fr] gap-2">
              ...
              <span>
                EL 페이로드 검증 —{" "}
                <code className="text-xs bg-background/50 px-1 rounded">
                  EL.newPayload(execution_payload)
                </code>{" "}
                → VALID가 아니면 REJECT
              </span>
            </div>
        ...
        <div className="bg-muted rounded-lg p-4 mb-4">
          <h4 className="font-semibold text-sm mb-3">
            Optimistic Payload Building (핵심 최적화)
          </h4>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
            <div>
              <p className="font-medium mb-1">기존 방식</p>
              <p className="text-muted-foreground text-xs">
                Block N 커밋 → Payload N+1 빌드 → N+1 제안 (순차 실행)
              </p>
            </div>
            <div>
