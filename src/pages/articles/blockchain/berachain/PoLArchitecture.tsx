...
const STEPS = [
  {
    label: "PoL 플라이휠 전체 구조",
    body: "유동성 제공 → BGT 보상 → 검증자 위임 → 블록 생성 → BGT 배분 → 재투자.\n3토큰 모델: BERA(가스) + BGT(거버넌스, 양도불가) + HONEY(스테이블).",
  },
  {
    label: "1단계: DeFi 프로토콜에 유동성 제공",
    body: "사용자가 DEX, 대출 풀에 유동성을 공급.\nEthereum PoS와 달리 스테이킹이 아닌 유동성 기여로 보안 확보.",
  },
  {
    label: "2단계: Reward Vault에서 BGT 수령",
    body: "유동성 공급 대가로 BGT(Bera Governance Token)를 보상.\nBGT는 양도 불가 — 위임만 가능하여 투기 방지.",
  },
  {
    label: "3단계: 검증자에게 BGT 위임",
    body: "BGT 홀더가 원하는 검증자에게 위임(boost).\n검증자의 투표 파워 = 위임받은 BGT 총량.",
  },
  {
    label: "4단계: 블록 생성 & BGT 배분",
    body: "검증자가 블록 제안 시 게이지 가중치에 따라 BGT를 프로토콜에 배분.\n프로토콜은 유동성 유치를 위해 검증자에게 인센티브 제공.",
  },
  {
    label: "5단계: 선순환 (플라이휠)",
    body: "프로토콜 인센티브 → 검증자 → 위임자 → 다시 유동성 제공.\n네트워크 보안 = 스테이킹 + DeFi TVL 합산.",
  },
];
          ...
          <div className="w-full">
            ...
            {onCodeRef && step >= 3 && (
              <div className="flex items-center gap-2 mt-3 justify-end">
                <CodeViewButton
                  onClick={() =>
                    onCodeRef("bk-block-builder", codeRefs["bk-block-builder"])
                  }
                />
      ...
      <div className="prose prose-neutral dark:prose-invert max-w-none mt-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-3 mb-4">
          ...
          <div className="bg-muted rounded-lg p-4">
            <h4 className="font-semibold text-sm mb-2">
              BGT{" "}
              <span className="text-xs text-muted-foreground">(soulbound)</span>
            </h4>
            <ul className="text-sm space-y-1 list-disc list-inside text-muted-foreground">
        ...
        <div className="bg-muted rounded-lg p-4 mb-4">
          ...
          <div className="space-y-2 text-sm">
            <div className="grid grid-cols-[24px_1fr] gap-2">
              ...
              <span>
                사용자가 BEX 풀(BERA/HONEY)에 LP 제공 → 100 HONEY + 50 BERA 예치
              </span>
            </div>
            <div className="grid grid-cols-[24px_1fr] gap-2">
              <span className="font-mono text-muted-foreground">2.</span>
              <span>
                LP 토큰을 Reward Vault에 스테이킹 → 거버넌스가 정한 비율로 BGT
                수령
              </span>
            </div>
            <div className="grid grid-cols-[24px_1fr] gap-2">
              ...
              <span>
                BGT가 사용자 주소에 축적 (soulbound) → 7일간 100 BGT 획득
              </span>
            </div>
            <div className="grid grid-cols-[24px_1fr] gap-2">
              <span className="font-mono text-muted-foreground">4.</span>
              <span>
                BGT를 원하는 검증자에게 위임 → 검증자 V의 투표 파워 증가
              </span>
            </div>
            <div className="grid grid-cols-[24px_1fr] gap-2">
              <span className="font-mono text-muted-foreground">5.</span>
              <span>
                검증자 V가 블록 제안자가 될 확률 증가 → 블록 보상 + 가스 수수료
                획득
              </span>
            </div>
            <div className="grid grid-cols-[24px_1fr] gap-2">
              ...
              <span>
                검증자 V가 인센티브 프로토콜에 BGT 배분 → BEX 50%, BEND 30%,
                기타 20%
              </span>
            </div>
            <div className="grid grid-cols-[24px_1fr] gap-2">
              ...
              <span>
                프로토콜이 BGT를 LP 보상으로 분배 → 1단계로 순환 (유동성 증가)
              </span>
            </div>
          </div>
        </div>
        ...
        <div className="bg-muted rounded-lg p-4 mb-4">
          ...
          <p className="text-sm text-muted-foreground mb-2">
            BGT 홀더가 게이지에 투표: "어떤 프로토콜에 검증자 보상을 지급할
            것인가?"
          </p>
          <ul className="text-sm space-y-1 list-disc list-inside text-muted-foreground">
            <li>
              검증자가 블록당 BGT를 투표 승리 게이지에 투표 가중치 비례 배분
            </li>
            <li>
              프로토콜은 BGT 홀더에게 투표 유도를 위한 인센티브 제공 (뇌물: 외부
              보상, 에어드롭 등)
            </li>
        ...
        <div className="bg-muted rounded-lg p-4 mb-4">
          ...
          <div className="border-t border-border/50 pt-2">
            ...
            <p className="text-xs text-muted-foreground">
              BGT 공급 증가 (배출) → BGT 소각 시 BERA 공급 증가 → 순 BERA
              인플레이션은 BGT 배출량에 연동
            </p>
          </div>
        </div>
        ...
        <div className="bg-muted rounded-lg p-4">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-x-6 gap-y-1 text-sm">
            <span>
              <span className="font-medium">BEX</span>{" "}
              <span className="text-muted-foreground">
                — DEX (Balancer fork)
              </span>
            </span>
            <span>
              ...
              <span className="text-muted-foreground">— 렌딩 마켓</span>
            </span>
            <span>
              <span className="font-medium">BERPS</span>{" "}
              <span className="text-muted-foreground">— 퍼페추얼</span>
            </span>
            <span>
              <span className="font-medium">InfraRed</span>{" "}
              <span className="text-muted-foreground">
                — 리퀴드 BGT 스테이킹
              </span>
            </span>
            <span>
              <span className="font-medium">Kodiak</span>{" "}
              <span className="text-muted-foreground">— 집중 유동성</span>
            </span>
          </div>
        </div>
      </div>
