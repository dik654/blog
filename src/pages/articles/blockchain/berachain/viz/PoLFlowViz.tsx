...
const STEPS = [
  {
    label: "유동성 제공",
    body: "사용자가 DEX, 대출 풀 등에 유동성을 공급합니다.",
  },
  {
    label: "게이지 투표",
    body: "BGT 홀더가 게이지 가중치를 투표로 결정합니다.",
  },
  {
    label: "검증자 선택",
    body: "유동성 부트스트래퍼가 원하는 검증자를 PoL로 지원합니다.",
  },
  {
    label: "블록 생성",
    body: "CometBFT 합의로 즉시 최종성을 가진 블록을 생성합니다.",
  },
  {
    label: "BGT 발행",
    body: "검증자가 블록 보상으로 BGT를 발행하고 게이지 가중치에 따라 배분합니다.",
  },
  {
    label: "플라이휠 완성",
    body: "BGT 보상이 다시 유동성 제공으로 이어지는 선순환 구조가 완성됩니다.",
  },
];
          ...
          {EDGES.map((lbl, i) => {
            ...
            if (isLoop && step === 5) {
              return (
                ...
                >
                  <path
                    d={`M${nx(5) + NW / 2},${SY + NH / 2} Q${nx(5) + NW / 2},${SY + 50} ${nx(0) + NW / 2},${SY + NH / 2}`}
                    fill="none"
                    stroke="#ef4444"
                    strokeWidth={1.2}
                    markerEnd="url(#pol-ah)"
                    strokeDasharray="3,3"
                  />
                  <rect
                    x={W / 2 - 14}
