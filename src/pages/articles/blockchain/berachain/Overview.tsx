...
const STEPS = [
  {
    label: "이더리움 Beacon Chain vs BeaconKit 전체 비교",
    body: "BeaconKit — 이더리움 Beacon Chain 스펙을 CometBFT 위에서 구현.\nEngine API로 이더리움 EL 클라이언트(geth/reth)를 그대로 재활용.",
  },
  {
    label: "CL: Casper FFG → CometBFT",
    body: "이더리움의 확률적 최종성(2 에폭, ~12.8분) 대신 CometBFT의 즉시 최종성(1 블록). reorg 발생 불가.",
  },
  {
    label: "포크 선택: LMD-GHOST → 불필요",
    body: "BFT 합의에서는 포크가 발생하지 않으므로 포크 선택 규칙 자체가 불필요. 아키텍처 단순화.",
  },
  {
    label: "P2P: libp2p → MConnection",
    body: "Tendermint의 MConnection 멀티플렉싱 P2P. 기존 Cosmos 생태계의 피어 디스커버리/가십과 호환.",
  },
];
  ...
  return (
    <section id="overview" className="mb-16 scroll-mt-20">
      ...
      <p className="text-muted-foreground mb-4 text-sm leading-relaxed">
        Berachain — Proof of Liquidity(PoL) 합의를 사용하는 EVM 호환 블록체인.
        <br />
        BeaconKit 프레임워크 — 이더리움 EL+CL 아키텍처를 Cosmos에 가장
        직접적으로 이식.
      </p>
      <StepViz steps={STEPS}>
        {() => (
          <div className="w-full">
            ...
            {onCodeRef && (
              <div className="flex items-center gap-2 mt-3 justify-end">
                <CodeViewButton
                  onClick={() =>
                    onCodeRef("bk-service", codeRefs["bk-service"])
                  }
                />
      ...
      <div className="prose prose-neutral dark:prose-invert max-w-none mt-6">
        ...
        <div className="bg-muted rounded-lg p-4 mb-4">
          ...
          <div className="grid grid-cols-2 gap-x-6 gap-y-1 text-sm">
            ...
            <span>EVM L1 + Proof of Liquidity (PoL)</span>
            <span className="text-muted-foreground">기반</span>
            <span>BeaconKit (CometBFT + Ethereum spec)</span>
            <span className="text-muted-foreground">EL</span>
            <span>Reth 또는 Geth (이더리움 클라이언트 그대로)</span>
            <span className="text-muted-foreground">CL</span>
            <span>BeaconKit (Prysm/Lighthouse 대신)</span>
            <span className="text-muted-foreground">합의 엔진</span>
            <span>CometBFT (Casper FFG 대신)</span>
            <span className="text-muted-foreground">경제 모델</span>
            <span>PoL (PoS 대신)</span>
          </div>
        </div>
          ...
          <div className="space-y-3 text-sm">
            ...
            <div className="border-t border-border/50 pt-2">
              ...
              <div className="grid grid-cols-[160px_1fr] gap-x-4 gap-y-1 text-xs">
                <span>PrepareProposal</span>
                <span className="text-muted-foreground">
                  → forkchoiceUpdated + getPayload
                </span>
                <span>ProcessProposal</span>
                <span className="text-muted-foreground">→ newPayload</span>
                <span>FinalizeBlock</span>
                <span className="text-muted-foreground">
                  → forkchoiceUpdated (head 확정)
                </span>
              </div>
            </div>
          </div>
