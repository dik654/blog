  ...
  return (
    ...
    >
      ...
      {NODES.map((n, i) => {
        ...
        return (
          ...
          >
            ...
            >
              {i === 2 ? "Attestation" : i % 2 === 1 ? "Contract" : "Chain"}
            </text>
          </motion.g>
        );
      })}
      ...
      {[
        ...
        <motion.g
          key={`e-${i}`}
          animate={{ opacity: e.show ? 0.8 : 0 }}
          transition={sp}
        >
          <line
            x1={NODES[e.from].x + 62}
            y1={48}
            x2={NODES[e.to].x - 2}
            y2={48}
            stroke="#10b981"
          ...
          />
          <rect
            x={(NODES[e.from].x + NODES[e.to].x) / 2 + 18}
            y={28}
            width={28}
            height={10}
            rx={2}
          ...
          />
          <text
            x={(NODES[e.from].x + NODES[e.to].x) / 2 + 32}
            y={35}
            textAnchor="middle"
            fontSize={7}
            fill="#10b981"
