  ...
  return (
    <section id="coeff-rearrange" className="mb-16 scroll-mt-20">
      ...
      <div className="prose prose-neutral dark:prose-invert max-w-none">
        <p>
          Fp12 원소는 12개의 Fp 계수로 표현된다.
          <br />
          Frobenius 사상을 적용하면 각 계수에 고정 상수를 곱하는 것으로
          귀결된다.
          <br />이 상수들은 p와 기약다항식으로부터 결정되며, 커브 파라미터가
          바뀌지 않는 한 불변이다.
        </p>
        <CodePanel
          title="Frobenius 계수 변환"
          code={`// Fp12 원소: (c₀, c₁, ..., c₁₁)
// Frobenius φ(f) = f^p
...
// γᵢ는 setup 시 한 번만 계산 → 테이블에 저장`}
          defaultOpen
          annotations={[
            {
              lines: [4, 4],
              color: "sky",
              note: "핵심: 미지수×미지수가 아닌, 미지수×상수",
            },
            {
              lines: [7, 7],
              color: "emerald",
              note: "c₀의 상수는 항상 1 (변화 없음)",
            },
            {
              lines: [14, 14],
              color: "amber",
              note: "상수 테이블은 커브별로 고정",
            },
          ]}
        />
        <p>
          ...
          이라는 것이다.
          <br />
          Fp 부분(c0)은 Frobenius에 영향을 받지 않는다. 나머지 상수도 일부는
          단순한 값이어서 실제 곱셈 횟수는 12보다 적다.
        </p>
      </div>
    </section>
  );
