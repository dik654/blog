...
export function ExponentFormula({ step }: Props) {
  return (
    <motion.g animate={{ opacity: step === 0 ? 1 : 0.2 }} transition={sp}>
      ...
      {/* Labels */}
      <text x={140} y={105} textAnchor="middle" fontSize={10} fill={C.green}>
        Easy Part 1
      </text>
      <text x={260} y={105} textAnchor="middle" fontSize={10} fill={C.green}>
        Easy Part 2
      </text>
      <text x={400} y={105} textAnchor="middle" fontSize={10} fill={C.amber}>
        Hard Part
      </text>
    </motion.g>
  );
