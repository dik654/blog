    ...
    annotations: [
      {
        lines: [2, 3],
        color: "sky",
        note: "pop+peek — x는 복사본, y는 포인터 → y에 결과를 직접 쓰면 push 불필요",
      },
      {
        lines: [13, 15],
        color: "emerald",
        note: "최적화 포인트 — 매 opcode마다 새 uint256 할당을 피하는 패턴",
      },
    ],
  },
  "op-sload": {
    ...
    annotations: [
      {
        lines: [2, 5],
        color: "sky",
        note: "SLOAD — peek()로 슬롯 주소 읽고, StateDB에서 값을 가져와 같은 위치에 덮어씀",
      },
      {
        lines: [10, 11],
        color: "amber",
        note: "readOnly 체크 — STATICCALL 중이면 상태 변경 즉시 revert",
      },
      {
        lines: [13, 16],
        color: "emerald",
        note: "SSTORE — 두 값을 pop하여 StateDB에 기록 (0→non-zero: 20000gas)",
      },
    ],
  },
  "op-call": {
    ...
    annotations: [
      {
        lines: [4, 4],
        color: "sky",
        note: "63/64 규칙 — 가용 가스의 1/64를 현재 프레임에 유보 (EIP-150)",
      },
      {
        lines: [7, 10],
        color: "emerald",
        note: "7개 파라미터 — gas, addr, value, inOffset, inSize, retOffset, retSize",
      },
      {
        lines: [16, 17],
        color: "amber",
        note: "evm.Call() — 새 ScopeContext와 인터프리터 루프를 재귀적으로 생성",
      },
      {
        lines: [20, 22],
        color: "violet",
        note: "반환 처리 — 결과를 메모리에 쓰고 성공 여부(0/1)를 스택에 push",
      },
    ],
  },
  stack: {
    ...
    annotations: [
      {
        lines: [1, 4],
        color: "sky",
        note: "sync.Pool — 호출마다 스택을 새로 할당하지 않음 (GC 부담 감소)",
      },
      {
        lines: [8, 8],
        color: "emerald",
        note: "256비트 정수 — 이더리움 워드 크기, holiman/uint256 라이브러리",
      },
      {
        lines: [13, 15],
        color: "amber",
        note: "returnStack — 길이만 0으로 리셋하여 용량 유지 후 풀에 반환",
      },
      {
        lines: [18, 20],
        color: "violet",
        note: "push — append로 추가, 1024 제한은 인터프리터 루프에서 maxStack으로 체크",
      },
    ],
  },
  memory: {
    ...
    annotations: [
      {
        lines: [2, 3],
        color: "sky",
        note: "lastGasCost — 메모리 가스는 증분만 과금 (이전까지의 비용을 기록)",
      },
      {
        lines: [9, 10],
        color: "emerald",
        note: "용량 내 확장 — cap이 충분하면 슬라이스 길이만 늘림 (재할당 없음)",
      },
      {
        lines: [19, 20],
        color: "amber",
        note: "16KB 제한 — 대형 메모리는 풀에 반환하지 않아 메모리 누적 방지",
      },
    ],
  },
};
