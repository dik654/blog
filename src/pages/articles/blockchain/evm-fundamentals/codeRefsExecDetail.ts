    ...
    annotations: [
      {
        lines: [5, 6],
        color: "sky",
        note: "SubBalance — 원자적 차감, 저널에 balanceChange 기록 (revert 가능)",
      },
      {
        lines: [8, 9],
        color: "emerald",
        note: "AddBalance — 계정이 없으면 자동 생성 (빈 계정 문제 → EIP-161)",
      },
      {
        lines: [12, 16],
        color: "amber",
        note: "CanTransfer — Call() 최상단에서 사전 검증, 실패 시 가스 소모 없이 반환",
      },
    ],
  },
  "dynamic-gas": {
    ...
    annotations: [
      {
        lines: [6, 6],
        color: "sky",
        note: "toWordSize — 32바이트 워드 올림, EVM 메모리는 워드 단위로 확장",
      },
      {
        lines: [12, 14],
        color: "emerald",
        note: "이차 공식 — 선형(3*w) + 이차(w²/512), 724워드(~22KB) 이후 이차항이 지배",
      },
      {
        lines: [16, 18],
        color: "amber",
        note: "증분 과금 — 이전 비용을 빼서 새로 확장한 부분만 청구",
      },
    ],
  },
  "op-return": {
    ...
    annotations: [
      {
        lines: [2, 5],
        color: "sky",
        note: "RETURN — 메모리에서 데이터 복사 후 errStopToken으로 루프 탈출",
      },
      {
        lines: [8, 11],
        color: "emerald",
        note: "REVERT — 동일하게 데이터 반환하지만 ErrExecutionReverted → 상태 롤백",
      },
      {
        lines: [14, 15],
        color: "amber",
        note: "STOP — 반환 데이터 없이 종료, RETURN(offset=0,size=0)과 동일 효과",
      },
    ],
  },
};
