    ...
    annotations: [
      {
        lines: [3, 4],
        color: "sky",
        note: "CreateAddress — keccak256(rlp([sender, nonce]))[12:]로 주소 도출",
      },
      {
        lines: [15, 16],
        color: "emerald",
        note: "init 코드 Run — 리턴값이 배포될 런타임 바이트코드 (생성자 로직)",
      },
      {
        lines: [19, 19],
        color: "amber",
        note: "EIP-170 — 최대 24KB, 이를 초과하면 배포 실패",
      },
      {
        lines: [22, 22],
        color: "violet",
        note: "코드 저장 가스 — 200 gas/byte, 배포 비용의 주요 부분",
      },
    ],
  },
  "evm-create2": {
    ...
    annotations: [
      {
        lines: [2, 4],
        color: "sky",
        note: "결정론적 주소 — salt + initCode 해시로 결정, nonce 무관",
      },
      {
        lines: [4, 4],
        color: "emerald",
        note: "CreateAddress2 — 팩토리 패턴에서 배포 전 주소 예측에 활용",
      },
    ],
  },
  "evm-delegatecall": {
    ...
    annotations: [
      {
        lines: [10, 10],
        color: "sky",
        note: "caller 보존 — 두 번째 인자도 caller, storage 컨텍스트가 호출자에 유지",
      },
      {
        lines: [12, 13],
        color: "emerald",
        note: "코드만 차용 — addr의 바이트코드를 가져오되 실행 주체는 caller",
      },
      {
        lines: [14, 14],
        color: "amber",
        note: "AsDelegate — 프록시 패턴의 핵심, SSTORE가 caller 스토리지에 기록",
      },
    ],
  },
  "evm-staticcall": {
    ...
    annotations: [
      {
        lines: [8, 10],
        color: "sky",
        note: "readOnly 설정 — 중첩 호출에도 유지, 최외곽에서만 해제 (defer)",
      },
      {
        lines: [14, 14],
        color: "emerald",
        note: "value=0 고정 — StaticCall은 ETH 전송 불가",
      },
      {
        lines: [17, 17],
        color: "amber",
        note: "readOnly=true — SSTORE, CREATE, LOG, SELFDESTRUCT 시 revert",
      },
    ],
  },
  "op-create": {
    ...
    annotations: [
      {
        lines: [5, 7],
        color: "sky",
        note: "스택 pop — value(ETH), offset, size 3개를 순서대로 꺼냄",
      },
      {
        lines: [10, 10],
        color: "emerald",
        note: "init 코드 — 메모리에서 복사, 컨트랙트 생성자 바이트코드",
      },
      {
        lines: [19, 20],
        color: "amber",
        note: "evm.Create 호출 — 주소 생성 + init 실행 + 코드 저장",
      },
      {
        lines: [23, 23],
        color: "violet",
        note: "결과 push — 성공 시 새 주소, 실패 시 0",
      },
    ],
  },
  "op-selfdestruct": {
    ...
    annotations: [
      {
        lines: [5, 7],
        color: "sky",
        note: "readOnly 검증 — STATICCALL 내에서 SELFDESTRUCT 불가",
      },
      {
        lines: [15, 16],
        color: "emerald",
        note: "EIP-6780 — Cancun 이후 같은 TX 생성분만 삭제, 나머지는 잔액 이동만",
      },
      {
        lines: [19, 20],
        color: "amber",
        note: "beneficiary — 남은 잔액 전체를 지정 주소로 전송",
      },
    ],
  },
};
