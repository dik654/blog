...
const STEP_LABELS = [
  "interpreter.go — Run() 루프",
  "interpreter.go — Run() 가스 차감",
  "interpreter.go — Run() ErrOutOfGas",
];
      ...
      {broken ? (
        ...
        >
          <line
            ...
            strokeWidth={0.7}
          />
          <text x={240} y={58} fontSize={12} fill={CR}>
            x
          </text>
        </motion.g>
      ) : (
        <path
          d="M216,52 Q248,52 248,72 Q248,92 128,92 Q40,92 40,72 Q40,64 40,64"
          fill="none"
          stroke={C1}
          strokeWidth={0.6}
          markerEnd="url(#gA)"
...
function GasBar({ ratio, label }: { ratio: number; label: string }) {
  ...
  return (
    <g>
      <text x={290} y={38} fontSize={10} fill="var(--muted-foreground)">
        gasRemaining
      </text>
      <rect
        x={290}
      ...
      <motion.rect
        ...
        initial={false}
        width={w * ratio}
        animate={{ width: w * ratio }}
        transition={{ duration: 1.2, ease: "easeOut" }}
      />
      <text x={290} y={68} fontSize={10} fontWeight={600} fill={c}>
        {label}
      </text>
    </g>
  );
...
export default function GasModelViz({
  onOpenCode,
}: {
          ...
          >
            ...
            {step === 0 && (
              ...
              >
                ...
                <text x={300} y={56} fontSize={10} fontWeight={500} fill={CR}>
                  네트워크 전체 멈춤!
                </text>
                <text
                  x={300}
