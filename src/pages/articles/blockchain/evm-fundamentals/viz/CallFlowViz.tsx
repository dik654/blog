...
export default function CallFlowViz({
  onOpenCode,
}: {
      ...
      {(step) => (
        <div className="w-full">
          ...
          >
            ...
            <path
              d={`M${436},${34 + R} C${470},${74} ${470},${74} ${436},${114 - R}`}
              stroke="var(--border)"
              strokeWidth={0.5}
              fill="none"
              strokeDasharray="3,2"
              ...
              return (
                ...
                >
                  <circle
                    ...
                    r={R}
                    fill={
                      cur ? `${C}22` : active ? `${C}0a` : "var(--background)"
                    }
                    stroke={active ? C : "var(--border)"}
                    strokeWidth={cur ? 1.6 : 0.8}
                  />
                  <text
                    x={n.x}
                    y={n.lines[1] ? n.y - 3 : n.y + 3}
                    textAnchor="middle"
                    fontSize={7.5}
                    fontWeight={cur ? 700 : 400}
                    fill={active ? C : "var(--muted-foreground)"}
                  >
                    {n.lines[0]}
                  </text>
                  {n.lines[1] && (
