  ...
  return (
    <section id="overview" className="mb-16 scroll-mt-20">
      ...
      <div className="prose prose-neutral dark:prose-invert max-w-none mb-8">
        ...
        <p className="leading-7">
          기본 빌딩 블록 세 가지: adc(add-with-carry),
          sbb(subtract-with-borrow), mac(multiply-accumulate)
          <br />이 세 함수 위에 모든 유한체 연산(+, -, *, inv, pow)을 쌓아 올림
        </p>
      </div>
      ...
      <div className="prose prose-neutral dark:prose-invert max-w-none">
        ...
        <p className="leading-7">
          ...
          <br />
          ZK 증명에서 필드 연산은 초당 수백만 회 — 이 수준의 최적화가 전체 성능
          좌우
        </p>
      </div>
      ...
      <div className="prose prose-neutral dark:prose-invert max-w-none mt-6">
        ...
        <div className="not-prose my-4 grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div className="bg-muted/50 rounded-xl p-5 border border-border">
            ...
            <M display>
              {
                "N = a_0 + a_1 \\cdot 2^{64} + a_2 \\cdot 2^{128} + a_3 \\cdot 2^{192}"
              }
            </M>
          </div>
        </div>
        ...
        <div className="not-prose my-4 bg-muted/50 rounded-xl border border-border overflow-hidden">
          <div className="bg-muted px-5 py-2 border-b border-border">
            <span className="text-sm font-semibold">
              Primitive Operations — 세 가지 빌딩 블록
            </span>
          </div>
          <div className="p-5 grid grid-cols-1 sm:grid-cols-3 gap-4 text-sm">
            <div>
              <p className="font-semibold mb-1">
                <code className="text-xs bg-muted px-1 rounded">adc</code>{" "}
                (add-with-carry)
              </p>
              ...
              <p className="text-xs mt-1">
                x86 <code className="text-xs bg-muted px-1 rounded">addc</code>{" "}
                명령어 사용
              </p>
            ...
            <div>
              <p className="font-semibold mb-1">
                <code className="text-xs bg-muted px-1 rounded">sbb</code>{" "}
              ...
              <p className="text-xs mt-1">
                x86 <code className="text-xs bg-muted px-1 rounded">sbb</code>{" "}
                명령어 사용
              </p>
            ...
            <div>
              <p className="font-semibold mb-1">
                <code className="text-xs bg-muted px-1 rounded">mac</code>{" "}
                (multiply-accumulate)
              </p>
              <p className="text-xs text-muted-foreground">
                (hi, lo) = a * b + c + d
        ...
        <div className="not-prose my-4 grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <div className="bg-muted/50 rounded-xl p-5 border border-border">
            ...
            <p className="text-sm">
              <code className="text-xs bg-muted px-1 rounded">
                product = full_mult(a, b)
              </code>
            </p>
            <p className="text-xs text-muted-foreground mt-1">
              4x4 = 8 limbs schoolbook
            </p>
            <p className="text-sm mt-1">
              <code className="text-xs bg-muted px-1 rounded">
                result = mod_reduce(product, p)
              </code>
            </p>
            <p className="text-xs text-muted-foreground mt-1">
              Montgomery 또는 Barrett 리덕션
            </p>
          </div>
        </div>
        <p className="text-sm text-muted-foreground ml-1">
          성능: Fp 곱셈 1회당 ~20-30 CPU 사이클
        </p>
        ...
        <div className="not-prose my-4 grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div className="bg-emerald-50 dark:bg-emerald-950/30 rounded-xl p-5 border border-emerald-200 dark:border-emerald-800">
            ...
            <p className="text-sm font-mono text-xs mt-1">
              result[i] = result[i].wrapping_add(p[i] & mask);
            </p>
            <p className="text-xs text-muted-foreground mt-2">
              조건 분기 없이 상수 시간 실행
            </p>
            <p className="text-xs text-muted-foreground">
              암호학 연산은 반드시 상수 시간이어야 함
            </p>
          </div>
        </div>
        ...
        <div className="not-prose my-4 bg-muted/50 rounded-xl border border-border overflow-hidden">
          ...
          <div className="p-5 grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm">
            <div>
              ...
              <p className="text-xs">
                4회 <code className="text-xs bg-muted px-1 rounded">adc</code>{" "}
                루프로 carry chain 덧셈
              </p>
              <p className="text-xs mt-1">
                <code className="text-xs bg-muted px-1 rounded">
            ...
            <div>
              <p className="text-xs text-muted-foreground mb-1">
                sub_if_gte — "일단 빼보고 borrow면 원래 값"
              </p>
              <p className="text-xs">
                4회 <code className="text-xs bg-muted px-1 rounded">sbb</code>{" "}
                루프로 a - p 시도
              </p>
              <p className="text-xs mt-1">
                borrow 발생 시 mask ={" "}
        ...
        <div className="not-prose my-4 grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="bg-muted/50 rounded-xl p-5 border border-border">
            ...
            <p className="text-sm">
              모든 값을 <M>{"x \\cdot R \\bmod p"}</M> 형태로 저장 (
              <M>{"R = 2^{256}"}</M>)
            </p>
          ...
          <div className="bg-muted/50 rounded-xl p-5 border border-border">
            ...
            <p className="text-sm">
              <strong>BLST</strong> (Supranational) — hand-tuned AVX/NEON asm
            </p>
            <p className="text-sm mt-1">
              <strong>BoringSSL</strong> (Google) — 상수 시간 곡선 연산
            </p>
            <p className="text-sm mt-1">
              <strong>ark-ff</strong> (Arkworks) — pure Rust +{" "}
              <code className="text-xs bg-muted px-1 rounded">#[asm]</code>
            </p>
          </div>
        </div>
        ...
        <div className="not-prose my-4 grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="bg-muted/50 rounded-xl p-5 border border-border">
            ...
            <p className="text-sm">
              전형적 증명 계산: <M>{"10^6 \\sim 10^9"}</M> Fp 곱셈
            </p>
            <p className="text-sm mt-1">20ns/회 기준: ~100ms ~ 20초</p>
            <p className="text-xs text-muted-foreground mt-2">
              SIMD (AVX-512): 배치 연산 2-3배 가속
            </p>
            <p className="text-xs text-muted-foreground">
              GPU: MSM에서 10-100배 가속
            </p>
            <p className="text-xs text-muted-foreground">
              ASIC: 반복 워크로드 1000배+ 가속
            </p>
          </div>
          <div className="bg-muted/50 rounded-xl p-5 border border-border">
            <p className="text-sm font-semibold text-muted-foreground mb-2">
            ...
            <div className="text-sm space-y-1">
              <p>
                <strong>BN254</strong>: <M>{"p \\approx 2^{254}"}</M> (4 limbs)
              </p>
              <p>
                <strong>BLS12-381</strong>: <M>{"p \\approx 2^{381}"}</M> (6
                limbs)
              </p>
              <p>
                <strong>secp256k1</strong>: <M>{"p \\approx 2^{256}"}</M> (4
                limbs)
              </p>
              <p>
                <strong>Pasta/Vesta</strong>: <M>{"p \\approx 2^{255}"}</M> (4
                limbs)
              </p>
            </div>
            <p className="text-xs text-muted-foreground mt-2">
              6-limb 필드는 4-limb 대비 ~2.5배 느림 — 곱셈이 <M>{"O(n^2)"}</M>
            </p>
          </div>
        </div>
      </div>
