...
export default function ModernFieldImplementationArticle() {
  return <article className="space-y-14">
    ...
    <section id="parameter-artifact" className="space-y-6">
      ...
      <ExplainedFormula question="Prime p를 L개 limbs와 Montgomery constants로 옮길 때 무엇을 계산할까?" idea={<>Word width에 맞춰 R을 p보다 큰 2의 거듭제곱으로 잡고, 입력 x를 xR mod p로 저장합니다.</>} formula={String.raw`L=\left\lceil\frac{\operatorname{bitlen}(p)}{w}\right\rceil,\quad R=2^{wL},\quad R_2=R^2\bmod p,\quad n'=-p_0^{-1}\bmod 2^w`} terms={[
    ...
    <section id="serialization" className="space-y-6">
      ...
      <ExplainedFormula question="같은 field element에 여러 byte 표현이 생기지 않게 하려면?" idea={<>정해진 길이와 byte order로 정수 x를 복원하고 canonical range에 들어온 경우만 승인합니다.</>} formula={String.raw`x=\sum_{i=0}^{B-1} b_i2^{8i},\qquad \operatorname{admit}(b)\iff |b|=B\ \land\ 0\le x<p`} terms={[
        {symbol:"b_i",name:"Byte",description:"Little-endian encoding의 i번째 byte입니다."},{symbol:"B",name:"Encoded length",description:"Profile이 정한 고정 byte 수입니다."},{symbol:"x",name:"Decoded integer",description:"Domain 변환 전 canonical 정수입니다."},{symbol:"p",name:"Prime modulus",description:"x가 반드시 작아야 하는 field modulus입니다."},{symbol:"\operatorname{admit}",name:"Admission predicate",description:"Bytes를 typed field element로 승격할 조건입니다."},
    ...
    <section id="execution-profile" className="space-y-6">
      ...
      <ExplainedFormula question="Montgomery reduction이 2L-limb 곱 T를 다시 L limbs로 만드는 핵심은?" idea={<>T의 낮은 R 영역을 상수 n′로 상쇄하는 m을 고른 뒤 p의 배수를 더하고 R로 나눕니다.</>} formula={String.raw`m=(T\bmod R)n'\bmod R,\qquad u=\frac{T+mp}{R},\qquad z=u\ge p\ ?\ u-p:u`} terms={[
    ...
    <section id="release-gate" className="space-y-6">
      ...
      <ExplainedFormula question="성능 수치가 correctness 실패를 숨기지 않게 어떤 비율을 보고할까?" idea={<>검증을 통과한 operations만 wall-clock 시간의 분자에 넣고 실패를 별도 count로 남깁니다.</>} formula={String.raw`R_{valid}=\frac{N_{total}-N_{failed}}{T_{setup}+T_{ops}+T_{check}}`} terms={[
