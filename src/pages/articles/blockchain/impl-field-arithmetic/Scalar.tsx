    ...
    <section id="scalar" className="mb-16 scroll-mt-20">
      ...
      <div className="prose prose-neutral dark:prose-invert max-w-none">
        ...
        <p className="leading-7">
          ...
          <br />
          매크로 내부에서 INV를 Newton법으로 컴파일 타임 자동 계산 — 수동 계산
          불필요
        </p>
      </div>
      ...
      <div className="prose prose-neutral dark:prose-invert max-w-none mt-6">
        ...
        <div className="not-prose my-4 grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="bg-muted/50 rounded-xl p-5 border border-border">
            ...
            <p className="text-sm">
              곡선 E는 base field <M>{"\\mathbb{F}_p"}</M> 위에 정의
            </p>
            <p className="text-sm mt-1">
              점 <M>{"P = (x, y)"}</M>, 좌표 <M>{"x, y \\in \\mathbb{F}_p"}</M>
            </p>
            <p className="text-sm mt-1">
              곡선 위수 <M>{"\\#E(\\mathbb{F}_p) = N \\approx p"}</M>
            </p>
            <p className="text-sm mt-1">
              N의 최대 소인수 r &rarr; scalar field <M>{"\\mathbb{F}_r"}</M>
            </p>
          </div>
          <div className="bg-muted/50 rounded-xl p-5 border border-border">
            <p className="text-sm font-semibold text-muted-foreground mb-2">
            ...
            <p className="text-sm">
              Base field: <M>{"\\mathbb{F}_p"}</M>,{" "}
              <M>{"p \\approx 2^{254}"}</M>
            </p>
            <p className="text-sm mt-1">
              Scalar field: <M>{"\\mathbb{F}_r"}</M>,{" "}
              <M>{"r \\approx 2^{254}"}</M>
            </p>
        ...
        <div className="not-prose my-4 grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div className="bg-muted/50 rounded-xl p-5 border border-border">
            <p className="text-sm font-semibold text-muted-foreground mb-2">
              <M>{"\\mathbb{F}_r"}</M> (Scalar Field) 용도
            </p>
            <p className="text-sm">
              스칼라 곱셈: <M>{"s \\cdot P"}</M>
            </p>
        ...
        <div className="not-prose my-4 bg-muted/50 rounded-xl border border-border overflow-hidden">
          ...
          <div className="p-5 text-sm space-y-2">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mt-3">
              <div>
                <p className="text-xs">
                  <M>{"p - 1"}</M>: 2-adicity = <strong>1</strong> (2로 1회만
                  나눠짐)
                </p>
              </div>
              <div>
                <p className="text-xs">
                  <M>{"r - 1"}</M>: 2-adicity = <strong>28</strong>{" "}
                  (FFT-friendly)
                </p>
                <p className="text-xs text-muted-foreground mt-1">
                  <M>{"r - 1 = 2^{28} \\cdot 3^2 \\cdot 13 \\cdot \\ldots"}</M>{" "}
                  &rarr; NTT <M>{"2^{28}"}</M>까지 가능
                </p>
              </div>
            </div>
          </div>
        ...
        <div className="not-prose my-4 bg-muted/50 rounded-xl p-5 border border-border">
          <p className="text-sm font-semibold text-muted-foreground mb-2">
            r-1이 2-smooth해야 하는 이유 (ZK)
          </p>
          <p className="text-sm">
            NTT는 transform 크기 n이 <M>{"n \\mid (p - 1)"}</M>이어야 동작
          </p>
          <p className="text-sm mt-1">
            ZK 회로 m개 제약 &rarr; <M>{"n = 2^{\\lceil \\log_2 m \\rceil}"}</M>
            , 일반적으로 <M>{"2^{20} \\sim 2^{28}"}</M>
          </p>
          <p className="text-sm mt-1 text-muted-foreground">
            <M>{"r - 1"}</M>의 2-adicity 28 덕분에 <M>{"2^{28}"}</M> 크기 NTT
            지원
          </p>
        </div>
        ...
        <div className="not-prose my-4 bg-muted/50 rounded-xl border border-border overflow-hidden">
          ...
          <div className="p-5 grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm">
            <div>
              ...
              <p>
                <code className="text-xs bg-muted px-1 rounded">
                  trait Field: Add + Mul + Neg + Inv + From&lt;u64&gt;
                </code>
              </p>
            ...
            <div>
              ...
              <p>
                <code className="text-xs bg-muted px-1 rounded">
                  fn polynomial_evaluate&lt;F: Field&gt;(coeffs: &[F], x: F)
                  -&gt; F
                </code>
              </p>
              <p className="text-xs text-muted-foreground mt-1">
        ...
        <div className="not-prose my-4 grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div className="bg-red-50 dark:bg-red-950/30 rounded-xl p-5 border border-red-200 dark:border-red-800">
            ...
            <p className="text-sm">
              <strong>1.</strong> 잘못된 필드로 스칼라 곱:{" "}
              <M>{"s \\in \\mathbb{F}_p"}</M>로 <M>{"s \\cdot P"}</M> &rarr;
              수학적 오류
            </p>
            <p className="text-sm mt-1">
              <strong>2.</strong> 리듀스 누락: 뺄셈 후 &gt; p일 수 있음 &rarr;
              조건부 감산 필수
            </p>
            <p className="text-sm mt-1">
              <strong>3.</strong> Montgomery 형태 혼동: Mont-form과 일반 값 혼합
              &rarr; 타입 구분으로 방지
            </p>
          </div>
        </div>
