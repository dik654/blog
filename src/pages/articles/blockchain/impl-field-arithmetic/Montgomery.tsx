    ...
    <section id="montgomery" className="mb-16 scroll-mt-20">
      ...
      <div className="prose prose-neutral dark:prose-invert max-w-none mb-8">
        <p className="leading-7">
          ...
          <br />
          Montgomery 해법: 수를 a * R mod p 형태로 저장하면 나눗셈을 시프트로
          대체
          <br />R = 2^256이므로 "R로 나누기" = 256비트 오른쪽 시프트
        </p>
        <p className="leading-7">
      ...
      <div className="prose prose-neutral dark:prose-invert max-w-none mt-6">
        ...
        <div className="not-prose my-4 grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="bg-muted/50 rounded-xl p-5 border border-border">
            ...
            <p className="text-sm">
              <M>{"(a \\times b) \\bmod p"}</M> 를 효율적으로 계산해야 함
            </p>
            <p className="text-sm mt-1">
              직접 방식: 곱한 뒤 p로 나눗셈 — CPU 나눗셈 ~20-40 사이클로 느림
            </p>
          </div>
          <div className="bg-muted/50 rounded-xl p-5 border border-border">
            <p className="text-sm font-semibold text-muted-foreground mb-2">
        ...
        <div className="not-prose my-4 bg-muted/50 rounded-xl border border-border overflow-hidden">
          ...
          <div className="p-5 grid grid-cols-1 sm:grid-cols-3 gap-4 text-sm">
            ...
            <div>
              ...
              <p>
                <M>{"x_{mont} = x \\cdot R \\bmod p"}</M>
              </p>
              <p className="text-xs text-muted-foreground mt-1">
                <M>{"R = 2^{256}"}</M> (256비트 필드)
              </p>
            </div>
            <div>
        ...
        <div className="not-prose my-4 bg-muted/50 rounded-xl border border-border overflow-hidden">
          ...
          <div className="p-5 grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm">
            <div>
              ...
              <p>
                <M>{"\\text{REDC}(T) = T \\cdot R^{-1} \\bmod p"}</M>
              </p>
              <p className="text-xs text-muted-foreground mt-1">
                T: 2n-limb 입력 (<M>{"0 \\le T < p \\cdot R"}</M>)
              </p>
            ...
            <div>
              ...
              <p className="text-xs mt-1">
                따라서 <M>{"T = (\\text{something}) \\cdot R"}</M>
              </p>
              <p className="text-xs mt-1">
                R로 나누기 = 256비트 오른쪽 시프트 = 하위 limbs 버림
              </p>
              <p className="text-xs text-muted-foreground mt-3 mb-1">
                핵심 합동식
              </p>
              <p className="text-xs">
                <M>{"T + q \\cdot p \\equiv T \\pmod{p}"}</M> — p의 배수 더하기
              </p>
              <p className="text-xs mt-1">
                <M>{"T + q \\cdot p \\equiv 0 \\pmod{R}"}</M> — q 선택에 의해
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="not-prose my-4 bg-muted/50 rounded-xl p-5 border border-border">
          ...
          <p className="text-sm">
            <strong>Step 1</strong>: schoolbook 곱셈 — 2n-limb 결과{" "}
            <M>{"T = a \\cdot b"}</M>
          </p>
          <p className="text-sm mt-1">
            <strong>Step 2</strong>: REDC(T) —{" "}
            <M>{"T \\cdot R^{-1} \\bmod p"}</M> 반환
          </p>
          <p className="text-xs text-muted-foreground mt-3">
            a, b가 Montgomery 형태 (<M>{"aR, bR"}</M>)일 때:
          </p>
          <p className="text-sm mt-1">
            <M>{"(aR)(bR) \\cdot R^{-1} = (ab) \\cdot R"}</M> — 결과도
            Montgomery 형태
          </p>
        </div>
        ...
        <div className="not-prose my-4 grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="bg-muted/50 rounded-xl border border-border overflow-hidden">
            ...
            <div className="p-5 text-sm">
              <p>
                <M>{"\\text{INV} = -p^{-1} \\bmod 2^{64}"}</M>
              </p>
              <p className="text-xs text-muted-foreground mt-2">
                Newton법 (매 단계 정밀도 2배)
              </p>
              <p className="text-xs mt-1">
                <code className="text-xs bg-muted px-1 rounded">inv = 1</code>
              </p>
              <p className="text-xs">
                <code className="text-xs bg-muted px-1 rounded">
                  inv = inv * (2 - p * inv)
              ...
              <p className="text-xs text-muted-foreground mt-1">
                정밀도: 1 &rarr; 2 &rarr; 4 &rarr; 8 &rarr; 16 &rarr; 32 &rarr;
                64비트
              </p>
              <p className="text-xs mt-1">
                <code className="text-xs bg-muted px-1 rounded">
          ...
          <div className="bg-muted/50 rounded-xl border border-border overflow-hidden">
            ...
            <div className="p-5 text-sm">
              <p>
                <M>{"a \\in \\mathbb{F}_p^*: a^{p-1} = 1 \\pmod{p}"}</M>
              </p>
              <p className="mt-1">
                <M>{"a^{p-2} = a^{-1} \\pmod{p}"}</M>
              </p>
              <p className="text-xs text-muted-foreground mt-2">
                구현:{" "}
                <code className="text-xs bg-muted px-1 rounded">
              ...
              <p className="text-xs text-muted-foreground mt-1">
                비용: ~256 제곱 + ~128 곱 = ~384 Montgomery mults
              </p>
              <p className="text-xs text-muted-foreground mt-1">
                상수 시간 — 암호학에 적합
              </p>
              <p className="text-xs text-muted-foreground mt-3">
                대안: 확장 유클리드 — 더 빠르지만 NOT 상수 시간
              </p>
            </div>
          </div>
        </div>
          ...
          <div className="bg-muted/50 rounded-xl border border-border overflow-hidden">
            ...
            <div className="p-5 text-sm space-y-1">
              <div className="flex justify-between">
                ...
                <span>~18 사이클 (adx/bmi2)</span>
              </div>
              <div className="flex justify-between">
                <span>
                  <code className="text-xs bg-muted px-1 rounded">mont_sq</code>
                </span>
                <span>~15 사이클</span>
              </div>
              <div className="flex justify-between">
                <span>
                  <code className="text-xs bg-muted px-1 rounded">add/sub</code>
                </span>
                <span>~2-3 사이클</span>
              </div>
              <div className="flex justify-between">
                <span>
                  <code className="text-xs bg-muted px-1 rounded">inv</code>{" "}
                  (pow)
                </span>
                <span>~7000 사이클</span>
              </div>
              ...
              <p className="text-xs text-muted-foreground">
                <M>{"\\text{inv}(a_1), \\ldots, \\text{inv}(a_n)"}</M> 한 번에 —
                비용: 3(n-1) mults + 1 inv
              </p>
            </div>
          </div>
          <div className="bg-muted/50 rounded-xl p-5 border border-border">
