...
export default function KinkModelViz() {
  ...
  const calcBorrow = (u: number) => {
    ...
    return (
      borrowBase +
      borrowSlopeLow +
      (borrowSlopeHigh * (u - borrowKink)) / (100 - borrowKink)
    );
  };
  ...
  const calcSupply = (u: number) => {
    ...
    return (
      supplyBase +
      supplySlopeLow +
      (supplySlopeHigh * (u - borrowKink)) / (100 - borrowKink)
    );
  };
  ...
  const borrowPath = borrowPoints
    .map(
      (p, i) =>
        `${i === 0 ? "M" : "L"} ${toX(p.u)} ${toY(Math.min(p.r, maxRate))}`,
    )
    .join(" ");
    ...
    <div className="not-prose my-6 rounded-lg border border-border bg-card p-4">
      ...
      >
        ...
        <rect
          ...
          fillOpacity={0.4}
        />
        <text x={50} y={50} fontSize={10} fontWeight={600} fill="#ef4444">
          Borrow rate
        </text>
        <rect
          x={160}
          ...
          fillOpacity={0.4}
        />
        <text x={180} y={50} fontSize={10} fontWeight={600} fill="#10b981">
          Supply rate
        </text>
        <rect
          x={290}
          ...
          fillOpacity={0.4}
        />
        <text x={310} y={50} fontSize={10} fontWeight={600} fill="#f59e0b">
          Kink (90%)
        </text>
        ...
        {/* Supply 영역 */}
        <path
          d={`${supplyPath} L ${padLeft + w} ${66 + h} L ${padLeft} ${66 + h} Z`}
          fill="#10b981"
          fillOpacity={0.1}
        />
        {/* Borrow 곡선 */}
        ...
        <circle
          cx={toX(utilization)}
          cy={toY(Math.min(borrowRate, maxRate))}
          r={5}
          fill="#ef4444"
          stroke="var(--card)"
          strokeWidth={1.5}
        ...
        <circle
          cx={toX(utilization)}
          cy={toY(Math.min(supplyRate, maxRate))}
          r={5}
          fill="#10b981"
          stroke="var(--card)"
          strokeWidth={1.5}
