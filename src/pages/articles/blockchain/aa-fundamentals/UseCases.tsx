    ...
    <section id="use-cases" className="mb-16 scroll-mt-20">
      ...
      <div className="prose prose-neutral dark:prose-invert max-w-none">
        ...
        <p className="leading-7">
          사용자가 생체 인증(Face ID, 지문)으로 트랜잭션에 서명합니다.
          <br />
          P-256(secp256r1) 서명을 validateUserOp()에서 검증합니다.
          <br />
        ...
        <p className="leading-7">
          제한된 권한의 임시 키를 발급합니다.
          <br />
          유효 기간, 호출 가능 함수, 지출 한도를 온체인에서 강제합니다.
          <br />
        ...
        <p className="leading-7">
          approve + swap을 하나의 UserOp로 묶어 실행합니다.
          <br />
          사용자 경험이 향상되고 가스도 절약됩니다.
          <br />
        ...
        <p className="leading-7">
          프로젝트가 사용자의 가스비를 대신 지불합니다.
          <br />
          ERC-20 토큰으로 가스 결제, 무료 체험, 구독 모델 등이 가능합니다.
          <br />
      ...
      <div className="prose prose-neutral dark:prose-invert max-w-none mt-6">
        ...
        <div className="rounded-xl border p-4 bg-card text-sm mb-6">
          <p className="mb-2">
            Secure Enclave에서 P-256 키쌍 생성. 개인키는 디바이스 밖으로 나가지
            않음. Face ID / Touch ID / Windows Hello로 서명.
          </p>
          <p className="mb-2 text-muted-foreground">
            <code>validateUserOp()</code>:{" "}
            <code>sha256(authenticatorData || sha256(clientData))</code> 해시에
            대해 P-256 <code>ecRecover</code>로 공개키 검증.
          </p>
          <p className="text-muted-foreground">
            가스: ~200K (RIP-7212 P-256 프리컴파일 적용 시 ~20K). 운영: Coinbase
            Smart Wallet, Safe&#123;Wallet&#125;, OKX.
          </p>
        </div>
        ...
        <div className="rounded-xl border p-4 bg-card text-sm mb-6">
          <p className="mb-3">
            DeFi 자동화 등 빈번한 서명 문제를 해결. 제한된 권한의 임시 키를
            위임.
          </p>
          ...
          <p className="text-muted-foreground">
            <code>validateUserOp()</code>: 세션 미만료 + 타겟 화이트리스트 확인
            후 <code>ecRecover</code>. 운영: Argent X, Zerion, Sequence.
          </p>
        </div>
        ...
        <div className="rounded-xl border p-4 bg-card text-sm mb-6">
          <p className="mb-2">
            <code>executeBatch(targets[], values[], datas[])</code> —
            EntryPoint만 호출 가능. 각 타겟에 대해{" "}
            <code>call&#123;value&#125;(data)</code> 순차 실행.
          </p>
          <div className="rounded-lg border p-3 bg-background mt-3">
            <p className="font-semibold text-xs mb-1">DEX 스왑 예시</p>
            <p className="text-xs text-muted-foreground">
              1. <code>approve(USDC, router, amount)</code> → 2.{" "}
              <code>router.swap(USDC, WETH, amount)</code> → 1개 UserOp, 원자적,
              가스 절약.
            </p>
          </div>
        </div>
        ...
        <div className="rounded-xl border p-4 bg-card text-sm mb-6">
          <p className="mb-2">
            <code>owners[]</code> + <code>threshold</code> (예: 3명 중 2명).{" "}
            <code>validateUserOp()</code>에서 서명 배열을 디코드하여 각 서명을{" "}
            <code>ecRecover</code>로 검증, 유효 서명 수가 threshold 이상이면
            통과.
          </p>
        </div>
        ...
        <div className="rounded-xl border p-4 bg-card text-sm mb-6">
          <p className="font-semibold text-xs mb-2">
            Guardian 시스템 (Argent 스타일)
          </p>
          <p className="mb-2 text-muted-foreground">
            <code>owner</code> + <code>guardians[]</code> +{" "}
            <code>recoveryThreshold</code>.
          </p>
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 mb-3">
            <div className="rounded-lg border p-2 bg-background text-center">
              <p className="text-xs font-semibold">1</p>
              <p className="text-xs text-muted-foreground">
                N명 가디언이 new_owner 메시지 서명
              </p>
            </div>
            <div className="rounded-lg border p-2 bg-background text-center">
            ...
            <div className="rounded-lg border p-2 bg-background text-center">
              ...
              <p className="text-xs text-muted-foreground">
                기간 경과 후 new_owner 인수
              </p>
            </div>
          </div>
          <p className="text-muted-foreground">
            변형: 이메일 OTP 가디언, 하드웨어 키 가디언, 친구 컨트랙트 복구,
            zk-guardian (비밀 지식 증명).
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-6">
          <div className="rounded-xl border p-4 bg-card">
            ...
            <ol className="text-xs text-muted-foreground space-y-1 list-decimal list-inside">
              ...
              <li>
                Paymaster가 <code>(op, validUntil, validAfter)</code> 서명
              </li>
              <li>
                서명을 <code>paymasterAndData</code>에 추가
              </li>
          ...
          <div className="rounded-xl border p-4 bg-card">
            ...
            <ol className="text-xs text-muted-foreground space-y-1 list-decimal list-inside">
              ...
              <li>
                Paymaster가 <code>USDC.transferFrom</code> 실행
              </li>
        ...
        <div className="rounded-xl border p-4 bg-card text-sm mb-6">
          <p className="mb-2">
            사용자가 USDC만 보유, ETH 없음:{" "}
            <code>UserOp.paymasterAndData = TokenPaymaster</code>
          </p>
          <p className="text-muted-foreground">
            1. Paymaster가 <code>transferFrom</code>으로 USDC 잠금 → 2.
            EntryPoint에 prefund → 3. <code>postOp</code>에서 USDC로 가스 환불.
          </p>
        </div>

        <h4 className="font-semibold mt-6 mb-3">
          8. 프로그래머블 권한 (ERC-7579 모듈)
        </h4>
        <div className="rounded-xl border p-4 bg-card text-sm mb-6">
          ...
          <p className="text-muted-foreground">
            플러그앤플레이: SocialRecoveryValidator, TwoFactorValidator,
            GaslessSessionKeyModule, IntentHandler.
          </p>
        </div>
        ...
        <div className="flex flex-wrap gap-2">
          {[
            ...
            <span
              key={p}
              className="rounded-full border px-3 py-1 text-sm bg-card"
            >
              {p}
            </span>
          ))}
