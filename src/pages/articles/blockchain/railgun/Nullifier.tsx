...
export default function Nullifier({
    ...
    <section id="nullifier" className="mb-16 scroll-mt-20">
      ...
      <div className="prose prose-neutral dark:prose-invert max-w-none mb-6">
        <p className="leading-7">
          ...
          <br />
          <code>nullifier = poseidon(spendingKey, leafIndex)</code>. 같은 Note는
          항상 같은 nullifier를 생성한다.
        </p>
        <p className="leading-7">
        ...
        <p className="leading-7">
          ...
          <br />
          Poseidon의 단방향성 때문이다. 어떤 commitment가 소비됐는지 외부에서 알
          수 없다.
        </p>
      </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm text-muted-foreground">
              <div>
                ...
                <ul className="space-y-0.5">
                  ...
                  <li>
                    <code>nullifier = Poseidon(spendingKey, leafIndex)</code>
                  </li>
                </ul>
              </div>
              <div>
                ...
                <ul className="space-y-0.5">
                  <li>
                    nullifier에서 <code>spendingKey</code> 추출 불가
                  </li>
                  <li>
                    nullifier에서 <code>leafIndex</code> 추출 불가
                  </li>
                </ul>
              </div>
              <div>
                ...
                <ul className="space-y-0.5">
                  ...
                  <li>
                    collision 확률 ~2<sup>-248</sup> (무시 가능)
                  </li>
                </ul>
              </div>
            </div>
          ...
          <div className="rounded-lg border border-red-500/30 p-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm text-muted-foreground">
              <div>
                ...
                <p>
                  nullifier로부터 <code>spendingKey</code> 역산 &rarr; 2
                  <sup>256</sup> search space, 불가능
                </p>
              </div>
              <div>
                <p className="font-medium text-foreground/80 mb-1">
                ...
                <p>
                  이전 tx의 nullifier 재사용 &rarr; <code>nullifiers</code>{" "}
                  map에서 이미 <code>true</code>면 revert
                </p>
              </div>
              <div>
                <p className="font-medium text-foreground/80 mb-1">
                ...
                <p>
                  mempool에서 proof 탈취 &rarr; proof binding으로 다른 addr
                  submit 시 verify 실패
                </p>
              </div>
            </div>
        ...
        <div className="not-prose space-y-4 my-4">
          <div className="rounded-lg border border-border/60 p-4">
            <p className="font-semibold text-sm text-muted-foreground mb-2">
              기본 구조:{" "}
              <code>mapping(bytes32 =&gt; bool) public nullifiers</code>
            </p>
            <p className="text-sm text-muted-foreground">
              문제: 매 tx마다 새 slot 할당 &rarr; 영구 storage (삭제 불가)
              &rarr; Note 증가에 비례해 cost 증가
            </p>
          </div>
          ...
          <div className="rounded-lg border border-violet-500/30 p-4">
            ...
            <ul className="text-sm space-y-0.5 text-muted-foreground">
              <li>
                별도 <strong>sparse Merkle tree</strong>로 nullifier 관리 &rarr;
                gas cost 일정 + batch verification 가능
              </li>
            </ul>
          </div>
        </div>
