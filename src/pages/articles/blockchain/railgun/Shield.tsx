    ...
    <section id="shield" className="mb-16 scroll-mt-20">
      ...
      <div className="prose prose-neutral dark:prose-invert max-w-none mb-6">
        <p className="leading-7">
          <code>shield()</code>는 ERC-20 토큰을 RAILGUN 컨트랙트에 입금하는
          함수다.
          <br />
          입금 후 토큰은 shielded 상태가 된다. 외부에서 잔액을 볼 수 없다.
        </p>
        <p className="leading-7">
          내부 동작은 4단계다. transferFrom → hashCommitment → insertLeaf → emit
          Shield.
          <br />
          각 단계에서 실제 변수값과 상태 변화를 추적한다.
          <CodeViewButton
            onClick={() => onCodeRef("rg-shield", codeRefs["rg-shield"])}
          />
        </p>
      </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm text-muted-foreground">
              <div className="space-y-3">
                <div>
                  ...
                  <p className="text-xs">
                    ERC-20 표준 호출 &mdash; user &rarr; RAILGUN contract
                  </p>
                </div>
                <div>
                  <p className="font-medium text-foreground/80">
                    Step 2: Note 생성 (off-chain)
                  </p>
                  <ul className="space-y-0.5">
                    <li>
                      <code>npk</code>: 수신자 public key (Poseidon)
                    </li>
                    <li>
                      <code>token</code>: ERC-20 주소
                    </li>
                    <li>
                      <code>amount</code>: <code>uint128</code>
                    </li>
                    <li>
                      <code>random</code>: <code>bytes31</code> (unique
                      randomness)
                    </li>
                  </ul>
                </div>
              ...
              <div className="space-y-3">
                <div>
                  ...
                  <p>
                    <code>merkle_tree.insert(commitment)</code>
                  </p>
                  <p className="text-xs">
                    depth 16 &rarr; 65,536 leaves per tree. overflow 시 새 tree
                    생성.
                  </p>
                </div>
                <div>
                  ...
                  <p>
                    <code>emit Shield(token, amount, npk, commitment)</code>
                  </p>
                  <p className="text-xs">
                    Encrypted memo 포함 &mdash; 수신자만 복호화 가능
                  </p>
                </div>
              </div>
            </div>
