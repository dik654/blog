    ...
    <section id="unshield" className="mb-16 scroll-mt-20">
      ...
      <div className="prose prose-neutral dark:prose-invert max-w-none mb-6">
        <p className="leading-7">
          <code>unshield()</code>는 shielded 잔액을 일반 ERC-20으로 출금하는
          함수다.
          <br />
          ZK 증명으로 Note 소유권을 입증한 뒤, ERC-20 transfer로 토큰을
          전송한다.
          <CodeViewButton
            onClick={() => onCodeRef("rg-unshield", codeRefs["rg-unshield"])}
          />
        </p>
        <p className="leading-7">
          출금 시점에 수신 주소(to)와 금액(amount)이 공개된다.
          <br />
          하지만 어떤 commitment에서 출금했는지는 nullifier 뒤에 숨는다. 자금
          출처를 추적할 수 없다.
        </p>
      </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="rounded-lg border border-red-500/30 p-4">
              ...
              <ul className="text-sm space-y-0.5 text-muted-foreground">
                <li>
                  Recipient address (<code>to</code>)
                </li>
                <li>Token address</li>
                <li>Amount</li>
          ...
          <div className="rounded-lg border border-border/60 p-4">
            ...
            <div className="grid grid-cols-3 gap-3 text-sm text-center text-muted-foreground">
              <div className="bg-muted/50 rounded p-2">
                Arbitrary amount
                <br />
                <span className="text-xs">(denomination lock 없음)</span>
              </div>
              <div className="bg-muted/50 rounded p-2">
                Timing 분산 가능
                <br />
                <span className="text-xs">(자유로운 출금 시점)</span>
              </div>
              <div className="bg-muted/50 rounded p-2">
                중간 private transfer
                <br />
                <span className="text-xs">(연속 내부 거래)</span>
              </div>
            </div>
          </div>
          <div className="rounded-lg border border-amber-500/30 p-4">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3 text-sm text-muted-foreground">
              ...
              <div>
                ...
                <p>
                  Shield &rarr; Transfer &rarr; Transfer &rarr; Unshield &mdash;
                  graph analysis 복잡화
                </p>
              </div>
              <div>
