...
import { STEPS, STEP_REFS, STEP_LABELS } from './SyncDetailVizData';
import { Step0, Step1, Step2, Step3 } from './SyncDetailSteps';

const R = [Step0, Step1, Step2, Step3];

interface Props { onOpenCode?: (key: string) => void }

export default function SyncDetailViz({ onOpenCode }: Props) {
  return (
    <StepViz steps={STEPS}>
      {(step) => {
        const S = R[step];
