...
const STEPS = [
  {
    label: "17개 커스텀 모듈 구성",
    body: "dYdX v4 — 표준 Cosmos SDK + 17개 dYdX 전용 모듈.\nCLOB이 가장 많은 의존성, Genesis 초기화 시 마지막 로드.",
  },
  {
    label: "Trading: 거래 핵심 모듈",
    body: "clob(주문서) + perpetuals(영구 선물) + subaccounts(서브계정) + feetiers(수수료 등급).\nBeginBlocker/EndBlocker에서 주문 정리 + 펀딩 레이트 업데이트.",
  },
  {
    label: "Core: 인프라 모듈",
    body: "prices(오라클 가격 피드) + assets(자산) + epochs(에포크) + blocktime(블록 시간).\nBeginBlocker에서 순서: blocktime → epochs → prices → assets.",
  },
  {
    label: "Support: 보조 모듈",
    body: "bridge(이더리움 브릿지) + rewards(보상) + delaymsg(지연 메시지) + stats(통계).\n크로스체인 브릿지와 보상 시스템을 위한 독립 모듈.",
  },
];
