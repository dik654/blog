...
const STEPS = [
  ...
  {
    label: "주문 검증",
    body: "계정 잔고, 마진, 주문 유효성, 속도 제한 등을 검증합니다.",
  },
  {
    label: "매칭 시도",
    body: "반대 방향에서 가격-시간 우선순위로 매칭 대상을 탐색합니다.",
  },
  {
    label: "체결 처리",
    body: "매칭 수량 결정 → 담보 확인 → 체결을 기록합니다.",
  },
  {
    label: "주문서 추가",
    body: "미체결 잔량이 있으면 해당 가격 레벨에 추가합니다.",
  },
];
        ...
        >
          ...
          {NODES.map((n, i) => {
            ...
            return (
              <motion.g
                key={i}
                animate={{ opacity: vis || active ? 1 : 0.15 }}
                transition={sp}
              >
                <rect
                  x={x}
