...
import {
  ...
  PACKING_CODE,
  PARTITION_CODE,
} from "./DataPackingData";
import { CodeViewButton } from "@/components/code";
import type { CodeRef } from "@/components/code/types";
import { codeRefs } from "./codeRefs";

export default function DataPacking({
  title,
  onCodeRef,
}: {
  title?: string;
    ...
    <section id="data-packing" className="mb-16 scroll-mt-20">
      ...
      <div className="prose prose-neutral dark:prose-invert max-w-none">
        <p>
          ...
          <strong>매트릭스 패킹</strong> 알고리즘을 적용해 효율적인 샘플링
          검증을 지원합니다.
        </p>
        ...
        {onCodeRef && (
          <div className="not-prose flex flex-wrap gap-2 my-4">
            <CodeViewButton
              onClick={() => onCodeRef("irys-unpack", codeRefs["irys-unpack"])}
            />
            ...
            <CodeViewButton
              onClick={() =>
                onCodeRef("irys-xor-pack", codeRefs["irys-xor-pack"])
              }
            />
            <span className="text-[10px] text-muted-foreground self-center">
              XOR 유틸
        ...
        <h3>매트릭스 패킹</h3>
        <p>
          노드 주소·파티션 해시·오프셋을 종자로 2단계 엔트로피를 생성한 후 청크 데이터와 XOR하여 특정 노드만이 특정 청크를 언패킹할 수 있게 합니다.
        </p>
        <CodePanel
          title="2-Phase 엔트로피 + XOR 패킹"
          code={PACKING_CODE}
          annotations={[
        ...
        <CodePanel
          title="파티션 구조"
          code={PARTITION_CODE}
          annotations={[{ lines: [2, 4], color: "sky", note: "파티션 식별" }]}
        />
      </div>
    </section>
  );
