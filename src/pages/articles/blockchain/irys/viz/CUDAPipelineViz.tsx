...
const STEPS = [
  {
    label: "CUDA 가속 파이프라인",
    body: "Rust FFI → CUDA C → GPU 커널 → 호스트 복사의 4단계 파이프라인.",
  },
  {
    label: "Rust → CUDA FFI",
    body: 'cfg(feature = "nvidia") 게이트로 CUDA 사용 가능 시에만 GPU 경로 활성화.',
  },
  {
    label: "GPU 커널 병렬 실행",
    body: "각 스레드가 하나의 청크 엔트로피를 독립 계산. 수천 스레드 동시 실행.",
  },
  {
    label: "결과 전송",
    body: "계산 완료 후 cudaMemcpy로 GPU → CPU 메모리 복사.",
  },
];
