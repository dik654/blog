...
const STEPS = [
  {
    label: "가십 프로토콜 아키텍처",
    body: "GossipServer가 수신, DataHandler가 검증·라우팅, Cache가 중복 제거.",
  },
  {
    label: "수신 & 검증",
    body: "GossipServer → DataHandler로 메시지 전달. IP 검증 + 피어 인증.",
  },
  {
    label: "중복 제거 & 라우팅",
    body: "GossipCache로 중복 확인 후 BlockPool/Mempool로 분배.",
  },
];
            ...
            {EDGES.map(([fi, ti], ei) => {
              ...
              return (
                <motion.line
                  ...
                  y2={t.y}
                  stroke="#666"
                  strokeWidth={show ? 1 : 0.4}
                  strokeDasharray="3 2"
                  animate={{ opacity: show ? 0.5 : 0.1 }}
