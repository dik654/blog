...
const STEPS = [
  {
    label: "구조화 로깅",
    body: "Rust tracing 크레이트로 모듈별 레벨 제어와 JSON 형식 출력을 지원합니다.",
  },
  {
    label: "내장 메트릭",
    body: "VDF 단계 수, 블록 높이, P2P 피어 수 등 핵심 메트릭을 실시간 수집합니다.",
  },
  {
    label: "시각화",
    body: "Prometheus가 메트릭을 스크랩하고 Grafana 대시보드에서 시각화합니다.",
  },
  {
    label: "알림",
    body: "블록 지연, 피어 감소 등 임계값 초과 시 Alertmanager로 알림을 전송합니다.",
  },
];
        ...
        >
          {EDGES.map((e, i) => {
            ...
            const f = NODES[e.from],
              t = NODES[e.to];
            const mx = (f.x + t.x) / 2 + 8,
              my = (f.y + 15 + t.y + 15) / 2 + 5;
            return (
              <motion.g
                key={i}
                animate={{ opacity: vis ? 0.7 : 0.05 }}
                transition={sp}
              >
                <line
                  x1={f.x}
