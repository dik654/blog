...
const STEPS = [
  {
    label: "이전 블록 해시",
    body: "이전 블록의 해시가 VDF 계산의 시드로 사용됩니다.",
  },
  {
    label: "VDF 순차 계산",
    body: "SHA256을 수십만 회 반복합니다. 병렬화 불가능하므로 시간 증명으로 기능합니다.",
  },
  {
    label: "VDF 검증",
    body: "중간 체크포인트로 O(root N) 복잡도로 빠르게 검증할 수 있습니다.",
  },
  {
    label: "리콜 범위 결정",
    body: "VDF 출력값으로 데이터셋 내 리콜 오프셋을 결정합니다.",
  },
  {
    label: "청크 리콜",
    body: "해당 오프셋의 청크를 로컬 스토리지에서 읽어 저장 증명을 생성합니다.",
  },
  {
    label: "매트릭스 패킹",
    body: "노드별 고유 엔트로피로 XOR 패킹된 데이터를 검증합니다.",
  },
  {
    label: "블록 생성",
    body: "VDF 증명 + 저장 증명을 포함한 블록을 생성하고 전파합니다.",
  },
];
          ...
          {EDGES.map(([a, b, lbl], i) => {
            ...
            return (
              ...
              >
                ...
                />
                {(() => {
                  const tx = (na.x + nb.x) / 2,
                    ty = Math.min(na.y, nb.y) - 4;
                  return (
                    <>
                      <rect
                        x={tx - 18}
