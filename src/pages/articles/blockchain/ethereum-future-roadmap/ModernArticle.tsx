...
export default function ModernEthereumFutureRoadmapArticle() {
  return <article className="space-y-14">
    ...
    <section id="pq-surfaces" className="space-y-6">
      ...
      <p>Account migration의 구체적인 ML-DSA·ERC-4337 경계는 <Link className="text-primary underline" to="/cs/blockchain/pq-account">Post-Quantum Account</Link> 글을 재사용합니다. SPHINCS+/SLH-DSA 같은 hash-based signature와 lean 계열 프로젝트 명칭은 후보·연구 방향으로 표시하고, roadmap 문구만으로 최종 protocol 채택을 확정하지 않습니다.</p>
    </section>
    <section id="proving-execution" className="space-y-6">
      ...
      <p>Poseidon처럼 workload를 field-friendly하게 바꾸는 선택과 SHA/BLAKE를 유지한 채 prover를 바꾸는 선택은 <Link to="/cs/crypto/binary-field-proving#overview" className="text-primary underline">Binary-field proving</Link>에서 비교합니다. PBT는 문맥에 따라 proposal-based builder/proposer separation 또는 tree/state commitment 문맥이 다를 수 있으므로 약어만으로 새 canonical component를 만들지 않습니다.</p>
    </section>
    <section id="formal-simplification" className="space-y-6">
      ...
      operations={[
        { expression: String.raw`\underbrace{G_{AI}(s,i)}_{\text{proof candidate 생성}}`, annotation: ["Formal specification이(가) 식의 결과에","기여하는 방식을 계산합니다.","AI가 만든 proof candidate를 채택하지 않고,","formal kernel이 type-check한 결과만 검증된"] },
        { expression: String.raw`\underbrace{K(s,i,c)}_{\text{deterministic kernel 검사}}`, annotation: ["Formal specification이(가) 식의 결과에","기여하는 방식을 계산합니다.","AI가 만든 proof candidate를 채택하지 않고,","formal kernel이 type-check한 결과만 검증된"] },
        { expression: String.raw`\underbrace{[v=1]}_{\text{최종 Boolean 판정}}`, annotation: ["왼쪽 결과를 오른쪽의 실제 항으로 계산합니다.","AI가 만든 proof candidate를 채택하지 않고,","formal kernel이 type-check한 결과만 검증된","theorem으로 인정합니다."] },
      ]} terms={[{symbol:"s",name:"Formal specification",description:"정확한 state와 transition semantics입니다."},{symbol:"i",name:"Invariant",description:"모든 허용 transition에서 보존해야 할 명제입니다."},{symbol:"c",name:"Candidate proof",description:"AI나 사람이 제안한 proof term 또는 tactics 결과입니다."},{symbol:"K",name:"Trusted kernel",description:"Candidate가 formal rules를 만족하는지 결정적으로 검사합니다."}]} assumptions={["Specification 자체가 의도한 system과 맞는지는 별도 review 대상입니다.","Kernel·compiler·dependency version을 고정합니다.","Accepted proof는 성능·liveness·implementation side channel을 자동 보장하지 않습니다.","AI confidence를 verification 결과로 사용하지 않습니다."]} interpretation="AI가 그럴듯한 proof를 만들어도 K=0이면 폐기합니다. K=1이어도 빠진 assumption이나 잘못 formalized된 spec이 없는지 traceability review가 필요합니다." />
