...
const STEPS = [
  {
    label: "실전 회로 패턴",
    body: "Poseidon 해시, Merkle 증명, 범위 검사 등 프로덕션 ZK 회로 빌딩 블록",
  },
  {
    label: "Poseidon 해시",
    body: "ZK 친화적 해시 함수. Full → Partial → Full 라운드 구조로 제약 수를 최소화합니다.",
  },
  {
    label: "Merkle Proof",
    body: "Poseidon을 반복 적용하여 리프에서 루트까지 경로를 검증합니다.",
  },
];
      ...
      {(step) => (
        ...
        >
          ...
          {BOXES.map((b, i) => (
            <motion.g
              key={b.label}
              animate={{ opacity: VN[step].includes(i) ? 1 : 0.12 }}
            >
              <rect
                x={b.x}
                y={b.y}
