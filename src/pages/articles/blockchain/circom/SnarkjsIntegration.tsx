  ...
  return (
    <section id="snarkjs" className="mb-16 scroll-mt-20">
      ...
      <div className="prose prose-neutral dark:prose-invert max-w-none">
        <p>
          Circom 컴파일 결과물(<code>.r1cs</code>, <code>.wasm</code>,{" "}
          <code>.sym</code>)은 snarkjs와 연동하여 완전한 zkSNARK 증명 시스템을
          구축합니다.
          <br />
          Powers of Tau 세레모니부터 Groth16 증명/검증까지의 전체
          워크플로우입니다.
        </p>
        ...
        <div className="not-prose flex flex-col gap-2 mb-6">
          <div className="rounded-lg border bg-sky-50 dark:bg-sky-950/30 p-4 text-sm">
            <p className="text-xs font-semibold text-sky-700 dark:text-sky-300 mb-2">
              ① Powers of Tau 세레모니 (신뢰 셋업)
            </p>
            <div className="font-mono text-xs space-y-1">
              <p>
                snarkjs.powersOfTau.<strong>newAccumulator</strong>("bn128", 12,
                "pot.ptau")
              </p>
              <p>
                snarkjs.powersOfTau.<strong>contribute</strong>("pot.ptau",
                "pot1.ptau", "contrib")
              </p>
              <p>
                snarkjs.powersOfTau.<strong>preparePhase2</strong>("pot1.ptau",
                "pot_final.ptau")
              </p>
            </div>
          </div>
          <div className="rounded-lg border bg-emerald-50 dark:bg-emerald-950/30 p-4 text-sm">
            ...
            <div className="font-mono text-xs space-y-1">
              <p>
                snarkjs.zKey.<strong>newZKey</strong>("circuit.r1cs",
                "pot_final.ptau", "circuit.zkey")
              </p>
              <p>
                snarkjs.zKey.<strong>contribute</strong>("circuit.zkey",
                "circuit_final.zkey", "contrib2")
              </p>
              <p>
                snarkjs.zKey.<strong>exportVerificationKey</strong>
                ("circuit_final.zkey") → vKey
              </p>
            </div>
          </div>
          <div className="rounded-lg border bg-amber-50 dark:bg-amber-950/30 p-4 text-sm">
            ...
            <div className="font-mono text-xs space-y-1">
              ...
              <p>
                const witness = await wc.<strong>calculateWitness</strong>
                (input)
              </p>
            </div>
          </div>
          <div className="rounded-lg border bg-violet-50 dark:bg-violet-950/30 p-4 text-sm">
            ...
            <div className="font-mono text-xs space-y-1">
              <p>
                ...
                <strong>prove</strong>(zkey, witness)
              </p>
              <p>
                const valid = await snarkjs.groth16.<strong>verify</strong>
                (vKey, publicSignals, proof)
              </p>
            </div>
          </div>
        </div>
      ...
      <div className="prose prose-neutral dark:prose-invert max-w-none mt-6">
        ...
        <div className="not-prose rounded-lg border p-4 text-sm space-y-3 mb-6">
          ...
          <div className="flex flex-wrap gap-2 mt-2">
            {["Semaphore", "Tornado Cash", "MACI", "주요 Circom dApp"].map(
              (p) => (
                <span key={p} className="rounded-full border px-3 py-1 text-xs">
                  {p}
                </span>
              ),
            )}
        ...
        <div className="not-prose rounded-lg border p-4 text-sm mb-6">
          ...
          <div className="bg-sky-50 dark:bg-sky-950/30 rounded p-3">
            ...
            <div className="font-mono text-xs space-y-1">
              <p>
                contract <strong>Verifier</strong> {"{"}
              </p>
            ...
            <tbody>
              {[
                ...
                },
                {
                  prover: "rapidSnark (C++)",
                  time: "~5-10초",
                  note: "프로덕션 (Polygon ID, Tornado Cash), x86_64",
                },
                {
                  prover: "GPU 증명자",
                  time: "<1초",
                  note: "롤업 사용 (Polygon zkEVM), CUDA 기반",
                },
              ].map((r) => (
        ...
        <div className="not-prose grid gap-2 sm:grid-cols-2 mb-4">
          {[
            {
              cmd: "snarkjs r1cs info circuit.r1cs",
              desc: "제약 수, 시그널 수 확인",
            },
            {
              cmd: "snarkjs r1cs print circuit.r1cs circuit.sym",
              desc: "시그널 이름 포함 사람이 읽을 수 있는 제약",
            },
            {
              cmd: "snarkjs r1cs export json circuit.r1cs circuit.json",
              desc: "분석용 JSON 포맷",
            },
            {
              cmd: "snarkjs wtns check circuit.r1cs witness.wtns",
              desc: "증인이 모든 제약을 만족하는지 검증",
            },
          ].map((d) => (
