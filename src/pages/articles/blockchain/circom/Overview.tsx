  ...
  return (
    <section id="overview" className="mb-16 scroll-mt-20">
      ...
      <div className="prose prose-neutral dark:prose-invert max-w-none">
        <p>
          <strong>Circom</strong>은 zkSNARK 증명 시스템을 위한 산술 회로를
          정의하는 도메인 특화 언어(DSL)입니다. <code>.circom</code> 소스를
          파싱하여 R1CS 제약 조건과 WASM 증인 계산기를 생성하고,
          ...
          {[
            {
              ...
              modules: [
                { name: "parser/", desc: "LALRPOP 기반 .circom 파싱 → AST" },
                {
                  ...
                  desc: "Template, Signal, ProgramArchive",
                },
                {
                  name: "type_analysis/",
                  desc: "신호 타입 검증, 매개변수 추론",
                },
              ],
            },
            {
            ...
            >
              ...
              <div
                className="grid gap-px sm:grid-cols-3 lg:grid-cols-4"
      ...
      <div className="prose prose-neutral dark:prose-invert max-w-none mt-6">
        ...
        <div className="not-prose rounded-lg border p-4 text-sm space-y-4 mb-6">
          <div className="grid gap-3 sm:grid-cols-2">
            ...
            <div>
              ...
              <ul className="space-y-1 text-sm">
                <li>
                  <strong>Circuit-as-code</strong> — 회로를 함수처럼 작성,
                  컴파일러가 R1CS 제약 방출
                </li>
                <li>
                  <strong>비결정적 계산</strong> — <code>{"<--"}</code> 값만
                  대입 / <code>{"<=="}</code> 대입+제약 / <code>{"==="}</code>{" "}
                  제약만
                </li>
                <li>
                  <strong>유한체 산술</strong> — 모든 연산은 F_p 위에서 수행
                  (기본: bn128 스칼라 필드)
                </li>
                <li>
                  <strong>명시적 증인 생성</strong> — WASM/C++ 증인 계산기 별도
                  출력
                </li>
              </ul>
            </div>
        ...
        <div className="not-prose grid gap-2 sm:grid-cols-3 mb-6">
          <div className="rounded-lg border p-3 text-sm">
            ...
            <ul className="space-y-1">
              <li>
                <code>signal</code> — 산술 회로의 와이어
              </li>
              <li>
                <code>template</code> — 재사용 가능한 회로 블록 (클래스와 유사)
              </li>
              <li>
                <code>component</code> — 템플릿 인스턴스
              </li>
              <li>
                <code>var</code> — 컴파일 타임 변수 (시그널 아님!)
              </li>
            </ul>
          </div>
          <div className="rounded-lg border p-3 text-sm">
          ...
          <div className="rounded-lg border p-3 text-sm">
            ...
            <ul className="space-y-1">
              <li>
                <code>var x = 5</code> → 컴파일 타임 상수, R1CS에서 사라짐
              </li>
              <li>
                <code>signal x</code> → 런타임 시그널, R1CS에 참여
              </li>
            </ul>
          </div>
        </div>
        ...
        <div className="not-prose grid gap-2 sm:grid-cols-2 mb-6">
          <div className="rounded-lg border bg-sky-50 dark:bg-sky-950/30 p-3 text-sm">
            ...
            <p className="text-muted-foreground">
              바이너리 R1CS 제약 시스템. snarkjs/rapidSnark가 증명에 사용
            </p>
          </div>
          ...
          <div className="rounded-lg border bg-violet-50 dark:bg-violet-950/30 p-3 text-sm">
            ...
            <p className="text-muted-foreground">
              C++ 증인 계산기 (네이티브, 더 빠름). 프로덕션용 빌드 스크립트로
              컴파일
            </p>
          </div>
        </div>
        ...
        <div className="not-prose flex flex-col gap-2 mb-6">
          {[
            { step: 1, text: "회로 작성", detail: "circuit.circom 파일 작성" },
            {
              step: 2,
              text: "컴파일",
              detail: "circom circuit.circom --r1cs --wasm --sym",
            },
            {
              step: 3,
              text: "Trusted Setup",
              detail: "R1CS를 Groth16 / PLONK 등에 전달",
            },
            {
              step: 4,
              text: "증인 생성",
              detail: "WASM에 입력 전달 → witness 계산",
            },
            {
              step: 5,
              text: "증명 생성",
              detail: "witness + proving key로 증명 생성",
            },
        ...
        <p className="text-sm text-muted-foreground mb-3">
          github.com/iden3/circomlib — 200+ 템플릿
        </p>
        <div className="not-prose grid gap-2 sm:grid-cols-2 lg:grid-cols-3 mb-6">
          {[
            {
              category: "해시 함수",
              items: "Poseidon, Pedersen, MiMC, Sha256",
            },
            { category: "서명", items: "EdDSA (Baby Jubjub), ECDSA" },
            { category: "머클 트리", items: "binary, sparse, LEAN trees" },
            {
              category: "기본 가젯",
              items: "AliasCheck, Num2Bits, Bits2Num, LessThan, IsEqual",
            },
            {
              category: "선택/스위칭",
              items: "MultiMux, Switcher, GreaterThan",
            },
