  ...
  return (
    <section id="r1cs-gen" className="mb-16 scroll-mt-20">
      ...
      <div className="prose prose-neutral dark:prose-invert max-w-none">
        <p>
          <code>constraint_generation</code> 모듈은 Circom 컴파일러의
          핵심입니다.
          <br />
        ...
        <div className="not-prose rounded-lg border p-4 text-sm space-y-3 mb-6">
          ...
          <div className="bg-emerald-50 dark:bg-emerald-950/30 rounded p-3">
            <p className="text-xs font-medium mb-1">
              ② 내보내기: ExecutedProgram → DAG + VCP
            </p>
            <div className="font-mono text-xs">
              <p>export(exe, program, flags)</p>
            </div>
            <p className="text-xs text-muted-foreground mt-1">
              DAG + Variable CP 구조로 변환
            </p>
          </div>
        ...
        <p>
          ...
          <code>{"c <== a * b + d"}</code>는 중간 시그널을 추가하여 두 개의
          제약으로 분해됩니다.
        </p>
        <M display>
          {
            "\\langle A, s \\rangle \\times \\langle B, s \\rangle = \\langle C, s \\rangle"
          }
        </M>
      </div>
      ...
      <div className="prose prose-neutral dark:prose-invert max-w-none mt-6">
          ...
          {[
            {
              ...
              label: "Parsing",
              detail:
                "LALRPOP LR(1) 파서 → AST (Template, Function, Component, Statement, Expression)",
            },
            {
              step: 2,
              label: "Type Analysis",
              detail:
                "시그널 타입 추론, 매개변수 검증, signal vs var 검사, 차원 불일치 감지",
            },
            {
              step: 3,
              label: "Instantiation",
              detail:
                "모든 루프 언롤링 (정적 바운드 필수), 구체적 매개변수로 템플릿 확장",
            },
            {
              step: 4,
              label: "DAG Construction",
              detail:
                "각 제약을 A, B, C 선형 결합으로 표현. 시그널 = 노드, 제약 = 하이퍼엣지",
            },
            {
              step: 5,
              label: "Simplification",
              detail:
                "미사용 시그널 제거, 선형 제약 대체, 항등 제약 제거, 상수 전파",
            },
            {
              step: 6,
              label: "Serialization",
              detail:
                ".r1cs 바이너리 (Header + Constraints + Map), .sym, .json (선택)",
            },
          ].map((s) => (
            <div
              key={s.step}
            ...
            >
              <span className="flex-none w-7 h-7 rounded-full bg-sky-100 dark:bg-sky-900/40 text-sky-700 dark:text-sky-300 flex items-center justify-center text-xs font-bold">
                {s.step}
              </span>
              <div>
        ...
        <div className="not-prose grid gap-2 sm:grid-cols-2 mb-6">
          ...
          <div className="rounded-lg border p-4 text-sm">
            ...
            <p className="text-xs text-muted-foreground">
              x = sum(k_i * s_i) 형태의 선형 관계 → x를 모든 곳에서 대체. 중간
              시그널 제거
            </p>
          </div>
          <div className="rounded-lg border p-4 text-sm">
            <p className="font-semibold text-xs text-amber-600 dark:text-amber-400 mb-2">
              Pass C: 항등 제약 감지
            </p>
            <p className="text-xs text-muted-foreground">
              0 = 0 제거, 상수 전파
          ...
          <div className="rounded-lg border p-4 text-sm">
            <p className="font-semibold text-xs text-violet-600 dark:text-violet-400 mb-2">
              Pass D: 커스텀 게이트 인식
            </p>
            <p className="text-xs text-muted-foreground">
              제약 그룹을 템플릿으로 매핑 (고급)
        ...
        <div className="not-prose rounded-lg border p-4 text-sm mb-6">
          ...
          <M display>
            {
              "LC = \\{ s_1: k_1, s_2: k_2, \\ldots \\} \\quad \\Rightarrow \\quad \\sum k_i \\cdot s_i"
            }
          </M>
          <p className="text-xs text-muted-foreground mt-2">
            가능하면 LC를 A 또는 B 열에 병합 → R1CS 제약 수 ~30-50% 감소
          </p>
        </div>
        ...
        <div className="not-prose grid gap-2 sm:grid-cols-2 mb-4">
          {[
            ...
            >
              <code className="font-mono text-xs font-semibold flex-none">
                {f.flag}
              </code>
