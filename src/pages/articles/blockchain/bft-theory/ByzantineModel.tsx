    ...
    <section id="byzantine-model" className="mb-16 scroll-mt-20">
      ...
      <div className="prose prose-neutral dark:prose-invert max-w-none mt-6">
        <p className="leading-7">
          장애 모델은 <strong>노드가 어떻게 실패할 수 있는가</strong>를
          규정한다.
          <br />
          약한 모델부터 강한 모델까지 단계별로 — 각 단계는 이전 모델을 포함한다.
          <br />
        ...
        <div className="not-prose space-y-3 mb-4">
          <p className="text-xs text-muted-foreground">
            장애 모델 계층 (약 → 강): Fail-Stop ⊂ Crash ⊂ Omission ⊂ Byzantine
          </p>
          ...
          <div className="rounded-lg border border-red-500/30 bg-red-500/5 p-4">
            ...
            <ul className="text-sm space-y-1 list-disc list-inside">
              <li>
                <strong>임의의 행동</strong> 가능 — 거짓 메시지 조작, 모순 전달,
                프로토콜 위반
              </li>
              <li>
                <M>{"f < n/3"}</M> 필요 (partial sync, unauthenticated)
              </li>
        ...
        <p className="leading-7">
          Crash는 <strong>정지만 (단순)</strong>, Byzantine은{" "}
          <strong>임의의 거짓 (복잡)</strong>.<br />
          Crash는 majority로 충분 (<M>{"f < n/2"}</M>), Byzantine은 2/3 이상
          필요 (<M>{"f < n/3"}</M>).
          <br />
        ...
        <div className="not-prose grid gap-3 sm:grid-cols-3 mb-4">
          <div className="rounded-lg border p-4">
            ...
            <ul className="text-sm space-y-1 list-disc list-inside">
              <li>
                지연 상한 <M>{String.raw`\Delta`}</M> 알려짐
              </li>
              <li>
                timeout = <M>{String.raw`\Delta + \varepsilon`}</M>으로 판별
              </li>
              <li>강한 가정 — 실제 인터넷에서 보장 어려움</li>
              <li>
                Dolev-Strong: <M>{"f < n"}</M>, <M>{"O(f)"}</M> round
              </li>
            </ul>
          </div>
          <div className="rounded-lg border p-4">
          ...
          <div className="rounded-lg border border-emerald-500/30 bg-emerald-500/5 p-4">
            ...
            <ul className="text-sm space-y-1 list-disc list-inside">
              ...
              <li>
                GST 이전: async, GST 이후:{" "}
                <M>{String.raw`\text{delay} < \Delta`}</M>
              </li>
        ...
        <div className="not-prose rounded-lg border p-4 mb-4">
          ...
          <ul className="text-sm space-y-1 list-disc list-inside">
            <li>
              평소: partial sync (<M>{String.raw`\Delta \approx`}</M> 100-500ms)
            </li>
            <li>DDoS/partition: asynchronous</li>
            <li>
              LAN: near-synchronous (<M>{String.raw`\Delta \approx`}</M> 1-10ms)
            </li>
          </ul>
        </div>
        <p className="leading-7">
          부분 동기 = <strong>GST 이전 비동기, 이후 동기</strong>.<br />
          Safety는 항상 보장, Liveness는 GST 이후에만.
          <br />
        ...
        <div className="not-prose grid gap-3 sm:grid-cols-2 mb-4">
          ...
          <div className="rounded-lg border p-4">
            ...
            <ul className="text-sm space-y-1 list-disc list-inside">
              ...
              <li>
                동기: <M>{"n \\geq f+2"}</M> (Dolev-Strong)
              </li>
              <li>
                부분 동기: 여전히 <M>{"n \\geq 3f+1"}</M>
              </li>
            </ul>
          </div>
          <div className="rounded-lg border p-4">
          ...
          <div className="rounded-lg border border-sky-500/30 bg-sky-500/5 p-4">
            <p className="font-semibold text-sm mb-2">
              왜 서명 있어도 <M>{"n \\geq 3f+1"}</M>?
            </p>
            <ul className="text-sm space-y-1 list-disc list-inside">
              <li>정족수 교차 (quorum intersection) 보장 필요</li>
              <li>
                <M>{"Q = 2f+1"}</M>, <M>{"|Q_1 \\cap Q_2| \\geq f+1"}</M>
              </li>
              <li>
                겹친 <M>{"f+1"}</M>명 중 정직 1명 이상 → 서로 다른 결정 불가
              </li>
            </ul>
          </div>
        </div>
        <p className="leading-7">
          ...
          서명 없으면 <M>{"n \\geq 3f+1"}</M>, 서명 있어도 partial sync에선
          여전히 3f+1.
          <br />
          이유는 <strong>quorum intersection</strong> — 두 quorum이 반드시
          겹쳐야 safety 보장.
        </p>
        ...
        <div className="not-prose grid gap-3 sm:grid-cols-2 mb-4">
          <div className="rounded-lg border p-4">
            ...
            <ul className="text-sm space-y-1 list-disc list-inside">
              <li>
                Byzantine + Partial sync, <M>{"f < n/3"}</M>
              </li>
          ...
          <div className="rounded-lg border p-4">
            ...
            <ul className="text-sm space-y-1 list-disc list-inside">
              ...
              <li>
                <M>{"O(n)"}</M> chained voting, responsive
              </li>
            </ul>
          </div>
        </div>
        <p className="leading-7">
          블록체인은 <strong>모두 Byzantine + Partial sync</strong> 모델.
          <br />
          Bitcoin은 probabilistic (formal BFT 아님), 나머지는 모두{" "}
          <M>{"f < n/3"}</M> BFT.
          <br />
        ...
        <p className="text-sm border-l-2 border-amber-500/50 pl-3 mt-4">
          <strong>💡 왜 partial sync가 "최선의 타협"인가</strong> — FLP
          불가능성과 현실의 절충.
          <br />
          Asynchronous는 deterministic consensus 불가, synchronous는 실제
          네트워크에 맞지 않음.
          <br />
          Partial sync는 safety 항상 보장하고 liveness는 네트워크 안정 시에만 —
          현실적이면서 안전.
        </p>
      </div>
    </section>
