  ...
  return (
    <section id="safety-liveness" className="mb-16 scroll-mt-20">
      ...
      <div className="prose prose-neutral dark:prose-invert max-w-none">
        <p>
          Leader가 Byzantine이거나 network가 지연되면 current view가 certificate를 만들지 못할 수 있습니다. Replica는 timeout을 모아 더
          높은 view로 이동하지만 이전 view에서 이미 만들어진 strongest safe certificate와 local lock은 새 leader에게 그대로 전달합니다. 새
          leader가 이를 무시해 임의 value를 제안하면 progress는 빨라 보여도 conflicting commit이 가능해집니다.
        </p>
      </div>
      <SafetyLivenessViz />
      <div className="prose prose-neutral dark:prose-invert max-w-none">
        ...
        <p>
          PBFT의 pre-prepare/prepare/commit, HotStuff의 chained quorum certificate처럼 phase 이름과 commit rule은 다를 수
          있습니다. 공통 질문은 “어떤 signed set이 어떤 phase의 certificate인가”, “replica가 언제 lock을 갱신하는가”, “더 높은 view가 어떤
          evidence를 이어받아야 하는가”입니다. 2f+1 vote를 모았다는 사실 하나로 어느 phase에서나 commit할 수는 없습니다.
        </p>
        ...
        <p>
          ...
          stale view replay를 같은 seed와 schedule로 주입합니다. Safety hard gate는 같은 height에서 conflicting committed
          digest 0건입니다. Liveness는 GST를 표시한 뒤 recovery time·view 수·message 수로 측정합니다. GST 이전의 halt는 이 측정에서 실패로
          치지 않습니다.
        </p>
      </div>
      <div id="paper-pbft" className="not-prose my-8 scroll-mt-24 border-l border-primary/50 pl-4">
        ...
        <p className="mt-2 text-sm leading-6 text-muted-foreground">
          문제는 asynchronous network에서 Byzantine replica를 견디는 practical state-machine replication입니다. Normal-
          case phases, checkpoint, view change와 implementation evaluation을 제시합니다. 현대 WAN이나 weighted
      ...
      <div id="paper-hotstuff" className="not-prose my-8 scroll-mt-24 border-l border-primary/50 pl-4">
        ...
        <p className="mt-2 text-sm leading-6 text-muted-foreground">
          문제는 partial synchrony BFT에서 leader replacement를 단순화하면서 communication과 responsiveness를 개선하는 것입니다.
          Chained quorum certificates와 pacemaker 분리를 제시합니다. 모든 validator count·implementation·network에서 선형 총
          bytes가 자동 달성된다는 주장까지는 아닙니다.
        </p>
