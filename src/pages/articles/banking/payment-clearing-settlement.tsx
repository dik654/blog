    ...
    <div className="space-y-16">
      ...
      <section id="three-layers" className="scroll-mt-20">
        ...
        <div className="prose prose-neutral max-w-none dark:prose-invert">
          ...
          <p className="leading-7">
            대신 그 대가가 생깁니다. 지시가 이미 전달되어 고객 통장에는 반영됐는데 은행 사이 정산은 아직이라면, 그사이 보내는 쪽 은행이 무너질 때 받는 쪽 은행이 돈을 못 받습니다.
            편의를 얻는 대신 위험을 잠시 떠안는 셈입니다.
          </p>

          <p className="leading-7">
            앞 글의 용어로 말하면 이 정산은 중앙은행 장부에서 일어납니다. 은행이 중앙은행에 둔 지급준비금 계정 잔액이 서로 반대로 움직이는 것이 곧 결제입니다. 그래서 결제의 최종성은
            결국 중앙은행 장부가 보증합니다.
          </p>
        </div>
      ...
      <section id="rtgs-vs-dns" className="scroll-mt-20">
        ...
        <div className="prose prose-neutral max-w-none dark:prose-invert">
          <p className="text-lg leading-8">
            결제를 언제 할지는 두 극단 사이의 선택입니다. 한쪽은 건마다 즉시 총액으로 넘깁니다. 다른 쪽은 하루치를 모아 차액만 정해진 시각에 넘깁니다. 앞은 위험이 거의 없는 대신
            자금이 많이 필요하고 뒤는 자금이 적게 드는 대신 위험이 쌓입니다.
          </p>

          <p className="leading-7">
            이 맞바꿈이 얼마나 큰지가 중요합니다. 서로 주고받을 것이 많은 참가자들 사이에서는 상계로 지워지는 금액이 대부분이라 실제로 옮겨야 할 돈이 총액의 몇 퍼센트로 줄기도 합니다.
            그만큼 준비해 둬야 할 자금이 줄어듭니다.
          </p>

          <p className="leading-7">
            줄어든 자금의 대가가 앞 절에서 말한 위험입니다. 정산 시각까지 쌓인 미결제 잔액은 곧 참가자 하나가 무너졌을 때 다른 참가자들이 떠안게 될 금액입니다. 상계로 자금을 아낄수록
            이 금액이 커지는 구간이 생깁니다.
          </p>
        </div>
        ...
        <div id="hybrid-design" className="scroll-mt-20">
          ...
          <div className="prose prose-neutral max-w-none dark:prose-invert">
            <p className="leading-7">
              거액은 건별 즉시 결제로, 소액은 모아서 차액 결제로 나누는 것이 일반적인 답입니다. 금액이 큰 거래일수록 미결제로 남았을 때 손실이 크니 자금을 더 쓰더라도 즉시
              끝냅니다. 작은 거래는 위험보다 비용이 중요하므로 모읍니다.
            </p>

            <p className="leading-7">
              그 사이를 메우는 설계도 있습니다. 건별 결제를 하되 대기 중인 지시 사이에서 상쇄 가능한 조합을 찾아 동시에 처리하면, 즉시성을 유지하면서 필요한 자금을 줄일 수
              있습니다.
            </p>
          </div>
      ...
      <section id="finality" className="scroll-mt-20">
        ...
        <div className="prose prose-neutral max-w-none dark:prose-invert">
          ...
          <p className="leading-7">
            최종성이 규칙의 문제인 이유는 파산 상황을 생각하면 분명합니다. 일반적인 도산 절차는 파산 직전의 재산 이전을 되돌릴 수 있게 합니다. 그 규칙이 결제 시스템에도 그대로
            적용되면 이미 끝났다고 믿었던 결제가 나중에 취소될 수 있습니다.
          </p>

          <p className="leading-7">
            그래서 각국은 결제 시스템을 거친 이체를 되돌리지 못하도록 별도의 법적 보호를 둡니다. 최종성은 시스템이 빨라서 생기는 것이 아니라, 언제 이후로는 취소할 수 없다고 법과
            규칙이 선을 그어 주기 때문에 생깁니다.
          </p>
        </div>
        ...
        >
          ...
          <p className="leading-7">
            지급결제 시스템의 최종성은 반대 방향입니다. 되돌릴 수 있느냐 없느냐를 규칙이 미리 선언하고 그 선언을 법이 떠받칩니다. 같은 단어를 쓰지만 보장의 원천이 다르므로 둘을 같은
            것으로 놓고 비교하면 어느 쪽도 제대로 평가되지 않습니다.
          </p>
          <p className="leading-7">
            공통점도 있습니다. 어느 쪽이든 &ldquo;언제부터 끝난 것으로 볼 것인가&rdquo;를 참가자들이 미리 합의해 두어야 합니다. 그 합의가 없으면 같은 장부를 보고도 서로
            다른 시점을 완료로 여기게 됩니다.
          </p>
        </ProgressiveDetail>
      </section>
      ...
      <section id="cross-currency" className="scroll-mt-20">
        ...
        <div className="prose prose-neutral max-w-none dark:prose-invert">
          <p className="text-lg leading-8">
            원화와 달러를 바꾸는 거래는 두 나라의 서로 다른 결제 시스템에서 따로 처리됩니다. 각 통화의 결제는 그 나라 중앙은행 장부에서만 일어나기 때문입니다. 한쪽을 넘긴 뒤 다른
            쪽을 받기까지 시차가 생깁니다.
          </p>

          <p className="leading-7">
            그사이 상대가 무너지면 넘긴 쪽은 원금 전부를 잃습니다. 가격 변동으로 일부를 잃는 것이 아니라 보낸 금액 자체를 못 받는 것이라, 규모가 다릅니다. 이 위험은 1974년
            독일의 한 은행이 영업시간 중 폐쇄되면서 실제로 현실이 됐고 그 이름을 따 부릅니다.
          </p>
      ...
      <section id="boundary" className="scroll-mt-20">
        ...
        <div className="prose prose-neutral max-w-none dark:prose-invert">
          <p className="text-lg leading-8">
            한국의 구조는 이 글의 층 구분과 그대로 겹칩니다. 개인과 기업의 자금 이체는 금융결제원이 운영하는 소액지급시스템에서 처리됩니다. 거기서 생긴 금융기관 사이의 채권·채무는
            한국은행이 운영하는 거액결제시스템 한은금융망에서 최종적으로 해소됩니다.
          </p>

          <p className="leading-7">
            앞 글에서 본 지급준비금 계정이 여기서 다시 등장합니다. 참가기관은 한국은행에 개설된 계좌로 서로 주고받을 자금을 결제합니다. 그 계정의 잔액 이동이 곧 최종성을 갖는
            결제입니다.
          </p>

          <p className="leading-7">
            여기까지가 처음 그림의 아래 두 층입니다. 돈이 무엇이고, 어떻게 만들어지며, 그 가격은 누가 정하고, 실제로 어떻게 옮겨지는지가 닫혔습니다. 남은 것은 위쪽입니다. 시장에서
            사고파는 청구권은 값이 어떻게 정해질까요.
          </p>
