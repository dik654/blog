...
const ANNOT = [
  "Docker Compose 제출",
  "Manifest 생성+vsock 설정",
  "QEMU TDX TD 기밀 VM 생성",
  "Guest Agent 부팅 초기화",
  "TDX Quote KMS 키 발급",
  "키로 상태 복호화+컨테이너 실행",
];
function bx(i: number) {
  return BOXES[i].x + W / 2;
}
function by(i: number) {
  return BOXES[i].y + H / 2;
}
          ...
          {ARROWS.map((a, i) => {
            ...
            return (
              ...
              >
                ...
                />
                {(() => {
                  const tx = (bx(a.from) + bx(a.to)) / 2,
                    ty = (by(a.from) + by(a.to)) / 2 + off - 5;
                  return (
                    <>
                      <rect
                        x={tx - 32}
