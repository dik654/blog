...
export default function DstackAttestation({
  title,
  onCodeRef,
}: {
  title?: string;
    ...
    <section id="tdx-attestation" className="mb-16 scroll-mt-20">
      ...
      {onCodeRef && (
        <div className="not-prose flex flex-wrap gap-2 mb-6">
          <CodeViewButton
            onClick={() =>
              onCodeRef("tdx-quote-gen", codeRefs["tdx-quote-gen"])
            }
          />
          ...
          <CodeViewButton
            onClick={() => onCodeRef("tdx-verify", codeRefs["tdx-verify"])}
          />
          <span className="text-[10px] text-muted-foreground self-center">
            verify_tdx_quote()
          </span>
      ...
      <div className="prose prose-neutral dark:prose-invert max-w-none">
        <h3 className="text-xl font-semibold mt-6 mb-3">
          dstack Attestation 체계
        </h3>
        <p>
          <strong>Quote 생성</strong>: Guest agent가 Intel TDX로부터 quote 발급
          <br />
          <strong>KMS 검증</strong>: Phala KMS가 quote 검증 + 정책 매치
          <br />
          <strong>Secret 발급</strong>: 검증 통과 시 app-specific 암호키·secrets
          전달
          <br />
          <strong>RA-TLS</strong>: 외부 사용자가 dstack VM과 통신 시 attestation
          내장 TLS
        </p>
      ...
      <div className="prose prose-neutral dark:prose-invert max-w-none">
        <div className="bg-amber-50 dark:bg-amber-950/30 border-l-4 border-amber-400 p-4 my-6 rounded-r-lg">
          <p className="font-semibold mb-2">
            인사이트: dstack vs kata confidential
          </p>
          <p>
            <strong>공통점</strong>:<br />
            - Intel TDX 사용
            <br />
            - Container workload 지원
            <br />- Attestation 통합
          </p>
          <p className="mt-2">
          ...
          <p className="mt-2">
            ...
            - Web3·AI agent 단일 서비스: dstack
            <br />
            - Enterprise K8s 워크로드: Kata CoCo
            <br />- 둘 다 confidential-containers 프로젝트 우산 아래
          </p>
        </div>
      </div>
    </section>
