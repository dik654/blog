  ...
  return (
    <section id="overview" className="mb-16 scroll-mt-20">
      ...
      <div className="prose prose-neutral dark:prose-invert max-w-none">
        ...
        <p>
          <strong>Oasis Network</strong>: 2020년 메인넷 런칭 — TEE 기반 기밀
          컴퓨팅 전용 L1
          <br />
          <strong>핵심 문제</strong>: Ethereum의 공개 트랜잭션 모델 → 금융·의료
          데이터 처리 불가
          <br />
          <strong>해결</strong>: 실행을 TEE(Intel SGX/TDX) 안에서 →
          입력·상태·출력 전부 기밀
        </p>
...
// - Cipher: Wasm 기반 기밀 (experimental)`}</pre>
        <p>
          <strong>분리의 의의</strong>: Consensus는 보안·안정성 최우선,
          Runtime은 성능·유연성 최우선
          <br />
          <strong>Cosmos와 유사</strong>: Oasis ParaTime ≈ Cosmos Zone
          <br />
          <strong>차이</strong>: Oasis는 TEE 기밀성을 1차 요건으로, 합의
          레이어가 명시적 shared security 제공
        </p>
          ...
          {[
            {
              ...
              color: "#6366f1",
              items: [
                "CometBFT 기반 BFT 합의 (2/3 정족수)",
                "검증인 관리 & 스테이킹 (≥100 ROSE delegation)",
                "거버넌스 & 레지스트리 (노드·ParaTime 등록)",
                "루트해시 서비스 (Runtime 상태 루트 커밋)",
                "Staking module: 위임·보상·slashing",
              ],
            },
            {
              ...
              color: "#10b981",
              items: [
                "컴퓨트 워커: 트랜잭션 실행 (TEE 안에서)",
                "Storage 워커: Merkle Patricia Trie 저장",
                "키매니저: per-contract 암호화 키 관리",
                "Client 노드: 게이트웨이/RPC",
                "Sapphire: EVM + 기밀 스토리지",
              ],
            },
          ].map((l) => (
            <div
...
// - Client → 쿼리 전용 게이트웨이`}</pre>
        {onCodeRef && (
          <div className="not-prose flex flex-wrap gap-2 my-4">
            <CodeViewButton
              onClick={() =>
                onCodeRef("full-service", codeRefs["full-service"])
              }
            />
            ...
            <CodeViewButton
              onClick={() =>
                onCodeRef("executor-worker", codeRefs["executor-worker"])
              }
            />
            <span className="text-[10px] text-muted-foreground self-center">
              worker.go · 런타임 워커
      ...
      <div className="prose prose-neutral dark:prose-invert max-w-none">
        <p>
          <strong>1. 독립적 확장</strong>: 합의 TPS 제약 없이 ParaTime별 병렬
          실행
          <br />
          <strong>2. Heterogeneous VM</strong>: EVM, Wasm, 네이티브 Go 코드 혼합
          가능
          <br />
          <strong>3. 선택적 기밀성</strong>: TEE 필요 ParaTime만 SGX 요구
          <br />
          <strong>4. Hot-swappable runtime</strong>: ParaTime 바이너리 교체 가능
          (거버넌스 통해)
        </p>
        ...
        <div className="bg-amber-50 dark:bg-amber-950/30 border-l-4 border-amber-400 p-4 my-6 rounded-r-lg">
          ...
          <p>
            <strong>SGX/TDX 필수</strong>: Sapphire 컴퓨트 노드는 하드웨어 TEE
            없으면 실행 불가
            <br />
            <strong>리스크</strong>:
          </p>
          <p className="mt-2">
            ✗ SGX EOL — Intel이 클라이언트 CPU SGX 단계적 종료
            <br />
            ✗ TDX 전환 필요 — 서버급 CPU만 지원 → 노드 진입 장벽
            <br />
            ✗ 사이드채널 공격에 TEE 공통 취약 (AEPIC, Downfall 등)
            <br />✗ Intel 의존성 — vendor lock-in
          </p>
          <p className="mt-2">
            <strong>대응</strong>:<br />
            - AMD SEV-SNP, ARM CCA 지원 진행 중<br />
            - Multi-TEE attestation 추상화 레이어 설계
            <br />- ZK 보완 연구 (TEE+ZK 하이브리드)
          </p>
        </div>
      </div>
    </section>
