  ...
  return (
    <section id="core-architecture" className="mb-16 scroll-mt-20">
      ...
      <div className="prose prose-neutral dark:prose-invert max-w-none">
        <p>
          <strong>Oasis Core</strong> (go/ 디렉토리): 합의·런타임·네트워킹을
          포함하는 Go 구현체
          <br />
          ABCI 인터페이스로 CometBFT와 통합 — Oasis 전용 로직은{" "}
          <code>abciMux</code>가 중재
          <br />
    ...
    return app.ExecuteTx(ctx, req.Tx[1:])
        ...
        {onCodeRef && (
          <div className="not-prose flex flex-wrap gap-2 my-4">
            ...
            <CodeViewButton
              onClick={() =>
                onCodeRef("abci-mux-inner", codeRefs["abci-mux-inner"])
              }
            />
            <span className="text-[10px] text-muted-foreground self-center">
              abciMux 내부
...
//    - Health endpoint`}</pre>
        ...
        <div className="bg-amber-50 dark:bg-amber-950/30 border-l-4 border-amber-400 p-4 my-6 rounded-r-lg">
          <p className="font-semibold mb-2">
            인사이트: 왜 모듈을 ABCI Application으로 분리했나
          </p>
          <p>
            <strong>대안 1 — 단일 거대 앱</strong>: 모든 로직을 하나의 ABCI
            Application에 집약
            <br />
            <strong>대안 2 — Oasis처럼 분리</strong>: Staking/Registry/Roothash
            등 독립 모듈
          </p>
          <p className="mt-2">
            ...
            ✓ 테스트 용이 — 각 모듈 독립 테스트
            <br />
            ✓ 업그레이드 단위 — 거버넌스로 모듈별 교체
            <br />✓ 코드 소유권 — 팀별 분담
          </p>
          <p className="mt-2">
            <strong>비용</strong>:<br />
            ✗ 상태 접근 오버헤드 — 모듈 간 storage 호출
            <br />
            ✗ 트랜잭션 라우팅 복잡도
            <br />✗ Cosmos SDK와 다른 자체 패턴 (학습 곡선)
          </p>
        </div>
      </div>
    </section>
