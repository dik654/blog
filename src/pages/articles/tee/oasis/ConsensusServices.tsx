...
export default function ConsensusServices({
  onCodeRef,
}: {
  ...
  return (
    <section id="consensus-services" className="mb-16 scroll-mt-20">
      ...
      <div className="prose prose-neutral dark:prose-invert max-w-none">
        <p>
          Oasis 합의는 <strong>CometBFT</strong>(구 Tendermint Core) 엔진 기반
          <br />
          <strong>Propose → Prevote → Precommit → Commit</strong> 4단계 BFT 합의
          <br />
          <strong>ABCI</strong>(Application Blockchain Interface)로 합의 로직과
          애플리케이션 로직 분리
          <br />
          <strong>즉시 확정성</strong> — 2/3 precommit 도달 시 최종 확정 (reorg
          불가)
        </p>
      </div>
...
// - 충분한 분산성 보장`}</pre>
        ...
        {onCodeRef && (
          <div className="not-prose flex flex-wrap gap-2 my-4">
            ...
            <CodeViewButton
              onClick={() => onCodeRef("abci-mux", codeRefs["abci-mux"])}
            />
            <span className="text-[10px] text-muted-foreground self-center">
              ABCI 앱 서버
            </span>
...
// - 포함 시 다음 라운드 참여 준비`}</pre>
        ...
        <div className="bg-amber-50 dark:bg-amber-950/30 border-l-4 border-amber-400 p-4 my-6 rounded-r-lg">
          ...
          <p>
            <strong>대안 비교</strong>:<br />- <strong>Ethereum Gasper</strong>:
            Probabilistic finality, slow (~15 min)
            <br />- <strong>HotStuff</strong>: 3-chain commit, linear view
            change
            <br />- <strong>CometBFT</strong>: 즉시 확정, 6초 블록타임, mature
          </p>
          <p className="mt-2">
            <strong>Oasis 요구사항</strong>:<br />
            ✓ 금융 앱 — 즉시 확정 필수 (reorg 절대 불가)
            <br />
            ✓ TEE commitment 검증 — fast finality로 slashing 신속
            <br />✓ 작은 검증인 집합 (120) — CometBFT 통신 복잡도 감당 가능
          </p>
          <p className="mt-2">
            <strong>단점 수용</strong>:<br />
            ✗ 검증인 수 확장 한계 (~300이 실용적 최대)
            <br />
            ✗ Liveness 요구 — 1/3 오프라인 시 정지
            <br />✗ 라이트 클라이언트 복잡 — committee rotation 추적 필요
          </p>
        </div>
      </div>
    </section>
