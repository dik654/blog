  ...
  return (
    <section id="runtime-system" className="mb-16 scroll-mt-20">
      ...
      <div className="prose prose-neutral dark:prose-invert max-w-none">
        <p>
          <strong>Bundle Manager</strong>가 .orc 번들을 다운로드·검증
          <br />
          <strong>Runtime Loader</strong>가 SGX 엔클레이브 또는 네이티브
          프로세스로 런타임 로딩
          <br />
          <strong>Runtime Host</strong>가 실행 환경 관리,{" "}
          <strong>컴퓨트 워커</strong>가 트랜잭션 처리
          <br />
...
// 4) RuntimeKeyManagerStatusUpdate: KM 상태 알림`}</pre>
        ...
        {onCodeRef && (
          <div className="not-prose flex flex-wrap gap-2 my-4">
            <CodeViewButton
              onClick={() => onCodeRef("dispatcher", codeRefs["dispatcher"])}
            />
            <span className="text-[10px] text-muted-foreground self-center">
              dispatcher.rs · 런타임 디스패처
            </span>
            <CodeViewButton
              onClick={() =>
...
// 4) SGX 로더 또는 네이티브 로더로 로딩`}</pre>
        ...
        <div className="bg-amber-50 dark:bg-amber-950/30 border-l-4 border-amber-400 p-4 my-6 rounded-r-lg">
          <p className="font-semibold mb-2">
            인사이트: Runtime 격리의 이중 레이어
          </p>
          <p>
            <strong>Layer 1: Process isolation</strong>
            <br />
            - Runtime = 별도 OS 프로세스
            <br />
            - Host 크래시해도 Runtime 독립 운영
            <br />- Sandbox(seccomp/namespace)로 시스템콜 제한
          </p>
          <p className="mt-2">
            <strong>Layer 2: TEE isolation (SGX)</strong>
            <br />
            - Runtime 프로세스가 엔클레이브 진입
            <br />
            - 모든 민감 로직은 엔클레이브 안에서만
            <br />- Host OS, 루트 사용자도 엔클레이브 내부 관측 불가
          </p>
          <p className="mt-2">
            <strong>왜 이중인가</strong>:<br />
            - Sandbox만: 커널 버그 시 탈출 가능
            <br />
            - TEE만: 성능·호환성 이슈 (엔클레이브 외 라이브러리 필요)
            <br />- 이중: 일상적 방어 + 극단 공격 대응
          </p>
        </div>
      </div>
    </section>
