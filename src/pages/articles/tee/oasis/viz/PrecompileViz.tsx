...
const STEPS = [
  {
    label: "난수 & 키 유도 프리컴파일",
    body: "RANDOM_BYTES(0x01...01): SGX 내 PRNG으로 안전한 난수 생성.\nDERIVE_KEY(0x01...02): HKDF 키 유도.",
  },
  {
    label: "대칭 암호화/복호화 프리컴파일",
    body: "ENCRYPT(0x01...03): DeoxysII-256 대칭 암호화.\nDECRYPT(0x01...04): 대칭 복호화. SGX 안에서만 수행.",
  },
  {
    label: "서명 프리컴파일",
    body: "GENERATE_SIGNING_KEYPAIR(0x01...05): 키쌍 생성.\nSIGN/VERIFY(0x01...06~07): Ed25519, Secp256k1 서명.",
  },
  {
    label: "기밀 난수 사용 예시",
    body: 'Sapphire.randomBytes(32, "") → 32바이트 기밀 난수.\nabi.decode로 uint256으로 변환하여 컨트랙트에서 활용.',
  },
];
            ...
            {BLOCKS.map((b, i) => (
              <g key={b.label}>
                ...
                <text
                  ...
                  fontSize={10}
                  fontWeight={600}
                  fill={active.includes(i) ? b.color : "var(--foreground)"}
                >
                  {b.label}
                </text>
                <text
