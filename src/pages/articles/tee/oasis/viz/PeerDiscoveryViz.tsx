...
const STEPS = [
  {
    label: "Bootstrap Discovery: 시드 노드 기반",
    body: "초기 부트스트랩 시드 노드에 연결. RetentionPeriod 동안 피어 정보 보존. 네트워크 진입점 역할.",
  },
  {
    label: "Registry Discovery: 합의 레지스트리 기반",
    body: "합의 레지스트리에서 프로토콜별 피어를 검색. connectMany()로 limit 수만큼 병렬 연결 수립.",
  },
  {
    label: "DHT Discovery: 분산 해시 테이블 기반",
    body: "FindPeers(ctx, topic)으로 토픽별 피어 검색. 중앙 서버 없이 분산 방식으로 동작.",
  },
];
            ...
            return (
              <g key={m.label}>
                ...
                <text
                  ...
                  fontSize={11}
                  fontWeight={600}
                  fill={active ? m.color : "var(--muted-foreground)"}
                >
                  {m.label}
                </text>
                <text
