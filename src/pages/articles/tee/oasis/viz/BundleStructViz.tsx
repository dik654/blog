...
const STEPS = [
  {
    label: ".orc 번들 파일 구조",
    body: "ZIP 아카이브 형태. META-INF/MANIFEST.MF(매니페스트), runtime.elf(ELF 실행), runtime.sgxs(SGX 엔클레이브), runtime.sig(서명).",
  },
  {
    label: "매니페스트 타입 정의",
    body: "ID: 런타임 식별자(Namespace). Components: RONL/ROFL 컴포넌트 목록. Digests: 파일별 무결성 해시.",
  },
  {
    label: "컴포넌트 SGX/ELF 분기",
    body: "RONL: 일반 온체인 런타임. ROFL: 오프체인 런타임. 각 컴포넌트는 ELFMetadata 또는 SGXMetadata를 포함.",
  },
];
          ...
          {FILES.map((f) => (
            <g key={f.name}>
              <motion.line
                ...
                x2={f.x}
                y2={78}
                stroke={step === 0 ? f.color : "var(--border)"}
                strokeWidth={0.8}
              ...
              <text
                ...
                fontSize={10}
                fontWeight={600}
                fill={step === 0 ? f.color : "var(--muted-foreground)"}
              >
                {f.name}
              </text>
            </g>
