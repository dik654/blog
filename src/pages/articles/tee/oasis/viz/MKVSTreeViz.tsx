...
const STEPS = [
  {
    label: "Internal Node: 트리 분기점",
    body: "Hash(자식 해시 결합), Label(엣지 레이블 — 경로 압축), Left/Right 자식 포인터로 구성됩니다.",
  },
  {
    label: "Leaf Node: 실제 데이터",
    body: "H(key || value) 해시, Key(키), Value(바이트 배열). 검색 결과를 담는 말단 노드입니다.",
  },
  {
    label: "Merkle Tree 핵심 특성",
    body: "루트 해시 하나로 전체 상태 무결성 검증 가능.\n차이점만 전송(diff sync). 경로 압축으로 효율 향상.",
  },
];
    ...
    <StepViz steps={STEPS}>
      {(step) => (
        ...
        >
          ...
          {/* Internal nodes */}
          {[
            { x: 150, label: "Internal A" },
            { x: 390, label: "Internal B" },
          ].map((n, i) => (
            <g key={n.label}>
              <line
                x1={270}
                ...
                x2={n.x}
                y2={65}
                stroke={step === 0 ? C.internal : "var(--border)"}
              ...
              <text
                ...
                fontSize={10}
                fontWeight={600}
                fill={step === 0 ? C.internal : "var(--muted-foreground)"}
              >
                {n.label}
              </text>
              <text
          ...
          {[100, 200, 340, 440].map((x, i) => (
            <g key={`leaf-${i}`}>
              <line
                ...
                x2={x}
                y2={122}
                stroke={step === 1 ? C.leaf : "var(--border)"}
              ...
              <text
                ...
                fontSize={10}
                fontWeight={600}
                fill={step === 1 ? C.leaf : "var(--muted-foreground)"}
              >
                Leaf {i + 1}
              </text>
            </g>
