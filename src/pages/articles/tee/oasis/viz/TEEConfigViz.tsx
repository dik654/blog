...
const STEPS = [
  {
    label: "TEE 선택 모드",
    body: "auto: 하드웨어 TEE 사용 가능하면 자동 선택.\nhardware: 하드웨어 TEE 강제.\nnone: TEE 비활성화 (개발용).",
  },
  {
    label: "TEE 종류 & 신원",
    body: "TEEKindNone(0): TEE 없음.\nTEEKindSGX(1): Intel SGX.\n엔클레이브 신원은 MRENCLAVE(코드 해시)와 MRSIGNER(서명자 해시)로 식별.",
  },
  {
    label: "엔클레이브 신원 (EnclaveIdentity)",
    body: "MrEnclave: 엔클레이브 코드 전체의 해시 — 무결성 검증.\nMrSigner: 서명자 키의 해시 — 개발자 신원 확인.",
  },
  {
    label: "RA-TLS 보안 채널",
    body: "TLS 인증서에 SGX Quote 첨부 → 상대방이 Quote 검증 후 TLS 연결 수립.\n키 매니저 ↔ 컴퓨트 워커 간 보안 채널.",
  },
];
      ...
      {(step) => (
        ...
        >
          ...
          {BLOCKS.map((b) => (
            <g key={b.label}>
              ...
              <text
                ...
                fontSize={10}
                fontWeight={600}
                fill={step === 0 ? b.color : "var(--muted-foreground)"}
              >
                {b.label}
              </text>
              <text
          ...
          <text
            ...
            fontSize={10}
            fontWeight={600}
            fill={step === 2 ? "#10b981" : "var(--muted-foreground)"}
          >
            MRENCLAVE
          </text>
          <text
          ...
          <text
            ...
            fontSize={11}
            fontWeight={700}
            fill={step === 3 ? "#f59e0b" : "var(--muted-foreground)"}
          >
            RA-TLS
          </text>
          {["TLS cert + SGX Quote", "→ Quote 검증", "→ 보안 채널"].map(
            (t, i) => (
              <text
                key={t}
                x={130 + i * 120}
