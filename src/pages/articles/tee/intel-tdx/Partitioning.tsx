  ...
  return (
    <section id="partitioning" className="mb-16 scroll-mt-20">
      ...
      <div className="prose prose-neutral dark:prose-invert max-w-none">
        ...
        <p>
          <strong>문제</strong>: 하나의 TD 안에 여러 워크로드 격리하고 싶음
          <br />
          <strong>예</strong>: Confidential Kubernetes — 노드는 TD, pod는 더
          격리
          <br />
          <strong>해결</strong>: L1 TD가 L2 TD를 생성·관리 — nested
          virtualization의 TDX 버전
        </p>

        <h3 className="text-xl font-semibold mt-8 mb-3">
          L1 TD의 역할 — Partitioning Manager
        </h3>
...
//   → 다시 tdg_vp_enter로 재진입`}</pre>
        <p>
          <strong>3-계층</strong>: SEAM → Host → L1 → L2
          <br />
          L2의 "hypervisor"는 L1 TD — Host VMM 아님
          <br />
...
// - MigTD가 정책 강제 (이동 허용 플랫폼 제한)`}</pre>
        ...
        <div className="bg-amber-50 dark:bg-amber-950/30 border-l-4 border-amber-400 p-4 my-6 rounded-r-lg">
          <p className="font-semibold mb-2">
            인사이트: Partitioning의 실제 용도
          </p>
          <p>
            <strong>시나리오 1 — Confidential K8s</strong>:<br />
            - L1 TD = 노드 (kubelet, containerd)
            <br />
            - L2 TD = 각 pod
            <br />- 멀티테넌트 격리 + 단일 노드 공유
          </p>
          <p className="mt-2">
            ...
            <br />
            - L2 TD = 각 함수 호출
            <br />- 짧은 수명 — 빠른 생성/파괴 필요
          </p>
          <p className="mt-2">
            <strong>시나리오 3 — Trusted I/O 중재</strong>:<br />
            - L1 TD가 storage·network 암호화 중재
            <br />
            - L2는 평문처럼 쓰되 I/O에서 L1이 암/복호화
            <br />- 앱 수정 없이 confidential 보장
          </p>
          <p className="mt-2">
            <strong>주의</strong>: TDX 1.5 필요 — Granite Rapids 이상
            <br />
