  ...
  return (
    <section id="attestation" className="mb-16 scroll-mt-20">
      ...
      <div className="prose prose-neutral dark:prose-invert max-w-none">
        ...
        <p>
          <strong>DCAP</strong>(Data Center Attestation Primitives): EPID 대체
          <br />
          Intel 서버 의존성 제거 — 운영자가 직접 PCS(Provisioning Cert. Service)
          캐시
          <br />
          <strong>3 주체</strong>: TD(증명 대상) · Quote Enclave(서명자) ·
          Verifier(검증자)
        </p>
...
struct TDINFO_STRUCT {
    ...
    u8  reserved[64];
        ...
        <p>
          <strong>REPORTDATA(64B)</strong>: 증명자가 임의 데이터 넣는 슬롯
          <br />
          일반적으로 <strong>nonce + 공개키 해시</strong> → replay 방어 + 키
          바인딩
          <br />
...
if (!policy.match_rtmr(quote.rtmr)) return FAIL;`}</pre>
        <p>
          <strong>검증은 다단계</strong>: 서명 → 인증서 체인 → TCB 상태 → 정책
          <br />
          <strong>PCS 캐시 필수</strong>: 온라인 의존 제거용 (pccs, SGX Caching
          Service)
          <br />
          <strong>정책</strong>: Relying Party가 결정 — 어떤 MRTD/RTMR 조합을
          받아줄지
        </p>

        <h3 className="text-xl font-semibold mt-8 mb-3">
          실전 — Azure CVM Attestation 예
        </h3>
...
jwt.verify(jwt, MAA_signing_key);`}</pre>
        ...
        <div className="bg-amber-50 dark:bg-amber-950/30 border-l-4 border-amber-400 p-4 my-6 rounded-r-lg">
          <p className="font-semibold mb-2">
            인사이트: Local vs Remote Attestation
          </p>
          <p>
            ...
            <br />
            - HMAC 기반 — 빠름, 오프라인 가능
            <br />- TDX Module이 MAC 키 관리
          </p>
          <p className="mt-2">
            <strong>Remote Attestation (Quote)</strong>:<br />
            - 외부 Relying Party가 검증
            <br />
            - ECDSA + PCK 인증서 체인 — Intel 신뢰 체인 활용
            <br />- JSON/binary 직렬화 가능
          </p>
          <p className="mt-2">
            <strong>SGX 대비 개선점</strong>:<br />
            - EPID 없음 → Intel IAS 서버 의존성 제거
            <br />
            - PCS 셀프호스팅 가능 (pccs Docker)
            <br />- TD단위 증명 (VM 전체) — 더 넓은 TCB
          </p>
        </div>
      </div>
    </section>
