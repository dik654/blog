  ...
  return (
    <section id="attacks" className="mb-16 scroll-mt-20">
      ...
      <div className="prose prose-neutral dark:prose-invert max-w-none">
        ...
        <p>
          TDX 위협 모델: <strong>Host(VMM), BIOS, 관리자, 물리적 공격자</strong>{" "}
          전부 untrusted
          <br />
          Trusted:{" "}
          <strong>CPU, TDX Module(SEAM), 메모리 컨트롤러 내부 로직</strong>
          <br />
          ...
          <table className="min-w-full text-sm border border-border">
            ...
            <tbody>
              ...
              <tr>
                ...
                <td className="border border-border px-3 py-2">
                  TDX speculative race — SEAMCALL 경합
                </td>
...
// - 핫패스(interrupt, TDVMCALL)에 누적 오버헤드`}</pre>
        ...
        <h3 className="text-xl font-semibold mt-8 mb-3">
          Transient Execution — IBRS/STIBP
        </h3>
...
// - 금융·의료 등 고민감도 워크로드엔 추가 방어 필요`}</pre>
        ...
        <div className="bg-amber-50 dark:bg-amber-950/30 border-l-4 border-amber-400 p-4 my-6 rounded-r-lg">
          ...
          <p>
            ...
            - 메모리 128MB 제한 (EPC)
            <br />
            - 페이지 swap 시 암호화·무결성·replay 완전 방어 (MEE)
            <br />- 작은 TCB → 검증 용이
          </p>
          <p className="mt-2">
            ...
            - 전체 물리 메모리 사용 가능
            <br />
            - TDX 1.0 — replay 취약, 1.5 — 무결성 추가
            <br />- 큰 TCB (Guest OS 포함) → 공격 표면 넓음
          </p>
          <p className="mt-2">
            <strong>선택 기준</strong>:<br />
            - 작은 기밀 코드 + 최고 보안 → SGX
            <br />
            - 기존 VM 마이그레이션 + 편의성 → TDX
            <br />- SGX는 EOL 진행 중 (클라이언트 CPU) → TDX가 서버 표준
          </p>
          <p className="mt-2">
            <strong>AMD SEV-SNP와 비교</strong>:<br />
            - SEV-SNP도 VM 단위, 무결성 기본 (RMP)
            <br />
            - TDX는 TDX Module이 추가 TCB → 공격 표면
            <br />- 성능·생태계는 비슷한 수준
          </p>
        </div>
      </div>
