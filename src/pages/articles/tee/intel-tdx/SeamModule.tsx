...
export default function SeamModule() {
    ...
    <section id="seam-module" className="mb-16 scroll-mt-20">
      ...
      <div className="prose prose-neutral dark:prose-invert max-w-none">
        <h3 className="text-xl font-semibold mt-6 mb-3">
          SEAM — 하이퍼바이저 위의 새 권한 레벨
        </h3>
        <p>
          <strong>SEAM(Secure Arbitration Mode)</strong>은 x86에 추가된{" "}
          <strong>새 CPU 실행 모드</strong>
          <br />
          위치: VMX Root(Ring -1) 보다 더 높은 권한 — "Ring -2" 개념
          <br />
  ...
  └─ (SEAM만의 독립 메모리 영역: SEAMRR)`}</pre>
        <p>
          <strong>SEAMRR(SEAM Range Register)</strong>: BIOS가 예약하는 메모리
          영역
          <br />
          - 크기: 보통 256MB
          <br />
          - 오직 SEAM 모드에서만 접근 가능
          <br />- 하이퍼바이저조차 이 영역 읽기/쓰기 불가
        </p>
        ...
        <p>
          P-SEAMLDR은 TD Module을 <strong>검증 후 SEAMRR에 로드</strong>하는
          프로그램
          <br />
...
- 기존 TD는 유지 (상태 마이그레이션)`}</pre>
        <p>
          <strong>핵심</strong>: TD Module 버전이 올라가도{" "}
          <strong>실행 중인 TD는 영향 없음</strong>
          <br />
...
└── 메모리 암호화 키 관리`}</pre>
        <p>
          <strong>TDH vs TDG</strong>:<br />- <code>TDH.*</code>: Host가{" "}
          <strong>SEAMCALL</strong>로 호출
          <br />- <code>TDG.*</code>: Guest(TD)가 <strong>TDCALL</strong>로 호출
          <br />둘 다 CPU가 <strong>SEAMCALL/TDCALL</strong> 명령으로 SEAM 진입
        </p>
            ...
            <tbody>
              <tr>
                ...
                <td className="border border-border px-3 py-2">
                  <code>CREATE, KEYCONFIG, INIT, FINAL, KEYRECLAIM</code>
                </td>
              ...
              <tr>
                ...
                <td className="border border-border px-3 py-2">
                  <code>PAGE.ADD, PAGE.AUG, PAGE.RELOCATE, RANGE.BLOCK</code>
                </td>
                <td className="border border-border px-3 py-2">
                  TD 메모리 관리
                </td>
              ...
              <tr>
                ...
                <td className="border border-border px-3 py-2">
                  <code>CREATE, INIT, ENTER, FLUSH, RD/WR</code>
                </td>
              ...
              <tr>
                ...
                <td className="border border-border px-3 py-2">
                  <code>PAGE.RECLAIM, PAGE.WBINVD, CACHE.WB</code>
                </td>
              ...
              <tr>
                ...
                <td className="border border-border px-3 py-2">
                  <code>INIT, LP.INIT, CONFIG, UPDATE, SHUTDOWN</code>
                </td>
                <td className="border border-border px-3 py-2">
                  TDX 시스템 관리
                </td>
...
총 시스템 메모리의 ~1.5% 예약됨`}</pre>
        ...
        <div className="bg-amber-50 dark:bg-amber-950/30 border-l-4 border-amber-400 p-4 my-6 rounded-r-lg">
          ...
          <p>
            기존 VMX(Ring -1)는 <strong>하이퍼바이저가 최상위</strong>
            <br />→ TD 보호 로직을 하이퍼바이저에 두면 하이퍼바이저가 악성일 때
            무력화
          </p>
          <p className="mt-2">
            SEAM은 <strong>하이퍼바이저 위의 새 레벨</strong>:<br />
            - TD Module 코드가 SEAMRR에서만 실행
            <br />
            - 하이퍼바이저가 Ring -1이지만 SEAMRR은 접근 불가
            <br />- CPU 마이크로아키텍처가 SEAM 경계 강제
          </p>
          <p className="mt-2">
            대안: <strong>Intel SMM</strong>(System Management Mode)을 재활용할
            수도 있었음
            <br />
