  ...
  return (
    <section id="tdvmcall" className="mb-16 scroll-mt-20">
      <h2 className="text-2xl font-bold mb-6">
        TDVMCALL — GHCI &amp; I/O 모델
      </h2>
      <div className="prose prose-neutral dark:prose-invert max-w-none">
        <h3 className="text-xl font-semibold mt-6 mb-3">
          GHCI — Guest-Host Communication Interface
        </h3>

        <TdvmcallViz />

        <p>
          <strong>GHCI</strong>: TD Guest가 Host에 요청하는 표준 프로토콜
          <br />
          모든 Host 서비스는 <code>TDG.VP.VMCALL</code>(TDVMCALL)로 통과
          <br />
...
// R10 = 0 (성공) 또는 TDX_VMCALL_RETRY 등`}</pre>
        ...
        <h3 className="text-xl font-semibold mt-8 mb-3">
          Linux TDX Guest의 __tdx_hypercall
        </h3>
...
SYM_FUNC_END(__tdx_hypercall)`}</pre>
        <p>
          <strong>EXPOSE_REGS_MASK</strong>: 어떤 레지스터를 Host에 노출할지
          비트마스크
          <br />
          TD 자체 비밀 레지스터(FLAGS, FS_BASE 등)는 자동 숨김
          <br />
          Host가 악의적으로 레지스터 읽어도 R10..R15만 공개
        </p>

        <h3 className="text-xl font-semibold mt-8 mb-3">
          주요 GHCI Sub-functions
        </h3>
...
set_memory_encrypted(vaddr, numpages);  // Shared → Private`}</pre>
        <p>
          <strong>MapGPA</strong>: DMA 수행 전 버퍼를 Shared로 전환 필수
          <br />
...
long tdx_get_quote(struct tdx_quote_hdr *quote_hdr)
{
    ...
    return 0;
        ...
        <p>
          <strong>2단계 증명</strong>: 로컬 TDREPORT → Host → Quote Service
          <br />
        ...
        <div className="bg-amber-50 dark:bg-amber-950/30 border-l-4 border-amber-400 p-4 my-6 rounded-r-lg">
          ...
          <p>
            <strong>전제</strong>: Host는 untrusted — 악의적으로 응답할 수 있음
            <br />
            <strong>방어 원칙</strong>:
          </p>
          <p className="mt-2">
            1. <strong>검증 가능한 응답만 신뢰</strong>
            <br />
            - CPUID 결과 중 보안 민감값은 TDX Module이 고정 (MAX_PA 등)
            <br />- MSR은 write-once/read-fixed 정책 사용 가능
          </p>
          <p className="mt-2">
            2. <strong>I/O 데이터는 신뢰 경계 밖</strong>
            <br />
            - 디스크 데이터 → dm-crypt로 TD 내부 암호화
            <br />
            - 네트워크 → TLS로 end-to-end 보호
            <br />- virtio 응답은 항상 검증 필요
          </p>
          <p className="mt-2">
            3. <strong>DoS는 허용</strong>
            <br />
            - Host가 응답 거부 → TD hang
            <br />- TDX는 기밀성·무결성만 보장, 가용성은 아님
          </p>
        </div>
      </div>
    </section>
