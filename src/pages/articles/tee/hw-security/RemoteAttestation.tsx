  ...
  return (
    <section id="remote-attestation" className="mb-16 scroll-mt-20">
      ...
      <div className="prose prose-neutral dark:prose-invert max-w-none">
        ...
        <p>
          <strong>원격 증명</strong>(Remote Attestation): TEE 내부 코드의
          무결성을 원격 검증
          <br />
          <strong>핵심 질문</strong>: "서버에서 내가 의도한 코드가 변조 없이
          실행 중인가?"
          <br />
          ...
          <table className="min-w-full text-sm border border-border">
            ...
            <tbody>
              <tr>
                ...
                <td className="border border-border px-3 py-2">
                  Intel Root → Platform CA → PCK → AK → Quote
                </td>
                <td className="border border-border px-3 py-2">
                  Binary struct + ECDSA P-256
                </td>
              </tr>
              <tr>
                <td className="border border-border px-3 py-2">Intel TDX</td>
                <td className="border border-border px-3 py-2">
                  Intel Root → PCK → AK → TD Quote
                </td>
                <td className="border border-border px-3 py-2">
                  Binary struct + ECDSA
                </td>
              ...
              <tr>
                ...
                <td className="border border-border px-3 py-2">
                  AMD Root → ARK → ASK → VCEK → Report
                </td>
                <td className="border border-border px-3 py-2">
                  Binary report + ECDSA P-384
                </td>
              </tr>
              <tr>
                <td className="border border-border px-3 py-2">ARM CCA</td>
                <td className="border border-border px-3 py-2">
                  SiP CA → IAK → RAK → CCA Token
                </td>
                <td className="border border-border px-3 py-2">
                  CBOR/COSE/EAT
                </td>
...
# - 공격 탐지 시 emergency revocation`}</pre>
        ...
        <div className="bg-amber-50 dark:bg-amber-950/30 border-l-4 border-amber-400 p-4 my-6 rounded-r-lg">
          <p className="font-semibold mb-2">
            인사이트: Attestation의 실전적 함정
          </p>
          <p>
            <strong>흔한 실수</strong>:<br />
            ✗ Nonce 없이 report 받기 (replay 가능)
            <br />
            ✗ Cert chain 검증 생략
            <br />
            ✗ MRENCLAVE hardcoding (업데이트 불가)
            <br />
            ✗ TCB version 체크 skipping
            <br />✗ user_data binding 없이 세션 키 사용
          </p>
          <p className="mt-2">
            <strong>Best practices</strong>:<br />
            ✓ Nonce 매번 새로 생성
            <br />
            ✓ MRSIGNER + SVN으로 업그레이드 허용
            <br />
            ✓ TCB policy 명시적으로
            <br />
            ✓ user_data에 세션 키 해시 binding
            <br />✓ 정책은 audit-loggable 형태로 저장
          </p>
          <p className="mt-2">
            <strong>프레임워크 활용</strong>:<br />
            - Intel SGX SDK Quote Verification Library
            <br />
            - Veraison (open-source verifier)
            <br />
            - Keylime (TPM-based attestation)
            <br />
            - Azure Attestation Service
            <br />→ 직접 구현보다 검증된 라이브러리 사용
          </p>
        </div>
      </div>
    </section>
