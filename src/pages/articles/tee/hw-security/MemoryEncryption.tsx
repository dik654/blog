  ...
  return (
    <section id="memory-encryption" className="mb-16 scroll-mt-20">
      ...
      <div className="prose prose-neutral dark:prose-invert max-w-none">
        ...
        <p>
          <strong>메모리 암호화</strong>: DRAM에 저장된 데이터를 HW 레벨 암호화
          <br />
          <strong>목적</strong>: 물리적 접근(콜드부트, 버스 스니핑, DRAM probe)
          방어
          <br />
          <strong>위치</strong>: 메모리 컨트롤러 내장 AES 엔진
          <br />
          <strong>오버헤드</strong>: ~2-5% (최신 HW에서 거의 무시 가능)
        </p>

        <h3 className="text-xl font-semibold mt-8 mb-3">
          AES-XTS 모드 — 주소 기반 암호화
        </h3>
...
// - 메모리 암호화는 SW 공격 방어 아님`}</pre>
        ...
        <div className="bg-amber-50 dark:bg-amber-950/30 border-l-4 border-amber-400 p-4 my-6 rounded-r-lg">
          ...
          <p>
            ...
            - 공격자가 ciphertext 변조 가능
            <br />
            - 복호화 시 random bytes 반환
            <br />- Deterministic 예측 가능한 변조 → 취약
          </p>
          <p className="mt-2">
            <strong>무결성 있음 (SGX MEE, SEV-SNP RMP, TDX 1.5)</strong>:<br />
            - MAC/hash tree로 변조 탐지
            <br />
            - Fault 발생 → 공격 중단
            <br />- Replay 방어도 포함 가능
          </p>
          <p className="mt-2">
            ...
            - 무결성 없는 암호화: ~2% 오버헤드
            <br />
            - 무결성 추가: ~5-30% (구현 방식에 따라)
            <br />
            - SGX MEE Merkle tree: 가장 강력 but 가장 느림
            <br />
            - SEV-SNP RMP: 적절한 균형
            <br />- TDX 1.5 28-bit MAC: 최소 오버헤드
          </p>
        </div>
      </div>
    </section>
