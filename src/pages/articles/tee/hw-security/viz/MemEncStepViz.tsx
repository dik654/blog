...
const STEPS = [
  {
    label:
      "AES-XTS 내부 — tweak = AES(key2, sector_addr), 블록별 XOR 후 AES(key1) 수행",
  },
  {
    label:
      "무결성 트리 — MAC[i] = CMAC(key3, data || addr || ver), Merkle Root까지 검증",
  },
  {
    label:
      "리플레이 방지 — VER_TABLE[phys_addr]++ 후 MAC 재계산, 카운터 불일치 = 탐지",
  },
];
        ...
        >
          ...
          {step === 1 && (
            <g>
              ...
              {[
                {
                  line: "MAC[i] = CMAC(key3, data_i || phys_addr || version)",
                  c: "#10b981",
                  y: 48,
                },
                { line: "H1 = SHA-256(MAC[0] || MAC[1])", c: "#6366f1", y: 78 },
                {
                  line: "Root = SHA-256(H1 || H2 || ... || Hn)",
                  c: "#6366f1",
                  y: 108,
                },
              ].map((l, i) => (
          ...
          {step === 2 && (
            <g>
              ...
              {[
                ...
                {
                  line: "MAC = CMAC(key, data || VER_TABLE[phys_addr])",
                  c: "#10b981",
                  y: 92,
                },
                {
                  line: "// 읽기 시: 저장된 MAC vs 재계산 MAC 비교",
                  c: "#6366f1",
                  y: 120,
                },
                {
                  line: "if (mac_stored != mac_calc) → SECURITY_FAULT",
                  c: "#ef4444",
                  y: 142,
                },
              ].map((l, i) => (
