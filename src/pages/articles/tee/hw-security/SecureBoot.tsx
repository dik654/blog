  ...
  return (
    <section id="secure-boot" className="mb-16 scroll-mt-20">
      ...
      <div className="prose prose-neutral dark:prose-invert max-w-none">
        ...
        <p>
          <strong>측정 부팅</strong>(Measured Boot): 각 부팅 단계의 바이너리를
          해시하여 TPM에 기록
          <br />
          <strong>차이점</strong>: Secure Boot가 "실행 차단"이라면 Measured
          Boot는 "사후 검증 가능"
          <br />
          <strong>TPM 역할</strong>: Platform Configuration Register(PCR)에
          측정값 누적
          <br />
          <strong>원격 증명</strong>: 부팅 후 PCR 값을 원격 서버가 검증 → "정상
          부팅됐나"
        </p>
...
// - rootfs verification`}</pre>
        ...
        <div className="bg-amber-50 dark:bg-amber-950/30 border-l-4 border-amber-400 p-4 my-6 rounded-r-lg">
          <p className="font-semibold mb-2">
            인사이트: DRTM (Dynamic RoT for Measurement)
          </p>
          <p>
            <strong>SRTM vs DRTM</strong>:<br />- <strong>SRTM</strong>(Static
            RoT): 전원 on부터 측정 → PCR 전체 chain
            <br />- <strong>DRTM</strong>(Dynamic RoT): 런타임에 late launch로
            새 chain 시작
          </p>
          <p className="mt-2">
            <strong>DRTM 장점</strong>:<br />
            ✓ BIOS·bootloader 측정 건너뛸 수 있음
            <br />
            ✓ Untrusted OS에서 trusted 환경 launch
            <br />✓ Intel TXT, AMD SKINIT 구현
          </p>
          <p className="mt-2">
            <strong>사용 사례</strong>:<br />
            - Trusted Boot (tboot)
            <br />
            - OpenXT (Xen-based secure VM)
            <br />
            - Windows VBS / Credential Guard
            <br />
            - Microsoft CHERIoT
            <br />- 일반 Linux distros는 주로 SRTM
          </p>
        </div>
      </div>
    </section>
