...
import { measuredBootFade as fade } from "./measuredBootMotion";
...
export function PcrExtendStep() {
  const lines = [
    ...
    {
      line: "PCR_Write(pcr_index, digest)      // 누적 갱신",
      c: C.hash,
      y: 88,
    },
...
export function AttestationStep() {
  const lines = [
    ...
    {
      line: "  quote_info = {pcr_digest, nonce, clock_info}",
      c: C.main,
      y: 66,
    },
    { line: "  sig = HMAC_SHA256(AIK_priv, quote_info)", c: C.attest, y: 88 },
    {
      line: "// 검증자: AIK_pub로 서명 확인 + 기대 PCR 비교",
      c: C.hash,
      y: 112,
    },
  ];
