    ...
    <section id="snp" className="mb-16 scroll-mt-20">
      ...
      {onCodeRef && (
        <div className="not-prose flex flex-wrap gap-2 mb-6">
          <CodeViewButton
            onClick={() => onCodeRef("rmp-entry", codeRefs["rmp-entry"])}
          />
          <span className="text-[10px] text-muted-foreground self-center">
            RMP 엔트리 구조
          </span>
          <CodeViewButton
            onClick={() => onCodeRef("vmpl-perms", codeRefs["vmpl-perms"])}
          />
          <span className="text-[10px] text-muted-foreground self-center">
            VMPL 권한 마스크
          </span>
          <CodeViewButton
            onClick={() => onCodeRef("pvalidate", codeRefs["pvalidate"])}
          />
          <span className="text-[10px] text-muted-foreground self-center">
            PVALIDATE 구현
          </span>
      ...
      <div className="prose prose-neutral dark:prose-invert max-w-none">
        ...
        <p>
          <strong>RMP</strong>: 모든 물리 메모리 페이지의 메타데이터 테이블
          (BIOS가 할당)
          <br />각 엔트리가 <strong>
            페이지 소유자
          </strong>(hypervisor/guest), <strong>유효 GPA</strong>,{" "}
          <strong>VMPL 권한</strong> 기록
          <br />
          메모리 접근마다 CPU가 RMP 조회 → 위반 시 <strong>RMP Fault</strong>
          <br />
...
// - Guest가 명시적 "이 페이지 받겠다" 표시`}</pre>
        ...
        <h3 className="text-xl font-semibold mt-8 mb-3">
          VMPL (VM Permission Level)
        </h3>
        <p>
          게스트 VM 내부를 <strong>0~3 단계</strong>로 권한 계층화
          <br />
          VMPL 0 = 최고 권한 (guest 내 신뢰 서비스), VMPL 3 = 최저 (일반 앱)
          <br />
...
// - Windows VBS, Linux SVSM이 이 모델 사용`}</pre>
      ...
      <div className="prose prose-neutral dark:prose-invert max-w-none">
        <h3 className="text-xl font-semibold mt-8 mb-3">
          측정 체인 (Measurement Chain)
        </h3>
        <p>
          <strong>SEV-SNP</strong>: 게스트 런치 각 단계를 측정(hash)하여 누적
          <br />
          펌웨어 → BIOS → 부트로더 → 커널 이미지 → 초기 데이터 전체 부트 체인
          반영
          <br />
      ...
      <div className="prose prose-neutral dark:prose-invert max-w-none">
        <div className="bg-amber-50 dark:bg-amber-950/30 border-l-4 border-amber-400 p-4 my-6 rounded-r-lg">
          ...
          <p>
            ...
            - Extended Page Table 기반 (MMU walk)
            <br />
            - Stage 2 번역 + 권한
            <br />- TD Module이 관리
          </p>
          ...
          <p className="mt-2">
            ...
            - RMP: 단순, 큰 메모리 → 큰 table (4GB/TB)
            <br />
            - S-EPT: 계층적, 희소 메모리 효율적
            <br />
            - 둘 다 hypervisor 재매핑 공격 방어
            <br />- SNP가 PVALIDATE로 guest-side validation 추가 (TDX의 ACCEPT와
            유사)
          </p>
        </div>
      </div>
