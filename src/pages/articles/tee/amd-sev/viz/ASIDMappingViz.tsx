...
const STEPS = [
  {
    label: "ASID — VM별 메모리 키 선택자",
    body: "각 SEV 게스트에 고유 ASID 할당. 메모리 컨트롤러가 ASID로 올바른 암호화 키 선택",
  },
  {
    label: "ASID 매핑 테이블",
    body: "ASID 0 = 하이퍼바이저(암호화 없음), ASID 1~N = 각 게스트 VM의 고유 키",
  },
  {
    label: "세대별 최대 동시 VM 수",
    body: "Naples 15개 → Rome/Milan 509개 → Genoa 1006개로 확장",
  },
  {
    label: "VEK 생명주기",
    body: "PSP가 런치 시 랜덤 생성 → 호스트/하이퍼바이저 접근 불가 → VM 종료 시 자동 폐기",
  },
];
        ...
        >
          ...
          <motion.g animate={{ opacity: step <= 1 ? 1 : 0.15 }}>
            <rect
              ...
              fill={`${C.indigo}10`}
            />
            <text x={30} y={26} fontSize={10} fontWeight={700} fill={C.indigo}>
              ASID
            </text>
            <text
              x={100}
              ...
              return (
                <g key={i}>
                  ...
                  <rect
                    ...
                    fill={
                      active && i > 0
                        ? `${typeof a.color === "string" && a.color.startsWith("#") ? a.color : C.indigo}08`
                        : "transparent"
                    }
                  />
                  <text
          ...
          <motion.g animate={{ opacity: step === 2 ? 1 : 0.12 }}>
            <rect
              ...
              fill={`${C.green}10`}
            />
            <text x={320} y={26} fontSize={10} fontWeight={700} fill={C.green}>
              세대
            </text>
            <text
              x={440}
