...
const STEPS = [
  {
    label: "PSP 펌웨어 모듈 구성",
    body: "firmware, launch, certs, session, vmsa — 각 모듈이 독립적 보안 기능 담당",
  },
  {
    label: "호스트 → PSP 요청 전달",
    body: "Application → LibVirt → QEMU/KVM → /dev/sev ioctl → CCP 드라이버",
  },
  {
    label: "CCP → PSP 메일박스 레지스터",
    body: "CCP 드라이버가 PSP 메일박스 레지스터에 명령 기록",
  },
  {
    label: "PSP 펌웨어 처리 → 응답 반환",
    body: "PSP가 암호화/서명 수행 후 동일 경로로 응답 반환",
  },
];
        ...
        >
          ...
          {LAYERS.map((l, i) => {
            ...
            return (
              <motion.g
                key={l.label}
                animate={{ opacity: step === 0 ? 0.1 : active ? 1 : 0.25 }}
              >
                <rect
                  x={80}
                  y={y}
                  ...
                  height={18}
                  rx={4}
                  fill={active ? "#6366f118" : "var(--card)"}
                  stroke={active ? "#6366f1" : "#88888830"}
                ...
                <text
                  ...
                  fontSize={10}
                  fontWeight={500}
                  fill={active ? "#6366f1" : "var(--muted-foreground)"}
                >
                  {l.label}
                </text>
                <text
          ...
          {step === 3 && (
            <motion.path
              d="M78,186 L60,186 L60,82 L78,82"
              fill="none"
              stroke="#10b981"
              strokeWidth={1.2}
              strokeDasharray="4 3"
