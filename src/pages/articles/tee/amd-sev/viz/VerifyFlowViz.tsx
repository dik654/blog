...
const STEPS = [
  {
    label: "게스트: nonce 기반 보고서 요청",
    body: "랜덤 64-byte nonce 생성 → PSP에 Attestation Report 요청 → VCEK 서명 포함",
  },
  {
    label: "보고서 전달",
    body: "게스트가 report + VCEK 인증서를 Verifier에게 전송",
  },
  {
    label: "Verifier: 인증서 체인 검증",
    body: "AMD KDS에서 VCEK 인증서 획득 → VCEK → ASK → ARK 체인 검증",
  },
  {
    label: "Verifier: 서명 + 핵심 필드 검증",
    body: "서명 검증, measurement == 예상값, report_data == nonce, debug 비활성 확인",
  },
];
