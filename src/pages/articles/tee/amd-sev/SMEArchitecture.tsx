  ...
  return (
    <section id="sme-architecture" className="mb-16 scroll-mt-20">
      ...
      <div className="prose prose-neutral dark:prose-invert max-w-none">
        <p>
          <strong>SME(Secure Memory Encryption)</strong>: AMD EPYC의 per-page
          메모리 암호화
          <br />
          <strong>C-bit</strong>으로 페이지 단위 암호화 여부 제어 (SEV의 기반)
          <br />
          <strong>TME(Transparent Memory Encryption)</strong>: 전체 DRAM을 단일
          키로 투명 암호화
          <br />
...
// SEV-SNP = 위 전부 + 무결성·replay 방어`}</pre>
        ...
        <div className="bg-amber-50 dark:bg-amber-950/30 border-l-4 border-amber-400 p-4 my-6 rounded-r-lg">
          ...
          <p>
            ...
            - 기존 PTE의 reserved 비트 재활용 → 하위 호환성
            <br />
            - OS가 페이지별 policy 직접 제어 가능
            <br />- 메모리 컨트롤러가 PTE 없이도 판단 (TLB에 C-bit cache)
          </p>
          <p className="mt-2">
            <strong>대안 비교</strong>:<br />
            - Intel KeyID(MKTME): PA 상위 비트 — OS가 KeyID 할당
            <br />
            - ARM GPT: 별도 테이블 — 2차 조회 필요
            <br />- AMD C-bit: PTE 1비트 — 가장 단순
          </p>
          <p className="mt-2">
            <strong>단점</strong>:<br />
            ✗ 48-bit PA 공간이 실제 47-bit (C-bit가 비트 47 사용)
            <br />
            ✗ OS가 C-bit 잘못 설정 시 random 데이터 접근
            <br />✗ DMA는 별도 IOMMU 설정 필요
          </p>
        </div>
      </div>
    </section>
