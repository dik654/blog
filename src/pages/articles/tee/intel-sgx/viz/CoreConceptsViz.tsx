...
const STEPS = [
  {
    label: "엔클레이브 측정값: MRENCLAVE / MRSIGNER",
    body: "MRENCLAVE = 코드+데이터의 SHA-256 해시 (ECREATE→EADD→EEXTEND→EINIT). MRSIGNER = 서명자 RSA 공개키 해시. 신원 증명의 핵심.",
  },
  {
    label: "봉인 정책: KEYPOLICY 비트",
    body: "SGX_KEYPOLICY_MRENCLAVE = 동일 코드만 복호화. SGX_KEYPOLICY_MRSIGNER = 동일 서명자면 업그레이드 후에도 복호화 가능.",
  },
  {
    label: "초기화 전역 상태",
    body: "g_enclave_state: NOT_STARTED → INIT_DONE. EDMM_supported: 동적 메모리 관리 지원 여부. g_enclave_base: 엔클레이브 베이스 주소(RELRO).",
  },
];
      ...
      {(step) => (
        ...
        >
          {ITEMS[step].map((item, i) => {
            ...
            const gap = 20;
            const total =
              ITEMS[step].length * w + (ITEMS[step].length - 1) * gap;
