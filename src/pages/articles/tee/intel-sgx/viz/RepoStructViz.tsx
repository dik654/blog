...
const STEPS = [
  {
    label: "SDK: 엔클레이브 내부 라이브러리",
    body: "trts(Trusted Runtime), tseal(봉인), selib(EGETKEY/EREPORT 래퍼), edger8r(EDL→C 코드 생성), tlibc(엔클레이브용 libc).",
  },
  {
    label: "PSW: 호스트 측 서비스",
    body: "urts(Untrusted Runtime), ae/pce(Provisioning Certificate Enclave), aesm_service(원격 증명 조율 데몬).",
  },
  {
    label: "전체 구조: 신뢰/비신뢰 경계",
    body: "SDK는 엔클레이브 내부(Trusted)에서 실행. PSW는 호스트(Untrusted)에서 실행. ECALL/OCALL로 경계를 넘나듦.",
  },
];
