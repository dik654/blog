...
const STEPS = [
  {
    label: "입력: target_info + report_data",
    body: "target_info = QE의 MRENCLAVE + 속성(512B). report_data = 사용자 데이터 64B (예: 공개키 해시). 모든 포인터는 엔클레이브 내부 메모리 필수.",
  },
  {
    label: "CPU가 EREPORT 실행",
    body: "MRENCLAVE, MRSIGNER, CPUSVN, attributes를 보고서 본문에 기록. target_info 엔클레이브(QE)만 검증 가능한 128-bit CMAC 태그 첨부.",
  },
  {
    label: "sgx_report_t 출력 (432 bytes)",
    body: "body(측정값) + key_id(보고서 키 식별자) + mac(CMAC 128-bit). QE만 이 CMAC을 검증할 수 있음 — 로컬 증명 완료.",
  },
];
