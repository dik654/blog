...
const STEPS = [
  {
    label: "EGETKEY: CPU가 봉인 키 파생",
    body: "MRENCLAVE/MRSIGNER 정책 + CPU SVN + ISV SVN → 128-bit 봉인 키. 동일 엔클레이브에서만 동일 키 재생성 가능.",
  },
  {
    label: "AES-128 키 준비",
    body: "EGETKEY 반환값(128-bit)을 AES-GCM 대칭 키로 사용. 키는 엔클레이브 EPC 메모리 안에서만 존재.",
  },
  {
    label: "AES-GCM 암호화 + MAC 생성",
    body: "평문 + AAD(추가 인증 데이터) → AES-GCM → 암호문 + 128-bit MAC. sgx_sealed_data_t 구조체로 패키징.",
  },
  {
    label: "OCALL로 호스트에 전달 → 디스크 저장",
    body: "암호문을 OCALL로 호스트에 전달. 호스트 파일시스템에 저장. 호스트는 암호문만 보유 — 복호화 불가능.",
  },
];
            ...
            return (
              <g key={s.label}>
                ...
                <text
                  ...
                  fontSize={10}
                  fontWeight={600}
                  fill={active ? s.color : "var(--foreground)"}
                >
                  {s.label}
                </text>
                <text
