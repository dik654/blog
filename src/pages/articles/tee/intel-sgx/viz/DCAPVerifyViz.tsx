...
const STEPS = [
  {
    label: "Quote 파싱 + PCK 체인 조회",
    body: "isv_enclave_report에서 MRENCLAVE/MRSIGNER 추출. Intel PCS에서 PCK Cert → PCE Cert → Intel SGX Root CA 인증서 체인 조회.",
  },
  {
    label: "ECDSA 서명 검증",
    body: "PCK로 Quote의 ECDSA-P256 서명 검증. CPUSVN, ISVSVN이 최소 버전 이상인지 확인. MRENCLAVE/MRSIGNER 예상값 대조.",
  },
  {
    label: "신뢰 체인 확립",
    body: "앱 → EREPORT → QE(ECDSA) → Quote → PCK 체인 → Intel SGX Root CA. Root CA가 최종 신뢰 앵커.",
  },
];
              ...
              <g key={c.label}>
                {i > 0 && (
                  <motion.line
                    ...
                    x2={c.x - 50}
                    y2={62}
                    stroke={reveal ? c.color : "var(--border)"}
                    strokeWidth={1.2}
                    markerEnd="url(#darr)"
                    initial={{ pathLength: 0 }}
                    animate={{ pathLength: reveal ? 1 : 0.3 }}
                ...
                <text
                  ...
                  fontSize={10}
                  fontWeight={600}
                  fill={active ? c.color : "var(--foreground)"}
                >
                  {c.label}
                </text>
                <text
