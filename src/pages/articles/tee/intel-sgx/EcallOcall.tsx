...
export default function EcallOcall({
  onCodeRef,
}: {
    ...
    <section id="ecall-ocall" className="mb-16 scroll-mt-20">
      ...
      <div className="prose prose-neutral dark:prose-invert max-w-none">
        <p>
          ...
          <strong>OCALL</strong>(엔클레이브 → 호스트)로 명확히 구분합니다.
          <br />
          Edger8r(코드 생성기)가 EDL 파일을 파싱해 양쪽의 마샬링 코드를 자동
          생성합니다.
        </p>
      </div>
      ...
      <div className="prose prose-neutral dark:prose-invert max-w-none">
        ...
        <div className="not-prose flex flex-wrap gap-2 mb-4">
          <CodeViewButton
            onClick={() =>
              onCodeRef("is-ecall-allowed", codeRefs["is-ecall-allowed"])
            }
          />
          ...
          <CodeViewButton
            onClick={() => onCodeRef("trts-ecall", codeRefs["trts-ecall"])}
          />
          <span className="text-[10px] text-muted-foreground self-center">
            trts_ecall()
          </span>
          <CodeViewButton
            onClick={() => onCodeRef("do-ecall", codeRefs["do-ecall"])}
          />
          <span className="text-[10px] text-muted-foreground self-center">
            do_ecall()
          </span>
          <CodeViewButton
            onClick={() => onCodeRef("sgx-ocall", codeRefs["sgx-ocall"])}
          />
          <span className="text-[10px] text-muted-foreground self-center">
            sgx_ocall()
          </span>
          <CodeViewButton
            onClick={() => onCodeRef("do-oret", codeRefs["do-oret"])}
          />
          <span className="text-[10px] text-muted-foreground self-center">
            do_oret()
          </span>
          <CodeViewButton
            onClick={() =>
              onCodeRef("ocall-context", codeRefs["ocall-context"])
            }
          />
          <span className="text-[10px] text-muted-foreground self-center">
            ocall_context_t
          ...
          <CodeViewButton
            onClick={() => onCodeRef("ecall-table", codeRefs["ecall-table"])}
          />
          <span className="text-[10px] text-muted-foreground self-center">
            ecall_table_t
          </span>
