  ...
  return (
    <section id="rme-gpt" className="mb-16 scroll-mt-20">
      ...
      <div className="prose prose-neutral dark:prose-invert max-w-none">
        ...
        <p>
          <strong>RME</strong>: ARMv9-A 선택적 확장(ID_AA64PFR0_EL1.RME 필드)
          <br />
          <strong>4개 PAS(Physical Address Space)</strong>와{" "}
          <strong>GPT</strong> 하드웨어 지원 추가
          <br />
...
isb`}</pre>
        ...
        <p>
          <strong>GPT</strong>: 물리 메모리 granule(보통 4KB)마다{" "}
          <strong>소유 PAS</strong> 기록하는 테이블
          <br />
          CPU/DMA가 메모리 접근 시 하드웨어가 자동 조회 → 불일치 시{" "}
          <strong>GPF(Granule Protection Fault)</strong> 발생
          <br />
...
// TLB 플러시 필요: tlbi paallos`}</pre>
        ...
        <h3 className="text-xl font-semibold mt-8 mb-3">
          PAS 전환 — SMC Delegate/Undelegate
        </h3>
...
// - 반드시 zeroize 후 전환 (정보 유출 방지)`}</pre>
        ...
        <h3 className="text-xl font-semibold mt-8 mb-3">
          GPF(Granule Protection Fault) 처리
        </h3>
...
// 5. Host kernel이 segfault 처리`}</pre>
        ...
        <h3 className="text-xl font-semibold mt-8 mb-3">
          MEC — Memory Encryption Contexts (옵션)
        </h3>
...
// - 차세대: MEC 통합 예정`}</pre>
        ...
        <div className="bg-amber-50 dark:bg-amber-950/30 border-l-4 border-amber-400 p-4 my-6 rounded-r-lg">
          ...
          <p>
            <strong>Intel EPT/S-EPT</strong>: 가상화 전용 — IPA→PA 매핑 + 권한
            <br />
          ...
          <p className="mt-2">
            ...
            ✓ 단순함 — 4개 PAS만 관리
            <br />
            ✓ DMA에도 자동 적용 — SMMU와 통합
            <br />✓ 외부 RAS 이벤트 처리 편함
          </p>
          <p className="mt-2">
            <strong>단점</strong>:<br />
            ✗ per-Realm 메모리 격리는 RMM이 Stage 2로 따로 구현
            <br />
            ✗ GPT 갱신은 TLB shootdown 비용
            <br />✗ 4KB granule 단위 → 대용량 메모리 시 GPT 크기 증가
          </p>
        </div>
      </div>
    </section>
