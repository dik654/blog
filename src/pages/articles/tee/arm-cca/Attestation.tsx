  ...
  return (
    <section id="attestation" className="mb-16 scroll-mt-20">
      ...
      <div className="prose prose-neutral dark:prose-invert max-w-none">
        ...
        <p>
          <strong>이중 토큰 구조</strong>: Realm Token + Platform Token
          <br />
          <strong>포맷</strong>: CBOR(RFC 8949) + COSE(RFC 9052) — IETF 표준
          <br />
...
void get_attestation_token(u8 *challenge, u8 *token_buf) {
    ...
    while (offset < total_size) {
        ...
        <p>
          <strong>2단계 호출</strong>: INIT으로 시작 → CONTINUE로 chunk 전송
          <br />
          <strong>이유</strong>: 토큰이 1페이지 초과 가능 (여러 REM + Platform
          Token)
          <br />
          <strong>INIT에 challenge</strong>: 사용자가 제공하는 nonce (replay
          방어)
        </p>
...
func verify(tokenBytes, iakPubKey, rakPubKey []byte) error {
    ...
    return nil
        ...
        <div className="bg-amber-50 dark:bg-amber-950/30 border-l-4 border-amber-400 p-4 my-6 rounded-r-lg">
          ...
          <p>
            <strong>IETF RATS(Remote Attestation Procedures)</strong> 표준 기반
            <br />
            <strong>EAT(Entity Attestation Token)</strong> 공통 포맷 사용
          </p>
          <p className="mt-2">
            <strong>장점</strong>:<br />
            ✓ 벤더 독립 — 여러 CCA 플랫폼 통합 검증
            <br />
            ✓ IoT·모바일 기존 PSA 인프라 재사용
            <br />✓ 오픈 검증자 생태계 (Veraison, parsec 등)
          </p>
          <p className="mt-2">
            <strong>단점</strong>:<br />
            ✗ CBOR 디코딩 오버헤드
            <br />
            ✗ 토큰 크기 큼 (2~4KB) — TDX Quote 대비
            <br />✗ SiP별 IAK 인증서 정책 차이
          </p>
          <p className="mt-2">
            <strong>실전</strong>:<br />
            - Confidential Containers 프로젝트가 CCA attestation 통합 중<br />
            - Veraison이 크로스 플랫폼 verifier 제공 (TDX/SGX/SEV/CCA)
            <br />- Arm CCA API 사양: architectures.docs.arm.com
          </p>
        </div>
      </div>
    </section>
