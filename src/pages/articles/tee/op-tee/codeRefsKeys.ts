    ...
    annotations: [
      {
        lines: [33, 36],
        color: "sky",
        note: "SSK 구조체 -- 초기화 플래그 + 키 바이트",
      },
      {
        lines: [38, 38],
        color: "emerald",
        note: "전역 싱글턴 (부팅 시 1회 초기화)",
      },
    ],
    desc: `Secure Storage Key(SSK) 구조체입니다.
  ...
  "key-hmac": {
    ...
    annotations: [
      ...
      { lines: [49, 50], color: "emerald", note: "HMAC-SHA256 컨텍스트 할당" },
      {
        lines: [52, 57],
        color: "amber",
        note: "init -> update -> final 3단계 HMAC",
      },
    ],
    desc: `HMAC-SHA256 래퍼 함수입니다.
키 파생 체인의 기본 빌딩 블록으로:
  ...
  "key-fek-crypt": {
    ...
    annotations: [
      { lines: [76, 80], color: "sky", note: "입력 검증 + SSK 초기화 확인" },
      {
        lines: [82, 95],
        color: "emerald",
        note: "TSK 파생: HMAC(SSK, UUID) -- TA별 고유 키",
      },
      {
        lines: [97, 109],
        color: "amber",
        note: "TSK로 FEK 암/복호화 (AES-ECB)",
      },
      {
        lines: [113, 115],
        color: "violet",
        note: "민감 데이터 명시적 제로화 (memzero_explicit)",
      },
    ],
    desc: `FEK(File Encryption Key) 암/복호화 함수입니다.
