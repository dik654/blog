  ...
  return (
    <section id="overview" className="mb-16 scroll-mt-20">
      ...
      <div className="prose prose-neutral dark:prose-invert max-w-none">
        ...
        <p>
          <strong>OP-TEE</strong>(Open Portable TEE): ARM TrustZone 위 오픈소스
          TEE OS
          <br />
          <strong>Linaro 주도</strong>: 2014년 ST-Ericsson에서 오픈소스화,
          Linaro가 유지보수
          <br />
          <strong>GlobalPlatform TEE 스펙 구현</strong>: Trusted Application API
          표준 준수
          <br />
          <strong>2-world 모델</strong>: Normal World(Linux) ↔ Secure
          World(OP-TEE OS)
        </p>
...
// - QEMU v8 virtual (테스트용)`}</pre>
        ...
        <h3 className="text-xl font-semibold mt-8 mb-3">
          entry_a64.S — AArch64 보안 세계 진입 (실제 코드)
        </h3>
      </div>
      <div className="not-prose mb-6">
        <EntryAsmViz />
...
// - Optee_client로 HAL 연결`}</pre>
        ...
        <div className="bg-amber-50 dark:bg-amber-950/30 border-l-4 border-amber-400 p-4 my-6 rounded-r-lg">
          <p className="font-semibold mb-2">
            인사이트: OP-TEE vs SGX 철학 차이
          </p>
          <p>
            <strong>OP-TEE (TrustZone)</strong>:<br />
            - 모놀리식 보안 OS (TEE OS kernel)
            <br />
            - Secure world 전체 신뢰
            <br />
            - 작은 TCB (200K LoC + TA 코드)
            <br />- 모바일·임베디드 강점
          </p>
          <p className="mt-2">
            <strong>Intel SGX</strong>:<br />
            - Per-enclave 격리 (앱 내부 작은 영역)
            <br />
            - No OS inside enclave
            <br />
            - 매우 작은 TCB (SGX SDK runtime만)
            <br />- 서버·데이터센터 강점
          </p>
          <p className="mt-2">
            <strong>선택 기준</strong>:<br />
            - 모바일/IoT + TEE OS 필요: OP-TEE
            <br />
            - 서버 + 작은 enclave: SGX
            <br />
            - VM 단위 격리: TDX/SEV/CCA
            <br />- 각각 다른 use case에 최적화
          </p>
        </div>
      </div>
    </section>
