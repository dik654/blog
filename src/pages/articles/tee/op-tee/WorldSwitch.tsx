  ...
  return (
    <section id="world-switch" className="mb-16 scroll-mt-20">
      ...
      <div className="not-prose mb-4 flex flex-wrap gap-2">
        <CodeViewButton
          onClick={() =>
            onCodeRef("smc-fast-handler", codeRefs["smc-fast-handler"])
          }
        />
        ...
        <CodeViewButton
          onClick={() =>
            onCodeRef("smc-std-handler", codeRefs["smc-std-handler"])
          }
        />
        <span className="text-[10px] text-muted-foreground self-center">
          Standard SMC 핸들러
        ...
        <CodeViewButton
          onClick={() =>
            onCodeRef("smc-vector-table", codeRefs["smc-vector-table"])
          }
        />
        <span className="text-[10px] text-muted-foreground self-center">
          벡터 테이블 (A64)
        ...
        <CodeViewButton
          onClick={() =>
            onCodeRef("smc-std-entry-asm", codeRefs["smc-std-entry-asm"])
          }
        />
        <span className="text-[10px] text-muted-foreground self-center">
          SMC 진입 어셈블리
      ...
      <div className="prose prose-neutral dark:prose-invert max-w-none">
        <p>
          Normal World에서 <code>smc</code> 명령어가 실행되면 Secure
          Monitor(EL3)가 레지스터 컨텍스트를 저장·복원하며 세계 전환을
          수행합니다.
          <br />
