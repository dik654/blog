  ...
  return (
    <section id="memory-management" className="mb-16 scroll-mt-20">
      ...
      <div className="prose prose-neutral dark:prose-invert max-w-none">
        ...
        <p>
          <strong>TZASC</strong>(TrustZone Address Space Controller)가 DRAM을
          Secure/Normal 영역으로 분할
          <br />
          <strong>S-bit</strong>(Security bit): bus transaction에 전파되어 HW
          레벨 접근 제어
          <br />
          <strong>TLB entry</strong>마다 NS 비트 포함 → 잘못된 world 접근 시
          fault
          <br />
...
// - Pointer bounds 엄격 체크`}</pre>
        ...
        <div className="bg-amber-50 dark:bg-amber-950/30 border-l-4 border-amber-400 p-4 my-6 rounded-r-lg">
          <p className="font-semibold mb-2">
            인사이트: OP-TEE 메모리 보호의 현실적 제약
          </p>
          <p>
            <strong>TZASC 의존성</strong>:<br />
            - Secure memory 영역은 부팅 시 고정
            <br />
            - 런타임 동적 크기 조정 어려움
            <br />- Secure DRAM 크기 사전 계획 필수 (보통 16~64MB)
          </p>
          <p className="mt-2">
            <strong>Cache coherency 이슈</strong>:<br />
            - Secure/Non-secure 캐시 line 분리 됨 (동일 PA 두 엔트리)
            <br />
            - Shared memory 동기화 시 cache flush 필수
            <br />- Performance impact 주의
          </p>
          <p className="mt-2">
            <strong>Side channel 취약성</strong>:<br />
            - SGX/TDX와 달리 LLC 파티셔닝 부재 (일부 ARM)
            <br />
            - 캐시 timing 공격 가능
            <br />- TrustZone 자체 완화책 제한적
          </p>
        </div>
      </div>
    </section>
