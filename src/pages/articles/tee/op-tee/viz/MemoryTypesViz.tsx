      ...
      {(step) => (
        ...
        >
          <text
            ...
            fill="var(--foreground)"
          >
            teecore_memtypes 메모리 영역 분류
          </text>

          {/* Secure column */}
          <text x={80} y={38} fontSize={10} fontWeight={700} fill="#6366f1">
            Secure
          </text>
          {/* Non-secure column */}
          <text x={380} y={38} fontSize={10} fontWeight={700} fill="#f59e0b">
            Non-Secure
          </text>

          {TYPES.filter((t) => t.secure).map((t, i) => {
            const highlight =
              (step === 0 && i < 3) || (step === 1 && i === 3) || step === 3;
            const y = 48 + i * 30;
            return (
              <g key={t.name}>
                <motion.rect
                  ...
                  stroke={highlight ? t.color : `${t.color}30`}
