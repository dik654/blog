  ...
  return (
    <section id="overview" className="mb-16 scroll-mt-20">
      ...
      <div className="prose prose-neutral max-w-none dark:prose-invert">
        <p className="text-lg leading-8">Slot마다 block이 없더라도 BeaconState는 앞으로 진행합니다. 다음 slot이 epoch 경계에 닿으면 Prysm은 지난 epoch 동안 모은 participation evidence를 읽어 checkpoint를 갱신하고, reward·penalty를 계산하며, activation·exit·slashing·pending queue를 처리한 뒤 다음 epoch에 필요한 순환 state를 준비합니다.</p>
      ...
      <div id="paper-consensus-epoch-transition" className="not-prose my-8 scroll-mt-24 border-l border-primary/50 pl-4"><p className="text-xs font-bold text-primary">공식 규격 읽기 · epoch transition</p><p className="mt-2 text-sm font-semibold">Ethereum Consensus Specifications v1.6.1 · Altair, Electra, Fulu</p><p className="mt-2 text-sm leading-6 text-muted-foreground">문제는 누적 participation과 validator queue를 모든 client가 같은 ordered state transition으로 정산하는 것입니다. 규격은 함수·상수·reference test를 제공하지만 Prysm의 cache·parallelization·latency를 규정하지 않습니다. 이 글은 v1.6.1 stable fork를 고정하고 unstable fork 초안을 현재 mainnet behavior로 확대하지 않습니다.</p><a href="https://github.com/ethereum/consensus-specs/tree/v1.6.1/specs" target="_blank" rel="noreferrer" className="mt-3 inline-block text-sm font-medium text-primary hover:underline">고정한 consensus specs 보기</a></div>
    </section>
  );
}
