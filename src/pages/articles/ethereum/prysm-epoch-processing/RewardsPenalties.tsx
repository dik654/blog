  ...
  return (
    <section id="rewards-penalties" className="mb-16 scroll-mt-20">
      ...
      <div className="prose prose-neutral max-w-none dark:prose-invert"><p>
            Altair 계열 회계는 source·target·head participation flag를 따로 봅니다. Eligible validator마다 base reward를 구한
            뒤 참여한 flag에는 해당 참여 balance 비율을 곱한 reward를 줍니다. 참여하지 않은 source·target에는 penalty를 적용합니다. Head는 같은 대칭
            penalty 규칙을 따르지 않으므로 “놓친 reward = 동일한 penalty”로 단순화하면 안 됩니다.
          </p></div>
      <ExplainedFormula
      ...
      <div className="prose prose-neutral max-w-none dark:prose-invert">
        ...
        <p>
            Finality가 지연되면 timely target에 참여하지 않은 validator의 inactivity score가 올라가고 추가 penalty가 커집니다. 참여한
            validator는 score가 회복되고 비참여 weight가 줄어 남은 참여 weight가 다시 threshold를 만들 수 있게 합니다. 이 구조는 network
            state·score·balance가 연결된 feedback이며 특정 validator를 즉시 퇴출하는 timer로 동작하지 않습니다.
          </p>
        <h3>Precompute는 값의 소유권을 바꾸지 않습니다</h3>
        <p>
            Active balance와 flag별 participating set을 한 번 집계해 validator loop에서 재사용하면 반복 scan을 줄일 수 있습니다. 하지만
            cache key에서 epoch·fork·state root·flag를 빠뜨리면 계산은 빨라도 다른 state의 reward를 적용하게 됩니다. Full reference
            transition과 per-validator delta parity를 correctness oracle로 둡니다.
          </p>
      </div>
    </section>
  );
