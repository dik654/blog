  ...
  return (
    <section id="registry-slashings" className="mb-16 scroll-mt-20">
      ...
      <div className="prose prose-neutral max-w-none dark:prose-invert">
        <p>
            Registry update가 처리하는 것은 activation eligibility, activation epoch, low-balance ejection입니다.
            Electra의 pending deposits·partial withdrawals·consolidations는 finality와 churn budget을 확인하는 queue로
            들어가므로 request가 block에 포함됐다는 사실과 validator balance/lifecycle에 적용됐다는 사실은 다릅니다.
          </p>
        <p>
            Slashing은 proposer·attester의 객관적 충돌 evidence가 block processing에서 확인될 때 시작됩니다. 다만 즉시 penalty와
            correlated penalty·withdrawability는 여러 epoch에 걸쳐 적용됩니다. Evidence 수신, slashed flag, exit, penalty,
            withdrawal은 서로 다른 시점의 이벤트이므로 하나로 합치지 않습니다.
          </p>
      </div>
      <ExplainedFormula
      ...
      <div className="prose prose-neutral max-w-none dark:prose-invert">
        ...
        <p>
            Electra pending deposit은 legacy bridge deposit 선행 처리, queue position finalization, epoch당 개수,
            activation/exit churn budget을 확인합니다. 작은 예로 budget이 64 balance-unit이고 queue 앞 요청이 40·32라면 첫 요청 뒤
            24만 남아 두 번째를 이번 epoch에 모두 적용할 수 없습니다. Queue는 fork rule에 따라 postpone·carry-over가 생기므로 단순 FIFO pop처럼
            움직이지 않습니다.
          </p>
        <h3>Release gate는 balance 합계만 보지 않습니다</h3>
        <p>
            Fork 전·activation epoch·후 fixture에서 participation flags, checkpoints, per-validator
            reward/penalty, inactivity score를 base/candidate로 비교합니다. Registry epochs, pending queue order,
            slashing vector, post-state root도 같은 방식으로 대조합니다. 포함하는 시나리오는 1/3 미만·정확히 threshold·초과 participation,
            correlated slash, churn exhaustion, reorg, epoch-boundary crash/restart입니다. Parity를 통과한 뒤에
            CPU·allocation·epoch-processing p99를 봅니다.
          </p>
      </div>
      <div id="paper-prysm-epoch-source" className="not-prose my-8 scroll-mt-24 border-l border-primary/50 pl-4"><p className="text-xs font-bold text-primary">공식 구현 읽기 · Prysm epoch processing</p><p className="mt-2 text-sm font-semibold">OffchainLabs/prysm source repository</p><p className="mt-2 text-sm leading-6 text-muted-foreground">
            Consensus spec의 ordered epoch transition이 선택한 Prysm release의 precompute·state mutation·error path로
            구현되는 위치를 확인합니다. 특정 cache나 benchmark를 protocol guarantee로 일반화하는 것은 구현과 규격을 뒤섞는 읽기입니다.
          </p><a href="https://github.com/OffchainLabs/prysm" target="_blank" rel="noreferrer" className="mt-3 inline-block text-sm font-medium text-primary hover:underline">Prysm 공식 source 보기</a></div>
    </section>
  );
}
