...
export const primCodeRefs: Record<string, CodeRef> = {
  "primitives-addr": {
    path: "alloy-primitives/src/lib.rs",
    lang: "rust",
    highlight: [3, 8],
    desc: "Address(20B), B256(32B) — FixedBytes 기반 고정 길이 타입",
    annotations: [
  ...
  "fixed-bytes-struct": {
    path: "alloy-primitives/src/bits/fixed.rs",
    lang: "rust",
    highlight: [4, 7],
    desc: "FixedBytes<N> — const 제네릭 바이트 배열 래퍼",
    annotations: [
      { lines: [4, 6], color: "violet", note: "#[derive] 자동 구현" },
      { lines: [7, 7], color: "sky", note: "[u8; N] 고정 길이 배열" },
    ],
    code: fixedSrc,
  },
  "fixed-bytes-deref": {
  ...
  "fixed-bytes-addr": {
    path: "alloy-primitives/src/bits/address.rs",
    lang: "rust",
    highlight: [19, 28],
  ...
  "u256-overflowing": {
    path: "ruint/src/algorithms/add.rs",
    lang: "rust",
    highlight: [13, 23],
