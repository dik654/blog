    ...
    annotations: [
      {
        lines: [7, 9],
        color: "sky",
        note: "insert: tx_hash → sidecar 매핑 저장",
      },
      {
        lines: [19, 20],
        color: "emerald",
        note: "delete_all: finalized 블록의 blob 일괄 삭제",
      },
      {
        lines: [22, 23],
        color: "amber",
        note: "cleanup: 지연 삭제 수행 (DiskFileBlobStore용)",
      },
      {
        lines: [31, 32],
        color: "violet",
        note: "get_by_versioned_hashes: engine API에서 blob 조회",
      },
    ],
  },
  "disk-blobstore": {
    ...
    annotations: [
      {
        lines: [3, 4],
        color: "sky",
        note: "DEFAULT_MAX_CACHED_BLOBS: LRU 캐시 100개",
      },
      {
        lines: [17, 27],
        color: "emerald",
        note: "Inner: blob_cache(LRU) + file_lock(RwLock) + txs_to_delete",
      },
      {
        lines: [32, 39],
        color: "amber",
        note: "open: 이전 blob 삭제 후 빈 디렉토리 재생성",
      },
      {
        lines: [52, 56],
        color: "violet",
        note: "delete: 즉시 삭제하지 않고 삭제 대기 목록에 추가",
      },
    ],
  },
  "disk-inner-ops": {
    ...
    annotations: [
      {
        lines: [7, 9],
        color: "sky",
        note: "RLP 인코딩 — 디스크 저장용 직렬화",
      },
      {
        lines: [13, 18],
        color: "emerald",
        note: "versioned_hash → tx_hash 매핑 (engine API용)",
      },
      {
        lines: [21, 22],
        color: "amber",
        note: "LRU 캐시에 저장 — 최근 blob 빠른 조회",
      },
      {
        lines: [35, 42],
        color: "violet",
        note: "get_one: 캐시 미스 시 디스크 → 캐시 추가",
      },
    ],
  },
  "mem-blobstore": {
    ...
    annotations: [
      {
        lines: [11, 13],
        color: "sky",
        note: "RwLock<B256Map>: 동시 읽기 허용, 쓰기 시 독점",
      },
      {
        lines: [23, 25],
        color: "emerald",
        note: "insert_size: blob.size() 계산 후 크기 추적",
      },
      {
        lines: [36, 38],
        color: "amber",
        note: "cleanup no-op — 메모리는 delete에서 즉시 제거",
      },
    ],
  },
};
