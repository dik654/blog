...
export const validationRefs: Record<string, CodeRef> = {
  "tx-validate-stateless": {
    ...
    lang: "rust",
    highlight: [4, 45],
    desc: "문제: Blob TX가 풀에 진입하기 전 값싼 구조 오류를 먼저 걸러야 합니다.\n\n해결: 포크 활성 여부, 크기 한도, versioned hash와 blob 개수를 상태 조회 없이 검사합니다. 비용은 원소 수에 비례합니다.",
    annotations: [
      {
        lines: [9, 12],
        color: "sky",
        note: "Cancun 포크 미활성 → EIP-4844 거부",
      },
      {
        lines: [18, 24],
        color: "emerald",
        note: "blob TX는 input 바이트 기준 크기 제한 (본체만 메모리)",
      },
      {
        lines: [28, 41],
        color: "amber",
        note: "blob 개수 0이면 거부, max_blob_count 초과도 거부",
      },
    ],
  },
  "tx-validate-eip4844": {
    ...
    annotations: [
      {
        lines: [56, 57],
        color: "sky",
        note: "None: blob TX인데 사이드카 자체가 없음 → 에러",
      },
      {
        lines: [60, 65],
        color: "emerald",
        note: "Missing: re-org 재주입. BlobStore에 있으면 OK",
      },
      {
        lines: [68, 71],
        color: "amber",
        note: "Present: KZG 검증 수행 → 성공 시 사이드카 반환",
      },
    ],
  },
  "header-blob-gas": {
    ...
    annotations: [
      {
        lines: [7, 9],
        color: "sky",
        note: "header의 blob_gas_used 필드 존재 확인",
      },
      {
        lines: [10, 15],
        color: "emerald",
        note: "본체 합계 ≠ 헤더 값이면 BlobGasUsedDiff 에러",
      },
    ],
  },
  "header-4844-standalone": {
    ...
    annotations: [
      { lines: [28, 29], color: "sky", note: "blob_gas_used 필드 존재 확인" },
      {
        lines: [32, 34],
        color: "emerald",
        note: "parent_beacon_block_root — Cancun 필수 필드",
      },
      {
        lines: [38, 42],
        color: "amber",
        note: "DATA_GAS_PER_BLOB(131072)의 배수 검증",
      },
      {
        lines: [46, 49],
        color: "violet",
        note: "블록당 최대 blob gas 초과 확인",
      },
    ],
  },
};
