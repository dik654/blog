    ...
    annotations: [
      {
        lines: [31, 32],
        color: "sky",
        note: "validator.validate(tx) — 서명, nonce, 잔액 등 검증",
      },
      {
        lines: [34, 35],
        color: "emerald",
        note: "inner.add_transaction() — ordering 기준으로 적절한 서브풀에 삽입",
      },
      {
        lines: [37, 38],
        color: "amber",
        note: "notify_listeners() — 새 TX 알림 (PayloadBuilder, P2P 등)",
      },
    ],
  },
  "tx-validator": {
    ...
    annotations: [
      {
        lines: [9, 17],
        color: "sky",
        note: "검증 체인: 체인ID → 서명 → nonce → 잔액 → intrinsic gas → base fee",
      },
      {
        lines: [20, 21],
        color: "emerald",
        note: "is_local(): 로컬 TX는 풀이 가득 찰 때 제거 우선순위가 낮음",
      },
    ],
  },
  "tx-ordering": {
    ...
    annotations: [
      {
        lines: [22, 31],
        color: "sky",
        note: "effective_tip = min(max_priority_fee, max_fee - base_fee) → U256 변환",
      },
    ],
  },
};
