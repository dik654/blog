    ...
    <section id="ordering" className="mb-16 scroll-mt-20">
      ...
      <div className="prose prose-neutral dark:prose-invert max-w-none mb-6">
        ...
        <p>
          ...
          resource 안에서 후보를 선택해야 한다. Dynamic-fee transaction의
          proposer 수익은 현재 base fee를 반영한 effective tip으로 계산된다.
        </p>
        <h3>문제</h3>
        <p>
          전역 priority만 보고 높은 transaction을 꺼내면 같은 sender의 낮은
          nonce를 건너뛸 수 있다. 또한 높은 tip이라도 실행 실패, gas limit, blob
          gas, duplicate inclusion 같은 builder constraints 때문에 제외될 수
          있다.
        </p>
        <h3>아이디어와 구현</h3>
        <p>
          Ordering은 각 sender의 현재 eligible head에 priority를 부여한다.
          Iterator가 head를 내보낸 뒤 그 transaction이 포함 가능한 경우에만 같은
          sender의 다음 nonce가 새 후보가 된다. Builder는 후보를 실행하고 block
          constraints에 따라 accept·skip·descendant pruning을 수행한다.
        </p>
        <p>
          Ordering trait은 priority policy를 교체할 수 있게 하지만 arbitrary
          bundle semantics를 자동 제공하지 않는다. Bundle atomicity, private
          orderflow와 simulation은 별도 builder policy와 데이터 모델의 책임이다.
        </p>
      </div>
