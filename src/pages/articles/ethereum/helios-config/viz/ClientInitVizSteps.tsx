    ...
    <g>
      ...
      {stages.map((s, i) => (
        <motion.g key={s.label} {...fade(0.1 + i * 0.15)}>
          ...
          {/* 단계 번호 뱃지 */}
          <circle cx={s.x + 10} cy={26} r={7} fill={s.color} opacity={0.9} />
          <text
            x={s.x + 10}
            y={29.5}
            textAnchor="middle"
  ...
  return (
    <g>
        ...
        return (
          <motion.g key={lv.label} {...fade(0.15 + i * 0.25)}>
            ...
            <rect
              ...
              fill="var(--card)"
              stroke={lv.color}
              strokeWidth={lv.solid ? 1.2 : 0.8}
              strokeDasharray={lv.solid ? "none" : "5 3"}
            />

            {/* 우선순위 번호 */}
            <circle cx={98} cy={y + 20} r={9} fill={lv.color} opacity={0.9} />
            <text
              x={98}
              y={y + 24}
              textAnchor="middle"
      ...
      <motion.g {...fade(1.0)}>
        ...
        <text x={400} y={91} textAnchor="middle" fontSize={7} fill={C.disk}>
          wins
        </text>
