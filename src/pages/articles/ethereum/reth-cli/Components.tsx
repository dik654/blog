  ...
  return (
    <section id="components" className="mb-16 scroll-mt-20">
      ...
      <div className="prose prose-neutral max-w-none dark:prose-invert">
        ...
        <p>
          Executor·pool·network·payload builder는 block lifecycle의 core
          dependency이며, RPC module·ExEx·observability hook은 이미 만들어진
          component를 소비하는 add-on입니다. Custom chain이 EVM config를 교체할
          때 chain-specific transaction과 receipt rule을 함께 바꿔야 할 수
          있지만, 호환되는 network transport는 재사용할 수 있습니다. 반대로 RPC
          extension 하나가 실패했다고 consensus execution까지 손상됐다고
          단정해서도 안 됩니다. Owner와 failure status를 분리해야 selective
          retry와 rollback이 가능합니다.
        </p>
        ...
        <p>
          Compile fixture는 잘못된 assembly 순서, incompatible associated type과
          빠진 add-on bound가 실패하는지 확인합니다. Runtime matrix는 같은 chain
          spec·storage snapshot·config에서 missing JWT, occupied port,
          incompatible schema, component init failure, hook timeout, crash,
          graceful shutdown을 base와 candidate에 동일하게 주입합니다. Config
          normalization·service state· canonical hash와 state root parity를 hard
          gate로 둔 뒤 startup latency와 resource 사용량을 비교합니다.
        </p>
        <p>
      ...
      >
        ...
        <p className="mt-2 text-sm leading-6 text-muted-foreground">
          문제는 default·storage·network·SDK 변화가 upgrade 절차에 어떤 영향을
          주는지 찾는 것입니다. 이 release는 Discv5 default와 여러 gated
          기능·호환성 주의를 기록하지만, 이후 2.x나 모든 custom node가 같은
          default·feature 조합이라는 뜻은 아닙니다.
        </p>
        <a
