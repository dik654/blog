...
export function Node({
  ...
  return (
    ...
    >
      ...
      <p className="text-[10px] text-foreground/75 mt-0.5 leading-tight">
        {mod.role}
      </p>
      <div className="mt-1.5 border-t border-border/50 pt-1.5">
        <p className="text-[10px] font-mono text-blue-600 dark:text-blue-400 leading-tight truncate">
          {mod.fns[0].sig}
        </p>
        <p className="text-[10px] text-foreground/75 leading-tight mt-0.5 line-clamp-2">
          {mod.fns[0].desc}
        </p>
      </div>
    </button>
    ...
    <div className="relative flex flex-col items-center justify-center px-2 shrink-0 min-w-[60px] gap-1">
      ...
      <div className="relative w-full h-5 flex items-center">
        <div className="absolute inset-x-3 h-px overflow-hidden">
          <div
            className={`absolute inset-0 transition-colors ${animated ? "bg-emerald-300 dark:bg-emerald-700" : "bg-border"}`}
          />
          {animated && (
            <motion.div
        ...
        {(dir === "→" || dir === "↔") && (
          <span
            className={`absolute right-0 text-[9px] leading-none transition-colors ${animated ? "text-emerald-500" : "text-muted-foreground"}`}
          >
            ▶
          </span>
        )}
        {(dir === "←" || dir === "↔") && (
          <span
            className={`absolute left-0 text-[9px] leading-none transition-colors ${animated ? "text-emerald-500" : "text-muted-foreground"}`}
          >
            ◀
          </span>
        )}
...
export function VConn({
  ...
  return (
    <div className="relative flex flex-col items-center gap-0.5 py-1">
      <span
        className={`text-[9px] leading-none transition-colors ${animated ? "text-emerald-500" : "text-muted-foreground"}`}
      >
        ▲
      </span>
