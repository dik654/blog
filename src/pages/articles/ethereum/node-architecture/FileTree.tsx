...
function Node({
  node, currentPath, onSelect, depth = 0,
}: {
  node: FileNode;
  currentPath: string;
    ...
    return (
      <div>
        ...
        {open && node.children?.map(c => (
          <Node key={c.name} node={c} currentPath={currentPath} onSelect={onSelect} depth={depth + 1} />
        ))}
      </div>
    );
  }
...
export default function FileTree({
  ...
  return (
    <div className="font-mono py-1 select-none">
      <Node node={root} currentPath={currentPath} onSelect={onSelect} depth={0} />
    </div>
  );
}
