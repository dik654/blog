      ...
      <div className="flex items-center gap-2">
        <div className="flex flex-col gap-1">
          {["addr-A", "addr-B", "addr-C"].map((a) => (
            <div
              key={a}
              className="rounded border border-border px-2 py-0.5 font-mono text-[9px]"
            >
              {a}
            </div>
          ))}
        ...
        <div className="rounded-lg border-2 border-emerald-400 bg-emerald-50/60 dark:bg-emerald-950/20 px-2 py-1.5 text-center text-[9px]">
          ...
          <p className="text-[8px] text-foreground/75 font-sans mt-0.5">
            모든 계정 잔액·코드·스토리지
            <br />
            변경을 하나의 해시로 압축
          </p>
  ...
  return (
    <div className="space-y-1.5 text-[10px]">
      <div className="rounded border border-border/50 px-2 py-1 text-[9px] text-foreground/75">
        ...
        — EVM 실행과 분리된 비동기 저장 서비스. 블록 처리 속도를 높이기 위해
        디스크 쓰기를 별도 스레드에서 처리.
      </div>
      <p className="font-mono text-[9px] text-foreground/75">
        PersistenceService (비동기)
      </p>
      <div className="flex items-center gap-2">
        <div className="space-y-1">
          {items.map((it, i) => (
            <div
              key={it}
              className={`rounded border px-2 py-0.5 font-mono text-[9px] transition-all duration-300
