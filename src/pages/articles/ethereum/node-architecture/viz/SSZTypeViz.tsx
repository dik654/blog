...
const types = [
  {
    name: "BeaconState",
    desc: "검증자 레지스트리, 잔액, 슬롯, 체크포인트 등 전체 상태",
    fields: "slot, fork, validators, balances, ...",
  },
  {
    name: "BeaconBlock",
    desc: "슬롯, 제안자, 블록 바디 (attestation, deposit 등)",
    fields: "slot, proposer_index, parent_root, body",
  },
  {
    name: "Attestation",
    desc: "검증자의 투표 데이터 (커밋티 비트, 서명)",
    fields: "aggregation_bits, data, signature",
  },
  {
    name: "Validator",
    desc: "검증자 정보 (공개키, 잔액, 활성화/탈출 에폭)",
    fields: "pubkey, withdrawal_credentials, ...",
  },
];

const sszFeatures = [
  {
    feature: "ssz_derive 매크로",
    desc: "#[derive(Encode, Decode, TreeHash)] 자동 구현",
  },
  {
    feature: "BitList / BitVector",
    desc: "가변/고정 길이 비트 컬렉션 (커밋티 투표)",
  },
          ...
          {tab === "fork" && (
            <div>
              ...
              <p className="text-[10px] text-muted-foreground mt-3">
                각 포크는 이전 필드를 상속하고 새 필드를 추가 (예: Electra에서
                committee_bits 추가)
              </p>
            </div>
          )}
