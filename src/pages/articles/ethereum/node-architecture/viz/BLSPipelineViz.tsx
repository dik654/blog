...
const archItems = [
  {
    label: "GenericSecretKey",
    desc: "비밀키 (point: Sec)",
    color: "bg-red-50 dark:bg-red-950/20 border-red-200 dark:border-red-800",
  },
  {
    label: "GenericPublicKey",
    desc: "공개키 (point: Pub)",
    color:
      "bg-blue-50 dark:bg-blue-950/20 border-blue-200 dark:border-blue-800",
  },
  {
    label: "GenericSignature",
    desc: "서명 (point, is_infinity)",
    color:
      "bg-emerald-50 dark:bg-emerald-950/20 border-emerald-200 dark:border-emerald-800",
  },
  {
    label: "GenericAggregateSignature",
    desc: "집계 서명 (여러 서명 합산)",
    color:
...
const aggSteps = [
  ...
  {
    step: "agg.fast_aggregate_verify",
    desc: "공개키 합산 후 단일 pairing 검증",
  },
];
          ...
          {tab === "arch" && (
            <div className="grid grid-cols-2 gap-2 sm:grid-cols-4">
              {archItems.map((a) => (
                <div
                  key={a.label}
                  className={`rounded-lg border p-2.5 ${a.color}`}
                >
