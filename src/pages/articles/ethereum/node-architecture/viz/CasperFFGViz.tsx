...
const STEPS = [
  {
    label: "에폭(32슬롯=6.4분) 끝의 첫 번째 블록이 체크포인트입니다",
    body: "Epoch 290,310 · 슬롯 9,289,920~9,289,951. 활성 검증자 약 1,010,000명.",
  },
  {
    label: "검증자들이 FFG 투표: source=E290310 → target=E290311",
    body: "source=0xa3f1…, target=0xb72e…. 약 680,000명이 1에폭 내 투표.",
  },
  {
    label: "2/3 이상이 투표 → E290311 Justified",
    body: "680,000 / 1,010,000 = 67.3% > 66.7% — supermajority 달성.",
  },
  {
    label: "E290312도 Justified → E290311 최종 확정(Finalized)",
    body: "E290311 finalized — 되돌리려면 1/3(약 10.8M ETH) 슬래싱 필요.",
  },
];
...
function getState(id: string, step: number): EpochState {
  ...
  if (id === "E1") {
    if (step >= 3) return "finalized";
    if (step >= 2) return "justified";
    return "pending";
  }
          ...
          {["E0", "E1", "E2"].map((id, idx) => {
            ...
            return (
              <div key={id} className="flex flex-col items-center gap-2">
                ...
                <motion.div
                  className={`w-20 h-20 rounded-xl border-2 flex flex-col items-center justify-center transition-colors duration-300 ${epochCls(state)}`}
                  animate={{ scale: state === "finalized" ? [1, 1.06, 1] : 1 }}
                ...
                <span className={`text-xs font-medium ${statusColor(state)}`}>
                  {statusLabel(state, id)}
                </span>
              </div>
            );
          })}
