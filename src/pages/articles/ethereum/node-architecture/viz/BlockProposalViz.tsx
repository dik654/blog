...
const STEPS = [
  ...
  {
    label:
      "engine_getPayload: ExecutionPayload + BlobsBundle + BlockValue 수신",
  },
  { label: "BeaconBlock 조립 & BLS 서명 → gossipsub 전파" },
];

const ACTOR_CLR: Record<string, string> = {
  CL: "#6366f1",
  EL: "#f97316",
  VC: "#ec4899",
};
const PIPELINE = [
  {
    ...
    from: "CL",
    color: "#818cf8",
    detail: "epoch 290,312 · reveal = BLS(sk, epoch)",
  },
  {
    label: "forkChoice\nUpdated",
    from: "CL→EL",
    color: "#f59e0b",
    detail: "head=0x3a8f…c1e2 · safe=0x3a8e…b7d0",
  },
  {
    label: "TxPool 선택",
    from: "EL",
    color: "#0ea5e9",
    detail: "154 txs · base_fee=8.2 gwei · max_gas=30M",
  },
  {
    label: "EVM 실행",
    from: "EL",
    color: "#10b981",
    detail: "gas_used=12.4M (41%) · 82 contract calls",
  },
  {
    label: "getPayload",
    from: "CL←EL",
    color: "#f59e0b",
    detail: "block_value=0.0284 ETH · 3 blobs 첨부",
  },
  {
    label: "서명 & 전파",
    from: "CL+VC",
    color: "#ec4899",
    detail: "slot 9,288,001 · BLS sig → gossipsub 전파",
  },
];
          ...
          <div className="flex gap-2 mb-1 justify-center flex-wrap">
            {(["CL", "EL", "VC"] as const).map((a) => (
              <span
                ...
                style={{
                  background: `${ACTOR_CLR[a]}22`,
                  color: ACTOR_CLR[a],
                  border: `1px solid ${ACTOR_CLR[a]}44`,
                }}
              >
                {a}
              </span>
              ...
              <div key={s.label} className="flex items-center gap-2">
                <div className="flex flex-col items-center w-5 shrink-0">
                  ...
                  <motion.div
                    ...
                    style={{
                      background: active || done ? s.color : "var(--muted)",
                    }}
                    animate={{
                      scale: active ? 1.5 : 1,
                ...
                <motion.div
                  className="flex-1 rounded-lg border px-3 py-1.5 flex flex-col gap-0.5"
                  animate={{
