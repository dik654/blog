...
const verifySteps = [
  ...
  {
    step: "verify_blob_kzg_proof_batch",
    desc: "여러 blob을 배치 검증 (random linear combination)",
  },
  {
    step: "PeerDAS Cell 증명",
    desc: "compute_cells_and_kzg_proofs로 DAS 샘플링 지원",
  },
];
          ...
          {tab === "struct" && (
            <div className="grid grid-cols-2 gap-2 sm:grid-cols-4">
              {structs.map((s) => (
                <div
                  key={s.label}
                  className={`rounded-lg border p-2.5 ${s.color}`}
                >
            ...
            <div className="flex flex-col gap-1.5">
              {flowSteps.map((s, i) => (
                <div key={i} className="flex items-start gap-3">
                  <span className="text-[10px] text-muted-foreground w-4 pt-0.5 shrink-0">
                    {i + 1}
                  </span>
