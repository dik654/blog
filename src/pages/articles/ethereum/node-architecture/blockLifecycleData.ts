export const steps = [
  ...
  },
  {
    ...
    actor: "CL → EL",
    detail:
      "engine_forkchoiceUpdated + PayloadAttributes로 EL에 블록 빌딩을 요청합니다.",
  },
  {
    phase: "3. 트랜잭션 실행",
    actor: "EL",
    detail:
      "PayloadBuilder가 풀에서 트랜잭션을 선별, EVM으로 실행, 상태 루트를 병렬 계산합니다.",
  },
  {
    phase: "4. 페이로드 수신",
    actor: "CL ← EL",
  ...
  },
  {
    ...
    actor: "CL → EL",
    detail:
      "engine_newPayload로 다른 노드들이 블록을 검증합니다. EL이 VALID를 반환하면 import.",
  },
  {
    phase: "8. 어테스테이션",
    actor: "CL",
    detail:
      "검증자 위원회가 블록에 투표하고, 포크 초이스 가중치가 업데이트됩니다.",
  },
  {
    phase: "9. 확정 (Finality)",
    actor: "CL",
