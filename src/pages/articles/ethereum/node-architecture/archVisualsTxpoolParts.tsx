    ...
    <div className="space-y-1 text-[10px]">
      <div className="flex items-center justify-between text-[9px]">
        ...
        <span className="rounded border border-amber-300 bg-amber-50/60 dark:bg-amber-950/20 px-1.5 py-0.5 text-amber-700 dark:text-amber-400">
          현재 base fee: <span className="font-bold">15 gwei</span> — 이 미만은
          포함 불가
        </span>
      </div>
      {txs.map((tx, i) => (
        <div
          key={tx.hash}
          className={`flex items-center gap-2 rounded px-2 py-1 border font-mono text-[9px] transition-all duration-300
        ...
        >
          <span
            className={`font-bold w-3 ${i < n ? (tx.ok ? "text-emerald-600" : "text-red-400") : "text-border"}`}
          >
            {i < n ? (tx.ok ? "✓" : "✗") : i + 1}
          </span>
          <span>{tx.hash}</span>
          <span
            className={`ml-auto font-bold ${tx.gwei > 10 ? "text-amber-600" : "text-red-400"}`}
          >
            {tx.gwei} gwei
          </span>
          <span
            className={`text-[8px] text-red-400 ${i < n && !tx.ok ? "" : "invisible"}`}
          >
            {tx.gwei} &lt; 15 → 제외
          </span>
        </div>
  ...
  return (
    <div className="space-y-2 text-[10px]">
      <div className="grid grid-cols-2 gap-2">
        <div>
          <p
            className={`text-[9px] font-bold mb-1 ${phase === 1 ? "text-red-500" : "text-muted-foreground"}`}
          >
          ...
          {pending.map((tx, i) => (
            ...
            >
              <span
                className={
                  phase === 1 && i < 2 ? "text-red-500" : "text-emerald-500"
                }
              >
                {phase === 1 && i < 2 ? "✗" : "✓"}
              </span>
        ...
        <div>
          <p
            className={`text-[9px] font-bold mb-1 ${phase === 2 ? "text-emerald-600" : "text-muted-foreground"}`}
          >
          ...
          {queued.map((tx) => (
            ...
            >
              {tx}
              {phase === 2 && (
                <span className="ml-auto text-[8px] text-emerald-600 font-bold">
                  ↑
                </span>
              )}
            </motion.div>
