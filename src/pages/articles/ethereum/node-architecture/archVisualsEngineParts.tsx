    ...
    <div className="space-y-2 text-[10px]">
      <div className="rounded border px-3 py-2 font-mono space-y-1">
        {txs.map((tx, i) => (
          ...
          >
            <span
              className={`shrink-0 font-bold ${i < done ? "text-emerald-600" : i === done ? "text-amber-500" : "text-border"}`}
            >
              {i < done ? "✓" : i === done ? "⟳" : "○"}
            </span>
      ...
      <div
        className={`flex items-center gap-2 transition-opacity duration-500 ${done === txs.length ? "opacity-100" : "opacity-30"}`}
      >
        <span className="text-foreground/75 font-mono text-[9px]">
          stateRoot:
        </span>
        <span className="font-mono text-blue-600 text-[9px]">0xa3f2…b9</span>
        {done === txs.length && (
          <span className="text-emerald-600 text-[9px]">✓ payload와 일치</span>
        )}
      </div>
    </div>
  );
...
export function EngineResponse() {
  const rows = [
    {
      status: "VALID",
      meaning: "tx 실행 성공 + stateRoot 일치",
      next: "fork_choice 업데이트 진행",
    ...
    {
      status: "INVALID",
      meaning: "tx 실패 또는 stateRoot 불일치",
      next: "블록 무시, 피어 페널티",
      b: "border-red-400",
    ...
    {
      status: "SYNCING",
      meaning: "EL이 아직 동기화 중",
      next: "optimistic 처리 후 재시도",
      b: "border-amber-400",
  ...
  return (
    <div className="space-y-1.5 text-[10px]">
      {rows.map((r) => (
        <div
          key={r.status}
          className={`rounded-lg border-2 ${r.b} ${r.bg} px-3 py-1.5`}
        >
