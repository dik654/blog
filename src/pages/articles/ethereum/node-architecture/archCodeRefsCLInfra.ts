    ...
    annotations: [
      {
        lines: [655, 660],
        color: "sky",
        note: "HTTP POST 요청 빌드 — URL, timeout, Content-Type, JSON body 설정",
      },
      {
        lines: [661, 664],
        color: "amber",
        note: "auth 존재 시 generate_token() 호출 → Authorization: Bearer 헤더 첨부",
      },
      {
        lines: [665, 666],
        color: "emerald",
        note: "요청 전송 · 응답 JSON 파싱 · HTTP 오류 상태 확인",
      },
    ],
    desc: `문제: CL(Lighthouse)과 EL(Reth)은 별도 프로세스로 실행됩니다. Engine API(HTTP JSON-RPC)가 외부에 노출되면 공격자가 가짜 엔진 요청을 보내 EL을 조작할 수 있습니다.
    ...
    annotations: [
      {
        lines: [674, 678],
        color: "sky",
        note: "SubscribeCoreTopics 수신 · 이미 구독 중이면 즉시 반환",
      },
      {
        lines: [679, 693],
        color: "violet",
        note: "shutdown_after_sync 플래그 — 동기화 완료 후 종료 요청 처리",
      },
      {
        lines: [694, 713],
        color: "emerald",
        note: "현재 포크의 core 토픽 목록 구성 · fork_digest별로 gossipsub.subscribe() 호출",
      },
      {
        lines: [714, 720],
        color: "amber",
        note: "subscribe_all_subnets 옵션 — 전체 어테스테이션 서브넷 구독 (연구/테스트용)",
      },
    ],
    desc: `문제: 비콘 체인의 블록·어테스테이션 같은 메시지를 모든 노드에 직접 보내면 O(n²) 트래픽이 발생합니다. 수천 개의 노드가 있는 네트워크에서는 불가능합니다.
    ...
    annotations: [
      {
        lines: [52, 73],
        color: "sky",
        note: "HotColdDB 구조체 — split(경계 슬롯), hot_db, cold_db, blobs_db 핵심 필드",
      },
      {
        lines: [74, 89],
        color: "emerald",
        note: "캐시 레이어 — block_cache(LRU), state_cache, historic_state_cache(API용)",
      },
      {
        lines: [108, 115],
        color: "amber",
        note: "BlockCache.put_block — 인메모리 LRU 캐시에 블록 삽입 (최근 N개만 유지)",
      },
    ],
    desc: `문제: 비콘 체인의 전체 히스토리는 수백 GB에 달합니다. 모든 데이터를 동일한 방식으로 저장하면 최근 블록 조회도 느려지고 저장 공간도 비효율적으로 낭비됩니다.
    ...
    annotations: [
      {
        lines: [600, 605],
        color: "sky",
        note: "update_sync_state 진입 · range_sync.state() 조회 — 오류면 crit 로그 후 반환",
      },
      {
        lines: [606, 633],
        color: "emerald",
        note: "RangeSync 없을 때 — 헤드와 현재 슬롯 차이, 앞선 피어 유무로 상태 판정",
      },
      {
        lines: [634, 660],
        color: "amber",
        note: "Backfill sync 확인 · Synced → BackFillSyncing 전환 · 네트워크 글로벌 상태 갱신",
      },
    ],
    desc: `문제: 노드가 재시작하거나 오프라인 상태였을 때 체인 헤드까지 따라잡아야 합니다. 뒤처진 정도에 따라 최적 전략이 다릅니다.
