    ...
    annotations: [
      {
        lines: [3459, 3464],
        color: "sky",
        note: "insert_block 진입 · 블록 번호 추출 (이후 body indices 조회에 사용)",
      },
      {
        lines: [3465, 3481],
        color: "emerald",
        note: "ExecutedBlock 래핑 (영수증·상태 비어있음) · save_blocks(BlocksOnly) 위임",
      },
      {
        lines: [3482, 3490],
        color: "amber",
        note: "block_body_indices 조회 · 없으면 BlockBodyIndicesNotFound 오류 반환",
      },
    ],
    desc: `문제: 블록 헤더·트랜잭션·영수증·상태를 빠르게 저장하고 O(log n) 시간에 조회해야 합니다. 일반 파일 I/O는 너무 느립니다.
    ...
    annotations: [
      {
        lines: [116, 139],
        color: "sky",
        note: "RpcModuleBuilder 구조체 — Provider, Pool, Network, EvmConfig 등 의존성 필드",
      },
      {
        lines: [140, 175],
        color: "emerald",
        note: "의존성 주입 메서드 체인 — with_provider, with_pool 등으로 빌더 패턴 구성",
      },
    ],
    desc: `문제: eth_getBalance, eth_call, eth_sendRawTransaction, debug_traceTransaction 등 수십 개의 RPC 메서드를 체계적으로 등록하고 HTTP/WebSocket 두 트랜스포트로 동시에 서빙해야 합니다.
    ...
    annotations: [
      {
        lines: [11, 17],
        color: "sky",
        note: "EthCall impl — eth_call, eth_estimateGas RPC 핸들러. 본문 없음 = 트레이트 기본 구현 사용",
      },
      {
        lines: [19, 38],
        color: "emerald",
        note: "Call impl — call_gas_limit, max_simulate_blocks, evm_memory_limit 설정 위임",
      },
      {
        lines: [41, 47],
        color: "amber",
        note: "EstimateCall impl — 가스 이진 탐색 로직. 본문 없음 = 트레이트 기본 구현 사용",
      },
    ],
    desc: `문제: eth_call 은 트랜잭션을 실제로 전송하지 않고 EVM 실행 결과(반환값, 가스 소비량)만 확인하고 싶을 때 사용합니다. EVM을 실행하면 잔액·스토리지가 변경되므로 상태를 격리해야 합니다.
