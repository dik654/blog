...
export {
  EngineTreeStateTrie,
  EngineTreePersist,
} from "./archVisualsEngineTreeParts";
...
export function EngineTreeReceive() {
  return (
    <div className="space-y-1.5 text-[10px] font-mono">
      ...
      <p className="text-[9px] text-foreground/75 font-sans">
        ↓ on_new_payload() 핸들러 진입
      </p>
    </div>
  );
}
  ...
  return (
    <div className="space-y-1 text-[10px]">
      <p className="font-mono text-[9px] text-foreground/75">
        PayloadProcessor: tx 순차 실행
      </p>
      {txs.map((tx, i) => (
        <div
          key={tx}
          className={`flex items-center gap-2 font-mono text-[9px] transition-colors duration-300
          ${i < done ? "text-foreground" : "text-muted-foreground/40"}`}
        >
          <span
            className={`font-bold ${i < done ? "text-emerald-600" : i === done ? "text-amber-500" : "text-border"}`}
          >
            {i < done ? "✓" : i === done ? "⟳" : "○"}
          </span>
          <span>{tx}</span>
  ...
  return (
    <div className="space-y-1.5 text-[10px]">
      <div className="rounded border border-emerald-300 bg-emerald-50/60 dark:bg-emerald-950/20 px-3 py-2 font-mono">
        ...
        <p className="text-[9px] text-foreground/75 font-sans">
          latestValidHash: 0xa3f2…
        </p>
      </div>
      <p className="text-[9px] text-foreground/75">
        ↑ CL로 반환 → fork_choice 업데이트 대기
      </p>
    </div>
  );
}
