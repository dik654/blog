    ...
    <div className="space-y-1.5 text-[10px]">
      ...
      <div className="flex items-center gap-2">
        ...
        <div className="flex-1 rounded border border-emerald-300 bg-emerald-50/60 dark:bg-emerald-950/20 px-2 py-1.5 font-mono text-[9px]">
          ...
          <p className="text-muted-foreground font-sans">put_block() · put_state()</p>
        </div>
      </div>
    </div>
  );
      ...
      <div className="flex items-center gap-0.5">
        {slots.map((s, i) => (
          <div key={s} className={`flex-1 rounded border text-center py-1 font-mono text-[8px]
            ${i === slots.length - 1 ? 'border-emerald-400 bg-emerald-50/60 dark:bg-emerald-950/20 font-bold' : 'border-border bg-muted/20'}`}>
            {s}
          </div>
        ))}
      </div>
    ...
    <div className="space-y-1.5 text-[10px]">
      ...
      <div className="flex items-center gap-2">
        <div className={`rounded border-2 px-2 py-1.5 text-center shrink-0 text-[9px] transition-all duration-500
          ${phase === 1 ? 'border-amber-300 bg-amber-50/60 dark:bg-amber-950/20' : 'border-border bg-muted/20'}`}>
          <p className="font-bold">Hot DB</p>
          <p className="font-mono text-[8px] text-muted-foreground">finalized</p>
        </div>
        ...
        <div className={`flex-1 rounded border-2 px-2 py-1.5 text-center text-[9px] transition-all duration-500
          ${phase === 2 ? 'border-purple-300 bg-purple-50/60 dark:bg-purple-950/20' : 'border-border bg-muted/20'}`}>
          <p className={`font-bold ${phase === 2 ? 'text-purple-700 dark:text-purple-400' : ''}`}>Cold Freezer</p>
          <p className="text-[8px] text-muted-foreground">불변 아카이브</p>
        </div>
      </div>
    </div>
  );
    ...
    <div className="space-y-1.5 text-[10px]">
      ...
      <div className="rounded border border-amber-300 bg-amber-50/60 dark:bg-amber-950/20 px-3 py-2">
        <div className="flex items-center justify-between mb-1.5">
          ...
          <span className="text-[9px] font-mono text-muted-foreground">TTL: 4,096 slots ≈ 18일</span>
        </div>
        ...
        <p className="text-[9px] text-muted-foreground mt-1.5">TTL 만료 후 삭제 (beacon state는 유지)</p>
      </div>
    </div>
  );
}
