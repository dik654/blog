    ...
    annotations: [
      {
        lines: [1516, 1529],
        color: "sky",
        note: "함수 진입 · 타이머 시작 · 현재 슬롯/epoch 계산",
      },
      {
        lines: [1530, 1565],
        color: "emerald",
        note: "비콘 노드에 GET /validator/duties/proposer/{epoch} 조회 (first_success 폴백)",
      },
      {
        lines: [1566, 1615],
        color: "amber",
        note: "응답 파싱 · 내 pubkey만 필터 · proposers 맵에 캐싱 · 추가 알림 발송",
      },
    ],
    desc: `문제: 수백 명의 검증자가 매 epoch마다 어느 슬롯에 블록을 제안해야 하는지 알아야 합니다. 매번 상태 트리 전체를 읽어 계산하면 너무 느립니다.
    ...
    annotations: [
      {
        lines: [99, 104],
        color: "sky",
        note: "sign_block 트레이트 메서드 — 슬래싱 보호 포함 BLS 서명의 인터페이스 정의",
      },
    ],
    desc: `문제: 같은 검증자 키가 실수로 두 노드에서 동시에 실행되면 동일 슬롯에 서로 다른 블록에 서명하게 됩니다. 이를 이중 서명(Double Signing)이라 하며 탐지 시 스테이킹한 ETH 일부가 강제 소각되는 슬래싱 패널티를 받습니다.
    ...
    annotations: [
      {
        lines: [1145, 1152],
        color: "sky",
        note: "tracing 계측 어노테이션 — 분산 추적 span 생성, 부모 없이 독립 실행",
      },
      {
        lines: [1153, 1162],
        color: "emerald",
        note: "process_gossip_block 함수 진입 · 블록 루트는 아직 미계산(field::Empty)",
      },
      {
        lines: [1163, 1193],
        color: "amber",
        note: "unverified_block 1차 검증 → duplicate_cache로 중복 방지 → verified_block 처리",
      },
    ],
    desc: `문제: P2P 네트워크를 통해 gossip으로 수신한 블록은 중복일 수도 있고, 이미 finalized(확정)된 과거 블록일 수도 있습니다. 이런 블록을 검증 없이 처리하면 CPU와 I/O가 낭비됩니다.
    ...
    annotations: [
      {
        lines: [3348, 3368],
        color: "sky",
        note: "함수 시그니처 · seen_timestamp 기록 · Prometheus 카운터 증가",
      },
      {
        lines: [3369, 3395],
        color: "emerald",
        note: "into_execution_pending_block — BLS 배치 검증 + 상태 전이 준비, publish_fn 호출",
      },
      {
        lines: [3396, 3415],
        color: "amber",
        note: "into_executed_block — EL에 엔진 페이로드 전달, 실패 시 DA 캐시에서 제거",
      },
    ],
    desc: `문제: gossip으로 받은 블록의 유효성을 어떻게 효율적으로 검증하고 저장할까요? 어테스테이션·제안자 서명을 하나씩 검증하면 너무 느리고, EVM 실행까지 직렬로 수행하면 다음 블록 처리가 지연됩니다.
