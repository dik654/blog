    ...
    <div className="flex items-center gap-2 text-[10px]">
      ...
      <div className="flex-1 rounded-lg border-2 border-border px-2 py-1.5">
        ...
        {txs.map((tx, i) => (
          <div
            key={tx}
            className={`font-mono text-[9px] rounded px-1 py-0.5 mb-0.5 transition-all duration-400
  ...
  return (
    <div className="space-y-2 text-[10px]">
      <div className="rounded border px-3 py-2 font-mono text-[9px]">
        ...
        <span className="">)</span>
        <span className="text-foreground/75 ml-2 text-[8px]">
          // EIP-1559 서명 tx
        </span>
      </div>
      <div className="flex items-center flex-wrap gap-1">
        {steps.map((s, i) => (
          <span key={s} className="flex items-center gap-1">
            <span
              className={`rounded px-1.5 py-0.5 border text-[9px] font-medium
              ${
                i === steps.length - 1
            ...
            {i < steps.length - 1 && (
              <span className="text-foreground/75 text-[9px]">→</span>
            )}
          </span>
        ))}
      </div>
