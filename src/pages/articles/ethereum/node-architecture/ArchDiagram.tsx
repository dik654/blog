...
export default function ArchDiagram() {
  ...
  const conn = sel
    ? new Set(modules[sel]?.links.map((l) => l.target) ?? [])
    : new Set<string>();
  const selLayer = sel ? modules[sel]?.layer : null;
  const engineHL =
    sel !== null &&
    (conn.has("engine") || sel === "engine" || flowIds.has("engine"));
  const handleFlow = useCallback(
    (ids: string[]) => setFlowIds(new Set(ids)),
    [],
  );
  ...
  const lm = (from: string, to: string) =>
    modules[from]?.links.find((l) => l.target === to)?.msgs;
  const np = (id: string) => (
    <Node
      id={id}
      sel={sel}
  ...
  return (
    <div className="not-prose space-y-3">
        ...
        <div className="overflow-x-auto">
          <div className="grid grid-cols-[minmax(130px,1fr)_auto_minmax(130px,1fr)_auto_minmax(130px,1fr)] min-w-[500px]">
            ...
            <HArrow
              label="Beacon API"
              dir="→"
              animated={fa("validator", "beacon")}
              msgs={lm("validator", "beacon")}
            />
            {np("beacon")}
            <HArrow
              label="직접 호출"
              dir="↔"
              animated={fa("beacon", "hotcold")}
              msgs={lm("beacon", "hotcold")}
            />
            {np("hotcold")}
            <div />
            <div />
            <VConn
              label="channel"
              animated={fa("beacon", "sync") || fa("sync", "beacon")}
              msgs={lm("beacon", "sync")}
            />
            <div />
            <div />
      ...
      {/* Engine API */}
      <div
        className={`rounded-xl border-2 border-dashed p-3 transition-colors ${engineHL ? "border-amber-500 bg-amber-50/40 dark:bg-amber-950/10" : "border-amber-400"}`}
      >
        <div className="flex items-center gap-2">
          <div
            className={`flex-1 border-t-2 border-dashed transition-colors ${engineHL ? "border-amber-500" : "border-amber-300"}`}
          />
          <button
      ...
      <CodeSidebar
        ...
        projectMetas={{
          lighthouse: {
            id: "lighthouse",
            label: "Lighthouse · CL",
            badgeClass: "bg-[#ddf4ff] border-[#54aeff] text-[#0550ae]",
          },
          reth: {
            id: "reth",
            label: "Reth · EL",
            badgeClass: "bg-[#fff3cd] border-[#d4a72c] text-[#7d4e00]",
          },
        }}
        flowData={flowData}
