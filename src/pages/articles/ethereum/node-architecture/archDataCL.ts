...
export const clModules: Record<string, Module> = {
    ...
    links: [
      {
        ...
        dir: "→",
        msgs: [
          "GET /validator/duties/proposer → ProposerDuties[]",
          "GET /validator/attestation_data → AttestationData",
          "POST /beacon/blocks ← SignedBeaconBlock",
        ],
      },
    ],
  },
  beacon: {
    ...
    fns: [
      ...
      {
        sig: "process_attestation()",
        desc: "OperationPool에 어테스테이션 추가·집계",
      },
    ],
    links: [
      { target: "validator", via: "ValidatorAPI", dir: "←" },
      {
        ...
        dir: "↔",
        msgs: [
          "put_block(SignedBeaconBlock) →",
          "get_state(slot) → BeaconState",
          "freeze_to_cold(finalized_blocks) →",
        ],
      },
      {
        target: "sync",
        ...
        dir: "↔",
        msgs: [
          "SyncMessage::AddPeer(peer_id, sync_info)",
          "SyncMessage::BlocksProcessed([blocks])",
          "ProcessBlock(signed_block) →",
        ],
      },
  ...
  hotcold: {
    ...
    links: [{ target: "beacon", via: "직접 호출", dir: "↔" }],
  },
  sync: {
    label: "SyncManager",
    layer: "cl",
    ...
    fns: [
      ...
      {
        sig: "process_sync_message()",
        desc: "Range/Backfill/Lookup 동기화 상태 전환",
      },
      {
        sig: "range_sync_block()",
        desc: "피어에서 배치 블록 요청 후 BeaconChain 제출",
      },
    ],
    links: [
      {
        ...
        dir: "↔",
        msgs: [
          "BlocksByRangeReq { start_slot, count } →",
          "BlocksByRangeResp([SignedBeaconBlock]) ←",
          "BlobsByRangeReq / BlobsByRangeResp",
        ],
      },
  ...
  libp2p: {
    ...
    fns: [
      {
        sig: "publish(topic, msg)",
        desc: "Gossipsub 토픽으로 블록·어테스테이션 전파",
      },
      { sig: "send_request(peer)", desc: "Req/Resp 프로토콜로 블록 개별 요청" },
      { sig: "discover_peers()", desc: "Discv5 ENR 탐색으로 새 피어 발견" },
    ],
    links: [{ target: "sync", via: "tokio channel", dir: "↔" }],
  },
  engine: {
    label: "Engine API (JWT)",
    layer: "api",
    role: "CL ↔ EL 실행 페이로드 교환 채널",
    fns: [
      {
        sig: "engine_newPayloadV3(payload)",
        desc: "CL→EL: 실행 페이로드 전달·검증·EVM 실행 요청",
      },
      {
        sig: "engine_forkchoiceUpdatedV3(fcu, attrs?)",
        desc: "CL→EL: canonical head 갱신, attrs 있으면 payload 빌드 시작",
      },
      {
        sig: "engine_getPayloadV4(payload_id)",
        desc: "CL←EL: 빌드 완료된 페이로드 + EIP-4844 블롭 수신",
      },
    ],
    links: [
