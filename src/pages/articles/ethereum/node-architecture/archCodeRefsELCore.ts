    ...
    annotations: [
      {
        lines: [592, 629],
        color: "sky",
        note: "함수 진입 · 타이밍 시작 · 페이로드 레이아웃 검증 (가스 한도, 버전드 해시 등)",
      },
      {
        lines: [630, 645],
        color: "emerald",
        note: "잘못된 조상 블록 확인 · backfill 상태에 따라 insert/buffer 분기",
      },
      {
        lines: [646, 655],
        color: "amber",
        note: "처리 결과 확인 · sync 타깃 헤드면 canonical 체인에 등록",
      },
    ],
    desc: `문제: CL이 보낸 실행 페이로드를 EVM으로 실행하고 stateRoot를 검증한 뒤 디스크에 저장하는 작업이 모두 완료될 때까지 CL에 응답을 못 보내면 다음 블록 처리가 지연됩니다.
    ...
    annotations: [
      {
        lines: [218, 222],
        color: "rose",
        note: "Status 메시지 거부 — 핸드셰이크 완료 후 재수신은 프로토콜 위반(BadMessage)",
      },
      {
        lines: [223, 241],
        color: "sky",
        note: "블록 관련 브로드캐스트 — NewBlockHashes, NewBlock, 트랜잭션 해시/데이터",
      },
      {
        lines: [242, 260],
        color: "emerald",
        note: "요청/응답 페어 — GetBlockHeaders ↔ BlockHeaders, GetBlockBodies ↔ BlockBodies",
      },
    ],
    desc: `문제: 이더리움 DevP2P 프로토콜에서 두 노드가 처음 연결되면 Status 메시지로 체인 정보를 교환하는 핸드셰이크를 수행합니다. 이 과정에서 다른 chainId이거나 다른 genesis 해시를 가진 노드는 차단해야 합니다.
    ...
    annotations: [
      {
        lines: [578, 589],
        color: "sky",
        note: "함수 진입 · 초기 동기화 중이거나 tx gossip 비활성화면 즉시 반환",
      },
      {
        lines: [590, 627],
        color: "emerald",
        note: "피어 세션 조회 · 중복 해시 카운트 · seen_transactions 집합 갱신",
      },
      {
        lines: [628, 640],
        color: "amber",
        note: "이미 본 해시 지표 기록 · report_already_seen으로 피어 평판 감점",
      },
    ],
    desc: `문제: 트랜잭션 전체(수 KB)를 먼저 전송하면 이미 가지고 있는 tx를 중복 수신합니다. 인기 있는 tx는 수십 개 피어에서 동시에 오기 때문에 대역폭이 크게 낭비됩니다.
    ...
    annotations: [
      {
        lines: [23, 37],
        color: "sky",
        note: "EthTransactions impl 시작 — signers, send_raw_transaction_sync_timeout 위임",
      },
      {
        lines: [39, 80],
        color: "emerald",
        note: "send_transaction — WithEncoded<Recovered<PoolPooledTx>> 수신 · blob 사이드카 업캐스팅 처리",
      },
    ],
    desc: `문제: 사용자가 eth_sendRawTransaction 으로 서명된 트랜잭션을 제출할 때, RLP 인코딩 오류·서명 불량·nonce 불일치·잔액 부족 등 다양한 오류가 있을 수 있습니다.
