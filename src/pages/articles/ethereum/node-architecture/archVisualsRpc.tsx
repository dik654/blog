...
export function RpcParse() {
  return (
    <div className="rounded border bg-muted/40 px-3 py-2 font-mono text-[10px] space-y-0.5">
      <p className="text-[9px] text-muted-foreground">HTTP POST · application/json</p>
      <div className="border-t border-border/50 pt-1 mt-1 space-y-0.5">
        <p><span className="text-muted-foreground">method: </span><span className="text-blue-600 font-bold">eth_call</span></p>
        <p><span className="text-muted-foreground">params: </span><span className="text-amber-600">[{'{to, data}'}, "latest"]</span></p>
        <p><span className="text-muted-foreground">id: </span><span>1</span></p>
      </div>
    </div>
  );
}
...
export function RpcStorageLookup() {
  ...
  return (
    <div className="space-y-1 text-[10px]">
      <p className="font-mono text-[9px] text-muted-foreground">상태 조회 순서 (찾으면 즉시 반환)</p>
      {tiers.map((t, i) => (
        <div key={t.label} className={`flex items-center gap-2 rounded px-2 py-1 border-2 transition-all duration-400
          ${i === active ? t.cls : 'border-border bg-muted/10 opacity-30'}`}>
          <span className="font-bold text-[9px]">{i + 1}</span>
          <span className="text-[9px]">{t.label}</span>
          <span className="text-[8px] text-muted-foreground ml-auto">{t.note}</span>
  ...
  return (
    <div className="space-y-1.5 text-[10px]">
      <div className="rounded border border-blue-300 bg-blue-50/60 dark:bg-blue-950/20 px-3 py-2 text-[9px]">
        ...
        <p className="text-muted-foreground mt-0.5">실제 tx를 블록에 포함시키지 않으므로 블록체인 상태가 바뀌면 안 됨</p>
      </div>
...
export function RpcResponse() {
  return (
    <div className="rounded border bg-muted/40 px-3 py-2 font-mono text-[10px] space-y-0.5">
      <p className="text-[9px] text-muted-foreground">HTTP 200 OK · application/json</p>
      <div className="border-t border-border/50 pt-1 mt-1 space-y-0.5">
        <p><span className="text-muted-foreground">id: </span><span>1</span></p>
        <p><span className="text-muted-foreground">jsonrpc: </span><span>"2.0"</span></p>
        <p><span className="text-muted-foreground">result: </span><span className="text-emerald-600 font-bold">"0x0000…1234"</span></p>
      </div>
    </div>
  );
}
