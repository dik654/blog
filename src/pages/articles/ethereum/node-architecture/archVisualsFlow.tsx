    ...
    <div className="flex items-center justify-between gap-3 text-[10px]">
      ...
      <div className="flex flex-col items-center gap-1">
        ...
        <div className="rounded bg-amber-50 dark:bg-amber-950/30 border border-amber-300 px-2 py-0.5">
          <p className="text-[9px] font-mono text-amber-700 dark:text-amber-400">
            gap: 2,270 slots
          </p>
        </div>
      </div>
    ...
    <div className="space-y-1.5 text-[10px] font-mono">
      {[
        ...
        <div key={range} className="flex items-center gap-2">
          <span
            className={`rounded px-2 py-0.5 border text-[9px] shrink-0 ${
              done
  ...
  return (
    <div className="flex items-center gap-1 flex-wrap text-[10px]">
      {[45231, 45232, 45233, 45234, 45235].map((slot) => (
        <div
          key={slot}
          className="rounded border border-emerald-400 bg-emerald-50/60 dark:bg-emerald-950/20 px-1.5 py-0.5 font-mono text-[9px]"
        >
          {slot}
        </div>
      ))}
    ...
    <div className="space-y-2 text-[10px]">
      ...
      <div className="flex items-center gap-1">
        {[8997, 8998, 8999, 9000].map((slot) => (
          <div
            key={slot}
            className="rounded border border-amber-300 bg-amber-50/60 dark:bg-amber-950/20 px-1.5 py-0.5 font-mono text-[9px]"
          >
            {slot}
          </div>
        ))}
        <span className="text-foreground/75 mx-1">···</span>
        <div className="rounded border-2 border-foreground/40 px-2 py-0.5 font-bold text-[9px]">
          HEAD
        </div>
      </div>
    </div>
