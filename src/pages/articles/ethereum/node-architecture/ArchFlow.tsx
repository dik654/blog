...
export default function ArchFlow({
  ...
  }, [flow, onStepModules, step]);
  if (!flow) return null;

  const total = flow.steps.length;
  const go = (n: number) =>
    setStep((s) => Math.max(0, Math.min(total - 1, s + n)));

  return (
    <div className="space-y-3">
      <div className="flex items-center gap-1">
        <p className="text-[11px] font-semibold text-foreground/60 shrink-0 mr-1">
          {flow.title}
        </p>
        <ArchFlowStepTrack
          moduleId={moduleId}
      ...
      <div className="min-h-[130px]">
        <AnimatePresence mode="wait">
          ...
          >
            {(() => {
              const V = stepVisuals[`${moduleId}-${step}`];
              return V ? (
                <>
                  <V />
            ...
            <div className="flex flex-wrap gap-1.5 items-center">
              {flow.steps[step].modules.map((id, i, arr) => (
                <span key={id} className="flex items-center gap-1">
                  <span
                    className={`text-[10px] font-medium px-2 py-0.5 rounded-full border ${badge(id)}`}
                  >
                    {modules[id]?.label ?? id}
                  </span>
                  {i < arr.length - 1 && (
                    <span className="text-foreground/75 text-xs">→</span>
                  )}
                </span>
              ))}
            </div>
      ...
      <div className="flex items-center gap-2">
        ...
        <span className="text-[10px] text-foreground/75 shrink-0 tabular-nums w-10 text-center">
          {step + 1} / {total}
        </span>
        <button
          onClick={() => go(1)}
