  ...
  return (
    <div className="space-y-1.5 text-[10px]">
      {TIERS.map((t, i) => (
        <div
          key={t.label}
          className={`rounded-lg border-2 p-2 transition-all ${i === activeIdx ? t.active + " shadow-sm" : "border-border opacity-40"}`}
        >
      ...
      <div className="flex items-center gap-1 flex-wrap">
        ...
        {TIERS.map((t, i) => (
          <span key={t.label} className="flex items-center gap-1">
            <span
              className={`rounded px-1.5 py-0.5 border-2 font-bold ${t.active}`}
            >
              {i + 1}
            </span>
        ...
        <span className="text-foreground/75 ml-1 text-[9px]">
          (찾으면 즉시 반환)
        </span>
      </div>
    </div>
