    ...
    <div className="space-y-1.5 text-[10px]">
      ...
      <div className="rounded border px-3 py-2 space-y-1.5 font-mono text-[9px]">
        <div className="flex items-center gap-2">
          <span className="text-foreground/75 shrink-0">
            이번 슬롯 제안자 주소
          </span>
...
export function ValidatorPayloadBuild() {
  ...
  const steps = [
    {
      label: "engine_forkchoiceUpdatedV3(attrs)",
      color: "text-amber-600",
      note: 'EL에게 "페이로드 빌드 시작" 신호 + 목표 head 전달',
    },
    {
      label: "TxPool에서 가스비 높은 순으로 tx 선별",
      color: "text-blue-600",
      note: "블록에 넣을 트랜잭션 모음 구성",
    },
    {
      label: "engine_getPayloadV4(id) → payload",
  ...
  return (
    <div className="space-y-1.5 text-[10px]">
      {steps.map((s, i) => (
        <div
          key={s.label}
          className={`flex items-center gap-2 rounded px-2 py-1 border transition-all duration-300
