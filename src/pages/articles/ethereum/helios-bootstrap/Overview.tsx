    ...
    <section id="overview" className="mb-16 scroll-mt-20">
      ...
      <div className="prose prose-neutral max-w-none dark:prose-invert">
        <p className="text-lg leading-8">
          처음 실행한 Helios에는 “어느 beacon header가 진짜인가”를 판단할 기준이 없습니다. 그래서 사용자가 신뢰한 최근 finalized checkpoint root를
          하나 받고 서버가 돌려준 header가 정확히 그 root인지 확인한 뒤, 그 state에 포함된 current sync committee만 첫 store에 넣습니다. 이 순서가
          바뀌면 공격자가 만든 committee로 공격자의 서명을 검증하는 순환 논리가 됩니다.
        </p>
        <p>
      ...
      <div className="prose prose-neutral max-w-none dark:prose-invert">
        ...
        <p>
          Network identity와 checkpoint provenance가 맞고 응답 header의 hash-tree-root가 checkpoint와 같으며 current
          committee의 Merkle branch가 그 header의 state root까지 재계산될 때만 store를 만듭니다. HTTP 200이나 JSON/SSZ decode 성공은
          구조를 읽었다는 뜻일 뿐 신뢰가 전달됐다는 뜻이 아닙니다.
        </p>
      </div>
    </section>
  );
