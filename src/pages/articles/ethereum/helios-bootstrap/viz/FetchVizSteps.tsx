    ...
    <g>
      ...
      >
        <DataBox
          ...
          h={38}
          label="header"
          sub="slot=8000000, state_root=0xab12.."
          color={C.http}
        />
      </motion.g>
      ...
      >
        <ModuleBox
          ...
          w={280}
          h={38}
          label="current_sync_committee"
          sub="512 pubkeys (24KB)"
          color={C.parse}
        />
      </motion.g>
  ...
  return (
    <g>
      <text
        ...
        fill={C.merkle}
      >
        Merkle 경로: committee → state_root
      </text>
      ...
      <DataBox
        ...
        w={150}
        h={24}
        label="committee_root"
        sub="hash(512 pubkeys)"
        color={C.parse}
      />
      ...
      {levels.map((lv, i) => {
        ...
        return (
          ...
          >
            {/* 경로 노드 — 해시 연산 */}
            <circle cx={pathX} cy={lv.y} r={8} fill={C.merkle} opacity={0.12} />
            <circle
              cx={pathX}
              cy={lv.y}
              r={8}
            ...
            {/* 연결선: branch → 경로 노드 */}
            <line
              x1={lv.side === "L" ? bx + 38 : bx - 38}
              y1={lv.y}
              x2={lv.side === "L" ? pathX - 8 : pathX + 8}
              y2={lv.y}
              stroke={C.merkle}
              strokeWidth={0.5}
              opacity={0.5}
      ...
      <text
        ...
        fill="var(--muted-foreground)"
      >
        index=22의 비트 패턴(10110)이 각 depth에서 좌/우를 결정 → 5개 해시로
        루트 재구성
      </text>
    </g>
  );
