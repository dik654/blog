...
export const codeRefs: Record<string, CodeRef> = {
  "chainspec-struct": {
    ...
    lang: "rust",
    highlight: [3, 15],
    desc: "ChainSpec의 책임을 보여주는 축약 snapshot. 실제 현재 field와 trait surface는 Reth source가 기준이다.",
    annotations: [
      { lines: [5, 7], color: "sky", note: "chain + genesis_header + genesis" },
      {
        lines: [10, 10],
        color: "emerald",
        note: "ChainHardforks — BTreeMap 기반 하드포크",
      },
  ...
  "fork-condition": {
    path: "crates/ethereum-forks/src/hardfork.rs",
    lang: "rust",
    highlight: [19, 29],
    desc: "ForkCondition의 여러 activation 축을 보여주는 축약 snapshot.",
    annotations: [
      {
        lines: [21, 21],
        color: "sky",
        note: "Block — 블록 번호 (Frontier~Istanbul)",
      },
      {
        lines: [22, 25],
        color: "amber",
        note: "TTD — 누적 난이도 (The Merge)",
      },
  ...
  "eth-chainspec-trait": {
    ...
    lang: "rust",
    highlight: [33, 43],
    desc: "체인 설정을 소비하는 공통 query boundary의 축약 snapshot.",
    annotations: [
      {
        lines: [36, 37],
        color: "sky",
        note: "chain() + chain_id() — 체인 식별",
      },
  ...
  "mainnet-spec": {
    ...
    lang: "rust",
    highlight: [3, 27],
    desc: "built-in mainnet ChainSpec 초기화의 축약 snapshot.",
    annotations: [
      {
        lines: [5, 7],
        color: "sky",
        note: "include_str! — 컴파일 타임에 json 임베딩",
      },
  ...
  "make-genesis": {
    ...
    lang: "rust",
    highlight: [30, 50],
    desc: "genesis context에서 조건부 header fields와 state root를 구성하는 축약 snapshot.",
    annotations: [
      { lines: [35, 38], color: "sky", note: "London 활성 → base_fee 초기값" },
      {
        lines: [40, 43],
        color: "emerald",
        note: "Shanghai 활성 → withdrawals root",
      },
