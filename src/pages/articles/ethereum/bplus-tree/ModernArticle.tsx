  ...
  return <article className="space-y-14">
    <section id="overview" className="space-y-5">
      ...
      <p className="text-lg leading-8">
            100만 개의 정렬된 key를 한 줄 배열에 두면 이진 탐색의 비교 횟수는 작아도 서로 떨어진 disk page를 여러 번 읽을 수 있습니다. B+ tree는 한 page에
            많은 separator와 child pointer를 넣어 tree를 낮게 만들고 실제 record나 record pointer는 leaf에 모읍니다.
          </p>
      <PageFlow />
      <p>
            B+ tree는 LSM의 append·compaction write path나 MDBX의 transaction durability, MPT의 cryptographic root가
            아닙니다. 이 글이 소유하는 범위는 page-oriented ordered index의 fanout·search·split/merge invariant이며, crash
            atomicity는 이를 채택하는 storage engine이 별도로 정의합니다.
          </p>
    </section>
    ...
    <section id="page-layout" className="space-y-5">
      ...
      <p>
            Internal page에는 header, slot directory, separator key, child page number가 들어가고 leaf에는 ordered key와
            value 또는 record pointer, sibling link가 들어갑니다. “key가 16 bytes니까 4 KiB에 256개”라고 나누면
            header·pointer·fragmentation을 빠뜨립니다.
          </p>
      <ExplainedFormula
      ...
      />
      <p>
            최소 fanout이 m이면 depth h가 가리킬 수 있는 leaf 수가 적어도 m의 h제곱으로 늘어나므로 높이는 로그에 비례합니다. 다만 root는 최소 occupancy
            예외이고 variable-length key와 낮은 fill factor는 실제 fanout을 줄입니다.
          </p>
    </section>
    ...
    <section id="search-range" className="space-y-5">
      ...
      <p>
            Internal separators가 [20, 40]이면 key 35는 가운데 child로 내려갑니다. 도착한 leaf에서 lower bound를 찾고 정확한 key와
            duplicate policy를 확인합니다. 이때 separator 자체를 record라고 해석하면 안 됩니다.
          </p>
      <p>
            Range [35, 62]는 35가 있을 첫 leaf까지 root-to-leaf 탐색을 한 번 수행한 뒤 right-sibling link를 따라 62를 넘을 때까지 걷습니다.
            비용은 대략 tree height와 결과가 걸친 leaf pages의 합입니다. build와 lookup은 comparator, collation, null ordering과
            endpoint의 inclusive/exclusive 규칙을 똑같이 써야 합니다. 둘이 다르면 존재하는 key가 도달 불가능하거나 range 순서가 어긋납니다.
          </p>
    </section>
    ...
    <section id="updates" className="space-y-5">
      ...
      <p>
            Leaf capacity가 4인 [10,20,30,40]에 25를 삽입하면 먼저 [10,20,25,30,40]으로 정렬한 뒤 [10,20]과 [25,30,40]으로 나누고
            parent에 새 right leaf의 경계 25를 올립니다. 그 parent도 overflow하면 같은 작업이 위로 전파되며 root가 split될 때만 모든 leaf
            path가 함께 한 단계 길어집니다.
          </p>
      <p>
            삭제로 minimum occupancy 아래가 되면 sibling에서 한 entry를 재분배하고 parent separator를 고치거나 재분배할 여유가 없으면 pages를
            merge합니다. 남은 root child가 하나뿐이면 root를 줄일 수 있습니다. 이 과정이 보존해야 하는 핵심은 global key ordering, root를 제외한
            occupancy bound, 모든 leaf의 동일 depth입니다. Leaf만 나누고 parent link를 publish하지 않는 구현은 새 page의 keys를 영구히
            잃는 반례입니다.
          </p>
    </section>
    ...
    <section id="release" className="space-y-5">
      ...
      <p>
            Empty/root-only tree, capacity-1·capacity·capacity+1을 reference ordered map과 differential 비교합니다.
            여기에 ascending·descending·random·duplicate inserts와 redistribute·merge·root shrink도 같은 방식으로 맞춰 봅니다.
            Crash-capable engine이라면 child page·parent separator·free list·root publish 사이 kill points도 engine
            transaction oracle에 포함합니다.
          </p>
      <p>
            Source SHA와 page format, comparator, fill factor를 pin하고 key/value distribution, cache state, sync
            policy, hardware까지 같이 pin합니다. 그 상태에서 height, allocated/live pages, point/range page reads,
            split/merge writes, p50/p99 latency를 기록합니다. Correctness나 format compatibility가 깨지면 이전 index
            generation으로 rollback하며 더 낮은 비교 횟수를 곧 durable transaction 우위로 확대하지 않습니다.
          </p>
