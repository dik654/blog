  ...
  return (
    <g>
      <rect
        ...
        stroke={highlight ? CA : "var(--border)"}
        strokeWidth={highlight ? 1.4 : 0.6}
      />
      {keys.map((k, i) => (
        <text
          key={i}
          ...
          fill={k === found ? CH : "var(--foreground)"}
        >
          {k}
        </text>
      ))}
    ...
    >
      ...
      {int1.map((n, i) => (
        <NodeBox
          key={i}
          {...n}
          highlight={step >= 1 && i === 1 && highlightPath}
        />
      ))}
      {leaves.map((n, i) => (
        <NodeBox
      ...
      {step === 3 && (
        ...
        >
          <line
            ...
            markerEnd="url(#arrS)"
          />
          <text x={275} y={158} textAnchor="middle" fontSize={10} fill={CR}>
            →next (범위 스캔)
          </text>
        </motion.g>
      )}
