...
const STEPS = [
  { label: "eth_call 요청 수신", body: "CallRequest { to, data, value, gas }" },
  {
    ...
    body: "Evm::builder().with_db(ProofDB).with_tx_env(tx)",
  },
  {
    label: "evm.transact() — 로컬 실행",
    body: "ProofDB가 필요한 계정·스토리지를 증명 기반으로 제공",
  },
  {
    label: "결과 반환 (상태 변경 없음)",
    body: "result.output() — 읽기 전용, 온체인 영향 없음",
  },
];
            ...
            return (
              ...
              >
                <rect
                  ...
                  height={36}
                  rx={8}
                  fill={colors[i] + (step === i ? "25" : "08")}
                  stroke={colors[i]}
                  strokeWidth={step === i ? 2 : 1}
                  strokeOpacity={step === i ? 1 : 0.3}
                />
                <text
                  x={70 + i * 115}
                  y={73}
