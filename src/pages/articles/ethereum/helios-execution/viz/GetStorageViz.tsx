...
const STEPS = [
  {
    label: "호출: eth_getStorageAt(addr, slot, block)",
    body: "특정 컨트랙트의 스토리지 슬롯 값을 읽는다. getBalance와 달리 slot 인자가 핵심 차이점이다.",
  },
  {
    label: "storageKeys: &[slot] -- 왜 슬롯을 지정하는가",
    body: "getBalance/getCode는 계정 레벨 정보 → storageKeys 빈 배열.\ngetStorageAt는 스토리지 트라이 내부 값 → 슬롯을 지정해야 storageProof를 받을 수 있다.",
  },
  {
    label: "2단계 MPT: State Trie → Storage Trie",
    body: "1단계: state_root에서 accountProof로 Account 검증 → storageRoot 추출\n2단계: storageRoot에서 storageProof로 slot 값 검증",
  },
  {
    label: "슬롯 값 추출: storage_proof[0].value",
    body: "H256(256비트) 값 반환. Solidity mapping은 slot = keccak256(key ++ slot_number)로 슬롯 위치를 결정한다.",
  },
];
