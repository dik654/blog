        ...
        return (
          <motion.g key={m.name} {...fade(delay)}>
            ...
            <motion.g {...fade(delay + 0.4)}>
              <text x={418} y={y + 18} fontSize={12} fill={C.ok}>
                {"✓"}
              </text>
            </motion.g>
          </motion.g>
...
export function Step1() {
  ...
  const noiseBits = new Set([
    1, 5, 11, 16, 22, 25, 31, 36, 42, 48, 55, 60, 67, 72, 80, 85, 90, 98, 105,
    110, 115, 120,
  ]);
  ...
  return (
    <g>
      ...
      <motion.g {...fade(0.1)}>
        ...
        >
          keccak256(address/topic) → 하위 11비트씩 3개 추출 → 2048비트 중 3곳에
          1 세팅
        </text>
      </motion.g>
...
export function Step2() {
  return (
    <g>
      ...
      {["getBalance", "getCode", "getStorageAt", "getTxCount"].map(
        (name, i) => {
          const y = 48 + i * 22;
          return (
            <motion.g key={name} {...fade(0.1 + i * 0.08)}>
              ...
              <text x={186} y={y + 12} fontSize={10} fill={C.ok}>
                {"✓"}
              </text>
            </motion.g>
          );
