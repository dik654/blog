...
const STEPS = [
  ...
  {
    label: "fork_hash: CRC32(genesis) XOR CRC32(fork blocks)",
    body: "예: CRC32(0xd4e5…cb8fa3) XOR CRC32(1150000) XOR … = 0xfc64ec04",
  },
  {
    label: "fork_next: 다음 예정 포크 블록/타임스탬프",
    body: "Cancun 이후: fork_next = 0 (예정 포크 없음). 새 포크 확정 시 갱신.",
  },
];
const ANNOT = [
  "TCP+RLPx 후 핸드셰이크",
        ...
        >
          {step === 0 && (
            <motion.g initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
              {["TCP/IP 연결", "RLPx 암호화", "eth 핸드셰이크", "종료"].map(
                (s, i) => {
                  const x = 30 + i * 90;
          ...
          {(step === 2 || step === 3) && (
            ...
            >
              ...
              <motion.rect
                ...
                height={40}
                rx={6}
                fill={FN_C + (step === 3 ? "25" : "0a")}
                stroke={FN_C}
                strokeWidth={step === 3 ? 2 : 1}
                strokeOpacity={step === 3 ? 1 : 0.3}
              />
              <text
                x={270}
                y={53}
          ...
          >
            <AnnotationBox
              ...
              h={64}
              label={ANNOT[step]}
              color={step === 0 ? OLD_C : step === 3 ? FN_C : FH_C}
              eyebrow="판별 결과"
            />
          </motion.g>
        </svg>
