...
- 18번째로 `reverse-mode-autodiff`를 완성했다. PyTorch 공식 문서(docs/source/notes/extending.md, "Extending torch.autograd")의 `LinearFunction` 예제를 검증했다. `torch.nn.Linear`는 실제로 native 연산이라 backward가 코드로 보이지 않지만, 이 문서는 VJP를 autograd engine 뒤에 숨기지 않고 forward·backward를 손으로 직접 쓰는 표준 방법을 공식적으로 보여준다. Forward의 `input.mm(weight.t())`가 ForwardPass의 Z=XW+1b^⊤와 일치했고, backward의 `grad_output.mm(weight)`가 ChainRule의 x̄=ȳ·J_f(x)를 Jacobian 전체를 만들지 않고 행렬곱 하나로 계산하는 실제 사례였다. 같은 grad_output이 weight 방향으로는 다른 local Jacobian(`grad_output.t().mm(input)`)과 곱해진다는 점도 fan-out의 실제 예로 남겼다. CodeViewButton 2개를 추가했다. `tsc`, `build`, `audit:graph --strict`, `audit:learning`, lint(0 error), route 회귀 테스트, Playwright 1440/390px + 버튼 2개 클릭 테스트(모두 katexErrors 0, console 0) 전부 통과.
- 19번째로 `normalizing-flows`를 완성했다. PyTorch v2.13.0의 `torch.distributions.AffineTransform`·`TransformedDistribution.log_prob`을 검증했다. article의 x=2z 예시를 실제 코드로 추적한 결과 흥미로운 구조를 발견했다 — `AffineTransform.log_abs_det_jacobian`은 article의 log r(x)=log|dz/dx|가 아니라 정반대 방향인 forward log|dy/dx|(base→data 방향)를 계산하고, `TransformedDistribution.log_prob`이 이 값을 **빼는** 것으로 최종 log r(x)를 만든다 — inverse Jacobian을 따로 유도하지 않고도 같은 결과를 내는 실제 구현 관례였다. article의 검산 예시(x=2z→pX(x)=pZ(x/2)/2)가 scale=2인 AffineTransform의 log_abs_det_jacobian=log2를 빼는 것과 정확히 일치함을 확인했다. CodeViewButton 2개를 추가했다. `tsc`, `build`, `audit:graph --strict`, `audit:learning`, lint(0 error), route 회귀 테스트, Playwright 1440/390px + 버튼 2개 클릭 테스트(모두 katexErrors 0, console 0) 전부 통과.

- 사용자가 "ICLORA가 뭔지 등은 개념에 정리되어있어?"라고 물어 지식 그래프에 없음을 확인했고, IC-LoRA(원 논문, arXiv 2410.23775)와 ID-LoRA(2026, arXiv 2603.10256, `github.com/ID-LoRA/ID-LoRA` 공개 repo)를 함께 이해한 뒤 "이것들을 개념 그래프들에 포함시키고 글도 작성해야해"라는 명시적 지시로 새 독립 글 `in-context-lora`를 만들었다. `knowledge-graph.ts`에 6개 개념(in-context-lora-adaptation·reference-target-context-concatenation·flow-matching-reference-conditioning·negative-temporal-position-conditioning·identity-guidance-delta·two-stage-distilled-refinement)과 11개 edge를, `article-learning.ts`에 정확히 6 basic+4 advanced exercise를 포함한 학습 계약을, `editorial-ownership.ts`에 소유 범위를 등록했다. ID-LoRA 공개 repo의 실제 소스 3개(`ltx2_v2v_ic_lora.yaml`의 lora rank/target_modules, `audio_ref_only_ic.py`의 flow-matching reference conditioning+negative position+loss mask, `inference_two_stage.py`의 identity guidance delta)를 검증했다 — 이 과정에서 처음에 `audio_loss_mask = ~ref_audio_conditioning_mask`를 검증 없이 작성했다가, 실제 fetch한 원본과 대조해 reference/target을 각각 zeros/ones tensor로 만들어 concat하는 실제 패턴으로 고쳐 자체 정정했다. Training section은 이미 이 사이트가 가진 `diffusion-continuous-time`의 flow-matching 유도(x_t=(1-t)x_0+tx_1, u_t=x_1-x_0)를 재유도하지 않고 링크로 재사용해 canonical owner 원칙을 지켰다. CodeViewButton 3개(config·training·inference)를 추가했다. `tsc`, `build`, `audit:graph --strict`(신규 개념 12건의 "owner 없음" 실패가 전부 해소), `audit:learning --require-registration ai/in-context-lora`, lint(0 error), route 회귀 테스트(catalog 길이 557→558로 갱신), Playwright 1440/390px(katexErrors 0, docOverflow 0, console 0) + 버튼 3개 클릭 테스트(코드 라인 수 41/80/43, 파일 실제 라인 수와 정확히 일치, katexErrors 0) 전부 통과.
- 20번째로 "주요 아키텍처" 목록의 마지막 미완료 항목이었던 `seq2seq`를 완성했다. torch/torchvision·transformers 코어가 아니라 PyTorch 팀이 직접 유지하는 공식 튜토리얼 저장소(`pytorch/tutorials`, main, commit 4ed884d)의 `seq2seq_translation_tutorial.py`를 검증했다. Encoder→decoder state handoff(encoder·decoder hidden size가 같아 실제로는 article의 W_h projection 없이 encoder_hidden을 그대로 넘김), decoder의 embedding→recurrent transition→output projection 세 단계, teacher forcing/autoregressive 분기(target_tensor 유무로 갈라짐)가 article과 정확히 대응했다. 실제 코드 대조로 두 가지 진짜 불일치도 발견했다 — (1) 튜토리얼 문서 주석은 `teacher_forcing_ratio`로 확률적 on/off를 설명하지만 실제 `train_epoch` 코드에는 그 분기가 없고 target_tensor를 항상 넘겨 100% teacher forcing한다(과거 버전의 남은 설명 텍스트로 보임), (2) `NLLLoss`에 `ignore_index`가 없어 EOS 뒤 padding(SOS_token=0으로 채움)도 loss에 그대로 포함되는데, article의 mask m_t 기반 masked-average는 이 pinned 튜토리얼에는 없는 더 신중한 production 관례다 — 두 차이 모두 annotation에 명시했다. CodeViewButton 3개(Encoder·Decoder·Training)를 추가했다. `tsc`, `build`, `audit:graph --strict`, `audit:learning --require-registration`, lint(0 error), route 회귀 테스트, Playwright 1440/390px + 버튼 3개 클릭 테스트(모두 katexErrors 0, docOverflow 0, console 0) 전부 통과. 이로써 "주요 아키텍처/알고리즘" 코드 그라운딩 목록(원래 후보 전체)을 모두 완료했다.

- 사용자가 "CodeSidebar 회귀 백로그 이어서 진행해"라고 요청해, 2026-08-18에 발견됐던 회귀(`ModernArticle` 패턴의 코드 분석형 글이 실제 오픈소스 근거 `CodeSidebar`를 빼먹은 문제, `blog-project-reth-codegrounding` 메모리에 기록)의 CometBFT 부분을 전부 처리했다. Reth(24/25 완료, `reth-mev` 1편만 잔존)와 Prysm(18/18 완료)은 이미 실제로는 회귀가 없었음을 코드 스캔으로 확인했고, CometBFT는 등록된 8개 글(`cometbft-abci`·`crypto`·`execution`·`mempool`·`state`·`types`·`consensus`·`p2p`) 전부가 실제로는 `codeRefs.ts`/`fileTrees.ts`에 이미 real CometBFT v0.40.0 Go 소스와 정확한 line-range annotation이 갖춰져 있었는데도 top-level 조립 파일과 section 컴포넌트 어디에도 `CodeSidebar`/`CodeViewButton`이 연결돼 있지 않아 전부 죽어 있었다 — 새 코드 조사·vendoring이 아니라 이미 존재하던 데이터를 다시 연결하는 작업이었다. `cometbft-abci`는 4개 section에 15개 버튼, `cometbft-crypto`(ModernArticle 단일 파일)는 7개, `cometbft-execution`은 3개, `cometbft-mempool`은 5개, `cometbft-state`는 7개, `cometbft-types`는 3개 section에 13개, `cometbft-consensus`는 6개 section에 10개, `cometbft-p2p`(ModernArticle)는 13개 — 총 8개 글에 63개 CodeViewButton을 실제 코드 위치에 맞춰 복원했다. `cometbft-crypto`·`cometbft-p2p`·`cometbft-execution`·`cometbft-mempool`·`cometbft-state`는 `ModernArticle.tsx` 단일 파일이 곧 top-level route라 그 안에서 직접 `useCodeSidebar`+`<CodeSidebar>`를 fragment로 감쌌고, 나머지 3개(`abci`·`types`·`consensus`)는 top-level 조립 파일에 `useCodeSidebar`/`<CodeSidebar>`를 두고 section들에 `onCodeRef` prop을 threading하는 기존 split-file 패턴을 그대로 따랐다. 버튼 배치는 각 codeRef의 `desc`가 가리키는 실제 함수와 본문 문단의 주제가 가장 가까운 지점을 골랐다(예: `cometbft-types`의 `tx-hash`는 evidence 그룹 파일에 있었지만 실제로는 Data.Hash()의 하위 building block이라 BlockHeader section에 배치, `try-add-vote`는 vote 그룹에 있었지만 EvidencePool 신고 로직이라 Byzantine section에 배치). `cometbft-mempool`(pre-existing `\mathrm`/`\max` escape 2건)과 `cometbft-consensus/RoundState.tsx`(pre-existing 미사용 `CometBFTCoreViz` import)에서 이번 diff와 무관한 lint 경고를 git show로 확인해 pre-existing임을 검증했다. `tsc`, `build`, `audit:graph --strict`, `audit:learning --require-registration`(9개 글 모두), lint(신규 diff 기준 0 error), route 회귀 테스트, Playwright 1440/390px 전 route(katexErrors 0, docOverflow 0, console 0) + 버튼 79개(15+7+3+5+7+13+10+13+... 실제로는 abci 15+crypto 7+execution 3+mempool 5+state 7+types 13+consensus 10+p2p 13=73개) 전부 클릭해 코드 라인 수가 실제 vendored 파일과 일치함을 확인. 남은 백로그는 `reth-mev`(1편, 신규 vendoring 필요)와 Filecoin(등록된 8개 글 중 6개는 codebase가 이미 있어 재연결만 필요, 2개는 codebase 자체가 없어 신규 조사 필요) — 다음 세션에서 이어간다.

- CodeSidebar 회귀 백로그를 이어서 Filecoin 부분까지 완료했다. 등록된 9개 Filecoin/GPU 글 중 codebase가 이미 vendored돼 있던 7개(`filecoin-f3`·`filecoin-fvm`·`filecoin-gpu-proofs`(gpu 카테고리)·`filecoin-ipc`·`filecoin-lotus`·`filecoin-pdp`·`filecoin-proofs`)가 CometBFT와 같은 패턴으로 죽어 있었다 — `codeRefs.ts`/`fileTree(s).ts`에 실제 rust-fil-proofs·bellperson·go-f3·ref-fvm·ipc·Lotus·Curio 소스가 이미 정확한 line-range annotation과 함께 있었는데 `ModernArticle.tsx` 어디에도 `CodeSidebar`가 연결돼 있지 않았다. 전부 `ModernArticle.tsx` 단일 파일이 곧 top-level route라, 각 파일 안에서 직접 `useCodeSidebar`+`<CodeSidebar>`를 fragment로 감싸 복원했다: f3(2버튼), fvm(1), gpu-proofs(5), ipc(1), lotus(5), pdp(1), proofs(6) — 총 21개. 버튼 배치는 각 codeRef의 실제 함수와 본문 section 주제를 대조해 정했다(예: `filecoin-proofs`의 seal PC1/PC2/C2와 StackedGraph를 "phase-envelope" section에, WindowPoSt 생성/검증 함수는 각각 "proof-profile"·"verification-router" section에 분리 배치). `filecoin-f3`(pre-existing `\preceq` escape)와 `filecoin-pdp`(pre-existing `\alpha` escape)는 git show로 diff 이전부터 있던 plain-string LaTeX macro 이슈임을 확인했다. `filecoin-gpu-proofs`는 `blockchain`이 아니라 `gpu` 카테고리에 등록돼 있어 `audit:learning`을 `gpu/filecoin-gpu-proofs` route로 다시 돌려 확인했다. `tsc`, `build`, `audit:graph --strict`, `audit:learning --require-registration`(7개 글 모두), lint(신규 diff 기준 0 error), route 회귀 테스트, Playwright 1440/390px 전 route(katexErrors 0, docOverflow 0, console 0) + 버튼 21개 전부 클릭(코드 라인 수가 실제 vendored 파일과 일치, 일부는 1000줄 넘는 실제 프로덕션 파일 그대로) 전부 통과. 이로써 이번 CodeSidebar 회귀 백로그(Reth·Prysm은 이미 정상, CometBFT 8편+Filecoin/GPU 7편 = 15편 복원)의 codebase-존재 부분을 모두 마쳤다. 남은 것은 codebase 자체가 없어 신규 vendoring이 필요한 `reth-mev`(1편), `filecoin-onchain-cloud`, `filecoin-storacha`(2편) — 총 3편.

- CodeSidebar 회귀 백로그의 마지막 blockchain 항목이었던 `reth-mev`를 완료했다. 이 글은 CometBFT/Filecoin과 달리 codebase 자체가 없어(코드 스캐폴딩만 `onCodeRef: _onCodeRef`로 미사용 처리돼 있었음) 처음부터 실제 오픈소스를 새로 조사·vendoring했다. 글이 이미 인용하고 있던 pinned commit들(mev-boost @203bb965, rbuilder @6037fa72)을 그대로 사용해 실제 소스를 fetch했다 — mev-boost의 `processBid`(get_header.go)가 article의 b*=argmax_{b∈V(t<t_d)} v(b) 식에서 유효 집합 V를 만드는 단계(pubkey·signature·parentHash·zero-value 검사, 그리고 article 식에는 없는 실제 운영 policy인 최소 bid 필터)와 argmax 갱신 단계를 정확히 대응시켰고, mev-boost의 `innerGetPayload`+`verifyPayload`(get_payload.go)가 Flashbots.tsx의 "Payload non-delivery" 실패 모드를 실제로 timeout 채널로 구현하는 방식을 보여줬다. rbuilder의 `reth_prov.rs`는 BuilderApi.tsx의 "rbuilder는 Reth의 crates와 provider를 재사용할 수 있는 별도 builder application"이라는 주장을 `StateProviderFactoryFromRethProvider`가 실제 `reth_provider` crate의 trait들을 where절로 요구하고 대부분의 메서드를 그대로 위임하되 `root_hasher`만 자체 로직을 추가하는 지점으로 구체화했다. Reth의 local payload builder 자체는 이미 `reth-payload-builder` 글이 실제 Reth `builder.rs`/`engine.rs`로 소유하고 있어 재유도하지 않았다. CodeViewButton 3개(Overview·BuilderApi·Flashbots 각 1개)를 추가했다. `tsc`, `build`, `audit:graph --strict`, `audit:learning`(reth-mev는 learning contract가 아예 없어 `--require-registration` 없이 확인 — 이는 CLAUDE.md의 별도 prose-depth 백로그 범위이므로 이번 작업 범위 밖으로 남김), lint(0 error), route 회귀 테스트, Playwright 1440/390px + 버튼 3개 클릭 테스트(코드 라인 수 105/84/97, 실제 vendored 파일과 정확히 일치, katexErrors 0, console 0) 전부 통과. 남은 CodeSidebar 백로그는 Filecoin의 `filecoin-onchain-cloud`·`filecoin-storacha` 2편.

- CodeSidebar 회귀 백로그의 마지막 두 항목 `filecoin-onchain-cloud`·`filecoin-storacha`를 완료했다. 둘 다 reth-mev와 같은 유형 — codebase가 없어 새로 실제 오픈소스를 조사·vendoring했다. 두 글 모두 이미 pinned commit으로 실제 저장소를 인용하고 있어(FilOzone/filecoin-services @a391c1c, FilOzone/filecoin-pay @04ded6a, storacha/specs @3b67918) 그 인용을 실제 구현 코드로 확장했다.
  - `filecoin-onchain-cloud`: FilecoinWarmStorageService.sol의 `DataSetInfo` struct가 dataset-service section의 "누가 어떤 파일을 누구에게 맡겼는가" 다섯 role과 정확히 대응했고, `possessionProven`/`nextProvingPeriod`가 proof-settlement section의 Period state 세 값(판정 전=NO_PROVING_DEADLINE·통과=provenThisPeriod·deadline 놓침=window 밖 revert)을 그대로 구현하고 있었다. FilecoinPayV1.sol의 `Rail`/`Account`/`OperatorApproval` struct가 payment-rail section의 "다섯 장부 항목"(Deposit·Operator allowance·Variable rate·Fixed lockup)에 대응했고, `_settleSegmentGross`의 `grossSettledAmount = rate * duration` 계산과 `validator.validatePayment` 호출이 article의 "rate*proven epoch" 공식과 "open/faulted epoch를 곱하지 않는다"는 proof-aware 조정 주장을 정확히 뒷받침했다. CodeViewButton 3개.
  - `filecoin-storacha`: 기존 인용은 storacha/specs의 markdown spec 문서뿐이었는데, 이를 실제 구현 repo(storacha/ucanto, storacha/w3up)로 확장했다. ucanto의 `defaultDerives`가 space-authorization section의 C_child⊆C_parent(resource 포함 관계 + caveat 일치) 검사를 정확히 구현하고 있었고, w3up의 `blobAddProvider`가 blob-upload section의 U=Auth∧Alloc∧Put∧Accept 공식을 allocate→put→accept 세 effect를 순서대로 호출하고 fork로 연결하는 실제 handler로 보여줬다. CodeViewButton 2개.
  - 두 글 다 learning contract가 아직 등록되지 않아(별도 CLAUDE.md prose-depth 백로그 범위) `--require-registration` 없이 확인했다. `tsc`, `build`, `audit:graph --strict`, `audit:learning`, lint(0 error), route 회귀 테스트, Playwright 1440/390px + 버튼 5개 클릭 테스트(코드 라인 수가 실제 vendored 파일과 정확히 일치, katexErrors 0, console 0) 전부 통과.
- 이로써 2026-08-18에 발견된 CodeSidebar 회귀 백로그(코드 분석형 글의 실제 오픈소스 근거 누락)가 Reth(24/25, 1편만 신규 vendoring 필요했음)·Prysm(18/18, 이미 정상)·CometBFT(8/8)·Filecoin/GPU(9/9) 전 범위에서 완전히 마무리됐다. 총 CodeViewButton 100개 이상을 복원·신규 추가했다.

- 사용자가 "AI 다른 글들도 CodeSidebar 회귀 있는지 확인해줘"라고 요청해 전체 AI 카테고리(308개 글)를 스캔했다. 실제 `codeRefs.ts`가 있는 29개 글은 전부 정상 연결돼 있어 CometBFT/Filecoin류 회귀는 없었지만, `claude-code`·`llm-serving-ops`·`open-r1` 세 글이 codebase 디렉터리 구조(하위 폴더까지)만 스캐폴딩돼 있고 실제 파일은 하나도 없는(git에도 추적된 적 없는) 상태임을 발견했다 — "복구"가 아니라 "신규 조사"가 필요한 항목이라고 보고했고, 사용자가 "3개 다 채워줘"로 확정해 순서대로 완료했다.
  - `open-r1`(huggingface/open-r1 @1416fa0, huggingface/trl @7bd0d61): RewardSystem section은 open-r1의 `accuracy_reward`(symbolic verify, 파싱 실패 시 0점이 아니라 None으로 학습 신호 제외)·`format_reward`(정규식 tag만 확인, correctness는 안 봄)·`get_reward_funcs`(config에서 verifier 조립)로 그라운딩했다. GRPOProcess section은 TRL GRPOTrainer의 advantage 계산(scale_rewards 분기, normalize_then_sum의 실제 reward_weights 가중합)과 clipped surrogate loss(coef_1=ρ, coef_2=clip(ρ), per_token_loss=-min(u,c))로 그라운딩했는데, 실제 코드를 보고 발견한 것 — grpo/bnpo/dr_grpo/dapo가 per-token surrogate는 완전히 같은 식을 쓰고 최종 scalar aggregation 방식만 다르다(dr_grpo=고정 max_completion_length로 나눔, dapo=batch 전체 active token 수로 나눔)는 걸 확인해 annotation에 명시했다. CodeViewButton 3개.
  - `llm-serving-ops`(BerriAI/litellm @007bd43): LiteLLMGateway section의 "eligibility before ranking" 주장을 실제 `Router._pre_call_checks`(context window·RPM·region 필터를 ranking 이전에 적용)로 그라운딩했다. retry budget 부분은 진짜 불일치를 발견했다 — article의 ExplainedFormula는 D_remain 기반 연속시간 deadline 부등식으로 설명하지만, 실제 LiteLLM router는 `get_num_retries_from_retry_policy`에서 exception 종류별 고정 retry count를 조회하는 count-based 방식이었다. 이 차이를 annotation과 desc에 명시적으로 flag했다. CodeViewButton 2개.
  - `claude-code`(anthropics/claude-code @c3d2e35, 공개 examples/plugins 저장소 — 기존 빈 스캐폴드가 정확히 이 저장소 구조를 가리키고 있었음): agent-loop section의 a_t=P(s_t), o_t=E(G(a_t)) 식(runtime gate가 action 실행 전에 판정) 중 gate 부분을 공식 `examples/hooks/bash_command_validator_example.py`(PreToolUse hook, exit code로 allow/deny 반환)로 그라운딩했다. tool-effect section의 "File operation·shell execution·search가 서로 다른 effect를 낸다"는 claim은 `plugins/hookify/hooks/pretooluse.py`(tool_name을 bash/file event로 분류해 다른 rule을 적용)로 그라운딩했다. Claude Code 핵심 agent loop 자체는 closed-source라 그 부분은 여전히 공식 문서 인용에 의존한다는 점을 그대로 유지했다. CodeViewButton 2개.
  - 3개 글 모두 split multi-file(RewardSystem/GRPOProcess) 또는 single ModernArticle 패턴에 맞춰 `onCodeRef` prop threading 또는 fragment-wrap 방식을 사용했고, top-level 조립 파일에 `useCodeSidebar`/`<CodeSidebar>`를 배치했다.
  - `tsc`, `build`, `audit:graph --strict`, `audit:learning`(3개 글 모두 기존 learning contract 유지), lint(0 error), route 회귀 테스트, Playwright 1440/390px 전 route(katexErrors 0, docOverflow 0, console 0) + 버튼 7개 클릭 테스트(코드 라인 수가 vendored 파일과 정확히 일치) 전부 통과.

- 사용자가 확인해달라고 한 나머지 4개(halo2·sp1·helios-config·helios-types)를 모두 완료했다. 전체 리포지토리를 codeRefs*.ts 존재 여부로 재스캔해 발견한 항목들로, 세 가지 서로 다른 패턴이었다.
  - `halo2`: CometBFT와 같은 패턴 — 실제 vendored `?raw` 파일 5개(zcash/halo2 pinned cafc26e)와 codeRefs.ts 6개 key가 이미 존재했지만 ModernArticle.tsx에 CodeSidebar가 전혀 연결돼 있지 않았다. 순수 재연결만 진행. CodeViewButton 6개(plonk.rs VK/PK, circuit.rs Chip/Cell/Region/Layouter, keygen_vk/keygen_pk/create_proof/verify_proof).
  - `sp1`: 가장 특이한 케이스. 현재 라이브 ModernArticle.tsx는 citation 링크만 있고, 같은 디렉터리의 버려진 legacy `codeRefsVM.ts` 등 8개 파일은 `?raw` import가 아니라 **문자열로 직접 타이핑된 코드**를 담고 있었다. 하나를 실제 SP1 v6.4.0 소스(commit f66b4bf)와 대조해보니 struct signature가 실제와 다르고(`CoreVM<'a>` vs 실제 `CoreVM<'a, M: ExecutionMode>`) 실제 필드 3개가 누락돼 있었으며 그 사실이 전혀 disclose되지 않은 상태였다 — 이 legacy 세트 전체를 신뢰하지 않기로 하고, ModernArticle.tsx가 실제로 인용하는 4개 파일(program.rs·record.rs·proof.rs·prover.rs)을 새로 fetch해 처음부터 다시 그라운딩했다. `Program` struct가 ELF artifact를, `ExecutionRecord::split()`의 `update_finalized_state_from_public_values` 호출이 shard 연속성 식(m_i^out=m_{i+1}^in)을, `verify_mock_public_inputs`가 statement digest 식(d_S=H(vkHash‖H(PV)))을, `Prover` trait의 setup/prove/execute/verify가 SDK 경계 주장을 정확히 뒷받침했다. 버려진 legacy 파일들(codeRefsVM.ts 등, AIR.tsx 등 20여 개 섹션)은 아무 데서도 import되지 않는 걸 확인했고 삭제하지 않고 그대로 남겨뒀다(범위 밖 판단). CodeViewButton 4개, 파일명은 `codeRefsReal.ts`/`fileTreeReal.ts`로 분리해 기존 미검증 `codeRefs.ts`와 충돌하지 않게 했다.
  - `helios-config`·`helios-types`: 가장 복잡한 구조적 문제. 활성 route(Overview/NetworkConfig/ClientInit/Persistence, Overview/CoreTypes/Encoding/SszInternal)가 전부 `onCodeRef: _onCodeRef`로 destructure해 실제로 호출한 적이 없는 상태였고, 같은 디렉터리의 legacy `NetworkEnum.tsx`/`ClientBuilder.tsx`/`BeaconHeader.tsx`/`LcUpdate.tsx`는 지금 쓰는 `CodeSidebar`가 아니라 더 예전의 `CodePanel`이라는 별도 컴포넌트로 코드를 보여주고 있었다. 게다가 유일하게 존재하던 codebase 파일(`helios-config/codebase/helios/config/src/lib.rs`)조차 `pub fn new() -> Self { .. }`처럼 실제 Rust로 컴파일될 수 없는 placeholder 문법을 담고 있어 신뢰할 수 없었다. 전부 무시하고 a16z/helios(commit 43a8c9f, 이미 각 글의 CitationBlock이 인용하던 실제 SHA)에서 새로 fetch했다. `helios-config`: `Config::from_file()`의 Figment `.merge(base).merge(toml).merge(cli)` 체인이 article의 "BaseConfig → TOML → CLI, 8545→9545→10545" 주장을, `mainnet()`의 BaseConfig 필드들이 network profile 주장을, `EthereumClientBuilder::build()`의 `unwrap_or_else`/`or_else` 패턴 반복이 explicit-builder-우선 fallback 주장을, `FileDB`(91줄 전체, truncate 후 바로 write·32-byte 검사)가 persistence 주장을 정확히 뒷받침했다. CodeViewButton 4개. `helios-types`: `BeaconBlockHeader`(u64 2개+B256 3개=112 bytes)·`LightClientHeader`(beacon/execution 분리)·`SyncAggregate`(160 bytes)·`Update`·`LightClientStore`가 article의 바이트 계산과 네 구조체 역할 구분을 정확히 뒷받침했다. Encoding/SszInternal 두 section은 일반 SSZ 수식 유도를 이미 다른 canonical 글(SSZ 정본)에 위임하고 있어 코드 버튼을 추가하지 않았다. CodeViewButton 2개.
  - 네 글 모두 `codeRefsReal.ts`/`fileTreeReal.ts` 파일명을 써서 기존의 미검증·미연결 legacy 파일과 충돌 없이 분리했다.
  - `tsc`, `build`, `audit:graph --strict`, `audit:learning`(4개 글 모두 기존 contract 유지), lint(0 error), route 회귀 테스트, Playwright 1440/390px 전 route(katexErrors 0, docOverflow 0, console 0) + 버튼 16개 클릭 테스트(코드 라인 수가 vendored 파일과 정확히 일치) 전부 통과. 이 과정에서 halo2/sp1의 실제 카테고리가 blockchain이 아니라 crypto임을 재확인(파일 위치와 카테고리가 다시 한번 불일치하는 사례).

- 사용자가 "unsloth의 팁들을 바탕으로 ai 모델 학습에 대해서 더 심도있게 작성할 수 있지않을까?"라고 물어, 조사 우선(둘 다 — 우선 조사 후 결정) 방침으로 Unsloth가 실제로 보고한 팁 3개를 리서치한 뒤 "3개 다 기존 글 보강"으로 확정받아 `optimizers`와 `lora-finetuning`에 코드 그라운딩을 추가했다. 세 주제 모두 Unsloth 블로그 주장에 그치지 않고 업스트림 저장소의 현재 실제 소스로 직접 검증했다.
  - `optimizers`(effective-batch section): 2024년 Unsloth가 보고한 gradient accumulation loss 정규화 버그를 huggingface/transformers(main, commit e12c79c)의 `fixed_cross_entropy`(loss_utils.py)·`Trainer.get_batch_samples`/`_get_num_items_in_batch`(trainer.py)로 그라운딩했다. 기존 ExplainedFormula의 "각 micro loss의 reduction scale이 같습니다"라는 assumption이 실전에서 깨지는 정확한 지점 — `num_items_in_batch`가 없으면(구버전 경로) micro-batch마다 자기 토큰 수로만 나누는 mean reduction으로 빠지고, sequence 길이가 다른 micro-batch 사이에서 전체 batch loss와 어긋난다. 실제 fix는 K개 micro-batch를 backward 전에 먼저 모아 accumulation window 전체의 유효 토큰 수(-100 제외 합산)를 미리 구해두고 그 공유 분모로 나누는 것 — 함수 이름 `fixed_cross_entropy` 자체가 수정 이력을 가리킨다. multi-GPU에서는 gather 후 재합산해야 한다는, article에는 없는 실제 세부도 annotation에 명시했다. CodeViewButton 2개.
  - `lora-finetuning`: 두 섹션에 각각 그라운딩했다. Data.tsx(이중 BOS)는 unslothai/unsloth(main, commit fdf83cb)의 `get_chat_template`의 Gemma BOS 주입 분기와 `remove_special_tokens`로 그라운딩했다 — chat template 문자열 자체가 `{{ bos_token }}`을 하드코딩해 넣고, 그 문자열을 다시 `add_special_tokens=True` 인코딩에 넣으면 BOS가 두 번 들어가는 실제 원인과, 인코딩 직전 중복 BOS를 지우는 실제 fix를 모두 코드로 보여준다(단, 실제 호출 지점은 GGUF↔HF tokenizer 동등성 검증 유틸리티였고, "학습 코드 어디서나 자동 호출"은 아니라는 점을 header 주석에 정직하게 밝혔다). LoRA.tsx(hyperparameter 배분)는 같은 저장소의 `FastLlamaModel.get_peft_model()` 시그니처로 그라운딩했다 — Unsloth의 실전 기본값(r=16, target_modules에 attention q·k·v·o + MLP gate·up·down 7개 전부, lora_alpha=16 즉 s=α/r=1)이 기존 문단의 "어디에 capacity를 배분할지 정해야 합니다"라는 질문에 대한 구체적 실전 답이며, article에는 없는 실제 옵션(`use_rslora`, 1/√r scaling)과 실제 검증(설치된 PEFT 버전이 rslora를 지원하지 않으면 즉시 RuntimeError)도 annotation에 명시했다. CodeViewButton 2개(LoRA·Data 각 1개).
  - `tsc`, `build`, `audit:graph --strict`, `audit:learning --strict`(둘 다 기존 contract 유지, `--require-registration` 불필요), lint(0 error), route 회귀 테스트(catalog 길이 불변 — 신규 route 아님), Playwright 1440/390px(katexErrors 0, docOverflow 0, console 0) + 버튼 4개 클릭 테스트(코드 라인 수가 CodePreview 렌더 결과와 정확히 대응) 전부 통과.

- 사용자가 "https://comfyui-wiki.com/ko/news 의 내용들을 바탕으로 보강해야될게 있는지 체크해줘"라고 요청했다. 뉴스 20개 항목(2026-08-08~17)을 확인한 결과 전부 특정 커뮤니티 모델·플러그인의 일일 릴리즈 공지(MiniMax H3 생태계 LoRA·체크포인트, ComfyUI 플러그인)라 이 블로그의 canonical-소스 딥다이브 형식과 맞지 않는다고 보고했다. 다만 뉴스가 가리킨 두 안정적 개념 — CUDA Graphs(ComfyUI v0.33.1이 채택)와 Normalized Attention Guidance(ComfyUI-Krea2-NAG) — 는 이미 검증 가능한 canonical 소스가 있어 후보로 제시했고, 사용자가 "CUDA Graphs 딥다이브, NAG 보강"으로 확정했다. 이어진 두 번째 확인에서 CUDA Graphs는 "신규 독립 글"로 확정받아 새 아티클 `cuda-graph-capture`(ai-llm-serving)를 만들었다.
  - `cuda-graph-capture`(신규): vllm-project/vllm(main, commit 842dd8f)의 `vllm/compilation/cuda_graph.py`를 grounding source로 삼아 4 section(overview/mechanics/implementation/tradeoffs)을 작성했다. Kernel launch overhead가 exec 시간을 압도하는 조건을 latency model(T_eager=N(τ_L+τ_E) vs T_replay≈Nτ_E+τ_L,graph)로 다루고, torch.cuda.graph의 static-address 계약을 프로즈로 설명한 뒤, 실제 vLLM `CUDAGraphWrapper.__call__`이 batch_descriptor를 key로 capture/replay를 분기하는 코드를 그대로 보여줬다. Dynamic shape 패딩·full vs piecewise capture·graph pool 공유 trade-off로 마무리했다. Viz는 `AnimatedSceneControls`/`VizFrame` 경량 패턴으로 eager/capture/replay 3-scene 타임라인을 새로 만들었다. Knowledge graph에 새 concept 3개(cuda-graph-capture-replay·cuda-graph-static-address-constraint·cuda-graph-batch-shape-dispatch)와 edge 5개를 등록했고(선수 개념은 gpu/cuda-sync-streams의 cuda-stream-ordering), learning contract(6 basic+4 advanced), editorial-ownership을 모두 새로 작성했다. CodeViewButton 1개. 기존에 "CUDA graph"를 언급만 하던 4개 글(`qwen36-hybrid-runtime`·`model-vram-budgeting`·`quantized-model-deployment`·`llm-serving-capacity`)의 prose 언급을 이 새 글로의 실제 링크로 바꿨다 — `TermBreakdown`/일반 prose는 `<Link>`(2개 글에 이미 import돼 있었고 quantized-model-deployment는 새로 추가), `llm-serving-capacity`는 그 파일이 이미 쓰던 `<a href>` 관례를 그대로 따랐다. `ExplainedFormula`의 `terms`(FormulaTerm.description은 string 타입이라 JSX 삽입 불가)에 있던 나머지 CUDA graph 언급은 링크로 바꾸지 못해, qwen36-hybrid-runtime에는 M_work 옆에 새 prose 문장 하나를 추가해 링크를 달았다.
  - `latent-diffusion-guidance`(보강): 기존 `guidance` section의 CFG vs classifier-guidance 비교 문단 바로 뒤에, 같은 extrapolation 아이디어를 attention output 레벨로 옮긴 NAG(Normalized Attention Guidance, ChenDarYen/Normalized-Attention-Guidance, main, commit 8bb34a1)를 추가했다. `nag/attention_nag.py`의 `NAGAttnProcessor2_0.__call__`을 grounding source로 삼아, positive·negative attention output에 CFG와 대수적으로 동일한 extrapolation(εc·scale−εu·(scale−1))을 적용한 뒤 L1-norm 기반 clamp(τ)와 원래 값과의 blend(α)로 다시 누르는 구조를 보여줬다 — 이 두 안전장치는 article의 CFG 식에는 없는 것으로, few-step·distilled 모델에서 CFG가 collapse하는 문제를 완화하기 위한 것이라고 명시했다. 코드에서 `origin_batch_size = batch_size - len(hidden_states)`라는, 2× guidance에서는 원래 batch 크기와 맞아떨어지지만 3×/4× 확장에서는 pipeline의 부분 확장을 전제하는 것으로 보이는 계산을 발견해 — 정확한 유도를 확신할 수 없어 annotation에 "이 글은 2× 경우만 다룬다"고 정직하게 hedge했다. 이 article은 처음으로 CodeSidebar를 도입해 `codeRefs.ts`/`fileTree.ts`를 새로 만들었다. CodeViewButton 1개.
  - `tsc`, `build`, `audit:graph --strict`, `audit:learning --strict`(cuda-graph-capture는 신규 route라 `--require-registration`으로 먼저 확인, 이후 둘 다 `--strict`로 재확인), lint(0 error), route 회귀 테스트(catalog 길이 558→559로 갱신 — 유일한 신규 route), Playwright 1440/390px 전 6개 route(katexErrors 0, docOverflow 0, console 0) + 버튼 2개 클릭 테스트(코드 라인 수 214/79, 실제 vendored 파일과 정확히 일치) + 4개 글의 cross-link가 실제로 `/ai/cuda-graph-capture`를 가리키는지 Playwright로 별도 확인 전부 통과.

- 사용자가 "/ai/math-vectors-inner-products 에서 선이 점에 안이어져있고 끊어져있어. 왜 이런 실수가 나온거야?"라고 버그를 신고했다. `Vectors.tsx`의 `VectorMeasurementViz.tsx`는 SVG가 아니라 CSS `<span>` 회전으로 (3,4) 벡터를 그리는데, 선(`width: 60%`, `rotate(-53deg)`, origin-left)의 실제 끝점을 계산하면 `left=60·cos53°≈36%, bottom=60·sin53°≈48%`인데, 점 마커는 `bottom-[48%] left-[58%]`로 하드코딩돼 있어 bottom은 우연히 맞고 left만 22%p 어긋나 있었다 — 선의 회전 수식과 점의 절대좌표가 같은 계산에서 나오지 않고 각각 눈대중으로 따로 박아넣어진 것이 원인이었다(라벨 위치도 점 기준 +4%/+4% 오프셋을 유지하려면 같이 어긋나 있었음). `left-[58%]→left-[36%]`, label `left-[62%]→left-[40%]`로 수정해 Playwright 스크린샷으로 선-점 연결을 직접 확인했다. `tsc`, `build`, lint(0 error), Playwright 1440/390px(katexErrors 0, docOverflow 0, console 0) 통과.

- 사용자가 공통 `ArticleLessonFlowViz`에서 `개념 3/11 · 컷 5/5`의 `다음 컷`을 누르면 `개념 4/11 · 컷 5/5`로 이동하는 버그를 신고했다. 자동 재생은 개념 경계에서 `setReveal(0)`을 쓰지만 수동 `moveForward`만 `setReveal(4)`를 쓰는 경로 불일치가 원인이었다. 정본에 순방향 개념 전환은 새 개념의 첫 컷, 역방향 전환은 이전 개념의 마지막 컷이라는 단조 탐색 규칙을 추가하고 수동 순방향 경로를 `setReveal(0)`으로 일치시켰다. `audit:viz --strict`(기존의 짧은 SVG 번호 text 1건은 브라우저 확인 항목), `audit:reading`, `audit:learning --strict --require-registration ai/vllm-serving`, ESLint, production build가 통과했다. Playwright 1440·1000·390·844px에서 `3/11·5/5 → 4/11·1/5` 순방향과 `4/11·1/5 → 3/11·5/5` 역방향을 직접 재현했고 document·Viz overflow 0, console error 0을 확인했다.

## 2026-08-24 · 용어집 없는 canonical graph 보강 · RAG · GPU authoring · CFD

- 대화의 1–1560 용어 목록을 별도 용어집 route로 복제하지 않고, 기존 knowledge graph의 coverage backlog로 사용했다. `KnowledgeConcept.aliases`와 전역 label·alias collision 감사를 추가해 MLA·DeltaNet처럼 표기가 달라도 하나의 canonical concept에 귀속되고, 같은 별칭이 둘 이상의 concept을 가리키거나 canonical label과 충돌하면 build 전에 실패하도록 했다. Graph는 2,453 concepts·3,720 relations, 공개 learning contract는 563개이며 strict graph audit의 invariant failure와 warning은 모두 0이다.
- `ai/rag-pipeline`의 retrieval을 BM25 lexical 후보 생성 → HNSW dense ANN → RRF rank fusion → cross-encoder reranking의 candidate funnel로 보강하고, GraphRAG는 단일 retriever 이름이 아니라 graph construction·provenance·hop budget을 별도로 소유하는 경계임을 명시했다. HNSW 원 논문과 BERT passage reranking 원 논문을 evidence contract에 연결했다.
- `gpu/cuda-kernel-fusion`에 CUDA C++·CUTLASS collective·CuTe layout/atom·Triton blocked program의 책임 층을 분리하는 section과 release gate를 추가했다. 공식 CUTLASS·Triton 문서를 근거로, 특정 DSL 이름을 성능 결론으로 취급하지 않고 target GPU·shape·dtype별 generated artifact·register·traffic·median/p95를 비교하도록 정리했다.
- 목록 소개에만 있고 실제 numbered concepts에는 없던 CFD는 추출 목록의 일부라고 주장하지 않고 별도 gap으로 처리했다. 신규 `gpu/cfd-finite-volume-gpu` 글에 conservation law → finite-volume cell balance → CFL time-step → stencil/GPU memory mapping → verification·validation 흐름, 수식 3개, responsive Viz 2개와 NASA Glenn·OpenFOAM Foundation·NASA LAVA 근거를 등록했다.
- `audit:graph --strict`, `audit:learning --strict`, 변경 글의 `audit:viz`·`audit:formula`·`audit:reading`·`audit:terms`, TypeScript, ESLint, production build가 통과했다. Playwright 1440×1000·390×844에서 세 route를 확인해 document overflow·KaTeX error·console error가 모두 0임을 확인하고 신규 retrieval cards·kernel stack·finite-volume pipeline을 육안 검수했다.

## 2026-08-26 · VLA Embodiment Gap · Local MoE evidence lifecycle

- `ai/vla-embodiment-gap`을 Computer Vision 학습 경로의 신규 정본으로 추가했다. Semantic grounding→3D frame→robot action contract→planner/controller→closed-loop verification의 다섯 경계를 설명하되, monolithic VLA와 hierarchical system 중 하나를 보편 정답으로 두지 않았다. RT-2·ACT·Diffusion Policy·Open X-Embodiment·OpenVLA·Octo·OK-Robot·Qwen-RobotNav·3D Diffuser Actor·Goal2Pixel·TAMP-Nav·RADAR·SO-101과 TMLR Embodiment Gap survey를 self-report/independent/freshness 경계와 함께 등록했다.
- 새 글에 article-owned concept 6개, responsive interactive Viz 2개, 기본 6·심화 4개 exercise, evidence rail 15개와 CitationBlock 14개를 연결했다. 기존 `cnn-task-spatial-contract`, `flow-matching-objective`, `agent-observation-action-loop`를 다시 정의하지 않고 prerequisite로 재사용했다.
- `model-vram-budgeting`을 보강해 MoE의 stored/resident bytes, active token path, request/runtime state를 세 장부로 분리했다. Active parameter는 exact FLOPs·bytes moved가 아니라 활성 경로 규모의 힌트라고 제한하고, Q8·Full Context·prefill/decode·MTP를 exact artifact와 stage measurement로 판정하도록 했다. Qwen3-Next 80B/A3B와 NVIDIA NVFP4 format은 공식 source로 연결했다.
- `docs/editorial-observation-ledger.md`를 만들고 `Qwen3.8-Flash-Next 176B/6B`, active 15B 임계값, Mac Studio 64K 1/10, dual RTX PRO 6000 sweet spot, DGX Spark 평가와 market segmentation을 LMI-001~008 `pending` 관찰로 격리했다. 공식 Qwen3.8-2.4T-A95B artifact가 존재해도 Flash-Next 이름·수치를 확인하지 않는다는 artifact identity 경계를 명시했다. Contract에는 official/theory/self-report/independent/project/experience/rumor 등급, environment fingerprint, promote/reject/supersede 수명주기를 추가했다.
- Claude adversarial review를 세 차례 사용했다. Native 3D·modular counterevidence, rumor leakage 제거, evidence lifecycle wording, 일반 decode와 speculative 용어 분리, sampling·tolerance fingerprint, citation order·survey citation·NVFP4 FP32 scale 지적은 채택했다. 마지막 검토의 “Qwen3.8 2.4T/95B artifact가 fabricated” 지적은 공식 Qwen model card를 다시 열어 기각했다. Survey는 추가 원문 확인으로 단순 preprint가 아니라 TMLR 2026년 8월 논문임을 반영했다.
- `audit:graph --strict`는 2,460 concepts·3,736 relations·564 contracts·invariant failure 0, `audit:learning --strict --require-registration`은 564/564, `audit:formula --strict --require-explicit`은 1,174/1,174, reading/viz/terms/topology 감사와 변경 파일 ESLint, production build(5,369 modules·571 static routes)가 통과했다. `audit:articles --strict`만 저장소 전체의 기존 이관 후보 74개 때문에 exit 1이며 이번 두 글은 경고 목록에 없다.
- Playwright 1440×1000·390×844에서 두 route의 horizontal overflow와 console error/warning이 0임을 확인했다. 공유 lesson flow의 과거 회귀도 `개념 3/11 · 컷 5/5 → 개념 4/11 · 컷 1/5`로 다시 직접 검증했다. Full-page·focused screenshots는 `output/playwright/`에 남겼다.

## 2026-08-28 · 읽기 흐름 · prerequisite 순서 · 안정된 Viz · 출처 경계

- 이번 작업은 사용자가 “Krea 2 설명처럼 채팅에서는 잘 읽히는데 블로그 artifact는 왜 압축돼 보이는가”라고 지적한 데서 시작했다. 원인은 개별 작성자의 문체만이 아니라 기존 계약이 구조·수식·근거·완결성은 강제하면서도 문단의 속도와 독자가 따라가는 서사를 명시하지 않았고, 과거의 150–200줄 목표가 표·칩·긴 복합 제목으로 정보를 압축하는 잘못된 유인을 만들었다는 데 있었다. 줄 수 목표를 제거하고 `claim → 이유 → 작은 예 → 한계 → 다음 질문`, 한 문단 한 생각, 표는 설명 뒤에만 배치, 편집 내부어 남용 금지를 정본 계약과 `CLAUDE.md`에 반영했다.
- 공개 글 569편 전체를 당장 “수동 재작성 완료”라고 선언하지 않았다. `audit-prose-readability.mjs`가 긴 제목·과밀 문단·가운데점 목록·내부 운영어·전환 부족을 route와 source fingerprint 단위로 추적하도록 만들었고, 현재 326개 legacy compression signal을 `prose-readability-baseline.json`에 known inventory로 고정했다. 이후 새 글·변경 글·악화된 글은 strict CI에서 실패한다. 이 baseline은 품질 인증이 아니라 재검토 queue다. 대표적으로 `ai/retrieval-ranking-funnel`은 product-code 예와 retrieval funnel의 원인–결과 서사로, `crypto/paillier-cryptosystem`은 활용 직관→수학→보안 경계 순서로 다시 썼다.
- Article listing은 manifest concat 순서가 아니라 learning contract의 `assumedKnowledge`를 concept owner로 해석한 prerequisite DAG로 stable topological sort한다. 직접 의존하지 않는 글은 manifest 순서를 tie-breaker로 유지하며 cycle과 prerequisite inversion은 `audit-article-reading-order.mjs`가 실패시킨다. Category UI에 `읽기 01` 번호와 학습 순서 설명을 추가했고 전역 검사 결과는 65개 listing·150개 prerequisite edge·cycle 0이다. Route 회귀 테스트의 하드코딩된 `563`도 현재 `ARTICLE_LEARNING` 수와 비교하는 invariant로 바꿔 새 글 추가 때 테스트 숫자를 손으로 맞추지 않게 했다.
- 공통 animated Viz와 lesson-flow Viz는 control row를 고정 높이로 예약하고, 가장 큰 장면 기준 frame을 `min(..., calc(100dvh - 5rem))`로 viewport 안에 제한했다. 내용이 커지면 document와 button을 밀지 않고 stage 내부만 scroll한다. Fixed header가 stage 상단을 가리는 문제도 `scroll-margin-top`으로 보정했다. Playwright 실측에서 desktop 1440×1000의 scene 1→4 frame·controls·각 button bbox 이동은 0px, mobile 390×844 lesson transition은 frame 0px·button y 최대 1px였고 document overflow는 모두 0이었다. 최종 mobile frame은 y=72, bottom=826으로 844px viewport 안에 들어왔다.
- vLLM 글에서 Orca가 근거 없이 나온 것처럼 보인다는 피드백을 원문으로 다시 검증했다. Orca는 실제 OSDI 2022 system이며 iteration-level scheduling과 selective batching을 제안했고, vLLM/PagedAttention 원 논문도 Orca를 baseline과 선행 system으로 다룬다. 사실 오류는 아니었지만 “왜 vLLM 글에 등장하는가”가 빠져 provenance conflation을 만들었다. 본문을 `Orca가 먼저 해결한 scheduling 문제 → vLLM이 공유한 fine-grained scheduling → vLLM이 새로 해결한 dynamic KV-memory 병목`으로 재작성하고, Orca가 vLLM 모듈이 아니며 selective-batching 구현도 현재 vLLM backend와 같지 않다고 명시했다. Sarathi-Serve·FastServe·SGLang/RadixAttention·SpecInfer도 원 논문과 대조해 주장은 유지하되 근거 지도에서 `선행·비교 논문`으로 분리했다.
- ChatGPT식 progressive disclosure 요구를 공용 `ProgressiveDetail`로 구현했다. 기본 화면에는 문제·mechanism·결론·핵심 한계가 이어지는 always-visible spine을 남기고, 증명·실험 조건·구현 차이처럼 첫 독해를 끊는 상세만 질문형 제목과 결론 preview 아래 접는다. 안전 단서나 사실을 뒤집는 조건은 숨기지 않는 규칙을 계약과 reading audit에 추가했다. vLLM Orca 절에서 dogfood했으며 390px에서 닫힌 preview가 보이고 keyboard Enter로 열림, horizontal overflow 0, console error 0을 확인했다. 1440px에서도 접힌 summary가 711px article 폭 안에 안정적으로 렌더됐다.
- 검증: `audit:graph --strict` 2,468 concepts·3,743 relations·invariant 0, `audit:learning --strict --require-registration --all-articles` 569/569, `audit:formula --strict --require-explicit` 1,176/1,176, `audit:order` 65/150, `audit:reading`, `audit:prose`, route regression test, ESLint, TypeScript production build와 576 static route generation이 통과했다. Prose baseline은 vLLM 재검토 후 score가 22→18로 낮아진 새 fingerprint만 승인했다.

### 2026-08-28 · 전역 article shell과 LoRA·Open-R1의 실제 독해 보강

- 다음 재작성 대상을 단순히 prose score 내림차순으로 선택하지 않았다. 진단 출력을 다시 보니 `snapshot`·`receipt`처럼 이 블로그가 실제 기술 개념으로 사용하는 단어도 editorial jargon 점수에 들어가 있었고, 이 신호를 그대로 문장 결함으로 취급하면 정확한 용어를 억지로 지우게 된다. 감사기를 `pacingScore`와 `editorialJargonScore`로 나누고 `--route`와 원인 snippet을 추가해, 긴 문단·목록형 문장과 도메인 용어를 사람이 구분한 뒤 고치도록 바꿨다. 총점은 regression gate로 유지하되 재작성 결정권으로 쓰지 않는다.
- 실제 브라우저에서 LoRA 글을 읽으며 source 파일 밖의 더 큰 원인을 발견했다. 모든 글 앞에 자동 생성되던 `다루는 범위: A · B · C...` 문장은 concept가 많을수록 첫 문단부터 압축 목록이 됐고, 본문 중간의 `ContentBoundary`는 `이 글이 직접 소유하는 내용 / 연결 글에서 재사용하는 내용 / 근거와 한계`를 3열 카드로 동시에 펼쳐 독자의 인과 흐름을 끊었다. 둘 다 편집상 유용하지만 첫 독해의 필수 내용은 아니므로 전역 progressive disclosure로 옮겼다.
- `ArticleOnboarding`은 prerequisite 링크와 읽기 방향만 기본 화면에 남긴다. 정확한 범위는 질문형 disclosure 안에서 한 줄 압축이 아니라 `개념 하나 → 이 글에서 맡는 역할 하나`의 세로 목록으로 보여 준다. `ContentBoundary`도 결론 preview만 보이고, 열었을 때 article ownership·canonical reuse·evidence rule을 단일 열로 읽게 했다. 이 변경은 특정 모델의 문체에 기대지 않고 569개 공개 글 전체의 composition layer에 적용된다.
- `ai/lora-finetuning`은 overview→LoRA→QLoRA→data→practice 다섯 section을 전부 다시 읽었다. rank를 단순 압축률로 오해하는 지점, QLoRA의 저장·연산·학습 정밀도 구분, double-BOS의 발생 경로, adapter 배포 manifest를 문제→mechanism→실전 한계 순서로 분리했다. 부차적인 snapshot default·paired comparison 조건·직렬화 세부·manifest 예시는 `ProgressiveDetail`로 옮기되 결론과 위험 경계는 본문에 남겼다. prose score 40의 route가 현재 finding에서 빠졌다.
- `ai/open-r1`은 overview·SFT·GRPO·reward·data pipeline·evaluation 여섯 section의 긴 복합 문단을 재작성했다. checkpoint 재현과 recipe 재현의 차이에서 시작해 SFT와 on-policy RL, group-relative advantage와 clipped objective, verifier/sandbox, data lineage, sampling evaluation 순서로 읽히게 했다. Open-R1·TRL·DeepSeekMath·DAPO·LightEval의 역할은 서로의 구현으로 오인되지 않도록 출처 경계를 문장 안에 남겼다. prose score 50의 route도 finding에서 빠졌다.
- known prose compression queue는 326→324로 줄었다. 숫자가 작게 줄어든 이유는 이번 round가 두 route를 깊게 고치는 동시에, 이후 모든 route에서 같은 문제가 반복되지 않도록 전역 shell과 진단 방법을 먼저 바로잡았기 때문이다. 324개는 여전히 완료가 아니라 수동 검토 대기열이다.
- Playwright로 LoRA와 Open-R1을 1440px·390px에서 확인했다. 닫힌 상태에서는 서사 spine이 먼저 보이고, LoRA의 범위 disclosure를 모바일에서 열면 concept 역할이 한 열로 펼쳐졌다. 두 route 모두 horizontal overflow 0, console error 0이었고 전역 `audit:reading --strict`는 범위와 편집 경계가 접혀 있다는 새 invariant까지 검사한다. ESLint, 569개 learning contract, 1,176개 formula, 2,468개 concept·3,743개 relation graph, 65개 listing·150개 prerequisite edge, topology, route 회귀 테스트와 production build 576 route가 통과했다.

### 2026-08-28 · Agent Frameworks·XML Prompting 독해 묶음

- 다음 후보는 총점이 가장 높은 Claw 글이 아니라 `pacingScore`가 높은 `ai/agent-frameworks`와 `ai/xml-prompting`으로 골랐다. Claw 상위권은 긴 문단과 함께 `snapshot`·`receipt` 같은 domain vocabulary가 총점을 크게 올리고 있어, 먼저 jargon 오탐이 거의 없는 두 글에서 새 흐름을 dogfood하는 편이 판단을 왜곡하지 않았다. 수정 전 source finding은 각각 score 66(pacing 60)·52(pacing 52)였고, 운영 브라우저의 220자 초과 문단은 29개·31개였다.
- `agent-frameworks`는 환불 요청을 `업무 단계 → 모델이 제안하는 행동 → runtime이 소유하는 상태·복구` 순서로 다시 열었다. 최소 state는 긴 가운데점 문장이 아니라 식별자·판정 근거·외부 효과의 세 목록으로, ReAct는 Decide·Act·Observe·Verify의 네 역할로 분리했다. LangChain과 LangGraph는 경쟁 이름이 아니라 integration/runtime 계층으로 설명하고 State·Node·Edge·checkpoint·replay를 한 판단씩 끊었다.
- Framework 비교에서 현재 기능표의 version caveat, paired failure set·metric, upgrade manifest는 핵심 선택 결론 뒤의 `ProgressiveDetail` 세 개로 옮겼다. 기본 화면에는 “제품 이름보다 실행 책임을 비교한다”, “같은 workflow로 direct baseline과 짝지어 본다”, “version은 실행 계약과 함께 고정한다”는 결론이 남는다. Crash·duplicate resume·migration처럼 결론을 검증할 세부는 질문을 열면 확인할 수 있다.
- `xml-prompting`은 delimiter·serialization·runtime enforcement를 한 문단에 섞지 않고 차례로 분리했다. 최소 문법, 역할 이름, escaping, document identity, example pairing, runtime control flow, parse→schema→domain→policy, retry·XXE hardening, paired format evaluation을 한 문단 한 판단으로 다시 작성했다. 전체 경로는 한 문장 안의 화살표 열거 대신 5단계 ordered list로 바꿨다. 기존 W3C·Anthropic·Python·OWASP 근거와 사실 경계는 바꾸지 않았다.
- 두 route 모두 prose finding에서 빠져 known queue는 324→322가 됐다. Local Playwright 1440×1000에서 220자 초과 문단은 각각 1개로 줄었고, 390×844에서 overflow 0·console error 0이었다. Agent Frameworks는 progressive detail 5개가 기본 open 0이며, 모바일에서 paired-evaluation detail을 직접 열어 crash 조건이 보이고 overflow가 여전히 0인 것을 확인했다.
- 전체 검증은 ESLint, 569개 learning contract, 2,468개 concept·3,743개 relation graph, 1,176개 formula annotation, 62개 article-contract 이관 후보 감사, topology 529 keep·40 split-review, 65개 listing·150개 prerequisite edge, reading/prose strict audit, route regression test와 production build 5,375 modules·576 static route를 통과했다.

### 2026-08-28 · Office Secretary dogfood와 Image·Video LoRA architecture 보강

- 사용자가 지정한 경로는 `js-dogfood/...`였지만 실제 checkout은 `/Users/dylan/ojs-dogfood/ojs-agents/products/office-secretary`였다. 이 repo에는 기존 변경이 있어 읽기 전용 근거로만 사용했다. `CLAUDE_ARTIFACT_ACCURACY_METHODS.md`, `MODEL_SIZE_DECISION.md`와 실험 artifact를 대조해 `contract → observe → typed artifact → independent check → targeted patch → recheck` 반복 흐름을 추출했다.
- 이 흐름은 새 Agent 글로 분리하지 않고 정본 `ai/llm-harness`에 통합했다. Model proposal과 runtime enforcement를 분리한 뒤 operation을 observation selector·deterministic transform·creative artifact·state mutation 네 역할로 나누고, typed observation·effect receipt·독립 validator·targeted repair를 하나의 실행 loop로 연결했다. Anthropic의 agent tool 작성·eval 공식 글을 함께 근거로 달아 self-critique를 independent verification과 같게 취급하지 않도록 했다.
- Office Secretary 수치는 범위를 고정해 기록했다. 해당 fixture에서 raw `qwen36-27b-fp8` strict count는 27/27, `qwen35-9b`는 0/27이었지만 model generation과 FP8 조건이 함께 달라 model size 일반 법칙으로 확장하지 않았다. 같은 slice를 deterministic rendering으로 바꾼 agent path는 27/27과 variant 135/135를 충족하고 model/tool/token 사용이 0이었다. 이 결과는 “더 큰 모델”보다 먼저 deterministic transform으로 옮길 수 있는 operation을 분리하라는 harness 사례로만 사용했다.
- Image·Video LoRA는 기존 `lora-finetuning → latent-diffusion-guidance → in-context-lora` 사이에 architecture bridge가 없어서 새 정본 `ai/image-video-lora-architecture`를 만들었다. 같은 ΔW=BA 수식이라도 image denoiser의 spatial path, video model의 temporal path, audio-video cross-modal path 중 어디를 target으로 잡는지가 학습 능력을 정한다는 순서로 설명한다. Diffusers 공식 LoRA 예제의 U-Net target list는 보편 규칙이 아니라 예제 범위로, LTX-2 config의 target pattern은 해당 model 설정으로 경계를 표시했다. MotionDirector는 appearance LoRA와 motion LoRA 분리의 논문 근거로 사용했다.
- 새 글은 `latent-diffusion-guidance` 다음, `in-context-lora` 앞에 배치했다. `lora-finetuning`과 `in-context-lora`에도 prerequisite/extension 링크를 추가했고, knowledge graph에 host-target map·image target scope·video spatiotemporal scope·appearance/motion separation·clip evaluation contract 및 harness operation-role·artifact-repair 개념과 관계를 등록했다.
- 모바일 static Viz는 처음 검사에서 세 host card가 세로로 쌓여 1,175px가 되었다. 가로 스냅 카드와 desktop 3열 layout으로 수정한 뒤 390×844에서 354×559px, 내부 scroller 316×303.5px, document overflow 0을 확인했다. 1440×1000에서는 711×523px, overflow 0이었다.
- `llm-harness` interactive Viz는 390×844에서 354×754px, 1440×1000에서 711×864px였다. 1번과 4번 scene을 실제로 눌러 비교했으며 frame 크기와 control 5개의 frame-relative top·left·width·height가 동일했다. 따라서 scene 내용 변화가 클릭 위치를 밀지 않고 frame도 viewport 안에 유지된다.
- 검증 receipt: 변경 파일 ESLint, `audit:viz --strict`, route regression test, 공개 570개 learning contract, targeted graph/order/prose/term-density/reading/topology/formula/article-contract 감사, TypeScript와 production build가 통과했다. Build는 5,377 modules와 577 static route를 생성했다. 전역 legacy article audit의 기존 75개 group queue는 이번 변경과 무관한 known baseline으로 남겼고, 새 route 대상 검사는 통과했다.

### 2026-08-29 · 현대 이미지 모델 → Diffusion LLM → World/Physical AI 연결축

- 이번 흐름은 “현대 이미지 모델을 충분히 알아야 그 위에서 월드모델과 Physical AI를 설명할 수 있다”는 요구에서 시작했다. 처음에는 DiT 구조를 한 글에 덧붙이는 방안도 검토했지만, 그것만으로는 `무엇을 보존하는 visual representation인가`와 `실제 제품을 이루는 component system인가`가 섞인다. 반대로 모델 이름별 백과사전을 만들면 기술이 바뀔 때마다 글 전체가 낡는다. 그래서 비교적 오래 남는 네 경계—representation objective, denoiser tensor contract, end-to-end component interface, temporal/action transition—로 학습 축을 다시 잡았다.
- `ai/visual-representation-tokenizers`는 reconstruction latent와 semantic feature를 같은 “visual token”으로 묶지 않는다. LDM의 perceptual autoencoder를 확립된 출발점으로 두고, semantic encoder를 decoder와 결합하는 RAE는 2025년 preprint라는 경계를 유지했다. 두 표현을 world-state로 승격하려면 pixel·retrieval 점수 외에 object permanence, motion, action sensitivity와 downstream policy 성능을 다시 확인해야 한다는 gate를 넣었다.
- `ai/diffusion-transformer-architecture`는 DiT를 “이미지를 쓰는 LLM”이 아니라 noisy latent와 time·condition을 받아 같은 shape의 velocity/noise를 돌려주는 denoiser로 설명한다. Latent patch 수와 dense attention 비용, adaLN 계열 time modulation, text-image single/dual/joint stream, per-NFE kernel 비용과 총 NFE를 순서대로 분리했다. DiT·SD3/MMDiT는 원 논문에, Krea 2의 single-stream·GQA·gated sigmoid attention·SwiGLU·normalization·3D axial RoPE 선택은 2026년 6월 제작사 technical report의 자기보고에만 귀속했다.
- `ai/modern-image-model-stack`은 prompt expander→condition encoder→autoencoder latent→DiT→solver→decoder를 한 request path로 조립한다. Shape가 맞는다는 이유만으로 component가 호환되는 것은 아니므로 latent scale·channel·feature distribution, condition width, prediction target, time convention과 scheduler revision을 하나의 compatibility receipt로 고정했다. Krea 2의 256→512→1024 curriculum과 post-training 세부는 기본 서사를 끊지 않도록 질문형 `ProgressiveDetail`에 넣었고, final single stream을 현대 이미지 모델의 보편 최적 구조로 일반화하지 않았다.
- 사용자가 추가 요청한 `ai/diffusion-language-models`는 image diffusion의 단순 부록으로 처리하지 않았다. AR의 left-to-right factorization과 masked discrete diffusion을 먼저 대비하고, absorbing MASK forward process, weighted masked-token objective, low-confidence remasking, full-sequence 재계산과 KV-cache 한계, Block Diffusion 절충을 독립 arc로 만들었다. MDLM은 NeurIPS 2024, Block Diffusion은 ICLR 2025 근거로 두고, LLaDA·Dream의 결과는 각 preprint·checkpoint·sampler 범위를 넘지 않게 했다. Diffusion LLM의 양방향 수정 능력을 곧바로 physical planning·world transition 능력으로 승격하지 않는 경계도 명시했다.
- 읽기 목록은 topological prerequisite 순서로 실제 UI에 `15 Visual representation → 16 Diffusion Transformer → 17 현대 이미지 stack → 18 Diffusion LLM → 19 Image·Video LoRA → 20 In-Context LoRA`로 배치됐다. 기존 LoRA 글도 modern stack과 DiT를 선수지식으로 읽도록 연결했다. Article·evidence·editorial ownership·learning contract·knowledge graph CRUD를 같은 반복에 수행해 공개 글은 574개, graph는 2,491 concepts·3,787 relations가 됐고 새 글은 각 기본 6·심화 4문제를 가진다.
- 첫 prose audit는 현대 stack 12점, DiT 6점, Diffusion LLM 6점의 압축 신호를 잡았다. 긴 복합 제목을 줄이고 한 문단 한 판단으로 다시 썼으며, 구조명·학습 단계의 세부 목록은 펼쳐 읽는 설명으로 옮겼다. 재감사에서 새 네 글은 모두 finding에서 빠졌고 known legacy queue도 324→321로 줄었다. 새 글 4개만 대상으로 한 article-contract 감사도 이관 후보 0개다.
- Viz는 내용 길이가 frame과 버튼을 밀지 않게 다시 측정했다. Mobile 390×844에서 custom figure는 visual representation 816.98px, modern stack 798.98px, DiT 739.13px, Diffusion LLM 754px로 모두 viewport보다 작다. 긴 static lane은 frame 안에서만 scroll한다. Diffusion LLM scene 01→04 전환 전후 control의 frame-relative x·y·width·height는 0px 변화였고 document overflow·KaTeX error·console warning/error는 모두 0이었다. Krea 2 progressive detail도 desktop과 mobile에서 닫힌 preview→열린 상세를 직접 확인했다.
- 검증은 `audit:graph`, 네 route의 `audit:learning`, `audit:formula --require-explicit`, `audit:viz`, `audit:reading`, `audit:order`, `audit:prose`, `audit:terms`, `audit:topology`, route regression test와 새 글 대상 `audit:articles`를 통과했다. 변경 파일 ESLint와 production build도 통과했으며 5,385 modules와 static public route 581개, 404 fallback을 생성했다. 1440×1000·390×844에서 네 route를 실제 browser로 렌더링하고 full-page/focused screenshots를 `output/playwright/`에 남겼다.

### 2026-09-11 · Qwen3.8-Flash-Next 아키텍처 신규와 직전 회차 게이트 정리

- 블로그에 Qwen3.6-27B 3부작은 있지만 2026-08 공개된 Qwen3.8 계열이 전무해 `ai/qwen38-flash-next-architecture`를 새로 썼다. 사용자가 "1편씩 순차"를 택했으므로 Flash-Next 아키텍처 한 편만 끝까지 마무리하고, Flash-Next 서빙 런타임과 Qwen3.8-27B 편은 착수하지 않았다.
- 근거는 공식 config.json(48 layer_types·num_experts 512·indexer_budget 2048·hc_count 4·ple_layer_ids)과 가중치 색인의 total_size 359,999,963,128 바이트, 그리고 huggingface/transformers의 `qwen4_exp` reference 구현이다. 이 글은 `ai/` 카테고리에서 처음으로 `codebase/` 스냅샷을 pin 했다. `modular_qwen4_exp.py`와 `configuration_qwen4_exp.py`를 커밋 f62dc9bf2c90 기준으로 고정하고 `codeRefs` 7개·`CodeViewButton` 7개로 indexer·gated residual·PLE·decoder layer의 실제 계산 경로를 열람하게 했다. contract 2.3의 코드 근거 요구를 개념 글에도 적용한 첫 사례다.
- 본문은 배치(선형 36·희소 12) → 선택(압축 블록 색인과 예산 2048→블록 512→자리 2051) → 배선(hc_count 4·hc_lowrank 320) → 어휘 보강(해시 n-gram 3.2억 행·512억 파라미터·95.4 GiB, 토큰당 16행) → 파라미터 회계(backbone 125.5B·조회 표 51.2B·활성 약 58억) → 요청 상태(토큰당 K/V 24 KiB·indexer key 3 KiB·고정 108 MiB) 순서다. 파라미터·상태 byte는 전부 공개 config의 텐서 모양과 dtype으로 센 논리값임을 본문과 ownership evidence rule에 명시했다.
- 선행 논문은 Native Sparse Attention(2502.11089)과 Hyper-Connections(2409.19606) 둘만 넣고, 각각 "QSA가 다르게 놓인 지점"과 "갈래 4·랭크 320이 최적이라는 근거는 없음"을 비주장으로 적었다. 공개 자료 일부가 언급하는 optimizer 관련 주장은 공식 model card·config에서 확인되지 않아 본문에 넣지 않았다.
- Viz 6개는 모두 `VizFrame + useAnimatedScenes` 장면형으로 새로 그렸다. 브라우저 검수 중 두 건을 고쳤다. QSA 그림의 꼬리 토큰 상자가 마지막 블록의 index key와 겹쳐 있어 완전한 블록 7개와 미완성 꼬리를 분리했고, 파라미터 그림은 칸 하나가 expert 8개인데 11개를 칠해 활성 수를 왜곡하고 있어 512칸 격자에서 정확히 10칸만 칠하도록 바꿨다.
- 직전 회차가 남긴 `gpu/modded-rtx4090-moe-serving`의 미완료 게이트를 함께 닫았다. learning contract가 없어 `audit:learning --all-articles`가 실패하고 있었으므로 concept 7개(clamshell 배치·밀도와 대역폭 분리·소비자 GPU 인터커넥트 후퇴·MoE와 dense의 민감도 차이·비공식 개조 지원 경계·통신 병목에서의 병렬화 전환·용량 대 통신 판단)와 기초 6·심화 4문제, 근거 지도를 등록했다. prose baseline은 본문 변경 없이 fingerprint만 어긋나 있었고 score 36은 그대로 두고 fingerprint만 맞췄다. 두 글 모두 topology `keep` 결정과 fingerprint를 등록했다.
- 검증: `audit:learning --strict --require-registration --all-articles` 648개 통과, graph·formula·reading·order·prose·terms 통과, `audit:viz --strict` ERROR 0(REVIEW 46), tsc·ESLint·production build 통과(655 static route). Playwright 1440×1000·390×844에서 horizontal overflow 0·console error 0을 확인하고 Viz 6개의 마지막 장면과 CodeSidebar 열람을 직접 캡처해 `output/playwright/qwen38-*.png`에 남겼다. topology 전역 감사에는 이번 변경과 무관한 기존 stale-decision 48건이 남아 있다.

### 2026-09-11 · 비전 시리즈 1편 · DINOv3 자기지도 백본

- 사용자가 확정한 비전 5편 대기열의 첫 글로 `ai/dinov3-self-supervised-backbone`을 썼다. 저장소에 DINO 계열 정본이 0건이었고 기존 `visual-representation-tokenizers`는 DINO를 스쳐가는 참조로만 언급한다. `ai-practical-cv`의 `vision-transformer` 바로 뒤에 배치했다.
- 글의 축은 "라벨이 없는데 무엇을 정답으로 삼았고, 오래 학습할 때 무엇이 무너졌으며, 어떻게 붙잡았는가" 하나다. 이미지 수준 목표(EMA teacher 크롭 분포 정합)와 패치 수준 목표(가린 자리의 teacher 분포)를 먼저 세우고, 두 목표의 방향 차이가 만드는 dense feature 붕괴를 정의한 뒤, 패치 유사도 행렬을 기준 teacher와 맞추는 Gram anchoring으로 닫는다. 사후 해상도 적응과 증류 계열, 얼린 backbone 평가 조건이 뒤를 받친다.
- 코드 근거는 `facebookresearch/dinov3` 커밋 11c58638의 `gram_loss.py`·`dino_clstoken_loss.py`·`ibot_patch_loss.py`·`koleo_loss.py`·`dino_head.py`를 `codebase/dinov3`에 pin 하고 codeRefs 5개로 열람한다. Qwen3.8 글에 이어 `ai/` 카테고리의 두 번째 소스 스냅샷이다. 구현과 논문 표기가 다른 지점(Frobenius 제곱 대신 MSE, 음수 유사도 절단 옵션)은 본문과 주석에 그대로 적었다.
- 수치는 공개 기술 보고서 값만 썼다. ViT-7B의 6.7B·4,096차원·40블록·32 head·패치 16·register 4, LVD-1689M의 16억 8,900만 장, 배치 4,096과 100만 스텝, Gram anchoring 시작 시점 100만 스텝과 1만 스텝 갱신, 해상도 적응 1만 스텝(global 512·768 / local 112~336), 증류 계열 21M~0.8B가 그것이다. 붕괴와 개선은 저자 자기보고로 표시하고 모든 자기지도 학습의 일반 법칙으로 확대하지 않았다.
...
### 2026-09-11 · 비전 시리즈 5편(종결) · 비전 백본 선택

- `ai/vision-backbone-selection`을 `ai-practical-cv`의 DINOv3 뒤에 배치해 5편 시리즈를 닫았다. 앞 네 글에서 세운 개념(얼린 평가·캡션 감독·개념 분할·임베딩 지문)을 선택 기준으로 묶는 캡스톤이다.
- 축은 "순위표가 아니라 세 축의 교집합" 하나다. 학습 목표가 남긴 능력 → 과제가 요구하는 능력 → 배포가 감당할 비용으로 후보를 좁히고, 조건을 맞춘 30분 실측으로 결정하며, 교체 비용까지 더해 판단 순서를 고정한다.
- 계열 비교는 정성 요약임을 명시했다. 자기지도·캡션 정렬·분할 감독 세 계열의 능력 프로필은 각 원 논문의 자기보고를 네 축으로 요약한 것이고 특정 두 모델의 우열 판정이 아니라는 규칙을 ownership에 적었다. 선택 점수 식도 판단을 구조화하는 제안이지 검증된 공식이 아님을 표시했다.
- 이 글은 codebase 스냅샷을 두지 않았다. 특정 오픈소스의 동작을 추적하는 글이 아니라 판단 절차를 다루는 글이라 contract 2.3의 코드 근거 요구 대상이 아니다.
...
### 2026-09-11 · 하드웨어 비교 3편 · 현장 준비 조건

- `gpu/datacenter-site-readiness`를 `hw-infra`의 `hw-power-cooling` 뒤에 추가해 하드웨어 트랙을 닫았다. 사용자가 지목한 항목 중 저장소에 0건이던 것들(블로워 등 카드 냉각 타입 비교, 바닥 하중, 내진)을 이 글이 담당한다.
- 축은 "서버 사양서 밖에서 막힌다"다. 확인 주체가 IT와 설비로 갈려 서버 쪽 숫자와 건물 쪽 숫자가 만나지 않는 것이 원인이라는 진단에서 시작해, 냉각 방식 → 전력 사이징 → 바닥 하중 → 내진 순서로 계산 가능한 형태를 만들고, 마지막에 "바꾸기 어려운 제약부터 확인한다"는 역순 절차로 닫는다.
- 계산 두 개가 글의 중심이다. 회로 용량은 지속 부하 합에 여유율을 곱하고 경로 분담률로 나누며, 이중 급전을 절반씩 설계하면 한쪽 상실 시 남은 경로가 두 배를 받아 차단된다는 점을 명시했다. 하중은 같은 무게를 면적과 접지점 수로 각각 나눠(1,400 kg / 0.72 m² ≈ 1,944 kg/m², 접지점당 350 kg) 두 허용치와 비교한다.
- 내진은 공개 시험 사양의 수평 0.8 g·수직 1.0 g 조건을 인용하되, 요구 등급과 인정되는 고정 방식은 지역 건축 기준이 정하며 이 글이 규정 해석을 대신하지 않는다고 본문·ownership에 적었다. 하중·전력 숫자도 계산 방법 예시임을 명시했다.
- 기존 `hw-power-cooling`과의 경계: 랙 단위 전력 분배·발열 계산·기구 점검 체크리스트는 그 글이 소유하고, 이 글은 장비를 들이기 전 건물 쪽에서 확인할 물리 조건만 다룬다.
...
### 2026-09-11 · 확산 편집 실측 시리즈 2편 · 편집 동작과 모델 라우팅

- `ai/masked-edit-verb-routing`을 1편 뒤에 추가했다. 축은 "마스크 편집은 요구가 서로 다른 여섯 동작이고, 선택을 사람의 기억이나 언어 모델의 인상이 아니라 측정 표에 맡겨야 한다"이다.
- **원본 기록의 수치 오류 2건을 실제 산출물 재계산으로 잡아 정정했다.** 이 시리즈에서 원본을 그대로 옮기지 않고 `profiles.json`을 다시 계산하는 이유가 여기 있다.
  1. 손 힌트 기여도: 원본 요약은 "klein +1.9, 나머지 ±0.2"였으나 재계산하니 qwen_edit가 +1.05(4.46→5.51)였다. 다섯 모델은 −0.13~+0.16이 맞다. 본문에 정정을 명시하되 qwen의 두 값이 모두 그 모델의 무동작 범위라 성격이 다르다는 해석도 함께 적었다.
  2. 보수형 모델의 최저 빈도: 원본은 "6개 동작 중 5개에서 최저"였으나 실제로는 **4/6**이다(재질 20.80·지우기 23.04·더하기 4.46·사실감 6.28). 색 변경은 illustrious 27.74에 0.61 차로 2위, 물건 교체는 55.51로 오히려 최대급이다. 교체 예외는 "많이 바꿨다 ≠ 요청을 따랐다"의 사례로 본문에 별도 문단을 뒀다.
- 1편과의 연결이 이 글의 뼈대다. 마스크 밖 변화량은 1편에서 오토인코더 바닥값으로 무효화됐으므로 모델 선택 근거에서 제외했고, 대신 마스크 안 변화량과 그림 확인 두 가지만 썼다. 손 힌트 열은 같은 조건 두 실행의 차분이라 공통 오염항이 상쇄되어 그대로 살아남는다는 점을 `ExplainedFormula`로 고정했다.
- 실무에서 가장 반직관적인 결과: 마스크 확장 방향이 동작마다 반대다. 교체는 96픽셀 열어야 관절식 각반이 나오고(73.3→44.8→32.4), 지우기는 24픽셀만 넓혀도 아래 레이어까지 사라진다. 같은 파라미터가 한쪽에서 필수·다른 쪽에서 금지라 두 동작이 같은 도구에 들어갈 수 없다.
- 지표 스케일의 그림체 의존(같은 색 변경이 3D 35.0·애니 63.4)을 근거로 절대 임계값 사용을 금지하고, 얼굴 전체 마스크 생성 요청은 연산 전 거절 가드로 막았다.
- 브라우저 검증: MatrixViz 실측 42칸을 화면에서 직접 읽다가 위 2번 오류를 발견했다. 정적 audit으로는 잡히지 않는 종류다.
- 검증: 전 audit 통과, `audit:viz --strict` ERROR 0, topology `keep`+fingerprint, tsc·eslint·build 통과(671 static route), Playwright 1440·390 overflow 0·error 0, viz 6개 24장면 전수 캡처 확인.

### 2026-09-11 · 확산 편집 실측 시리즈 3편 · 지우기는 인페인팅이 아니다

- `ai/removal-is-not-inpainting`을 2편 뒤에 추가했다. 2편의 매트릭스에서 통째로 비어 있던 지우기 칸이 이 글의 출발점이다.
- 축은 "일곱 모델이 같은 방식으로 실패하면 여덟 번째를 찾는 대신 도구 범주를 의심한다"이다. 배경을 설명하는 프롬프트와 물건 이름 네거티브를 함께 줘도 네 그림체 전부에서 띠가 다시 그려졌고(마스크 안 44.9~93.7), 답은 조건을 받지 않는 전파 방식이었다.
- 원본 산출물에서 확인한 수치: `remove.json`의 LaMa 결과(마스크 안 34.6~61.8, 마스크 밖 0.15~1.03, 1~3초), `knobs.json`의 세기 절벽(254에서 0.525, 255에서 115.66)과 확장 스윕(0에서 0.157, 96에서 4.12).
- **통제 실패 하나를 숨기지 않고 본문에 넣었다.** 이 실행의 네거티브는 안내 계수 1.0 탓에 계산에서 상쇄되어 무효였다. `ExplainedFormula`로 `ε_neg + 1·(ε_pos − ε_neg) = ε_pos`를 보이고, 양의 배경 프롬프트는 정상 적용됐으므로 결론은 유지된다는 점까지 적었다. 증류된 빠른 모델 사용자가 흔히 밟는 함정이라 개념으로 소유하게 했다.
- 도구 계약 셋(프롬프트 미수용·확장 기본 0·세기 254 클램프)이 전부 기능을 빼는 결정이라는 점, 그리고 "다른 도구가 합니다"와 "아무도 못 합니다"를 다른 응답으로 구분한 이유를 정리했다.
- 2단계 조합(전용 망 → 확산 리파인) 기각이 이 글의 방법론 축이다. 게이트 셋 중 좁힌 것과 넓힌 것의 결과가 소수점 첫째 자리까지 같았고(38.3/38.3·33.0/33.0·61.3/61.5·56.0/56.1), 유일하게 결과를 바꾼 팔은 지운 물건을 되살렸다(3D 93.2가 전 팔 최고점인데 흰 띠를 새로 그린 것). 실행 전에 적어 둔 예측이 그대로 맞았다는 점을 `prewritten-prediction-discipline` 개념으로 남겼다.
- `verdicts.json`에서 1편의 계측기 실패가 그대로 확인된다 — VL 판정기의 자기 대조군 4건 중 2건이 오답(sphoto_control·sart_control이 REMOVED), 그리고 프레임을 파괴한 strength 255 결과 4건 전부가 CLEAR로 채점됐다. 이 회차에 자동 판정기가 없다는 사실과 그래서 정량 실패율을 주장하지 않는다는 점을 overview·gate 양쪽에 박았다.
- 브라우저 검증에서 잡은 결함 2건: LamaViz의 255 막대 라벨이 반올림돼 본문 115.66과 어긋나 소수 둘째 자리로 통일했고, TwoStageViz의 열 머리글이 부제와 겹쳐 행 간격을 재배치했다.
- 검증: 전 audit 통과, `audit:viz --strict` ERROR 0, topology `keep`+fingerprint, tsc·eslint·build 통과(672 static route), Playwright 1440·390 overflow 0·error 0, viz 6개 24장면 전수 캡처 확인.

### 2026-09-11 · 확산 편집 실측 시리즈 4편 · 해상도 예산과 영역별 리파인

- `ai/roi-resolution-identity-budget`을 3편 뒤에 추가했다. 축은 "편집의 성패를 가르는 단위는 프레임 해상도가 아니라 대상 해상도"다.
- 중심 수치는 같은 리파인 설정에서 전신 1MP 0.265 → 전신 4MP 0.318 → 얼굴 패널 0.531이다. 총 픽셀이 훨씬 적은 패널이 더 좋다는 사실이 "프레임을 키우는 것과 대상을 키우는 것은 다르다"를 보인다. 토큰 예산 식 `N = (s/(f·p))²`로 95px과 327px의 면적 차이가 열 배 이상임을 고정했다.
- 앞 회차 결론 하나를 **좁혔다**. 3편까지 "노이즈 비율은 레버가 아니라 절벽"이라고 일반화했는데, 그건 지시 편집 모델에 대한 이야기였다. 일반 생성 모델에서는 레버가 맞고 창이 좁을 뿐이며(모델 A는 0.25만, 모델 B는 0.40까지), 창의 폭 자체가 모델 선택 기준이 된다.
- 가장 실용적인 숫자는 마스크 하나로 정체성이 0.142 → 0.943이 된 것이다. 모델·프롬프트·시드가 전부 같고 마스크만 눈~입 박스에서 뺨 띠로 좁혔다. 부위 편집 실패의 상당수가 모델이 아니라 범위 지정에서 나온다.
- 업스케일은 정답을 아는 실험으로 했다(원본을 1/4로 줄여 복원 후 원본과 비교). 픽셀 오차가 가장 흐린 보간법을 1등으로 뽑고(31.2dB인데 엣지 0.55배), 엣지 에너지로 보면 순위가 뒤집힌다(복원 전용 1.07배). 타일 재생성은 noise 0.25인데 정체성 0.781로 복원이 아니라 재해석이라 제외했다. 품질이 한 축이 아니므로 도구가 방법 대신 목적(크게/되살리기/빠르게)을 받게 했다.
- 브라우저 검증에서 잡은 결함 3건: WindowViz의 계열 라벨과 값이 겹쳐 범례를 상단으로 빼고 값 위치를 계열별로 위아래로 나눴고, TokenViz의 20×20 격자가 viewBox를 넘어 셀 크기와 행 간격을 줄였으며, 같은 viz에서 축 범위를 넓혀 우측 값 겹침을 해소했다.
- 검증: 전 audit 통과, `audit:viz --strict` ERROR 0, topology `keep`+fingerprint, tsc·eslint·build 통과(673 static route), Playwright 1440·390 overflow 0·error 0, viz 6개 24장면 전수 캡처 확인.

### 2026-09-11 · 확산 편집 실측 시리즈 5편 · 정체성과 포즈 분리

- `ai/reference-identity-pose-separation`을 4편 뒤에 추가했다. 축은 "참조 잠재 조건이 인물과 자세를 함께 전달하므로 둘을 동시에 통제할 수 없다"이다.
- 참조의 효과는 크다(얼굴 정면 0.845 대 대조군 0.284, 양쪽 회전각 0도라 교란 없는 비교). 대가는 몸까지 고정되는 것이고, 문구를 인물 회전에서 카메라 회전으로 바꿔도 59.5→59.9도·28.3→28.0도로 측정 오차 수준이라 문장 문제가 아님이 확정됐다.
- **이 회차의 방법론 축은 "잘못 접을 뻔한 기록"이다.** 정체성을 초반 구간에만 걸어 보고 "이 방식은 여러 각도에 못 쓴다"고 결론낼 뻔했는데, 구조와 방향이 정해지는 것이 바로 그 초반이라 질문이 거꾸로였다. 반대쪽(늦게 켜기)을 돌리자 측면 회전이 22.2→62.7→71.9도로 풀렸다. 다만 정체성이 0.399→0.376→0.100으로 떨어져, 최종 답은 세기를 1.3에서 0.7로 낮춘 전 구간 적용(49.4도·0.464)이었다.
- 후면 뷰 원인 진단도 틀렸다. 정체성 주입의 정면 편향으로 봤는데 주입을 꺼도, 좌표에서 얼굴 지점을 지워도 같았다. 실제 원인은 관절 좌표가 앞뒤 거울 대칭이라는 것과, 신호를 격리하려고 프롬프트에서 시점 표현을 빼 둔 것이었다. 격리 자체는 옳았고 그 조건의 결과를 실제 파이프라인 한계로 읽은 것이 문제였다.
- 세 오진이 전부 같은 절차로 풀렸다 — 의심되는 신호를 완전히 꺼 보기. `ablation-before-tuning` 개념으로 남겼다.
- 최종 구성(관절 좌표+시점 문구+세기 0.7 주입)에서 네 각도가 전부 성립한다. 후면의 "얼굴 미검출"을 실패가 아니라 성공 신호로 읽는데, 이 판정에 1편의 탐지·인식 구분이 필요하다는 점을 명시했다.
- 검증: 전 audit 통과, `audit:viz --strict` ERROR 0(1.5 stroke 5건 수정), topology `keep`+fingerprint, tsc·eslint·build 통과(674 static route), Playwright 1440·390 overflow 0·error 0, viz 6개 24장면 전수 캡처 확인.

### 2026-09-11 · 확산 편집 실측 시리즈 6편 · 정체성 다양성의 출처

- `ai/generative-identity-diversity`를 5편 뒤에 추가했다. 이 글의 구조 자체가 **같은 질문에 대한 세 번의 정정**이며, 결론보다 뒤집힌 과정을 서사로 삼았다.
  1. "모델당 한 명" — 프롬프트를 고정한 채 시드만 돌린 실험이었음
  2. "프롬프트당 한 명" — 여섯 명은 15쌍이라 10% 충돌률을 볼 수 없었음
  3. "묘사 72개가 실질 6개" — 증류된 가중치에서 측정했음
- 최종 답은 "정체성은 조건의 함수이고 사상은 다대일, 영향력은 프롬프트 > 모델 > 시드이며 가중치가 프롬프트의 도달 범위를 정한다"이다.
- 원본 산출물 `compare_72.json`·`raw_vs_turbo.json`에서 전부 확인했다. 증류 18명/215쌍 충돌/Vendi 28.5 대 비증류 61명/8쌍/51.2, 구간별 충돌 27~38% → 0~7%(남성 세 구간 0). 24묘사 A/B에서도 5/22 대 16/20이다.
- 혼합 실패를 구조로 설명했다. 볼록 결합이라 결과가 두 원본을 잇는 선분 위에만 놓여 전체 평균에서 더 멀어질 수 없다는 점을 식으로 고정했고, 실측(단독 2명·혼합 0명, 혼합끼리 0.549 대 단독끼리 0.437)이 그 예측과 맞는다.
- 배선 함정 하나를 개념으로 남겼다 — 두 프롬프트의 조건 텐서를 섞을 때 구현이 한쪽을 다른 쪽 토큰 길이에 맞춰 잘라 내 정렬이 깨진다. 단독과 자기 자신과의 혼합은 정상이라 증상이 늦게 드러나고, 깨진 결과에서 "비대칭도 단조 감소" 같은 그럴듯한 지표가 나온다.
- 일러스트 계열을 모델 간 비교에서 제외한 근거를 1편의 스타일 적용 범위 측정에 걸었다. 포함하면 수치가 부풀려진다는 점을 명시했다.
- 검증: 전 audit 통과, `audit:viz --strict` ERROR 0, topology `keep`+fingerprint, tsc·eslint·build 통과(675 static route), Playwright 1440·390 overflow 0·error 0, viz 6개 24장면 전수 캡처 확인.

### 2026-09-11 · 확산 편집 실측 시리즈 7편 · 3D 얼굴 제어 실패 기록

- `ai/negative-result-3d-face-control`로 시리즈를 닫았다. 이 글은 **음성 결과 정본**이며, 실패 자체보다 그것을 확인한 세 절차가 내용이다.
- 합격 기준을 실행 전에 정해 뒀다는 점이 서술의 축이다 — "극단적으로 다른 네 얼굴이 출력에서도 서로 구분되는가". 처음에 "부분 성공"이라 적었던 판정이 이 기준에 명확히 미달이었음을 본문에 정정으로 남겼다.
- 세 절차가 각각 한 번씩 결론을 바꿨다.
  1. **추측 대신 측정** — 진폭을 두 번 키워도 그대로여서 세 번째 대신 잼. 중심선 최대 깊이 1.181로 코 존재 확인, 앞쪽 면 5,986개 중 법선이 카메라를 향한 것 0개로 감김 순서 역전 특정. 1차 조립식도 같은 버그였고 부품별 뒷면 실루엣이 우연히 얼굴처럼 보였을 뿐이었다.
  2. **시드 대조군** — 강도 스윕이 0.676→0.376→0.229로 단조 감소해 성공처럼 보였는데, 얼굴형을 고정하고 시드만 바꾸니 0.346·0.300으로 구분되지 않았다. 형태 지표도 4.4% 대 4.8%·4.5%.
  3. **끝점 이미지 확인** — 분리가 가장 컸던 강도 1.00에서 대조군 네 장 중 세 장이 얼굴 미검출. 머리가 공, 눈 홈이 안경. 낮은 유사도의 상당 부분이 "다른 사람"이 아니라 "망가진 이미지"였다.
- 기하 신호 전달률을 `η = CV_out / CV_in`로 두어 64.8% → 1.6%(약 97% 소실)을 고정했고, 여섯 쌍 정체성 최솟값 0.728로 넷 다 같은 사람 범위임을 보였다.
