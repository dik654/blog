...
이후 범위를 전체 블로그로 확장했다. 2026-08-14 현재 audit가 발견한 article entry는 400개다. Knowledge graph와 learning contract가 등록된 글은 101개이고 아직 contract가 없는 글은 299개다. 이 가운데 새 DoD의 정확한 기초 6개+심화 4개와 본문·근거 anchor까지 strict audit를 통과한 글은 50개다. 이전 문제 수 기준으로 등록된 51개는 보완 대기다. Audit는 category root의 재사용 Viz component를 article route로 잘못 세지 않도록 제외하며, 새 route가 추가되면 분모도 함께 갱신한다. 또한 같은 concept를 두 글이 동시에 새 개념으로 소유하거나 concept의 `canonicalHref`가 실제 owner와 어긋나거나 새 concept가 relation edge 없이 고립되면 해당 글을 완료로 처리하지 않는다. 현재 902개 concept·1,375개 relation은 정본 owner 중복·경로 불일치·dangling/self/duplicate edge·고립 node가 모두 0개이며 `audit:graph --strict`를 통과한다. 완료된 정본·응용 글의 상세 근거는 아래 한 곳에서만 관리하므로, 아래 기존 체크 표시는 해당 글의 과거 검수 이력이며 현행 6+4 재검수 전에는 전체 DoD 완료를 뜻하지 않는다.
...
- [x] Knowledge graph 전용 `audit:graph --strict`: 902개 concept·1,375개 relation의 owner·canonical path·edge·고립·화면 stage coverage 통과
- [x] 현재 strict 완료 50개 글은 canonical owner·anchor·entry-level 재귀 선수 경로·기초 6개+심화 4개 coverage 통과
- [ ] 이전 learning contract 51개를 정확한 6+4 문제와 article-only 역검사 기준으로 재개방해 보완
- [x] 확률·통계 기반 글은 optimizer 소비 경로에서 필요한 범위까지 첫 canonical closure 완료
- [ ] 물리·컴퓨터 구조·분산 시스템의 기반 글은 실제 소비 글을 이관하면서 필요한 node부터 같은 방식으로 계속 확장

- [ ] Foundations·NLP: 완료된 기반은 위 재귀 경로에 기록. 남은 ARIMA·LSTM time-series 등은 새 learning contract 기준으로 재검수
- [x] Generative: 생성 모델 전체 지도·deterministic autoencoder·VAE·GAN·Diffusion을 새 learning contract 기준으로 재검수
- [ ] Practical ML: data augmentation·imbalanced data·GBDT·tabular/sequence modeling·transfer learning·LR scheduling·regularization·image/video/multiview·contrastive/sentence embedding·quantization·pruning·distillation·RAG·LoRA
- [ ] LLM·Agents: Open-R1·ViT·YaRN·Agentic patterns·hybrid attention serving·SAE·vLLM speculative decoding
- [ ] From scratch: dezero-nn·dezero-advanced
...
- [ ] 전역 audit:learning --require-registration --all-articles: 400개 중 101개 등록, 299개 미등록. 등록 글도 exact 6+4·article-only 역검사·근거 anchor를 통과할 때만 완료
