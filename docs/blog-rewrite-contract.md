...
- 긴 용어 정의는 좁은 다중 열에 밀어 넣지 않는다. 390px와 1440px 실제 article 폭에서 각 용어 카드가 최소 250px을 확보하고, 정의·현재 글에서의 역할을 읽을 수 있는 본문 크기와 행간으로 렌더링돼야 한다.
...
4. 기호 각각의 역할
5. 성립 전제와 생략한 차원·조건
6. 식에서 읽어야 할 결과와 읽으면 안 되는 과도한 결론

새 본문에서는 `ExplainedFormula`를 사용한다. 식을 먼저 보여 주고 아래에서 사후 설명하지
않는다. 단순 표기나 이미 충분히 설명된 inline math에는 이 블록을 반복하지 않는다.
