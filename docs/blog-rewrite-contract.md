...
- 글은 정확한 항목을 많이 싣는 artifact ledger가 아니라 독자가 한 생각씩 따라가게 하는 설명이다. `claim → 왜 그런가 → 작은 예 → 여기서 생기는 한계 → 다음 질문`의 연결을 본문 문장으로 드러낸다.
- 한 문단은 하나의 중심 생각을 맡긴다. 문장 수를 기계적으로 제한하지 않지만, 원인·관찰·결론이 서로 다른 문단으로 흩어지거나 반대로 독립 주장 세 개 이상이 한 문단에 압축되면 나눈다.
- 새 절은 앞 절의 결과가 왜 다음 질문을 만드는지 한 문장으로 이어 준다. `여기서 중요한 점`, `하지만 이 설명만으로는`, `예를 들어`, `따라서` 같은 전환은 장식이 아니라 논리 관계가 실제로 있을 때 사용한다.
- 핵심 결론은 처음 한 번만 암호처럼 선언하고 지나가지 않는다. mechanism을 설명한 뒤 더 쉬운 말이나 작은 수치 예로 한 번 재진술해, 반복이 아니라 이해 확인 역할을 하게 한다.
- 표·카드·칩·definition list는 이미 설명한 내용을 비교하거나 찾아보기 쉽게 정리할 때만 쓴다. 독자가 처음 만나는 인과관계, 반례와 선택 이유를 표에 먼저 압축하지 않는다. 표를 제거했을 때 논리가 사라진다면 본문 설명이 빠진 것이다.
- Evidence rail, contract, artifact, receipt, fixture, gate 같은 편집·운영 용어를 모든 글의 결론 문법으로 반복하지 않는다. 주제 자체에 필요한 경우에만 첫 등장 때 쉬운 말로 정의하고, 독자 질문에 직접 답하는 표준 용어를 우선한다.
- 제목과 소제목은 절의 질문 하나를 예고한다. 아직 설명하지 않은 약어·방법·운영 판정을 세 개 이상 한 제목에 결합하지 않는다.
- 줄 수·section 수·citation 수·term density는 누락을 찾는 신호일 뿐 글의 품질 목표가 아니다. 내용을 늘릴 때도 새로운 정보가 앞 문장의 질문에 답하거나 다음 판단을 가능하게 할 때만 남긴다.
- 본문에는 접지 않아도 이해되는 `always-visible spine`을 둔다. 문제, 핵심 mechanism, 결론, 중요한 한계와 다음 절로 이어지는 이유는 기본 화면에서 읽혀야 한다.
- 증명 세부, 논문별 실험 조건, 구현 차이, 긴 실패 목록처럼 중요하지만 첫 독해의 흐름을 끊는 내용은 `ProgressiveDetail`로 내려 필요할 때 펼치게 한다. 접힌 영역은 본문 누락을 숨기는 창고가 아니며, 그 내용을 몰라도 바로 앞뒤 문장이 이어져야 한다.
- 펼침 제목은 `자세히 보기`처럼 모호하게 쓰지 않고 독자가 열어서 답을 얻을 질문을 적는다. 접힌 상태의 preview에도 결론 한 줄을 남겨, 열지 않은 독자도 중요한 경계와 위험을 놓치지 않게 한다.
- 안전 경고, 사실 여부를 뒤집는 단서, 필수 선수 개념, 글의 유일한 수치 근거는 기본값으로 숨기지 않는다. 같은 상세가 여러 글에서 반복되면 접힘을 복사하지 말고 canonical article로 분리한다.
...
- 현재 글의 주제가 아닌 선행 system·경쟁 구현·인접 방법을 가져오면 근거 지도에서 `선행·비교 논문`으로 구분하고, 본문 첫 문장에 왜 등장하는지와 현재 주제에 물려주는 정확한 아이디어를 쓴다. 이어서 `현재 system의 내부 구성 요소가 아님`, `같은 이름의 model이 아님`, `현재 구현이 그 architecture를 그대로 쓴다는 뜻이 아님`처럼 혼동 가능한 경계를 명시한 뒤 현재 주제의 고유 기여로 돌아온다.
- 계보를 설명할 때는 `선행 연구가 해결한 문제 → 현재 주제가 재사용한 원리 → 현재 주제가 새로 해결한 병목`을 분리한다. 관련 논문을 많이 나열하는 것은 계보 설명이 아니며, 연결 문장이 없으면 삭제하거나 별도 비교 절로 옮긴다.
...
- [ ] 문단마다 중심 생각이 하나이고, 절 사이의 원인·예시·한계·다음 질문이 연결 문장으로 이어진다.
