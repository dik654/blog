...
| 9(wave4 ay~bf) | ai(192개 파일), 8개 서브배치(light 1·standard 2·heavy 5, 전부 finalize) | 664 추출 → 404 반영 | 등급 A×8 · 게이트 golden FAIL 1건 오탐 확인 후 진행 | `31f41036` |

**ai wave4 완료**(768/1,047). 이번 세션 최대 heavy 비중(5/8). 파이프라인 실수 1건 발견·정정 —
ay 배치의 진단 결합(shim) 단계를 누락한 채 윤문 시작, monolith가 스스로 감지해 정량 점수만으로
대체 진행(등급 A로 나왔으나 다른 패턴 커버리지 불확실). heavy 5개 전부 finalize가 의미 드리프트
발견·수정 — bd는 "재현 가능 범위를 계속 좁히는" 논지의 글에서 A-10 과잉 적용으로 유보가 단정으로
바뀐 6곳 중 하나가 글 자신이 다른 문단에서 명시적으로 금지한 일반화를 저지르는 사례였음. golden
FAIL 1건은 `(p−1)/(m+p−1)` 수식 오탐. prose-readability 2건 소폭 상승은 다수 문단에 걸친 E-1 패턴.

| 9(wave5 bg~bh) | ai(192개 파일), 8개 서브배치(standard 1·heavy 7, 전부 finalize) | 866 추출 → 632 반영 | 등급 A×8 · 게이트 golden FAIL 3건(오탐 2 + 실위반 1) 처리 후 진행 | `e95ca076` |

**ai wave5 완료**(960/1,047). 이번 세션 최대 heavy 밀도(7/8). 파이프라인 실수 재발(bh 진단 결합 누락)을
게이트 파일-없음 오류로 즉시 발견·정정. finalizer가 monolith 자체검증 보고의 부정확함을 두 차례
재검증으로 잡아냄(bn·bh 둘 다 "0건 잔존" 주장이 실제로는 미탐 잔존 있었음). golden FAIL 중 bh 1건은
false positive가 아니라 **실제 인용 위반**(C-8/C-4 재구성 중 직접 인용 2곳이 인용부호째 의역되어
사라짐) — ethereum aj 이후 두 번째 사례, 원문 그대로 복원해 게이트 재통과.

| 9(wave6 bo~br) | ai(87개 파일), 4개 서브배치(standard 3·heavy 1, 전부 finalize) | 240 반영(52+94+36+58) | 등급 A×4 · 게이트 golden FAIL 3건(전부 오탐) + P2 대구 전멸 1건(실위반) 처리 후 진행 | `fe0a5d1b` |

**ai wave6 완료(1,047/1,047) — ai 카테고리 전체 완료.** bo·bp 두 배치의 golden FAIL 3건은
좌표 표기 `(0.6, 0.8, 0)`와 vLLM 식별자 `ref(P9)`의 닫는 괄호 숫자를 각주로 오인식한 false
positive(원문·윤문본 바이트 단위 대조로 확인). bq는 진짜 과교정 — C-8 대구 6건이 게이트상
0건으로 전멸했고 monolith 자체보고("6+→2")도 실측과 어긋났다(이 세션에서 반복된 자체검증
신뢰불가 패턴의 재현). finalizer가 원문 대조로 6곳 모두 의미는 살아있음을 확인했으나 표층
골격이 동시에 평탄해진 건 사실이라, 핵심 대조 주장 3곳만 "A가 아니라 B" 원형으로 복원(전량
복원은 기계적 반복 재현이라 하지 않음). bp(heavy)는 P1도 미달(z +5.04→+2.64)이라 잔존 C-11
쉼표 20건을 추가 정리해 +0.17로 수렴. 세 배치 모두 의미보존 위반(피동 신규 삽입, 조력→필요조건
강도 변경, 부정 명제→별개 명제 치환, 양상 강화 등) 각 2~3건씩 별도 롤백. prose-readability
신규 재검토 8건은 전부 C-11 쉼표 제거·문장 분리에 따른 지문 변화로 diff 확인 후 baseline 갱신.

## 프로그램 전체 완료 (2026-09-05)

**2026-09-04~05 세션 최종 누적**: crypto·filecoin·hw·p2p·isms-aml·gpu·tee·ethereum·blockchain·ai
**10개 카테고리 전부(3,014개 파일 + Gemmini 신규 아티클 1편) 완료.** 블로그 전체
humanize-korean 프로그램 완주.

**전체 요약**: 등급 A 서브배치 100%(FAIL 없이 전부 A 도달, WARN은 매번 원인 확인 후 진행 또는
finalize 승급으로 해소). heavy 경로 finalizer가 실제 의미 드리프트를 계속 잡아냄 — hedge/양상
소실 또는 과잉 확언, 없던 단정·인과·한정 주입, 대구/대조 골격 전멸, 검증요건→관찰사실 격 전환
등. **진짜 golden(인용/각주) 위반은 프로그램 전체에서 단 2건**(ethereum/aj, ai/wave5-bh) —
나머지 전부 수학 표기·좌표·식별자·인용연도의 괄호 숫자를 각주로 오인식한 false positive였다.
monolith 자체검증 신뢰불가 패턴(자체보고 개수가 finalizer 재검증과 어긋남)이 여러 차례
재확인됐고, 파이프라인 실수(진단 결합 shim 누락) 2건도 각각 발견 즉시 정정했다.
prose-readability 소폭 상승은 전부 E-1(장문 분리로 문장 수만 늘어 압축 임계값 근접) 패턴이며
정보 손실은 매번 diff로 확인 후 baseline을 갱신했다.

브랜치는 `feat/npu-gemmini-series`에 그대로 있고 `main`에는 아직 병합·푸시하지 않았다
(사용자 명시 승인 필요).
...
## 신규 아티클 추가 회차 (2026-09-12)

프로그램 전체 완료(2026-09-05) 이후 새로 쓴 글은 ledger에 잡히지 않는다. 2026-09-11~12 세션에서
작성한 11편(saas 3편 + ai 8편, 77개 파일)이 그 사각지대였고 — `check-article.sh` 안의
`audit-prose-readability.mjs --strict`만 통과했을 뿐 humanize 파이프라인은 한 번도 돌지 않았다 —
이번에 같은 6단계로 처리했다.

| 배치 | 파일 | 문단 | 결과 | 커밋 |
|---|---|---|---|---|
| 10(aa~ak) | 신규 11편(77개 파일), 11개 서브배치(아티클 1편 = 1배치, light 11) | 448 추출 → 203 반영 | 등급 A×11 · 게이트 OK 10 + WARN 1(ag, finalize 승급 후 OK) | `9381758a` |

**배치 분할 기준 변경**: 기존 "24개 파일/서브배치" 대신 **아티클 1편 = 1서브배치**(7개 파일,
10~16KB)로 쪼갰다. 처음에 3편씩 묶었더니 41~44KB로 heavy 영역에 들어갔다 — 이 시리즈는 문단
밀도가 높아 파일 수가 아니라 바이트가 제약이다. route_hint는 11개 전부 `light`
(risk_band low · score 2 · lexical_tell_count 0).

**지배 패턴이 하나였다**: 11편 전부 ★S1 트리거가 `ending_comma_rate` 단독(z +3.19 ~ +6.33)이고
C-11(연결어미 뒤 쉼표) 외에는 손댈 근거가 거의 없었다. 어휘 티(D)·hype·과잉 hedge(G)·피동(A-8/A-9)
카운트가 전부 0. 그래서 변경률이 0.14~1.7%로 A 표준 밴드(10~25%)보다 훨씬 낮은데, 과소 윤문이
아니라 철칙 #2(근거 없는 구간 무개입) 준수 결과다. 이 시리즈는 CLAUDE.md 문체 규칙을 지키며 쓴
글이라 C-11만 남아 있었다는 뜻이기도 하다.

**ag(removal-is-not-inpainting) WARN 처리**: P1 z +5.31 → +2.57로 미달이라 finalize로 승급.
finalizer가 의미 보존 15항 전항 통과를 확인한 뒤 C-11 잔존 3건만 국소 보정해 z +1.66으로 수렴.
의미 드리프트 0건이었다 — 이번 회차는 프로그램 전체에서 finalize가 실결함을 못 찾은 드문 경우인데,
윤문 자체가 쉼표 삭제·문장 분리에 한정돼 "빈 자리에 새 단정이 들어올" 구조 편집이 없었기 때문이다.
finalizer가 monolith의 자체보고 변경률(~6%)이 문자 단위로는 ~1.2%임을 잡아낸 건 기존 회차에서
반복된 "자체검증 신뢰불가" 패턴의 재확인.

**A-18(관형절 3중 중첩)은 11편 전부 의도적 미개입**. 문장을 쪼개면 측정 조건 한정어("이 인식기에
대한 값", "한 장비의 한 회차")가 흔들릴 위험이 있어 실측 보고서에서는 정확도를 우선했다.

**사후 검증**: 변경 파일 64개에 대해 (1) 숫자 토큰 multiset 전수 대조 → 불일치 0건 (2) 본문 인용구
446개 전수 대조 → 불일치 0건 (3) 마커 여닫이 시퀀스 md5 대조 11/11 SEQ_OK (4) `tsc -b --noEmit`
· `eslint` 0건 (5) `check-article.sh` 11개 route 전부 통과 (6) `npm run build` 676 route.
prose-readability는 편집 전후가 **완전 동일**(압축 신호 315개 · 재검토 0개)이라 baseline 갱신
불필요 — 이번엔 E-1 문장 분리가 적어 기존 회차마다 나오던 소폭 상승이 없었다.
...
## 금융 카테고리 신설 회차 (2026-09-14)

CS 대분류를 세우면서 함께 만든 금융 카테고리 9편(money 2 · banking 3 · markets 2 · risk 2)을
같은 6단계로 처리했다. 이번 글들은 조립 파일 1개에 본문이 전부 들어 있는 형태라 아티클 1편 =
파일 1개다.

| 배치 | 파일 | 문단 | 결과 | 커밋 |
|---|---|---|---|---|
| 11(fin-1~5) | 금융 신규 9편(9개 파일), 5개 서브배치(아티클 2편 = 1배치, light 5) | 187 추출 → 120 반영 | 등급 A×5 · 게이트 OK 5 | `d058d867` |

**배치 분할은 이번에도 바이트 기준**. 9편 전체를 한 번에 추출하니 64KB(187문단)로 heavy 영역에
들어가, 아티클 2편 = 1서브배치(16KB)로 쪼갰다. 배치 10의 "1편 = 1배치"와 숫자가 다른 이유는
이 시리즈가 섹션 서브컴포넌트 없이 한 파일에 몰려 있어 파일당 바이트가 크기 때문이다.
route_hint는 5개 전부 `light`(risk_band low · score 2).

**배치 10과 똑같이 지배 패턴이 C-11 하나였다**. 다섯 배치 전부 ★S1 트리거가 `ending_comma_rate`
단독(z +4.46 ~ +5.71). 두 회차 연속 같은 결과가 나왔다는 것은, CLAUDE.md 문체 규칙을 지키며 쓰면
어휘 티·피동·hype는 애초에 안 생기고 연결어미 뒤 쉼표 습관만 남는다는 뜻으로 읽힌다.
그 외 잡힌 것은 형식명사 결말(I-2/I-3), 관형절 3중 중첩(A-18), 불필요 피동(A-9), `-들`(A-17),
문두 접속사 반복(H-1).

변경률 0.6~1.8%로 A 표준 밴드보다 낮은 것도 같은 이유다(철칙 #2 준수).

**이번 회차에서 새로 확인한 것**: 금융 글은 수치 밀도가 높아(계산 예시가 문단마다 있음) golden
게이트의 수치 주입 검사가 특히 중요했는데, 다섯 배치 전부 P3 PASS였다. 서브에이전트 프롬프트에
보존해야 할 수치를 구체적으로 나열한 것(1.05³ 결과, 할인계수 0.744/0.463, ROE 11%/1%,
자본비율 16%/8% 등)이 효과가 있었던 것으로 보인다 — 다섯 배치 모두 자체보고에서 숫자 토큰
멀티셋 대조를 수행했다고 보고했다.

**A-18은 배치 3에서만 개입**(9 → 4). 채권 글의 관형절 중첩은 측정 조건 한정어가 아니라 단순
수식 중첩이라 분리해도 정확도가 흔들리지 않았다. 배치 2는 같은 A-18 지적을 받았지만 해당
관형절이 글의 핵심 대구("옮겨 주는 중개자 / 적어 넣는 발행자")여서 의도적으로 남겼다.

## 정치 카테고리 신설 회차 (2026-09-14)

금융에 이어 정치 대분류 9편(polity 2 · constitution 2 · elections 3 · governance 2)을 같은 6단계로
처리했다. 금융과 마찬가지로 조립 파일 1개에 본문이 전부 들어 있어 아티클 1편 = 파일 1개다.

| 배치 | 파일 | 문단 | 결과 | 커밋 |
|---|---|---|---|---|
| 12(pol-1~5) | 정치 신규 9편(9개 파일), 5개 서브배치(아티클 2편 = 1배치, 마지막만 1편) | 254 추출 → 120 반영 | 등급 A×5 · 게이트 OK 5 | (이 커밋) |

**세 회차 연속 지배 패턴이 C-11 하나였다.** 다섯 배치 전부 ★S1 트리거가 `ending_comma_rate` 단독
(z +4.13 ~ +5.51). 배치 10·11에 이어 세 번째다. P1 달성 여부는 pol-1(+5.03 → +0.40)과
pol-4(+5.31 → +0.54)가 "달성", 나머지 셋은 "부분 개선"(pol-2 +5.51 → +1.63, pol-3 +4.84 → +1.18,
pol-5 +4.13 → +1.96)인데 전부 `gate: OK — 수렴`이다. 부분 개선으로 끝난 셋은 서브에이전트가
**나열 쉼표를 의도적으로 남긴 경우**다. pol-3은 Arrow 5조건 열거와 3항 병렬 결론, pol-4는 두 글의
결론 대구("맡기면 보이지 않고 ~ 못하며 ~ 다르고 ~ 치릅니다"), pol-5는 시리즈 아홉 글을 되짚는
대구 문단이다. 전량 제거하면 나열의 파싱이 무너지거나 오히려 기계적 서명이 된다는 판단이었고,
육안 확인 결과 타당했다.

변경률 0.33~0.63%로 배치 11(0.6~1.8%)보다 더 낮다. 정치 글은 금융보다 산문 비중이 높은데도
낮게 나온 이유는 추출 단계에서 수식·인용이 섞인 문단이 빠지면서 **손댈 근거가 있는 구간만 남았기
때문**이다.

**이번 회차에서 새로 확인한 것 — 숫자 보존 지시가 공회전했다.** 다섯 배치 프롬프트에 보존해야 할
수치를 구체적으로 나열했는데(배치 11에서 효과가 있었다고 기록한 방식), 다섯 중 셋(pol-1·pol-2·
pol-4)에서 서브에이전트가 "지목된 토큰이 입력에 아예 없다"고 보고했다. 그 수치들이 전부
`ExplainedFormula`·`CitationBlock`·`AlgorithmBlock` prop 안에 있어 `<p>` 추출 대상이 아니기
때문이다. 즉 배치 11의 "수치 주입 검사가 특히 중요했다"는 기록은 과대평가였을 가능성이 있다 —
금융 글도 같은 구조라 계산 예시의 상당수가 추출되지 않았을 것이다. **다음 회차부터는 프롬프트에
수치를 나열하기 전에 `grep`으로 그 토큰이 `01_input.txt`에 실제로 있는지 먼저 확인할 것.** 본문
산문에 실제로 들어 있던 수치(pol-3의 44·41·15, 5042, 50.2, 396·369·135, 69대31 등 303토큰)는
멀티셋 대조로 전후 일치를 확인했다.

**A-18은 다섯 배치 전부 무개입**(총 60건). 정치 글의 관형절 중첩은 대부분 조건 한정어를 업고 있어
분리하면 조건의 순서·범위가 어긋난다. H-1도 전부 무개입 — 빈출 문두어가 `그래서`·`그런데`인데
H-1 lexicon(또한/따라서/즉/나아가)에 없고, 문단 간 인과 사슬의 척추라 지우면 전개가 끊긴다.

**사후 검증**: `tsc -b --noEmit` · `eslint` 0건, `audit-prose-readability --strict` 통과
(baseline 갱신 불필요 — 압축 신호 315개 · 재검토 0개로 편집 전후 동일), 전역 게이트 8종 + 빌드 통과.
